...
/**
 * Defines the Moodle forum used to add random questions to the flipquiz.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class flipquiz_add_random_form extends moodleform {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'existingcategoryheader',
                get_string('randomfromexistingcategory', 'flipquiz'));
        ...
        $mform->addElement('checkbox', 'includesubcategories', '', get_string('recurse', 'flipquiz'));
        ...
        $options = array(
            'multiple' => true,
            'noselectionstring' => get_string('anytags', 'flipquiz'),
        );
        $mform->addElement('autocomplete', 'fromtags', get_string('randomquestiontags', 'mod_flipquiz'), $tagstrings, $options);
        $mform->addHelpButton('fromtags', 'randomquestiontags', 'mod_flipquiz');

        $mform->addElement('select', 'numbertoadd', get_string('randomnumber', 'flipquiz'),
                $this->get_number_of_questions_to_add_choices());

        $previewhtml = $OUTPUT->render_from_template('mod_flipquiz/random_question_form_preview', []);
        $mform->addElement('html', $previewhtml);

        $mform->addElement('submit', 'existingcategory', get_string('addrandomquestion', 'flipquiz'));

        // Random from a new category section.
        $mform->addElement('header', 'newcategoryheader',
                get_string('randomquestionusinganewcategory', 'flipquiz'));
        ...
        $mform->addElement('submit', 'newcategory',
                get_string('createcategoryandaddrandomquestion', 'flipquiz'));
        ...
        $PAGE->requires->js_call_amd('mod_flipquiz/add_random_form', 'init', [
