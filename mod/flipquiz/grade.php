...
$cm = get_coursemodule_from_id('flipquiz', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$flipquiz = $DB->get_record('flipquiz', array('id' => $cm->instance), '*', MUST_EXIST);
require_login($course, false, $cm);

$reportlist = flipquiz_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see flipquiz reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/flipquiz/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = flipquiz_get_user_attempts($flipquiz->id, $userid, 'finished');
    $attempt = null;
    ...
    if ($attempt) {
        $attemptobj = new flipquiz_attempt($attempt, $flipquiz, $cm, $course, false);
...
redirect(new moodle_url('/mod/flipquiz/report.php', array(
