...
/**
 * Defines the renderer for the flipquiz module.
 *
...
/**
 * The renderer for the flipquiz module.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_flipquiz_renderer extends plugin_renderer_base {
    /**
     * Builds the review page
     *
     * @param flipquiz_attempt $attemptobj an instance of flipquiz_attempt.
     ...
     * @param mod_flipquiz_display_options $displayoptions instance of mod_flipquiz_display_options.
     * @param array $summarydata contains all table data
     * @return $output containing html data.
     */
    public function review_page(flipquiz_attempt $attemptobj, $slots, $page, $showall,
                                $lastpage, mod_flipquiz_display_options $displayoptions,
                                $summarydata) {
     ...
     * @return $output containing html data.
     */
    public function review_question_page(flipquiz_attempt $attemptobj, $slot, $seq,
            mod_flipquiz_display_options $displayoptions, $summarydata) {
     ...
     */
    public function review_question_not_allowed(flipquiz_attempt $attemptobj, $message) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_flipquiz_name(), true,
                                  array("context" => $attemptobj->get_flipquizobj()->get_context())));
    ...
    public function review_summary_table($summarydata, $page) {
        ...
        $output .= html_writer::start_tag('table', array(
                'class' => 'generaltable generalbox flipquizreviewsummary'));
     ...
     *
     * @param flipquiz_attempt $attemptobj instance of flipquiz_attempt
     ...
     * @param mod_flipquiz_display_options $displayoptions instance of mod_flipquiz_display_options
     */
    public function questions(flipquiz_attempt $attemptobj, $reviewing, $slots, $page, $showall,
                              mod_flipquiz_display_options $displayoptions) {
        $output = '';
    ...
    public function review_form($page, $showall, $displayoptions, $content, $attemptobj) {
        ...
        $this->page->requires->js_init_call('M.mod_flipquiz.init_review_form', null, false,
                flipquiz_get_js_module());
     ...
     * @param flipquiz_attempt $attemptobj instance of flipquiz_attempt
     */
    public function finish_review_link(flipquiz_attempt $attemptobj) {
        $url = $attemptobj->view_url();

        if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            $this->page->requires->js_init_call('M.mod_flipquiz.secure_window.init_close_button',
                    array($url), false, flipquiz_get_js_module());
            return html_writer::empty_tag('input', array('type' => 'button',
                    'value' => get_string('finishreview', 'flipquiz'),
                    'id' => 'secureclosebutton',
                    'class' => 'mod_flipquiz-next-nav btn btn-primary'));

        } else {
            return html_writer::link($url, get_string('finishreview', 'flipquiz'),
                    array('class' => 'mod_flipquiz-next-nav'));
        }
    }
     ...
     */
    public function review_next_navigation(flipquiz_attempt $attemptobj, $page, $lastpage, $showall = null) {
        $nav = '';
        if ($page > 0) {
            $nav .= link_arrow_left(get_string('navigateprevious', 'flipquiz'),
                    $attemptobj->review_url(null, $page - 1, $showall), false, 'mod_flipquiz-prev-nav');
        }
        if ($lastpage) {
            $nav .= $this->finish_review_link($attemptobj);
        } else {
            $nav .= link_arrow_right(get_string('navigatenext', 'flipquiz'),
                    $attemptobj->review_url(null, $page + 1, $showall), false, 'mod_flipquiz-next-nav');
        }
        return html_writer::tag('div', $nav, array('class' => 'submitbtns'));
    }

    /**
     * Return the HTML of the flipquiz timer.
     * @return string HTML content.
     */
    public function countdown_timer(flipquiz_attempt $attemptobj, $timenow) {
        ...
        if ($timeleft !== false) {
            ...
            if (!$ispreview) {
                ...
                // this will just have the effect of causing the flipquiz to be submitted immediately.
                $timerstartvalue = max($timerstartvalue, 1);
            }
            $this->initialise_timer($timerstartvalue, $ispreview);
        }

        return html_writer::tag('div', get_string('timeleft', 'flipquiz') . ' ' .
                html_writer::tag('span', '', array('id' => 'flipquiz-time-left')),
                array('id' => 'flipquiz-timer', 'role' => 'timer',
    ...
    public function restart_preview_button($url) {
        return $this->single_button($url, get_string('startnewpreview', 'flipquiz'));
    }

    /**
     * Outputs the navigation block panel
     *
     * @param flipquiz_nav_panel_base $panel instance of flipquiz_nav_panel_base
     */
    public function navigation_panel(flipquiz_nav_panel_base $panel) {
        ...
        $this->page->requires->js_init_call('M.mod_flipquiz.nav.init', null, false,
                flipquiz_get_js_module());
    ...
    /**
     * Display a flipquiz navigation button.
     *
     * @param flipquiz_nav_question_button $button
     * @return string HTML fragment.
     */
    protected function render_flipquiz_nav_question_button(flipquiz_nav_question_button $button) {
        $classes = array('qnbutton', $button->stateclass, $button->navmethod, 'btn', 'btn-secondary');
        $extrainfo = array();

        if ($button->currentpage) {
            $classes[] = 'thispage';
            $extrainfo[] = get_string('onthispage', 'flipquiz');
        }
        ...
        $tagcontents = html_writer::tag('span', '', array('class' => 'thispageholder')) .
                        ...
                        get_string($qnostring, 'flipquiz', $a);
        $tagattributes = array('class' => implode(' ', $classes), 'id' => $button->id,
                                  'title' => $button->statestring, 'data-flipquiz-page' => $button->page);
    ...
    /**
     * Display a flipquiz navigation heading.
     *
     * @param flipquiz_nav_section_heading $heading the heading.
     * @return string HTML fragment.
     */
    protected function render_flipquiz_nav_section_heading(flipquiz_nav_section_heading $heading) {
        return $this->heading($heading->heading, 3, 'mod_flipquiz-section-heading');
    }

    /**
     * outputs the link the other attempts.
     *
     * @param mod_flipquiz_links_to_other_attempts $links
     */
    protected function render_mod_flipquiz_links_to_other_attempts(
            mod_flipquiz_links_to_other_attempts $links) {
        $attemptlinks = array();
    ...
    public function start_attempt_page(flipquiz $flipquizobj, mod_flipquiz_preflight_check_form $mform) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($flipquizobj->get_flipquiz_name(), true,
                                  array("context" => $flipquizobj->get_context())));
        $output .= $this->flipquiz_intro($flipquizobj->get_flipquiz(), $flipquizobj->get_cm());
        $output .= $mform->render();
        $output .= $this->footer();
        return $output;
    }
     ...
     * @param flipquiz_attempt $attemptobj Instance of flipquiz_attempt
     * @param int $page Current page number
     * @param flipquiz_access_manager $accessmanager Instance of flipquiz_access_manager
    ...
    public function attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id,
        ...
        $output = '';
        $output .= $this->header();
        $output .= $this->flipquiz_notices($messages);
     ...
     */
    public function flipquiz_notices($messages) {
        if (!$messages) {
            return '';
        }
        return $this->box($this->heading(get_string('accessnoticesheader', 'flipquiz'), 3) .
                $this->access_messages($messages), 'flipquizaccessnotices');
    }

    /**
     * Ouputs the form for making an attempt
     *
     * @param flipquiz_attempt $attemptobj
     * @param int $page Current page number
    ...
    public function attempt_form($attemptobj, $page, $slots, $id, $nextpage) {
        ...
        $navmethod = $attemptobj->get_flipquiz()->navmethod;
     ...
     * @param bool $lastpage is this the last page in the flipquiz?
     * @param string $navmethod Optional flipquiz attribute, 'free' (default) or 'sequential'
     * @return string HTML fragment.
     */
    protected function attempt_navigation_buttons($page, $lastpage, $navmethod = 'free') {
        ...
        if ($page > 0 && $navmethod == 'free') {
            $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'previous',
                    'value' => get_string('navigateprevious', 'flipquiz'), 'class' => 'mod_flipquiz-prev-nav btn btn-secondary'));
        }
        if ($lastpage) {
            $nextlabel = get_string('endtest', 'flipquiz');
        } else {
            $nextlabel = get_string('navigatenext', 'flipquiz');
        }
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'next',
                'value' => $nextlabel, 'class' => 'mod_flipquiz-next-nav btn btn-primary'));
        $output .= html_writer::end_tag('div');
    ...
    public function redo_question_button($slot, $disabled) {
        $attributes = array('type' => 'submit',  'name' => 'redoslot' . $slot,
                'value' => get_string('redoquestion', 'flipquiz'), 'class' => 'mod_flipquiz-redo_question_button');
        if ($disabled) {
    ...
    public function initialise_timer($timerstartvalue, $ispreview) {
        ...
        $this->page->requires->js_init_call('M.mod_flipquiz.timer.init', $options, false, flipquiz_get_js_module());
    }
    ...
    public function close_attempt_popup($url, $message = '') {
        ...
        if ($message) {
            ...
            $output .= html_writer::tag('p', get_string('windowclosing', 'flipquiz'));
            $delay = 5;
        } else {
            $output .= html_writer::tag('p', get_string('pleaseclose', 'flipquiz'));
            $delay = 0;
        }
        $this->page->requires->js_init_call('M.mod_flipquiz.secure_window.close',
                array($url, $delay), false, flipquiz_get_js_module());
     ...
     *
     * @param flipquiz_attempt $attemptobj
     * @param mod_flipquiz_display_options $displayoptions
     */
    public function summary_page($attemptobj, $displayoptions) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_flipquiz_name()));
        $output .= $this->heading(get_string('summaryofattempt', 'flipquiz'), 3);
    ...
    public function summary_table($attemptobj, $displayoptions) {
        ...
        $table->attributes['class'] = 'generaltable flipquizsummaryofattempt boxaligncenter';
        $table->head = array(get_string('question', 'flipquiz'), get_string('status', 'flipquiz'));
        $table->align = array('left', 'left');
        $table->size = array('', '');
        $markscolumn = $displayoptions->marks >= question_display_options::MARK_AND_MAX;
        if ($markscolumn) {
            $table->head[] = get_string('marks', 'flipquiz');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($slots as $slot) {
            ...
            if ($heading) {
                ...
                $table->rowclasses[] = 'flipquizsummaryheading';
            }
            ...
            $table->data[] = $row;
            $table->rowclasses[] = 'flipquizsummary' . $slot . ' ' . $attemptobj->get_question_state_class(
    ...
    public function summary_page_controls($attemptobj) {
        ...
        if ($attemptobj->get_state() == flipquiz_attempt::IN_PROGRESS) {
            $button = new single_button(
                    new moodle_url($attemptobj->attempt_url(null, $attemptobj->get_currentpage())),
                    get_string('returnattempt', 'flipquiz'));
        ...
        $button = new single_button(
                ...
                get_string('submitallandfinish', 'flipquiz'));
        $button->id = 'responseform';
        if ($attemptobj->get_state() == flipquiz_attempt::IN_PROGRESS) {
            $button->add_action(new confirm_action(get_string('confirmclose', 'flipquiz'), null,
                    get_string('submitallandfinish', 'flipquiz')));
        }

        $duedate = $attemptobj->get_due_date();
        $message = '';
        if ($attemptobj->get_state() == flipquiz_attempt::OVERDUE) {
            $message = get_string('overduemustbesubmittedby', 'flipquiz', userdate($duedate));

        } else if ($duedate) {
            $message = get_string('mustbesubmittedby', 'flipquiz', userdate($duedate));
        }
     ...
     * @param array $flipquiz Array conting flipquiz data
     * @param int $cm Course Module ID
     * @param int $context The page context ID
     * @param array $infomessages information about this flipquiz
     * @param mod_flipquiz_view_object $viewobj
     ...
     *      attempt this flipquiz now, if appicable this flipquiz
     */
    public function view_page($course, $flipquiz, $cm, $context, $viewobj) {
        $output = '';
        $output .= $this->view_information($flipquiz, $cm, $context, $viewobj->infomessages);
        $output .= $this->view_table($flipquiz, $context, $viewobj);
        $output .= $this->view_result_info($flipquiz, $context, $cm, $viewobj);
        $output .= $this->box($this->view_page_buttons($viewobj), 'flipquizattempt');
        return $output;
    }
     ...
     * @param mod_flipquiz_view_object $viewobj the information required to display
     * the view page.
     * @return string HTML to output.
     */
    public function view_page_buttons(mod_flipquiz_view_object $viewobj) {
        global $CFG;
        $output = '';

        if (!$viewobj->flipquizhasquestions) {
        ...
        if ($viewobj->showbacktocourse) {
            $output .= $this->single_button($viewobj->backtocourseurl,
                    get_string('backtocourse', 'flipquiz'), 'get',
     ...
     * @param mod_flipquiz_preflight_check_form $preflightcheckform deprecated.
    ...
    public function start_attempt_button($buttontext, moodle_url $url,
            mod_flipquiz_preflight_check_form $preflightcheckform = null,
        ...
        if (is_string($preflightcheckform)) {
            ...
            debugging('The third argument to start_attempt_button should now be the ' .
                    'mod_flipquiz_preflight_check_form from ' .
                    'flipquiz_access_manager::get_preflight_check_form, not a warning message string.');
        }

        $button = new single_button($url, $buttontext);
        $button->class .= ' flipquizstartbuttondiv';
        if ($popuprequired) {
            $button->class .= ' flipquizsecuremoderequired';
        }
        ...
        $this->page->requires->js_call_amd('mod_flipquiz/preflightcheck', 'init',
                array('.flipquizstartbuttondiv [type=submit]', get_string('startattempt', 'flipquiz'),
                       '#mod_flipquiz_preflight_form', $popupjsoptions));
    ...
    public function no_questions_message($canedit, $editurl) {
        $output = '';
        $output .= $this->notification(get_string('noquestions', 'flipquiz'));
        if ($canedit) {
            $output .= $this->single_button($editurl, get_string('editflipquiz', 'flipquiz'), 'get');
        }
    ...
    /**
     * Outputs an error message for any guests accessing the flipquiz
     *
     * @param int $course The course ID
     * @param array $flipquiz Array contingin flipquiz data
     * @param int $cm Course Module ID
     * @param int $context The page contect ID
     * @param array $messages Array containing any messages
     */
    public function view_page_guest($course, $flipquiz, $cm, $context, $messages) {
        $output = '';
        $output .= $this->view_information($flipquiz, $cm, $context, $messages);
        $guestno = html_writer::tag('p', get_string('guestsno', 'flipquiz'));
     ...
     * @param array $messages Array containing any messages
     */
    public function view_page_notenrolled($course, $flipquiz, $cm, $context, $messages) {
        global $CFG;
        $output = '';
        $output .= $this->view_information($flipquiz, $cm, $context, $messages);
        $youneedtoenrol = html_writer::tag('p', get_string('youneedtoenrol', 'flipquiz'));
     ...
     * @return string HTML to output.
     */
    public function view_information($flipquiz, $cm, $context, $messages) {
        global $CFG;

        $output = '';

        // Print flipquiz name and description.
        $output .= $this->heading(format_string($flipquiz->name));
        $output .= $this->flipquiz_intro($flipquiz, $cm);

        // Output any access messages.
        if ($messages) {
            $output .= $this->box($this->access_messages($messages), 'flipquizinfo');
        }

        // Show number of attempts summary to those who can view reports.
        if (has_capability('mod/flipquiz:viewreports', $context)) {
            if ($strattemptnum = $this->flipquiz_attempt_summary_link_to_reports($flipquiz, $cm,
                    $context)) {
                $output .= html_writer::tag('div', $strattemptnum,
                        array('class' => 'flipquizattemptcounts'));
            }
        }
        return $output;
    }
    ...
    /**
     * Output the flipquiz intro.
     * @param object $flipquiz the flipquiz settings.
     * @param object $cm the course_module object.
     * @return string HTML to output.
     */
    public function flipquiz_intro($flipquiz, $cm) {
        if (html_is_blank($flipquiz->intro)) {
            return '';
        }

        return $this->box(format_module_intro('flipquiz', $flipquiz, $cm->id), 'generalbox', 'intro');
    }
    ...
    public function view_table_heading() {
        return $this->heading(get_string('summaryofattempts', 'flipquiz'), 3);
    }

    /**
     * Generates the table of data
     *
     * @param array $flipquiz Array contining flipquiz data
     * @param int $context The page context ID
     * @param mod_flipquiz_view_object $viewobj
     */
    public function view_table($flipquiz, $context, $viewobj) {
        if (!$viewobj->attempts) {
            return '';
        }
        ...
        $table = new html_table();
        $table->attributes['class'] = 'generaltable flipquizattemptsummary';
        $table->head = array();
        $table->align = array();
        $table->size = array();
        if ($viewobj->attemptcolumn) {
            $table->head[] = get_string('attemptnumber', 'flipquiz');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        $table->head[] = get_string('attemptstate', 'flipquiz');
        $table->align[] = 'left';
        $table->size[] = '';
        if ($viewobj->markcolumn) {
            $table->head[] = get_string('marks', 'flipquiz') . ' / ' .
                    flipquiz_format_grade($flipquiz, $flipquiz->sumgrades);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->gradecolumn) {
            $table->head[] = get_string('grade') . ' / ' .
                    flipquiz_format_grade($flipquiz, $flipquiz->grade);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->canreviewmine) {
            $table->head[] = get_string('review', 'flipquiz');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->feedbackcolumn) {
            $table->head[] = get_string('feedback', 'flipquiz');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($viewobj->attemptobjs as $attemptobj) {
            ...
            if ($viewobj->attemptcolumn) {
                if ($attemptobj->is_preview()) {
                    $row[] = get_string('preview', 'flipquiz');
                } else {
            ...
            if ($viewobj->markcolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    $row[] = flipquiz_format_grade($flipquiz, $attemptobj->get_sum_marks());
                } else {
                    $row[] = '';
                }
            }
            ...
            $attemptgrade = flipquiz_rescale_grade($attemptobj->get_sum_marks(), $flipquiz, false);
            ...
            if ($viewobj->gradecolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    if ($viewobj->overallstats && !$attemptobj->is_preview()
                            ...
                            && $attemptobj->get_state() == flipquiz_attempt::FINISHED
                            && $attemptgrade == $viewobj->mygrade
                            && $flipquiz->grademethod == FLIPQUIZ_GRADEHIGHEST) {
                        $table->rowclasses[$attemptobj->get_attempt_number()] = 'bestrow';
                    }

                    $row[] = flipquiz_format_grade($flipquiz, $attemptgrade);
                } else {
                    $row[] = '';
                }
            }
            ...
            if ($viewobj->feedbackcolumn && $attemptobj->is_finished()) {
                if ($attemptoptions->overallfeedback) {
                    $row[] = flipquiz_feedback_for_grade($attemptgrade, $flipquiz, $context);
                } else {
                    $row[] = '';
                }
            }
     ...
     * @param flipquiz_attempt $attemptobj the attempt
    ...
    public function attempt_state($attemptobj) {
        switch ($attemptobj->get_state()) {
            ...
            case flipquiz_attempt::OVERDUE:
                return get_string('stateoverdue', 'flipquiz') . html_writer::tag('span',
                        get_string('stateoverduedetails', 'flipquiz',
                                userdate($attemptobj->get_due_date())),
                        array('class' => 'statedetails'));

            case flipquiz_attempt::FINISHED:
                return get_string('statefinished', 'flipquiz') . html_writer::tag('span',
                        get_string('statefinisheddetails', 'flipquiz',
    ...
    /**
     * Generates data pertaining to flipquiz results
     *
     * @param array $flipquiz Array containing flipquiz data
     * @param int $context The page context ID
     * @param int $cm The Course Module Id
     * @param mod_flipquiz_view_object $viewobj
     */
    public function view_result_info($flipquiz, $context, $cm, $viewobj) {
        $output = '';
        ...
        if ($viewobj->overallstats) {
            if ($viewobj->moreattempts) {
                $a = new stdClass();
                $a->method = flipquiz_get_grading_option_name($flipquiz->grademethod);
                $a->mygrade = flipquiz_format_grade($flipquiz, $viewobj->mygrade);
                $a->flipquizgrade = flipquiz_format_grade($flipquiz, $flipquiz->grade);
                $resultinfo .= $this->heading(get_string('gradesofar', 'flipquiz', $a), 3);
            } else {
                $a = new stdClass();
                $a->grade = flipquiz_format_grade($flipquiz, $viewobj->mygrade);
                $a->maxgrade = flipquiz_format_grade($flipquiz, $flipquiz->grade);
                $a = get_string('outofshort', 'flipquiz', $a);
                $resultinfo .= $this->heading(get_string('yourfinalgradeis', 'flipquiz', $a), 3);
            }
        }
        ...
        if ($viewobj->gradebookfeedback) {
            $resultinfo .= $this->heading(get_string('comment', 'flipquiz'), 3);
            $resultinfo .= html_writer::div($viewobj->gradebookfeedback, 'flipquizteacherfeedback') . "\n";
        }
        if ($viewobj->feedbackcolumn) {
            $resultinfo .= $this->heading(get_string('overallfeedback', 'flipquiz'), 3);
            $resultinfo .= html_writer::div(
                    flipquiz_feedback_for_grade($viewobj->mygrade, $flipquiz, $context),
                    'flipquizgradefeedback') . "\n";
        }
    ...
    public function review_link($url, $reviewinpopup, $popupoptions) {
        if ($reviewinpopup) {
            $button = new single_button($url, get_string('review', 'flipquiz'));
            $button->add_action(new popup_action('click', $url, 'flipquizpopup', $popupoptions));
            return $this->render($button);

        } else {
            return html_writer::link($url, get_string('review', 'flipquiz'),
                    array('title' => get_string('reviewthisattempt', 'flipquiz')));
        }
    }
     ...
     */
    public function flipquiz_attempt_summary_link_to_reports($flipquiz, $cm, $context,
    ...
    public function connection_warning() {
        ...
        $warning = format_text(get_string('connectionerror', 'flipquiz'), FORMAT_MARKDOWN, $options);
        $ok = format_text(get_string('connectionok', 'flipquiz'), FORMAT_MARKDOWN, $options);
...
class mod_flipquiz_links_to_other_attempts implements renderable {
    /**
...
class mod_flipquiz_view_object {
    /** @var array $infomessages of messages with information to display about the flipquiz. */
    public $infomessages;
    /** @var array $attempts contains all the user's attempts at this flipquiz. */
    public $attempts;
    /** @var array $attemptobjs flipquiz_attempt objects corresponding to $attempts. */
    public $attemptobjs;
    /** @var flipquiz_access_manager $accessmanager contains various access rules. */
    public $accessmanager;
    /** @var bool $canreviewmine whether the current user has the capability to
     *       review their own attempts. */
    public $canreviewmine;
    /** @var bool $canedit whether the current user has the capability to edit the flipquiz. */
    public $canedit;
    /** @var moodle_url $editurl the URL for editing this flipquiz. */
    public $editurl;
    ...
    public $numattempts;
    /** @var float $mygrade contains the user's final grade for a flipquiz. */
    public $mygrade;
    ...
    public $popupoptions;
    /** @var bool $flipquizhasquestions whether the flipquiz has any questions. */
    public $flipquizhasquestions;
