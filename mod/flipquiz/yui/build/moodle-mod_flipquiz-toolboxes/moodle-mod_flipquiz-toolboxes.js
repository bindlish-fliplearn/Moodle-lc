YUI.add('moodle-mod_flipquiz-toolboxes', function (Y, NAME) {
...
var CSS = {
    ...
    SELECTOR = {
        ...
        SHOW: 'a.' + CSS.SHOW,
        SLOTLI: 'li.slot',
        SUMMARKS: '.mod_flipquiz_summarks'
    },
...
Y.extend(TOOLBOX, Y.Base, {
    ...
    send_request: function(data, statusspinner, success_callback, optionalconfig) {
        ...
        data.flipquizid = this.get('flipquizid');
                ...
                success: function(tid, response) {
                    ...
                    if (responsetext.hasOwnProperty('newnumquestions')) {
                        Y.one(SELECTOR.NUMQUESTIONS).setHTML(
                                M.util.get_string('numquestionsx', 'flipquiz', responsetext.newnumquestions)
                            );
                    }
                    if (success_callback) {
 ...
 *
 * This class is responsible for managing AJAX interactions with activities and resources
 * when viewing a flipquiz in editing mode.
 *
 * @module mod_flipquiz-resource-toolbox
 * @namespace M.mod_flipquiz.resource_toolbox
 */
...
Y.extend(RESOURCETOOLBOX, TOOLBOX, {
    ...
    delete_with_confirmation: function(ev, button, activity) {
        ...
        confirmstring = M.util.get_string('confirmremovequestion', 'flipquiz', qtypename);
        ...
        confirm.on('complete-yes', function() {
            ...
            var data = {
                'class': 'resource',
                'action': 'DELETE',
                'id': Y.Moodle.mod_flipquiz.util.slot.getId(element)
            };
            this.send_request(data, spinner, function(response) {
                if (response.deleted) {
                    // Actually remove the element.
                    Y.Moodle.mod_flipquiz.util.slot.remove(element);
    ...
    delete_multiple_with_confirmation: function(ev) {
        ...
        Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
            var slot = Y.Moodle.mod_flipquiz.util.slot.getSlotFromComponent(node);
            ids += ids === '' ? '' : ',';
            ids += Y.Moodle.mod_flipquiz.util.slot.getId(slot);
            slots.push(slot);
        });
        var element = Y.one('div.mod-flipquiz-edit-content');
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('areyousureremoveselected', 'flipquiz'),
            modal: true
        });
        ...
        confirm.on('complete-yes', function() {
            ...
            this.send_request(data, spinner, function(response) {
                ...
                if (response.deleted) {
                    // Actually remove the element.
                    Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
                        Y.Moodle.mod_flipquiz.util.slot.remove(node.ancestor('li.activity'));
                    });
    ...
    edit_maxmark: function(ev, button, activity) {
        ...
        var instancemaxmark = activity.one(SELECTOR.INSTANCEMAXMARK),
            ...
            data = {
                'class': 'resource',
                'field': 'getmaxmark',
                'id': Y.Moodle.mod_flipquiz.util.slot.getId(activity)
            };
    ...
    update_page_break: function(ev, button, activity, action) {
        ...
        var data = {
            ...
            'id':    Y.Moodle.mod_flipquiz.util.slot.getId(nextactivity),
            'value': value
        };
        ...
        this.send_request(data, spinner, function(response) {
            if (response.slots) {
                if (action === 'addpagebreak') {
                    Y.Moodle.mod_flipquiz.util.page.add(activity);
                } else {
                    var page = activity.next(Y.Moodle.mod_flipquiz.util.page.SELECTORS.PAGE);
                    Y.Moodle.mod_flipquiz.util.page.remove(page, true);
                }
                this.reorganise_edit_page();
            }
        });
    ...
    update_dependency: function(ev, button, activity, action) {
        ...
        this.send_request(data, spinner, function(response) {
            if (response.hasOwnProperty('requireprevious')) {
                Y.Moodle.mod_flipquiz.util.slot.updateDependencyIcon(activity, response.requireprevious);
            }
        });
    ...
    reorganise_edit_page: function() {
        Y.Moodle.mod_flipquiz.util.slot.reorderSlots();
        Y.Moodle.mod_flipquiz.util.slot.reorderPageBreaks();
        Y.Moodle.mod_flipquiz.util.page.reorderPages();
        Y.Moodle.mod_flipquiz.util.slot.updateOneSlotSections();
        Y.Moodle.mod_flipquiz.util.slot.updateAllDependencyIcons();
    },

    NAME: 'mod_flipquiz-resource-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_flipquiz.resource_toolbox = null;
M.mod_flipquiz.init_resource_toolbox = function(config) {
    M.mod_flipquiz.resource_toolbox = new RESOURCETOOLBOX(config);
    return M.mod_flipquiz.resource_toolbox;
};
/* global TOOLBOX, BODY, SELECTOR */
 ...
 *
 * @module moodle-mod_flipquiz-toolboxes
 * @namespace M.mod_flipquiz.toolboxes
 */
 ...
 *
 * This class is responsible for managing AJAX interactions with sections
 * when adding, editing, removing section headings when editing a flipquiz.
 *
 * @class section
 * @constructor
 * @extends M.mod_flipquiz.toolboxes.toolbox
 */
...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
    ...
    delete_section_with_confirmation: function(ev, button, activity) {
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('confirmremovesectionheading', 'flipquiz', activity.get('aria-label')),
            modal: true
        });
            ...
            this.send_request(data, spinner, function(response) {
                if (response) {
                    ...
                    activity.one(SELECTOR.EDITSECTIONICON).set('title',
                            M.util.get_string('sectionheadingedit', 'flipquiz', response.instancesection));
                    activity.one(SELECTOR.EDITSECTIONICON).set('alt',
                            M.util.get_string('sectionheadingedit', 'flipquiz', response.instancesection));
                    var deleteicon = activity.one(SELECTOR.DELETESECTIONICON);
                    if (deleteicon) {
                        deleteicon.set('title', M.util.get_string('sectionheadingremove', 'flipquiz', response.instancesection));
                        deleteicon.set('alt', M.util.get_string('sectionheadingremove', 'flipquiz', response.instancesection));
                    }
                }
            });
        }
    ...
    NAME: 'mod_flipquiz-section-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_flipquiz.init_section_toolbox = function(config) {
