...
/**
 * Unit tests for the flipquizaccess_ipaddress plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/flipquiz/accessrule/ipaddress/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class flipquizaccess_ipaddress_testcase extends basic_testcase {
    public function test_ipaddress_access_rule() {
        ...
        $flipquiz->subnet = getremoteaddr(null);
        if (!empty($flipquiz->subnet)) {
            $flipquizobj = new flipquiz($flipquiz, $cm, null);
            $rule = new flipquizaccess_ipaddress($flipquizobj, 0);
