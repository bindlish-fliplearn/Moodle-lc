...
/**
 * Unit tests for the flipquizaccess_timelimit plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/flipquiz/accessrule/timelimit/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class flipquizaccess_timelimit_testcase extends basic_testcase {
    public function test_time_limit_access_rule() {
        $flipquiz = new stdClass();
        $flipquiz->timelimit = 3600;
        $cm = new stdClass();
        $cm->id = 0;
        $flipquizobj = new flipquiz($flipquiz, $cm, null);
        $rule = new flipquizaccess_timelimit($flipquizobj, 10000);
        $attempt = new stdClass();

        $this->assertEquals($rule->description(),
            get_string('flipquiztimelimit', 'flipquizaccess_timelimit', format_time(3600)));
