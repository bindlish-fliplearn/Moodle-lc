...
/**
 * Unit tests for the flipquizaccess_numattempts plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/flipquiz/accessrule/numattempts/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class flipquizaccess_numattempts_testcase extends basic_testcase {
    public function test_num_attempts_access_rule() {
        ...
        $rule = new flipquizaccess_numattempts($flipquizobj, 0);
        $attempt = new stdClass();

        $this->assertEquals($rule->description(),
            get_string('attemptsallowedn', 'flipquizaccess_numattempts', 3));

        $this->assertFalse($rule->prevent_new_attempt(0, $attempt));
        $this->assertFalse($rule->prevent_new_attempt(2, $attempt));
        $this->assertEquals($rule->prevent_new_attempt(3, $attempt),
            get_string('nomoreattempts', 'flipquiz'));
