...
/**
 * This page handles listing of flipquiz overrides
 *
...
require_once($CFG->dirroot.'/mod/flipquiz/override_form.php');
...
$url = new moodle_url('/mod/flipquiz/overrides.php', array('cmid'=>$cm->id, 'mode'=>$mode));
...
require_capability('mod/flipquiz:manageoverrides', $context);

// Display a list of overrides.
$PAGE->set_pagelayout('admin');
$PAGE->set_title(get_string('overrides', 'flipquiz'));
...
$sql = 'SELECT o.id
            FROM {flipquiz_overrides} o LEFT JOIN {groups} g
            ON o.groupid = g.id
...
if (!empty($orphaned)) {
    $DB->delete_records_list('flipquiz_overrides', 'id', array_keys($orphaned));
}
...
if ($groupmode) {
    ...
    $sql = 'SELECT o.*, g.name
                ...
                WHERE o.flipquiz = :flipquizid
                ORDER BY g.name';
    $params = array('flipquizid' => $flipquiz->id);
} else {
...
$overridedeleteurl = new moodle_url('/mod/flipquiz/overridedelete.php');
$overrideediturl = new moodle_url('/mod/flipquiz/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    if (!$groupmode) {
        if (!has_capability('mod/flipquiz:attempt', $context, $override->userid)) {
            // User not allowed to take the flipquiz.
            $active = false;
    ...
    if (isset($override->timeopen)) {
        $fields[] = get_string('flipquizopens', 'flipquiz');
        $values[] = $override->timeopen > 0 ?
                userdate($override->timeopen) : get_string('noopen', 'flipquiz');
    }

    // Format timeclose.
    if (isset($override->timeclose)) {
        $fields[] = get_string('flipquizcloses', 'flipquiz');
        $values[] = $override->timeclose > 0 ?
                userdate($override->timeclose) : get_string('noclose', 'flipquiz');
    }

    // Format timelimit.
    if (isset($override->timelimit)) {
        $fields[] = get_string('timelimit', 'flipquiz');
        $values[] = $override->timelimit > 0 ?
                format_time($override->timelimit) : get_string('none', 'flipquiz');
    }

    // Format number of attempts.
    if (isset($override->attempts)) {
        $fields[] = get_string('attempts', 'flipquiz');
    ...
    if (isset($override->password)) {
        $fields[] = get_string('requirepassword', 'flipquiz');
        $values[] = $override->password !== '' ?
                get_string('enabled', 'flipquiz') : get_string('none', 'flipquiz');
    }
...
echo html_writer::start_tag('div', array('id' => 'flipquizoverrides'));
if (count($table->data)) {
    echo html_writer::table($table);
}
if ($hasinactive) {
    echo $OUTPUT->notification(get_string('inactiveoverridehelp', 'flipquiz'), 'dimmed_text');
}
...
if ($groupmode) {
    if (empty($groups)) {
        // There are no groups.
        echo $OUTPUT->notification(get_string('groupsnone', 'flipquiz'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'addgroup', 'cmid' => $cm->id)),
            get_string('addnewgroupoverride', 'flipquiz'), 'post', $options);
} else {
    $users = array();
    // See if there are any students in the flipquiz.
    $users = get_users_by_capability($context, 'mod/flipquiz:attempt', 'u.id');
    $info = new \core_availability\info_module($cm);
    $users = $info->filter_user_list($users);

    if (empty($users)) {
        // There are no students.
        echo $OUTPUT->notification(get_string('usersnone', 'flipquiz'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'adduser', 'cmid' => $cm->id)),
            get_string('addnewuseroverride', 'flipquiz'), 'get', $options);
}
