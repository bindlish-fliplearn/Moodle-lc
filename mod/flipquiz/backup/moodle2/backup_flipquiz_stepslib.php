...
/**
 * Define all the backup steps that will be used by the backup_flipquiz_activity_task
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_flipquiz_activity_structure_step extends backup_questions_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $flipquiz = new backup_nested_element('flipquiz', array('id'), array(
        ...
        $this->add_subplugin_structure('flipquizaccess', $flipquiz, true);
        ...
        $this->add_subplugin_structure('flipquizaccess', $attempt, true);

        // Build the tree.
        $flipquiz->add_child($qinstances);
        $qinstances->add_child($qinstance);

        $qinstance->add_child($qinstancetags);
        $qinstancetags->add_child($qinstancetag);

        $flipquiz->add_child($sections);
        $sections->add_child($section);

        $flipquiz->add_child($feedbacks);
        $feedbacks->add_child($feedback);

        $flipquiz->add_child($overrides);
        $overrides->add_child($override);

        $flipquiz->add_child($grades);
        $grades->add_child($grade);

        $flipquiz->add_child($attempts);
        $attempts->add_child($attempt);

        // Define sources.
        $flipquiz->set_source_table('flipquiz', array('id' => backup::VAR_ACTIVITYID));

        $qinstance->set_source_table('flipquiz_slots',
                array('flipquizid' => backup::VAR_PARENTID));

        $qinstancetag->set_source_table('flipquiz_slot_tags',
                array('slotid' => backup::VAR_PARENTID));

        $section->set_source_table('flipquiz_sections',
                array('flipquizid' => backup::VAR_PARENTID));

        $feedback->set_source_table('flipquiz_feedback',
                array('flipquizid' => backup::VAR_PARENTID));

        // Quiz overrides to backup are different depending of user info.
        $overrideparams = array('flipquiz' => backup::VAR_PARENTID);
        if (!$userinfo) { //  Without userinfo, skip user overrides.
            $overrideparams['userid'] = backup_helper::is_sqlparam(null);

        }
        $override->set_source_table('flipquiz_overrides', $overrideparams);

        // All the rest of elements only happen if we are including user info.
        if ($userinfo) {
            $grade->set_source_table('flipquiz_grades', array('flipquiz' => backup::VAR_PARENTID));
            $attempt->set_source_sql('
                    SELECT *
                    FROM {flipquiz_attempts}
                    WHERE flipquiz = :flipquiz AND preview = 0',
                    array('flipquiz' => backup::VAR_PARENTID));
        }

        // Define source alias.
        $flipquiz->set_source_alias('attempts', 'attempts_number');
        ...
        $flipquiz->annotate_files('mod_flipquiz', 'intro', null); // This file area hasn't itemid.
        $feedback->annotate_files('mod_flipquiz', 'feedback', 'id');

        // Return the root element (flipquiz), wrapped into standard activity structure.
        return $this->prepare_activity_structure($flipquiz);
    }
}
