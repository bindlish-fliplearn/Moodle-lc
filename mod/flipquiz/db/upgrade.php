...
/**
 * Upgrade script for the flipquiz module.
 *
 ...
 */
function xmldb_flipquiz_upgrade($oldversion) {
    global $CFG, $DB;

    $dbman = $DB->get_manager();

    if ($oldversion < 2016092000) {
        // Define new fields to be added to flipquiz.
        $table = new xmldb_table('flipquiz');
        ...
        upgrade_mod_savepoint(true, 2016092000, 'flipquiz');
    }

    if ($oldversion < 2016092001) {
        // New field for flipquiz_attemps.
        $table = new xmldb_table('flipquiz_attempts');
        ...
        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2016092001, 'flipquiz');
    }

    if ($oldversion < 2016100300) {
        // Find flipquizzes with the combination of require passing grade and grade to pass 0.
        ...
        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2016100300, 'flipquiz');
    }
    ...
    if ($oldversion < 2018020700) {
        ...
        $table = new xmldb_table('flipquiz_slots');

        // Define field questioncategoryid to be added to flipquiz_slots.
        ...
        // Define key questioncategoryid (foreign) to be added to flipquiz_slots.
        ...
        // Define field includingsubcategories to be added to flipquiz_slots.
    ...
    if ($oldversion < 2018020701) {
        ...
        // For each question, move the configuration data to the flipquiz_slots table.
        $pbar = new progress_bar('updateflipquizslotswithrandom', 500, true);
        foreach ($rs as $record) {
            $data = new stdClass();
            $data->id = $record->id;
            $data->questioncategoryid = $record->category;
            $data->includingsubcategories = empty($record->questiontext) ? 0 : 1;
            $DB->update_record('flipquiz_slots', $data);

            // Update progress.
            $a->done++;
            $pbar->update($a->done, $a->total, get_string('updateflipquizslotswithrandomxofy', 'flipquiz', $a));
        }
        $rs->close();

        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2018020701, 'flipquiz');
    }

    if ($oldversion < 2018040700) {

        // Define field tags to be dropped from flipquiz_slots. This field was added earlier to master only.
        $table = new xmldb_table('flipquiz_slots');
        $field = new xmldb_field('tags');

        // Conditionally launch drop field flipquizid.
        if ($dbman->field_exists($table, $field)) {
            $dbman->drop_field($table, $field);
        }

        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2018040700, 'flipquiz');
    }

    if ($oldversion < 2018040800) {

        // Define table flipquiz_slot_tags to be created.
        $table = new xmldb_table('flipquiz_slot_tags');

        // Adding fields to table flipquiz_slot_tags.
        ...
        // Adding keys to table flipquiz_slot_tags.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->add_key('slotid', XMLDB_KEY_FOREIGN, array('slotid'), 'flipquiz_slots', array('id'));
        $table->add_key('tagid', XMLDB_KEY_FOREIGN, array('tagid'), 'tag', array('id'));

        // Conditionally launch create table for flipquiz_slot_tags.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2018040800, 'flipquiz');
    }
