...
/**
 * Base class for flipquiz reports that are basically a table with one row for each attempt.
 *
 * @copyright 2010 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class flipquiz_attempts_report extends flipquiz_default_report {
    ...
    /** @var object the flipquiz context. */
    protected $context;

    /** @var mod_flipquiz_attempts_report_form The settings form to use. */
    protected $form;
     ...
     */
    protected function init($mode, $formclass, $flipquiz, $cm, $course) {
        $this->mode = $mode;
        ...
        $this->qmsubselect = flipquiz_report_qm_filter_select($flipquiz);

        $this->form = new $formclass($this->get_base_url(),
                array('flipquiz' => $flipquiz, 'currentgroup' => $currentgroup, 'context' => $this->context));
    ...
    protected function get_students_joins($cm, $course = null) {
        ...
        $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                array('mod/flipquiz:attempt', 'mod/flipquiz:reviewmyattempts'), $currentgroup);

        return array($currentgroup, $studentsjoins, $groupstudentsjoins, $groupstudentsjoins);
    }

    /**
     * Outputs the things you commonly want at the top of a flipquiz report.
     *
     ...
     * @param mod_flipquiz_attempts_report_options $options the current report settings.
     * @param int $currentgroup the current group.
     * @param bool $hasquestions whether there are any questions in the flipquiz.
     * @param bool $hasstudents whether there are any relevant students.
     */
    protected function print_standard_header_and_messages($cm, $course, $flipquiz,
            $options, $currentgroup, $hasquestions, $hasstudents) {
        global $OUTPUT;

        $this->print_header_and_tabs($cm, $course, $flipquiz, $this->mode);
        ...
        if ($strattemptnum = flipquiz_num_attempt_summary($flipquiz, $cm, true, $currentgroup)) {
            echo '<div class="flipquizattemptcounts">' . $strattemptnum . '</div>';
        }
...
//        }
    ...
    protected function add_state_column(&$columns, &$headers) {
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'flipquiz');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'flipquiz');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'flipquiz');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'flipquiz');
    }
     ...
     * @param bool $usercanseegrades whether the user is allowed to see grades for this flipquiz.
     * @param array $columns the list of columns. Added to.
     * @param array $headers the columns headings. Added to.
     * @param bool $includefeedback whether to include the feedbacktext columns
     */
    protected function add_grade_columns($flipquiz, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            $columns[] = 'sumgrades';
            $headers[] = get_string('grade', 'flipquiz') . '/' .
                    flipquiz_format_grade($flipquiz, $flipquiz->grade);
        }

        if ($includefeedback && flipquiz_has_feedback($flipquiz)) {
            $columns[] = 'feedbacktext';
            $headers[] = get_string('feedback', 'flipquiz');
        }
    }
     ...
     * @param mod_flipquiz_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_flipquiz_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/flipquiz:deleteattempts', $this->context);
                    $this->delete_selected_attempts($flipquiz, $cm, $attemptids, $allowedjoins);
    ...
    /**
     * Delete the flipquiz attempts
     * @param object $flipquiz the flipquiz settings. Attempts that don't belong to
     * this flipquiz are not deleted.
     ...
     */
    protected function delete_selected_attempts($flipquiz, $cm, $attemptids, \core\dml\sql_join $allowedjoins) {
        global $DB;
        ...
        foreach ($attemptids as $attemptid) {
            if (empty($allowedjoins->joins)) {
                $sql = "SELECT flipquiza.*
                         ...
                         WHERE flipquiza.id = :attemptid";
            } else {
                $sql = "SELECT flipquiza.*
                          FROM {flipquiz_attempts} flipquiza
                          JOIN {user} u ON u.id = flipquiza.userid
                        {$allowedjoins->joins}
                         WHERE {$allowedjoins->wheres} AND flipquiza.id = :attemptid";
            }
            $params = $allowedjoins->params + array('attemptid' => $attemptid);
            $attempt = $DB->get_record_sql($sql, $params);
            if (!$attempt || $attempt->flipquiz != $flipquiz->id || $attempt->preview != 0) {
                // Ensure the attempt exists, belongs to this flipquiz and belongs to
                // a student included in the report. If not skip.
                continue;
            }

            // Set the course module id before calling flipquiz_delete_attempt().
            $flipquiz->cmid = $cm->id;
