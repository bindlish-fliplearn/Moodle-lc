...
/**
 * This file defines the flipquiz responses report class.
 *
...
require_once($CFG->dirroot . '/mod/flipquiz/report/responses/responses_options.php');
require_once($CFG->dirroot . '/mod/flipquiz/report/responses/responses_form.php');
require_once($CFG->dirroot . '/mod/flipquiz/report/responses/last_responses_table.php');
require_once($CFG->dirroot . '/mod/flipquiz/report/responses/first_or_all_responses_table.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class flipquiz_responses_report extends flipquiz_attempts_report {

    public function display($flipquiz, $cm, $course) {
        global $OUTPUT, $DB;

        list($currentgroup, $studentsjoins, $groupstudentsjoins, $allowedjoins) = $this->init(
                'responses', 'flipquiz_responses_settings_form', $flipquiz, $cm, $course);

        $options = new flipquiz_responses_options('responses', $flipquiz, $cm, $course);
        ...
        if ($options->whichtries === question_attempt::LAST_TRY) {
            $tableclassname = 'flipquiz_last_responses_table';
        } else {
            $tableclassname = 'flipquiz_first_or_all_responses_table';
        }
        $table = new $tableclassname($flipquiz, $this->context, $this->qmsubselect,
                $options, $groupstudentsjoins, $studentsjoins, $questions, $options->get_url());
        $filename = flipquiz_report_download_filename(get_string('responsesfilename', 'flipquiz_responses'),
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
            ...
            $this->add_grade_columns($flipquiz, $options->usercanseegrades, $columns, $headers);
            ...
            foreach ($questions as $id => $question) {
                ...
                if ($options->showresponses) {
                    $columns[] = 'response' . $id;
                    $headers[] = get_string('responsex', 'flipquiz_responses', $question->number);
                }
                if ($options->showright) {
                    $columns[] = 'right' . $id;
                    $headers[] = get_string('rightanswerx', 'flipquiz_responses', $question->number);
                }
            }
