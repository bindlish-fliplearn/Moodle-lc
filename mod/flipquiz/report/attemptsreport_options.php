...
/**
 * Base class for the options that control what is visible in an {@link flipquiz_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_flipquiz_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the flipquiz being reported on. */
    public $flipquiz;

    /** @var object the course module objects for the flipquiz being reported on. */
    public $cm;

    /** @var object the course settings for the course the flipquiz is in. */
    public $course;

    /**
     * @var array form field name => corresponding flipquiz_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => flipquiz_attempt::IN_PROGRESS,
        'stateoverdue'    => flipquiz_attempt::OVERDUE,
        'statefinished'   => flipquiz_attempt::FINISHED,
        'stateabandoned'  => flipquiz_attempt::ABANDONED,
    );

    /**
     * @var string flipquiz_attempts_report::ALL_WITH or flipquiz_attempts_report::ENROLLED_WITH
     *      flipquiz_attempts_report::ENROLLED_WITHOUT or flipquiz_attempts_report::ENROLLED_ALL
     */
    public $attempts = flipquiz_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of flipquiz_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(flipquiz_attempt::IN_PROGRESS, flipquiz_attempt::OVERDUE,
            flipquiz_attempt::FINISHED, flipquiz_attempt::ABANDONED);
    ...
    public $pagesize = flipquiz_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $flipquiz the settings for the flipquiz being reported on.
     * @param object $cm the course module objects for the flipquiz being reported on.
     * @param object $coures the course settings for the coures this flipquiz is in.
     */
    public function __construct($mode, $flipquiz, $cm, $course) {
        $this->mode   = $mode;
        $this->flipquiz   = $flipquiz;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = flipquiz_report_should_show_grades($flipquiz, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/flipquiz/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('flipquiz_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('flipquiz_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == flipquiz_attempts_report::ALL_WITH) {
                $this->attempts = flipquiz_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = flipquiz_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(flipquiz_attempts_report::ALL_WITH, flipquiz_attempts_report::ENROLLED_WITH,
                flipquiz_attempts_report::ENROLLED_WITHOUT, flipquiz_attempts_report::ENROLLED_ALL))) {
            $this->attempts = flipquiz_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!flipquiz_report_can_filter_only_graded($this->flipquiz)) {
        ...
        if ($this->attempts == flipquiz_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = flipquiz_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(flipquiz_attempt::FINISHED, $this->states);
    }
}
