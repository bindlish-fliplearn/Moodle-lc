 ...
 *
 * The flipquiz stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-flipquiz-tools/tree/master/statsspreadsheet
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class flipquiz_report_statistics_from_steps_testcase extends mod_flipquiz_attempt_walkthrough_from_csv_testcase {

    /**
     * @var flipquiz_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    public function test_walkthrough_from_csv($flipquizsettings, $csvdata) {
        ...
        $whichattempts = FLIPQUIZ_GRADEAVERAGE; // All attempts.
        $whichtries = question_attempt::ALL_TRIES;
        $groupstudentsjoins = new \core\dml\sql_join();
        list($questions, $flipquizstats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata,
                            $whichattempts, $whichtries, $groupstudentsjoins);
        if ($flipquizsettings['testnumber'] === '00') {
            $this->check_variants_count_for_flipquiz_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_flipquiz_stats_for_flipquiz_00($flipquizstats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_flipquiz_00($questions, $questionstats, $whichtries, $qubaids) {
     ...
     * @param $flipquizstats
     */
    protected function check_flipquiz_stats_for_flipquiz_00($flipquizstats) {
        $flipquizstatsexpected = array(
            'median'             => 4.5,
        ...
        foreach ($flipquizstatsexpected as $statname => $statvalue) {
            $this->assertEquals($statvalue, $flipquizstats->$statname, $flipquizstats->$statname, abs($statvalue) * 1.5e-5);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $flipquizstats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries,
        ...
        $this->report = new flipquiz_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->flipquiz);
        list($flipquizstats, $questionstats) = $this->report->get_all_stats_and_analysis($this->flipquiz,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudentsjoins,
                                                                                     $questions);

        $qubaids = flipquiz_statistics_qubaids_condition($this->flipquiz->id, $groupstudentsjoins, $whichattempts);

        // We will create some flipquiz and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $flipquizcalc = new \flipquiz_statistics\calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($flipquizcalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $flipquizstats, $questionstats, $qubaids);
        }
        return array($questions, $flipquizstats, $questionstats, $qubaids);
    }
