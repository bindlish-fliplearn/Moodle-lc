...
/**
 * Quiz statistics report, table for showing statistics of each question in the flipquiz.
 *
...
/**
 * This table has one row for each question in the flipquiz, with sub-rows when
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class flipquiz_statistics_table extends flexible_table {
    /** @var object the flipquiz settings. */
    protected $flipquiz;

    /** @var integer the flipquiz course_module id. */
    protected $cmid;
    ...
    public function __construct() {
        parent::__construct('mod-flipquiz-report-statistics-report');
    }
     ...
     * @param int $cmid the flipquiz course_module id
     * @param moodle_url $reporturl the URL to redisplay this report.
     * @param int $s number of attempts included in the statistics.
     */
    public function statistics_setup($flipquiz, $cmid, $reporturl, $s) {
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'flipquiz_statistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'flipquiz_statistics');
        }

        $columns[] = 'name';
        $headers[] = get_string('questionname', 'flipquiz');

        $columns[] = 's';
        $headers[] = get_string('attempts', 'flipquiz_statistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'flipquiz_statistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'flipquiz_statistics');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'flipquiz_statistics');

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'flipquiz_statistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'flipquiz_statistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'flipquiz_statistics');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'flipquiz_statistics');
    ...
    protected function col_actions($questionstat) {
        return flipquiz_question_action_icons($this->flipquiz, $this->cmid,
    ...
    protected function col_name($questionstat) {
        ...
        if (!is_null($questionstat->variant)) {
            ...
            $name = get_string('nameforvariant', 'flipquiz_statistics', $a);
        }
        ...
        if (!is_null($questionstat->variant)) {
            ...
            if ($questionstat->subquestion && !$questionstat->get_variants()) {
                ...
                $name = html_writer::link($url, $name, array('title' => get_string('detailedanalysis', 'flipquiz_statistics')));
                ...
                if ($questionstat->get_variants() || $questionstat->get_sub_question_ids()) {
                    ...
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('slotstructureanalysis', 'flipquiz_statistics', $number)));
                } else {
                    // Question cannot be broken down into sub-questions or variants. Link will show response analysis page.
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('detailedanalysis', 'flipquiz_statistics')));
                }
            }
        }
        ...
        if (!empty($questionstat->minmedianmaxnotice)) {
            $name = get_string($questionstat->minmedianmaxnotice, 'flipquiz_statistics') . '<br />' . $name;
        }
     ...
     * of maximum mark for the flipquiz. That is, the indended influence this question
     * on the student's overall mark.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
    protected function col_intended_weight($questionstat) {
        return flipquiz_report_scale_summarks_as_percentage($questionstat->maxmark, $this->flipquiz);
    }
    ...
    protected function col_effective_weight($questionstat) {
        ...
        if ($questionstat->negcovar) {
            $negcovar = get_string('negcovar', 'flipquiz_statistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'flipquiz_statistics'),
     ...
     * other questions in this flipquiz.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
