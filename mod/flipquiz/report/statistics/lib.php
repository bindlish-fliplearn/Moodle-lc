...
/**
 * Standard plugin entry points of the flipquiz statistics report.
 *
 ...
 * @package  flipquiz_statistics
 * @category files
 * @param context $previewcontext the flipquiz context
 ...
 */
function flipquiz_statistics_question_preview_pluginfile($previewcontext, $questionid,
    ...
    require_capability('flipquiz/statistics:view', $context);
 ...
 */
function flipquiz_statistics_cron() {
    global $DB;

    mtrace("\n  Cleaning up old flipquiz statistics cache records...", '');

    $expiretime = time() - 5*HOURSECS;
    $DB->delete_records_select('flipquiz_statistics', 'timemodified < ?', array($expiretime));
