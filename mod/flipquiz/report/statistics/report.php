...
require_once($CFG->dirroot . '/mod/flipquiz/report/statistics/statistics_form.php');
require_once($CFG->dirroot . '/mod/flipquiz/report/statistics/statistics_table.php');
require_once($CFG->dirroot . '/mod/flipquiz/report/statistics/statistics_question_table.php');
require_once($CFG->dirroot . '/mod/flipquiz/report/statistics/statisticslib.php');
/**
 * The flipquiz statistics report provides summary information about each question in
 * a flipquiz, compared to the whole flipquiz. It also provides a drill-down to more
 * detailed information about each question.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class flipquiz_statistics_report extends flipquiz_default_report {

    /** @var context_module context of this flipquiz.*/
    protected $context;

    /** @var flipquiz_statistics_table instance of table class used for main questions stats table. */
    protected $table;
    ...
    public function display($flipquiz, $cm, $course) {
        ...
        if (!flipquiz_has_questions($flipquiz->id)) {
            $this->print_header_and_tabs($cm, $course, $flipquiz, 'statistics');
            echo flipquiz_no_questions_message($flipquiz, $cm, $this->context);
            return true;
        }
        ...
        $whichattempts = optional_param('whichattempts', $flipquiz->grademethod, PARAM_INT);
        ...
        $reporturl = new moodle_url('/mod/flipquiz/report.php', $pageoptions);

        $mform = new flipquiz_statistics_settings_form($reporturl, compact('flipquiz'));

        $mform->set_data(array('whichattempts' => $whichattempts, 'whichtries' => $whichtries));

        if ($whichattempts != $flipquiz->grademethod) {
        ...
        if (empty($currentgroup)) {
            ...
            // All users who can attempt flipquizzes and who are in the currently selected group.
            $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                    array('mod/flipquiz:reviewmyattempts', 'mod/flipquiz:attempt'), $currentgroup);
        ...
        $qubaids = flipquiz_statistics_qubaids_condition($flipquiz->id, $groupstudentsjoins, $whichattempts);
        ...
        $this->table = new flipquiz_statistics_table();
        if ($everything) {
            $report = get_string('completestatsfilename', 'flipquiz_statistics');
        } else {
            $report = get_string('questionstatsfilename', 'flipquiz_statistics');
        }
        $courseshortname = format_string($course->shortname, true,
                array('context' => context_course::instance($course->id)));
        $filename = flipquiz_report_download_filename($report, $courseshortname, $flipquiz->name);
        $this->table->is_downloading($download, $filename,
                get_string('flipquizstructureanalysis', 'flipquiz_statistics'));
        $questions = $this->load_and_initialise_questions_for_calculations($flipquiz);
        ...
        if (!$nostudentsingroup) {
            ...
            list($flipquizstats, $questionstats) =
                $this->get_all_stats_and_analysis($flipquiz, $whichattempts, $whichtries, $groupstudentsjoins, $questions, $progress);
        } else {
            // Or create empty stats containers.
            $flipquizstats = new \flipquiz_statistics\calculated($whichattempts);
            $questionstats = new \core_question\statistics\questions\all_calculated_for_qubaid_condition();
        }

        // Set up the table.
        $this->table->statistics_setup($flipquiz, $cm->id, $reporturl, $flipquizstats->s());
        ...
        if (!$this->table->is_downloading()) {
            ...
            if (groups_get_activity_groupmode($cm)) {
                ...
                if ($currentgroup && $nostudentsingroup) {
                    $OUTPUT->notification(get_string('nostudentsingroup', 'flipquiz_statistics'));
                }
            }

            if (!$this->table->is_downloading() && $flipquizstats->s() == 0) {
                echo $OUTPUT->notification(get_string('nogradedattempts', 'flipquiz_statistics'));
            }
        ...
        if ($everything) { // Implies is downloading.
            ...
            $flipquizinfo = $flipquizstats->get_formatted_flipquiz_info_data($course, $cm, $flipquiz);
            $this->download_flipquiz_info_table($flipquizinfo);

            if ($flipquizstats->s()) {
                $this->output_flipquiz_structure_analysis_table($questionstats);

                if ($this->table->is_downloading() == 'html' && $flipquizstats->s() != 0) {
                    $this->output_statistics_graph($flipquiz->id, $qubaids);
                }
            ...
            $this->output_individual_question_data($flipquiz, $questionstats->for_subq($qid, $variantno));
            ...
            echo $OUTPUT->box('<a href="' . $reporturl->out() . '">' .
                              get_string('backtoflipquizreport', 'flipquiz_statistics') . '</a>',
            ...
            if ($variantno === null &&
                ...
                if (!$this->table->is_downloading()) {
                    ...
                    echo $OUTPUT->heading(get_string('slotstructureanalysis', 'flipquiz_statistics', $number), 3);
                }
                ...
                $this->output_individual_question_data($flipquiz, $questionstats->for_slot($slot, $variantno));
            ...
            echo $OUTPUT->heading(get_string('flipquizinformation', 'flipquiz_statistics'), 3);
            echo $this->output_caching_info($flipquizstats->timemodified, $flipquiz->id, $groupstudentsjoins, $whichattempts, $reporturl);
            echo $this->everything_download_options($reporturl);
            $flipquizinfo = $flipquizstats->get_formatted_flipquiz_info_data($course, $cm, $flipquiz);
            echo $this->output_flipquiz_info_table($flipquizinfo);
            if ($flipquizstats->s()) {
                echo $OUTPUT->heading(get_string('flipquizstructureanalysis', 'flipquiz_statistics'), 3);
                $this->output_flipquiz_structure_analysis_table($questionstats);
                $this->output_statistics_graph($flipquiz, $qubaids);
            }
        }
     ...
     */
    protected function output_individual_question_data($flipquiz, $questionstat) {
        global $OUTPUT;

        // On-screen display. Show a summary of the question's place in the flipquiz,
        ...
        $questioninfotable->data[] = array(get_string('modulename', 'flipquiz'), $flipquiz->name);
        $questioninfotable->data[] = array(get_string('questionname', 'flipquiz_statistics'),
                $questionstat->question->name.'&nbsp;'.$datumfromtable['actions']);

        if ($questionstat->variant !== null) {
            $questioninfotable->data[] = array(get_string('variant', 'flipquiz_statistics'), $questionstat->variant);

        }
        $questioninfotable->data[] = array(get_string('questiontype', 'flipquiz_statistics'),
                $datumfromtable['icon'] . '&nbsp;' .
                question_bank::get_qtype($questionstat->question->qtype, false)->menu_name() . '&nbsp;' .
                $datumfromtable['icon']);
        $questioninfotable->data[] = array(get_string('positions', 'flipquiz_statistics'),
        ...
        $labels = array(
            's' => get_string('attempts', 'flipquiz_statistics'),
            'facility' => get_string('facility', 'flipquiz_statistics'),
            'sd' => get_string('standarddeviationq', 'flipquiz_statistics'),
            'random_guess_score' => get_string('random_guess_score', 'flipquiz_statistics'),
            'intended_weight' => get_string('intended_weight', 'flipquiz_statistics'),
            'effective_weight' => get_string('effective_weight', 'flipquiz_statistics'),
            'discrimination_index' => get_string('discrimination_index', 'flipquiz_statistics'),
            'discriminative_efficiency' =>
                                get_string('discriminative_efficiency', 'flipquiz_statistics')
        );
        ...
        echo $OUTPUT->heading(get_string('questioninformation', 'flipquiz_statistics'), 3);
        echo html_writer::table($questioninfotable);
        echo $this->render_question_text($questionstat->question);
        echo $OUTPUT->heading(get_string('questionstatistics', 'flipquiz_statistics'), 3);
    ...
    protected function render_question_text($question) {
        ...
        $text = question_rewrite_question_preview_urls($question->questiontext, $question->id,
                ...
                $this->context->id, 'flipquiz_statistics');
    ...
    protected function output_individual_question_response_analysis($question, $variantno, $s, $reporturl, $qubaids,
        ...
        $qtable = new flipquiz_statistics_question_table($question->id);
        $exportclass = $this->table->export_class_instance();
        $qtable->export_class_instance($exportclass);
        if (!$this->table->is_downloading()) {
            // Output an appropriate title.
            echo $OUTPUT->heading(get_string('analysisofresponses', 'flipquiz_statistics'), 3);
            ...
            if (!empty($question->number) && !is_null($variantno)) {
                $questiontabletitle = get_string('analysisnovariant', 'flipquiz_statistics', $a);
            } else if (!empty($question->number)) {
                $questiontabletitle = get_string('analysisno', 'flipquiz_statistics', $a);
            } else if (!is_null($variantno)) {
                $questiontabletitle = get_string('analysisvariant', 'flipquiz_statistics', $a);
            } else {
                $questiontabletitle = get_string('analysisnameonly', 'flipquiz_statistics', $a);
            }

            if ($this->table->is_downloading() == 'html') {
                $questiontabletitle = get_string('analysisofresponsesfor', 'flipquiz_statistics', $questiontabletitle);
            }
    ...
    /**
     * Output the table that lists all the questions in the flipquiz with their statistics.
     *
     ...
     *                                                                                               the flipquiz including subqs and
     *                                                                                               variants.
     */
    protected function output_flipquiz_structure_analysis_table($questionstats) {
        $tooutput = array();
    ...
    /**
     * Return HTML for table of overall flipquiz statistics.
     *
     * @param array $flipquizinfo as returned by {@link get_formatted_flipquiz_info_data()}.
     * @return string the HTML.
     */
    protected function output_flipquiz_info_table($flipquizinfo) {

        $flipquizinfotable = new html_table();
        $flipquizinfotable->align = array('center', 'center');
        $flipquizinfotable->width = '60%';
        $flipquizinfotable->attributes['class'] = 'generaltable titlesleft';
        $flipquizinfotable->data = array();

        foreach ($flipquizinfo as $heading => $value) {
             $flipquizinfotable->data[] = array($heading, $value);
        }

        return html_writer::table($flipquizinfotable);
    }

    /**
     * Download the table of overall flipquiz statistics.
     *
     * @param array $flipquizinfo as returned by {@link get_formatted_flipquiz_info_data()}.
     */
    protected function download_flipquiz_info_table($flipquizinfo) {
        global $OUTPUT;
        ...
        // Do the output.
        $exportclass = $this->table->export_class_instance();
        $exportclass->start_table(get_string('flipquizinformation', 'flipquiz_statistics'));
     ...
     *
     * @param int|object $flipquizorid The flipquiz, or its ID.
     ...
     */
    protected function output_statistics_graph($flipquizorid, $qubaids) {
        global $DB, $PAGE;

        $flipquiz = $flipquizorid;
        if (!is_object($flipquiz)) {
            $flipquiz = $DB->get_record('flipquiz', array('id' => $flipquizorid), '*', MUST_EXIST);
        }
        ...
        $fieldstoplot = [
            'facility' => get_string('facility', 'flipquiz_statistics'),
            'discriminativeefficiency' => get_string('discriminative_efficiency', 'flipquiz_statistics')
        ];
        ...
        $chart->get_xaxis(0, true)->set_label(get_string('position', 'flipquiz_statistics'));
        ...
        $graphname = get_string('statisticsreportgraph', 'flipquiz_statistics');
        echo $output->chart($chart, $graphname);
    }

    /**
     * Get the flipquiz and question statistics, either by loading the cached results,
     ...
     * @return array with 2 elements:    - $flipquizstats The statistics for overall attempt scores.
    ...
    public function get_all_stats_and_analysis(
            $flipquiz, $whichattempts, $whichtries, \core\dml\sql_join $groupstudentsjoins, $questions, $progress = null) {
        ...
        $flipquizcalc = new \flipquiz_statistics\calculator($progress);

        $progress->start_progress('', 3);
        if ($flipquizcalc->get_last_calculated_time($qubaids) === false) {

            // Recalculate now.
            $questionstats = $qcalc->calculate($qubaids);
            $progress->progress(1);

            $flipquizstats = $flipquizcalc->calculate($flipquiz->id, $whichattempts, $groupstudentsjoins, count($questions),
                                              $qcalc->get_sum_of_mark_variance());
            $progress->progress(2);
        } else {
            $flipquizstats = $flipquizcalc->get_cached($qubaids);
            $progress->progress(1);
        ...
        return array($flipquizstats, $questionstats);
    }
    ...
    protected function get_progress_trace_instance() {
        if ($this->progress === null) {
            if (!$this->table->is_downloading()) {
                $this->progress = new \core\progress\display_if_slow(get_string('calculatingallstats', 'flipquiz_statistics'));
    ...
    /**
     * Analyse responses for all questions and sub questions in this flipquiz.
     *
    ...
    /**
     * Return a little form for the user to request to download the full report, including flipquiz stats and response analysis for
    ...
    protected function everything_download_options(moodle_url $reporturl) {
        global $OUTPUT;
        return $OUTPUT->download_dataformat_selector(get_string('downloadeverything', 'flipquiz_statistics'),
     ...
     * @return string HTML.
     */
    protected function output_caching_info($lastcachetime, $flipquizid, $groupstudentsjoins, $whichattempts, $reporturl) {
        global $DB, $OUTPUT;
        ...
        list($fromqa, $whereqa, $qaparams) = flipquiz_statistics_attempts_sql($flipquizid, $groupstudentsjoins, $whichattempts, true);
        $count = $DB->count_records_sql("
                SELECT COUNT(1)
                FROM $fromqa
                WHERE $whereqa
                AND flipquiza.timefinish > {$lastcachetime}", $qaparams);
        ...
        $output .= get_string('lastcalculated', 'flipquiz_statistics', $a);
        $output .= $OUTPUT->single_button($recalcualteurl,
                get_string('recalculatenow', 'flipquiz_statistics'));
    ...
    protected function clear_cached_data($qubaids) {
        global $DB;
        $DB->delete_records('flipquiz_statistics', array('hashcode' => $qubaids->get_hash_code()));
    ...
    /**
     * Load the questions in this flipquiz and add some properties to the objects needed in the reports.
     *
     * @param object $flipquiz the flipquiz.
     * @return array of questions for this flipquiz.
     */
    public function load_and_initialise_questions_for_calculations($flipquiz) {
        // Load the questions.
        $questions = flipquiz_report_get_significant_questions($flipquiz);
        $questionids = array();
        foreach ($questions as $question) {
