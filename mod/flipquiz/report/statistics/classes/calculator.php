...
/**
 * Class to calculate and also manage caching of flipquiz statistics.
 *
...
class calculator {
    ...
    /**
     * Compute the flipquiz statistics.
     *
     ...
     * @return calculated $flipquizstats The statistics for overall attempt scores.
     */
    public function calculate($flipquizid, $whichattempts, \core\dml\sql_join $groupstudentsjoins, $p, $sumofmarkvariance) {

        $this->progress->start_progress('', 3);

        $flipquizstats = new calculated($whichattempts);

        $countsandaverages = $this->attempt_counts_and_averages($flipquizid, $groupstudentsjoins);
        $this->progress->progress(1);

        foreach ($countsandaverages as $propertyname => $value) {
            $flipquizstats->{$propertyname} = $value;
        }
        ...
        if ($s != 0) {
            ...
            list($fromqa, $whereqa, $qaparams) =
                flipquiz_statistics_attempts_sql($flipquizid, $groupstudentsjoins, $whichattempts);

            $flipquizstats->median = $this->median($s, $fromqa, $whereqa, $qaparams);
            $this->progress->progress(2);

            if ($s > 1) {

                $powers = $this->sum_of_powers_of_difference_to_mean($flipquizstats->avg(), $fromqa, $whereqa, $qaparams);
                $this->progress->progress(3);

                $flipquizstats->standarddeviation = sqrt($powers->power2 / ($s - 1));
                ...
                if ($s > 2) {
                    ...
                    $k3 = $s * $s * $m3 / (($s - 1) * ($s - 2));
                    if ($k2 != 0) {
                        $flipquizstats->skewness = $k3 / (pow($k2, 3 / 2));

                        // Kurtosis.
                        if ($s > 3) {
                            $k4 = $s * $s * ((($s + 1) * $m4) - (3 * ($s - 1) * $m2 * $m2)) / (($s - 1) * ($s - 2) * ($s - 3));
                            $flipquizstats->kurtosis = $k4 / ($k2 * $k2);
                        }

                        if ($p > 1) {
                            $flipquizstats->cic = (100 * $p / ($p - 1)) * (1 - ($sumofmarkvariance / $k2));
                            $flipquizstats->errorratio = 100 * sqrt(1 - ($flipquizstats->cic / 100));
                            $flipquizstats->standarderror = $flipquizstats->errorratio *
                                $flipquizstats->standarddeviation / 100;
                        }
                    }
            ...
            $flipquizstats->cache(flipquiz_statistics_qubaids_condition($flipquizid, $groupstudentsjoins, $whichattempts));
        }
        $this->progress->end_progress();
        return $flipquizstats;
    }
    ...
    public function get_cached($qubaids) {
        ...
        $fromdb = $DB->get_record_select('flipquiz_statistics', 'hashcode = ? AND timemodified > ?',
    ...
    public function get_last_calculated_time($qubaids) {
        ...
        $timemodified = time() - self::TIME_TO_CACHE;
        return $DB->get_field_select('flipquiz_statistics', 'timemodified', 'hashcode = ? AND timemodified > ?',
                                         array($qubaids->get_hash_code(), $timemodified));
    }

    /**
     * Given a particular flipquiz grading method return a lang string describing which attempts contribute to grade.
     *
    ...
    public static function using_attempts_lang_string($whichattempts) {
         return get_string(static::using_attempts_string_id($whichattempts), 'flipquiz_statistics');
    }

    /**
     * Given a particular flipquiz grading method return a string id for use as a field name prefix in mdl_flipquiz_statistics or to
     ...
     */
    protected function attempt_counts_and_averages($flipquizid, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;

        $attempttotals = new \stdClass();
        foreach (array_keys(flipquiz_get_grading_options()) as $which) {

            list($fromqa, $whereqa, $qaparams) = flipquiz_statistics_attempts_sql($flipquizid, $groupstudentsjoins, $which);
    ...
    protected function median($s, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT flipquiza.id, flipquiza.sumgrades
                  FROM $fromqa
                 WHERE $whereqa
              ORDER BY sumgrades";
    ...
    protected function sum_of_powers_of_difference_to_mean($mean, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT
                    SUM(POWER((flipquiza.sumgrades - $mean), 2)) AS power2,
                    SUM(POWER((flipquiza.sumgrades - $mean), 3)) AS power3,
                    SUM(POWER((flipquiza.sumgrades - $mean), 4)) AS power4
                  FROM $fromqa
                 WHERE $whereqa";
