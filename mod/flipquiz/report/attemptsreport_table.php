...
/**
 * Base class for the table used by a {@link flipquiz_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class flipquiz_attempts_report_table extends table_sql {
    ...
    /** @var object the flipquiz settings for the flipquiz we are reporting on. */
    protected $flipquiz;
    ...
    /** @var object mod_flipquiz_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this flipquiz.. */
    protected $questions;
     ...
     * @param mod_flipquiz_attempts_report_options $options
     ...
     */
    public function __construct($uniqueid, $flipquiz, $context, $qmsubselect,
            mod_flipquiz_attempts_report_options $options, \core\dml\sql_join $groupstudentsjoins, \core\dml\sql_join $studentsjoins,
            $questions, $reporturl) {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/flipquiz/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'flipquiz'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return flipquiz_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = flipquiz_report_feedback_for_grade(
                flipquiz_rescale_grade($attempt->sumgrades, $this->flipquiz, false),
                $this->flipquiz->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/flipquiz/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'flipquiz')));
    ...
    public function base_sql(\core\dml\sql_join $allowedstudentsjoins) {
        ...
        // Please note this uniqueid column is not the same as flipquiza.uniqueid.
        $fields = 'DISTINCT ' . $DB->sql_concat('u.id', "'#'", 'COALESCE(flipquiza.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                flipquiza.uniqueid AS usageid,
                flipquiza.id AS attempt,
                u.id AS userid,
                u.idnumber, ' . $allnames . ',
                u.picture,
                u.imagealt,
                ...
                flipquiza.state,
                flipquiza.sumgrades,
                flipquiza.timefinish,
                flipquiza.timestart,
                CASE WHEN flipquiza.timefinish = 0 THEN null
                     WHEN flipquiza.timefinish > flipquiza.timestart THEN flipquiza.timefinish - flipquiza.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and flipquiz_attempts tables.
        $from = " {user} u";
        $from .= "\nLEFT JOIN {flipquiz_attempts} flipquiza ON
                                    flipquiza.userid = u.id AND flipquiza.flipquiz = :flipquizid";
        $params = array('flipquizid' => $this->flipquiz->id);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (flipquiza.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = flipquiz_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case flipquiz_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'flipquiza.id IS NOT NULL AND flipquiza.preview = 0';
                break;
            case flipquiz_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "flipquiza.preview = 0 AND flipquiza.id IS NOT NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case flipquiz_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "flipquiza.id IS NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case flipquiz_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "(flipquiza.preview = 0 OR flipquiza.preview IS NULL) AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (flipquiza.state $statesql OR flipquiza.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{flipquiz_attempts} {$alias}flipquiza", "{$alias}flipquiza.uniqueid",
                "{$alias}flipquiza.flipquiz = :{$alias}flipquizid", array("{$alias}flipquizid" => $this->sql->params['flipquizid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = flipquiza.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join("(
                SELECT DISTINCT flipquiza.uniqueid
                  FROM " . $this->sql->from . "
                 WHERE " . $this->sql->where . "
                    ) flipquizasubquery", 'flipquizasubquery.uniqueid',
                    "1 = 1", $this->sql->params);
        }
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['flipquiza.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    public function wrap_html_finish() {
        ...
        echo '<a id="checkattempts" href="#">' .
                get_string('selectall', 'flipquiz') . '</a> / ';
        echo '<a id="uncheckattempts" href="#">' .
                get_string('selectnone', 'flipquiz') . '</a> ';
