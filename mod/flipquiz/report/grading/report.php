...
/**
 * This file defines the flipquiz manual grading report class.
 *
...
require_once($CFG->dirroot . '/mod/flipquiz/report/grading/gradingsettings_form.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class flipquiz_grading_report extends flipquiz_default_report {
    ...
    public function display($flipquiz, $cm, $course) {
        ...
        require_capability('mod/flipquiz:grade', $this->context);
        $shownames = has_capability('flipquiz/grading:viewstudentnames', $this->context);
        $showidnumbers = has_capability('flipquiz/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this flipquiz.
        $this->questions = flipquiz_report_get_significant_questions($flipquiz);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'flipquiz_grading');
        }
        ...
        if ($this->currentgroup == self::NO_GROUPS_ALLOWED) {
            ...
            $this->userssql = get_enrolled_sql($this->context,
                    array('mod/flipquiz:reviewmyattempts', 'mod/flipquiz:attempt'), $this->currentgroup);
        }
        ...
        if ($slot && $hasquestions) {
            ...
            if (!$counts || $counts->$grade == 0) {
                redirect($this->list_questions_url(), get_string('alldoneredirecting', 'flipquiz_grading'));
            }
        }

        // Start output.
        $this->print_header_and_tabs($cm, $course, $flipquiz, 'grading');
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "flipquiza.flipquiz = :mangrflipquizid AND
                flipquiza.preview = 0 AND
                flipquiza.state = :statefinished";
        $params = array('mangrflipquizid' => $this->cm->instance, 'statefinished' => flipquiz_attempt::FINISHED);
        ...
        if ($currentgroup) {
            ...
            if ($enrolleduserscount < 1) {
                $where .= ' AND flipquiza.userid = 0';
            } else {
                $usersjoin = "JOIN ({$userssql[0]}) AS enr ON flipquiza.userid = enr.id";
                $params += $userssql[1];
            }
        }

        return new qubaid_join("{flipquiz_attempts} flipquiza $usersjoin ", 'flipquiza.uniqueid', $where, $params);
    }
    ...
    protected function load_attempts_by_usage_ids($qubaids) {
        ...
        $params[] = flipquiz_attempt::FINISHED;
        $params[] = $this->flipquiz->id;

        $fields = 'flipquiza.*, u.idnumber, ';
        $fields .= get_all_user_name_fields(true, 'u');
        $attemptsbyid = $DB->get_records_sql("
                SELECT $fields
                FROM {flipquiz_attempts} flipquiza
                JOIN {user} u ON u.id = flipquiza.userid
                WHERE flipquiza.uniqueid $asql AND flipquiza.state = ? AND flipquiza.flipquiz = ?",
                $params);
    ...
    protected function base_url() {
        return new moodle_url('/mod/flipquiz/report.php',
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                    ...
                    get_string($gradestring, 'flipquiz_grading'),
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'flipquiz_grading'), 3);
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'flipquiz_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'flipquiz_grading');
        }
        ...
        if (empty($data)) {
            echo $OUTPUT->notification(get_string('nothingfound', 'flipquiz_grading'));
            return;
        }
        ...
        $table->head[] = get_string('qno', 'flipquiz_grading');
        $table->head[] = get_string('qtypeveryshort', 'question');
        $table->head[] = get_string('questionname', 'flipquiz_grading');
        $table->head[] = get_string('tograde', 'flipquiz_grading');
        $table->head[] = get_string('alreadygraded', 'flipquiz_grading');
        if ($includeauto) {
            $table->head[] = get_string('automaticallygraded', 'flipquiz_grading');
        }
        $table->head[] = get_string('total', 'flipquiz_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new flipquiz_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'flipquiz_grading', $a), 3);
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(),
                get_string('backtothelistofquestions', 'flipquiz_grading')),
        ...
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'flipquiz_grading', $a), 3);
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = flipquiz_get_review_options($this->flipquiz, $attempt, $this->context);
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                'type' => 'submit', 'class' => 'btn btn-primary', 'value' => get_string('saveandnext', 'flipquiz_grading'))),
                array('class' => 'mdl-align')) .
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'flipquiz_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'flipquiz_grading', $a);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new flipquiz_attempt($attempt, $this->flipquiz, $this->cm, $this->course);
            ...
            $events[] = \mod_flipquiz\event\question_manually_graded::create($params);
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        if ($orderby == 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON flipquiza.userid = u.id ";
