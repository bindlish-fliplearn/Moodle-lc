...
// Get the flipquiz slot.
$slot = $DB->get_record('flipquiz_slots', array('id' => $slotid));
if (!$slot || empty($slot->questioncategoryid)) {
    print_error('invalidrandomslot', 'mod_flipquiz');
}

if (!$flipquiz = $DB->get_record('flipquiz', array('id' => $slot->flipquizid))) {
    print_error('invalidflipquizid', 'flipquiz');
}

$cm = get_coursemodule_from_instance('flipquiz', $slot->flipquizid, $flipquiz->course);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/flipquiz/edit.php', array('cmid' => $cm->id));
}

$url = new moodle_url('/mod/flipquiz/editrandom.php', array('slotid' => $slotid));
$PAGE->set_url($url);
...
$mform = new mod_flipquiz\form\randomquestion_form(new moodle_url('/mod/flipquiz/editrandom.php'),
...
$currentslottags = flipquiz_retrieve_slot_tags($slot->id);
...
if ($mform->is_cancelled()) {
    ...
    // We need to save some data into the flipquiz_slots table.
    ...
    foreach ($tags as $tag) {
        if ($key = array_search(['tagid' => $tag->id, 'tagname' => $tag->name], $searchableslottags)) {
            // If found, $key would be the id field in the flipquiz_slot_tags table.
    ...
    if (!empty($recordstokeep)) {
        ...
        $DB->delete_records_select('flipquiz_slot_tags', "id $select", $params);
    } else {
...
echo $OUTPUT->header();
$heading = get_string('randomediting', 'mod_flipquiz');
echo $OUTPUT->heading_with_help($heading, 'randomquestion', 'mod_flipquiz');
