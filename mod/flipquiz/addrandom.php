...
/**
 * Fallback page of /mod/flipquiz/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $flipquiz, $pagevars) =
        question_edit_setup('editq', '/mod/flipquiz/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/flipquiz/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new flipquiz_add_random_form(new moodle_url('/mod/flipquiz/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    flipquiz_add_random_questions($flipquiz, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories, $tagids);
...
// Setup $PAGE.
$streditingflipquiz = get_string('editinga', 'moodle', get_string('modulename', 'flipquiz'));
$PAGE->navbar->add($streditingflipquiz);
$PAGE->set_title($streditingflipquiz);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$flipquizname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            print_error('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontoflipquiz', 'flipquiz', $flipquizname), 2);
$mform->display();
echo $OUTPUT->footer();
