...
/**
 * Defines the flipquiz repaginate class.
 *
 ...
 *
 * The flipquiz setting allows users to write flipquizzes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the flipquiz being manipulated. */
    private $flipquizid;
    /** @var array the flipquiz_slots for that flipquiz. */
    private $slots;

    /**
     * Constructor.
     * @param int $flipquizid the id of the flipquiz being manipulated.
     * @param stdClass[] $slots the flipquiz_slots for that flipquiz.
     */
    public function __construct($flipquizid = 0, $slots = null) {
        global $DB;
        $this->flipquizid = $flipquizid;
        if (!$this->flipquizid) {
            $this->slots = array();
        }
        if (!$slots) {
            $this->slots = $DB->get_records('flipquiz_slots', array('flipquizid' => $this->flipquizid), 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('flipquiz_slots', $nextslot, true);
    ...
    /**
     * Repaginate the rest.
     * @param stdClass[] $flipquizslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($flipquizslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$flipquizslots) {
            return null;
        }
        $newslots = array();
        foreach ($flipquizslots as $slot) {
            if ($type == self::LINK) {
