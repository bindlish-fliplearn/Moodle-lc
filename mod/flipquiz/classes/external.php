 ...
 * @since      Moodle 3.1
 */
class mod_flipquiz_external extends external_api {

    /**
     * Describes the parameters for get_flipquizzes_by_courses.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_flipquizzes_by_courses_parameters() {
    ...
    /**
     * Returns a list of flipquizzes in a provided list of courses,
     * if no list is provided all flipquizzes that the user can view will be returned.
     *
     * @param array $courseids Array of course ids
     * @return array of flipquizzes details
     * @since Moodle 3.1
     */
    public static function get_flipquizzes_by_courses($courseids = array()) {
        global $USER;

        $warnings = array();
        $returnedflipquizzes = array();

        $params = array(
            'courseids' => $courseids,
        );
        $params = self::validate_parameters(self::get_flipquizzes_by_courses_parameters(), $params);
        ...
        if (!empty($params['courseids'])) {
            ...
            // Get the flipquizzes in this course, this function checks users visibility permissions.
            // We can avoid then additional validate_context calls.
            $flipquizzes = get_all_instances_in_courses("flipquiz", $courses);
            foreach ($flipquizzes as $flipquiz) {
                ...
                $flipquiz = flipquiz_update_effective_access($flipquiz, $USER->id);

                // Entry to return.
                $flipquizdetails = array();
                // First, we return information that any user can see in the web interface.
                $flipquizdetails['id'] = $flipquiz->id;
                $flipquizdetails['coursemodule']      = $flipquiz->coursemodule;
                $flipquizdetails['course']            = $flipquiz->course;
                $flipquizdetails['name']              = external_format_string($flipquiz->name, $context->id);

                if (has_capability('mod/flipquiz:view', $context)) {
                    // Format intro.
                    list($flipquizdetails['intro'], $flipquizdetails['introformat']) = external_format_text($flipquiz->intro,
                                                                    $flipquiz->introformat, $context->id, 'mod_flipquiz', 'intro', null);

                    $flipquizdetails['introfiles'] = external_util::get_area_files($context->id, 'mod_flipquiz', 'intro', false, false);
                    ...
                    $hasfeedback = flipquiz_has_feedback($flipquiz);
                    $flipquizdetails['hasfeedback'] = (!empty($hasfeedback)) ? 1 : 0;

                    $timenow = time();
                    $flipquizobj = flipquiz::create($flipquiz->id, $USER->id);
                    $accessmanager = new flipquiz_access_manager($flipquizobj, $timenow, has_capability('mod/flipquiz:ignoretimelimits',
                                                                $context, null, false));

                    // Fields the user could see if have access to the flipquiz.
                    if (!$accessmanager->prevent_access()) {
                        $flipquizdetails['hasquestions'] = (int) $flipquizobj->has_questions();
                        $flipquizdetails['autosaveperiod'] = get_config('flipquiz', 'autosaveperiod');
                    ...
                    foreach ($viewablefields as $field) {
                        $flipquizdetails[$field] = $flipquiz->{$field};
                    }
                }
                $returnedflipquizzes[] = $flipquizdetails;
            }
        }
        $result = array();
        $result['flipquizzes'] = $returnedflipquizzes;
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_flipquizzes_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_flipquizzes_by_courses_returns() {
        return new external_single_structure(
            array(
                'flipquizzes' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            'id' => new external_value(PARAM_INT, 'Standard Moodle primary key.'),
                            'course' => new external_value(PARAM_INT, 'Foreign key reference to the course this flipquiz is part of.'),
                            ...
                            'timeopen' => new external_value(PARAM_INT, 'The time when this flipquiz opens. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timeclose' => new external_value(PARAM_INT, 'The time when this flipquiz closes. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timelimit' => new external_value(PARAM_INT, 'The time limit for flipquiz attempts, in seconds.',
                                                                VALUE_OPTIONAL),
                            ...
                            'canredoquestions' => new external_value(PARAM_INT, 'Allows students to redo any completed question
                                                                        within a flipquiz attempt.', VALUE_OPTIONAL),
                            ...
                            'attemptonlast' => new external_value(PARAM_INT, 'Whether subsequent attempts start from the answer
                                                                    ...
                                                                    VALUE_OPTIONAL),
                            'grademethod' => new external_value(PARAM_INT, 'One of the values FLIPQUIZ_GRADEHIGHEST, FLIPQUIZ_GRADEAVERAGE,
                                                                    FLIPQUIZ_ATTEMPTFIRST or FLIPQUIZ_ATTEMPTLAST.', VALUE_OPTIONAL),
                            ...
                            'reviewattempt' => new external_value(PARAM_INT, 'Whether users are allowed to review their flipquiz
                                                                    attempts at various times. This is a bit field, decoded by the
                                                                    mod_flipquiz_display_options class. It is formed by ORing together
                                                                    the constants defined there.', VALUE_OPTIONAL),
                            'reviewcorrectness' => new external_value(PARAM_INT, 'Whether users are allowed to review their flipquiz
                                                                        attempts at various times.
                                                                        A bit field, like reviewattempt.', VALUE_OPTIONAL),
                            'reviewmarks' => new external_value(PARAM_INT, 'Whether users are allowed to review their flipquiz attempts
                            ...
                            'reviewspecificfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            flipquiz attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'reviewgeneralfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            flipquiz attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'reviewrightanswer' => new external_value(PARAM_INT, 'Whether users are allowed to review their flipquiz
                                                                        attempts at various times. A bit field, like
                                                                        reviewattempt.', VALUE_OPTIONAL),
                            'reviewoverallfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their flipquiz
                                                                            attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'questionsperpage' => new external_value(PARAM_INT, 'How often to insert a page break when editing
                                                                        the flipquiz, or when shuffling the question order.',
                                                                        VALUE_OPTIONAL),
                            'navmethod' => new external_value(PARAM_ALPHA, 'Any constraints on how the user is allowed to navigate
                                                                around the flipquiz. Currently recognised values are
                            ...
                            'sumgrades' => new external_value(PARAM_FLOAT, 'The total of all the question instance maxmarks.',
                                                                VALUE_OPTIONAL),
                            'grade' => new external_value(PARAM_FLOAT, 'The total that the flipquiz overall grade is scaled to be
                                                            out of.', VALUE_OPTIONAL),
                            'timecreated' => new external_value(PARAM_INT, 'The time when the flipquiz was added to the course.',
                                                                VALUE_OPTIONAL),
                            ...
                            'password' => new external_value(PARAM_RAW, 'A password that the student must enter before starting or
                                                                continuing a flipquiz attempt.', VALUE_OPTIONAL),
                            'subnet' => new external_value(PARAM_RAW, 'Used to restrict the IP addresses from which this flipquiz can
                            ...
                            'completionattemptsexhausted' => new external_value(PARAM_INT, 'Mark flipquiz complete when the student has
                            ...
                            'allowofflineattempts' => new external_value(PARAM_INT, 'Whether to allow the flipquiz to be attempted
                                                                            offline in the mobile app', VALUE_OPTIONAL),
                            'autosaveperiod' => new external_value(PARAM_INT, 'Auto-save delay', VALUE_OPTIONAL),
                            'hasfeedback' => new external_value(PARAM_INT, 'Whether the flipquiz has any non-blank feedback text',
                                                                VALUE_OPTIONAL),
                            'hasquestions' => new external_value(PARAM_INT, 'Whether the flipquiz has questions', VALUE_OPTIONAL),
    ...
    /**
     * Utility function for validating a flipquiz.
     *
     * @param int $flipquizid flipquiz instance id
     * @return array array containing the flipquiz, course, context and course module objects
     * @since  Moodle 3.1
     */
    protected static function validate_flipquiz($flipquizid) {
        global $DB;

        // Request and permission validation.
        $flipquiz = $DB->get_record('flipquiz', array('id' => $flipquizid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($flipquiz, 'flipquiz');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        return array($flipquiz, $course, $cm, $context);
    }

    /**
     * Describes the parameters for view_flipquiz.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function view_flipquiz_parameters() {
        return new external_function_parameters (
            array(
                'flipquizid' => new external_value(PARAM_INT, 'flipquiz instance id'),
            )
        );
    }
     ...
     */
    public static function view_flipquiz($flipquizid) {
        global $DB;

        $params = self::validate_parameters(self::view_flipquiz_parameters(), array('flipquizid' => $flipquizid));
        $warnings = array();

        list($flipquiz, $course, $cm, $context) = self::validate_flipquiz($params['flipquizid']);

        // Trigger course_module_viewed event and completion.
        flipquiz_view($flipquiz, $course, $cm, $context);
    ...
    /**
     * Describes the view_flipquiz return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function view_flipquiz_returns() {
        return new external_single_structure(
            array(
    ...
    public static function get_user_attempts_parameters() {
        return new external_function_parameters (
            array(
                ...
                'status' => new external_value(PARAM_ALPHA, 'flipquiz status: all, finished or unfinished', VALUE_DEFAULT, 'finished'),
    ...
    /**
     * Return a list of attempts for the given flipquiz and user.
     *
     * @param int $flipquizid flipquiz instance id
     * @param int $userid user id
     * @param string $status flipquiz status: all, finished or unfinished
     * @param bool $includepreviews whether to include previews or not
     * @return array of warnings and the list of attempts
     * @since Moodle 3.1
     * @throws invalid_parameter_exception
     */
    public static function get_user_attempts($flipquizid, $userid = 0, $status = 'finished', $includepreviews = false) {
        global $DB, $USER;

        $warnings = array();

        $params = array(
            'flipquizid' => $flipquizid,
            'userid' => $userid,
            'status' => $status,
        ...
        if ($USER->id != $user->id) {
            require_capability('mod/flipquiz:viewreports', $context);
        }

        $attempts = flipquiz_get_user_attempts($flipquiz->id, $user->id, $params['status'], $params['includepreviews']);
    ...
    private static function attempt_structure() {
        return new external_single_structure(
            array(
                ...
                'flipquiz' => new external_value(PARAM_INT, 'Foreign key reference to the flipquiz that was attempted.',
                                                VALUE_OPTIONAL),
                'userid' => new external_value(PARAM_INT, 'Foreign key reference to the user whose attempt this is.',
                                                VALUE_OPTIONAL),
                'attempt' => new external_value(PARAM_INT, 'Sequentially numbers this students attempts at this flipquiz.',
                                                VALUE_OPTIONAL),
                'uniqueid' => new external_value(PARAM_INT, 'Foreign key reference to the question_usage that holds the
                                                    details of the the question_attempts that make up this flipquiz
                ...
                'timecheckstate' => new external_value(PARAM_INT, 'Next time flipquiz cron should check attempt for
    ...
    /**
     * Get the best current grade for the given user on a flipquiz.
     *
     * @param int $flipquizid flipquiz instance id
     * @param int $userid user id
     ...
     * @since Moodle 3.1
     */
    public static function get_user_best_grade($flipquizid, $userid = 0) {
        global $DB, $USER;
        ...
        $result = array();
        $grade = flipquiz_get_best_grade($flipquiz, $user->id);
    ...
    public static function get_user_best_grade_returns() {
        return new external_single_structure(
            array(
                'hasgrade' => new external_value(PARAM_BOOL, 'Whether the user has a grade on the given flipquiz.'),
     ...
     * @since Moodle 3.1
     */
    public static function get_combined_review_options($flipquizid, $userid = 0) {
        global $DB, $USER;
        ...
        $attempts = flipquiz_get_user_attempts($flipquiz->id, $user->id, 'all', true);

        $result = array();
        $result['someoptions'] = [];
        $result['alloptions'] = [];

        list($someoptions, $alloptions) = flipquiz_get_combined_reviewoptions($flipquiz, $attempts);
    ...
    /**
     * Starts a new attempt at a flipquiz.
     *
     * @param int $flipquizid flipquiz instance id
     ...
     */
    public static function start_attempt($flipquizid, $preflightdata = array(), $forcenew = false) {
        global $DB, $USER;
        ...
        if (!$flipquizobj->has_questions()) {
            throw new moodle_flipquiz_exception($flipquizobj, 'noquestionsfound');
        }
        ...
        list($currentattemptid, $attemptnumber, $lastattempt, $messages, $page) =
            flipquiz_validate_new_attempt($flipquizobj, $accessmanager, $forcenew, -1, false);
        ...
        if (!$flipquizobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new moodle_flipquiz_exception($flipquizobj, array_shift($errors));
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == flipquiz_attempt::OVERDUE) {
                    throw new moodle_flipquiz_exception($flipquizobj, 'stateoverdue');
                } else {
                    throw new moodle_flipquiz_exception($flipquizobj, 'attemptstillinprogress');
                }
            }
            $offlineattempt = WS_SERVER ? true : false;
            $attempt = flipquiz_prepare_and_start_new_attempt($flipquizobj, $attemptnumber, $lastattempt, $offlineattempt);
        }
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        $attemptobj = flipquiz_attempt::create($params['attemptid']);
        ...
        if ($checkaccessrules) {
            ...
            if (!$ispreviewuser && $messages) {
                throw new moodle_flipquiz_exception($attemptobj->get_flipquizobj(), 'attempterror');
            }
        }

        // Attempt closed?.
        if ($attemptobj->is_finished()) {
            throw new moodle_flipquiz_exception($attemptobj->get_flipquizobj(), 'attemptalreadyclosed');
        } else if ($failifoverdue && $attemptobj->get_state() == flipquiz_attempt::OVERDUE) {
            throw new moodle_flipquiz_exception($attemptobj->get_flipquizobj(), 'stateoverdue');
        }

        // User submitted data (like the flipquiz password).
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new moodle_flipquiz_exception($attemptobj->get_flipquizobj(), array_shift($errors));
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new moodle_flipquiz_exception($attemptobj->get_flipquizobj(), 'Invalid page number');
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new moodle_flipquiz_exception($attemptobj->get_flipquizobj(), 'Out of sequence access');
            }
     ...
     *
     * @return external_single_structure the question data. Some fields may not be returned depending on the flipquiz display settings.
     * @since  Moodle 3.1
    ...
    private static function question_structure() {
        return new external_single_structure(
            array(
                ...
                'page' => new external_value(PARAM_INT, 'page of the flipquiz this question appears on'),
                ...
                'number' => new external_value(PARAM_INT, 'question ordering number in the flipquiz', VALUE_OPTIONAL),
                'state' => new external_value(PARAM_ALPHA, 'the state where the question is in.
                    It will not be returned if the user cannot see it due to the flipquiz display correctness settings.',
                    VALUE_OPTIONAL),
            ...
            'The question data. Some fields may not be returned depending on the flipquiz display settings.'
        );
    }
     ...
     *
     * @param  flipquiz_attempt  $attemptobj  the flipquiz attempt object
     * @param  bool  $review  whether if we are in review mode or not
     * @param  mixed  $page  string 'all' or integer page number
     * @return array array of questions including data
     */
    private static function get_attempt_questions_data(flipquiz_attempt $attemptobj, $review, $page = 'all') {
        global $PAGE;

        $questions = array();
        $contextid = $attemptobj->get_flipquizobj()->get_context()->id;
        $displayoptions = $attemptobj->get_display_options($review);
        $renderer = $PAGE->get_renderer('mod_flipquiz');
    ...
    /**
     * Returns information for the given attempt page for a flipquiz attempt in progress.
     *
     ...
     * @since Moodle 3.1
     * @throws moodle_flipquiz_exceptions
     */
        ...
        return new external_single_structure(
            array(
                ...
                'messages' => new external_multiple_structure(
                    ...
                    'access messages, will only be returned for users with mod/flipquiz:preview capability,
    ...
    /**
     * Returns a summary of a flipquiz attempt before it is submitted.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Processes save requests during the flipquiz. This function is intended for the flipquiz auto-save feature.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Process responses during an attempt at a flipquiz and also deals with attempts finishing.
     *
     * @param int $attemptid attempt id
     * @param array $data the data to be saved
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new moodle_flipquiz_exception($attemptobj->get_flipquizobj(), 'attemptclosed');
            } else if (!$displayoptions->attempt) {
                throw new moodle_exception($attemptobj->cannot_review_message());
            }
        } else if (!$attemptobj->is_review_allowed()) {
            throw new moodle_flipquiz_exception($attemptobj->get_flipquizobj(), 'noreviewattempt');
        }
    ...
    public static function get_attempt_review($attemptid, $page = -1) {
        ...
        foreach ($summarydata as $key => $data) {
            ...
            $result['additionaldata'][] = array(
                'id' => $key,
                'title' => $data['title'], $attemptobj->get_flipquizobj()->get_context()->id,
                'content' => $data['content'],
            );
        }

        // Feedback if there is any, and the user is allowed to see it now.
        $grade = flipquiz_rescale_grade($attemptobj->get_attempt()->sumgrades, $attemptobj->get_flipquiz(), false);

        $feedback = $attemptobj->get_overall_feedback($grade);
        if ($displayoptions->overallfeedback && $feedback) {
            $result['additionaldata'][] = array(
                'id' => 'feedback',
                'title' => get_string('feedback', 'flipquiz'),
                'content' => $feedback,
            );
        }
    ...
    public static function get_attempt_review_returns() {
        return new external_single_structure(
            array(
                'grade' => new external_value(PARAM_RAW, 'grade for the flipquiz (or empty or "notyetgraded")'),
                'attempt' => self::attempt_structure(),
     ...
     * @since Moodle 3.1
     */
    public static function get_flipquiz_feedback_for_grade_parameters() {
        return new external_function_parameters (
            array(
                'flipquizid' => new external_value(PARAM_INT, 'flipquiz instance id'),
    ...
    /**
     * Get the feedback text that should be show to a student who got the given grade in the given flipquiz.
     *
     * @param int $flipquizid flipquiz instance id
     ...
     * @throws moodle_exception
     */
    public static function get_flipquiz_feedback_for_grade($flipquizid, $grade) {
        global $DB;
        ...
        $params = self::validate_parameters(self::get_flipquiz_feedback_for_grade_parameters(), $params);
        $warnings = array();
        ...
        $feedback = flipquiz_feedback_record_for_grade($params['grade'], $flipquiz);
        if (!empty($feedback->feedbacktext)) {
            list($text, $format) = external_format_text($feedback->feedbacktext, $feedback->feedbacktextformat, $context->id,
                                                        'mod_flipquiz', 'feedback', $feedback->id);
            $result['feedbacktext'] = $text;
            $result['feedbacktextformat'] = $format;
            $feedbackinlinefiles = external_util::get_area_files($context->id, 'mod_flipquiz', 'feedback', $feedback->id);
    ...
    /**
     * Describes the get_flipquiz_feedback_for_grade return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_flipquiz_feedback_for_grade_returns() {
        return new external_single_structure(
            array(
    ...
    /**
     * Describes the parameters for get_flipquiz_access_information.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_flipquiz_access_information_parameters() {
        return new external_function_parameters (
            array(
                'flipquizid' => new external_value(PARAM_INT, 'flipquiz instance id')
            )
    ...
    /**
     * Return access information for a given flipquiz.
     *
     * @param int $flipquizid flipquiz instance id
     ...
     * @throws  moodle_flipquiz_exception
     */
    public static function get_flipquiz_access_information($flipquizid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_flipquiz_access_information_parameters(), $params);

        list($flipquiz, $course, $cm, $context) = self::validate_flipquiz($params['flipquizid']);

        $result = array();
        // Capabilities first.
        $result['canattempt'] = has_capability('mod/flipquiz:attempt', $context);;
        $result['canmanage'] = has_capability('mod/flipquiz:manage', $context);;
        $result['canpreview'] = has_capability('mod/flipquiz:preview', $context);;
        $result['canreviewmyattempts'] = has_capability('mod/flipquiz:reviewmyattempts', $context);;
        $result['canviewreports'] = has_capability('mod/flipquiz:viewreports', $context);;

        // Access manager now.
        $flipquizobj = flipquiz::create($cm->instance, $USER->id);
        $ignoretimelimits = has_capability('mod/flipquiz:ignoretimelimits', $context, null, false);
        $timenow = time();
        $accessmanager = new flipquiz_access_manager($flipquizobj, $timenow, $ignoretimelimits);
    ...
    /**
     * Describes the get_flipquiz_access_information return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_flipquiz_access_information_returns() {
        return new external_single_structure(
            array(
                'canattempt' => new external_value(PARAM_BOOL, 'Whether the user can do the flipquiz or not.'),
                'canmanage' => new external_value(PARAM_BOOL, 'Whether the user can edit the flipquiz settings or not.'),
                'canpreview' => new external_value(PARAM_BOOL, 'Whether the user can preview the flipquiz or not.'),
                'canreviewmyattempts' => new external_value(PARAM_BOOL, 'Whether the users can review their previous attempts
                                                                or not.'),
                'canviewreports' => new external_value(PARAM_BOOL, 'Whether the user can view the flipquiz reports or not.'),
    ...
    /**
     * Return access information for a given attempt in a flipquiz.
     *
     * @param int $flipquizid flipquiz instance id
     ...
     * @throws  moodle_flipquiz_exception
     */
    public static function get_attempt_access_information($flipquizid, $attemptid = 0) {
        global $DB, $USER;
        ...
        $attempts = flipquiz_get_user_attempts($flipquiz->id, $USER->id, 'finished', true);
        $lastfinishedattempt = end($attempts);
        if ($unfinishedattempt = flipquiz_get_user_attempt_unfinished($flipquiz->id, $USER->id)) {
            $attempts[] = $unfinishedattempt;

            // Check if the attempt is now overdue. In that case the state will change.
            $flipquizobj->create_attempt_object($unfinishedattempt)->handle_if_time_expired(time(), false);

            if ($unfinishedattempt->state != flipquiz_attempt::IN_PROGRESS and $unfinishedattempt->state != flipquiz_attempt::OVERDUE) {
    ...
    /**
     * Describes the parameters for get_flipquiz_required_qtypes.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_flipquiz_required_qtypes_parameters() {
        return new external_function_parameters (
            array(
                'flipquizid' => new external_value(PARAM_INT, 'flipquiz instance id')
            )
    ...
    /**
     * Return the potential question types that would be required for a given flipquiz.
     ...
     * @throws  moodle_flipquiz_exception
     */
    public static function get_flipquiz_required_qtypes($flipquizid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_flipquiz_required_qtypes_parameters(), $params);
        ...
        // Question types used.
        $result = array();
        $result['questiontypes'] = $flipquizobj->get_all_question_types_used(true);
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_flipquiz_required_qtypes return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_flipquiz_required_qtypes_returns() {
        return new external_single_structure(
            array(
                'questiontypes' => new external_multiple_structure(
                                    new external_value(PARAM_PLUGIN, 'question type'), 'list of question types used in the flipquiz'),
                'warnings' => new external_warnings(),
            )
        );
    }
