...
/**
 * Defines the \mod_flipquiz\structure class.
 *
...
namespace mod_flipquiz;
defined('MOODLE_INTERNAL') || die();

/**
 * Quiz structure class.
 *
 * The structure of the flipquiz. That is, which questions it is built up
 * from. This is used on the Edit flipquiz page (edit.php) and also when
 * starting an attempt at the flipquiz (startattempt.php). Once an attempt
 ...
 */
class structure {
    /** @var \flipquiz the flipquiz this is the structure of. */
    protected $flipquizobj = null;

    /**
     * @var \stdClass[] the questions in this flipquiz. Contains the row from the questions
     * table, with the data from the flipquiz_slots table added, and also question_categories.contextid.
     */
    protected $questions = array();

    /** @var \stdClass[] flipquiz_slots.id => the flipquiz_slots rows for this flipquiz, agumented by sectionid. */
    protected $slots = array();

    /** @var \stdClass[] flipquiz_slots.slot => the flipquiz_slots rows for this flipquiz, agumented by sectionid. */
    protected $slotsinorder = array();

    /**
     * @var \stdClass[] currently a dummy. Holds data that will match the
     * flipquiz_sections, once it exists.
     */
    ...
    /**
     * Create an instance of this class representing an empty flipquiz.
     * @return structure
     */
    ...
    /**
     * Create an instance of this class representing the structure of a given flipquiz.
     * @param \flipquiz $flipquizobj the flipquiz.
     * @return structure
     */
    public static function create_for_flipquiz($flipquizobj) {
        $structure = self::create();
        $structure->flipquizobj = $flipquizobj;
        $structure->populate_structure($flipquizobj->get_flipquiz());
        return $structure;
    }

    /**
     * Whether there are any questions in the flipquiz.
     * @return bool true if there is at least one question in the flipquiz.
     */
    ...
    /**
     * Get the number of questions in the flipquiz.
     * @return int the number of questions in the flipquiz.
     */
     ...
     * question_category.contextid, and the flipquiz_slots data for the question in this flipquiz.
     */
     ...
     * questions cannot be depended upon because of flipquiz options.
     * @param int $slotnumber the index of the slot in question.
    ...
    public function can_finish_during_the_attempt($slotnumber) {
        if ($this->flipquizobj->get_navigation_method() == FLIPQUIZ_NAVMETHOD_SEQ) {
            return false;
        }
        ...
        if (in_array($this->get_question_type_for_slot($slotnumber), array('random', 'missingtype'))) {
            return \question_engine::can_questions_finish_during_the_attempt(
                    $this->flipquizobj->get_flipquiz()->preferredbehaviour);
        }
        ...
        try {
            $quba = \question_engine::make_questions_usage_by_activity('mod_flipquiz', $this->flipquizobj->get_context());
            $tempslot = $quba->add_question(\question_bank::load_question(
                    $this->slotsinorder[$slotnumber]->questionid));
            $quba->set_preferred_behaviour($this->flipquizobj->get_flipquiz()->preferredbehaviour);
    ...
    /**
     * Get the course id that the flipquiz belongs to.
     * @return int the course.id for the flipquiz.
     */
    ...
    /**
     * Get the course module id of the flipquiz.
     * @return int the course_modules.id for the flipquiz.
     */
    ...
    /**
     * Get id of the flipquiz.
     * @return int the flipquiz.id for the flipquiz.
     */
     ...
     * @return \stdClass the flipquiz settings row from the database.
     */
     ...
     * @return bool whether this flipquiz can be repaginated.
     */
     ...
     * @return bool whether the flipquiz can be edited.
     */
    public function can_be_edited() {
        if ($this->canbeedited === null) {
            $this->canbeedited = !flipquiz_has_attempts($this->flipquizobj->get_flipquizid());
        }
        return $this->canbeedited;
    }

    /**
     * This flipquiz can only be edited if they have not been attempted.
     * Throw an exception if this is not the case.
     */
    public function check_can_be_edited() {
        if (!$this->can_be_edited()) {
            $reportlink = flipquiz_attempt_summary_link_to_reports($this->get_flipquiz(),
                    $this->flipquizobj->get_cm(), $this->flipquizobj->get_context());
            throw new \moodle_exception('cannoteditafterattempts', 'flipquiz',
                    new \moodle_url('/mod/flipquiz/edit.php', array('cmid' => $this->get_cmid())), $reportlink);
        }
    }

    /**
     * How many questions are allowed per page in the flipquiz.
     * This setting controls how frequently extra page-breaks should be inserted
     * automatically when questions are added to the flipquiz.
     * @return int the number of questions that should be on each page of the
     * flipquiz by default.
     */
    public function get_questions_per_page() {
        return $this->flipquizobj->get_flipquiz()->questionsperpage;
    }

    /**
     * Get flipquiz slots.
     * @return \stdClass[] the slots in this flipquiz.
     */
    ...
    /**
     * Is this slot the last one in the flipquiz?
     * @param int $slotnumber the index of the slot in question.
     * @return bool whether this slot the last one in the flipquiz.
     */
    public function is_last_slot_in_flipquiz($slotnumber) {
        end($this->slotsinorder);
        return $slotnumber == key($this->slotsinorder);
    }

    /**
     * Is this the first section in the flipquiz?
     * @param \stdClass $section the flipquiz_sections row.
     * @return bool whether this is first section in the flipquiz.
     */
    ...
    /**
     * Is this the last section in the flipquiz?
     * @param \stdClass $section the flipquiz_sections row.
     * @return bool whether this is first section in the flipquiz.
     */
    ...
    /**
     * Get the final slot in the flipquiz.
     * @return \stdClass the flipquiz_slots for for the final slot in the flipquiz.
     */
     ...
     * @return \stdClass the requested flipquiz_slots row.
     */
    ...
    /**
     * Get all the slots in a section of the flipquiz.
    ...
    /**
     * Get all the sections of the flipquiz.
     * @return \stdClass[] the sections in this flipquiz.
     */
    ...
    /**
     * Get the overall flipquiz grade formatted for display.
     * @return string the maximum grade for this flipquiz.
     */
    public function formatted_flipquiz_grade() {
        return flipquiz_format_grade($this->get_flipquiz(), $this->get_flipquiz()->grade);
    }
    ...
    public function formatted_question_grade($slotnumber) {
        return flipquiz_format_question_grade($this->get_flipquiz(), $this->slotsinorder[$slotnumber]->maxmark);
    }

    /**
     * Get the number of decimal places for displyaing overall flipquiz grades or marks.
     * @return int the number of decimal places.
     */
    public function get_decimal_places_for_grades() {
        return $this->get_flipquiz()->decimalpoints;
    }
    ...
    public function get_decimal_places_for_question_marks() {
        return flipquiz_get_grade_format($this->get_flipquiz());
    }
    ...
    public function get_edit_page_warnings() {
        ...
        if (flipquiz_has_attempts($this->flipquizobj->get_flipquizid())) {
            $reviewlink = flipquiz_attempt_summary_link_to_reports($this->flipquizobj->get_flipquiz(),
                    $this->flipquizobj->get_cm(), $this->flipquizobj->get_context());
            $warnings[] = get_string('cannoteditafterattempts', 'flipquiz', $reviewlink);
        }
    ...
    /**
     * Get the date information about the current state of the flipquiz.
    ...
    public function get_dates_summary() {
        $timenow = time();
        $flipquiz = $this->flipquizobj->get_flipquiz();

        // Exact open and close dates for the tool-tip.
        $dates = array();
        if ($flipquiz->timeopen > 0) {
            if ($timenow > $flipquiz->timeopen) {
                $dates[] = get_string('flipquizopenedon', 'flipquiz', userdate($flipquiz->timeopen));
            } else {
                $dates[] = get_string('flipquizwillopen', 'flipquiz', userdate($flipquiz->timeopen));
            }
        }
        if ($flipquiz->timeclose > 0) {
            if ($timenow > $flipquiz->timeclose) {
                $dates[] = get_string('flipquizclosed', 'flipquiz', userdate($flipquiz->timeclose));
            } else {
                $dates[] = get_string('flipquizcloseson', 'flipquiz', userdate($flipquiz->timeclose));
            }
        }
        if (empty($dates)) {
            $dates[] = get_string('alwaysavailable', 'flipquiz');
        }
        $explanation = implode(', ', $dates);

        // Brief summary on the page.
        if ($timenow < $flipquiz->timeopen) {
            $currentstatus = get_string('flipquizisclosedwillopen', 'flipquiz',
                    userdate($flipquiz->timeopen, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($flipquiz->timeclose && $timenow <= $flipquiz->timeclose) {
            $currentstatus = get_string('flipquizisopenwillclose', 'flipquiz',
                    userdate($flipquiz->timeclose, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($flipquiz->timeclose && $timenow > $flipquiz->timeclose) {
            $currentstatus = get_string('flipquizisclosed', 'flipquiz');
        } else {
            $currentstatus = get_string('flipquizisopen', 'flipquiz');
        }
    ...
    /**
     * Set up this class with the structure for a given flipquiz.
     * @param \stdClass $flipquiz the flipquiz settings.
     */
    public function populate_structure($flipquiz) {
        global $DB;
        ...
        // Get flipquiz sections in ascending order of the firstslot.
        $this->sections = $DB->get_records('flipquiz_sections', array('flipquizid' => $flipquiz->id), 'firstslot ASC');
     ...
     * @param \stdClass[] $slots the data about the slots and questions in the flipquiz.
    ...
    protected function populate_missing_questions($slots) {
        ...
        foreach ($slots as $slot) {
            if ($slot->qtype === null) {
                ...
                $slot->name = get_string('missingquestion', 'flipquiz');
                $slot->slot = $slot->slot;
                $slot->maxmark = 0;
    ...
    protected function populate_question_numbers() {
        ...
        foreach ($this->slots as $slot) {
            if ($this->questions[$slot->questionid]->length == 0) {
                $slot->displayednumber = get_string('infoshort', 'flipquiz');
            } else {
    ...
    public function move_slot($idmove, $idmoveafter, $page) {
        ...
        if ($followingslotnumber == $movingslotnumber && !$this->is_last_slot_in_flipquiz($followingslotnumber)) {
        ...
        } else if (!$this->is_last_slot_in_flipquiz($moveafterslotnumber) &&
        ...
        if ($moveafterslotnumber > $movingslotnumber) {
            ...
            if ($this->is_last_slot_in_flipquiz($moveafterslotnumber) ||
        ...
        if ($slotreorder) {
            update_field_with_unique_index('flipquiz_slots', 'slot', $slotreorder,
                    array('flipquizid' => $this->get_flipquizid()));
        }

        // Page has changed. Record it.
        if ($movingslot->page != $page) {
            $DB->set_field('flipquiz_slots', 'page', $page,
                    array('id' => $movingslot->id));
        }

        // Update section fist slots.
        flipquiz_update_section_firstslots($this->get_flipquizid(), $headingmovedirection,
        ...
        $emptypages = $DB->get_fieldset_sql("
                SELECT DISTINCT page - 1
                 ...
                 WHERE flipquizid = ?
                   AND page > 1
                   AND NOT EXISTS (SELECT 1 FROM {flipquiz_slots} WHERE flipquizid = ? AND page = slot.page - 1)
              ORDER BY page - 1 DESC
                ", array($this->get_flipquizid(), $this->get_flipquizid()));

        foreach ($emptypages as $page) {
            $DB->execute("
                    UPDATE {flipquiz_slots}
                       SET page = page - 1
                     WHERE flipquizid = ?
                       AND page > ?
                    ", array($this->get_flipquizid(), $page));
        }
    ...
    /**
     * Refresh page numbering of flipquiz slots.
    ...
    public function refresh_page_numbers($slots = array()) {
        ...
        if (!count($slots)) {
            $slots = $DB->get_records('flipquiz_slots', array('flipquizid' => $this->get_flipquizid()), 'slot, page');
        }
    ...
    /**
     * Refresh page numbering of flipquiz slots and save to the database.
     * @param \stdClass $flipquiz the flipquiz object.
     * @return \stdClass[] array of slot objects.
     */
    public function refresh_page_numbers_and_update_db() {
        ...
        foreach ($slots as $slot) {
            $DB->set_field('flipquiz_slots', 'page', $slot->page,
                    array('id' => $slot->id));
        }
    ...
    public function remove_slot($slotnumber) {
        ...
        $slot = $DB->get_record('flipquiz_slots', array('flipquizid' => $this->get_flipquizid(), 'slot' => $slotnumber));
        if (!$slot) {
            return;
        }
        $maxslot = $DB->get_field_sql('SELECT MAX(slot) FROM {flipquiz_slots} WHERE flipquizid = ?', array($this->get_flipquizid()));

        $trans = $DB->start_delegated_transaction();
        $DB->delete_records('flipquiz_slot_tags', array('slotid' => $slot->id));
        $DB->delete_records('flipquiz_slots', array('id' => $slot->id));
        for ($i = $slot->slot + 1; $i <= $maxslot; $i++) {
            $DB->set_field('flipquiz_slots', 'slot', $i - 1,
                    array('flipquizid' => $this->get_flipquizid(), 'slot' => $i));
        }
        ...
        flipquiz_update_section_firstslots($this->get_flipquizid(), -1, $slotnumber);
     ...
     *
     * Saves changes to the question grades in the flipquiz_slots table and any
     * corresponding question_attempts.
     * It does not update 'sumgrades' in the flipquiz table.
     *
     * @param \stdClass $slot row from the flipquiz_slots table.
    ...
    public function update_slot_maxmark($slot, $maxmark) {
        ...
        $DB->update_record('flipquiz_slots', $slot);
        \question_engine::set_max_mark_in_attempts(new \qubaids_for_flipquiz($slot->flipquizid),
                $slot->slot, $maxmark);
        $trans->allow_commit();
    ...
    public function update_question_dependency($slotid, $requireprevious) {
        global $DB;
        $DB->set_field('flipquiz_slots', 'requireprevious', $requireprevious, array('id' => $slotid));
    }

    /**
     * Add/Remove a pagebreak.
     *
     * Saves changes to the slot page relationship in the flipquiz_slots table and reorders the paging
    ...
    public function update_page_break($slotid, $type) {
        ...
        $flipquizslots = $DB->get_records('flipquiz_slots', array('flipquizid' => $this->get_flipquizid()), 'slot');
        $repaginate = new \mod_flipquiz\repaginate($this->get_flipquizid(), $flipquizslots);
        $repaginate->repaginate_slots($flipquizslots[$slotid]->slot, $type);
    ...
    public function add_section_heading($pagenumber, $heading = null) {
        ...
        if ($heading !== null) {
            ...
            $section->heading = get_string('newsectionheading', 'flipquiz');
        }
        $section->flipquizid = $this->get_flipquizid();
        $slotsonpage = $DB->get_records('flipquiz_slots', array('flipquizid' => $this->get_flipquizid(), 'page' => $pagenumber), 'slot DESC');
        $section->firstslot = end($slotsonpage)->slot;
        $section->shufflequestions = 0;
        return $DB->insert_record('flipquiz_sections', $section);
    }
    ...
    public function set_section_heading($id, $newheading) {
        global $DB;
        $section = $DB->get_record('flipquiz_sections', array('id' => $id), '*', MUST_EXIST);
        $section->heading = $newheading;
        $DB->update_record('flipquiz_sections', $section);
    }
    ...
    public function remove_section_heading($sectionid) {
        global $DB;
        $section = $DB->get_record('flipquiz_sections', array('id' => $sectionid), '*', MUST_EXIST);
        if ($section->firstslot == 1) {
            throw new \coding_exception('Cannot remove the first section in a flipquiz.');
        }
        $DB->delete_records('flipquiz_sections', array('id' => $sectionid));
    }
    ...
    protected function populate_slot_tags() {
        ...
        $this->slottags = flipquiz_retrieve_tags_for_slot_ids($slotids);
    }
