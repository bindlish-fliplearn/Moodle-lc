...
/**
 * This page handles editing and creation of flipquiz overrides
 *
...
if ($overrideid) {
    ...
    if (! $override = $DB->get_record('flipquiz_overrides', array('id' => $overrideid))) {
        print_error('invalidoverrideid', 'flipquiz');
    }
    if (! $flipquiz = $DB->get_record('flipquiz', array('id' => $override->flipquiz))) {
...
$url = new moodle_url('/mod/flipquiz/overrideedit.php');
if ($action) {
...
// Merge flipquiz defaults with data.
$keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
foreach ($keys as $key) {
    if (!isset($data->{$key}) || $reset) {
        $data->{$key} = $flipquiz->{$key};
    }
}
...
$overridelisturl = new moodle_url('/mod/flipquiz/overrides.php', array('cmid'=>$cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new flipquiz_override_form($url, $cm, $flipquiz, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->flipquiz = $flipquiz->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if ($fromform->{$key} == $flipquiz->{$key}) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('flipquiz_overrides', $conditions)) {
            ...
            // Set the course module id before calling flipquiz_delete_override().
            $flipquiz->cmid = $cm->id;
            flipquiz_delete_override($flipquiz, $oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('flipquiz_overrides', $fromform);
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('flipquiz_overrides', $fromform);
    ...
    if ($groupmode) {
        ...
        flipquiz_update_events($flipquiz, $fromform);
    }
...
// Print the form.
$pagetitle = get_string('editoverride', 'flipquiz');
