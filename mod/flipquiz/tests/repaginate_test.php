...
/**
 * Unit tests for the {@link \mod_flipquiz\repaginate} class.
...
require_once($CFG->dirroot . '/mod/flipquiz/classes/repaginate.php');


/**
 * Testable subclass, giving access to the protected methods of {@link \mod_flipquiz\repaginate}
 * @copyright 2014 The Open Univsersity
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_flipquiz_repaginate_testable extends \mod_flipquiz\repaginate {

    public function __construct($flipquizid = 0, $slots = null) {
        return parent::__construct($flipquizid, $slots);
    }
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_flipquiz_repaginate_test extends advanced_testcase {

    /** @var array stores the slots. */
    private $flipquizslots;
    /** @var mod_flipquiz_repaginate_testable the object being tested. */
    private $repaginate = null;

    public function setUp() {
        $this->set_flipquiz_slots($this->get_flipquiz_object()->get_slots());
        $this->repaginate = new mod_flipquiz_repaginate_testable(0, $this->flipquizslots);
    }
    ...
    /**
     * Create a flipquiz, add five questions to the flipquiz
     * which are all on one page and return the flipquiz object.
     */
    private function get_flipquiz_object() {
        global $SITE;
        ...
        $flipquiz = $flipquizgenerator->create_instance(array(
                'course' => $SITE->id, 'questionsperpage' => 0, 'grade' => 100.0, 'sumgrades' => 2));
        $cm = get_coursemodule_from_instance('flipquiz', $flipquiz->id, $SITE->id);
        ...
        flipquiz_add_flipquiz_question($shortanswer->id, $flipquiz);
        flipquiz_add_flipquiz_question($numerical->id, $flipquiz);
        flipquiz_add_flipquiz_question($essay->id, $flipquiz);
        flipquiz_add_flipquiz_question($truefalse->id, $flipquiz);
        flipquiz_add_flipquiz_question($match->id, $flipquiz);

        // Return the flipquiz object.
        $flipquizobj = new flipquiz($flipquiz, $cm, $SITE);
        return \mod_flipquiz\structure::create_for_flipquiz($flipquizobj);
    }

    /**
     * Set the flipquiz slots
     * @param string $slots
     */
    private function set_flipquiz_slots($slots = null) {
        if (!$slots) {
            $this->flipquizslots = $this->get_flipquiz_object()->get_slots();
        } else {
            $this->flipquizslots = $slots;
        }
    }
    ...
    public function test_get_this_slot() {
        $this->set_flipquiz_slots();
        $actual = array();
        $expected = $this->repaginate->get_slots_by_slot_number();
        $this->assertEquals($expected, $actual);

        $slotsbyno = $this->repaginate->get_slots_by_slot_number($this->flipquizslots);
        $slotnumber = 5;
        $thisslot = $this->repaginate->get_this_slot($this->flipquizslots, $slotnumber);
    ...
    public function test_get_slots_by_slotnumber() {
        ...
        foreach ($this->flipquizslots as $slot) {
            $expected[$slot->slot] = $slot;
        }
        $actual = $this->repaginate->get_slots_by_slot_number($this->flipquizslots);
        $this->assertEquals($expected, $actual);
    }
    ...
    public function test_get_slots_by_slotid() {
        ...
        $this->assertEquals($this->flipquizslots, $actual);
    }
    ...
    public function test_repaginate_n_questions_per_page() {
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->flipquizslots, 2);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->flipquizslots, 3);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->flipquizslots, 5);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->flipquizslots, 10);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->flipquizslots, 1);
        $this->assertEquals($expected, $actual);
    }

    public function test_repaginate_this_slot() {
        $this->set_flipquiz_slots();
        $slotsbyslotno = $this->repaginate->get_slots_by_slot_number($this->flipquizslots);
        $slotnumber = 3;
        $newpagenumber = 2;
    ...
    public function test_repaginate_the_rest() {
        $this->set_flipquiz_slots();
        $slotfrom = 1;
        $type = \mod_flipquiz\repaginate::LINK;
        $expected = array();
        foreach ($this->flipquizslots as $slot) {
        ...
        $actual = $this->repaginate->repaginate_the_rest($this->flipquizslots, $slotfrom, $type, false);
        $this->assertEquals($expected, $actual);

        $slotfrom = 2;
        $newslots = array();
        foreach ($this->flipquizslots as $s) {
            if ($s->slot === $slotfrom) {
                $s->page = $s->page - 1;
            }
            $newslots[$s->id] = $s;
        ...
        $type = \mod_flipquiz\repaginate::UNLINK;
        $expected = array();
        foreach ($this->flipquizslots as $slot) {
