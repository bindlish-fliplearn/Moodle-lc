 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_flipquiz_structure_testcase extends advanced_testcase {

    /**
     * Create a course with an empty flipquiz.
     * @return array with three elements flipquiz, cm and course.
     */
    protected function prepare_flipquiz_data() {
        ...
        return array($flipquiz, $cm, $course);
    }

    /**
     * Creat a test flipquiz.
     *
     ...
     * @return flipquiz the created flipquiz.
     */
    protected function create_test_flipquiz($layout) {
        list($flipquiz, $cm, $course) = $this->prepare_flipquiz_data();
        ...
        foreach ($layout as $item) {
            if (is_string($item)) {
                ...
                flipquiz_add_flipquiz_question($q->id, $flipquiz, $page);
                $lastpage = $page;
            }
        }

        $flipquizobj = new flipquiz($flipquiz, $cm, $course);
        $structure = \mod_flipquiz\structure::create_for_flipquiz($flipquizobj);
        if (isset($headings[1])) {
     ...
     * @param array $expectedlayout as for $layout in {@link create_test_flipquiz()}.
     * @param \mod_flipquiz\structure $structure the structure to test.
     */
    protected function assert_flipquiz_layout($expectedlayout, \mod_flipquiz\structure $structure) {
        $sections = $structure->get_sections();
        ...
        foreach ($expectedlayout as $item) {
            if (is_string($item)) {
                ...
                if ($slot > 1 && $section->heading == '' && $section->firstslot == 1) {
                    // The array $expectedlayout did not contain default first flipquiz section, so skip over it.
                    $section = array_shift($sections);
                }
        ...
        if ($slot - 1 != count($structure->get_slots())) {
            $this->fail('The flipquiz contains more slots than expected.');
        }

        if (!empty($sections)) {
            $section = array_shift($sections);
            if ($section->heading != '' || $section->firstslot != 1) {
                $this->fail('Unexpected section (' . $section->heading .') found in the flipquiz.');
            }
        }
    }
     ...
     * used by create_test_flipquiz as assert_flipquiz_layout.
    ...
    public function test_get_flipquiz_slots() {
        $flipquizobj = $this->create_test_flipquiz(array(
                array('TF1', 1, 'truefalse'),
    ...
    public function test_flipquiz_has_one_section_by_default() {
        $flipquizobj = $this->create_test_flipquiz(array(
                array('TF1', 1, 'truefalse'),
            ));
        $structure = \mod_flipquiz\structure::create_for_flipquiz($flipquizobj);
    ...
    public function test_remove_section_heading() {
        ...
        $structure = \mod_flipquiz\structure::create_for_flipquiz($flipquizobj);
        $this->assert_flipquiz_layout(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
                array('TF2', 2, 'truefalse'),
            ), $structure);
    ...
    public function test_flipquiz_remove_slot() {
        $flipquizobj = $this->create_test_flipquiz(array(
                array('TF1', 1, 'truefalse'),
                array('TF2', 1, 'truefalse'),
                'Heading 2',
    ...
    public function test_flipquiz_removing_a_random_question_deletes_the_question() {
        global $DB;
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category();
        flipquiz_add_random_questions($flipquizobj->get_flipquiz(), 1, $cat->id, 1, false);
        $structure = \mod_flipquiz\structure::create_for_flipquiz($flipquizobj);
    ...
    public function test_can_remove_last_question_in_a_flipquiz() {
        $flipquizobj = $this->create_test_flipquiz(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
            ));
        ...
        flipquiz_add_flipquiz_question($q->id, $flipquizobj->get_flipquiz(), 0);
        $structure = \mod_flipquiz\structure::create_for_flipquiz($flipquizobj);
    ...
    public function test_add_question_updates_headings() {
        ...
        flipquiz_add_flipquiz_question($q->id, $flipquizobj->get_flipquiz(), 1);
    ...
    public function test_remove_page_break() {
        ...
        $slots = $structure->update_page_break($slotid, \mod_flipquiz\repaginate::LINK);
    ...
    public function test_add_page_break() {
        ...
        $slotid = $structure->get_question_in_slot(2)->slotid;
        $slots = $structure->update_page_break($slotid, \mod_flipquiz\repaginate::UNLINK);
     ...
     * @param  array $layout Quiz layout for create_test_flipquiz function
    ...
    public function test_get_slot_tags_for_slot($layout, $tagnames, $slotnumber, $expected) {
        ...
        $flipquiz = $this->create_test_flipquiz($layout);
        $structure = \mod_flipquiz\structure::create_for_flipquiz($flipquiz);
        ...
        $DB->insert_records('flipquiz_slot_tags', $slottagrecords);
