...
/**
 * Unit tests for (some of) mod/flipquiz/report/reportlib.php
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU Public License
 */
class mod_flipquiz_reportlib_testcase extends advanced_testcase {
    public function test_flipquiz_report_index_by_keys() {
        $datum = array();
        $object = new stdClass();
        $object->qid = 3;
        $object->aid = 101;
        ...
        $indexed = flipquiz_report_index_by_keys($datum, array('aid', 'qid'));
        ...
        $indexed = flipquiz_report_index_by_keys($datum, array('aid', 'qid'), false);
    ...
    public function test_flipquiz_report_scale_summarks_as_percentage() {
        $flipquiz = new stdClass();
        $flipquiz->sumgrades = 10;
        $flipquiz->decimalpoints = 2;

        $this->assertEquals('12.34567%',
            flipquiz_report_scale_summarks_as_percentage(1.234567, $flipquiz, false));
        $this->assertEquals('12.35%',
            flipquiz_report_scale_summarks_as_percentage(1.234567, $flipquiz, true));
        $this->assertEquals('-',
            flipquiz_report_scale_summarks_as_percentage('-', $flipquiz, true));
    }

    public function test_flipquiz_report_qm_filter_select_only_one_attempt_allowed() {
        $flipquiz = new stdClass();
        $flipquiz->attempts = 1;
        $this->assertSame('', flipquiz_report_qm_filter_select($flipquiz));
    }

    public function test_flipquiz_report_qm_filter_select_average() {
        $flipquiz = new stdClass();
        $flipquiz->attempts = 10;
        $flipquiz->grademethod = FLIPQUIZ_GRADEAVERAGE;
        $this->assertSame('', flipquiz_report_qm_filter_select($flipquiz));
    }

    public function test_flipquiz_report_qm_filter_select_first_last_best() {
        global $DB;
        $this->resetAfterTest();

        $fakeattempt = new stdClass();
        $fakeattempt->userid = 123;
        $fakeattempt->flipquiz = 456;
        $fakeattempt->layout = '1,2,0,3,4,0,5';
        $fakeattempt->state = flipquiz_attempt::FINISHED;

        // We intentionally insert these in a funny order, to test the SQL better.
        // The test data is:
        // id | flipquizid | user | attempt | sumgrades | state
        // ---------------------------------------------------
        ...
        $DB->insert_record('flipquiz_attempts', $fakeattempt);
        ...
        $fakeattempt->state = flipquiz_attempt::IN_PROGRESS;
        $DB->insert_record('flipquiz_attempts', $fakeattempt);
        ...
        $flipquiz->grademethod = FLIPQUIZ_ATTEMPTFIRST;
        $firstattempt = $DB->get_records_sql("
                SELECT * FROM {flipquiz_attempts} flipquiza WHERE userid = ? AND flipquiz = ? AND "
                        . flipquiz_report_qm_filter_select($flipquiz), array(123, 456));
        $this->assertEquals(1, count($firstattempt));
        $firstattempt = reset($firstattempt);
        $this->assertEquals(1, $firstattempt->attempt);

        $flipquiz->grademethod = FLIPQUIZ_ATTEMPTLAST;
        ...
        $bestattempt = $DB->get_records_sql("
                SELECT * FROM {flipquiz_attempts} qa_alias WHERE userid = ? AND flipquiz = ? AND "
                . flipquiz_report_qm_filter_select($flipquiz, 'qa_alias'), array(123, 456));
