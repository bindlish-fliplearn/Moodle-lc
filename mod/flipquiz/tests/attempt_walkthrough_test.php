 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_flipquiz_attempt_walkthrough_testcase extends advanced_testcase {

    /**
     * Create a flipquiz with questions and walk through a flipquiz attempt.
     */
    public function test_flipquiz_attempt_walkthrough() {
        global $SITE;
        ...
        $flipquiz = $flipquizgenerator->create_instance(array('course'=>$SITE->id, 'questionsperpage' => 0, 'grade' => 100.0,
                                                      'sumgrades' => 2));
        ...
        $grades = flipquiz_get_user_grades($flipquiz, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'flipquiz', $flipquiz->id, $user1->id);
    ...
    /**
     * Create a flipquiz with a random as well as other questions and walk through flipquiz attempts.
     */
    public function test_flipquiz_with_random_question_attempt_walkthrough() {
        global $SITE;
        ...
        $flipquiz = $flipquizgenerator->create_instance(array('course' => $SITE->id, 'questionsperpage' => 2, 'grade' => 100.0,
                                                      'sumgrades' => 4));
        ...
        // Add random question to the flipquiz.
        ...
        foreach (array($saq->id => 'frog', $numq->id => '3.14') as $randomqidtoselect => $randqanswer) {
            ...
            $timenow = time();
            $attempt = flipquiz_create_attempt($flipquizobj, 1, false, $timenow);

            flipquiz_start_new_attempt($flipquizobj, $quba, $attempt, 1, $timenow, array(1 => $randomqidtoselect));
    ...
    protected $flipquizwithvariants = null;

    /**
     * Create a flipquiz with a single question with variants and walk through flipquiz attempts.
     *
     * @dataProvider get_correct_response_for_variants
     */
    public function test_flipquiz_with_question_with_variants_attempt_walkthrough($variantno, $correctresponse, $done = false) {
        global $SITE;
        ...
        if ($this->flipquizwithvariants === null) {
            // Make a flipquiz.
            $flipquizgenerator = $this->getDataGenerator()->get_plugin_generator('mod_flipquiz');

            $this->flipquizwithvariants = $flipquizgenerator->create_instance(array('course'=>$SITE->id,
                                                                            'questionsperpage' => 0,
                                                                            'grade' => 100.0,
                                                                            'sumgrades' => 1));
            ...
            flipquiz_add_flipquiz_question($calc->id, $this->flipquizwithvariants, 0);
        }


        // Make a new user to do the flipquiz.
        $user1 = $this->getDataGenerator()->create_user();
        $this->setUser($user1);
        $flipquizobj = flipquiz::create($this->flipquizwithvariants->id, $user1->id);
        ...
        // Select variant.
        flipquiz_start_new_attempt($flipquizobj, $quba, $attempt, 1, $timenow, array(), array(1 => $variantno));
        ...
        // Check flipquiz grades.
        $grades = flipquiz_get_user_grades($this->flipquizwithvariants, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'flipquiz', $this->flipquizwithvariants->id, $user1->id);
        $gradebookitem = array_shift($gradebookgrades->items);
        $gradebookgrade = array_shift($gradebookitem->grades);
        $this->assertEquals(100, $gradebookgrade->grade);
    }
