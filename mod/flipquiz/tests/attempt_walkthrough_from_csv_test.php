 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_flipquiz_attempt_walkthrough_from_csv_testcase extends advanced_testcase {

    protected $files = array('questions', 'steps', 'results');

    /**
     * @var stdClass the flipquiz record we create.
     */
    protected $flipquiz;
     ...
     * directory.
     *
     * @param array $flipquizsettings of settings read from csv file flipquizzes.csv
    ...
    public function test_walkthrough_from_csv($flipquizsettings, $csvdata) {
        ...
        // https://github.com/jamiepratt/moodle-flipquiz-tools/tree/master/responsegenerator

        $this->create_flipquiz_simulate_attempts_and_check_results($flipquizsettings, $csvdata);
    }

    public function create_flipquiz($flipquizsettings, $qs) {
        global $SITE, $DB;
        $this->setAdminUser();
        ...
        $aggregratedsettings = $flipquizsettings + array('course' => $SITE->id,
                                                     'questionsperpage' => 0,
                                                     'grade' => 100.0,
                                                     'sumgrades' => $sumofgrades);

        $this->flipquiz = $flipquizgenerator->create_instance($aggregratedsettings);

        $this->randqids = array();
        foreach ($slots as $slotno => $slotquestion) {
            if ($slotquestion['type'] !== 'random') {
                flipquiz_add_flipquiz_question($slotquestion['id'], $this->flipquiz, 0, $slotquestion['mark']);
            } else {
                flipquiz_add_random_questions($this->flipquiz, 0, $slotquestion['catid'], 1, 0);
    ...
    /**
     * Create flipquiz, simulate attempts and check results (if resultsXX.csv exists).
     *
     * @param array $flipquizsettings Quiz overrides for this flipquiz.
     * @param PHPUnit\DbUnit\DataSet\ITable[] $csvdata Data loaded from csv files for this test.
     */
    protected function create_flipquiz_simulate_attempts_and_check_results($flipquizsettings, $csvdata) {
    ...
    public function get_data_for_walkthrough() {
        $flipquizzes = $this->load_csv_data_file('flipquizzes');
        $datasets = array();
        for ($rowno = 0; $rowno < $flipquizzes->getRowCount(); $rowno++) {
            $flipquizsettings = $flipquizzes->getRow($rowno);
            $dataset = array();
            foreach ($this->files as $file) {
                if (file_exists($this->get_full_path_of_csv_file($file, $flipquizsettings['testnumber']))) {
                    $dataset[$file] = $this->load_csv_data_file($file, $flipquizsettings['testnumber']);
                }
            }
            $datasets[] = array($flipquizsettings, $dataset);
        }
        return $datasets;
    }
     ...
     * @return array attempt no as in csv file => the id of the flipquiz_attempt as stored in the db.
     */
    protected function walkthrough_attempts($steps) {
        ...
        for ($rowno = 0; $rowno < $steps->getRowCount(); $rowno++) {
            ...
            // Find existing user or make a new user to do the flipquiz.
            ...
            if (!isset($attemptids[$step['flipquizattempt']])) {
                // Start the attempt.
                $flipquizobj = flipquiz::create($this->flipquiz->id, $user->id);
                ...
                $prevattempts = flipquiz_get_user_attempts($this->flipquiz->id, $user->id, 'all', true);
                $attemptnumber = count($prevattempts) + 1;
                $timenow = time();
                $attempt = flipquiz_create_attempt($flipquizobj, $attemptnumber, false, $timenow, false, $user->id);
                ...
                flipquiz_start_new_attempt($flipquizobj, $quba, $attempt, $attemptnumber, $timenow, $step['randqs'], $step['variants']);
                flipquiz_attempt_save_started($flipquizobj, $quba, $attempt);
                $attemptid = $attemptids[$step['flipquizattempt']] = $attempt->id;
            } else {
                $attemptid = $attemptids[$step['flipquizattempt']];
            }
     ...
     * @param $attemptids array attempt no as in csv file => the id of the flipquiz_attempt as stored in the db.
     */
    protected function check_attempts_results($results, $attemptids) {
        for ($rowno = 0; $rowno < $results->getRowCount(); $rowno++) {
            ...
            // Re-load flipquiz attempt data.
            $attemptobj = flipquiz_attempt::create($attemptids[$result['flipquizattempt']]);
     ...
     * @param flipquiz_attempt $attemptobj         the attempt object loaded from db.
        ...
        foreach ($result as $fieldname => $value) {
            ...
            switch ($fieldname) {
                            ...
                            switch ($slotfieldname) {
                                case 'mark' :
                                    $this->assertEquals(round($slotvalue, 2), $attemptobj->get_question_mark($slotno),
                                                        "Mark for slot $slotno of attempt {$result['flipquizattempt']}.");
                                    break;
                                default :
                ...
                case 'finished' :
                    ...
                    break;
                case 'summarks' :
                    $this->assertEquals($value, $attemptobj->get_sum_marks(), "Sum of marks of attempt {$result['flipquizattempt']}.");
                    break;
                case 'flipquizgrade' :
                    // Check flipquiz grades.
                    $grades = flipquiz_get_user_grades($attemptobj->get_flipquiz(), $attemptobj->get_userid());
                    $grade = array_shift($grades);
                    $this->assertEquals($value, $grade->rawgrade, "Quiz grade for attempt {$result['flipquizattempt']}.");
                    break;
                case 'gradebookgrade' :
                    // Check grade book.
                    $gradebookgrades = grade_get_grades($attemptobj->get_courseid(),
                                                        'mod', 'flipquiz',
                                                        $attemptobj->get_flipquizid(),
                                                        $attemptobj->get_userid());
                    $gradebookitem = array_shift($gradebookgrades->items);
                    $gradebookgrade = array_shift($gradebookitem->grades);
                    $this->assertEquals($value, $gradebookgrade->grade, "Gradebook grade for attempt {$result['flipquizattempt']}.");
                    break;
                default :
