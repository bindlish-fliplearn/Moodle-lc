...
/**
 * Settings form for overrides in the flipquiz module.
 *
...
require_once($CFG->dirroot . '/mod/flipquiz/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class flipquiz_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the flipquiz settings object. */
    protected $flipquiz;
     ...
     * @param object the flipquiz settings object.
     ...
     */
    public function __construct($submiturl, $cm, $flipquiz, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'flipquiz'));
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'flipquiz'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/flipquiz/overrides.php', array('cmid'=>$cm->id));
                    print_error('groupsnone', 'flipquiz', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'flipquiz'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                $users = get_users_by_capability($this->context, 'mod/flipquiz:attempt',
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/flipquiz/overrides.php', array('cmid'=>$cm->id));
                    print_error('usersnone', 'flipquiz', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'flipquiz'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'flipquiz');
        $mform->setDefault('password', $this->flipquiz->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('flipquizopen', 'flipquiz'), mod_flipquiz_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->flipquiz->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('flipquizclose', 'flipquiz'), mod_flipquiz_mod_form::$datefieldoptions);
        $mform->setDefault('timeclose', $this->flipquiz->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'flipquiz'), array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'flipquiz');
        $mform->setDefault('timelimit', $this->flipquiz->timelimit);
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'flipquiz'), $attemptoptions);
        $mform->addHelpButton('attempts', 'attempts', 'flipquiz');
        $mform->setDefault('attempts', $this->flipquiz->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'flipquiz'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'flipquiz'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'flipquiz'));
    ...
    public function validation($data, $files) {
        ...
        // Ensure that at least one flipquiz setting was changed.
        $changed = false;
        $keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
        foreach ($keys as $key) {
            if ($data[$key] != $flipquiz->{$key}) {
                $changed = true;
                break;
            }
        }
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'flipquiz');
        }
