...
/**
 * Classes to enforce the various access rules that can apply to a flipquiz.
 *
 ...
 * flipquiz, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
 */
class flipquiz_access_manager {
    /** @var flipquiz the flipquiz settings object. */
    protected $flipquizobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of flipquiz_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular flipquiz.
     * @param object $flipquizobj An instance of the class flipquiz from attemptlib.php.
     *      The flipquiz we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/flipquiz:ignoretimelimits', ...)).
     */
    public function __construct($flipquizobj, $timenow, $canignoretimelimits) {
        $this->flipquizobj = $flipquizobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($flipquizobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular flipquiz.
     * @param flipquiz $flipquizobj information about the flipquiz in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/flipquiz:ignoretimelimits capability.
     * @return array of {@link flipquiz_access_rule_base}s.
     */
    protected function make_rules($flipquizobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($flipquizobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['flipquizaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('flipquizaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the flipquiz table.
     *
     * @param mod_flipquiz_mod_form $flipquizform the flipquiz settings form that is being built.
    ...
    public static function add_settings_form_fields(
        ...
        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($flipquizform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'flipquiz'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_flipquiz_mod_form $flipquizform the flipquiz form object.
    ...
    public static function validate_settings_form_fields(array $errors,
        ...
        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $flipquizform);
        }
    ...
    /**
     * Save any submitted settings when the flipquiz settings form is submitted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the flipquiz table.
     *
     * @param object $flipquiz the data from the flipquiz form, including $flipquiz->id
     *      which is the id of the flipquiz being saved.
     */
    public static function save_settings($flipquiz) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($flipquiz);
        }
    }

    /**
     * Delete any rule-specific settings when the flipquiz is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the flipquiz table.
     *
     * @param object $flipquiz the data from the database, including $flipquiz->id
     *      which is the id of the flipquiz being deleted.
     * @since Moodle 2.7.1, 2.6.4, 2.5.7
     */
    public static function delete_settings($flipquiz) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($flipquiz);
        }
    }
     ...
     */
    protected static function get_load_sql($flipquizid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{flipquiz} flipquiz';
        $allparams = array('flipquizid' => $flipquizid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($flipquizid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE flipquiz.id = :flipquizid", $allparams);
    }
     ...
     */
    public static function load_settings($flipquizid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($flipquizid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($flipquizid);
        }
    ...
    /**
     * Load the flipquiz settings and any settings required by the access rules.
     ...
     *
     * @param int $flipquizid the flipquiz id.
     * @return object mdl_flipquiz row with extra fields.
     */
    public static function load_flipquiz_and_settings($flipquizid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($flipquizid, $rules, 'flipquiz.*');
        $flipquiz = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($flipquizid) as $name => $value) {
                $flipquiz->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this flipquiz, such
     * as is shown at the top of the flipquiz view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this flipquiz now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_flipquiz_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        $method = 'post';
        if (!empty($_GET['_qf__mod_flipquiz_preflight_check_form'])) {
            $method = 'get';
        }
        return new mod_flipquiz_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'flipquizobj' => $this->flipquizobj,
     ...
     * flipquiz. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant flipquiz_attempts row.
    ...
    /**
     * @return bolean if this flipquiz should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the flipquiz view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_flipquiz_renderer $output the flipquiz renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->flipquizobj->view_url(), $message);
            die();
        } else {
            redirect($this->flipquizobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the flipquiz, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(flipquiz_attempt::IN_PROGRESS, flipquiz_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = flipquiz_attempt_state($this->flipquizobj->get_flipquiz(), $attempt);
        $reviewoptions = mod_flipquiz_display_options::make_from_flipquiz(
                $this->flipquizobj->get_flipquiz(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->flipquizobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->flipquizobj->review_url($attempt->id),
