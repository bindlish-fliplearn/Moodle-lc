...
/**
 * Back-end code for handling data about flipquizzes and the current user's attempt.
 *
 * There are classes for loading all the information about a flipquiz and attempts,
...
/**
 * Class for flipquiz exceptions. Just saves a couple of arguments on the
 ...
 * @since     Moodle 2.0
 */
class moodle_flipquiz_exception extends moodle_exception {
    public function __construct($flipquizobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $flipquizobj->view_url();
        }
        parent::__construct($errorcode, 'flipquiz', $link, $a, $debuginfo);
    }
}


/**
 * A class encapsulating a flipquiz and the questions it contains, and making the
...
class flipquiz {
    ...
    protected $cm;
    /** @var stdClass the flipquiz settings from the database. */
    protected $flipquiz;
    /** @var context the flipquiz context. */
    protected $context;

    /** @var array of questions augmented with slot information. */
    protected $questions = null;
    /** @var array of flipquiz_section rows. */
    protected $sections = null;
    /** @var flipquiz_access_manager the access manager for this flipquiz. */
    protected $accessmanager = null;
    /** @var bool whether the current user has capability mod/flipquiz:preview. */
     ...
     *
     * @param object $flipquiz the row from the flipquiz table.
     * @param object $cm the course_module object for this flipquiz.
     * @param object $course the row from the course table for the course we belong to.
     * @param bool $getcontext intended for testing - stops the constructor getting the context.
     */
    public function __construct($flipquiz, $cm, $course, $getcontext = true) {
        $this->flipquiz = $flipquiz;
        $this->cm = $cm;
        $this->flipquiz->cmid = $this->cm->id;
        $this->course = $course;
    ...
    /**
     * Static function to create a new flipquiz object for a specific user.
     *
     * @param int $flipquizid the the flipquiz id.
     * @param int $userid the the userid.
     * @return flipquiz the new flipquiz object
     */
    public static function create($flipquizid, $userid = null) {
        global $DB;

        $flipquiz = flipquiz_access_manager::load_flipquiz_and_settings($flipquizid);
        $course = $DB->get_record('course', array('id' => $flipquiz->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('flipquiz', $flipquiz->id, $course->id, false, MUST_EXIST);

        // Update flipquiz with override information.
        if ($userid) {
            $flipquiz = flipquiz_update_effective_access($flipquiz, $userid);
        }

        return new flipquiz($flipquiz, $cm, $course);
    }

    /**
     * Create a {@link flipquiz_attempt} for an attempt at this flipquiz.
     * @param object $attemptdata row from the flipquiz_attempts table.
     * @return flipquiz_attempt the new flipquiz_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new flipquiz_attempt($attemptdata, $this->flipquiz, $this->cm, $this->course);
    }

    // Functions for loading more data =========================================

    /**
     * Load just basic information about all the questions in this flipquiz.
     */
    public function preload_questions() {
        $this->questions = question_preload_questions(null,
                ...
                '{flipquiz_slots} slot ON slot.flipquizid = :flipquizid AND q.id = slot.questionid',
                array('flipquizid' => $this->flipquiz->id), 'slot.slot');
    }

    /**
     * Fully load some or all of the questions for this flipquiz. You must call
    ...
    /**
     * Get an instance of the {@link \mod_flipquiz\structure} class for this flipquiz.
     * @return \mod_flipquiz\structure describes the questions in the flipquiz.
     */
    public function get_structure() {
        return \mod_flipquiz\structure::create_for_flipquiz($this);
    }
    ...
    /** @return int the flipquiz id. */
    public function get_flipquizid() {
        return $this->flipquiz->id;
    }

    /** @return object the row of the flipquiz table. */
    public function get_flipquiz() {
        return $this->flipquiz;
    }

    /** @return string the name of this flipquiz. */
    public function get_flipquiz_name() {
        return $this->flipquiz->name;
    }

    /** @return int the flipquiz navigation method. */
    public function get_navigation_method() {
        return $this->flipquiz->navmethod;
    }

    /** @return int the number of attempts allowed at this flipquiz (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->flipquiz->attempts;
    }
    ...
    /** @return object the module context for this flipquiz. */
    public function get_context() {
        return $this->context;
    }

    /**
     * @return bool wether the current user is someone who previews the flipquiz,
     * rather than attempting it.
     */
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/flipquiz:preview', $this->context);
        }
        return $this->ispreviewuser;
    }

    /**
     * @return whether any questions have been added to this flipquiz.
     */
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'flipquiz', $this->view_url());
            }
    ...
    /**
     * Get all the sections in this flipquiz.
     * @return array 0, 1, 2, ... => flipquiz_sections row from the database.
     */
    public function get_sections() {
        global $DB;
        if ($this->sections === null) {
            $this->sections = array_values($DB->get_records('flipquiz_sections',
                    array('flipquizid' => $this->get_flipquizid()), 'firstslot'));
        }
        return $this->sections;
    }

    /**
     * Return flipquiz_access_manager and instance of the flipquiz_access_manager class
     * for this flipquiz at this time.
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new flipquiz_access_manager($this, $timenow,
                    has_capability('mod/flipquiz:ignoretimelimits', $this->context, null, false));
        }
        return $this->accessmanager;
    }

    /**
     * Wrapper round the has_capability funciton that automatically passes in the flipquiz context.
     */
    ...
    /**
     * Wrapper round the require_capability funciton that automatically passes in the flipquiz context.
     */
    ...
    /**
     * @return string the URL of this flipquiz's view page.
     */
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/flipquiz/view.php?id=' . $this->cm->id;
    }

    /**
     * @return string the URL of this flipquiz's edit page.
     */
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/flipquiz/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/flipquiz/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
    ...
    /**
     * @return string the URL of this flipquiz's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/flipquiz/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/flipquiz/review.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/flipquiz/summary.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
     ...
     * @deprecated since 3.1. This sort of functionality is now entirely handled by flipquiz access rules.
     */
    public function confirm_start_attempt_message($notused) {
        debugging('confirm_start_attempt_message is deprecated. ' .
                'This sort of functionality is now entirely handled by flipquiz access rules.');
        return '';
    }
     ...
     *
     * @param int $when One of the mod_flipquiz_display_options::DURING,
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_flipquiz_display_options::DURING ||
                $when == mod_flipquiz_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_flipquiz_display_options::LATER_WHILE_OPEN && $this->flipquiz->timeclose &&
                $this->flipquiz->reviewattempt & mod_flipquiz_display_options::AFTER_CLOSE) {
            return get_string('noreviewuntil' . $langstrsuffix, 'flipquiz',
                    userdate($this->flipquiz->timeclose, $dateformat));
        } else {
            return get_string('noreview' . $langstrsuffix, 'flipquiz');
        }
    }

    /**
     * @param string $title the name of this particular flipquiz page.
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_flipquiz_exception($this, 'questionnotloaded', $id);
        }
    }

    /**
     * Return all the question types used in this flipquiz.
     *
     * @param  boolean $includepotential if the flipquiz include random questions, setting this flag to true will make the function to
    ...
    public function get_all_question_types_used($includepotential = false) {
        ...
        // We must be careful with random questions, if we find a random question we must assume that the flipquiz may content
...
/**
 * This class extends the flipquiz class to hold data about the state of a particular attempt,
 * in addition to the data about the flipquiz.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class flipquiz_attempt {
    ...
    /** @var int maximum number of slots in the flipquiz for the review page to default to show all. */
    const MAX_SLOTS_FOR_DEFAULT_REVIEW_SHOW_ALL = 50;

    /** @var flipquiz object containing the flipquiz settings. */
    protected $flipquizobj;

    /** @var stdClass the flipquiz_attempts row. */
    protected $attempt;

    /** @var question_usage_by_activity the question usage for this flipquiz attempt. */
    protected $quba;
    ...
    /** @var array of flipquiz_sections rows, with a ->lastslot field added. */
    protected $sections;
    ...
    /** @var mod_flipquiz_display_options cache for the appropriate review options. */
     ...
     * Constructor assuming we already have the necessary data loaded.
     *
     * @param object $attempt the row of the flipquiz_attempts table.
     * @param object $flipquiz the flipquiz object for this attempt and user.
     * @param object $cm the course_module object for this flipquiz.
     * @param object $course the row from the course table for the course we belong to.
     ...
     */
    public function __construct($attempt, $flipquiz, $cm, $course, $loadquestions = true) {
        global $DB;

        $this->attempt = $attempt;
        $this->flipquizobj = new flipquiz($flipquiz, $cm, $course);
        ...
        $this->slots = $DB->get_records('flipquiz_slots',
                array('flipquizid' => $this->get_flipquizid()), 'slot',
     ...
     * @param array $conditions passed to $DB->get_record('flipquiz_attempts', $conditions).
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('flipquiz_attempts', $conditions, '*', MUST_EXIST);
        $flipquiz = flipquiz_access_manager::load_flipquiz_and_settings($attempt->flipquiz);
        $course = $DB->get_record('course', array('id' => $flipquiz->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('flipquiz', $flipquiz->id, $course->id, false, MUST_EXIST);

        // Update flipquiz with override information.
        $flipquiz = flipquiz_update_effective_access($flipquiz, $attempt->userid);

        return new flipquiz_attempt($attempt, $flipquiz, $cm, $course);
    }

    /**
     * Static function to create a new flipquiz_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new flipquiz_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return flipquiz_attempt_state_name($state);
    }
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'flipquiz');
                }
    ...
    // Simple getters ==========================================================
    public function get_flipquiz() {
        return $this->flipquizobj->get_flipquiz();
    }

    public function get_flipquizobj() {
        return $this->flipquizobj;
    }

    /** @return int the course id. */
    public function get_courseid() {
        return $this->flipquizobj->get_courseid();
    }

    /** @return int the course id. */
    public function get_course() {
        return $this->flipquizobj->get_course();
    }

    /** @return int the flipquiz id. */
    public function get_flipquizid() {
        return $this->flipquizobj->get_flipquizid();
    }

    /** @return string the name of this flipquiz. */
    public function get_flipquiz_name() {
        return $this->flipquizobj->get_flipquiz_name();
    }

    /** @return int the flipquiz navigation method. */
    public function get_navigation_method() {
        return $this->flipquizobj->get_navigation_method();
    }

    /** @return object the course_module object. */
    public function get_cm() {
        return $this->flipquizobj->get_cm();
    }

    /** @return object the course_module object. */
    public function get_cmid() {
        return $this->flipquizobj->get_cmid();
    }
     ...
     */
    public function is_preview_user() {
        return $this->flipquizobj->is_preview_user();
    }

    /** @return int the number of attempts allowed at this flipquiz (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->flipquizobj->get_num_attempts_allowed();
    }

    /** @return int number fo pages in this flipquiz. */
     ...
     */
    public function get_access_manager($timenow) {
        return $this->flipquizobj->get_access_manager($timenow);
    }
    ...
    /** @return object the row from the flipquiz_attempts table. */
    ...
    /** @return string one of the flipquiz_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/flipquiz:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the flipquiz in a non-trivial way?
     ...
     * To see how this works, try setting a flipquiz to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the flipquiz
     ...
     *
     * @param question_display_options $options the display options for this flipquiz attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return flipquiz_feedback_for_grade($grade, $this->get_flipquiz(),
                $this->flipquizobj->get_context());
    }
     ...
     */
    public function has_capability($capability, $userid = null, $doanything = true) {
        return $this->flipquizobj->has_capability($capability, $userid, $doanything);
    }
     ...
     */
    public function require_capability($capability, $userid = null, $doanything = true) {
        return $this->flipquizobj->require_capability($capability, $userid, $doanything);
    }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_flipquiz_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/flipquiz:attempt';
        } else {
            $capability = 'mod/flipquiz:reviewmyattempts';
        }

        // These next tests are in a slighly funny order. The point is that the
        // common and most performance-critical case is students attempting a flipquiz
        // so we want to check that permisison first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the flipquiz as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/flipquiz:viewreports') ||
                $this->has_capability('mod/flipquiz:preview')) {
    ...
    public function can_navigate_to($slot) {
        switch ($this->get_navigation_method()) {
            case FLIPQUIZ_NAVMETHOD_FREE:
                return true;
                break;
            case FLIPQUIZ_NAVMETHOD_SEQ:
                return false;
                break;
        }
        return true;
    ...
    public function get_attempt_state() {
        return flipquiz_attempt_state($this->get_flipquiz(), $this->attempt);
    }

    /**
     * Wrapper that the correct mod_flipquiz_display_options for this flipquiz at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = flipquiz_get_review_options($this->get_flipquiz(),
                        $this->attempt, $this->flipquizobj->get_context());
            ...
            $options = mod_flipquiz_display_options::make_from_flipquiz($this->get_flipquiz(),
                    mod_flipquiz_display_options::DURING);
            $options->flags = flipquiz_get_flag_option($this->attempt, $this->flipquizobj->get_context());
            return $options;
        }
    }
     ...
     * @return bool true if this is the last page of the flipquiz.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the flipquiz, or for the
     * whole flipquiz.
     *
    ...
    public function get_question_usage() {
        if (!PHPUNIT_TEST) {
            throw new coding_exception('get_question_usage is only for use in unit tests. ' .
                    'For other operations, use the flipquiz_attempt api, or extend it properly.');
        }
        return $this->quba;
    }
    ...
    public function is_blocked_by_previous_question($slot) {
        return $slot > 1 && isset($this->slots[$slot]) && $this->slots[$slot]->requireprevious &&
                ...
                $this->get_navigation_method() != FLIPQUIZ_NAVMETHOD_SEQ &&
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_flipquiz()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the flipquiz where this question appears.
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the flipquiz this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the flipquiz.
     */
    public function get_question_mark($slot) {
        return flipquiz_format_question_grade($this->get_flipquiz(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = array();
        if ($this->flipquizobj->get_flipquiz()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->flipquizobj->get_flipquiz()->timelimit;
        }
        if ($this->flipquizobj->get_flipquiz()->timeclose) {
            $deadlines[] = $this->flipquizobj->get_flipquiz()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->flipquizobj->get_flipquiz()->graceperiod;
    ...
    // URLs related to this attempt ============================================
    /**
     * @return string flipquiz view url.
     */
    public function view_url() {
        return $this->flipquizobj->view_url();
    }
    ...
    public function start_attempt_url($slot = null, $page = -1) {
        ...
        return $this->flipquizobj->start_attempt_url($page);
    }
    ...
    /**
     * @return string the URL of this flipquiz's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/flipquiz/summary.php', array('attempt' => $this->attempt->id, 'cmid' => $this->get_cmid()));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/flipquiz/processattempt.php');
    }
    ...
    public function cannot_review_message($short = false) {
        return $this->flipquizobj->cannot_review_message(
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'flipquiz'));
        } else {
            return '';
        }
    }
     ...
     * @param mod_flipquiz_renderer $renderer the flipquiz renderer.
     ...
     */
    public function render_question($slot, $reviewing, mod_flipquiz_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions,
                    ...
                    'mod_flipquiz-blocked_question_warning');
        }
     ...
     */
    protected function render_question_helper($slot, $reviewing, $thispageurl, mod_flipquiz_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'flipquiz', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'flipquiz');
        ...
        $placeholderqa->start($this->get_flipquiz()->preferredbehaviour, 1);
     ...
     *
     * @param int $id the id of a question in this flipquiz attempt.
     ...
     * @return string HTML for the question in its current state.
     */
    public function render_question_at_step($slot, $seq, $reviewing, mod_flipquiz_renderer $renderer, $thispageurl = '') {
     ...
     *
     * @param $panelclass The type of panel, flipquiz_attempt_nav_panel or flipquiz_review_nav_panel
     * @param $page the current page number.
     * @param $showall whether we are showing the whole flipquiz on one page. (Used by review.php)
     * @return flipquiz_nav_panel_base the requested object.
     */
    public function get_navigation_panel(mod_flipquiz_renderer $output,
             $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_flipquiz_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->attributes['aria-labelledby'] = 'mod_flipquiz_navblock_title';
        $bc->title = html_writer::span(get_string('flipquiznavigation', 'flipquiz'), '', array('id' => 'mod_flipquiz_navblock_title'));
        $bc->content = $output->navigation_panel($panel);
        return $bc;
    }

    /**
     * Return an array of variant URLs to other attempts at this flipquiz.
     *
     * The $url passed in must contain an attempt parameter.
     *
     * The {@link mod_flipquiz_links_to_other_attempts} object returned contains an
     ...
     * @return mod_flipquiz_links_to_other_attempts containing array int => null|moodle_url.
     */
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = flipquiz_get_user_attempts($this->get_flipquiz()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }

        $links = new mod_flipquiz_links_to_other_attempts();
     ...
     * @return mod_flipquiz_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->flipquizobj->get_flipquiz()->graceperiod;
        ...
        // Otherwise, we were in flipquiz_attempt::IN_PROGRESS, and time has now expired.
        // Transition to the appropriate state.
        switch ($this->flipquizobj->get_flipquiz()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if ($becomingoverdue) {
            ...
            $DB->update_record('flipquiz_attempts', $this->attempt);
        }

        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            flipquiz_save_best_grade($this->get_flipquiz(), $this->get_userid());
        }
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_flipquiz\question\qubaids_for_users_attempts(
                $this->get_flipquizid(), $this->get_userid());
        ...
        if ($questiondata->qtype != 'random') {
            ...
            $tagids = flipquiz_retrieve_slot_tag_ids($this->slots[$slot]->id);
            ...
            if ($newqusetionid === null) {
                throw new moodle_exception('notenoughrandomquestions', 'flipquiz',
                        $this->flipquizobj->view_url(), $questiondata);
            }
        }
    ...
    public function process_finish($timestamp, $processsubmitted) {
        ...
        if (!$this->is_preview()) {
            flipquiz_save_best_grade($this->get_flipquiz(), $this->attempt->userid);

            // Trigger event.
            $this->fire_state_transition_event('\mod_flipquiz\event\attempt_submitted', $timestamp);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('flipquiz_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_flipquiz\event\attempt_becameoverdue', $timestamp);

        $transaction->allow_commit();

        flipquiz_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_flipquiz\event\attempt_abandoned', $timestamp);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp) {
        global $USER;
        $flipquizrecord = $this->get_flipquiz();
        $params = array(
            'context' => $this->get_flipquizobj()->get_context(),
            'courseid' => $this->get_courseid(),
            'objectid' => $this->attempt->id,
            'relateduserid' => $this->attempt->userid,
            'other' => array(
                'submitterid' => CLI_SCRIPT ? null : $USER->id,
                'flipquizid' => $flipquizrecord->id
            )
        );

        $event = $eventclass::create($params);
        $event->add_record_snapshot('flipquiz', $this->get_flipquiz());
        $event->add_record_snapshot('flipquiz_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    /**
     * Get a URL for a particular question on a particular page of the flipquiz.
     * Used by {@link attempt_url()} and {@link review_url()}.
     *
     * @param string $script. Used in the URL like /mod/flipquiz/$script.php
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/flipquiz/' . $script . '.php' . $fragment,
    ...
    /**
     * Process responses during an attempt at a flipquiz.
     *
    ...
    public function process_attempt($timenow, $finishattempt, $timeup, $thispage) {
        ...
        // to show the student another page of the flipquiz. Just finish now.
        $graceperiodmin = null;
        ...
        $toolate = false;
        if ($timeclose !== false && $timenow > $timeclose - FLIPQUIZ_MIN_TIME_TO_CONTINUE) {
            $timeup = true;
            $graceperiodmin = get_config('flipquiz', 'graceperiodmin');
        ...
        if ($timeup) {
            if ($this->get_flipquiz()->overduehandling == 'graceperiod') {
                if (is_null($graceperiodmin)) {
                    $graceperiodmin = get_config('flipquiz', 'graceperiodmin');
                }
                if ($timenow > $timeclose + $this->get_flipquiz()->graceperiod + $graceperiodmin) {
        ...
        // Update the flipquiz attempt record.
        try {
    ...
    public function check_page_access($page) {
        ...
        if ($this->get_currentpage() != $page) {
            if ($this->get_navigation_method() == FLIPQUIZ_NAVMETHOD_SEQ && $this->get_currentpage() > $page) {
                return false;
            }
        }
        return true;
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('flipquiz_attempts', 'currentpage', $page, array('id' => $this->get_attemptid()));
            return true;
        }
        return false;
    }
    ...
    public function fire_attempt_viewed_event() {
        $params = array(
            ...
            'other' => array(
                'flipquizid' => $this->get_flipquizid()
            )
        );
 ...
 * @since      Moodle 2.9
 */
class flipquiz_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class flipquiz_nav_question_button implements renderable {
 ...
 * @since      Moodle 2.0
 */
abstract class flipquiz_nav_panel_base {
    /** @var flipquiz_attempt */
    ...
    public function __construct(flipquiz_attempt $attemptobj,
    ...
    /**
     * Get the buttons and section headings to go in the flipquiz navigation block.
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            if ($heading = $this->attemptobj->get_heading_before_slot($slot)) {
                $buttons[] = new flipquiz_nav_section_heading(format_string($heading));
            }

            $qa = $this->attemptobj->get_question_attempt($slot);
            $showcorrectness = $this->options->correctness && $qa->has_marks();

            $button = new flipquiz_nav_question_button();
            $button->id          = 'flipquiznavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'flipquiz');
            }
            $buttons[] = $button;
        }
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'flipquiz');
        } else {
            return get_string('viewed', 'flipquiz');
        }
    }

    public function render_before_button_bits(mod_flipquiz_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_flipquiz_renderer $output);
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_flipquiz()->showuserpicture == FLIPQUIZ_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', array('id' => $this->attemptobj->get_userid()));
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_flipquiz()->showuserpicture == FLIPQUIZ_SHOWIMAGE_LARGE) {
...
/**
 * Specialisation of {@link flipquiz_nav_panel_base} for the attempt flipquiz page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class flipquiz_attempt_nav_panel extends flipquiz_nav_panel_base {
    ...
    public function render_before_button_bits(mod_flipquiz_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'flipquiz'),
                array('id' => 'flipquiznojswarning'));
    }

    public function render_end_bits(mod_flipquiz_renderer $output) {
        return html_writer::link($this->attemptobj->summary_url(),
                get_string('endtest', 'flipquiz'), array('class' => 'endtestlink')) .
...
/**
 * Specialisation of {@link flipquiz_nav_panel_base} for the review flipquiz page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class flipquiz_review_nav_panel extends flipquiz_nav_panel_base {
    public function get_question_url($slot) {
