...
/**
 * This page handles editing and creation of hwork overrides
 *
 * @package   mod_hwork
...
if ($overrideid) {
    ...
    if (! $override = $DB->get_record('hwork_overrides', array('id' => $overrideid))) {
        print_error('invalidoverrideid', 'hwork');
    }
    if (! $hwork = $DB->get_record('hwork', array('id' => $override->hwork))) {
...
$url = new moodle_url('/mod/hwork/overrideedit.php');
if ($action) {
...
// Merge hwork defaults with data.
...
$overridelisturl = new moodle_url('/mod/hwork/overrides.php', array('cmid'=>$cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new hwork_override_form($url, $cm, $hwork, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->hwork = $hwork->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if ($fromform->{$key} == $hwork->{$key}) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('hwork_overrides', $conditions)) {
            ...
            // Set the course module id before calling hwork_delete_override().
            $hwork->cmid = $cm->id;
            hwork_delete_override($hwork, $oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('hwork_overrides', $fromform);
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('hwork_overrides', $fromform);
    ...
    if ($groupmode) {
        ...
        hwork_update_events($hwork, $fromform);
    }
...
// Print the form.
$pagetitle = get_string('editoverride', 'hwork');
