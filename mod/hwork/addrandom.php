...
/**
 * Fallback page of /mod/hwork/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $hwork, $pagevars) =
        question_edit_setup('editq', '/mod/hwork/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/hwork/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new hwork_add_random_form(new moodle_url('/mod/hwork/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    hwork_add_random_questions($hwork, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories, $tagids);
...
// Setup $PAGE.
$streditinghwork = get_string('editinga', 'moodle', get_string('modulename', 'hwork'));
$PAGE->navbar->add($streditinghwork);
$PAGE->set_title($streditinghwork);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$hworkname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            print_error('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontohwork', 'hwork', $hworkname), 2);
$mform->display();
echo $OUTPUT->footer();
