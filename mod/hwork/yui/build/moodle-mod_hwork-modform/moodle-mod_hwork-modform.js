YUI.add('moodle-mod_hwork-modform', function (Y, NAME) {

/**
 * The modform class has all the JavaScript specific to mod/hwork/mod_form.php.
 *
 * @module moodle-mod_hwork-modform
 */
...
Y.extend(MODFORM, Y.Base, {
    ...
    initializer: function() {
        ...
        if (!this.repaginateCheckbox) {
            // The checkbox only appears when editing an existing hwork.
            return;
        }
...
// Ensure that M.mod_hwork exists and that coursebase is initialised correctly
M.mod_hwork = M.mod_hwork || {};
M.mod_hwork.modform = M.mod_hwork.modform || new MODFORM();
M.mod_hwork.modform.init = function() {
    return new MODFORM();
};
