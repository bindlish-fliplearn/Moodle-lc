...
/**
 * Back-end code for handling data about hworkzes and the current user's attempt.
 *
 * There are classes for loading all the information about a hwork and attempts,
...
/**
 * Class for hwork exceptions. Just saves a couple of arguments on the
 ...
 * @since     Moodle 2.0
 */
class moodle_hwork_exception extends moodle_exception {
    public function __construct($hworkobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $hworkobj->view_url();
        }
        parent::__construct($errorcode, 'hwork', $link, $a, $debuginfo);
    }
}


/**
 * A class encapsulating a hwork and the questions it contains, and making the
...
class hwork {
    ...
    protected $cm;
    /** @var stdClass the hwork settings from the database. */
    protected $hwork;
    /** @var context the hwork context. */
    protected $context;

    /** @var array of questions augmented with slot information. */
    protected $questions = null;
    /** @var array of hwork_section rows. */
    protected $sections = null;
    /** @var hwork_access_manager the access manager for this hwork. */
    protected $accessmanager = null;
    /** @var bool whether the current user has capability mod/hwork:preview. */
     ...
     *
     * @param object $hwork the row from the hwork table.
     * @param object $cm the course_module object for this hwork.
     * @param object $course the row from the course table for the course we belong to.
     * @param bool $getcontext intended for testing - stops the constructor getting the context.
     */
    public function __construct($hwork, $cm, $course, $getcontext = true) {
        $this->hwork = $hwork;
        $this->cm = $cm;
        $this->hwork->cmid = $this->cm->id;
        $this->course = $course;
    ...
    /**
     * Static function to create a new hwork object for a specific user.
     *
     * @param int $hworkid the the hwork id.
     * @param int $userid the the userid.
     * @return hwork the new hwork object
     */
    public static function create($hworkid, $userid = null) {
        global $DB;

        $hwork = hwork_access_manager::load_hwork_and_settings($hworkid);
        $course = $DB->get_record('course', array('id' => $hwork->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('hwork', $hwork->id, $course->id, false, MUST_EXIST);

        // Update hwork with override information.
        if ($userid) {
            $hwork = hwork_update_effective_access($hwork, $userid);
        }

        return new hwork($hwork, $cm, $course);
    }

    /**
     * Create a {@link hwork_attempt} for an attempt at this hwork.
     * @param object $attemptdata row from the hwork_attempts table.
     * @return hwork_attempt the new hwork_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new hwork_attempt($attemptdata, $this->hwork, $this->cm, $this->course);
    }

    // Functions for loading more data =========================================

    /**
     * Load just basic information about all the questions in this hwork.
     */
    public function preload_questions() {
        $this->questions = question_preload_questions(null,
                ...
                '{hwork_slots} slot ON slot.hworkid = :hworkid AND q.id = slot.questionid',
                array('hworkid' => $this->hwork->id), 'slot.slot');
    }

    /**
     * Fully load some or all of the questions for this hwork. You must call
    ...
    /**
     * Get an instance of the {@link \mod_hwork\structure} class for this hwork.
     * @return \mod_hwork\structure describes the questions in the hwork.
     */
    public function get_structure() {
        return \mod_hwork\structure::create_for_hwork($this);
    }
    ...
    /** @return int the hwork id. */
    public function get_hworkid() {
        return $this->hwork->id;
    }

    /** @return object the row of the hwork table. */
    public function get_hwork() {
        return $this->hwork;
    }

    /** @return string the name of this hwork. */
    public function get_hwork_name() {
        return $this->hwork->name;
    }

    /** @return int the hwork navigation method. */
    public function get_navigation_method() {
        return $this->hwork->navmethod;
    }

    /** @return int the number of attempts allowed at this hwork (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->hwork->attempts;
    }
    ...
    /** @return object the module context for this hwork. */
    public function get_context() {
        return $this->context;
    }

    /**
     * @return bool wether the current user is someone who previews the hwork,
     * rather than attempting it.
     */
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/hwork:preview', $this->context);
        }
        return $this->ispreviewuser;
    }

    /**
     * @return whether any questions have been added to this hwork.
     */
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'hwork', $this->view_url());
            }
    ...
    /**
     * Get all the sections in this hwork.
     * @return array 0, 1, 2, ... => hwork_sections row from the database.
     */
    public function get_sections() {
        global $DB;
        if ($this->sections === null) {
            $this->sections = array_values($DB->get_records('hwork_sections',
                    array('hworkid' => $this->get_hworkid()), 'firstslot'));
        }
        return $this->sections;
    }

    /**
     * Return hwork_access_manager and instance of the hwork_access_manager class
     * for this hwork at this time.
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new hwork_access_manager($this, $timenow,
                    has_capability('mod/hwork:ignoretimelimits', $this->context, null, false));
        }
        return $this->accessmanager;
    }

    /**
     * Wrapper round the has_capability funciton that automatically passes in the hwork context.
     */
    ...
    /**
     * Wrapper round the require_capability funciton that automatically passes in the hwork context.
     */
    ...
    /**
     * @return string the URL of this hwork's view page.
     */
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/hwork/view.php?id=' . $this->cm->id;
    }

    /**
     * @return string the URL of this hwork's edit page.
     */
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/hwork/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/hwork/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
    ...
    /**
     * @return string the URL of this hwork's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/hwork/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/hwork/review.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/hwork/summary.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
     ...
     * @deprecated since 3.1. This sort of functionality is now entirely handled by hwork access rules.
     */
    public function confirm_start_attempt_message($notused) {
        debugging('confirm_start_attempt_message is deprecated. ' .
                'This sort of functionality is now entirely handled by hwork access rules.');
        return '';
    }
     ...
     *
     * @param int $when One of the mod_hwork_display_options::DURING,
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_hwork_display_options::DURING ||
                $when == mod_hwork_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_hwork_display_options::LATER_WHILE_OPEN && $this->hwork->timeclose &&
                $this->hwork->reviewattempt & mod_hwork_display_options::AFTER_CLOSE) {
            return get_string('noreviewuntil' . $langstrsuffix, 'hwork',
                    userdate($this->hwork->timeclose, $dateformat));
        } else {
            return get_string('noreview' . $langstrsuffix, 'hwork');
        }
    }

    /**
     * @param string $title the name of this particular hwork page.
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_hwork_exception($this, 'questionnotloaded', $id);
        }
    }

    /**
     * Return all the question types used in this hwork.
     *
     * @param  boolean $includepotential if the hwork include random questions, setting this flag to true will make the function to
    ...
    public function get_all_question_types_used($includepotential = false) {
        ...
        // We must be careful with random questions, if we find a random question we must assume that the hwork may content
...
/**
 * This class extends the hwork class to hold data about the state of a particular attempt,
 * in addition to the data about the hwork.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
...
class hwork_attempt {
    ...
    /** @var int maximum number of slots in the hwork for the review page to default to show all. */
    const MAX_SLOTS_FOR_DEFAULT_REVIEW_SHOW_ALL = 50;

    /** @var hwork object containing the hwork settings. */
    protected $hworkobj;

    /** @var stdClass the hwork_attempts row. */
    protected $attempt;

    /** @var question_usage_by_activity the question usage for this hwork attempt. */
    protected $quba;
    ...
    /** @var array of hwork_sections rows, with a ->lastslot field added. */
    protected $sections;
    ...
    /** @var mod_hwork_display_options cache for the appropriate review options. */
     ...
     * Constructor assuming we already have the necessary data loaded.
     *
     * @param object $attempt the row of the hwork_attempts table.
     * @param object $hwork the hwork object for this attempt and user.
     * @param object $cm the course_module object for this hwork.
     * @param object $course the row from the course table for the course we belong to.
     ...
     */
    public function __construct($attempt, $hwork, $cm, $course, $loadquestions = true) {
        global $DB;

        $this->attempt = $attempt;
        $this->hworkobj = new hwork($hwork, $cm, $course);
        ...
        $this->slots = $DB->get_records('hwork_slots',
                array('hworkid' => $this->get_hworkid()), 'slot',
     ...
     * @param array $conditions passed to $DB->get_record('hwork_attempts', $conditions).
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('hwork_attempts', $conditions, '*', MUST_EXIST);
        $hwork = hwork_access_manager::load_hwork_and_settings($attempt->hwork);
        $course = $DB->get_record('course', array('id' => $hwork->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('hwork', $hwork->id, $course->id, false, MUST_EXIST);

        // Update hwork with override information.
        $hwork = hwork_update_effective_access($hwork, $attempt->userid);

        return new hwork_attempt($attempt, $hwork, $cm, $course);
    }

    /**
     * Static function to create a new hwork_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new hwork_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return hwork_attempt_state_name($state);
    }
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'hwork');
                }
    ...
    // Simple getters ==========================================================
    public function get_hwork() {
        return $this->hworkobj->get_hwork();
    }

    public function get_hworkobj() {
        return $this->hworkobj;
    }

    /** @return int the course id. */
    public function get_courseid() {
        return $this->hworkobj->get_courseid();
    }

    /** @return int the course id. */
    public function get_course() {
        return $this->hworkobj->get_course();
    }

    /** @return int the hwork id. */
    public function get_hworkid() {
        return $this->hworkobj->get_hworkid();
    }

    /** @return string the name of this hwork. */
    public function get_hwork_name() {
        return $this->hworkobj->get_hwork_name();
    }

    /** @return int the hwork navigation method. */
    public function get_navigation_method() {
        return $this->hworkobj->get_navigation_method();
    }

    /** @return object the course_module object. */
    public function get_cm() {
        return $this->hworkobj->get_cm();
    }

    /** @return object the course_module object. */
    public function get_cmid() {
        return $this->hworkobj->get_cmid();
    }
     ...
     */
    public function is_preview_user() {
        return $this->hworkobj->is_preview_user();
    }

    /** @return int the number of attempts allowed at this hwork (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->hworkobj->get_num_attempts_allowed();
    }

    /** @return int number fo pages in this hwork. */
     ...
     */
    public function get_access_manager($timenow) {
        return $this->hworkobj->get_access_manager($timenow);
    }
    ...
    /** @return object the row from the hwork_attempts table. */
    ...
    /** @return string one of the hwork_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/hwork:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the hwork in a non-trivial way?
     ...
     * To see how this works, try setting a hwork to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the hwork
     ...
     *
     * @param question_display_options $options the display options for this hwork attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return hwork_feedback_for_grade($grade, $this->get_hwork(),
                $this->hworkobj->get_context());
    }
     ...
     */
    public function has_capability($capability, $userid = null, $doanything = true) {
        return $this->hworkobj->has_capability($capability, $userid, $doanything);
    }
     ...
     */
    public function require_capability($capability, $userid = null, $doanything = true) {
        return $this->hworkobj->require_capability($capability, $userid, $doanything);
    }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_hwork_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/hwork:attempt';
        } else {
            $capability = 'mod/hwork:reviewmyattempts';
        }

        // These next tests are in a slighly funny order. The point is that the
        // common and most performance-critical case is students attempting a hwork
        // so we want to check that permisison first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the hwork as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/hwork:viewreports') ||
                $this->has_capability('mod/hwork:preview')) {
    ...
    public function get_attempt_state() {
        return hwork_attempt_state($this->get_hwork(), $this->attempt);
    }

    /**
     * Wrapper that the correct mod_hwork_display_options for this hwork at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = hwork_get_review_options($this->get_hwork(),
                        $this->attempt, $this->hworkobj->get_context());
            ...
            $options = mod_hwork_display_options::make_from_hwork($this->get_hwork(),
                    mod_hwork_display_options::DURING);
            $options->flags = hwork_get_flag_option($this->attempt, $this->hworkobj->get_context());
            return $options;
        }
    }
     ...
     * @return bool true if this is the last page of the hwork.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the hwork, or for the
     * whole hwork.
     *
    ...
    public function get_question_usage() {
        if (!PHPUNIT_TEST) {
            throw new coding_exception('get_question_usage is only for use in unit tests. ' .
                    'For other operations, use the hwork_attempt api, or extend it properly.');
        }
        return $this->quba;
    }
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_hwork()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the hwork where this question appears.
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the hwork this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the hwork.
     */
    public function get_question_mark($slot) {
        return hwork_format_question_grade($this->get_hwork(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = array();
        if ($this->hworkobj->get_hwork()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->hworkobj->get_hwork()->timelimit;
        }
        if ($this->hworkobj->get_hwork()->timeclose) {
            $deadlines[] = $this->hworkobj->get_hwork()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->hworkobj->get_hwork()->graceperiod;
    ...
    // URLs related to this attempt ============================================
    /**
     * @return string hwork view url.
     */
    public function view_url() {
        return $this->hworkobj->view_url();
    }
    ...
    public function start_attempt_url($slot = null, $page = -1) {
        ...
        return $this->hworkobj->start_attempt_url($page);
    }
    ...
    /**
     * @return string the URL of this hwork's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/hwork/summary.php', array('attempt' => $this->attempt->id, 'cmid' => $this->get_cmid()));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/hwork/processattempt.php');
    }
    ...
    public function cannot_review_message($short = false) {
        return $this->hworkobj->cannot_review_message(
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'hwork'));
        } else {
            return '';
        }
    }
     ...
     * @param mod_hwork_renderer $renderer the hwork renderer.
     ...
     */
    public function render_question($slot, $reviewing, mod_hwork_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions,
                    ...
                    'mod_hwork-blocked_question_warning');
        }
     ...
     */
    protected function render_question_helper($slot, $reviewing, $thispageurl, mod_hwork_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'hwork', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'hwork');
        ...
        $placeholderqa->start($this->get_hwork()->preferredbehaviour, 1);
     ...
     *
     * @param int $id the id of a question in this hwork attempt.
     ...
     * @return string HTML for the question in its current state.
     */
    public function render_question_at_step($slot, $seq, $reviewing, mod_hwork_renderer $renderer, $thispageurl = '') {
     ...
     *
     * @param $panelclass The type of panel, hwork_attempt_nav_panel or hwork_review_nav_panel
     * @param $page the current page number.
     * @param $showall whether we are showing the whole hwork on one page. (Used by review.php)
     * @return hwork_nav_panel_base the requested object.
     */
    public function get_navigation_panel(mod_hwork_renderer $output,
             $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_hwork_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->attributes['aria-labelledby'] = 'mod_hwork_navblock_title';
        $bc->title = html_writer::span(get_string('hworknavigation', 'hwork'), '', array('id' => 'mod_hwork_navblock_title'));
        $bc->content = $output->navigation_panel($panel);
        return $bc;
    }

    /**
     * Return an array of variant URLs to other attempts at this hwork.
     *
     * The $url passed in must contain an attempt parameter.
     *
     * The {@link mod_hwork_links_to_other_attempts} object returned contains an
     ...
     * @return mod_hwork_links_to_other_attempts containing array int => null|moodle_url.
     */
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = hwork_get_user_attempts($this->get_hwork()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }

        $links = new mod_hwork_links_to_other_attempts();
     ...
     * @return mod_hwork_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->hworkobj->get_hwork()->graceperiod;
        ...
        // Otherwise, we were in hwork_attempt::IN_PROGRESS, and time has now expired.
        // Transition to the appropriate state.
        switch ($this->hworkobj->get_hwork()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if ($becomingoverdue) {
            ...
            $DB->update_record('hwork_attempts', $this->attempt);
        }

        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            hwork_save_best_grade($this->get_hwork(), $this->get_userid());
        }
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_hwork\question\qubaids_for_users_attempts(
                $this->get_hworkid(), $this->get_userid());
        ...
        if ($questiondata->qtype != 'random') {
            ...
            $tagids = hwork_retrieve_slot_tag_ids($this->slots[$slot]->id);
            ...
            if ($newqusetionid === null) {
                throw new moodle_exception('notenoughrandomquestions', 'hwork',
                        $this->hworkobj->view_url(), $questiondata);
            }
        }
    ...
    public function process_finish($timestamp, $processsubmitted) {
        ...
        if (!$this->is_preview()) {
            hwork_save_best_grade($this->get_hwork(), $this->attempt->userid);

            // Trigger event.
            $this->fire_state_transition_event('\mod_hwork\event\attempt_submitted', $timestamp);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('hwork_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_hwork\event\attempt_becameoverdue', $timestamp);

        $transaction->allow_commit();

        hwork_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_hwork\event\attempt_abandoned', $timestamp);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp) {
        global $USER;
        $hworkrecord = $this->get_hwork();
        $params = array(
            'context' => $this->get_hworkobj()->get_context(),
            'courseid' => $this->get_courseid(),
            'objectid' => $this->attempt->id,
            'relateduserid' => $this->attempt->userid,
            'other' => array(
                'submitterid' => CLI_SCRIPT ? null : $USER->id,
                'hworkid' => $hworkrecord->id
            )
        );

        $event = $eventclass::create($params);
        $event->add_record_snapshot('hwork', $this->get_hwork());
        $event->add_record_snapshot('hwork_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    /**
     * Get a URL for a particular question on a particular page of the hwork.
     * Used by {@link attempt_url()} and {@link review_url()}.
     *
     * @param string $script. Used in the URL like /mod/hwork/$script.php
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/hwork/' . $script . '.php' . $fragment,
    ...
    /**
     * Process responses during an attempt at a hwork.
     *
    ...
    public function process_attempt($timenow, $finishattempt, $timeup, $thispage) {
        ...
        // to show the student another page of the hwork. Just finish now.
        $graceperiodmin = null;
        ...
        if ($timeclose !== false && $timenow > $timeclose - QUIZ_MIN_TIME_TO_CONTINUE) {
            $timeup = true;
            $graceperiodmin = get_config('hwork', 'graceperiodmin');
        ...
        if ($timeup) {
            if ($this->get_hwork()->overduehandling == 'graceperiod') {
                if (is_null($graceperiodmin)) {
                    $graceperiodmin = get_config('hwork', 'graceperiodmin');
                }
                if ($timenow > $timeclose + $this->get_hwork()->graceperiod + $graceperiodmin) {
        ...
        // Update the hwork attempt record.
        try {
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('hwork_attempts', 'currentpage', $page, array('id' => $this->get_attemptid()));
            return true;
        }
        return false;
    }
 ...
 * @since      Moodle 2.9
 */
class hwork_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class hwork_nav_question_button implements renderable {
 ...
 * @since      Moodle 2.0
 */
abstract class hwork_nav_panel_base {
    /** @var hwork_attempt */
    ...
    public function __construct(hwork_attempt $attemptobj,
    ...
    /**
     * Get the buttons and section headings to go in the hwork navigation block.
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            if ($heading = $this->attemptobj->get_heading_before_slot($slot)) {
                $buttons[] = new hwork_nav_section_heading(format_string($heading));
            }

            $qa = $this->attemptobj->get_question_attempt($slot);
            $showcorrectness = $this->options->correctness && $qa->has_marks();

            $button = new hwork_nav_question_button();
            $button->id          = 'hworknavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'hwork');
            }
            $buttons[] = $button;
        }
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'hwork');
        } else {
            return get_string('viewed', 'hwork');
        }
    }

    public function render_before_button_bits(mod_hwork_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_hwork_renderer $output);
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_hwork()->showuserpicture == QUIZ_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', array('id' => $this->attemptobj->get_userid()));
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_hwork()->showuserpicture == QUIZ_SHOWIMAGE_LARGE) {
...
/**
 * Specialisation of {@link hwork_nav_panel_base} for the attempt hwork page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class hwork_attempt_nav_panel extends hwork_nav_panel_base {
    ...
    public function render_before_button_bits(mod_hwork_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'hwork'),
                array('id' => 'hworknojswarning'));
    }

    public function render_end_bits(mod_hwork_renderer $output) {
        return html_writer::link($this->attemptobj->summary_url(),
                get_string('endtest', 'hwork'), array('class' => 'endtestlink')) .
...
/**
 * Specialisation of {@link hwork_nav_panel_base} for the review hwork page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class hwork_review_nav_panel extends hwork_nav_panel_base {
    public function get_question_url($slot) {
