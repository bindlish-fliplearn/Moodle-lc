...
/**
 * Classes to enforce the various access rules that can apply to a hwork.
 *
 * @package   mod_hwork
 ...
 * hwork, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
 */
class hwork_access_manager {
    /** @var hwork the hwork settings object. */
    protected $hworkobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of hwork_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular hwork.
     * @param object $hworkobj An instance of the class hwork from attemptlib.php.
     *      The hwork we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/hwork:ignoretimelimits', ...)).
     */
    public function __construct($hworkobj, $timenow, $canignoretimelimits) {
        $this->hworkobj = $hworkobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($hworkobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular hwork.
     * @param hwork $hworkobj information about the hwork in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/hwork:ignoretimelimits capability.
     * @return array of {@link hwork_access_rule_base}s.
     */
    protected function make_rules($hworkobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($hworkobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['hworkaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('hworkaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the hwork table.
     *
     * @param mod_hwork_mod_form $hworkform the hwork settings form that is being built.
    ...
    public static function add_settings_form_fields(
        ...
        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($hworkform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'hwork'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_hwork_mod_form $hworkform the hwork form object.
    ...
    public static function validate_settings_form_fields(array $errors,
        ...
        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $hworkform);
        }
    ...
    /**
     * Save any submitted settings when the hwork settings form is submitted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the hwork table.
     *
     * @param object $hwork the data from the hwork form, including $hwork->id
     *      which is the id of the hwork being saved.
     */
    public static function save_settings($hwork) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($hwork);
        }
    }

    /**
     * Delete any rule-specific settings when the hwork is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the hwork table.
     *
     * @param object $hwork the data from the database, including $hwork->id
     *      which is the id of the hwork being deleted.
     * @since Moodle 2.7.1, 2.6.4, 2.5.7
     */
    public static function delete_settings($hwork) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($hwork);
        }
    }
     ...
     */
    protected static function get_load_sql($hworkid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{hwork} hwork';
        $allparams = array('hworkid' => $hworkid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($hworkid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE hwork.id = :hworkid", $allparams);
    }
     ...
     */
    public static function load_settings($hworkid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($hworkid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($hworkid);
        }
    ...
    /**
     * Load the hwork settings and any settings required by the access rules.
     ...
     *
     * @param int $hworkid the hwork id.
     * @return object mdl_hwork row with extra fields.
     */
    public static function load_hwork_and_settings($hworkid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($hworkid, $rules, 'hwork.*');
        $hwork = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($hworkid) as $name => $value) {
                $hwork->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this hwork, such
     * as is shown at the top of the hwork view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this hwork now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_hwork_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        $method = 'post';
        if (!empty($_GET['_qf__mod_hwork_preflight_check_form'])) {
            $method = 'get';
        }
        return new mod_hwork_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'hworkobj' => $this->hworkobj,
     ...
     * hwork. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant hwork_attempts row.
    ...
    /**
     * @return bolean if this hwork should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the hwork view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_hwork_renderer $output the hwork renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->hworkobj->view_url(), $message);
            die();
        } else {
            redirect($this->hworkobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the hwork, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(hwork_attempt::IN_PROGRESS, hwork_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = hwork_attempt_state($this->hworkobj->get_hwork(), $attempt);
        $reviewoptions = mod_hwork_display_options::make_from_hwork(
                $this->hworkobj->get_hwork(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->hworkobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->hworkobj->review_url($attempt->id),
