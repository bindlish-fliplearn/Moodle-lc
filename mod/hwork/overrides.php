...
/**
 * This page handles listing of hwork overrides
 *
 * @package    mod_hwork
...
require_once($CFG->dirroot.'/mod/hwork/override_form.php');
...
$url = new moodle_url('/mod/hwork/overrides.php', array('cmid'=>$cm->id, 'mode'=>$mode));
...
require_capability('mod/hwork:manageoverrides', $context);

// Display a list of overrides.
$PAGE->set_pagelayout('admin');
$PAGE->set_title(get_string('overrides', 'hwork'));
...
$sql = 'SELECT o.id
            FROM {hwork_overrides} o LEFT JOIN {groups} g
            ON o.groupid = g.id
...
if (!empty($orphaned)) {
    $DB->delete_records_list('hwork_overrides', 'id', array_keys($orphaned));
}
...
if ($groupmode) {
    ...
    $sql = 'SELECT o.*, g.name
                ...
                WHERE o.hwork = :hworkid
                ORDER BY g.name';
    $params = array('hworkid' => $hwork->id);
} else {
...
$overridedeleteurl = new moodle_url('/mod/hwork/overridedelete.php');
$overrideediturl = new moodle_url('/mod/hwork/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    if (!$groupmode) {
        if (!has_capability('mod/hwork:attempt', $context, $override->userid)) {
            // User not allowed to take the hwork.
            $active = false;
    ...
    if (isset($override->timeopen)) {
        $fields[] = get_string('hworkopens', 'hwork');
        $values[] = $override->timeopen > 0 ?
                userdate($override->timeopen) : get_string('noopen', 'hwork');
    }

    // Format timeclose.
    if (isset($override->timeclose)) {
        $fields[] = get_string('hworkcloses', 'hwork');
        $values[] = $override->timeclose > 0 ?
                userdate($override->timeclose) : get_string('noclose', 'hwork');
    }

    // Format timelimit.
    if (isset($override->timelimit)) {
        $fields[] = get_string('timelimit', 'hwork');
        $values[] = $override->timelimit > 0 ?
                format_time($override->timelimit) : get_string('none', 'hwork');
    }

    // Format number of attempts.
    if (isset($override->attempts)) {
        $fields[] = get_string('attempts', 'hwork');
    ...
    if (isset($override->password)) {
        $fields[] = get_string('requirepassword', 'hwork');
        $values[] = $override->password !== '' ?
                get_string('enabled', 'hwork') : get_string('none', 'hwork');
    }
...
echo html_writer::start_tag('div', array('id' => 'hworkoverrides'));
if (count($table->data)) {
    echo html_writer::table($table);
}
if ($hasinactive) {
    echo $OUTPUT->notification(get_string('inactiveoverridehelp', 'hwork'), 'dimmed_text');
}
...
if ($groupmode) {
    if (empty($groups)) {
        // There are no groups.
        echo $OUTPUT->notification(get_string('groupsnone', 'hwork'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'addgroup', 'cmid' => $cm->id)),
            get_string('addnewgroupoverride', 'hwork'), 'post', $options);
} else {
    $users = array();
    // See if there are any students in the hwork.
    $users = get_users_by_capability($context, 'mod/hwork:attempt', 'u.id');
    $info = new \core_availability\info_module($cm);
    $users = $info->filter_user_list($users);

    if (empty($users)) {
        // There are no students.
        echo $OUTPUT->notification(get_string('usersnone', 'hwork'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'adduser', 'cmid' => $cm->id)),
            get_string('addnewuseroverride', 'hwork'), 'get', $options);
}
