...
/**
 * Rest endpoint for ajax editing of hwork structure.
 *
 * @package   mod_hwork
...
$PAGE->set_url('/mod/hwork/edit-rest.php',
        array('hworkid' => $hworkid, 'class' => $class));
...
switch($requestmethod) {
    ...
    case 'GET': // For debugging.
        switch ($class) {
            case 'section':
                $table = 'hwork_sections';
                $section = $structure->get_section_by_id($id);
                switch ($field) {
                    case 'getsectiontitle':
                        require_capability('mod/hwork:manage', $modcontext);
            ...
            case 'resource':
                switch ($field) {
                    ...
                    case 'getmaxmark':
                        require_capability('mod/hwork:manage', $modcontext);
                        $slot = $DB->get_record('hwork_slots', array('id' => $id), '*', MUST_EXIST);
                        $result = array('instancemaxmark' => hwork_format_question_grade($hwork, $slot->maxmark));
                        break;
                    ...
                    case 'updatemaxmark':
                        ...
                        $result = array('instancemaxmark' => hwork_format_question_grade($hwork, $maxmark),
                                'newsummarks' => hwork_format_grade($hwork, $hwork->sumgrades));
                        break;
                    ...
                    case 'deletemultiple':
                        ...
                        $ids = explode(',', $ids);
                        foreach ($ids as $id) {
                            $slot = $DB->get_record('hwork_slots', array('hworkid' => $hwork->id, 'id' => $id),
                                    '*', MUST_EXIST);
                            if (hwork_has_question_use($hwork, $slot->slot)) {
                        ...
                        $result = array('newsummarks' => hwork_format_grade($hwork, $hwork->sumgrades),
    ...
    case 'DELETE':
        switch ($class) {
            ...
            case 'resource':
                require_capability('mod/hwork:manage', $modcontext);
                if (!$slot = $DB->get_record('hwork_slots', array('hworkid' => $hwork->id, 'id' => $id))) {
