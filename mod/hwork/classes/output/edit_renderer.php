...
/**
 * Renderer outputting the hwork editing UI.
 *
 * @package mod_hwork
...
namespace mod_hwork\output;
defined('MOODLE_INTERNAL') || die();

use \mod_hwork\structure;
use \html_writer;
use renderable;
...
class edit_renderer extends \plugin_renderer_base {
     ...
     *
     * @param \hwork $hworkobj object containing all the hwork settings information.
     * @param structure $structure object containing the structure of the hwork.
     * @param \question_edit_contexts $contexts the relevant question bank contexts.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @param array $pagevars the variables from {@link question_edit_setup()}.
     * @return string HTML to output.
     */
    public function edit_page(\hwork $hworkobj, structure $structure,
            \question_edit_contexts $contexts, \moodle_url $pageurl, array $pagevars) {
        $output = '';

        // Page title.
        $output .= $this->heading_with_help(get_string('editinghworkx', 'hwork',
                format_string($hworkobj->get_hwork_name())), 'editinghwork', 'hwork', '',
                get_string('basicideasofhwork', 'hwork'), 2);

        // Information at the top.
        $output .= $this->hwork_state_warnings($structure);

        $output .= html_writer::start_div('mod_hwork-edit-top-controls');
        $output .= $this->hwork_information($structure);
        $output .= $this->maximum_grade_input($structure, $pageurl);

        $output .= html_writer::start_div('mod_hwork-edit-action-buttons btn-group edit-toolbar', ['role' => 'group']);
        $output .= $this->repaginate_button($structure, $pageurl);
        $output .= $this->selectmultiple_button($structure);
        $output .= html_writer::end_tag('div');

        $output .= $this->total_marks($hworkobj->get_hwork());
        ...
        if ($structure->can_be_edited()) {
            ...
            $this->page->requires->js_call_amd('mod_hwork/hworkquestionbank', 'init', [
                $thiscontext->id
            ]);

            $this->page->requires->js_call_amd('mod_hwork/add_random_question', 'init', [
                $thiscontext->id,
                $pagevars['cat'],
    ...
    /**
     * Render any warnings that might be required about the state of the hwork,
     * e.g. if it has been attempted, or if the shuffle questions option is
     * turned on.
     *
     * @param structure $structure the hwork structure.
     * @return string HTML to output.
     */
    public function hwork_state_warnings(structure $structure) {
     ...
     * @return string HTML to output.
     */
    public function hwork_information(structure $structure) {
        list($currentstatus, $explanation) = $structure->get_dates_summary();

        $output = html_writer::span(
                    get_string('numquestionsx', 'hwork', $structure->get_question_count()),
                    'numberofquestions') . ' | ' .
                html_writer::span($currentstatus, 'hworkopeningstatus',
    ...
    /**
     * Render the form for setting a hwork' overall grade
     *
     * @param structure $structure the hwork structure.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
    ...
    public function maximum_grade_input($structure, \moodle_url $pageurl) {
        ...
        $output .= html_writer::start_tag('form', array('method' => 'post', 'action' => 'edit.php',
                'class' => 'hworksavegradesform form-inline'));
     ...
     * @param structure $structure the structure of the hwork being edited.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     */
    protected function repaginate_button(structure $structure, \moodle_url $pageurl) {

        $header = html_writer::tag('span', get_string('repaginatecommand', 'hwork'), array('class' => 'repaginatecommand'));
        ...
        $buttonoptions = array(
            ...
            'value' => get_string('repaginatecommand', 'hwork'),
        ...
        if (!$structure->can_be_repaginated()) {
            ...
            $this->page->requires->yui_module('moodle-mod_hwork-repaginate', 'M.mod_hwork.repaginate.init');
        }
    ...
    protected function selectmultiple_button(structure $structure) {
        $buttonoptions = array(
            ...
            'value' => get_string('selectmultipleitems', 'hwork'),
            'class' => 'btn btn-secondary m-b-1'
        );
        if (!$structure->can_be_edited()) {
            $buttonoptions['disabled'] = 'disabled';
        ...
        return html_writer::tag('button', get_string('selectmultipleitems', 'hwork'), $buttonoptions);
    }
    ...
    protected function selectmultiple_controls(structure $structure) {
        ...
        $buttondeleteoptions = array(
            ...
            'value' => get_string('deleteselected', 'mod_hwork'),
        ...
        $output .= html_writer::tag('div',
                        html_writer::tag('button', get_string('deleteselected', 'mod_hwork'), $buttondeleteoptions) .
                        " " .
        ...
        $toolbaroptions = array(
            'class' => 'btn-toolbar',
            'role' => 'toolbar',
            'aria-label' => get_string('selectmultipletoolbar', 'hwork'),
        );
        ...
        $output .= html_writer::tag('div',
                html_writer::tag('div',
                        html_writer::link('#', get_string('selectall', 'hwork'), $buttonselectalloptions) .
                        html_writer::tag('span', "/", ['class' => 'separator']) .
                        html_writer::link('#', get_string('selectnone', 'hwork'), $buttondeselectalloptions),
    ...
    protected function repaginate_form(structure $structure, \moodle_url $pageurl) {
        $perpage = array();
        $perpage[0] = get_string('allinone', 'hwork');
        for ($i = 1; $i <= 50; ++$i) {
            $perpage[$i] = $i;
        }
        ...
        $formcontent = html_writer::tag('form', html_writer::div(
                    ...
                    get_string('repaginate', 'hwork', $select) .
    ...
    /**
     * Render the total marks available for the hwork.
     *
     * @param \stdClass $hwork the hwork settings from the database.
     * @return string HTML to output.
     */
    public function total_marks($hwork) {
        $totalmark = html_writer::span(hwork_format_grade($hwork, $hwork->sumgrades), 'mod_hwork_summarks');
        return html_writer::tag('span',
                get_string('totalmarksx', 'hwork', $totalmark),
     ...
     *
     * @param structure $structure the structure of the hwork being edited.
     * @param \stdClass $section The hwork_section entry from DB
     * @return string HTML to output.
     */
    protected function start_section($structure, $section) {
        ...
        if (!$structure->can_be_edited()) {
            ...
            $editsectionheadingicon = html_writer::link(new \moodle_url('#'),
                $this->pix_icon('t/editstring', get_string('sectionheadingedit', 'hwork', $section->heading),
     ...
     *
     * @param structure $structure object containing the structure of the hwork.
     * @param \stdClass $section data from the hwork_section table.
     * @return string HTML to output.
     */
    public function section_shuffle_questions(structure $structure, $section) {
        ...
        if ($structure->is_first_section($section)) {
            $help = $this->help_icon('shufflequestions', 'hwork');
        } else {
            $help = '';
        }
        ...
        $label = html_writer::label(get_string('shufflequestions', 'hwork'),
    ...
    /**
     * Render an icon to remove a section from the hwork.
     *
     * @param object $section the section to be removed.
     * @return string HTML to output.
     */
    public function section_remove_icon($section) {
        $title = get_string('sectionheadingremove', 'hwork', $section->heading);
        $url = new \moodle_url('/mod/hwork/edit.php',
    ...
    /**
     * Renders HTML to display the questions in a section of the hwork.
     *
     * This function calls {@link core_course_renderer::hwork_section_question()}
     *
     * @param structure $structure object containing the structure of the hwork.
    ...
    public function question_row(structure $structure, $slot, $contexts, $pagevars, $pageurl) {
        ...
        $joinhtml = '';
        if ($structure->can_be_edited() && !$structure->is_last_slot_in_hwork($slot) &&
    ...
    public function add_menu_actions(structure $structure, $page, \moodle_url $pageurl,
        ...
        $menu->set_constraint('.mod-hwork-edit-content');
        $trigger = html_writer::tag('span', get_string('add', 'hwork'), array('class' => 'add-menu'));
        ...
        // Disable the link if hwork has attempts.
        if (!$structure->can_be_edited()) {
    ...
    public function edit_menu_actions(structure $structure, $page,
        ...
        if (!isset($str)) {
            $str = get_strings(array('addasection', 'addaquestion', 'addarandomquestion',
                    'addarandomselectedquestion', 'questionbank'), 'hwork');
        }

        // Get section, page, slotnumber and maxmark.
        $actions = array();

        // Add a new question to the hwork.
        ...
        if ($page) {
            $title = get_string('addquestionfrombanktopage', 'hwork', $page);
        } else {
            $title = get_string('addquestionfrombankatend', 'hwork');
        }
        ...
        $returnurl = new \moodle_url('/mod/hwork/edit.php', array('cmid' => $structure->get_cmid(), 'data-addonpage' => $page));
        $params = array('returnurl' => $returnurl, 'cmid' => $structure->get_cmid(), 'appendqnumstring' => 'addarandomquestion');
        $url = new \moodle_url('/mod/hwork/addrandom.php', $params);
        ...
        if ($page) {
            $title = get_string('addrandomquestiontopage', 'hwork', $page);
        } else {
            $title = get_string('addrandomquestionatend', 'hwork');
        }
    ...
    /**
     * Render the form that contains the data for adding a new question to the hwork.
     *
     * @param structure $structure object containing the structure of the hwork.
     * @param int $page the page number that this menu will add to.
     * @param \moodle_url $pageurl the canonical URL of this page.
    ...
    public function question(structure $structure, $slot, \moodle_url $pageurl) {
        ...
        // Action icons.
        $questionicons = '';
        $questionicons .= $this->question_preview_icon($structure->get_hwork(), $structure->get_question_in_slot($slot));
        if ($structure->can_be_edited()) {
     ...
     *
     * @param \stdClass $hwork the hwork settings from the database.
     * @param \stdClass $question data from the question and hwork_slots tables.
     * @param bool $label if true, show the preview question label after the icon
     * @param int $variant which question variant to preview (optional).
     * @return string HTML to output.
     */
    public function question_preview_icon($hwork, $question, $label = null, $variant = null) {
        $url = hwork_question_preview_url($hwork, $question, $variant);

        // Do we want a label?
        $strpreviewlabel = '';
        if ($label) {
            $strpreviewlabel = ' ' . get_string('preview', 'hwork');
        }

        // Build the icon.
        $strpreviewquestion = get_string('previewquestion', 'hwork');
    ...
    /**
     * Render an icon to remove a question from the hwork.
     *
     * @param structure $structure object containing the structure of the hwork.
     * @param int $slot the first slot on the page we are outputting.
    ...
    /**
     * Display an icon to split or join two pages of the hwork.
     *
     * @param structure $structure object containing the structure of the hwork.
     * @param int $slot the first slot on the page we are outputting.
     * @return string HTML to output.
    ...
    public function page_split_join_button($structure, $slot) {
        ...
        $url = new \moodle_url('repaginate.php', array('hworkid' => $structure->get_hworkid(),
                'slot' => $slot, 'repag' => $insertpagebreak ? 2 : 1, 'sesskey' => sesskey()));

        if ($insertpagebreak) {
            $title = get_string('addpagebreak', 'hwork');
            $image = $this->image_icon('e/insert_page_break', $title);
            $action = 'addpagebreak';
        } else {
            $title = get_string('removepagebreak', 'hwork');
    ...
    public function question_dependency_icon($structure, $slot) {
        ...
        if ($structure->is_question_dependent_on_previous_slot($slot)) {
            $title = get_string('questiondependencyremove', 'hwork', $a);
            $image = $this->pix_icon('t/locked', get_string('questiondependsonprevious', 'hwork'),
                    'moodle', array('title' => ''));
            $action = 'removedependency';
        } else {
            $title = get_string('questiondependencyadd', 'hwork', $a);
            $image = $this->pix_icon('t/unlocked', get_string('questiondependencyfree', 'hwork'),
                    'moodle', array('title' => ''));
    ...
    /**
     * Renders html to display a name with the link to the question on a hwork edit page
     *
    ...
    public function question_name(structure $structure, $slot, $pageurl) {
        ...
        $instancename = hwork_question_tostring($question);
        ...
        $output .= html_writer::link($editurl, $activitylink,
                array('title' => get_string('editquestion', 'hwork').' '.$title));
    ...
    public function random_question(structure $structure, $slotnumber, $pageurl) {
        ...
        $editurl = new \moodle_url('/mod/hwork/editrandom.php',
                array('returnurl' => $pageurl->out_as_local_url(), 'slotid' => $slot->id));

        $temp = clone($question);
        $temp->questiontext = '';
        $instancename = hwork_question_tostring($temp);

        $configuretitle = get_string('configurerandomquestion', 'hwork');
        $qtype = \question_bank::get_qtype($question->qtype, false);
        $namestr = $qtype->local_name();
        $icon = $this->pix_icon('icon', $namestr, $qtype->plugin_name(), array('title' => $namestr,
                'class' => 'icon activityicon', 'alt' => ' ', 'role' => 'presentation'));
        ...
        $qbanklink = ' ' . \html_writer::link($qbankurl,
                get_string('seequestions', 'hwork'), array('class' => 'mod_hwork_random_qbank_link'));
    ...
    public function marked_out_of_field(structure $structure, $slot) {
        ...
        $output = html_writer::span($structure->formatted_question_grade($slot),
                'instancemaxmark decimalplaces_' . $structure->get_decimal_places_for_question_marks(),
                array('title' => get_string('maxmark', 'hwork')));
        ...
        $output .= html_writer::span(
            html_writer::link(
                ...
                array(
                    ...
                    'title' => get_string('editmaxmark', 'hwork'),
                )
            )
        );
    ...
    public function render_question_chooser(renderable $chooser) {
        return $this->render_from_template('mod_hwork/question_chooser', $chooser->export_for_template($this));
    }
    ...
    public function question_chooser() {
        $chooser = \mod_hwork\output\question_chooser::get($this->page->course, [], null);
    ...
    protected function initialise_editing_javascript(structure $structure,
        ...
        $config = new \stdClass();
        $config->resourceurl = '/mod/hwork/edit_rest.php';
        $config->sectionurl = '/mod/hwork/edit_rest.php';
        ...
        $this->page->requires->yui_module('moodle-mod_hwork-toolboxes',
                'M.mod_hwork.init_resource_toolbox',
                array(array(
                        'courseid' => $structure->get_courseid(),
                        'hworkid' => $structure->get_hworkid(),
        ...
        $this->page->requires->strings_for_js(array('areyousureremoveselected'), 'hwork');
        $this->page->requires->yui_module('moodle-mod_hwork-toolboxes',
                'M.mod_hwork.init_section_toolbox',
                array(array(
                        'courseid' => $structure,
                        'hworkid' => $structure->get_hworkid(),
        ...
        $this->page->requires->yui_module('moodle-mod_hwork-dragdrop', 'M.mod_hwork.init_section_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'hworkid' => $structure->get_hworkid(),
                        'ajaxurl' => $config->sectionurl,
        ...
        $this->page->requires->yui_module('moodle-mod_hwork-dragdrop', 'M.mod_hwork.init_resource_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'hworkid' => $structure->get_hworkid(),
                        'ajaxurl' => $config->resourceurl,
    ...
    protected function new_page_template(structure $structure,
        ...
        $strcontexts[] = 'addonpage="';
        $strcontexts[] = get_string('addquestionfrombanktopage', 'hwork', '');
     ...
     *
     * @param \mod_hwork\question\bank\custom_view $questionbank the question bank view object.
     * @param array $pagevars the variables from {@link \question_edit_setup()}.
     * @return string HTML to output / send back in response to an AJAX request.
     */
    public function question_bank_contents(\mod_hwork\question\bank\custom_view $questionbank, array $pagevars) {
