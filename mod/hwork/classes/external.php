 ...
 * @since      Moodle 3.1
 */
class mod_hwork_external extends external_api {

    /**
     * Describes the parameters for get_hworkzes_by_courses.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_hworkzes_by_courses_parameters() {
    ...
    /**
     * Returns a list of hworkzes in a provided list of courses,
     * if no list is provided all hworkzes that the user can view will be returned.
     *
     * @param array $courseids Array of course ids
     * @return array of hworkzes details
     * @since Moodle 3.1
     */
    public static function get_hworkzes_by_courses($courseids = array()) {
        global $USER;

        $warnings = array();
        $returnedhworkzes = array();

        $params = array(
            'courseids' => $courseids,
        );
        $params = self::validate_parameters(self::get_hworkzes_by_courses_parameters(), $params);
        ...
        if (!empty($params['courseids'])) {
            ...
            // Get the hworkzes in this course, this function checks users visibility permissions.
            // We can avoid then additional validate_context calls.
            $hworkzes = get_all_instances_in_courses("hwork", $courses);
            foreach ($hworkzes as $hwork) {
                $context = context_module::instance($hwork->coursemodule);

                // Update hwork with override information.
                $hwork = hwork_update_effective_access($hwork, $USER->id);

                // Entry to return.
                $hworkdetails = array();
                // First, we return information that any user can see in the web interface.
                $hworkdetails['id'] = $hwork->id;
                $hworkdetails['coursemodule']      = $hwork->coursemodule;
                $hworkdetails['course']            = $hwork->course;
                $hworkdetails['name']              = external_format_string($hwork->name, $context->id);

                if (has_capability('mod/hwork:view', $context)) {
                    // Format intro.
                    list($hworkdetails['intro'], $hworkdetails['introformat']) = external_format_text($hwork->intro,
                                                                    $hwork->introformat, $context->id, 'mod_hwork', 'intro', null);

                    $hworkdetails['introfiles'] = external_util::get_area_files($context->id, 'mod_hwork', 'intro', false, false);
                    ...
                    $hasfeedback = hwork_has_feedback($hwork);
                    $hworkdetails['hasfeedback'] = (!empty($hasfeedback)) ? 1 : 0;

                    $timenow = time();
                    $hworkobj = hwork::create($hwork->id, $USER->id);
                    $accessmanager = new hwork_access_manager($hworkobj, $timenow, has_capability('mod/hwork:ignoretimelimits',
                                                                $context, null, false));

                    // Fields the user could see if have access to the hwork.
                    if (!$accessmanager->prevent_access()) {
                        $hworkdetails['hasquestions'] = (int) $hworkobj->has_questions();
                        $hworkdetails['autosaveperiod'] = get_config('hwork', 'autosaveperiod');
                    ...
                    foreach ($viewablefields as $field) {
                        $hworkdetails[$field] = $hwork->{$field};
                    }
                }
                $returnedhworkzes[] = $hworkdetails;
            }
        }
        $result = array();
        $result['hworkzes'] = $returnedhworkzes;
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_hworkzes_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_hworkzes_by_courses_returns() {
        return new external_single_structure(
            array(
                'hworkzes' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            'id' => new external_value(PARAM_INT, 'Standard Moodle primary key.'),
                            'course' => new external_value(PARAM_INT, 'Foreign key reference to the course this hwork is part of.'),
                            ...
                            'timeopen' => new external_value(PARAM_INT, 'The time when this hwork opens. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timeclose' => new external_value(PARAM_INT, 'The time when this hwork closes. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timelimit' => new external_value(PARAM_INT, 'The time limit for hwork attempts, in seconds.',
                                                                VALUE_OPTIONAL),
                            ...
                            'canredoquestions' => new external_value(PARAM_INT, 'Allows students to redo any completed question
                                                                        within a hwork attempt.', VALUE_OPTIONAL),
                            ...
                            'reviewattempt' => new external_value(PARAM_INT, 'Whether users are allowed to review their hwork
                                                                    attempts at various times. This is a bit field, decoded by the
                                                                    mod_hwork_display_options class. It is formed by ORing together
                                                                    the constants defined there.', VALUE_OPTIONAL),
                            'reviewcorrectness' => new external_value(PARAM_INT, 'Whether users are allowed to review their hwork
                                                                        attempts at various times.
                                                                        A bit field, like reviewattempt.', VALUE_OPTIONAL),
                            'reviewmarks' => new external_value(PARAM_INT, 'Whether users are allowed to review their hwork attempts
                            ...
                            'sumgrades' => new external_value(PARAM_FLOAT, 'The total of all the question instance maxmarks.',
                                                                VALUE_OPTIONAL),
                            'grade' => new external_value(PARAM_FLOAT, 'The total that the hwork overall grade is scaled to be
                                                            out of.', VALUE_OPTIONAL),
                            'timecreated' => new external_value(PARAM_INT, 'The time when the hwork was added to the course.',
                                                                VALUE_OPTIONAL),
                            ...
                            'password' => new external_value(PARAM_RAW, 'A password that the student must enter before starting or
                                                                continuing a hwork attempt.', VALUE_OPTIONAL),
                            'subnet' => new external_value(PARAM_RAW, 'Used to restrict the IP addresses from which this hwork can
                            ...
                            'completionattemptsexhausted' => new external_value(PARAM_INT, 'Mark hwork complete when the student has
                            ...
                            'allowofflineattempts' => new external_value(PARAM_INT, 'Whether to allow the hwork to be attempted
                                                                            offline in the mobile app', VALUE_OPTIONAL),
                            'autosaveperiod' => new external_value(PARAM_INT, 'Auto-save delay', VALUE_OPTIONAL),
                            'hasfeedback' => new external_value(PARAM_INT, 'Whether the hwork has any non-blank feedback text',
                                                                VALUE_OPTIONAL),
                            'hasquestions' => new external_value(PARAM_INT, 'Whether the hwork has questions', VALUE_OPTIONAL),
    ...
    /**
     * Utility function for validating a hwork.
     *
     * @param int $hworkid hwork instance id
     * @return array array containing the hwork, course, context and course module objects
     * @since  Moodle 3.1
     */
    protected static function validate_hwork($hworkid) {
        global $DB;

        // Request and permission validation.
        $hwork = $DB->get_record('hwork', array('id' => $hworkid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($hwork, 'hwork');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        return array($hwork, $course, $cm, $context);
    }

    /**
     * Describes the parameters for view_hwork.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function view_hwork_parameters() {
        return new external_function_parameters (
            array(
                'hworkid' => new external_value(PARAM_INT, 'hwork instance id'),
            )
        );
    }
     ...
     */
    public static function view_hwork($hworkid) {
        global $DB;

        $params = self::validate_parameters(self::view_hwork_parameters(), array('hworkid' => $hworkid));
        $warnings = array();

        list($hwork, $course, $cm, $context) = self::validate_hwork($params['hworkid']);
    ...
    /**
     * Describes the view_hwork return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function view_hwork_returns() {
        return new external_single_structure(
            array(
    ...
    public static function get_user_attempts_parameters() {
        return new external_function_parameters (
            array(
                ...
                'status' => new external_value(PARAM_ALPHA, 'hwork status: all, finished or unfinished', VALUE_DEFAULT, 'finished'),
    ...
    /**
     * Return a list of attempts for the given hwork and user.
     *
     * @param int $hworkid hwork instance id
     * @param int $userid user id
     * @param string $status hwork status: all, finished or unfinished
     * @param bool $includepreviews whether to include previews or not
     * @return array of warnings and the list of attempts
     * @since Moodle 3.1
     * @throws invalid_parameter_exception
     */
    public static function get_user_attempts($hworkid, $userid = 0, $status = 'finished', $includepreviews = false) {
        global $DB, $USER;
        ...
        if ($USER->id != $user->id) {
            require_capability('mod/hwork:viewreports', $context);
        }

        $attempts = hwork_get_user_attempts($hwork->id, $user->id, $params['status'], $params['includepreviews']);
    ...
    private static function attempt_structure() {
        return new external_single_structure(
            array(
                ...
                'hwork' => new external_value(PARAM_INT, 'Foreign key reference to the hwork that was attempted.',
                                                VALUE_OPTIONAL),
                'userid' => new external_value(PARAM_INT, 'Foreign key reference to the user whose attempt this is.',
                                                VALUE_OPTIONAL),
                'attempt' => new external_value(PARAM_INT, 'Sequentially numbers this students attempts at this hwork.',
                                                VALUE_OPTIONAL),
                'uniqueid' => new external_value(PARAM_INT, 'Foreign key reference to the question_usage that holds the
                                                    details of the the question_attempts that make up this hwork
                ...
                'timecheckstate' => new external_value(PARAM_INT, 'Next time hwork cron should check attempt for
    ...
    /**
     * Get the best current grade for the given user on a hwork.
     *
     * @param int $hworkid hwork instance id
     * @param int $userid user id
     ...
     * @since Moodle 3.1
     */
    public static function get_user_best_grade($hworkid, $userid = 0) {
        global $DB, $USER;
        ...
        $result = array();
        $grade = hwork_get_best_grade($hwork, $user->id);
    ...
    public static function get_user_best_grade_returns() {
        return new external_single_structure(
            array(
                'hasgrade' => new external_value(PARAM_BOOL, 'Whether the user has a grade on the given hwork.'),
     ...
     * @since Moodle 3.1
     */
    public static function get_combined_review_options($hworkid, $userid = 0) {
        global $DB, $USER;
        ...
        $attempts = hwork_get_user_attempts($hwork->id, $user->id, 'all', true);
    ...
    /**
     * Starts a new attempt at a hwork.
     *
     * @param int $hworkid hwork instance id
     ...
     */
    public static function start_attempt($hworkid, $preflightdata = array(), $forcenew = false) {
        global $DB, $USER;
        ...
        // Check questions.
        if (!$hworkobj->has_questions()) {
            throw new moodle_hwork_exception($hworkobj, 'noquestionsfound');
        }

        // Create an object to manage all the other (non-roles) access rules.
        $timenow = time();
        $accessmanager = $hworkobj->get_access_manager($timenow);

        // Validate permissions for creating a new attempt and start a new preview attempt if required.
        list($currentattemptid, $attemptnumber, $lastattempt, $messages, $page) =
            hwork_validate_new_attempt($hworkobj, $accessmanager, $forcenew, -1, false);

        // Check access.
        if (!$hworkobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new moodle_hwork_exception($hworkobj, array_shift($errors));
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == hwork_attempt::OVERDUE) {
                    throw new moodle_hwork_exception($hworkobj, 'stateoverdue');
                } else {
                    throw new moodle_hwork_exception($hworkobj, 'attemptstillinprogress');
                }
            }
            $offlineattempt = WS_SERVER ? true : false;
            $attempt = hwork_prepare_and_start_new_attempt($hworkobj, $attemptnumber, $lastattempt, $offlineattempt);
        }
     ...
     * @param  bool $checkaccessrules whether to check the hwork access rules or not
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        $attemptobj = hwork_attempt::create($params['attemptid']);
        ...
        if ($attemptobj->get_userid() != $USER->id) {
            throw new moodle_hwork_exception($attemptobj->get_hworkobj(), 'notyourattempt');
        }
        ...
        if ($checkaccessrules) {
            ...
            if (!$ispreviewuser && $messages) {
                throw new moodle_hwork_exception($attemptobj->get_hworkobj(), 'attempterror');
            }
        }

        // Attempt closed?.
        if ($attemptobj->is_finished()) {
            throw new moodle_hwork_exception($attemptobj->get_hworkobj(), 'attemptalreadyclosed');
        } else if ($failifoverdue && $attemptobj->get_state() == hwork_attempt::OVERDUE) {
            throw new moodle_hwork_exception($attemptobj->get_hworkobj(), 'stateoverdue');
        }

        // User submitted data (like the hwork password).
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new moodle_hwork_exception($attemptobj->get_hworkobj(), array_shift($errors));
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new moodle_hwork_exception($attemptobj->get_hworkobj(), 'Invalid page number');
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new moodle_hwork_exception($attemptobj->get_hworkobj(), 'Out of sequence access');
            }
            ...
            if (empty($slots)) {
                throw new moodle_hwork_exception($attemptobj->get_hworkobj(), 'noquestionsfound');
            }
        }
     ...
     *
     * @return external_single_structure the question data. Some fields may not be returned depending on the hwork display settings.
     * @since  Moodle 3.1
    ...
    private static function question_structure() {
        return new external_single_structure(
            array(
                ...
                'page' => new external_value(PARAM_INT, 'page of the hwork this question appears on'),
                ...
                'number' => new external_value(PARAM_INT, 'question ordering number in the hwork', VALUE_OPTIONAL),
                'state' => new external_value(PARAM_ALPHA, 'the state where the question is in.
                    It will not be returned if the user cannot see it due to the hwork display correctness settings.',
                    VALUE_OPTIONAL),
            ...
            'The question data. Some fields may not be returned depending on the hwork display settings.'
        );
    }
     ...
     *
     * @param  hwork_attempt  $attemptobj  the hwork attempt object
     * @param  bool  $review  whether if we are in review mode or not
     * @param  mixed  $page  string 'all' or integer page number
     * @return array array of questions including data
     */
    private static function get_attempt_questions_data(hwork_attempt $attemptobj, $review, $page = 'all') {
        global $PAGE;

        $questions = array();
        $contextid = $attemptobj->get_hworkobj()->get_context()->id;
        $displayoptions = $attemptobj->get_display_options($review);
        $renderer = $PAGE->get_renderer('mod_hwork');
    ...
    /**
     * Returns information for the given attempt page for a hwork attempt in progress.
     *
     ...
     * @since Moodle 3.1
     * @throws moodle_hwork_exceptions
     */
        ...
        return new external_single_structure(
            array(
                ...
                'messages' => new external_multiple_structure(
                    ...
                    'access messages, will only be returned for users with mod/hwork:preview capability,
    ...
    /**
     * Returns a summary of a hwork attempt before it is submitted.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Processes save requests during the hwork. This function is intended for the hwork auto-save feature.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Process responses during an attempt at a hwork and also deals with attempts finishing.
     *
     * @param int $attemptid attempt id
     * @param array $data the data to be saved
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new moodle_hwork_exception($attemptobj->get_hworkobj(), 'attemptclosed');
    ...
    public static function get_attempt_review($attemptid, $page = -1) {
        ...
        foreach ($summarydata as $key => $data) {
            ...
            $result['additionaldata'][] = array(
                'id' => $key,
                'title' => $data['title'], $attemptobj->get_hworkobj()->get_context()->id,
                'content' => $data['content'],
            );
        }

        // Feedback if there is any, and the user is allowed to see it now.
        $grade = hwork_rescale_grade($attemptobj->get_attempt()->sumgrades, $attemptobj->get_hwork(), false);
    ...
    public static function get_attempt_review_returns() {
        return new external_single_structure(
            array(
                'grade' => new external_value(PARAM_RAW, 'grade for the hwork (or empty or "notyetgraded")'),
                'attempt' => self::attempt_structure(),
     ...
     * @since Moodle 3.1
     */
    public static function get_hwork_feedback_for_grade_parameters() {
        return new external_function_parameters (
            array(
                'hworkid' => new external_value(PARAM_INT, 'hwork instance id'),
    ...
    /**
     * Get the feedback text that should be show to a student who got the given grade in the given hwork.
     *
     * @param int $hworkid hwork instance id
     ...
     * @throws moodle_exception
     */
    public static function get_hwork_feedback_for_grade($hworkid, $grade) {
        global $DB;
        ...
        $params = self::validate_parameters(self::get_hwork_feedback_for_grade_parameters(), $params);
        $warnings = array();
        ...
        $feedback = hwork_feedback_record_for_grade($params['grade'], $hwork);
        if (!empty($feedback->feedbacktext)) {
            list($text, $format) = external_format_text($feedback->feedbacktext, $feedback->feedbacktextformat, $context->id,
                                                        'mod_hwork', 'feedback', $feedback->id);
            $result['feedbacktext'] = $text;
            $result['feedbacktextformat'] = $format;
            $feedbackinlinefiles = external_util::get_area_files($context->id, 'mod_hwork', 'feedback', $feedback->id);
    ...
    /**
     * Describes the get_hwork_feedback_for_grade return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_hwork_feedback_for_grade_returns() {
        return new external_single_structure(
            array(
    ...
    /**
     * Describes the parameters for get_hwork_access_information.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_hwork_access_information_parameters() {
        return new external_function_parameters (
            array(
                'hworkid' => new external_value(PARAM_INT, 'hwork instance id')
            )
    ...
    /**
     * Return access information for a given hwork.
     *
     * @param int $hworkid hwork instance id
     ...
     * @throws  moodle_hwork_exception
     */
    public static function get_hwork_access_information($hworkid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_hwork_access_information_parameters(), $params);

        list($hwork, $course, $cm, $context) = self::validate_hwork($params['hworkid']);

        $result = array();
        // Capabilities first.
        $result['canattempt'] = has_capability('mod/hwork:attempt', $context);;
        $result['canmanage'] = has_capability('mod/hwork:manage', $context);;
        $result['canpreview'] = has_capability('mod/hwork:preview', $context);;
        $result['canreviewmyattempts'] = has_capability('mod/hwork:reviewmyattempts', $context);;
        $result['canviewreports'] = has_capability('mod/hwork:viewreports', $context);;

        // Access manager now.
        $hworkobj = hwork::create($cm->instance, $USER->id);
        $ignoretimelimits = has_capability('mod/hwork:ignoretimelimits', $context, null, false);
        $timenow = time();
        $accessmanager = new hwork_access_manager($hworkobj, $timenow, $ignoretimelimits);
    ...
    /**
     * Describes the get_hwork_access_information return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_hwork_access_information_returns() {
        return new external_single_structure(
            array(
                'canattempt' => new external_value(PARAM_BOOL, 'Whether the user can do the hwork or not.'),
                'canmanage' => new external_value(PARAM_BOOL, 'Whether the user can edit the hwork settings or not.'),
                'canpreview' => new external_value(PARAM_BOOL, 'Whether the user can preview the hwork or not.'),
                'canreviewmyattempts' => new external_value(PARAM_BOOL, 'Whether the users can review their previous attempts
                                                                or not.'),
                'canviewreports' => new external_value(PARAM_BOOL, 'Whether the user can view the hwork reports or not.'),
    ...
    /**
     * Return access information for a given attempt in a hwork.
     *
     * @param int $hworkid hwork instance id
     ...
     * @throws  moodle_hwork_exception
     */
    public static function get_attempt_access_information($hworkid, $attemptid = 0) {
        global $DB, $USER;
        ...
        if ($unfinishedattempt = hwork_get_user_attempt_unfinished($hwork->id, $USER->id)) {
            ...
            if ($unfinishedattempt->state != hwork_attempt::IN_PROGRESS and $unfinishedattempt->state != hwork_attempt::OVERDUE) {
    ...
    /**
     * Describes the parameters for get_hwork_required_qtypes.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_hwork_required_qtypes_parameters() {
        return new external_function_parameters (
            array(
                'hworkid' => new external_value(PARAM_INT, 'hwork instance id')
            )
    ...
    /**
     * Return the potential question types that would be required for a given hwork.
     ...
     * @throws  moodle_hwork_exception
     */
    public static function get_hwork_required_qtypes($hworkid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_hwork_required_qtypes_parameters(), $params);
        ...
        // Question types used.
        $result = array();
        $result['questiontypes'] = $hworkobj->get_all_question_types_used(true);
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_hwork_required_qtypes return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_hwork_required_qtypes_returns() {
        return new external_single_structure(
            array(
                'questiontypes' => new external_multiple_structure(
                                    new external_value(PARAM_PLUGIN, 'question type'), 'list of question types used in the hwork'),
                'warnings' => new external_warnings(),
            )
        );
    }
