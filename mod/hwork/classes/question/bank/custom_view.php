...
/**
 * Defines the custom question bank view used on the Edit hwork page.
 *
 * @package   mod_hwork
 * @category  question
...
/**
 * Subclass to customise the view of the question bank for the hwork editing screen.
 *
 * @copyright  2009 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class custom_view extends \core_question\bank\view {
    /** @var bool whether the hwork this is used by has been attemptd. */
    protected $hworkhasattempts = false;
    /** @var \stdClass the hwork settings. */
    protected $hwork = false;
     ...
     * @param \stdClass $hwork hwork settings.
     */
    public function __construct($contexts, $pageurl, $course, $cm, $hwork) {
    ...
    protected function wanted_columns() {
        ...
        if (empty($CFG->hworkquestionbankcolumns)) {
            $hworkquestionbankcolumns = array(
                'add_action_column',
                'checkbox_column',
            ...
            $hworkquestionbankcolumns = explode(',', $CFG->hworkquestionbankcolumns);
        }

        foreach ($hworkquestionbankcolumns as $fullname) {
            if (!class_exists($fullname)) {
                if (class_exists('mod_hwork\\question\\bank\\' . $fullname)) {
                    $fullname = 'mod_hwork\\question\\bank\\' . $fullname;
                    ...
                    debugging('Legacy question bank column class question_bank_' .
                            $fullname . ' should be renamed to mod_hwork\\question\\bank\\' .
    ...
    protected function heading_column() {
        return 'mod_hwork\\question\\bank\\question_name_text_column';
    }

    protected function default_sort() {
        return array(
            'core_question\\bank\\question_type_column' => 1,
            'mod_hwork\\question\\bank\\question_name_text_column' => 1,
        );
    }

    /**
     * Let the question bank display know whether the hwork has been attempted,
     * hence whether some bits of UI, like the add this question to the hwork icon,
     * should be displayed.
     * @param bool $hworkhasattempts whether the hwork has attempts.
     */
    public function set_hwork_has_attempts($hworkhasattempts) {
        $this->hworkhasattempts = $hworkhasattempts;
        if ($hworkhasattempts && isset($this->visiblecolumns['addtohworkaction'])) {
            unset($this->visiblecolumns['addtohworkaction']);
        }
    }

    public function preview_question_url($question) {
        return hwork_question_preview_url($this->hwork, $question);
    }

    public function add_to_hwork_url($questionid) {
        global $CFG;
        $params = $this->baseurl->params();
        $params['addquestion'] = $questionid;
        $params['sesskey'] = sesskey();
        return new \moodle_url('/mod/hwork/edit.php', $params);
    }
    ...
    protected function display_bottom_controls($totalnumber, $recurse, $category, \context $catcontext, array $addcontexts) {
        ...
        $cmoptions->hasattempts = !empty($this->hworkhasattempts);
        ...
        if ($canuseall) {
            ...
            // Add selected questions to the hwork.
            $params = array(
                    'type' => 'submit',
                    'name' => 'add',
                    'class' => 'btn btn-primary',
                    'value' => get_string('addselectedquestionstohwork', 'hwork'),
            );
    ...
    protected function display_options_form($showquestiontext, $scriptpath = '/mod/hwork/edit.php',
    ...
    protected function print_category_info($category) {
        ...
        $strcategory = get_string('category', 'hwork');
