...
/**
 * Defines the \mod_hwork\structure class.
 *
 * @package   mod_hwork
 ...
 *
 * The structure of the hwork. That is, which questions it is built up
 * from. This is used on the Edit hwork page (edit.php) and also when
 * starting an attempt at the hwork (startattempt.php). Once an attempt
 ...
 */
class structure {
    /** @var \hwork the hwork this is the structure of. */
    protected $hworkobj = null;

    /**
     * @var \stdClass[] the questions in this hwork. Contains the row from the questions
     * table, with the data from the hwork_slots table added, and also question_categories.contextid.
     */
    protected $questions = array();

    /** @var \stdClass[] hwork_slots.id => the hwork_slots rows for this hwork, agumented by sectionid. */
    protected $slots = array();

    /** @var \stdClass[] hwork_slots.slot => the hwork_slots rows for this hwork, agumented by sectionid. */
    protected $slotsinorder = array();

    /**
     * @var \stdClass[] currently a dummy. Holds data that will match the
     * hwork_sections, once it exists.
     */
    ...
    /**
     * Create an instance of this class representing an empty hwork.
     * @return structure
     */
    ...
    /**
     * Create an instance of this class representing the structure of a given hwork.
     * @param \hwork $hworkobj the hwork.
     * @return structure
     */
    public static function create_for_hwork($hworkobj) {
        $structure = self::create();
        $structure->hworkobj = $hworkobj;
        $structure->populate_structure($hworkobj->get_hwork());
        return $structure;
    }

    /**
     * Whether there are any questions in the hwork.
     * @return bool true if there is at least one question in the hwork.
     */
    ...
    /**
     * Get the number of questions in the hwork.
     * @return int the number of questions in the hwork.
     */
     ...
     * question_category.contextid, and the hwork_slots data for the question in this hwork.
     */
     ...
     * questions cannot be depended upon because of hwork options.
     * @param int $slotnumber the index of the slot in question.
    ...
    public function can_finish_during_the_attempt($slotnumber) {
        if ($this->hworkobj->get_navigation_method() == QUIZ_NAVMETHOD_SEQ) {
            return false;
        }
        ...
        if (in_array($this->get_question_type_for_slot($slotnumber), array('random', 'missingtype'))) {
            return \question_engine::can_questions_finish_during_the_attempt(
                    $this->hworkobj->get_hwork()->preferredbehaviour);
        }
        ...
        try {
            $quba = \question_engine::make_questions_usage_by_activity('mod_hwork', $this->hworkobj->get_context());
            $tempslot = $quba->add_question(\question_bank::load_question(
                    $this->slotsinorder[$slotnumber]->questionid));
            $quba->set_preferred_behaviour($this->hworkobj->get_hwork()->preferredbehaviour);
    ...
    /**
     * Get the course id that the hwork belongs to.
     * @return int the course.id for the hwork.
     */
    ...
    /**
     * Get the course module id of the hwork.
     * @return int the course_modules.id for the hwork.
     */
    ...
    /**
     * Get id of the hwork.
     * @return int the hwork.id for the hwork.
     */
     ...
     * @return \stdClass the hwork settings row from the database.
     */
     ...
     * @return bool whether this hwork can be repaginated.
     */
     ...
     * @return bool whether the hwork can be edited.
     */
    public function can_be_edited() {
        if ($this->canbeedited === null) {
            $this->canbeedited = !hwork_has_attempts($this->hworkobj->get_hworkid());
        }
        return $this->canbeedited;
    }

    /**
     * This hwork can only be edited if they have not been attempted.
     * Throw an exception if this is not the case.
     */
    public function check_can_be_edited() {
        if (!$this->can_be_edited()) {
            $reportlink = hwork_attempt_summary_link_to_reports($this->get_hwork(),
                    $this->hworkobj->get_cm(), $this->hworkobj->get_context());
            throw new \moodle_exception('cannoteditafterattempts', 'hwork',
                    new \moodle_url('/mod/hwork/edit.php', array('cmid' => $this->get_cmid())), $reportlink);
        }
    }

    /**
     * How many questions are allowed per page in the hwork.
     * This setting controls how frequently extra page-breaks should be inserted
     * automatically when questions are added to the hwork.
     * @return int the number of questions that should be on each page of the
     * hwork by default.
     */
    public function get_questions_per_page() {
        return $this->hworkobj->get_hwork()->questionsperpage;
    }

    /**
     * Get hwork slots.
     * @return \stdClass[] the slots in this hwork.
     */
    ...
    /**
     * Is this slot the last one in the hwork?
     * @param int $slotnumber the index of the slot in question.
     * @return bool whether this slot the last one in the hwork.
     */
    public function is_last_slot_in_hwork($slotnumber) {
        end($this->slotsinorder);
        return $slotnumber == key($this->slotsinorder);
    }

    /**
     * Is this the first section in the hwork?
     * @param \stdClass $section the hwork_sections row.
     * @return bool whether this is first section in the hwork.
     */
    ...
    /**
     * Is this the last section in the hwork?
     * @param \stdClass $section the hwork_sections row.
     * @return bool whether this is first section in the hwork.
     */
    ...
    /**
     * Get the final slot in the hwork.
     * @return \stdClass the hwork_slots for for the final slot in the hwork.
     */
     ...
     * @return \stdClass the requested hwork_slots row.
     */
    ...
    /**
     * Get all the slots in a section of the hwork.
    ...
    /**
     * Get all the sections of the hwork.
     * @return \stdClass[] the sections in this hwork.
     */
    ...
    /**
     * Get the overall hwork grade formatted for display.
     * @return string the maximum grade for this hwork.
     */
    public function formatted_hwork_grade() {
        return hwork_format_grade($this->get_hwork(), $this->get_hwork()->grade);
    }
    ...
    public function formatted_question_grade($slotnumber) {
        return hwork_format_question_grade($this->get_hwork(), $this->slotsinorder[$slotnumber]->maxmark);
    }

    /**
     * Get the number of decimal places for displyaing overall hwork grades or marks.
     * @return int the number of decimal places.
     */
    public function get_decimal_places_for_grades() {
        return $this->get_hwork()->decimalpoints;
    }
    ...
    public function get_decimal_places_for_question_marks() {
        return hwork_get_grade_format($this->get_hwork());
    }
    ...
    public function get_edit_page_warnings() {
        ...
        if (hwork_has_attempts($this->hworkobj->get_hworkid())) {
            $reviewlink = hwork_attempt_summary_link_to_reports($this->hworkobj->get_hwork(),
                    $this->hworkobj->get_cm(), $this->hworkobj->get_context());
            $warnings[] = get_string('cannoteditafterattempts', 'hwork', $reviewlink);
        }
    ...
    /**
     * Get the date information about the current state of the hwork.
    ...
    public function get_dates_summary() {
        $timenow = time();
        $hwork = $this->hworkobj->get_hwork();

        // Exact open and close dates for the tool-tip.
        $dates = array();
        if ($hwork->timeopen > 0) {
            if ($timenow > $hwork->timeopen) {
                $dates[] = get_string('hworkopenedon', 'hwork', userdate($hwork->timeopen));
            } else {
                $dates[] = get_string('hworkwillopen', 'hwork', userdate($hwork->timeopen));
            }
        }
        if ($hwork->timeclose > 0) {
            if ($timenow > $hwork->timeclose) {
                $dates[] = get_string('hworkclosed', 'hwork', userdate($hwork->timeclose));
            } else {
                $dates[] = get_string('hworkcloseson', 'hwork', userdate($hwork->timeclose));
            }
        }
        if (empty($dates)) {
            $dates[] = get_string('alwaysavailable', 'hwork');
        }
        $explanation = implode(', ', $dates);

        // Brief summary on the page.
        if ($timenow < $hwork->timeopen) {
            $currentstatus = get_string('hworkisclosedwillopen', 'hwork',
                    userdate($hwork->timeopen, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($hwork->timeclose && $timenow <= $hwork->timeclose) {
            $currentstatus = get_string('hworkisopenwillclose', 'hwork',
                    userdate($hwork->timeclose, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($hwork->timeclose && $timenow > $hwork->timeclose) {
            $currentstatus = get_string('hworkisclosed', 'hwork');
        } else {
            $currentstatus = get_string('hworkisopen', 'hwork');
        }
    ...
    /**
     * Set up this class with the structure for a given hwork.
     * @param \stdClass $hwork the hwork settings.
     */
    public function populate_structure($hwork) {
        global $DB;
        ...
        // Get hwork sections in ascending order of the firstslot.
        $this->sections = $DB->get_records('hwork_sections', array('hworkid' => $hwork->id), 'firstslot ASC');
     ...
     * @param \stdClass[] $slots the data about the slots and questions in the hwork.
    ...
    protected function populate_missing_questions($slots) {
        ...
        foreach ($slots as $slot) {
            if ($slot->qtype === null) {
                ...
                $slot->name = get_string('missingquestion', 'hwork');
                $slot->slot = $slot->slot;
                $slot->maxmark = 0;
    ...
    protected function populate_question_numbers() {
        ...
        foreach ($this->slots as $slot) {
            if ($this->questions[$slot->questionid]->length == 0) {
                $slot->displayednumber = get_string('infoshort', 'hwork');
            } else {
    ...
    public function move_slot($idmove, $idmoveafter, $page) {
        ...
        if ($followingslotnumber == $movingslotnumber && !$this->is_last_slot_in_hwork($followingslotnumber)) {
        ...
        } else if (!$this->is_last_slot_in_hwork($moveafterslotnumber) &&
        ...
        if ($moveafterslotnumber > $movingslotnumber) {
            ...
            if ($this->is_last_slot_in_hwork($moveafterslotnumber) ||
        ...
        if ($slotreorder) {
            update_field_with_unique_index('hwork_slots', 'slot', $slotreorder,
                    array('hworkid' => $this->get_hworkid()));
        }

        // Page has changed. Record it.
        if ($movingslot->page != $page) {
            $DB->set_field('hwork_slots', 'page', $page,
                    array('id' => $movingslot->id));
        }

        // Update section fist slots.
        hwork_update_section_firstslots($this->get_hworkid(), $headingmovedirection,
        ...
        $emptypages = $DB->get_fieldset_sql("
                SELECT DISTINCT page - 1
                 ...
                 WHERE hworkid = ?
                   AND page > 1
                   AND NOT EXISTS (SELECT 1 FROM {hwork_slots} WHERE hworkid = ? AND page = slot.page - 1)
              ORDER BY page - 1 DESC
                ", array($this->get_hworkid(), $this->get_hworkid()));
    ...
    /**
     * Refresh page numbering of hwork slots.
    ...
    public function refresh_page_numbers($slots = array()) {
        ...
        if (!count($slots)) {
            $slots = $DB->get_records('hwork_slots', array('hworkid' => $this->get_hworkid()), 'slot, page');
        }
    ...
    /**
     * Refresh page numbering of hwork slots and save to the database.
     * @param \stdClass $hwork the hwork object.
     * @return \stdClass[] array of slot objects.
     */
    public function refresh_page_numbers_and_update_db() {
        ...
        foreach ($slots as $slot) {
            $DB->set_field('hwork_slots', 'page', $slot->page,
                    array('id' => $slot->id));
        }
    ...
    public function remove_slot($slotnumber) {
        ...
        $slot = $DB->get_record('hwork_slots', array('hworkid' => $this->get_hworkid(), 'slot' => $slotnumber));
        if (!$slot) {
            return;
        }
        $maxslot = $DB->get_field_sql('SELECT MAX(slot) FROM {hwork_slots} WHERE hworkid = ?', array($this->get_hworkid()));

        $trans = $DB->start_delegated_transaction();
        $DB->delete_records('hwork_slot_tags', array('slotid' => $slot->id));
        $DB->delete_records('hwork_slots', array('id' => $slot->id));
        for ($i = $slot->slot + 1; $i <= $maxslot; $i++) {
            $DB->set_field('hwork_slots', 'slot', $i - 1,
                    array('hworkid' => $this->get_hworkid(), 'slot' => $i));
        }
        ...
        hwork_update_section_firstslots($this->get_hworkid(), -1, $slotnumber);
     ...
     *
     * Saves changes to the question grades in the hwork_slots table and any
     * corresponding question_attempts.
     * It does not update 'sumgrades' in the hwork table.
     *
     * @param \stdClass $slot row from the hwork_slots table.
    ...
    public function update_slot_maxmark($slot, $maxmark) {
        ...
        $DB->update_record('hwork_slots', $slot);
        \question_engine::set_max_mark_in_attempts(new \qubaids_for_hwork($slot->hworkid),
                $slot->slot, $maxmark);
        $trans->allow_commit();
    ...
    public function update_question_dependency($slotid, $requireprevious) {
        global $DB;
        $DB->set_field('hwork_slots', 'requireprevious', $requireprevious, array('id' => $slotid));
    }

    /**
     * Add/Remove a pagebreak.
     *
     * Saves changes to the slot page relationship in the hwork_slots table and reorders the paging
    ...
    public function update_page_break($slotid, $type) {
        ...
        $hworkslots = $DB->get_records('hwork_slots', array('hworkid' => $this->get_hworkid()), 'slot');
        $repaginate = new \mod_hwork\repaginate($this->get_hworkid(), $hworkslots);
        $repaginate->repaginate_slots($hworkslots[$slotid]->slot, $type);
    ...
    public function add_section_heading($pagenumber, $heading = null) {
        ...
        if ($heading !== null) {
            ...
            $section->heading = get_string('newsectionheading', 'hwork');
        }
        $section->hworkid = $this->get_hworkid();
        $slotsonpage = $DB->get_records('hwork_slots', array('hworkid' => $this->get_hworkid(), 'page' => $pagenumber), 'slot DESC');
        $section->firstslot = end($slotsonpage)->slot;
        $section->shufflequestions = 0;
        return $DB->insert_record('hwork_sections', $section);
    }
    ...
    public function set_section_heading($id, $newheading) {
        global $DB;
        $section = $DB->get_record('hwork_sections', array('id' => $id), '*', MUST_EXIST);
        $section->heading = $newheading;
        $DB->update_record('hwork_sections', $section);
    }
    ...
    public function remove_section_heading($sectionid) {
        global $DB;
        $section = $DB->get_record('hwork_sections', array('id' => $sectionid), '*', MUST_EXIST);
        if ($section->firstslot == 1) {
            throw new \coding_exception('Cannot remove the first section in a hwork.');
        }
        $DB->delete_records('hwork_sections', array('id' => $sectionid));
    }
    ...
    protected function populate_slot_tags() {
        ...
        $this->slottags = hwork_retrieve_tags_for_slot_ids($slotids);
    }
