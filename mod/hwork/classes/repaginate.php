...
/**
 * Defines the hwork repaginate class.
 *
 * @package   mod_hwork
 ...
 *
 * The hwork setting allows users to write hworkzes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the hwork being manipulated. */
    private $hworkid;
    /** @var array the hwork_slots for that hwork. */
    private $slots;

    /**
     * Constructor.
     * @param int $hworkid the id of the hwork being manipulated.
     * @param stdClass[] $slots the hwork_slots for that hwork.
     */
    public function __construct($hworkid = 0, $slots = null) {
        ...
        if (!$slots) {
            $this->slots = $DB->get_records('hwork_slots', array('hworkid' => $this->hworkid), 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('hwork_slots', $nextslot, true);
    ...
    /**
     * Repaginate the rest.
     * @param stdClass[] $hworkslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($hworkslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$hworkslots) {
            return null;
        }
        $newslots = array();
        foreach ($hworkslots as $slot) {
            if ($type == self::LINK) {
