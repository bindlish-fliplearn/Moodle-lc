...
/**
 * Defines the \mod_pratest\structure class.
 *
 * @package   mod_pratest
...
namespace mod_pratest;
defined('MOODLE_INTERNAL') || die();

/**
 * Quiz structure class.
 *
 * The structure of the pratest. That is, which questions it is built up
 * from. This is used on the Edit pratest page (edit.php) and also when
 * starting an attempt at the pratest (startattempt.php). Once an attempt
 ...
 */
class structure {
    /** @var \pratest the pratest this is the structure of. */
    protected $pratestobj = null;

    /**
     * @var \stdClass[] the questions in this pratest. Contains the row from the questions
     * table, with the data from the pratest_slots table added, and also question_categories.contextid.
     */
    protected $questions = array();

    /** @var \stdClass[] pratest_slots.id => the pratest_slots rows for this pratest, agumented by sectionid. */
    protected $slots = array();

    /** @var \stdClass[] pratest_slots.slot => the pratest_slots rows for this pratest, agumented by sectionid. */
    protected $slotsinorder = array();

    /**
     * @var \stdClass[] currently a dummy. Holds data that will match the
     * pratest_sections, once it exists.
     */
    ...
    /**
     * Create an instance of this class representing an empty pratest.
     * @return structure
     */
    ...
    /**
     * Create an instance of this class representing the structure of a given pratest.
     * @param \pratest $pratestobj the pratest.
     * @return structure
     */
    public static function create_for_pratest($pratestobj) {
        $structure = self::create();
        $structure->pratestobj = $pratestobj;
        $structure->populate_structure($pratestobj->get_pratest());
        return $structure;
    }

    /**
     * Whether there are any questions in the pratest.
     * @return bool true if there is at least one question in the pratest.
     */
    ...
    /**
     * Get the number of questions in the pratest.
     * @return int the number of questions in the pratest.
     */
     ...
     * question_category.contextid, and the pratest_slots data for the question in this pratest.
     */
     ...
     * questions cannot be depended upon because of pratest options.
     * @param int $slotnumber the index of the slot in question.
    ...
    public function can_finish_during_the_attempt($slotnumber) {
        if ($this->pratestobj->get_navigation_method() == QUIZ_NAVMETHOD_SEQ) {
            return false;
        }
        ...
        if (in_array($this->get_question_type_for_slot($slotnumber), array('random', 'missingtype'))) {
            return \question_engine::can_questions_finish_during_the_attempt(
                    $this->pratestobj->get_pratest()->preferredbehaviour);
        }
        ...
        try {
            $quba = \question_engine::make_questions_usage_by_activity('mod_pratest', $this->pratestobj->get_context());
            $tempslot = $quba->add_question(\question_bank::load_question(
                    $this->slotsinorder[$slotnumber]->questionid));
            $quba->set_preferred_behaviour($this->pratestobj->get_pratest()->preferredbehaviour);
    ...
    /**
     * Get the course id that the pratest belongs to.
     * @return int the course.id for the pratest.
     */
    ...
    /**
     * Get the course module id of the pratest.
     * @return int the course_modules.id for the pratest.
     */
    ...
    /**
     * Get id of the pratest.
     * @return int the pratest.id for the pratest.
     */
     ...
     * @return \stdClass the pratest settings row from the database.
     */
     ...
     * @return bool whether this pratest can be repaginated.
     */
     ...
     * @return bool whether the pratest can be edited.
     */
    public function can_be_edited() {
        if ($this->canbeedited === null) {
            $this->canbeedited = !pratest_has_attempts($this->pratestobj->get_pratestid());
        }
        return $this->canbeedited;
    }

    /**
     * This pratest can only be edited if they have not been attempted.
     * Throw an exception if this is not the case.
     */
    public function check_can_be_edited() {
        if (!$this->can_be_edited()) {
            $reportlink = pratest_attempt_summary_link_to_reports($this->get_pratest(),
                    $this->pratestobj->get_cm(), $this->pratestobj->get_context());
            throw new \moodle_exception('cannoteditafterattempts', 'pratest',
                    new \moodle_url('/mod/pratest/edit.php', array('cmid' => $this->get_cmid())), $reportlink);
        }
    }

    /**
     * How many questions are allowed per page in the pratest.
     * This setting controls how frequently extra page-breaks should be inserted
     * automatically when questions are added to the pratest.
     * @return int the number of questions that should be on each page of the
     * pratest by default.
     */
    public function get_questions_per_page() {
        return $this->pratestobj->get_pratest()->questionsperpage;
    }

    /**
     * Get pratest slots.
     * @return \stdClass[] the slots in this pratest.
     */
    ...
    /**
     * Is this slot the last one in the pratest?
     * @param int $slotnumber the index of the slot in question.
     * @return bool whether this slot the last one in the pratest.
     */
    public function is_last_slot_in_pratest($slotnumber) {
        end($this->slotsinorder);
        return $slotnumber == key($this->slotsinorder);
    }

    /**
     * Is this the first section in the pratest?
     * @param \stdClass $section the pratest_sections row.
     * @return bool whether this is first section in the pratest.
     */
    ...
    /**
     * Is this the last section in the pratest?
     * @param \stdClass $section the pratest_sections row.
     * @return bool whether this is first section in the pratest.
     */
    ...
    /**
     * Get the final slot in the pratest.
     * @return \stdClass the pratest_slots for for the final slot in the pratest.
     */
     ...
     * @return \stdClass the requested pratest_slots row.
     */
    ...
    /**
     * Get all the slots in a section of the pratest.
    ...
    /**
     * Get all the sections of the pratest.
     * @return \stdClass[] the sections in this pratest.
     */
    ...
    /**
     * Get the overall pratest grade formatted for display.
     * @return string the maximum grade for this pratest.
     */
    public function formatted_pratest_grade() {
        return pratest_format_grade($this->get_pratest(), $this->get_pratest()->grade);
    }
    ...
    public function formatted_question_grade($slotnumber) {
        return pratest_format_question_grade($this->get_pratest(), $this->slotsinorder[$slotnumber]->maxmark);
    }

    /**
     * Get the number of decimal places for displyaing overall pratest grades or marks.
     * @return int the number of decimal places.
     */
    public function get_decimal_places_for_grades() {
        return $this->get_pratest()->decimalpoints;
    }
    ...
    public function get_decimal_places_for_question_marks() {
        return pratest_get_grade_format($this->get_pratest());
    }
    ...
    public function get_edit_page_warnings() {
        ...
        if (pratest_has_attempts($this->pratestobj->get_pratestid())) {
            $reviewlink = pratest_attempt_summary_link_to_reports($this->pratestobj->get_pratest(),
                    $this->pratestobj->get_cm(), $this->pratestobj->get_context());
            $warnings[] = get_string('cannoteditafterattempts', 'pratest', $reviewlink);
        }
    ...
    /**
     * Get the date information about the current state of the pratest.
    ...
    public function get_dates_summary() {
        $timenow = time();
        $pratest = $this->pratestobj->get_pratest();

        // Exact open and close dates for the tool-tip.
        $dates = array();
        if ($pratest->timeopen > 0) {
            if ($timenow > $pratest->timeopen) {
                $dates[] = get_string('pratestopenedon', 'pratest', userdate($pratest->timeopen));
            } else {
                $dates[] = get_string('pratestwillopen', 'pratest', userdate($pratest->timeopen));
            }
        }
        if ($pratest->timeclose > 0) {
            if ($timenow > $pratest->timeclose) {
                $dates[] = get_string('pratestclosed', 'pratest', userdate($pratest->timeclose));
            } else {
                $dates[] = get_string('pratestcloseson', 'pratest', userdate($pratest->timeclose));
            }
        }
        if (empty($dates)) {
            $dates[] = get_string('alwaysavailable', 'pratest');
        }
        $explanation = implode(', ', $dates);

        // Brief summary on the page.
        if ($timenow < $pratest->timeopen) {
            $currentstatus = get_string('pratestisclosedwillopen', 'pratest',
                    userdate($pratest->timeopen, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($pratest->timeclose && $timenow <= $pratest->timeclose) {
            $currentstatus = get_string('pratestisopenwillclose', 'pratest',
                    userdate($pratest->timeclose, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($pratest->timeclose && $timenow > $pratest->timeclose) {
            $currentstatus = get_string('pratestisclosed', 'pratest');
        } else {
            $currentstatus = get_string('pratestisopen', 'pratest');
        }
    ...
    /**
     * Set up this class with the structure for a given pratest.
     * @param \stdClass $pratest the pratest settings.
     */
    public function populate_structure($pratest) {
        global $DB;
        ...
        // Get pratest sections in ascending order of the firstslot.
        $this->sections = $DB->get_records('pratest_sections', array('pratestid' => $pratest->id), 'firstslot ASC');
     ...
     * @param \stdClass[] $slots the data about the slots and questions in the pratest.
    ...
    protected function populate_missing_questions($slots) {
        ...
        foreach ($slots as $slot) {
            if ($slot->qtype === null) {
                ...
                $slot->name = get_string('missingquestion', 'pratest');
                $slot->slot = $slot->slot;
                $slot->maxmark = 0;
    ...
    protected function populate_question_numbers() {
        ...
        foreach ($this->slots as $slot) {
            if ($this->questions[$slot->questionid]->length == 0) {
                $slot->displayednumber = get_string('infoshort', 'pratest');
            } else {
    ...
    public function move_slot($idmove, $idmoveafter, $page) {
        ...
        if ($followingslotnumber == $movingslotnumber && !$this->is_last_slot_in_pratest($followingslotnumber)) {
        ...
        } else if (!$this->is_last_slot_in_pratest($moveafterslotnumber) &&
        ...
        if ($moveafterslotnumber > $movingslotnumber) {
            ...
            if ($this->is_last_slot_in_pratest($moveafterslotnumber) ||
        ...
        if ($slotreorder) {
            update_field_with_unique_index('pratest_slots', 'slot', $slotreorder,
                    array('pratestid' => $this->get_pratestid()));
        }

        // Page has changed. Record it.
        if ($movingslot->page != $page) {
            $DB->set_field('pratest_slots', 'page', $page,
                    array('id' => $movingslot->id));
        }

        // Update section fist slots.
        pratest_update_section_firstslots($this->get_pratestid(), $headingmovedirection,
        ...
        $emptypages = $DB->get_fieldset_sql("
                SELECT DISTINCT page - 1
                 ...
                 WHERE pratestid = ?
                   AND page > 1
                   AND NOT EXISTS (SELECT 1 FROM {pratest_slots} WHERE pratestid = ? AND page = slot.page - 1)
              ORDER BY page - 1 DESC
                ", array($this->get_pratestid(), $this->get_pratestid()));

        foreach ($emptypages as $page) {
            $DB->execute("
                    UPDATE {pratest_slots}
                       SET page = page - 1
                     WHERE pratestid = ?
                       AND page > ?
                    ", array($this->get_pratestid(), $page));
        }
    ...
    /**
     * Refresh page numbering of pratest slots.
    ...
    public function refresh_page_numbers($slots = array()) {
        ...
        if (!count($slots)) {
            $slots = $DB->get_records('pratest_slots', array('pratestid' => $this->get_pratestid()), 'slot, page');
        }
    ...
    /**
     * Refresh page numbering of pratest slots and save to the database.
     * @param \stdClass $pratest the pratest object.
     * @return \stdClass[] array of slot objects.
     */
    public function refresh_page_numbers_and_update_db() {
        ...
        foreach ($slots as $slot) {
            $DB->set_field('pratest_slots', 'page', $slot->page,
                    array('id' => $slot->id));
        }
    ...
    public function remove_slot($slotnumber) {
        ...
        $slot = $DB->get_record('pratest_slots', array('pratestid' => $this->get_pratestid(), 'slot' => $slotnumber));
        if (!$slot) {
            return;
        }
        $maxslot = $DB->get_field_sql('SELECT MAX(slot) FROM {pratest_slots} WHERE pratestid = ?', array($this->get_pratestid()));

        $trans = $DB->start_delegated_transaction();
        $DB->delete_records('pratest_slot_tags', array('slotid' => $slot->id));
        $DB->delete_records('pratest_slots', array('id' => $slot->id));
        for ($i = $slot->slot + 1; $i <= $maxslot; $i++) {
            $DB->set_field('pratest_slots', 'slot', $i - 1,
                    array('pratestid' => $this->get_pratestid(), 'slot' => $i));
        }
        ...
        pratest_update_section_firstslots($this->get_pratestid(), -1, $slotnumber);
     ...
     *
     * Saves changes to the question grades in the pratest_slots table and any
     * corresponding question_attempts.
     * It does not update 'sumgrades' in the pratest table.
     *
     * @param \stdClass $slot row from the pratest_slots table.
    ...
    public function update_slot_maxmark($slot, $maxmark) {
        ...
        $DB->update_record('pratest_slots', $slot);
        \question_engine::set_max_mark_in_attempts(new \qubaids_for_pratest($slot->pratestid),
                $slot->slot, $maxmark);
        $trans->allow_commit();
    ...
    public function update_question_dependency($slotid, $requireprevious) {
        global $DB;
        $DB->set_field('pratest_slots', 'requireprevious', $requireprevious, array('id' => $slotid));
    }

    /**
     * Add/Remove a pagebreak.
     *
     * Saves changes to the slot page relationship in the pratest_slots table and reorders the paging
    ...
    public function update_page_break($slotid, $type) {
        ...
        $pratestslots = $DB->get_records('pratest_slots', array('pratestid' => $this->get_pratestid()), 'slot');
        $repaginate = new \mod_pratest\repaginate($this->get_pratestid(), $pratestslots);
        $repaginate->repaginate_slots($pratestslots[$slotid]->slot, $type);
    ...
    public function add_section_heading($pagenumber, $heading = null) {
        ...
        if ($heading !== null) {
            ...
            $section->heading = get_string('newsectionheading', 'pratest');
        }
        $section->pratestid = $this->get_pratestid();
        $slotsonpage = $DB->get_records('pratest_slots', array('pratestid' => $this->get_pratestid(), 'page' => $pagenumber), 'slot DESC');
        $section->firstslot = end($slotsonpage)->slot;
        $section->shufflequestions = 0;
        return $DB->insert_record('pratest_sections', $section);
    }
    ...
    public function set_section_heading($id, $newheading) {
        global $DB;
        $section = $DB->get_record('pratest_sections', array('id' => $id), '*', MUST_EXIST);
        $section->heading = $newheading;
        $DB->update_record('pratest_sections', $section);
    }
    ...
    public function remove_section_heading($sectionid) {
        global $DB;
        $section = $DB->get_record('pratest_sections', array('id' => $sectionid), '*', MUST_EXIST);
        if ($section->firstslot == 1) {
            throw new \coding_exception('Cannot remove the first section in a pratest.');
        }
        $DB->delete_records('pratest_sections', array('id' => $sectionid));
    }
    ...
    protected function populate_slot_tags() {
        ...
        $this->slottags = pratest_retrieve_tags_for_slot_ids($slotids);
    }
