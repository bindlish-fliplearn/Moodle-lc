...
/**
 * Defines the pratest repaginate class.
 *
 * @package   mod_pratest
 ...
 *
 * The pratest setting allows users to write pratestzes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the pratest being manipulated. */
    private $pratestid;
    /** @var array the pratest_slots for that pratest. */
    private $slots;

    /**
     * Constructor.
     * @param int $pratestid the id of the pratest being manipulated.
     * @param stdClass[] $slots the pratest_slots for that pratest.
     */
    public function __construct($pratestid = 0, $slots = null) {
        global $DB;
        $this->pratestid = $pratestid;
        if (!$this->pratestid) {
            $this->slots = array();
        }
        if (!$slots) {
            $this->slots = $DB->get_records('pratest_slots', array('pratestid' => $this->pratestid), 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('pratest_slots', $nextslot, true);
    ...
    /**
     * Repaginate the rest.
     * @param stdClass[] $pratestslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($pratestslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$pratestslots) {
            return null;
        }
        $newslots = array();
        foreach ($pratestslots as $slot) {
            if ($type == self::LINK) {
