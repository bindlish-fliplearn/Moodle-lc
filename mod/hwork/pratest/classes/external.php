 ...
 * @since      Moodle 3.1
 */
class mod_pratest_external extends external_api {

    /**
     * Describes the parameters for get_pratestzes_by_courses.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_pratestzes_by_courses_parameters() {
    ...
    /**
     * Returns a list of pratestzes in a provided list of courses,
     * if no list is provided all pratestzes that the user can view will be returned.
     *
     * @param array $courseids Array of course ids
     * @return array of pratestzes details
     * @since Moodle 3.1
     */
    public static function get_pratestzes_by_courses($courseids = array()) {
        global $USER;

        $warnings = array();
        $returnedpratestzes = array();

        $params = array(
            'courseids' => $courseids,
        );
        $params = self::validate_parameters(self::get_pratestzes_by_courses_parameters(), $params);
        ...
        if (!empty($params['courseids'])) {
            ...
            // Get the pratestzes in this course, this function checks users visibility permissions.
            // We can avoid then additional validate_context calls.
            $pratestzes = get_all_instances_in_courses("pratest", $courses);
            foreach ($pratestzes as $pratest) {
                ...
                $pratest = pratest_update_effective_access($pratest, $USER->id);

                // Entry to return.
                $pratestdetails = array();
                // First, we return information that any user can see in the web interface.
                $pratestdetails['id'] = $pratest->id;
                $pratestdetails['coursemodule']      = $pratest->coursemodule;
                $pratestdetails['course']            = $pratest->course;
                $pratestdetails['name']              = external_format_string($pratest->name, $context->id);

                if (has_capability('mod/pratest:view', $context)) {
                    // Format intro.
                    list($pratestdetails['intro'], $pratestdetails['introformat']) = external_format_text($pratest->intro,
                                                                    $pratest->introformat, $context->id, 'mod_pratest', 'intro', null);

                    $pratestdetails['introfiles'] = external_util::get_area_files($context->id, 'mod_pratest', 'intro', false, false);
                    ...
                    $hasfeedback = pratest_has_feedback($pratest);
                    $pratestdetails['hasfeedback'] = (!empty($hasfeedback)) ? 1 : 0;

                    $timenow = time();
                    $pratestobj = pratest::create($pratest->id, $USER->id);
                    $accessmanager = new pratest_access_manager($pratestobj, $timenow, has_capability('mod/pratest:ignoretimelimits',
                                                                $context, null, false));

                    // Fields the user could see if have access to the pratest.
                    if (!$accessmanager->prevent_access()) {
                        $pratestdetails['hasquestions'] = (int) $pratestobj->has_questions();
                        $pratestdetails['autosaveperiod'] = get_config('pratest', 'autosaveperiod');
                    ...
                    foreach ($viewablefields as $field) {
                        $pratestdetails[$field] = $pratest->{$field};
                    }
                }
                $returnedpratestzes[] = $pratestdetails;
            }
        }
        $result = array();
        $result['pratestzes'] = $returnedpratestzes;
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_pratestzes_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_pratestzes_by_courses_returns() {
        return new external_single_structure(
            array(
                'pratestzes' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            'id' => new external_value(PARAM_INT, 'Standard Moodle primary key.'),
                            'course' => new external_value(PARAM_INT, 'Foreign key reference to the course this pratest is part of.'),
                            ...
                            'timeopen' => new external_value(PARAM_INT, 'The time when this pratest opens. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timeclose' => new external_value(PARAM_INT, 'The time when this pratest closes. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timelimit' => new external_value(PARAM_INT, 'The time limit for pratest attempts, in seconds.',
                                                                VALUE_OPTIONAL),
                            ...
                            'canredoquestions' => new external_value(PARAM_INT, 'Allows students to redo any completed question
                                                                        within a pratest attempt.', VALUE_OPTIONAL),
                            ...
                            'reviewattempt' => new external_value(PARAM_INT, 'Whether users are allowed to review their pratest
                                                                    attempts at various times. This is a bit field, decoded by the
                                                                    mod_pratest_display_options class. It is formed by ORing together
                                                                    the constants defined there.', VALUE_OPTIONAL),
                            'reviewcorrectness' => new external_value(PARAM_INT, 'Whether users are allowed to review their pratest
                                                                        attempts at various times.
                                                                        A bit field, like reviewattempt.', VALUE_OPTIONAL),
                            'reviewmarks' => new external_value(PARAM_INT, 'Whether users are allowed to review their pratest attempts
                            ...
                            'sumgrades' => new external_value(PARAM_FLOAT, 'The total of all the question instance maxmarks.',
                                                                VALUE_OPTIONAL),
                            'grade' => new external_value(PARAM_FLOAT, 'The total that the pratest overall grade is scaled to be
                                                            out of.', VALUE_OPTIONAL),
                            'timecreated' => new external_value(PARAM_INT, 'The time when the pratest was added to the course.',
                                                                VALUE_OPTIONAL),
                            ...
                            'password' => new external_value(PARAM_RAW, 'A password that the student must enter before starting or
                                                                continuing a pratest attempt.', VALUE_OPTIONAL),
                            'subnet' => new external_value(PARAM_RAW, 'Used to restrict the IP addresses from which this pratest can
                            ...
                            'completionattemptsexhausted' => new external_value(PARAM_INT, 'Mark pratest complete when the student has
                            ...
                            'allowofflineattempts' => new external_value(PARAM_INT, 'Whether to allow the pratest to be attempted
                                                                            offline in the mobile app', VALUE_OPTIONAL),
                            'autosaveperiod' => new external_value(PARAM_INT, 'Auto-save delay', VALUE_OPTIONAL),
                            'hasfeedback' => new external_value(PARAM_INT, 'Whether the pratest has any non-blank feedback text',
                                                                VALUE_OPTIONAL),
                            'hasquestions' => new external_value(PARAM_INT, 'Whether the pratest has questions', VALUE_OPTIONAL),
    ...
    /**
     * Utility function for validating a pratest.
     *
     * @param int $pratestid pratest instance id
     * @return array array containing the pratest, course, context and course module objects
     * @since  Moodle 3.1
     */
    protected static function validate_pratest($pratestid) {
        global $DB;

        // Request and permission validation.
        $pratest = $DB->get_record('pratest', array('id' => $pratestid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($pratest, 'pratest');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        return array($pratest, $course, $cm, $context);
    }

    /**
     * Describes the parameters for view_pratest.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function view_pratest_parameters() {
        return new external_function_parameters (
            array(
                'pratestid' => new external_value(PARAM_INT, 'pratest instance id'),
            )
        );
    }
     ...
     */
    public static function view_pratest($pratestid) {
        global $DB;

        $params = self::validate_parameters(self::view_pratest_parameters(), array('pratestid' => $pratestid));
        $warnings = array();

        list($pratest, $course, $cm, $context) = self::validate_pratest($params['pratestid']);
    ...
    /**
     * Describes the view_pratest return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function view_pratest_returns() {
        return new external_single_structure(
            array(
    ...
    public static function get_user_attempts_parameters() {
        return new external_function_parameters (
            array(
                ...
                'status' => new external_value(PARAM_ALPHA, 'pratest status: all, finished or unfinished', VALUE_DEFAULT, 'finished'),
    ...
    /**
     * Return a list of attempts for the given pratest and user.
     *
     * @param int $pratestid pratest instance id
     * @param int $userid user id
     * @param string $status pratest status: all, finished or unfinished
     * @param bool $includepreviews whether to include previews or not
     * @return array of warnings and the list of attempts
     * @since Moodle 3.1
     * @throws invalid_parameter_exception
     */
    public static function get_user_attempts($pratestid, $userid = 0, $status = 'finished', $includepreviews = false) {
        global $DB, $USER;
        ...
        if ($USER->id != $user->id) {
            require_capability('mod/pratest:viewreports', $context);
        }

        $attempts = pratest_get_user_attempts($pratest->id, $user->id, $params['status'], $params['includepreviews']);
    ...
    private static function attempt_structure() {
        return new external_single_structure(
            array(
                ...
                'pratest' => new external_value(PARAM_INT, 'Foreign key reference to the pratest that was attempted.',
                                                VALUE_OPTIONAL),
                'userid' => new external_value(PARAM_INT, 'Foreign key reference to the user whose attempt this is.',
                                                VALUE_OPTIONAL),
                'attempt' => new external_value(PARAM_INT, 'Sequentially numbers this students attempts at this pratest.',
                                                VALUE_OPTIONAL),
                'uniqueid' => new external_value(PARAM_INT, 'Foreign key reference to the question_usage that holds the
                                                    details of the the question_attempts that make up this pratest
                ...
                'timecheckstate' => new external_value(PARAM_INT, 'Next time pratest cron should check attempt for
    ...
    /**
     * Get the best current grade for the given user on a pratest.
     *
     * @param int $pratestid pratest instance id
     * @param int $userid user id
     ...
     * @since Moodle 3.1
     */
    public static function get_user_best_grade($pratestid, $userid = 0) {
        global $DB, $USER;
        ...
        $result = array();
        $grade = pratest_get_best_grade($pratest, $user->id);
    ...
    public static function get_user_best_grade_returns() {
        return new external_single_structure(
            array(
                'hasgrade' => new external_value(PARAM_BOOL, 'Whether the user has a grade on the given pratest.'),
     ...
     * @since Moodle 3.1
     */
    public static function get_combined_review_options($pratestid, $userid = 0) {
        global $DB, $USER;
        ...
        $attempts = pratest_get_user_attempts($pratest->id, $user->id, 'all', true);
    ...
    /**
     * Starts a new attempt at a pratest.
     *
     * @param int $pratestid pratest instance id
     ...
     */
    public static function start_attempt($pratestid, $preflightdata = array(), $forcenew = false) {
        global $DB, $USER;
        ...
        if (!$pratestobj->has_questions()) {
            throw new moodle_pratest_exception($pratestobj, 'noquestionsfound');
        }
        ...
        list($currentattemptid, $attemptnumber, $lastattempt, $messages, $page) =
            pratest_validate_new_attempt($pratestobj, $accessmanager, $forcenew, -1, false);
        ...
        if (!$pratestobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new moodle_pratest_exception($pratestobj, array_shift($errors));
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == pratest_attempt::OVERDUE) {
                    throw new moodle_pratest_exception($pratestobj, 'stateoverdue');
                } else {
                    throw new moodle_pratest_exception($pratestobj, 'attemptstillinprogress');
                }
            }
            $offlineattempt = WS_SERVER ? true : false;
            $attempt = pratest_prepare_and_start_new_attempt($pratestobj, $attemptnumber, $lastattempt, $offlineattempt);
        }
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        $attemptobj = pratest_attempt::create($params['attemptid']);
        ...
        if ($attemptobj->get_userid() != $USER->id) {
            throw new moodle_pratest_exception($attemptobj->get_pratestobj(), 'notyourattempt');
        }
        ...
        if ($checkaccessrules) {
            ...
            if (!$ispreviewuser && $messages) {
                throw new moodle_pratest_exception($attemptobj->get_pratestobj(), 'attempterror');
            }
        }

        // Attempt closed?.
        if ($attemptobj->is_finished()) {
            throw new moodle_pratest_exception($attemptobj->get_pratestobj(), 'attemptalreadyclosed');
        } else if ($failifoverdue && $attemptobj->get_state() == pratest_attempt::OVERDUE) {
            throw new moodle_pratest_exception($attemptobj->get_pratestobj(), 'stateoverdue');
        }

        // User submitted data (like the pratest password).
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new moodle_pratest_exception($attemptobj->get_pratestobj(), array_shift($errors));
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new moodle_pratest_exception($attemptobj->get_pratestobj(), 'Invalid page number');
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new moodle_pratest_exception($attemptobj->get_pratestobj(), 'Out of sequence access');
            }
            ...
            if (empty($slots)) {
                throw new moodle_pratest_exception($attemptobj->get_pratestobj(), 'noquestionsfound');
            }
        }
     ...
     *
     * @return external_single_structure the question data. Some fields may not be returned depending on the pratest display settings.
     * @since  Moodle 3.1
    ...
    private static function question_structure() {
        return new external_single_structure(
            array(
                ...
                'page' => new external_value(PARAM_INT, 'page of the pratest this question appears on'),
                ...
                'number' => new external_value(PARAM_INT, 'question ordering number in the pratest', VALUE_OPTIONAL),
                'state' => new external_value(PARAM_ALPHA, 'the state where the question is in.
                    It will not be returned if the user cannot see it due to the pratest display correctness settings.',
                    VALUE_OPTIONAL),
            ...
            'The question data. Some fields may not be returned depending on the pratest display settings.'
        );
    }
     ...
     *
     * @param  pratest_attempt  $attemptobj  the pratest attempt object
     * @param  bool  $review  whether if we are in review mode or not
     * @param  mixed  $page  string 'all' or integer page number
     * @return array array of questions including data
     */
    private static function get_attempt_questions_data(pratest_attempt $attemptobj, $review, $page = 'all') {
        global $PAGE;

        $questions = array();
        $contextid = $attemptobj->get_pratestobj()->get_context()->id;
        $displayoptions = $attemptobj->get_display_options($review);
        $renderer = $PAGE->get_renderer('mod_pratest');
    ...
    /**
     * Returns information for the given attempt page for a pratest attempt in progress.
     *
     ...
     * @since Moodle 3.1
     * @throws moodle_pratest_exceptions
     */
        ...
        return new external_single_structure(
            array(
                ...
                'messages' => new external_multiple_structure(
                    ...
                    'access messages, will only be returned for users with mod/pratest:preview capability,
    ...
    /**
     * Returns a summary of a pratest attempt before it is submitted.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Processes save requests during the pratest. This function is intended for the pratest auto-save feature.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Process responses during an attempt at a pratest and also deals with attempts finishing.
     *
     * @param int $attemptid attempt id
     * @param array $data the data to be saved
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new moodle_pratest_exception($attemptobj->get_pratestobj(), 'attemptclosed');
    ...
    public static function get_attempt_review($attemptid, $page = -1) {
        ...
        foreach ($summarydata as $key => $data) {
            ...
            $result['additionaldata'][] = array(
                'id' => $key,
                'title' => $data['title'], $attemptobj->get_pratestobj()->get_context()->id,
                'content' => $data['content'],
            );
        }

        // Feedback if there is any, and the user is allowed to see it now.
        $grade = pratest_rescale_grade($attemptobj->get_attempt()->sumgrades, $attemptobj->get_pratest(), false);
    ...
    public static function get_attempt_review_returns() {
        return new external_single_structure(
            array(
                'grade' => new external_value(PARAM_RAW, 'grade for the pratest (or empty or "notyetgraded")'),
                'attempt' => self::attempt_structure(),
     ...
     * @since Moodle 3.1
     */
    public static function get_pratest_feedback_for_grade_parameters() {
        return new external_function_parameters (
            array(
                'pratestid' => new external_value(PARAM_INT, 'pratest instance id'),
    ...
    /**
     * Get the feedback text that should be show to a student who got the given grade in the given pratest.
     *
     * @param int $pratestid pratest instance id
     ...
     * @throws moodle_exception
     */
    public static function get_pratest_feedback_for_grade($pratestid, $grade) {
        global $DB;
        ...
        $params = self::validate_parameters(self::get_pratest_feedback_for_grade_parameters(), $params);
        $warnings = array();
        ...
        $feedback = pratest_feedback_record_for_grade($params['grade'], $pratest);
        if (!empty($feedback->feedbacktext)) {
            list($text, $format) = external_format_text($feedback->feedbacktext, $feedback->feedbacktextformat, $context->id,
                                                        'mod_pratest', 'feedback', $feedback->id);
            $result['feedbacktext'] = $text;
            $result['feedbacktextformat'] = $format;
            $feedbackinlinefiles = external_util::get_area_files($context->id, 'mod_pratest', 'feedback', $feedback->id);
    ...
    /**
     * Describes the get_pratest_feedback_for_grade return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_pratest_feedback_for_grade_returns() {
        return new external_single_structure(
            array(
    ...
    /**
     * Describes the parameters for get_pratest_access_information.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_pratest_access_information_parameters() {
        return new external_function_parameters (
            array(
                'pratestid' => new external_value(PARAM_INT, 'pratest instance id')
            )
    ...
    /**
     * Return access information for a given pratest.
     *
     * @param int $pratestid pratest instance id
     ...
     * @throws  moodle_pratest_exception
     */
    public static function get_pratest_access_information($pratestid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_pratest_access_information_parameters(), $params);

        list($pratest, $course, $cm, $context) = self::validate_pratest($params['pratestid']);

        $result = array();
        // Capabilities first.
        $result['canattempt'] = has_capability('mod/pratest:attempt', $context);;
        $result['canmanage'] = has_capability('mod/pratest:manage', $context);;
        $result['canpreview'] = has_capability('mod/pratest:preview', $context);;
        $result['canreviewmyattempts'] = has_capability('mod/pratest:reviewmyattempts', $context);;
        $result['canviewreports'] = has_capability('mod/pratest:viewreports', $context);;

        // Access manager now.
        $pratestobj = pratest::create($cm->instance, $USER->id);
        $ignoretimelimits = has_capability('mod/pratest:ignoretimelimits', $context, null, false);
        $timenow = time();
        $accessmanager = new pratest_access_manager($pratestobj, $timenow, $ignoretimelimits);
    ...
    /**
     * Describes the get_pratest_access_information return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_pratest_access_information_returns() {
        return new external_single_structure(
            array(
                'canattempt' => new external_value(PARAM_BOOL, 'Whether the user can do the pratest or not.'),
                'canmanage' => new external_value(PARAM_BOOL, 'Whether the user can edit the pratest settings or not.'),
                'canpreview' => new external_value(PARAM_BOOL, 'Whether the user can preview the pratest or not.'),
                'canreviewmyattempts' => new external_value(PARAM_BOOL, 'Whether the users can review their previous attempts
                                                                or not.'),
                'canviewreports' => new external_value(PARAM_BOOL, 'Whether the user can view the pratest reports or not.'),
    ...
    /**
     * Return access information for a given attempt in a pratest.
     *
     * @param int $pratestid pratest instance id
     ...
     * @throws  moodle_pratest_exception
     */
    public static function get_attempt_access_information($pratestid, $attemptid = 0) {
        global $DB, $USER;
        ...
        if ($unfinishedattempt = pratest_get_user_attempt_unfinished($pratest->id, $USER->id)) {
            ...
            if ($unfinishedattempt->state != pratest_attempt::IN_PROGRESS and $unfinishedattempt->state != pratest_attempt::OVERDUE) {
    ...
    /**
     * Describes the parameters for get_pratest_required_qtypes.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_pratest_required_qtypes_parameters() {
        return new external_function_parameters (
            array(
                'pratestid' => new external_value(PARAM_INT, 'pratest instance id')
            )
    ...
    /**
     * Return the potential question types that would be required for a given pratest.
     ...
     * @throws  moodle_pratest_exception
     */
    public static function get_pratest_required_qtypes($pratestid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_pratest_required_qtypes_parameters(), $params);
        ...
        // Question types used.
        $result = array();
        $result['questiontypes'] = $pratestobj->get_all_question_types_used(true);
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_pratest_required_qtypes return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_pratest_required_qtypes_returns() {
        return new external_single_structure(
            array(
                'questiontypes' => new external_multiple_structure(
                                    new external_value(PARAM_PLUGIN, 'question type'), 'list of question types used in the pratest'),
                'warnings' => new external_warnings(),
            )
        );
    }
