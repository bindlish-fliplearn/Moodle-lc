...
/**
 * This page handles editing and creation of pratest overrides
 *
 * @package   mod_pratest
...
$url = new moodle_url('/mod/pratest/overrideedit.php');
if ($action) {
...
// Merge pratest defaults with data.
$keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
foreach ($keys as $key) {
    if (!isset($data->{$key}) || $reset) {
        $data->{$key} = $pratest->{$key};
    }
}
...
$overridelisturl = new moodle_url('/mod/pratest/overrides.php', array('cmid'=>$cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new pratest_override_form($url, $cm, $pratest, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->pratest = $pratest->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if ($fromform->{$key} == $pratest->{$key}) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('pratest_overrides', $conditions)) {
            ...
            $pratest->cmid = $cm->id;
            pratest_delete_override($pratest, $oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('pratest_overrides', $fromform);
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('pratest_overrides', $fromform);
    ...
    if ($groupmode) {
        ...
        pratest_update_events($pratest, $fromform);
    }
...
// Print the form.
$pagetitle = get_string('editoverride', 'pratest');
