...
/**
 * Define all the backup steps that will be used by the backup_pratest_activity_task
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_pratest_activity_structure_step extends backup_questions_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $pratest = new backup_nested_element('pratest', array('id'), array(
        ...
        $this->add_subplugin_structure('pratestaccess', $pratest, true);
        ...
        $this->add_subplugin_structure('pratestaccess', $attempt, true);

        // Build the tree.
        $pratest->add_child($qinstances);
        $qinstances->add_child($qinstance);

        $qinstance->add_child($qinstancetags);
        $qinstancetags->add_child($qinstancetag);

        $pratest->add_child($sections);
        $sections->add_child($section);

        $pratest->add_child($feedbacks);
        $feedbacks->add_child($feedback);

        $pratest->add_child($overrides);
        $overrides->add_child($override);

        $pratest->add_child($grades);
        $grades->add_child($grade);

        $pratest->add_child($attempts);
        $attempts->add_child($attempt);

        // Define sources.
        $pratest->set_source_table('pratest', array('id' => backup::VAR_ACTIVITYID));

        $qinstance->set_source_table('pratest_slots',
                array('pratestid' => backup::VAR_PARENTID));

        $qinstancetag->set_source_table('pratest_slot_tags',
                array('slotid' => backup::VAR_PARENTID));

        $section->set_source_table('pratest_sections',
                array('pratestid' => backup::VAR_PARENTID));

        $feedback->set_source_table('pratest_feedback',
                array('pratestid' => backup::VAR_PARENTID));

        // Quiz overrides to backup are different depending of user info.
        $overrideparams = array('pratest' => backup::VAR_PARENTID);
        if (!$userinfo) { //  Without userinfo, skip user overrides.
            $overrideparams['userid'] = backup_helper::is_sqlparam(null);

        }
        $override->set_source_table('pratest_overrides', $overrideparams);

        // All the rest of elements only happen if we are including user info.
        if ($userinfo) {
            $grade->set_source_table('pratest_grades', array('pratest' => backup::VAR_PARENTID));
            $attempt->set_source_sql('
                    SELECT *
                    FROM {pratest_attempts}
                    WHERE pratest = :pratest AND preview = 0',
                    array('pratest' => backup::VAR_PARENTID));
        }

        // Define source alias.
        $pratest->set_source_alias('attempts', 'attempts_number');
        ...
        $pratest->annotate_files('mod_pratest', 'intro', null); // This file area hasn't itemid.
        $feedback->annotate_files('mod_pratest', 'feedback', 'id');

        // Return the root element (pratest), wrapped into standard activity structure.
        return $this->prepare_activity_structure($pratest);
    }
}
