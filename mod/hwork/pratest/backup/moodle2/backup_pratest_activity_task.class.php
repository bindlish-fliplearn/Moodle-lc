...
/**
 * Defines backup_pratest_activity_task class
 *
 * @package     mod_pratest
 * @category    backup
...
require_once($CFG->dirroot . '/mod/pratest/backup/moodle2/backup_pratest_stepslib.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_pratest_activity_task extends backup_activity_task {
    ...
    protected function define_my_steps() {
        // Generate the pratest.xml file containing all the pratest information
        // and annotating used questions.
        $this->add_step(new backup_pratest_activity_structure_step('pratest_structure', 'pratest.xml'));

        // Note: Following  steps must be present
        // in all the activities using question banks (only pratest for now)
    ...
    public static function encode_content_links($content) {
        ...
        // Link to the list of pratestzes.
        $search="/(".$base."\/mod\/pratest\/index.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@QUIZINDEX*$2@$', $content);

        // Link to pratest view by moduleid.
        $search="/(".$base."\/mod\/pratest\/view.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@QUIZVIEWBYID*$2@$', $content);

        // Link to pratest view by pratestid.
        $search="/(".$base."\/mod\/pratest\/view.php\?q\=)([0-9]+)/";
