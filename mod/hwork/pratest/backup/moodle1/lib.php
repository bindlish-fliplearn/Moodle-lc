 ...
 */
class moodle1_mod_pratest_handler extends moodle1_mod_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path(
                'pratest', '/MOODLE_BACKUP/COURSE/MODULES/MOD/QUIZ',
                array(
                    'newfields' => array(
                        'showuserpicture'       => 0,
            ...
            new convert_path('pratest_question_instances',
                    '/MOODLE_BACKUP/COURSE/MODULES/MOD/QUIZ/QUESTION_INSTANCES'),
            new convert_path('pratest_question_instance',
            ...
            new convert_path('pratest_feedbacks',
                    '/MOODLE_BACKUP/COURSE/MODULES/MOD/QUIZ/FEEDBACKS'),
            new convert_path('pratest_feedback',
     ...
     * data available
     */
    public function process_pratest($data) {
        global $CFG;

        // Replay the upgrade step 2008081501.
        if (is_null($data['sumgrades'])) {
            $data['sumgrades'] = 0;
            // TODO for user data: pratest_attempts SET sumgrades=0 WHERE sumgrades IS NULL.
            // TODO for user data: pratest_grades.grade should be not be null , convert to default 0.
        }
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_pratest');
        ...
        // Start writing pratest.xml.
        $this->open_xml_writer("activities/pratest_{$this->moduleid}/pratest.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid,
                'moduleid' => $this->moduleid, 'modulename' => 'pratest',
                'contextid' => $contextid));
        $this->xmlwriter->begin_tag('pratest', array('id' => $instanceid));
    ...
    public function on_pratest_question_instances_start() {
        $this->xmlwriter->begin_tag('question_instances');
    }

    public function on_pratest_question_instances_end() {
        $this->xmlwriter->end_tag('question_instances');
    }

    public function process_pratest_question_instance($data) {
        $this->write_xml('question_instance', $data, array('/question_instance/id'));
    }

    public function on_pratest_feedbacks_start() {
        $this->xmlwriter->begin_tag('feedbacks');
    }

    public function on_pratest_feedbacks_end() {
        $this->xmlwriter->end_tag('feedbacks');
    }

    public function process_pratest_feedback($data) {
    ...
    /**
     * This is executed when we reach the closing </MOD> tag of our 'pratest' path
     */
    public function on_pratest_end() {

        // Append empty <overrides> subpath element.
        $this->write_xml('overrides', array());

        // Finish writing pratest.xml.
        $this->xmlwriter->end_tag('pratest');
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // Write inforef.xml.
        $this->open_xml_writer("activities/pratest_{$this->moduleid}/inforef.xml");
