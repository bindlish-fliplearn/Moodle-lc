...
/**
 * Back-end code for handling data about pratestzes and the current user's attempt.
 *
 * There are classes for loading all the information about a pratest and attempts,
...
/**
 * Class for pratest exceptions. Just saves a couple of arguments on the
 ...
 * @since     Moodle 2.0
 */
class moodle_pratest_exception extends moodle_exception {
    public function __construct($pratestobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $pratestobj->view_url();
        }
        parent::__construct($errorcode, 'pratest', $link, $a, $debuginfo);
    }
}


/**
 * A class encapsulating a pratest and the questions it contains, and making the
...
class pratest {
    ...
    protected $cm;
    /** @var stdClass the pratest settings from the database. */
    protected $pratest;
    /** @var context the pratest context. */
    protected $context;

    /** @var array of questions augmented with slot information. */
    protected $questions = null;
    /** @var array of pratest_section rows. */
    protected $sections = null;
    /** @var pratest_access_manager the access manager for this pratest. */
    protected $accessmanager = null;
    /** @var bool whether the current user has capability mod/pratest:preview. */
     ...
     *
     * @param object $pratest the row from the pratest table.
     * @param object $cm the course_module object for this pratest.
     * @param object $course the row from the course table for the course we belong to.
     * @param bool $getcontext intended for testing - stops the constructor getting the context.
     */
    public function __construct($pratest, $cm, $course, $getcontext = true) {
        $this->pratest = $pratest;
        $this->cm = $cm;
        $this->pratest->cmid = $this->cm->id;
        $this->course = $course;
    ...
    /**
     * Static function to create a new pratest object for a specific user.
     *
     * @param int $pratestid the the pratest id.
     * @param int $userid the the userid.
     * @return pratest the new pratest object
     */
    public static function create($pratestid, $userid = null) {
        global $DB;

        $pratest = pratest_access_manager::load_pratest_and_settings($pratestid);
        $course = $DB->get_record('course', array('id' => $pratest->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('pratest', $pratest->id, $course->id, false, MUST_EXIST);

        // Update pratest with override information.
        if ($userid) {
            $pratest = pratest_update_effective_access($pratest, $userid);
        }

        return new pratest($pratest, $cm, $course);
    }

    /**
     * Create a {@link pratest_attempt} for an attempt at this pratest.
     * @param object $attemptdata row from the pratest_attempts table.
     * @return pratest_attempt the new pratest_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new pratest_attempt($attemptdata, $this->pratest, $this->cm, $this->course);
    }

    // Functions for loading more data =========================================

    /**
     * Load just basic information about all the questions in this pratest.
     */
    public function preload_questions() {
        $this->questions = question_preload_questions(null,
                ...
                '{pratest_slots} slot ON slot.pratestid = :pratestid AND q.id = slot.questionid',
                array('pratestid' => $this->pratest->id), 'slot.slot');
    }

    /**
     * Fully load some or all of the questions for this pratest. You must call
    ...
    /**
     * Get an instance of the {@link \mod_pratest\structure} class for this pratest.
     * @return \mod_pratest\structure describes the questions in the pratest.
     */
    public function get_structure() {
        return \mod_pratest\structure::create_for_pratest($this);
    }
    ...
    /** @return int the pratest id. */
    public function get_pratestid() {
        return $this->pratest->id;
    }

    /** @return object the row of the pratest table. */
    public function get_pratest() {
        return $this->pratest;
    }

    /** @return string the name of this pratest. */
    public function get_pratest_name() {
        return $this->pratest->name;
    }

    /** @return int the pratest navigation method. */
    public function get_navigation_method() {
        return $this->pratest->navmethod;
    }

    /** @return int the number of attempts allowed at this pratest (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->pratest->attempts;
    }
    ...
    /** @return object the module context for this pratest. */
    public function get_context() {
        return $this->context;
    }

    /**
     * @return bool wether the current user is someone who previews the pratest,
     * rather than attempting it.
     */
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/pratest:preview', $this->context);
        }
        return $this->ispreviewuser;
    }

    /**
     * @return whether any questions have been added to this pratest.
     */
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'pratest', $this->view_url());
            }
    ...
    /**
     * Get all the sections in this pratest.
     * @return array 0, 1, 2, ... => pratest_sections row from the database.
     */
    public function get_sections() {
        global $DB;
        if ($this->sections === null) {
            $this->sections = array_values($DB->get_records('pratest_sections',
                    array('pratestid' => $this->get_pratestid()), 'firstslot'));
        }
        return $this->sections;
    }

    /**
     * Return pratest_access_manager and instance of the pratest_access_manager class
     * for this pratest at this time.
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new pratest_access_manager($this, $timenow,
                    has_capability('mod/pratest:ignoretimelimits', $this->context, null, false));
        }
        return $this->accessmanager;
    }

    /**
     * Wrapper round the has_capability funciton that automatically passes in the pratest context.
     */
    ...
    /**
     * Wrapper round the require_capability funciton that automatically passes in the pratest context.
     */
    ...
    /**
     * @return string the URL of this pratest's view page.
     */
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/pratest/view.php?id=' . $this->cm->id;
    }

    /**
     * @return string the URL of this pratest's edit page.
     */
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/pratest/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/pratest/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
    ...
    /**
     * @return string the URL of this pratest's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/pratest/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/pratest/review.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/pratest/summary.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
     ...
     * @deprecated since 3.1. This sort of functionality is now entirely handled by pratest access rules.
     */
    public function confirm_start_attempt_message($notused) {
        debugging('confirm_start_attempt_message is deprecated. ' .
                'This sort of functionality is now entirely handled by pratest access rules.');
        return '';
    }
     ...
     *
     * @param int $when One of the mod_pratest_display_options::DURING,
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_pratest_display_options::DURING ||
                $when == mod_pratest_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_pratest_display_options::LATER_WHILE_OPEN && $this->pratest->timeclose &&
                $this->pratest->reviewattempt & mod_pratest_display_options::AFTER_CLOSE) {
            return get_string('noreviewuntil' . $langstrsuffix, 'pratest',
                    userdate($this->pratest->timeclose, $dateformat));
        } else {
            return get_string('noreview' . $langstrsuffix, 'pratest');
        }
    }

    /**
     * @param string $title the name of this particular pratest page.
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_pratest_exception($this, 'questionnotloaded', $id);
        }
    }

    /**
     * Return all the question types used in this pratest.
     *
     * @param  boolean $includepotential if the pratest include random questions, setting this flag to true will make the function to
    ...
    public function get_all_question_types_used($includepotential = false) {
        ...
        // We must be careful with random questions, if we find a random question we must assume that the pratest may content
...
/**
 * This class extends the pratest class to hold data about the state of a particular attempt,
 * in addition to the data about the pratest.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class pratest_attempt {
    ...
    /** @var int maximum number of slots in the pratest for the review page to default to show all. */
    const MAX_SLOTS_FOR_DEFAULT_REVIEW_SHOW_ALL = 50;

    /** @var pratest object containing the pratest settings. */
    protected $pratestobj;

    /** @var stdClass the pratest_attempts row. */
    protected $attempt;

    /** @var question_usage_by_activity the question usage for this pratest attempt. */
    protected $quba;
    ...
    /** @var array of pratest_sections rows, with a ->lastslot field added. */
    protected $sections;
    ...
    /** @var mod_pratest_display_options cache for the appropriate review options. */
     ...
     * Constructor assuming we already have the necessary data loaded.
     *
     * @param object $attempt the row of the pratest_attempts table.
     * @param object $pratest the pratest object for this attempt and user.
     * @param object $cm the course_module object for this pratest.
     * @param object $course the row from the course table for the course we belong to.
     ...
     */
    public function __construct($attempt, $pratest, $cm, $course, $loadquestions = true) {
        global $DB;

        $this->attempt = $attempt;
        $this->pratestobj = new pratest($pratest, $cm, $course);
        ...
        $this->slots = $DB->get_records('pratest_slots',
                array('pratestid' => $this->get_pratestid()), 'slot',
     ...
     * @param array $conditions passed to $DB->get_record('pratest_attempts', $conditions).
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('pratest_attempts', $conditions, '*', MUST_EXIST);
        $pratest = pratest_access_manager::load_pratest_and_settings($attempt->pratest);
        $course = $DB->get_record('course', array('id' => $pratest->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('pratest', $pratest->id, $course->id, false, MUST_EXIST);

        // Update pratest with override information.
        $pratest = pratest_update_effective_access($pratest, $attempt->userid);

        return new pratest_attempt($attempt, $pratest, $cm, $course);
    }

    /**
     * Static function to create a new pratest_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new pratest_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return pratest_attempt_state_name($state);
    }
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'pratest');
                }
    ...
    // Simple getters ==========================================================
    public function get_pratest() {
        return $this->pratestobj->get_pratest();
    }

    public function get_pratestobj() {
        return $this->pratestobj;
    }

    /** @return int the course id. */
    public function get_courseid() {
        return $this->pratestobj->get_courseid();
    }

    /** @return int the course id. */
    public function get_course() {
        return $this->pratestobj->get_course();
    }

    /** @return int the pratest id. */
    public function get_pratestid() {
        return $this->pratestobj->get_pratestid();
    }

    /** @return string the name of this pratest. */
    public function get_pratest_name() {
        return $this->pratestobj->get_pratest_name();
    }

    /** @return int the pratest navigation method. */
    public function get_navigation_method() {
        return $this->pratestobj->get_navigation_method();
    }

    /** @return object the course_module object. */
    public function get_cm() {
        return $this->pratestobj->get_cm();
    }

    /** @return object the course_module object. */
    public function get_cmid() {
        return $this->pratestobj->get_cmid();
    }
     ...
     */
    public function is_preview_user() {
        return $this->pratestobj->is_preview_user();
    }

    /** @return int the number of attempts allowed at this pratest (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->pratestobj->get_num_attempts_allowed();
    }

    /** @return int number fo pages in this pratest. */
     ...
     */
    public function get_access_manager($timenow) {
        return $this->pratestobj->get_access_manager($timenow);
    }
    ...
    /** @return object the row from the pratest_attempts table. */
    ...
    /** @return string one of the pratest_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/pratest:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the pratest in a non-trivial way?
     ...
     * To see how this works, try setting a pratest to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the pratest
     ...
     *
     * @param question_display_options $options the display options for this pratest attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return pratest_feedback_for_grade($grade, $this->get_pratest(),
                $this->pratestobj->get_context());
    }
     ...
     */
    public function has_capability($capability, $userid = null, $doanything = true) {
        return $this->pratestobj->has_capability($capability, $userid, $doanything);
    }
     ...
     */
    public function require_capability($capability, $userid = null, $doanything = true) {
        return $this->pratestobj->require_capability($capability, $userid, $doanything);
    }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_pratest_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/pratest:attempt';
        } else {
            $capability = 'mod/pratest:reviewmyattempts';
        }

        // These next tests are in a slighly funny order. The point is that the
        // common and most performance-critical case is students attempting a pratest
        // so we want to check that permisison first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the pratest as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/pratest:viewreports') ||
                $this->has_capability('mod/pratest:preview')) {
    ...
    public function get_attempt_state() {
        return pratest_attempt_state($this->get_pratest(), $this->attempt);
    }

    /**
     * Wrapper that the correct mod_pratest_display_options for this pratest at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = pratest_get_review_options($this->get_pratest(),
                        $this->attempt, $this->pratestobj->get_context());
            ...
            $options = mod_pratest_display_options::make_from_pratest($this->get_pratest(),
                    mod_pratest_display_options::DURING);
            $options->flags = pratest_get_flag_option($this->attempt, $this->pratestobj->get_context());
            return $options;
        }
    }
     ...
     * @return bool true if this is the last page of the pratest.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the pratest, or for the
     * whole pratest.
     *
    ...
    public function get_question_usage() {
        if (!PHPUNIT_TEST) {
            throw new coding_exception('get_question_usage is only for use in unit tests. ' .
                    'For other operations, use the pratest_attempt api, or extend it properly.');
        }
        return $this->quba;
    }
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_pratest()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the pratest where this question appears.
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the pratest this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the pratest.
     */
    public function get_question_mark($slot) {
        return pratest_format_question_grade($this->get_pratest(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = array();
        if ($this->pratestobj->get_pratest()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->pratestobj->get_pratest()->timelimit;
        }
        if ($this->pratestobj->get_pratest()->timeclose) {
            $deadlines[] = $this->pratestobj->get_pratest()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->pratestobj->get_pratest()->graceperiod;
    ...
    // URLs related to this attempt ============================================
    /**
     * @return string pratest view url.
     */
    public function view_url() {
        return $this->pratestobj->view_url();
    }
    ...
    public function start_attempt_url($slot = null, $page = -1) {
        ...
        return $this->pratestobj->start_attempt_url($page);
    }
    ...
    /**
     * @return string the URL of this pratest's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/pratest/summary.php', array('attempt' => $this->attempt->id, 'cmid' => $this->get_cmid()));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/pratest/processattempt.php');
    }
    ...
    public function cannot_review_message($short = false) {
        return $this->pratestobj->cannot_review_message(
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'pratest'));
        } else {
            return '';
        }
    }
     ...
     * @param mod_pratest_renderer $renderer the pratest renderer.
     ...
     */
    public function render_question($slot, $reviewing, mod_pratest_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions,
                    ...
                    'mod_pratest-blocked_question_warning');
        }
     ...
     */
    protected function render_question_helper($slot, $reviewing, $thispageurl, mod_pratest_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'pratest', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'pratest');
        ...
        $placeholderqa->start($this->get_pratest()->preferredbehaviour, 1);
     ...
     *
     * @param int $id the id of a question in this pratest attempt.
     ...
     * @return string HTML for the question in its current state.
     */
    public function render_question_at_step($slot, $seq, $reviewing, mod_pratest_renderer $renderer, $thispageurl = '') {
     ...
     *
     * @param $panelclass The type of panel, pratest_attempt_nav_panel or pratest_review_nav_panel
     * @param $page the current page number.
     * @param $showall whether we are showing the whole pratest on one page. (Used by review.php)
     * @return pratest_nav_panel_base the requested object.
     */
    public function get_navigation_panel(mod_pratest_renderer $output,
             $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_pratest_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->attributes['aria-labelledby'] = 'mod_pratest_navblock_title';
        $bc->title = html_writer::span(get_string('pratestnavigation', 'pratest'), '', array('id' => 'mod_pratest_navblock_title'));
        $bc->content = $output->navigation_panel($panel);
        return $bc;
    }

    /**
     * Return an array of variant URLs to other attempts at this pratest.
     *
     * The $url passed in must contain an attempt parameter.
     *
     * The {@link mod_pratest_links_to_other_attempts} object returned contains an
     ...
     * @return mod_pratest_links_to_other_attempts containing array int => null|moodle_url.
     */
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = pratest_get_user_attempts($this->get_pratest()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }

        $links = new mod_pratest_links_to_other_attempts();
     ...
     * @return mod_pratest_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->pratestobj->get_pratest()->graceperiod;
        ...
        // Otherwise, we were in pratest_attempt::IN_PROGRESS, and time has now expired.
        // Transition to the appropriate state.
        switch ($this->pratestobj->get_pratest()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if ($becomingoverdue) {
            ...
            $DB->update_record('pratest_attempts', $this->attempt);
        }

        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            pratest_save_best_grade($this->get_pratest(), $this->get_userid());
        }
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_pratest\question\qubaids_for_users_attempts(
                $this->get_pratestid(), $this->get_userid());
        ...
        if ($questiondata->qtype != 'random') {
            ...
            $tagids = pratest_retrieve_slot_tag_ids($this->slots[$slot]->id);
            ...
            if ($newqusetionid === null) {
                throw new moodle_exception('notenoughrandomquestions', 'pratest',
                        $this->pratestobj->view_url(), $questiondata);
            }
        }
    ...
    public function process_finish($timestamp, $processsubmitted) {
        ...
        if (!$this->is_preview()) {
            pratest_save_best_grade($this->get_pratest(), $this->attempt->userid);

            // Trigger event.
            $this->fire_state_transition_event('\mod_pratest\event\attempt_submitted', $timestamp);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('pratest_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_pratest\event\attempt_becameoverdue', $timestamp);

        $transaction->allow_commit();

        pratest_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_pratest\event\attempt_abandoned', $timestamp);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp) {
        global $USER;
        $pratestrecord = $this->get_pratest();
        $params = array(
            'context' => $this->get_pratestobj()->get_context(),
            'courseid' => $this->get_courseid(),
            'objectid' => $this->attempt->id,
            'relateduserid' => $this->attempt->userid,
            'other' => array(
                'submitterid' => CLI_SCRIPT ? null : $USER->id,
                'pratestid' => $pratestrecord->id
            )
        );

        $event = $eventclass::create($params);
        $event->add_record_snapshot('pratest', $this->get_pratest());
        $event->add_record_snapshot('pratest_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    /**
     * Get a URL for a particular question on a particular page of the pratest.
     * Used by {@link attempt_url()} and {@link review_url()}.
     *
     * @param string $script. Used in the URL like /mod/pratest/$script.php
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/pratest/' . $script . '.php' . $fragment,
    ...
    /**
     * Process responses during an attempt at a pratest.
     *
    ...
    public function process_attempt($timenow, $finishattempt, $timeup, $thispage) {
        ...
        // to show the student another page of the pratest. Just finish now.
        $graceperiodmin = null;
        ...
        if ($timeclose !== false && $timenow > $timeclose - QUIZ_MIN_TIME_TO_CONTINUE) {
            $timeup = true;
            $graceperiodmin = get_config('pratest', 'graceperiodmin');
        ...
        if ($timeup) {
            if ($this->get_pratest()->overduehandling == 'graceperiod') {
                if (is_null($graceperiodmin)) {
                    $graceperiodmin = get_config('pratest', 'graceperiodmin');
                }
                if ($timenow > $timeclose + $this->get_pratest()->graceperiod + $graceperiodmin) {
        ...
        // Update the pratest attempt record.
        try {
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('pratest_attempts', 'currentpage', $page, array('id' => $this->get_attemptid()));
            return true;
        }
        return false;
    }
 ...
 * @since      Moodle 2.9
 */
class pratest_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class pratest_nav_question_button implements renderable {
 ...
 * @since      Moodle 2.0
 */
abstract class pratest_nav_panel_base {
    /** @var pratest_attempt */
    ...
    public function __construct(pratest_attempt $attemptobj,
    ...
    /**
     * Get the buttons and section headings to go in the pratest navigation block.
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            if ($heading = $this->attemptobj->get_heading_before_slot($slot)) {
                $buttons[] = new pratest_nav_section_heading(format_string($heading));
            }

            $qa = $this->attemptobj->get_question_attempt($slot);
            $showcorrectness = $this->options->correctness && $qa->has_marks();

            $button = new pratest_nav_question_button();
            $button->id          = 'pratestnavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'pratest');
            }
            $buttons[] = $button;
        }
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'pratest');
        } else {
            return get_string('viewed', 'pratest');
        }
    }

    public function render_before_button_bits(mod_pratest_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_pratest_renderer $output);
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_pratest()->showuserpicture == QUIZ_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', array('id' => $this->attemptobj->get_userid()));
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_pratest()->showuserpicture == QUIZ_SHOWIMAGE_LARGE) {
...
/**
 * Specialisation of {@link pratest_nav_panel_base} for the attempt pratest page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class pratest_attempt_nav_panel extends pratest_nav_panel_base {
    ...
    public function render_before_button_bits(mod_pratest_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'pratest'),
                array('id' => 'pratestnojswarning'));
    }

    public function render_end_bits(mod_pratest_renderer $output) {
        return html_writer::link($this->attemptobj->summary_url(),
                get_string('endtest', 'pratest'), array('class' => 'endtestlink')) .
...
/**
 * Specialisation of {@link pratest_nav_panel_base} for the review pratest page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class pratest_review_nav_panel extends pratest_nav_panel_base {
    public function get_question_url($slot) {
