...
/**
 * Base class for the table used by a {@link pratest_attempts_report}.
 *
 * @package   mod_pratest
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class pratest_attempts_report_table extends table_sql {
    ...
    /** @var object the pratest settings for the pratest we are reporting on. */
    protected $pratest;
    ...
    /** @var object mod_pratest_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this pratest.. */
    protected $questions;
     ...
     * @param mod_pratest_attempts_report_options $options
     ...
     */
    public function __construct($uniqueid, $pratest, $context, $qmsubselect,
            mod_pratest_attempts_report_options $options, \core\dml\sql_join $groupstudentsjoins, \core\dml\sql_join $studentsjoins,
            $questions, $reporturl) {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/pratest/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'pratest'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return pratest_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = pratest_report_feedback_for_grade(
                pratest_rescale_grade($attempt->sumgrades, $this->pratest, false),
                $this->pratest->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/pratest/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'pratest')));
    ...
    public function base_sql(\core\dml\sql_join $allowedstudentsjoins) {
        ...
        // Please note this uniqueid column is not the same as pratesta.uniqueid.
        $fields = 'DISTINCT ' . $DB->sql_concat('u.id', "'#'", 'COALESCE(pratesta.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                pratesta.uniqueid AS usageid,
                pratesta.id AS attempt,
                u.id AS userid,
                u.idnumber, ' . $allnames . ',
                u.picture,
                u.imagealt,
                ...
                pratesta.timefinish,
                pratesta.timestart,
                CASE WHEN pratesta.timefinish = 0 THEN null
                     WHEN pratesta.timefinish > pratesta.timestart THEN pratesta.timefinish - pratesta.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and pratest_attempts tables.
        $from = " {user} u";
        $from .= "\nLEFT JOIN {pratest_attempts} pratesta ON
                                    pratesta.userid = u.id AND pratesta.pratest = :pratestid";
        $params = array('pratestid' => $this->pratest->id);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (pratesta.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = pratest_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case pratest_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'pratesta.id IS NOT NULL AND pratesta.preview = 0';
                break;
            case pratest_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "pratesta.preview = 0 AND pratesta.id IS NOT NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case pratest_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "pratesta.id IS NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case pratest_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "(pratesta.preview = 0 OR pratesta.preview IS NULL) AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (pratesta.state $statesql OR pratesta.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{pratest_attempts} {$alias}pratesta", "{$alias}pratesta.uniqueid",
                "{$alias}pratesta.pratest = :{$alias}pratestid", array("{$alias}pratestid" => $this->sql->params['pratestid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = pratesta.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join("(
                SELECT DISTINCT pratesta.uniqueid
                  FROM " . $this->sql->from . "
                 WHERE " . $this->sql->where . "
                    ) pratestasubquery", 'pratestasubquery.uniqueid',
                    "1 = 1", $this->sql->params);
        }
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['pratesta.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    public function wrap_html_finish() {
        ...
        echo '<a id="checkattempts" href="#">' .
                get_string('selectall', 'pratest') . '</a> / ';
        echo '<a id="uncheckattempts" href="#">' .
                get_string('selectnone', 'pratest') . '</a> ';
