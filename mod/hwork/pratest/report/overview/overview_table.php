...
/**
 * This file defines the pratest grades table.
 *
...
require_once($CFG->dirroot . '/mod/pratest/report/attemptsreport_table.php');


/**
 * This is a table subclass for displaying the pratest grades report.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class pratest_overview_table extends pratest_attempts_report_table {
     ...
     * @param pratest_overview_options $options
     * @param \core\dml\sql_join $groupstudentsjoins
     * @param \core\dml\sql_join $studentsjoins
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($pratest, $context, $qmsubselect,
            pratest_overview_options $options, \core\dml\sql_join $groupstudentsjoins,
            \core\dml\sql_join $studentsjoins, $questions, $reporturl) {
        parent::__construct('mod-pratest-report-overview-report', $pratest , $context,
    ...
    /**
     * Calculate the average overall and question scores for a set of attempts at the pratest.
     *
    ...
    public function compute_average_row($label, \core\dml\sql_join $usersjoins) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(pratestaouter.sumgrades) AS grade, COUNT(pratestaouter.sumgrades) AS numaveraged
                  FROM {pratest_attempts} pratestaouter
                  JOIN (
                       SELECT DISTINCT pratesta.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON pratestaouter.id = relevant_attempt_ids.id
                ", $params);
        $record->grade = pratest_rescale_grade($record->grade, $this->pratest, false);
        ...
        $averagerow = array(
            ...
            'feedbacktext' => strip_tags(pratest_report_feedback_for_grade(
                                         $record->grade, $this->pratest->id, $this->context))
        );

        if ($this->options->slotmarks) {
            $dm = new question_engine_data_mapper();
            $qubaids = new qubaid_join("{pratest_attempts} pratestaouter
                  JOIN (
                       SELECT DISTINCT pratesta.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON pratestaouter.id = relevant_attempt_ids.id",
                    'pratestaouter.uniqueid', '1 = 1', $params);
    ...
    protected function format_average_grade_for_questions($gradeaverages) {
        ...
        foreach ($this->questions as $question) {
            if (isset($gradeaverages[$question->slot]) && $question->maxmark > 0) {
                ...
                $record->grade = pratest_rescale_grade(
                        $record->averagefraction * $question->maxmark, $this->pratest, false);
    ...
    protected function format_average($record, $question = false) {
        if (is_null($record->grade)) {
            ...
            $average = pratest_format_question_grade($this->pratest, $record->grade);
        } else {
            $average = pratest_format_grade($this->pratest, $record->grade);
        }
    ...
    protected function submit_buttons() {
        if (has_capability('mod/pratest:regrade', $this->context)) {
            echo '<input type="submit" class="btn btn-secondary m-r-1" name="regrade" value="' .
                    get_string('regradeselected', 'pratest_overview') . '"/>';
        }
    ...
    public function col_sumgrades($attempt) {
        ...
        $grade = pratest_rescale_grade($attempt->sumgrades, $this->pratest);
        if ($this->is_downloading()) {
            return $grade;
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid])) {
            ...
            $newsumgrade = pratest_rescale_grade($newsumgrade, $this->pratest);
            $oldsumgrade = pratest_rescale_grade($oldsumgrade, $this->pratest);
            $grade = html_writer::tag('del', $oldsumgrade) . '/' .
                    html_writer::empty_tag('br') . $newsumgrade;
        }
        return html_writer::link(new moodle_url('/mod/pratest/review.php',
                array('attempt' => $attempt->attempt)), $grade,
                array('title' => get_string('reviewattempt', 'pratest')));
    }

    /**
     * @param string $colname the name of the column.
     * @param object $attempt the row of data - see the SQL in display() in
     * mod/pratest/report/overview/report.php to see what fields are present,
    ...
    public function other_cols($colname, $attempt) {
        ...
        if ($question->maxmark == 0) {
            ...
            $grade = pratest_rescale_grade(
                    $stepdata->fraction * $question->maxmark, $this->pratest, 'question');
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid][$slot])) {
            $gradefromdb = $grade;
            $newgrade = pratest_rescale_grade(
                    $this->regradedqs[$attempt->usageid][$slot]->newfraction * $question->maxmark,
                    $this->pratest, 'question');
            $oldgrade = pratest_rescale_grade(
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'pratest_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'pratest_overview');
        }
    }
    ...
    protected function update_sql_after_count($fields, $from, $where, $params) {
        $fields .= ", COALESCE((
                                SELECT MAX(qqr.regraded)
                                  FROM {pratest_overview_regrades} qqr
                                 WHERE qqr.questionusageid = pratesta.uniqueid
                          ), -1) AS regraded";
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/pratest:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('pratest_overview_regrades',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return pratest_report_index_by_keys($regradedqs, array('questionusageid', 'slot'));
    }
}
