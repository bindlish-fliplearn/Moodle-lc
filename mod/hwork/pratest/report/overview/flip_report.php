...
/**
 * This file defines the pratest overview report class.
 *
 * @package   pratest_overview
...
require_once($CFG->dirroot . '/mod/pratest/report/attemptsreport.php');
require_once($CFG->dirroot . '/mod/pratest/report/overview/overview_options.php');
require_once($CFG->dirroot . '/mod/pratest/report/overview/overview_form.php');
require_once($CFG->dirroot . '/mod/pratest/report/overview/overview_table.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class pratest_filp_overview_report extends pratest_attempts_report {

    public function display($pratest, $cm, $course) {
        global $DB, $OUTPUT, $PAGE;

        list($currentgroup, $studentsjoins, $groupstudentsjoins, $allowedjoins) = $this->init(
                'overview', 'pratest_overview_settings_form', $pratest, $cm, $course);

        $options = new pratest_overview_options('overview', $pratest, $cm, $course);
        ...
        $table = new pratest_overview_table($pratest, $this->context, $this->qmsubselect,
                $options, $groupstudentsjoins, $studentsjoins, $questions, $options->get_url());
//        $filename = pratest_report_download_filename(get_string('overviewfilename', 'pratest_overview'),
//                $courseshortname, $pratest->name);
//        $table->is_downloading($options->download, $filename,
//                $courseshortname . ' ' . format_string($pratest->name, true));
...
//        }
        ...
        if ($options->attempts == self::ALL_WITH) {
            ...
            // groups mode, so setting allowed to empty (which means all pratest attempts
            // are accessible, is not a security porblem.
            $allowedjoins = new \core\dml\sql_join();
        }

        $this->course = $course; // Hack to make this available in process_actions.
        $this->process_actions($pratest, $cm, $currentgroup, $groupstudentsjoins, $allowedjoins, $options->get_url());

        $hasquestions = pratest_has_questions($pratest->id);

        // Start output.
        if (!$table->is_downloading()) {
            // Only print headers if not asked to download data.
            $this->print_standard_header_and_messages($cm, $course, $pratest,
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
            ...
            if (!$table->is_downloading()) {
                ...
                if (has_capability('mod/pratest:regrade', $this->context)) {
                    $regradesneeded = $this->count_question_attempts_needing_regrade(
                            $pratest, $groupstudentsjoins);
//                    if ($currentgroup) {
//                        $a= new stdClass();
...
//                                get_string('regradealldrydogroup', 'pratest_overview', $a);
//                        $regradealldrylabel =
//                                get_string('regradealldrygroup', 'pratest_overview', $a);
//                        $regradealllabel =
//                                get_string('regradeallgroup', 'pratest_overview', $a);
//                    } else {
//                        $regradealldrydolabel =
//                                get_string('regradealldrydo', 'pratest_overview', $regradesneeded);
//                        $regradealldrylabel =
//                                get_string('regradealldry', 'pratest_overview');
//                        $regradealllabel =
//                                get_string('regradeall', 'pratest_overview');
//                    }
...
//                    echo '</div>';
                ...
                if ($strattempthighlight = pratest_report_highlighting_grading_method(
                        $pratest, $this->qmsubselect, $options->onlygraded)) {
                    echo '<div class="pratestattemptcounts">' . $strattempthighlight . '</div>';
                }
            }
...
//            $this->add_time_columns($columns, $headers);
            ...
            $this->add_grade_columns($pratest, $options->usercanseegrades, $columns, $headers, false);

            if (!$table->is_downloading() && has_capability('mod/pratest:regrade', $this->context) &&
                    $this->has_regraded_questions($table->sql->from, $table->sql->where, $table->sql->params)) {
                $columns[] = 'regraded';
                $headers[] = get_string('regrade', 'pratest_overview');
            }
            ...
            if ($options->slotmarks) {
                foreach ($questions as $slot => $question) {
                    ...
                    $header = get_string('qbrief', 'pratest', $question->number);
                    if (!$table->is_downloading()) {
                        $header .= '<br />';
                    } else {
                        $header .= ' ';
                    }
                    $header .= '/' . pratest_rescale_grade($question->maxmark, $pratest, 'question');
                    $headers[] = $header;
                }
            }
        ...
        if (!$table->is_downloading() && $options->usercanseegrades) {
            ...
            list($bands, $bandwidth) = self::get_bands_count_and_width($pratest);
            $labels = self::get_bands_labels($bands, $bandwidth, $pratest);
            ...
            if ($currentgroup && $this->hasgroupstudents) {
                $sql = "SELECT qg.id
                        ...
                        {$groupstudentsjoins->joins}
                          WHERE qg.pratest = $pratest->id AND {$groupstudentsjoins->wheres}";
                if ($DB->record_exists_sql($sql, $groupstudentsjoins->params)) {
                    $data = pratest_report_grade_bands($bandwidth, $bands, $pratest->id, $groupstudentsjoins);
                    $chart = self::get_chart($labels, $data);
                    $graphname = get_string('overviewreportgraphgroup', 'pratest_overview', groups_get_group_name($currentgroup));
                    //echo $output->chart($chart, $graphname);
                }
            }

            if ($DB->record_exists('pratest_grades', array('pratest'=> $pratest->id))) {
                $data = pratest_report_grade_bands($bandwidth, $bands, $pratest->id, new \core\dml\sql_join());
                $chart = self::get_chart($labels, $data);
                $graphname = get_string('overviewreportgraph', 'pratest_overview');
                //echo $output->chart($chart, $graphname);
            }
        }
        return true;
    ...
    protected function process_actions($pratest, $cm, $currentgroup, \core\dml\sql_join $groupstudentsjoins,
        ...
        parent::process_actions($pratest, $cm, $currentgroup, $groupstudentsjoins, $allowedjoins, $redirecturl);

        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('regrade', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    $this->start_regrade($pratest, $cm);
                    $this->regrade_attempts($pratest, false, $groupstudentsjoins, $attemptids);
        ...
        if (optional_param('regradeall', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($pratest, $cm);
            $this->regrade_attempts($pratest, false, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);

        } else if (optional_param('regradealldry', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($pratest, $cm);
            $this->regrade_attempts($pratest, true, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);

        } else if (optional_param('regradealldrydo', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($pratest, $cm);
            $this->regrade_attempts_needing_it($pratest, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);
        }
    }
     ...
     */
    protected function start_regrade($pratest, $cm) {
        require_capability('mod/pratest:regrade', $this->context);
        $this->print_header_and_tabs($cm, $this->course, $pratest, $this->mode);
    }
    ...
    protected function finish_regrade($nexturl) {
        global $OUTPUT;
        \core\notification::success(get_string('regradecomplete', 'pratest_overview'));
    ...
    /**
     * Regrade a particular pratest attempt. Either for real ($dryrun = false), or
     * as a pretend regrade to see which fractions would change. The outcome is
     * stored in the pratest_overview_regrades table.
     *
     ...
     *
     * @param object $attempt the pratest attempt to regrade.
    ...
    protected function regrade_attempt($attempt, $dryrun = false, $slots = null) {
        global $DB;
        // Need more time for a pratest with many questions.
        ...
        $finished = $attempt->state == pratest_attempt::FINISHED;
        foreach ($slots as $slot) {
            ...
            if (abs($qqr->oldfraction - $qqr->newfraction) > 1e-7) {
                ...
                $DB->insert_record('pratest_overview_regrades', $qqr, false);
            }
        }
    ...
    /**
     * Regrade attempts for this pratest, exactly which attempts are regraded is
     ...
     */
    protected function regrade_attempts($pratest, $dryrun = false,
        ...
        $where = "pratest = :qid AND preview = 0";
        $params = array('qid' => $pratest->id);
        ...
        $this->clear_regrade_table($pratest, $groupstudentsjoins);

        $progressbar = new progress_bar('pratest_overview_regrade', 500, true);
        $a = array(
        ...
        foreach ($attempts as $attempt) {
            ...
            $progressbar->update($a['done'], $a['count'],
                    get_string('regradingattemptxofy', 'pratest_overview', $a));
        }

        if (!$dryrun) {
            $this->update_overall_grades($pratest);
        }
    }

    /**
     * Regrade those questions in those attempts that are marked as needing regrading
     * in the pratest_overview_regrades table.
     * @param object $pratest the pratest settings.
     * @param \core\dml\sql_join $groupstudentsjoins empty for all attempts, otherwise regrade attempts
     * for these users.
     */
    protected function regrade_attempts_needing_it($pratest, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        $this->unlock_session();

        $join = '{pratest_overview_regrades} qqr ON qqr.questionusageid = pratesta.uniqueid';
        $where = "pratesta.pratest = :qid AND pratesta.preview = 0 AND qqr.regraded = 0";
        $params = array('qid' => $pratest->id);
        ...
        $toregrade = $DB->get_recordset_sql("
                SELECT pratesta.uniqueid, qqr.slot
                  FROM {pratest_attempts} pratesta
                  JOIN $join
                 WHERE $where", $params);
        ...
        $attempts = $DB->get_records_list('pratest_attempts', 'uniqueid',
     ...
     */
    protected function count_question_attempts_needing_regrade($pratest, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        ...
        if ($this->hasgroupstudents) {
            $userjoin = "JOIN {user} u ON u.id = pratesta.userid
                    {$groupstudentsjoins->joins}";
            $usertest = "{$groupstudentsjoins->wheres} AND u.id = pratesta.userid AND ";
            $params = $groupstudentsjoins->params;
        }

        $params['cpratest'] = $pratest->id;
        $sql = "SELECT COUNT(DISTINCT pratesta.id)
                  FROM {pratest_attempts} pratesta
                  JOIN {pratest_overview_regrades} qqr ON pratesta.uniqueid = qqr.questionusageid
                $userjoin
                 WHERE
                      $usertest
                      pratesta.pratest = :cpratest AND
    ...
    protected function has_regraded_questions($from, $where, $params) {
        ...
        return $DB->record_exists_sql("
                SELECT 1
                  FROM {$from}
                  JOIN {pratest_overview_regrades} qor ON qor.questionusageid = pratesta.uniqueid
                 WHERE {$where}", $params);
    }
     ...
     * to these users is cleared.
     */
    protected function clear_regrade_table($pratest, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        ...
        $select = "questionusageid IN (
                    SELECT uniqueid
                      FROM {pratest_attempts} pratesta";
        $where = "WHERE pratesta.pratest = :qid";
        $params = array('qid' => $pratest->id);
        if ($this->hasgroupstudents && !empty($groupstudentsjoins->joins)) {
            $select .= "\nJOIN {user} u ON u.id = pratesta.userid
                    {$groupstudentsjoins->joins}";
        ...
        $DB->delete_records_select('pratest_overview_regrades', $select, $params);
    }
     ...
     */
    protected function update_overall_grades($pratest) {
        pratest_update_all_attempt_sumgrades($pratest);
        pratest_update_all_final_grades($pratest);
        pratest_update_grades($pratest);
    }

    /**
     * Get the bands configuration for the pratest.
     *
     * This returns the configuration for having between 11 and 20 bars in
     * a chart based on the maximum grade to be given on a pratest. The width of
     ...
     */
    public static function get_bands_count_and_width($pratest) {
        $bands = $pratest->grade;
        ...
        return [$bands, $pratest->grade / $bands];
    }
     ...
     */
    public static function get_bands_labels($bands, $bandwidth, $pratest) {
        $bandlabels = [];
        for ($i = 1; $i <= $bands; $i++) {
            $bandlabels[] = pratest_format_grade($pratest, ($i - 1) * $bandwidth) . ' - ' . pratest_format_grade($pratest, $i * $bandwidth);
        }
        return $bandlabels;
    }
