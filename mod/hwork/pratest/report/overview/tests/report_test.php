...
/**
 * Tests for the pratest overview report.
 *
...
require_once($CFG->dirroot . '/mod/pratest/report/overview/report.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class pratest_overview_report_testcase extends advanced_testcase {
    ...
    public function test_report_sql($isdownloading) {
        ...
        // Create a course and a pratest.
        $generator = $this->getDataGenerator();
        $course = $generator->create_course();
        $pratestgenerator = $generator->get_plugin_generator('mod_pratest');
        $pratest = $pratestgenerator->create_instance(array('course' => $course->id,
        ...
        pratest_add_pratest_question($q->id, $pratest, 0 , 10);
        ...
        $attempts = array(
            array($pratest, $student1, 1, 0.0,  pratest_attempt::FINISHED),
            array($pratest, $student1, 2, 5.0,  pratest_attempt::FINISHED),
            array($pratest, $student1, 3, 8.0,  pratest_attempt::FINISHED),
            array($pratest, $student1, 4, null, pratest_attempt::ABANDONED),
            array($pratest, $student1, 5, null, pratest_attempt::IN_PROGRESS),
            array($pratest, $student2, 1, null, pratest_attempt::ABANDONED),
            array($pratest, $student2, 2, null, pratest_attempt::ABANDONED),
            array($pratest, $student2, 3, 7.0,  pratest_attempt::FINISHED),
            array($pratest, $student2, 4, null, pratest_attempt::ABANDONED),
            array($pratest, $student2, 5, null, pratest_attempt::ABANDONED),
        );

        // Load it in to pratest attempts table.
        foreach ($attempts as $attemptdata) {
            list($pratest, $student, $attemptnumber, $sumgrades, $state) = $attemptdata;
            ...
            $attempt = pratest_create_attempt($pratestobj, $attemptnumber, null, $timestart, false, $student->id);

            $attempt = pratest_start_new_attempt($pratestobj, $quba, $attempt, $attemptnumber, $timestamp);
            ...
            switch ($state) {
                ...
                case pratest_attempt::FINISHED:
                    ...
                    $DB->update_record('pratest_attempts', $update);
                    pratest_save_best_grade($attemptobj->get_pratest(), $student->id);
                    break;
            }
        }
        ...
        $qmsubselect = pratest_report_qm_filter_select($pratest);
        $studentsjoins = get_enrolled_with_capabilities_join($context);
        $empty = new \core\dml\sql_join();

        // Set the options.
        $reportoptions = new pratest_overview_options('overview', $pratest, $cm, null);
        $reportoptions->attempts = pratest_attempts_report::ENROLLED_ALL;
        $reportoptions->onlygraded = true;
        $reportoptions->states = array(pratest_attempt::IN_PROGRESS, pratest_attempt::OVERDUE, pratest_attempt::FINISHED);

        // Now do a minimal set-up of the table class.
        $q->slot = 1;
        $q->maxmark = 10;
        $table = new pratest_overview_table($pratest, $context, $qmsubselect, $reportoptions,
        ...
        $table->define_baseurl(new moodle_url('/mod/pratest/report.php'));
        $table->setup();
    ...
    public function test_get_bands_count_and_width($grade, $expected) {
        ...
        $pratest = $pratestgenerator->create_instance(['course' => SITEID, 'grade' => $grade]);
        $this->assertEquals($expected, pratest_overview_report::get_bands_count_and_width($pratest));
    }
