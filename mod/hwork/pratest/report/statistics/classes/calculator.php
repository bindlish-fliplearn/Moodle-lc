...
/**
 * Class to calculate and also manage caching of pratest statistics.
 *
...
class calculator {
    ...
    /**
     * Compute the pratest statistics.
     *
     ...
     * @return calculated $prateststats The statistics for overall attempt scores.
     */
    public function calculate($pratestid, $whichattempts, \core\dml\sql_join $groupstudentsjoins, $p, $sumofmarkvariance) {

        $this->progress->start_progress('', 3);

        $prateststats = new calculated($whichattempts);

        $countsandaverages = $this->attempt_counts_and_averages($pratestid, $groupstudentsjoins);
        $this->progress->progress(1);

        foreach ($countsandaverages as $propertyname => $value) {
            $prateststats->{$propertyname} = $value;
        }
        ...
        if ($s != 0) {
            ...
            list($fromqa, $whereqa, $qaparams) =
                pratest_statistics_attempts_sql($pratestid, $groupstudentsjoins, $whichattempts);

            $prateststats->median = $this->median($s, $fromqa, $whereqa, $qaparams);
            $this->progress->progress(2);

            if ($s > 1) {

                $powers = $this->sum_of_powers_of_difference_to_mean($prateststats->avg(), $fromqa, $whereqa, $qaparams);
                $this->progress->progress(3);

                $prateststats->standarddeviation = sqrt($powers->power2 / ($s - 1));
                ...
                if ($s > 2) {
                    ...
                    $k3 = $s * $s * $m3 / (($s - 1) * ($s - 2));
                    if ($k2 != 0) {
                        $prateststats->skewness = $k3 / (pow($k2, 3 / 2));

                        // Kurtosis.
                        if ($s > 3) {
                            $k4 = $s * $s * ((($s + 1) * $m4) - (3 * ($s - 1) * $m2 * $m2)) / (($s - 1) * ($s - 2) * ($s - 3));
                            $prateststats->kurtosis = $k4 / ($k2 * $k2);
                        }

                        if ($p > 1) {
                            $prateststats->cic = (100 * $p / ($p - 1)) * (1 - ($sumofmarkvariance / $k2));
                            $prateststats->errorratio = 100 * sqrt(1 - ($prateststats->cic / 100));
                            $prateststats->standarderror = $prateststats->errorratio *
                                $prateststats->standarddeviation / 100;
                        }
                    }
            ...
            $prateststats->cache(pratest_statistics_qubaids_condition($pratestid, $groupstudentsjoins, $whichattempts));
        }
    ...
    public function get_cached($qubaids) {
        ...
        $fromdb = $DB->get_record_select('pratest_statistics', 'hashcode = ? AND timemodified > ?',
    ...
    public function get_last_calculated_time($qubaids) {
        ...
        $timemodified = time() - self::TIME_TO_CACHE;
        return $DB->get_field_select('pratest_statistics', 'timemodified', 'hashcode = ? AND timemodified > ?',
                                         array($qubaids->get_hash_code(), $timemodified));
    }

    /**
     * Given a particular pratest grading method return a lang string describing which attempts contribute to grade.
     *
    ...
    public static function using_attempts_lang_string($whichattempts) {
         return get_string(static::using_attempts_string_id($whichattempts), 'pratest_statistics');
    }

    /**
     * Given a particular pratest grading method return a string id for use as a field name prefix in mdl_pratest_statistics or to
     ...
     */
    protected function attempt_counts_and_averages($pratestid, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;

        $attempttotals = new \stdClass();
        foreach (array_keys(pratest_get_grading_options()) as $which) {

            list($fromqa, $whereqa, $qaparams) = pratest_statistics_attempts_sql($pratestid, $groupstudentsjoins, $which);
    ...
    protected function median($s, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT pratesta.id, pratesta.sumgrades
                  FROM $fromqa
                 WHERE $whereqa
              ORDER BY sumgrades";
    ...
    protected function sum_of_powers_of_difference_to_mean($mean, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT
                    SUM(POWER((pratesta.sumgrades - $mean), 2)) AS power2,
                    SUM(POWER((pratesta.sumgrades - $mean), 3)) AS power3,
                    SUM(POWER((pratesta.sumgrades - $mean), 4)) AS power4
                  FROM $fromqa
                 WHERE $whereqa";
