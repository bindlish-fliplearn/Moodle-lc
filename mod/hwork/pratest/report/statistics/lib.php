...
/**
 * Standard plugin entry points of the pratest statistics report.
 *
 ...
 * @package  pratest_statistics
 * @category files
 * @param context $previewcontext the pratest context
 ...
 */
function pratest_statistics_question_preview_pluginfile($previewcontext, $questionid,
    ...
    require_capability('pratest/statistics:view', $context);
 ...
 */
function pratest_statistics_cron() {
    global $DB;

    mtrace("\n  Cleaning up old pratest statistics cache records...", '');

    $expiretime = time() - 5*HOURSECS;
    $DB->delete_records_select('pratest_statistics', 'timemodified < ?', array($expiretime));
