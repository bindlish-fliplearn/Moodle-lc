...
/**
 * This file defines the pratest manual grading report class.
 *
...
require_once($CFG->dirroot . '/mod/pratest/report/grading/gradingsettings_form.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class pratest_grading_report extends pratest_default_report {
    ...
    public function display($pratest, $cm, $course) {
        ...
        require_capability('mod/pratest:grade', $this->context);
        $shownames = has_capability('pratest/grading:viewstudentnames', $this->context);
        $showidnumbers = has_capability('pratest/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this pratest.
        $this->questions = pratest_report_get_significant_questions($pratest);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'pratest_grading');
        }
        ...
        if ($this->currentgroup == self::NO_GROUPS_ALLOWED) {
            ...
            $this->userssql = get_enrolled_sql($this->context,
                    array('mod/pratest:reviewmyattempts', 'mod/pratest:attempt'), $this->currentgroup);
        }
        ...
        if ($slot && $hasquestions) {
            ...
            if (!$counts || $counts->$grade == 0) {
                redirect($this->list_questions_url(), get_string('alldoneredirecting', 'pratest_grading'));
            }
        }

        // Start output.
        $this->print_header_and_tabs($cm, $course, $pratest, 'grading');
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "pratesta.pratest = :mangrpratestid AND
                pratesta.preview = 0 AND
                pratesta.state = :statefinished";
        $params = array('mangrpratestid' => $this->cm->instance, 'statefinished' => pratest_attempt::FINISHED);
        ...
        if ($currentgroup) {
            ...
            if ($enrolleduserscount < 1) {
                $where .= ' AND pratesta.userid = 0';
            } else {
                $usersjoin = "JOIN ({$userssql[0]}) AS enr ON pratesta.userid = enr.id";
                $params += $userssql[1];
            }
        }

        return new qubaid_join("{pratest_attempts} pratesta $usersjoin ", 'pratesta.uniqueid', $where, $params);
    }
    ...
    protected function load_attempts_by_usage_ids($qubaids) {
        ...
        $params[] = pratest_attempt::FINISHED;
        $params[] = $this->pratest->id;

        $fields = 'pratesta.*, u.idnumber, ';
        $fields .= get_all_user_name_fields(true, 'u');
        $attemptsbyid = $DB->get_records_sql("
                SELECT $fields
                FROM {pratest_attempts} pratesta
                JOIN {user} u ON u.id = pratesta.userid
                WHERE pratesta.uniqueid $asql AND pratesta.state = ? AND pratesta.pratest = ?",
                $params);
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                    ...
                    get_string($gradestring, 'pratest_grading'),
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'pratest_grading'), 3);
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'pratest_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'pratest_grading');
        }
        ...
        if (empty($data)) {
            echo $OUTPUT->notification(get_string('nothingfound', 'pratest_grading'));
            return;
        }
        ...
        $table->head[] = get_string('qno', 'pratest_grading');
        $table->head[] = get_string('qtypeveryshort', 'question');
        $table->head[] = get_string('questionname', 'pratest_grading');
        $table->head[] = get_string('tograde', 'pratest_grading');
        $table->head[] = get_string('alreadygraded', 'pratest_grading');
        if ($includeauto) {
            $table->head[] = get_string('automaticallygraded', 'pratest_grading');
        }
        $table->head[] = get_string('total', 'pratest_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new pratest_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'pratest_grading', $a), 3);
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(),
                get_string('backtothelistofquestions', 'pratest_grading')),
        ...
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'pratest_grading', $a), 3);
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = pratest_get_review_options($this->pratest, $attempt, $this->context);
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                'type' => 'submit', 'class' => 'btn btn-primary', 'value' => get_string('saveandnext', 'pratest_grading'))),
                array('class' => 'mdl-align')) .
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'pratest_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'pratest_grading', $a);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new pratest_attempt($attempt, $this->pratest, $this->cm, $this->course);
            ...
            $events[] = \mod_pratest\event\question_manually_graded::create($params);
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        if ($orderby == 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON pratesta.userid = u.id ";
