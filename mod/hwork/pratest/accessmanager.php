...
/**
 * Classes to enforce the various access rules that can apply to a pratest.
 *
 * @package   mod_pratest
 ...
 * pratest, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
 */
class pratest_access_manager {
    /** @var pratest the pratest settings object. */
    protected $pratestobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of pratest_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular pratest.
     * @param object $pratestobj An instance of the class pratest from attemptlib.php.
     *      The pratest we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/pratest:ignoretimelimits', ...)).
     */
    public function __construct($pratestobj, $timenow, $canignoretimelimits) {
        $this->pratestobj = $pratestobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($pratestobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular pratest.
     * @param pratest $pratestobj information about the pratest in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/pratest:ignoretimelimits capability.
     * @return array of {@link pratest_access_rule_base}s.
     */
    protected function make_rules($pratestobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($pratestobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['pratestaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('pratestaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the pratest table.
     *
     * @param mod_pratest_mod_form $pratestform the pratest settings form that is being built.
    ...
    public static function add_settings_form_fields(
        ...
        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($pratestform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'pratest'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_pratest_mod_form $pratestform the pratest form object.
    ...
    public static function validate_settings_form_fields(array $errors,
        ...
        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $pratestform);
        }
    ...
    /**
     * Save any submitted settings when the pratest settings form is submitted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the pratest table.
     *
     * @param object $pratest the data from the pratest form, including $pratest->id
     *      which is the id of the pratest being saved.
     */
    public static function save_settings($pratest) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($pratest);
        }
    }

    /**
     * Delete any rule-specific settings when the pratest is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the pratest table.
     *
     * @param object $pratest the data from the database, including $pratest->id
     *      which is the id of the pratest being deleted.
     * @since Moodle 2.7.1, 2.6.4, 2.5.7
     */
    public static function delete_settings($pratest) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($pratest);
        }
    }
     ...
     */
    protected static function get_load_sql($pratestid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{pratest} pratest';
        $allparams = array('pratestid' => $pratestid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($pratestid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE pratest.id = :pratestid", $allparams);
    }
     ...
     */
    public static function load_settings($pratestid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($pratestid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($pratestid);
        }
    ...
    /**
     * Load the pratest settings and any settings required by the access rules.
     ...
     *
     * @param int $pratestid the pratest id.
     * @return object mdl_pratest row with extra fields.
     */
    public static function load_pratest_and_settings($pratestid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($pratestid, $rules, 'pratest.*');
        $pratest = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($pratestid) as $name => $value) {
                $pratest->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this pratest, such
     * as is shown at the top of the pratest view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this pratest now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_pratest_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        $method = 'post';
        if (!empty($_GET['_qf__mod_pratest_preflight_check_form'])) {
            $method = 'get';
        }
        return new mod_pratest_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'pratestobj' => $this->pratestobj,
     ...
     * pratest. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant pratest_attempts row.
    ...
    /**
     * @return bolean if this pratest should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the pratest view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_pratest_renderer $output the pratest renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->pratestobj->view_url(), $message);
            die();
        } else {
            redirect($this->pratestobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the pratest, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(pratest_attempt::IN_PROGRESS, pratest_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = pratest_attempt_state($this->pratestobj->get_pratest(), $attempt);
        $reviewoptions = mod_pratest_display_options::make_from_pratest(
                $this->pratestobj->get_pratest(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->pratestobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->pratestobj->review_url($attempt->id),
