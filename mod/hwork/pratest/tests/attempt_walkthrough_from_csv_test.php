 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_pratest_attempt_walkthrough_from_csv_testcase extends advanced_testcase {

    protected $files = array('questions', 'steps', 'results');

    /**
     * @var stdClass the pratest record we create.
     */
    protected $pratest;
     ...
     * directory.
     *
     * @param array $pratestsettings of settings read from csv file pratestzes.csv
    ...
    public function test_walkthrough_from_csv($pratestsettings, $csvdata) {
        ...
        // https://github.com/jamiepratt/moodle-pratest-tools/tree/master/responsegenerator

        $this->create_pratest_simulate_attempts_and_check_results($pratestsettings, $csvdata);
    }

    public function create_pratest($pratestsettings, $qs) {
        global $SITE, $DB;
        $this->setAdminUser();
        ...
        $aggregratedsettings = $pratestsettings + array('course' => $SITE->id,
                                                     'questionsperpage' => 0,
                                                     'grade' => 100.0,
                                                     'sumgrades' => $sumofgrades);

        $this->pratest = $pratestgenerator->create_instance($aggregratedsettings);

        $this->randqids = array();
        foreach ($slots as $slotno => $slotquestion) {
            if ($slotquestion['type'] !== 'random') {
                pratest_add_pratest_question($slotquestion['id'], $this->pratest, 0, $slotquestion['mark']);
            } else {
                pratest_add_random_questions($this->pratest, 0, $slotquestion['catid'], 1, 0);
    ...
    /**
     * Create pratest, simulate attempts and check results (if resultsXX.csv exists).
     *
     * @param array $pratestsettings Quiz overrides for this pratest.
     * @param PHPUnit\DbUnit\DataSet\ITable[] $csvdata Data loaded from csv files for this test.
     */
    protected function create_pratest_simulate_attempts_and_check_results($pratestsettings, $csvdata) {
    ...
    public function get_data_for_walkthrough() {
        $pratestzes = $this->load_csv_data_file('pratestzes');
        $datasets = array();
        for ($rowno = 0; $rowno < $pratestzes->getRowCount(); $rowno++) {
            $pratestsettings = $pratestzes->getRow($rowno);
            $dataset = array();
            foreach ($this->files as $file) {
                if (file_exists($this->get_full_path_of_csv_file($file, $pratestsettings['testnumber']))) {
                    $dataset[$file] = $this->load_csv_data_file($file, $pratestsettings['testnumber']);
                }
            }
            $datasets[] = array($pratestsettings, $dataset);
        }
        return $datasets;
    }
     ...
     * @return array attempt no as in csv file => the id of the pratest_attempt as stored in the db.
     */
    protected function walkthrough_attempts($steps) {
        ...
        for ($rowno = 0; $rowno < $steps->getRowCount(); $rowno++) {
            ...
            // Find existing user or make a new user to do the pratest.
            ...
            if (!isset($attemptids[$step['pratestattempt']])) {
                // Start the attempt.
                $pratestobj = pratest::create($this->pratest->id, $user->id);
                ...
                $prevattempts = pratest_get_user_attempts($this->pratest->id, $user->id, 'all', true);
                $attemptnumber = count($prevattempts) + 1;
                $timenow = time();
                $attempt = pratest_create_attempt($pratestobj, $attemptnumber, false, $timenow, false, $user->id);
                ...
                pratest_start_new_attempt($pratestobj, $quba, $attempt, $attemptnumber, $timenow, $step['randqs'], $step['variants']);
                pratest_attempt_save_started($pratestobj, $quba, $attempt);
                $attemptid = $attemptids[$step['pratestattempt']] = $attempt->id;
            } else {
                $attemptid = $attemptids[$step['pratestattempt']];
            }
     ...
     * @param $attemptids array attempt no as in csv file => the id of the pratest_attempt as stored in the db.
     */
    protected function check_attempts_results($results, $attemptids) {
        for ($rowno = 0; $rowno < $results->getRowCount(); $rowno++) {
            ...
            // Re-load pratest attempt data.
            $attemptobj = pratest_attempt::create($attemptids[$result['pratestattempt']]);
     ...
     * @param pratest_attempt $attemptobj         the attempt object loaded from db.
        ...
        foreach ($result as $fieldname => $value) {
            ...
            switch ($fieldname) {
                            ...
                            switch ($slotfieldname) {
                                case 'mark' :
                                    $this->assertEquals(round($slotvalue, 2), $attemptobj->get_question_mark($slotno),
                                                        "Mark for slot $slotno of attempt {$result['pratestattempt']}.");
                                    break;
                                default :
                ...
                case 'finished' :
                    ...
                    break;
                case 'summarks' :
                    $this->assertEquals($value, $attemptobj->get_sum_marks(), "Sum of marks of attempt {$result['pratestattempt']}.");
                    break;
                case 'pratestgrade' :
                    // Check pratest grades.
                    $grades = pratest_get_user_grades($attemptobj->get_pratest(), $attemptobj->get_userid());
                    $grade = array_shift($grades);
                    $this->assertEquals($value, $grade->rawgrade, "Quiz grade for attempt {$result['pratestattempt']}.");
                    break;
                case 'gradebookgrade' :
                    // Check grade book.
                    $gradebookgrades = grade_get_grades($attemptobj->get_courseid(),
                                                        'mod', 'pratest',
                                                        $attemptobj->get_pratestid(),
                                                        $attemptobj->get_userid());
                    $gradebookitem = array_shift($gradebookgrades->items);
                    $gradebookgrade = array_shift($gradebookitem->grades);
                    $this->assertEquals($value, $gradebookgrade->grade, "Gradebook grade for attempt {$result['pratestattempt']}.");
                    break;
                default :
