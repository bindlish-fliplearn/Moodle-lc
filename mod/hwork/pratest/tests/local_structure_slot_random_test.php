...
/**
 * Unit tests for the {@link \mod_pratest\local\structure\slot_random} class.
 *
 * @package    mod_pratest
 * @category   test
...
/**
 * Class mod_pratest_local_structure_slot_random_test
 * Class for tests related to the {@link \mod_pratest\local\structure\slot_random} class.
 *
 * @copyright  2018 Shamim Rezaie <shamim@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_pratest_local_structure_slot_random_test extends advanced_testcase {
    /**
     * Constructor test.
     */
    public function test_constructor() {
        ...
        // Create a random question without adding it to a pratest.
        // We don't want to use pratest_add_random_questions because that itself, instantiates an object from the slot_random class.
        $form = new stdClass();
        ...
        $rc = new ReflectionClass('\mod_pratest\local\structure\slot_random');
        $rcp = $rc->getProperty('record');
        $rcp->setAccessible(true);
        $record = $rcp->getValue($randomslot);

        $this->assertEquals($pratest->id, $record->pratestid);
    ...
    public function test_get_pratest_pratest() {
        global $SITE, $DB;
        ...
        pratest_add_random_questions($pratest, 0, $category->id, 1, false);

        // Get the random question's id. It is at the first slot.
        $questionid = $DB->get_field('pratest_slots', 'questionid', array('pratestid' => $pratest->id, 'slot' => 1));
        ...
        // The create_instance had injected an additional cmid propery to the pratest. Let's remove that.
        unset($pratest->cmid);

        $this->assertEquals($pratest, $randomslot->get_pratest());
    }

    public function test_set_pratest() {
        global $SITE, $DB;
        ...
        $rc = new ReflectionClass('\mod_pratest\local\structure\slot_random');
        $rcp = $rc->getProperty('pratest');
        $rcp->setAccessible(true);
        $pratestpropery = $rcp->getValue($randomslot);

        $this->assertEquals($pratest, $pratestpropery);
    }
    ...
    public function test_insert() {
        ...
        // Insert the random question to the pratest.
        $randomslot = new \mod_pratest\local\structure\slot_random($randomslotdata);
        $randomslot->set_tags([$footag, $bartag]);
        $randomslot->insert(1); // Put the question on the first page of the pratest.

        // Get the random question's pratest_slot. It is at the first slot.
        $pratestslot = $DB->get_record('pratest_slots', array('pratestid' => $pratest->id, 'slot' => 1));
        // Get the random question's tags from pratest_slot_tags. It is at the first slot.
        $pratestslottags = $DB->get_records('pratest_slot_tags', array('slotid' => $pratestslot->id));

        $this->assertEquals($question->id, $pratestslot->questionid);
        $this->assertEquals($category->id, $pratestslot->questioncategoryid);
        $this->assertEquals(1, $pratestslot->includingsubcategories);
        $this->assertEquals(1, $pratestslot->maxmark);

        $this->assertCount(2, $pratestslottags);
        $this->assertEquals(
                [
