@mod @mod_pratest
Feature: Allow students to redo questions in a practice pratest, without starting a whole new attempt
  In order to practice particular skills I am struggling with
  As a student
  I need to be able to redo each question in a pratest as often as necessary without starting a whole new attempt, if my teacher allows it.
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | pratest       | Quiz 1 | Quiz 1 description | C1     | pratest1    | immediatefeedback  | 1                |
  ...
  Scenario: Teachers reviewing can see all the qestions attempted in a slot
    ...
    And I follow "TF1"
    And "False" row "Frequency" column of "pratestresponseanalysis" table should contain "100.00%"
    And "True" row "Frequency" column of "pratestresponseanalysis" table should contain "0.00%"
    And "[No response]" row "Frequency" column of "pratestresponseanalysis" table should contain "100.00%"
