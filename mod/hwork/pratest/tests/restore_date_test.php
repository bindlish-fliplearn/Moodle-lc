 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_pratest_restore_date_testcase extends restore_date_testcase
{
    ...
    public function test_restore_dates() {
        ...
        list($course, $pratest) = $this->create_course_and_module('pratest', $record);
        ...
        // Create an attempt.
        $timestamp = 100;
        $pratestobj = pratest::create($pratest->id);
        $attempt = pratest_create_attempt($pratestobj, 1, false, $timestamp, false);
        ...
        pratest_start_new_attempt($pratestobj, $quba, $attempt, 1, $timestamp);
        ...
        $DB->insert_record('pratest_overrides', $override);

        // Set time fields to a constant for easy validation.
        $DB->set_field('pratest_attempts', 'timefinish', $timestamp);

        // Do backup and restore.
        $newcourseid = $this->backup_and_restore($course);
        $newpratest = $DB->get_record('pratest', ['course' => $newcourseid]);

        $this->assertFieldsNotRolledForward($pratest, $newpratest, ['timecreated', 'timemodified']);
        $props = ['timeclose', 'timeopen'];
        $this->assertFieldsRolledForward($pratest, $newpratest, $props);

        $newattempt = $DB->get_record('pratest_attempts', ['pratest' => $newpratest->id]);
        $newoverride = $DB->get_record('pratest_overrides', ['pratest' => $newpratest->id]);
        $newgrade = $DB->get_record('pratest_grades', ['pratest' => $newpratest->id]);
