...
/**
 * Defines the Moodle forum used to add random questions to the hwork.
 *
 * @package   mod_hwork
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class hwork_add_random_form extends moodleform {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'existingcategoryheader',
                get_string('randomfromexistingcategory', 'hwork'));
        ...
        $mform->addElement('checkbox', 'includesubcategories', '', get_string('recurse', 'hwork'));
        ...
        $options = array(
            'multiple' => true,
            'noselectionstring' => get_string('anytags', 'hwork'),
        );
        $mform->addElement('autocomplete', 'fromtags', get_string('randomquestiontags', 'mod_hwork'), $tagstrings, $options);
        $mform->addHelpButton('fromtags', 'randomquestiontags', 'mod_hwork');

        $mform->addElement('select', 'numbertoadd', get_string('randomnumber', 'hwork'),
                $this->get_number_of_questions_to_add_choices());

        $previewhtml = $OUTPUT->render_from_template('mod_hwork/random_question_form_preview', []);
        $mform->addElement('html', $previewhtml);

        $mform->addElement('submit', 'existingcategory', get_string('addrandomquestion', 'hwork'));

        // Random from a new category section.
        $mform->addElement('header', 'newcategoryheader',
                get_string('randomquestionusinganewcategory', 'hwork'));
        ...
        $mform->addElement('submit', 'newcategory',
                get_string('createcategoryandaddrandomquestion', 'hwork'));
        ...
        $PAGE->requires->js_call_amd('mod_hwork/add_random_form', 'init', [
