...
/**
 * Settings form for overrides in the hwork module.
 *
 * @package    mod_hwork
...
require_once($CFG->dirroot . '/mod/hwork/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class hwork_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the hwork settings object. */
    protected $hwork;
     ...
     * @param object the hwork settings object.
     ...
     */
    public function __construct($submiturl, $cm, $hwork, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'hwork'));
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'hwork'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/hwork/overrides.php', array('cmid'=>$cm->id));
                    print_error('groupsnone', 'hwork', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'hwork'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                $users = get_users_by_capability($this->context, 'mod/hwork:attempt',
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/hwork/overrides.php', array('cmid'=>$cm->id));
                    print_error('usersnone', 'hwork', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'hwork'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'hwork');
        $mform->setDefault('password', $this->hwork->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('hworkopen', 'hwork'), mod_hwork_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->hwork->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('hworkclose', 'hwork'), mod_hwork_mod_form::$datefieldoptions);
        $mform->setDefault('timeclose', $this->hwork->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'hwork'), array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'hwork');
        $mform->setDefault('timelimit', $this->hwork->timelimit);
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'hwork'), $attemptoptions);
        $mform->addHelpButton('attempts', 'attempts', 'hwork');
        $mform->setDefault('attempts', $this->hwork->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'hwork'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'hwork'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'hwork'));
    ...
    public function validation($data, $files) {
        ...
        // Ensure that at least one hwork setting was changed.
        $changed = false;
        $keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
        foreach ($keys as $key) {
            if ($data[$key] != $hwork->{$key}) {
                $changed = true;
                break;
            }
        }
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'hwork');
        }
