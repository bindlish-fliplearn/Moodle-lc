...
/**
 * Defines the renderer for the hwork module.
 *
 * @package   mod_hwork
...
/**
 * The renderer for the hwork module.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_hwork_renderer extends plugin_renderer_base {
    /**
     * Builds the review page
     *
     * @param hwork_attempt $attemptobj an instance of hwork_attempt.
     ...
     * @param mod_hwork_display_options $displayoptions instance of mod_hwork_display_options.
     * @param array $summarydata contains all table data
     * @return $output containing html data.
     */
    public function review_page(hwork_attempt $attemptobj, $slots, $page, $showall,
                                $lastpage, mod_hwork_display_options $displayoptions,
                                $summarydata) {
     ...
     * @return $output containing html data.
     */
    public function review_question_page(hwork_attempt $attemptobj, $slot, $seq,
            mod_hwork_display_options $displayoptions, $summarydata) {
     ...
     */
    public function review_question_not_allowed(hwork_attempt $attemptobj, $message) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_hwork_name(), true,
                                  array("context" => $attemptobj->get_hworkobj()->get_context())));
    ...
    public function review_summary_table($summarydata, $page) {
        ...
        $output .= html_writer::start_tag('table', array(
                'class' => 'generaltable generalbox hworkreviewsummary'));
     ...
     *
     * @param hwork_attempt $attemptobj instance of hwork_attempt
     ...
     * @param mod_hwork_display_options $displayoptions instance of mod_hwork_display_options
     */
    public function questions(hwork_attempt $attemptobj, $reviewing, $slots, $page, $showall,
                              mod_hwork_display_options $displayoptions) {
        $output = '';
    ...
    public function review_form($page, $showall, $displayoptions, $content, $attemptobj) {
        ...
        $this->page->requires->js_init_call('M.mod_hwork.init_review_form', null, false,
                hwork_get_js_module());
     ...
     * @param hwork_attempt $attemptobj instance of hwork_attempt
     */
    public function finish_review_link(hwork_attempt $attemptobj) {
        $url = $attemptobj->view_url();

        if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            $this->page->requires->js_init_call('M.mod_hwork.secure_window.init_close_button',
                    array($url), false, hwork_get_js_module());
            return html_writer::empty_tag('input', array('type' => 'button',
                    'value' => get_string('finishreview', 'hwork'),
                    'id' => 'secureclosebutton',
                    'class' => 'mod_hwork-next-nav btn btn-primary'));

        } else {
            return html_writer::link($url, get_string('finishreview', 'hwork'),
                    array('class' => 'mod_hwork-next-nav'));
        }
    }
     ...
     */
    public function review_next_navigation(hwork_attempt $attemptobj, $page, $lastpage, $showall = null) {
        $nav = '';
        if ($page > 0) {
            $nav .= link_arrow_left(get_string('navigateprevious', 'hwork'),
                    $attemptobj->review_url(null, $page - 1, $showall), false, 'mod_hwork-prev-nav');
        }
        if ($lastpage) {
            $nav .= $this->finish_review_link($attemptobj);
        } else {
            $nav .= link_arrow_right(get_string('navigatenext', 'hwork'),
                    $attemptobj->review_url(null, $page + 1, $showall), false, 'mod_hwork-next-nav');
        }
        return html_writer::tag('div', $nav, array('class' => 'submitbtns'));
    }

    /**
     * Return the HTML of the hwork timer.
     * @return string HTML content.
     */
    public function countdown_timer(hwork_attempt $attemptobj, $timenow) {
        ...
        if ($timeleft !== false) {
            ...
            if (!$ispreview) {
                ...
                // this will just have the effect of causing the hwork to be submitted immediately.
                $timerstartvalue = max($timerstartvalue, 1);
            }
            $this->initialise_timer($timerstartvalue, $ispreview);
        }

        return html_writer::tag('div', get_string('timeleft', 'hwork') . ' ' .
                html_writer::tag('span', '', array('id' => 'hwork-time-left')),
                array('id' => 'hwork-timer', 'role' => 'timer',
    ...
    public function restart_preview_button($url) {
        return $this->single_button($url, get_string('startnewpreview', 'hwork'));
    }

    /**
     * Outputs the navigation block panel
     *
     * @param hwork_nav_panel_base $panel instance of hwork_nav_panel_base
     */
    public function navigation_panel(hwork_nav_panel_base $panel) {
        ...
        $this->page->requires->js_init_call('M.mod_hwork.nav.init', null, false,
                hwork_get_js_module());
    ...
    /**
     * Display a hwork navigation button.
     *
     * @param hwork_nav_question_button $button
     * @return string HTML fragment.
     */
    protected function render_hwork_nav_question_button(hwork_nav_question_button $button) {
        $classes = array('qnbutton', $button->stateclass, $button->navmethod, 'btn', 'btn-secondary');
        $extrainfo = array();

        if ($button->currentpage) {
            $classes[] = 'thispage';
            $extrainfo[] = get_string('onthispage', 'hwork');
        }
        ...
        $tagcontents = html_writer::tag('span', '', array('class' => 'thispageholder')) .
                        ...
                        get_string($qnostring, 'hwork', $a);
        $tagattributes = array('class' => implode(' ', $classes), 'id' => $button->id,
                                  'title' => $button->statestring, 'data-hwork-page' => $button->page);
    ...
    /**
     * Display a hwork navigation heading.
     *
     * @param hwork_nav_section_heading $heading the heading.
     * @return string HTML fragment.
     */
    protected function render_hwork_nav_section_heading(hwork_nav_section_heading $heading) {
        return $this->heading($heading->heading, 3, 'mod_hwork-section-heading');
    }

    /**
     * outputs the link the other attempts.
     *
     * @param mod_hwork_links_to_other_attempts $links
     */
    protected function render_mod_hwork_links_to_other_attempts(
            mod_hwork_links_to_other_attempts $links) {
        $attemptlinks = array();
    ...
    public function start_attempt_page(hwork $hworkobj, mod_hwork_preflight_check_form $mform) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($hworkobj->get_hwork_name(), true,
                                  array("context" => $hworkobj->get_context())));
        $output .= $this->hwork_intro($hworkobj->get_hwork(), $hworkobj->get_cm());
        $output .= $mform->render();
        $output .= $this->footer();
        return $output;
    }
     ...
     * @param hwork_attempt $attemptobj Instance of hwork_attempt
     * @param int $page Current page number
     * @param hwork_access_manager $accessmanager Instance of hwork_access_manager
    ...
    public function attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id,
        ...
        $output = '';
        $output .= $this->header();
        $output .= $this->hwork_notices($messages);
     ...
     */
    public function hwork_notices($messages) {
        if (!$messages) {
            return '';
        }
        return $this->box($this->heading(get_string('accessnoticesheader', 'hwork'), 3) .
                $this->access_messages($messages), 'hworkaccessnotices');
    }

    /**
     * Ouputs the form for making an attempt
     *
     * @param hwork_attempt $attemptobj
     * @param int $page Current page number
    ...
    public function attempt_form($attemptobj, $page, $slots, $id, $nextpage) {
        ...
        $navmethod = $attemptobj->get_hwork()->navmethod;
     ...
     * @param bool $lastpage is this the last page in the hwork?
     * @param string $navmethod Optional hwork attribute, 'free' (default) or 'sequential'
     * @return string HTML fragment.
     */
    protected function attempt_navigation_buttons($page, $lastpage, $navmethod = 'free') {
        ...
        if ($page > 0 && $navmethod == 'free') {
            $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'previous',
                    'value' => get_string('navigateprevious', 'hwork'), 'class' => 'mod_hwork-prev-nav btn btn-secondary'));
        }
        if ($lastpage) {
            $nextlabel = get_string('endtest', 'hwork');
        } else {
            $nextlabel = get_string('navigatenext', 'hwork');
        }
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'next',
                'value' => $nextlabel, 'class' => 'mod_hwork-next-nav btn btn-primary'));
        $output .= html_writer::end_tag('div');
    ...
    public function redo_question_button($slot, $disabled) {
        $attributes = array('type' => 'submit',  'name' => 'redoslot' . $slot,
                'value' => get_string('redoquestion', 'hwork'), 'class' => 'mod_hwork-redo_question_button');
        if ($disabled) {
    ...
    public function initialise_timer($timerstartvalue, $ispreview) {
        ...
        $this->page->requires->js_init_call('M.mod_hwork.timer.init', $options, false, hwork_get_js_module());
    }
    ...
    public function close_attempt_popup($url, $message = '') {
        ...
        if ($message) {
            ...
            $output .= html_writer::tag('p', get_string('windowclosing', 'hwork'));
            $delay = 5;
        } else {
            $output .= html_writer::tag('p', get_string('pleaseclose', 'hwork'));
            $delay = 0;
        }
        $this->page->requires->js_init_call('M.mod_hwork.secure_window.close',
                array($url, $delay), false, hwork_get_js_module());
     ...
     *
     * @param hwork_attempt $attemptobj
     * @param mod_hwork_display_options $displayoptions
     */
    public function summary_page($attemptobj, $displayoptions) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_hwork_name()));
        $output .= $this->heading(get_string('summaryofattempt', 'hwork'), 3);
    ...
    public function summary_table($attemptobj, $displayoptions) {
        ...
        $table->attributes['class'] = 'generaltable hworksummaryofattempt boxaligncenter';
        $table->head = array(get_string('question', 'hwork'), get_string('status', 'hwork'));
        $table->align = array('left', 'left');
        $table->size = array('', '');
        $markscolumn = $displayoptions->marks >= question_display_options::MARK_AND_MAX;
        if ($markscolumn) {
            $table->head[] = get_string('marks', 'hwork');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($slots as $slot) {
            ...
            if ($heading) {
                ...
                $table->rowclasses[] = 'hworksummaryheading';
            }
            ...
            $table->data[] = $row;
            $table->rowclasses[] = 'hworksummary' . $slot . ' ' . $attemptobj->get_question_state_class(
    ...
    public function summary_page_controls($attemptobj) {
        ...
        if ($attemptobj->get_state() == hwork_attempt::IN_PROGRESS) {
            $button = new single_button(
                    new moodle_url($attemptobj->attempt_url(null, $attemptobj->get_currentpage())),
                    get_string('returnattempt', 'hwork'));
        ...
        $button = new single_button(
                ...
                get_string('submitallandfinish', 'hwork'));
        $button->id = 'responseform';
        if ($attemptobj->get_state() == hwork_attempt::IN_PROGRESS) {
            $button->add_action(new confirm_action(get_string('confirmclose', 'hwork'), null,
                    get_string('submitallandfinish', 'hwork')));
        }

        $duedate = $attemptobj->get_due_date();
        $message = '';
        if ($attemptobj->get_state() == hwork_attempt::OVERDUE) {
            $message = get_string('overduemustbesubmittedby', 'hwork', userdate($duedate));

        } else if ($duedate) {
            $message = get_string('mustbesubmittedby', 'hwork', userdate($duedate));
        }
     ...
     * @param array $hwork Array conting hwork data
     * @param int $cm Course Module ID
     * @param int $context The page context ID
     * @param array $infomessages information about this hwork
     * @param mod_hwork_view_object $viewobj
     ...
     *      attempt this hwork now, if appicable this hwork
     */
    public function view_page($course, $hwork, $cm, $context, $viewobj) {
        $output = '';
        $output .= $this->view_information($hwork, $cm, $context, $viewobj->infomessages);
        $output .= $this->view_table($hwork, $context, $viewobj);
        $output .= $this->view_result_info($hwork, $context, $cm, $viewobj);
        $output .= $this->box($this->view_page_buttons($viewobj), 'hworkattempt');
        return $output;
    }
     ...
     * @param mod_hwork_view_object $viewobj the information required to display
     * the view page.
     * @return string HTML to output.
     */
    public function view_page_buttons(mod_hwork_view_object $viewobj) {
        global $CFG;
        $output = '';
        ...
        if ($viewobj->showbacktocourse) {
            $output .= $this->single_button($viewobj->backtocourseurl,
                    get_string('backtocourse', 'hwork'), 'get',
     ...
     * @param mod_hwork_preflight_check_form $preflightcheckform deprecated.
    ...
    public function start_attempt_button($buttontext, moodle_url $url,
            mod_hwork_preflight_check_form $preflightcheckform = null,
        ...
        if (is_string($preflightcheckform)) {
            ...
            debugging('The third argument to start_attempt_button should now be the ' .
                    'mod_hwork_preflight_check_form from ' .
                    'hwork_access_manager::get_preflight_check_form, not a warning message string.');
        }

        $button = new single_button($url, $buttontext);
        $button->class .= ' hworkstartbuttondiv';
        if ($popuprequired) {
            $button->class .= ' hworksecuremoderequired';
        }
        ...
        $this->page->requires->js_call_amd('mod_hwork/preflightcheck', 'init',
                array('.hworkstartbuttondiv [type=submit]', get_string('startattempt', 'hwork'),
                       '#mod_hwork_preflight_form', $popupjsoptions));
    ...
    public function no_questions_message($canedit, $editurl) {
        $output = '';
        $output .= $this->notification(get_string('noquestions', 'hwork'));
        if ($canedit) {
            $output .= $this->single_button($editurl, get_string('edithwork', 'hwork'), 'get');
        }
    ...
    /**
     * Outputs an error message for any guests accessing the hwork
     *
     * @param int $course The course ID
     * @param array $hwork Array contingin hwork data
     * @param int $cm Course Module ID
     * @param int $context The page contect ID
     * @param array $messages Array containing any messages
     */
    public function view_page_guest($course, $hwork, $cm, $context, $messages) {
        $output = '';
        $output .= $this->view_information($hwork, $cm, $context, $messages);
        $guestno = html_writer::tag('p', get_string('guestsno', 'hwork'));
     ...
     * @param array $messages Array containing any messages
     */
    public function view_page_notenrolled($course, $hwork, $cm, $context, $messages) {
        global $CFG;
        $output = '';
        $output .= $this->view_information($hwork, $cm, $context, $messages);
        $youneedtoenrol = html_writer::tag('p', get_string('youneedtoenrol', 'hwork'));
     ...
     * @return string HTML to output.
     */
    public function view_information($hwork, $cm, $context, $messages) {
        global $CFG;

        $output = '';

        // Print hwork name and description.
        $output .= $this->heading(format_string($hwork->name));
        $output .= $this->hwork_intro($hwork, $cm);

        // Output any access messages.
        if ($messages) {
            $output .= $this->box($this->access_messages($messages), 'hworkinfo');
        }

        // Show number of attempts summary to those who can view reports.
        if (has_capability('mod/hwork:viewreports', $context)) {
            if ($strattemptnum = $this->hwork_attempt_summary_link_to_reports($hwork, $cm,
                    $context)) {
                $output .= html_writer::tag('div', $strattemptnum,
                        array('class' => 'hworkattemptcounts'));
            }
        }
        return $output;
    }
    ...
    /**
     * Output the hwork intro.
     * @param object $hwork the hwork settings.
     * @param object $cm the course_module object.
     * @return string HTML to output.
     */
    public function hwork_intro($hwork, $cm) {
        if (html_is_blank($hwork->intro)) {
            return '';
        }

        return $this->box(format_module_intro('hwork', $hwork, $cm->id), 'generalbox', 'intro');
    }
    ...
    public function view_table_heading() {
        return $this->heading(get_string('summaryofattempts', 'hwork'), 3);
    }

    /**
     * Generates the table of data
     *
     * @param array $hwork Array contining hwork data
     * @param int $context The page context ID
     * @param mod_hwork_view_object $viewobj
     */
    public function view_table($hwork, $context, $viewobj) {
        if (!$viewobj->attempts) {
            return '';
        }
        ...
        $table = new html_table();
        $table->attributes['class'] = 'generaltable hworkattemptsummary';
        $table->head = array();
        $table->align = array();
        $table->size = array();
        if ($viewobj->attemptcolumn) {
            $table->head[] = get_string('attemptnumber', 'hwork');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        $table->head[] = get_string('attemptstate', 'hwork');
        $table->align[] = 'left';
        $table->size[] = '';
        if ($viewobj->markcolumn) {
            $table->head[] = get_string('marks', 'hwork') . ' / ' .
                    hwork_format_grade($hwork, $hwork->sumgrades);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->gradecolumn) {
            $table->head[] = get_string('grade') . ' / ' .
                    hwork_format_grade($hwork, $hwork->grade);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->canreviewmine) {
            $table->head[] = get_string('review', 'hwork');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->feedbackcolumn) {
            $table->head[] = get_string('feedback', 'hwork');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($viewobj->attemptobjs as $attemptobj) {
            ...
            if ($viewobj->markcolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    $row[] = hwork_format_grade($hwork, $attemptobj->get_sum_marks());
                } else {
                    $row[] = '';
                }
            }
            ...
            $attemptgrade = hwork_rescale_grade($attemptobj->get_sum_marks(), $hwork, false);
            ...
            if ($viewobj->gradecolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    if ($viewobj->overallstats && !$attemptobj->is_preview()
                            ...
                            && $attemptobj->get_state() == hwork_attempt::FINISHED
                            && $attemptgrade == $viewobj->mygrade
                            && $hwork->grademethod == QUIZ_GRADEHIGHEST) {
                        $table->rowclasses[$attemptobj->get_attempt_number()] = 'bestrow';
                    }

                    $row[] = hwork_format_grade($hwork, $attemptgrade);
                } else {
                    $row[] = '';
                }
            }
            ...
            if ($viewobj->feedbackcolumn && $attemptobj->is_finished()) {
                if ($attemptoptions->overallfeedback) {
                    $row[] = hwork_feedback_for_grade($attemptgrade, $hwork, $context);
                } else {
                    $row[] = '';
                }
            }
     ...
     * @param hwork_attempt $attemptobj the attempt
    ...
    public function attempt_state($attemptobj) {
        switch ($attemptobj->get_state()) {
            ...
            case hwork_attempt::OVERDUE:
                return get_string('stateoverdue', 'hwork') . html_writer::tag('span',
                        get_string('stateoverduedetails', 'hwork',
                                userdate($attemptobj->get_due_date())),
                        array('class' => 'statedetails'));

            case hwork_attempt::FINISHED:
                return get_string('statefinished', 'hwork') . html_writer::tag('span',
                        get_string('statefinisheddetails', 'hwork',
    ...
    /**
     * Generates data pertaining to hwork results
     *
     * @param array $hwork Array containing hwork data
     * @param int $context The page context ID
     * @param int $cm The Course Module Id
     * @param mod_hwork_view_object $viewobj
     */
    public function view_result_info($hwork, $context, $cm, $viewobj) {
        $output = '';
        ...
        if ($viewobj->overallstats) {
            if ($viewobj->moreattempts) {
                $a = new stdClass();
                $a->method = hwork_get_grading_option_name($hwork->grademethod);
                $a->mygrade = hwork_format_grade($hwork, $viewobj->mygrade);
                $a->hworkgrade = hwork_format_grade($hwork, $hwork->grade);
                $resultinfo .= $this->heading(get_string('gradesofar', 'hwork', $a), 3);
            } else {
                $a = new stdClass();
                $a->grade = hwork_format_grade($hwork, $viewobj->mygrade);
                $a->maxgrade = hwork_format_grade($hwork, $hwork->grade);
                $a = get_string('outofshort', 'hwork', $a);
                $resultinfo .= $this->heading(get_string('yourfinalgradeis', 'hwork', $a), 3);
            }
        }
        ...
        if ($viewobj->gradebookfeedback) {
            $resultinfo .= $this->heading(get_string('comment', 'hwork'), 3);
            $resultinfo .= html_writer::div($viewobj->gradebookfeedback, 'hworkteacherfeedback') . "\n";
        }
        if ($viewobj->feedbackcolumn) {
            $resultinfo .= $this->heading(get_string('overallfeedback', 'hwork'), 3);
            $resultinfo .= html_writer::div(
                    hwork_feedback_for_grade($viewobj->mygrade, $hwork, $context),
                    'hworkgradefeedback') . "\n";
        }
    ...
    public function review_link($url, $reviewinpopup, $popupoptions) {
        if ($reviewinpopup) {
            $button = new single_button($url, get_string('review', 'hwork'));
            $button->add_action(new popup_action('click', $url, 'hworkpopup', $popupoptions));
            return $this->render($button);

        } else {
            return html_writer::link($url, get_string('review', 'hwork'),
                    array('title' => get_string('reviewthisattempt', 'hwork')));
        }
    }
     ...
     */
    public function hwork_attempt_summary_link_to_reports($hwork, $cm, $context,
    ...
    public function connection_warning() {
        ...
        $warning = format_text(get_string('connectionerror', 'hwork'), FORMAT_MARKDOWN, $options);
        $ok = format_text(get_string('connectionok', 'hwork'), FORMAT_MARKDOWN, $options);
...
class mod_hwork_links_to_other_attempts implements renderable {
    /**
...
class mod_hwork_view_object {
    /** @var array $infomessages of messages with information to display about the hwork. */
    public $infomessages;
    /** @var array $attempts contains all the user's attempts at this hwork. */
    public $attempts;
    /** @var array $attemptobjs hwork_attempt objects corresponding to $attempts. */
    public $attemptobjs;
    /** @var hwork_access_manager $accessmanager contains various access rules. */
    public $accessmanager;
    /** @var bool $canreviewmine whether the current user has the capability to
     *       review their own attempts. */
    public $canreviewmine;
    /** @var bool $canedit whether the current user has the capability to edit the hwork. */
    public $canedit;
    /** @var moodle_url $editurl the URL for editing this hwork. */
    public $editurl;
    ...
    public $numattempts;
    /** @var float $mygrade contains the user's final grade for a hwork. */
    public $mygrade;
    ...
    public $popupoptions;
    /** @var bool $hworkhasquestions whether the hwork has any questions. */
    public $hworkhasquestions;
