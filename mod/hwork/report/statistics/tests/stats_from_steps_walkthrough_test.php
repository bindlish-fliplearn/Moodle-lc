 ...
 *
 * The hwork stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-hwork-tools/tree/master/statsspreadsheet
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/hwork/tests/attempt_walkthrough_from_csv_test.php');
require_once($CFG->dirroot . '/mod/hwork/report/default.php');
require_once($CFG->dirroot . '/mod/hwork/report/statistics/report.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class hwork_report_statistics_from_steps_testcase extends mod_hwork_attempt_walkthrough_from_csv_testcase {

    /**
     * @var hwork_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    /**
     * Create a hwork add questions to it, walk through hwork attempts and then check results.
     *
    ...
    public function test_walkthrough_from_csv($hworksettings, $csvdata) {
        ...
        list($questions, $hworkstats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata,
                            $whichattempts, $whichtries, $groupstudentsjoins);
        if ($hworksettings['testnumber'] === '00') {
            $this->check_variants_count_for_hwork_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_hwork_stats_for_hwork_00($hworkstats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_hwork_00($questions, $questionstats, $whichtries, $qubaids) {
     ...
     * @param $hworkstats
     */
    protected function check_hwork_stats_for_hwork_00($hworkstats) {
        $hworkstatsexpected = array(
            'median'             => 4.5,
        ...
        foreach ($hworkstatsexpected as $statname => $statvalue) {
            $this->assertEquals($statvalue, $hworkstats->$statname, $hworkstats->$statname, abs($statvalue) * 1.5e-5);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $hworkstats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries,
        ...
        $this->report = new hwork_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->hwork);
        list($hworkstats, $questionstats) = $this->report->get_all_stats_and_analysis($this->hwork,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudentsjoins,
                                                                                     $questions);

        $qubaids = hwork_statistics_qubaids_condition($this->hwork->id, $groupstudentsjoins, $whichattempts);

        // We will create some hwork and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $hworkcalc = new \hwork_statistics\calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($hworkcalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $hworkstats, $questionstats, $qubaids);
        }
        return array($questions, $hworkstats, $questionstats, $qubaids);
    }
