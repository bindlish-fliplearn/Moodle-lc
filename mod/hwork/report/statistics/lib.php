...
/**
 * Standard plugin entry points of the hwork statistics report.
 *
 ...
 * @package  hwork_statistics
 * @category files
 * @param context $previewcontext the hwork context
 ...
 */
function hwork_statistics_question_preview_pluginfile($previewcontext, $questionid,
    ...
    require_capability('hwork/statistics:view', $context);
 ...
 */
function hwork_statistics_cron() {
    global $DB;

    mtrace("\n  Cleaning up old hwork statistics cache records...", '');

    $expiretime = time() - 5*HOURSECS;
    $DB->delete_records_select('hwork_statistics', 'timemodified < ?', array($expiretime));
