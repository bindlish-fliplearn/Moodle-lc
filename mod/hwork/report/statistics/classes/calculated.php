...
class calculated {
     ...
     */
    public function get_formatted_hwork_info_data($course, $cm, $hwork) {
        ...
        // General information about the hwork.
        $hworkinfo = array();
        $hworkinfo[get_string('hworkname', 'hwork_statistics')] = format_string($hwork->name);
        $hworkinfo[get_string('coursename', 'hwork_statistics')] = format_string($course->fullname);
        if ($cm->idnumber) {
            $hworkinfo[get_string('idnumbermod')] = $cm->idnumber;
        }
        if ($hwork->timeopen) {
            $hworkinfo[get_string('hworkopen', 'hwork')] = userdate($hwork->timeopen);
        }
        if ($hwork->timeclose) {
            $hworkinfo[get_string('hworkclose', 'hwork')] = userdate($hwork->timeclose);
        }
        if ($hwork->timeopen && $hwork->timeclose) {
            $hworkinfo[get_string('duration', 'hwork_statistics')] =
                format_time($hwork->timeclose - $hwork->timeopen);
        }
        ...
        foreach ($todisplay as $property => $format) {
            ...
            switch ($format) {
                case 'summarks_as_percentage':
                    $formattedvalue = hwork_report_scale_summarks_as_percentage($value, $hwork);
                    break;
                case 'number_format_percent':
                    $formattedvalue = hwork_format_grade($hwork, $value) . '%';
                    break;
                case 'number_format':
                    // 2 extra decimal places, since not a percentage,
                    // and we want the same number of sig figs.
                    $formattedvalue = format_float($value, $hwork->decimalpoints + 2);
                    break;
                case 'number':
            ...
            $hworkinfo[get_string($property, 'hwork_statistics',
    ...
    public function cache($qubaids) {
        ...
        // Store the data.
        $DB->insert_record('hwork_statistics', $toinsert);

    }

    /**
     * Given a record from 'hwork_statistics' table load the data into the properties of this class.
     *
