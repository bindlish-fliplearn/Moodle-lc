...
/**
 * Base class for the table used by a {@link hwork_attempts_report}.
 *
 * @package   mod_hwork
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class hwork_attempts_report_table extends table_sql {
    ...
    /** @var object the hwork settings for the hwork we are reporting on. */
    protected $hwork;
    ...
    /** @var object mod_hwork_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this hwork.. */
    protected $questions;
     ...
     * @param mod_hwork_attempts_report_options $options
     ...
     */
    public function __construct($uniqueid, $hwork, $context, $qmsubselect,
            mod_hwork_attempts_report_options $options, \core\dml\sql_join $groupstudentsjoins, \core\dml\sql_join $studentsjoins,
            $questions, $reporturl) {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/hwork/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'hwork'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return hwork_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = hwork_report_feedback_for_grade(
                hwork_rescale_grade($attempt->sumgrades, $this->hwork, false),
                $this->hwork->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/hwork/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'hwork')));
    ...
    public function base_sql(\core\dml\sql_join $allowedstudentsjoins) {
        ...
        // Please note this uniqueid column is not the same as hworka.uniqueid.
        $fields = 'DISTINCT ' . $DB->sql_concat('u.id', "'#'", 'COALESCE(hworka.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                hworka.uniqueid AS usageid,
                hworka.id AS attempt,
                u.id AS userid,
                u.idnumber, ' . $allnames . ',
                u.picture,
                u.imagealt,
                ...
                hworka.timefinish,
                hworka.timestart,
                CASE WHEN hworka.timefinish = 0 THEN null
                     WHEN hworka.timefinish > hworka.timestart THEN hworka.timefinish - hworka.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and hwork_attempts tables.
        $from = " {user} u";
        $from .= "\nLEFT JOIN {hwork_attempts} hworka ON
                                    hworka.userid = u.id AND hworka.hwork = :hworkid";
        $params = array('hworkid' => $this->hwork->id);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (hworka.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = hwork_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case hwork_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'hworka.id IS NOT NULL AND hworka.preview = 0';
                break;
            case hwork_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "hworka.preview = 0 AND hworka.id IS NOT NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case hwork_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "hworka.id IS NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case hwork_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "(hworka.preview = 0 OR hworka.preview IS NULL) AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (hworka.state $statesql OR hworka.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{hwork_attempts} {$alias}hworka", "{$alias}hworka.uniqueid",
                "{$alias}hworka.hwork = :{$alias}hworkid", array("{$alias}hworkid" => $this->sql->params['hworkid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = hworka.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join("(
                SELECT DISTINCT hworka.uniqueid
                  FROM " . $this->sql->from . "
                 WHERE " . $this->sql->where . "
                    ) hworkasubquery", 'hworkasubquery.uniqueid',
                    "1 = 1", $this->sql->params);
        }
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['hworka.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    public function wrap_html_finish() {
        ...
        echo '<a id="checkattempts" href="#">' .
                get_string('selectall', 'hwork') . '</a> / ';
        echo '<a id="uncheckattempts" href="#">' .
                get_string('selectnone', 'hwork') . '</a> ';
