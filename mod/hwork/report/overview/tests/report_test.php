...
/**
 * Tests for the hwork overview report.
 *
...
require_once($CFG->dirroot . '/mod/hwork/report/overview/report.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class hwork_overview_report_testcase extends advanced_testcase {
    ...
    public function test_report_sql($isdownloading) {
        ...
        // Create a course and a hwork.
        $generator = $this->getDataGenerator();
        $course = $generator->create_course();
        $hworkgenerator = $generator->get_plugin_generator('mod_hwork');
        $hwork = $hworkgenerator->create_instance(array('course' => $course->id,
        ...
        hwork_add_hwork_question($q->id, $hwork, 0 , 10);
        ...
        $attempts = array(
            array($hwork, $student1, 1, 0.0,  hwork_attempt::FINISHED),
            array($hwork, $student1, 2, 5.0,  hwork_attempt::FINISHED),
            array($hwork, $student1, 3, 8.0,  hwork_attempt::FINISHED),
            array($hwork, $student1, 4, null, hwork_attempt::ABANDONED),
            array($hwork, $student1, 5, null, hwork_attempt::IN_PROGRESS),
            array($hwork, $student2, 1, null, hwork_attempt::ABANDONED),
            array($hwork, $student2, 2, null, hwork_attempt::ABANDONED),
            array($hwork, $student2, 3, 7.0,  hwork_attempt::FINISHED),
            array($hwork, $student2, 4, null, hwork_attempt::ABANDONED),
            array($hwork, $student2, 5, null, hwork_attempt::ABANDONED),
        );

        // Load it in to hwork attempts table.
        foreach ($attempts as $attemptdata) {
            list($hwork, $student, $attemptnumber, $sumgrades, $state) = $attemptdata;
            ...
            $attempt = hwork_create_attempt($hworkobj, $attemptnumber, null, $timestart, false, $student->id);

            $attempt = hwork_start_new_attempt($hworkobj, $quba, $attempt, $attemptnumber, $timestamp);
            ...
            switch ($state) {
                ...
                case hwork_attempt::FINISHED:
                    ...
                    $DB->update_record('hwork_attempts', $update);
                    hwork_save_best_grade($attemptobj->get_hwork(), $student->id);
                    break;
            }
        }
        ...
        $qmsubselect = hwork_report_qm_filter_select($hwork);
        $studentsjoins = get_enrolled_with_capabilities_join($context);
        $empty = new \core\dml\sql_join();

        // Set the options.
        $reportoptions = new hwork_overview_options('overview', $hwork, $cm, null);
        $reportoptions->attempts = hwork_attempts_report::ENROLLED_ALL;
        $reportoptions->onlygraded = true;
        $reportoptions->states = array(hwork_attempt::IN_PROGRESS, hwork_attempt::OVERDUE, hwork_attempt::FINISHED);

        // Now do a minimal set-up of the table class.
        $q->slot = 1;
        $q->maxmark = 10;
        $table = new hwork_overview_table($hwork, $context, $qmsubselect, $reportoptions,
        ...
        $table->define_baseurl(new moodle_url('/mod/hwork/report.php'));
        $table->setup();
    ...
    public function test_get_bands_count_and_width($grade, $expected) {
        ...
        $hwork = $hworkgenerator->create_instance(['course' => SITEID, 'grade' => $grade]);
        $this->assertEquals($expected, hwork_overview_report::get_bands_count_and_width($hwork));
    }
