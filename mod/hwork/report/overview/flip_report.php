...
/**
 * This file defines the hwork overview report class.
 *
 * @package   hwork_overview
...
require_once($CFG->dirroot . '/mod/hwork/report/attemptsreport.php');
require_once($CFG->dirroot . '/mod/hwork/report/overview/overview_options.php');
require_once($CFG->dirroot . '/mod/hwork/report/overview/overview_form.php');
require_once($CFG->dirroot . '/mod/hwork/report/overview/overview_table.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class hwork_filp_overview_report extends hwork_attempts_report {

    public function display($hwork, $cm, $course) {
        global $DB, $OUTPUT, $PAGE;

        list($currentgroup, $studentsjoins, $groupstudentsjoins, $allowedjoins) = $this->init(
                'overview', 'hwork_overview_settings_form', $hwork, $cm, $course);

        $options = new hwork_overview_options('overview', $hwork, $cm, $course);
        ...
        $table = new hwork_overview_table($hwork, $this->context, $this->qmsubselect,
                $options, $groupstudentsjoins, $studentsjoins, $questions, $options->get_url());
//        $filename = hwork_report_download_filename(get_string('overviewfilename', 'hwork_overview'),
//                $courseshortname, $hwork->name);
//        $table->is_downloading($options->download, $filename,
//                $courseshortname . ' ' . format_string($hwork->name, true));
...
//        }
        ...
        if ($options->attempts == self::ALL_WITH) {
            ...
            // groups mode, so setting allowed to empty (which means all hwork attempts
            // are accessible, is not a security porblem.
            $allowedjoins = new \core\dml\sql_join();
        }

        $this->course = $course; // Hack to make this available in process_actions.
        $this->process_actions($hwork, $cm, $currentgroup, $groupstudentsjoins, $allowedjoins, $options->get_url());

        $hasquestions = hwork_has_questions($hwork->id);

        // Start output.
        if (!$table->is_downloading()) {
            // Only print headers if not asked to download data.
            $this->print_standard_header_and_messages($cm, $course, $hwork,
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
            ...
            if (!$table->is_downloading()) {
                ...
                if (has_capability('mod/hwork:regrade', $this->context)) {
                    $regradesneeded = $this->count_question_attempts_needing_regrade(
                            $hwork, $groupstudentsjoins);
//                    if ($currentgroup) {
//                        $a= new stdClass();
...
//                                get_string('regradealldrydogroup', 'hwork_overview', $a);
//                        $regradealldrylabel =
//                                get_string('regradealldrygroup', 'hwork_overview', $a);
//                        $regradealllabel =
//                                get_string('regradeallgroup', 'hwork_overview', $a);
//                    } else {
//                        $regradealldrydolabel =
//                                get_string('regradealldrydo', 'hwork_overview', $regradesneeded);
//                        $regradealldrylabel =
//                                get_string('regradealldry', 'hwork_overview');
//                        $regradealllabel =
//                                get_string('regradeall', 'hwork_overview');
//                    }
...
//                    echo '</div>';
                ...
                if ($strattempthighlight = hwork_report_highlighting_grading_method(
                        $hwork, $this->qmsubselect, $options->onlygraded)) {
                    echo '<div class="hworkattemptcounts">' . $strattempthighlight . '</div>';
                }
            }
...
//            $this->add_time_columns($columns, $headers);
            ...
            $this->add_grade_columns($hwork, $options->usercanseegrades, $columns, $headers, false);

            if (!$table->is_downloading() && has_capability('mod/hwork:regrade', $this->context) &&
                    $this->has_regraded_questions($table->sql->from, $table->sql->where, $table->sql->params)) {
                $columns[] = 'regraded';
                $headers[] = get_string('regrade', 'hwork_overview');
            }
            ...
            if ($options->slotmarks) {
                foreach ($questions as $slot => $question) {
                    ...
                    $header = get_string('qbrief', 'hwork', $question->number);
                    if (!$table->is_downloading()) {
                        $header .= '<br />';
                    } else {
                        $header .= ' ';
                    }
                    $header .= '/' . hwork_rescale_grade($question->maxmark, $hwork, 'question');
                    $headers[] = $header;
                }
            }
        ...
        if (!$table->is_downloading() && $options->usercanseegrades) {
            ...
            list($bands, $bandwidth) = self::get_bands_count_and_width($hwork);
            $labels = self::get_bands_labels($bands, $bandwidth, $hwork);
            ...
            if ($currentgroup && $this->hasgroupstudents) {
                $sql = "SELECT qg.id
                        ...
                        {$groupstudentsjoins->joins}
                          WHERE qg.hwork = $hwork->id AND {$groupstudentsjoins->wheres}";
                if ($DB->record_exists_sql($sql, $groupstudentsjoins->params)) {
                    $data = hwork_report_grade_bands($bandwidth, $bands, $hwork->id, $groupstudentsjoins);
                    $chart = self::get_chart($labels, $data);
                    $graphname = get_string('overviewreportgraphgroup', 'hwork_overview', groups_get_group_name($currentgroup));
                    //echo $output->chart($chart, $graphname);
                }
            }

            if ($DB->record_exists('hwork_grades', array('hwork'=> $hwork->id))) {
                $data = hwork_report_grade_bands($bandwidth, $bands, $hwork->id, new \core\dml\sql_join());
                $chart = self::get_chart($labels, $data);
                $graphname = get_string('overviewreportgraph', 'hwork_overview');
                //echo $output->chart($chart, $graphname);
            }
        }
        return true;
     ...
     */
    protected function process_actions($hwork, $cm, $currentgroup, \core\dml\sql_join $groupstudentsjoins,
            \core\dml\sql_join $allowedjoins, $redirecturl) {
        parent::process_actions($hwork, $cm, $currentgroup, $groupstudentsjoins, $allowedjoins, $redirecturl);

        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('regrade', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    $this->start_regrade($hwork, $cm);
                    $this->regrade_attempts($hwork, false, $groupstudentsjoins, $attemptids);
        ...
        if (optional_param('regradeall', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($hwork, $cm);
            $this->regrade_attempts($hwork, false, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);

        } else if (optional_param('regradealldry', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($hwork, $cm);
            $this->regrade_attempts($hwork, true, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);

        } else if (optional_param('regradealldrydo', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($hwork, $cm);
            $this->regrade_attempts_needing_it($hwork, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);
        }
    }
     ...
     * @param object $cm the cm object for the hwork.
     */
    protected function start_regrade($hwork, $cm) {
        require_capability('mod/hwork:regrade', $this->context);
        $this->print_header_and_tabs($cm, $this->course, $hwork, $this->mode);
    }
    ...
    protected function finish_regrade($nexturl) {
        global $OUTPUT;
        \core\notification::success(get_string('regradecomplete', 'hwork_overview'));
    ...
    /**
     * Regrade a particular hwork attempt. Either for real ($dryrun = false), or
     * as a pretend regrade to see which fractions would change. The outcome is
     * stored in the hwork_overview_regrades table.
     *
     ...
     *
     * @param object $attempt the hwork attempt to regrade.
    ...
    protected function regrade_attempt($attempt, $dryrun = false, $slots = null) {
        global $DB;
        // Need more time for a hwork with many questions.
        ...
        $finished = $attempt->state == hwork_attempt::FINISHED;
        foreach ($slots as $slot) {
            ...
            if (abs($qqr->oldfraction - $qqr->newfraction) > 1e-7) {
                ...
                $DB->insert_record('hwork_overview_regrades', $qqr, false);
            }
        }
    ...
    /**
     * Regrade attempts for this hwork, exactly which attempts are regraded is
     ...
     */
    protected function regrade_attempts($hwork, $dryrun = false,
        ...
        $where = "hwork = :qid AND preview = 0";
        $params = array('qid' => $hwork->id);
        ...
        $this->clear_regrade_table($hwork, $groupstudentsjoins);

        $progressbar = new progress_bar('hwork_overview_regrade', 500, true);
        $a = array(
        ...
        foreach ($attempts as $attempt) {
            ...
            $progressbar->update($a['done'], $a['count'],
                    get_string('regradingattemptxofy', 'hwork_overview', $a));
        }

        if (!$dryrun) {
            $this->update_overall_grades($hwork);
        }
    }

    /**
     * Regrade those questions in those attempts that are marked as needing regrading
     * in the hwork_overview_regrades table.
     * @param object $hwork the hwork settings.
     * @param \core\dml\sql_join $groupstudentsjoins empty for all attempts, otherwise regrade attempts
     * for these users.
     */
    protected function regrade_attempts_needing_it($hwork, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        $this->unlock_session();

        $join = '{hwork_overview_regrades} qqr ON qqr.questionusageid = hworka.uniqueid';
        $where = "hworka.hwork = :qid AND hworka.preview = 0 AND qqr.regraded = 0";
        $params = array('qid' => $hwork->id);
        ...
        $toregrade = $DB->get_recordset_sql("
                SELECT hworka.uniqueid, qqr.slot
                  FROM {hwork_attempts} hworka
                  JOIN $join
                 WHERE $where", $params);
        ...
        $attempts = $DB->get_records_list('hwork_attempts', 'uniqueid',
     ...
     */
    protected function count_question_attempts_needing_regrade($hwork, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        ...
        if ($this->hasgroupstudents) {
            $userjoin = "JOIN {user} u ON u.id = hworka.userid
                    {$groupstudentsjoins->joins}";
            $usertest = "{$groupstudentsjoins->wheres} AND u.id = hworka.userid AND ";
            $params = $groupstudentsjoins->params;
        }

        $params['chwork'] = $hwork->id;
        $sql = "SELECT COUNT(DISTINCT hworka.id)
                  FROM {hwork_attempts} hworka
                  JOIN {hwork_overview_regrades} qqr ON hworka.uniqueid = qqr.questionusageid
                $userjoin
                 WHERE
                      $usertest
                      hworka.hwork = :chwork AND
    ...
    protected function has_regraded_questions($from, $where, $params) {
        ...
        return $DB->record_exists_sql("
                SELECT 1
                  FROM {$from}
                  JOIN {hwork_overview_regrades} qor ON qor.questionusageid = hworka.uniqueid
                 WHERE {$where}", $params);
    }
     ...
     * to these users is cleared.
     */
    protected function clear_regrade_table($hwork, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        ...
        $DB->delete_records_select('hwork_overview_regrades', $select, $params);
    }
     ...
     */
    protected function update_overall_grades($hwork) {
        hwork_update_all_attempt_sumgrades($hwork);
        hwork_update_all_final_grades($hwork);
        hwork_update_grades($hwork);
    }

    /**
     * Get the bands configuration for the hwork.
     *
     * This returns the configuration for having between 11 and 20 bars in
     * a chart based on the maximum grade to be given on a hwork. The width of
     ...
     */
    public static function get_bands_count_and_width($hwork) {
        $bands = $hwork->grade;
        ...
        return [$bands, $hwork->grade / $bands];
    }
     ...
     */
    public static function get_bands_labels($bands, $bandwidth, $hwork) {
        $bandlabels = [];
        for ($i = 1; $i <= $bands; $i++) {
            $bandlabels[] = hwork_format_grade($hwork, ($i - 1) * $bandwidth) . ' - ' . hwork_format_grade($hwork, $i * $bandwidth);
        }
        return $bandlabels;
    }
