...
/**
 * This file defines the hwork grades table.
 *
...
require_once($CFG->dirroot . '/mod/hwork/report/attemptsreport_table.php');


/**
 * This is a table subclass for displaying the hwork grades report.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class hwork_overview_table extends hwork_attempts_report_table {
     ...
     * @param hwork_overview_options $options
     * @param \core\dml\sql_join $groupstudentsjoins
     * @param \core\dml\sql_join $studentsjoins
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($hwork, $context, $qmsubselect,
            hwork_overview_options $options, \core\dml\sql_join $groupstudentsjoins,
            \core\dml\sql_join $studentsjoins, $questions, $reporturl) {
        parent::__construct('mod-hwork-report-overview-report', $hwork , $context,
    ...
    /**
     * Calculate the average overall and question scores for a set of attempts at the hwork.
     *
    ...
    public function compute_average_row($label, \core\dml\sql_join $usersjoins) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(hworkaouter.sumgrades) AS grade, COUNT(hworkaouter.sumgrades) AS numaveraged
                  FROM {hwork_attempts} hworkaouter
                  JOIN (
                       SELECT DISTINCT hworka.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON hworkaouter.id = relevant_attempt_ids.id
                ", $params);
        $record->grade = hwork_rescale_grade($record->grade, $this->hwork, false);
        ...
        $averagerow = array(
            ...
            'feedbacktext' => strip_tags(hwork_report_feedback_for_grade(
                                         $record->grade, $this->hwork->id, $this->context))
        );

        if ($this->options->slotmarks) {
            $dm = new question_engine_data_mapper();
            $qubaids = new qubaid_join("{hwork_attempts} hworkaouter
                  JOIN (
                       SELECT DISTINCT hworka.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON hworkaouter.id = relevant_attempt_ids.id",
                    'hworkaouter.uniqueid', '1 = 1', $params);
    ...
    protected function format_average_grade_for_questions($gradeaverages) {
        ...
        foreach ($this->questions as $question) {
            if (isset($gradeaverages[$question->slot]) && $question->maxmark > 0) {
                ...
                $record->grade = hwork_rescale_grade(
                        $record->averagefraction * $question->maxmark, $this->hwork, false);
    ...
    protected function format_average($record, $question = false) {
        if (is_null($record->grade)) {
            ...
            $average = hwork_format_question_grade($this->hwork, $record->grade);
        } else {
            $average = hwork_format_grade($this->hwork, $record->grade);
        }
    ...
    protected function submit_buttons() {
        if (has_capability('mod/hwork:regrade', $this->context)) {
            echo '<input type="submit" class="btn btn-secondary m-r-1" name="regrade" value="' .
                    get_string('regradeselected', 'hwork_overview') . '"/>';
        }
    ...
    public function col_sumgrades($attempt) {
        ...
        $grade = hwork_rescale_grade($attempt->sumgrades, $this->hwork);
        if ($this->is_downloading()) {
            return $grade;
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid])) {
            ...
            $newsumgrade = hwork_rescale_grade($newsumgrade, $this->hwork);
            $oldsumgrade = hwork_rescale_grade($oldsumgrade, $this->hwork);
            $grade = html_writer::tag('del', $oldsumgrade) . '/' .
                    html_writer::empty_tag('br') . $newsumgrade;
        }
        return html_writer::link(new moodle_url('/mod/hwork/review.php',
                array('attempt' => $attempt->attempt)), $grade,
                array('title' => get_string('reviewattempt', 'hwork')));
    }

    /**
     * @param string $colname the name of the column.
     * @param object $attempt the row of data - see the SQL in display() in
     * mod/hwork/report/overview/report.php to see what fields are present,
    ...
    public function other_cols($colname, $attempt) {
        ...
        if ($question->maxmark == 0) {
            ...
            $grade = hwork_rescale_grade(
                    $stepdata->fraction * $question->maxmark, $this->hwork, 'question');
        }
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'hwork_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'hwork_overview');
        }
    }
    ...
    protected function update_sql_after_count($fields, $from, $where, $params) {
        $fields .= ", COALESCE((
                                SELECT MAX(qqr.regraded)
                                  FROM {hwork_overview_regrades} qqr
                                 WHERE qqr.questionusageid = hworka.uniqueid
                          ), -1) AS regraded";
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/hwork:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('hwork_overview_regrades',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return hwork_report_index_by_keys($regradedqs, array('questionusageid', 'slot'));
    }
}
