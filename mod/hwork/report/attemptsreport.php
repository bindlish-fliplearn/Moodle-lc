...
/**
 * Base class for hwork reports that are basically a table with one row for each attempt.
 *
 * @copyright 2010 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class hwork_attempts_report extends hwork_default_report {
    ...
    /** @var object the hwork context. */
    protected $context;

    /** @var mod_hwork_attempts_report_form The settings form to use. */
    protected $form;
     ...
     */
    protected function init($mode, $formclass, $hwork, $cm, $course) {
        $this->mode = $mode;
        ...
        $this->qmsubselect = hwork_report_qm_filter_select($hwork);

        $this->form = new $formclass($this->get_base_url(),
                array('hwork' => $hwork, 'currentgroup' => $currentgroup, 'context' => $this->context));
    ...
    protected function get_students_joins($cm, $course = null) {
        ...
        $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                array('mod/hwork:attempt', 'mod/hwork:reviewmyattempts'), $currentgroup);

        return array($currentgroup, $studentsjoins, $groupstudentsjoins, $groupstudentsjoins);
    }

    /**
     * Outputs the things you commonly want at the top of a hwork report.
     *
     ...
     * @param mod_hwork_attempts_report_options $options the current report settings.
     * @param int $currentgroup the current group.
     * @param bool $hasquestions whether there are any questions in the hwork.
     * @param bool $hasstudents whether there are any relevant students.
     */
    protected function print_standard_header_and_messages($cm, $course, $hwork,
            $options, $currentgroup, $hasquestions, $hasstudents) {
        global $OUTPUT;

        $this->print_header_and_tabs($cm, $course, $hwork, $this->mode);
        ...
        if ($strattemptnum = hwork_num_attempt_summary($hwork, $cm, true, $currentgroup)) {
            echo '<div class="hworkattemptcounts">' . $strattemptnum . '</div>';
        }
...
//        }
    ...
    protected function add_state_column(&$columns, &$headers) {
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'hwork');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'hwork');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'hwork');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'hwork');
    }
     ...
     * @param bool $usercanseegrades whether the user is allowed to see grades for this hwork.
     * @param array $columns the list of columns. Added to.
     * @param array $headers the columns headings. Added to.
     * @param bool $includefeedback whether to include the feedbacktext columns
     */
    protected function add_grade_columns($hwork, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            $columns[] = 'sumgrades';
            $headers[] = get_string('grade', 'hwork') . '/' .
                    hwork_format_grade($hwork, $hwork->grade);
        }

        if ($includefeedback && hwork_has_feedback($hwork)) {
            $columns[] = 'feedbacktext';
            $headers[] = get_string('feedback', 'hwork');
        }
    }
     ...
     * @param mod_hwork_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_hwork_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/hwork:deleteattempts', $this->context);
                    $this->delete_selected_attempts($hwork, $cm, $attemptids, $allowedjoins);
    ...
    /**
     * Delete the hwork attempts
     * @param object $hwork the hwork settings. Attempts that don't belong to
     * this hwork are not deleted.
     ...
     */
    protected function delete_selected_attempts($hwork, $cm, $attemptids, \core\dml\sql_join $allowedjoins) {
        global $DB;
        ...
        foreach ($attemptids as $attemptid) {
            if (empty($allowedjoins->joins)) {
                $sql = "SELECT hworka.*
                         ...
                         WHERE hworka.id = :attemptid";
            } else {
                $sql = "SELECT hworka.*
                          FROM {hwork_attempts} hworka
                          JOIN {user} u ON u.id = hworka.userid
                        {$allowedjoins->joins}
                         WHERE {$allowedjoins->wheres} AND hworka.id = :attemptid";
            }
            $params = $allowedjoins->params + array('attemptid' => $attemptid);
            $attempt = $DB->get_record_sql($sql, $params);
            if (!$attempt || $attempt->hwork != $hwork->id || $attempt->preview != 0) {
                // Ensure the attempt exists, belongs to this hwork and belongs to
                // a student included in the report. If not skip.
                continue;
            }

            // Set the course module id before calling hwork_delete_attempt().
            $hwork->cmid = $cm->id;
