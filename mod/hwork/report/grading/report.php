...
/**
 * This file defines the hwork manual grading report class.
 *
...
require_once($CFG->dirroot . '/mod/hwork/report/grading/gradingsettings_form.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class hwork_grading_report extends hwork_default_report {
    ...
    public function display($hwork, $cm, $course) {
        ...
        require_capability('mod/hwork:grade', $this->context);
        $shownames = has_capability('hwork/grading:viewstudentnames', $this->context);
        $showidnumbers = has_capability('hwork/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this hwork.
        $this->questions = hwork_report_get_significant_questions($hwork);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'hwork_grading');
        }
        ...
        if ($this->currentgroup == self::NO_GROUPS_ALLOWED) {
            ...
            $this->userssql = get_enrolled_sql($this->context,
                    array('mod/hwork:reviewmyattempts', 'mod/hwork:attempt'), $this->currentgroup);
        }
        ...
        if ($slot && $hasquestions) {
            ...
            if (!$counts || $counts->$grade == 0) {
                redirect($this->list_questions_url(), get_string('alldoneredirecting', 'hwork_grading'));
            }
        }

        // Start output.
        $this->print_header_and_tabs($cm, $course, $hwork, 'grading');
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "hworka.hwork = :mangrhworkid AND
                hworka.preview = 0 AND
                hworka.state = :statefinished";
        $params = array('mangrhworkid' => $this->cm->instance, 'statefinished' => hwork_attempt::FINISHED);
        ...
        if ($currentgroup) {
            ...
            if ($enrolleduserscount < 1) {
                $where .= ' AND hworka.userid = 0';
            } else {
                $usersjoin = "JOIN ({$userssql[0]}) AS enr ON hworka.userid = enr.id";
                $params += $userssql[1];
            }
        }

        return new qubaid_join("{hwork_attempts} hworka $usersjoin ", 'hworka.uniqueid', $where, $params);
    }
    ...
    protected function load_attempts_by_usage_ids($qubaids) {
        ...
        $params[] = hwork_attempt::FINISHED;
        $params[] = $this->hwork->id;

        $fields = 'hworka.*, u.idnumber, ';
        $fields .= get_all_user_name_fields(true, 'u');
        $attemptsbyid = $DB->get_records_sql("
                SELECT $fields
                FROM {hwork_attempts} hworka
                JOIN {user} u ON u.id = hworka.userid
                WHERE hworka.uniqueid $asql AND hworka.state = ? AND hworka.hwork = ?",
                $params);
    ...
    protected function base_url() {
        return new moodle_url('/mod/hwork/report.php',
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                    ...
                    get_string($gradestring, 'hwork_grading'),
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'hwork_grading'), 3);
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'hwork_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'hwork_grading');
        }
        ...
        if (empty($data)) {
            echo $OUTPUT->notification(get_string('nothingfound', 'hwork_grading'));
            return;
        }
        ...
        $table->head[] = get_string('qno', 'hwork_grading');
        $table->head[] = get_string('qtypeveryshort', 'question');
        $table->head[] = get_string('questionname', 'hwork_grading');
        $table->head[] = get_string('tograde', 'hwork_grading');
        $table->head[] = get_string('alreadygraded', 'hwork_grading');
        if ($includeauto) {
            $table->head[] = get_string('automaticallygraded', 'hwork_grading');
        }
        $table->head[] = get_string('total', 'hwork_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new hwork_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'hwork_grading', $a), 3);
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(),
                get_string('backtothelistofquestions', 'hwork_grading')),
        ...
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'hwork_grading', $a), 3);
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = hwork_get_review_options($this->hwork, $attempt, $this->context);
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                'type' => 'submit', 'class' => 'btn btn-primary', 'value' => get_string('saveandnext', 'hwork_grading'))),
                array('class' => 'mdl-align')) .
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'hwork_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'hwork_grading', $a);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new hwork_attempt($attempt, $this->hwork, $this->cm, $this->course);
            ...
            $params = array(
                ...
                'other' => array(
                    'hworkid' => $attemptobj->get_hworkid(),
                    'attemptid' => $attemptobj->get_attemptid(),
                    'slot' => $assumedslotforevents
                )
            );
            $events[] = \mod_hwork\event\question_manually_graded::create($params);
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        if ($orderby == 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON hworka.userid = u.id ";
