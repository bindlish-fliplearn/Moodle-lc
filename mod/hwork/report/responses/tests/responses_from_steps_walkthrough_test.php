 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class hwork_report_responses_from_steps_testcase extends mod_hwork_attempt_walkthrough_from_csv_testcase {
     ...
     *
     * @param array $hworksettings settings to override default settings for hwork created by generator. Taken from hworkzes.csv.
    ...
    public function test_walkthrough_from_csv($hworksettings, $csvdata) {
        ...
        $hworkattemptids = $this->walkthrough_attempts($csvdata['steps']);
        ...
        for ($rowno = 0; $rowno < $csvdata['responses']->getRowCount(); $rowno++) {
            ...
            if (!isset($hworkattemptids[$responses['hworkattempt']])) {
                throw new coding_exception("There is no hworkattempt {$responses['hworkattempt']}!");
            }
            $this->assert_response_test($hworkattemptids[$responses['hworkattempt']], $responses);
        }
    }

    protected function assert_response_test($hworkattemptid, $responses) {
        $hworkattempt = hwork_attempt::create($hworkattemptid);
        ...
        foreach ($responses['slot'] as $slot => $tests) {
            ...
            $qa = $hworkattempt->get_question_attempt($slot);
            ...
            if (null === $step) {
                throw new coding_exception("There is no step no {$responses['submittedstepno']} ".
                                           "for slot $slot in hworkattempt {$responses['hworkattempt']}!");
            }
            foreach (array('responsesummary', 'fraction', 'state') as $column) {
                if (isset($tests[$column]) && $tests[$column] != '') {
                    ...
                    $failuremessage = "Error in  hworkattempt {$responses['hworkattempt']} in $column, slot $slot, ".
