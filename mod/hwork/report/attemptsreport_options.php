...
/**
 * Base class for the options that control what is visible in an {@link hwork_attempts_report}.
 *
 * @package   mod_hwork
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_hwork_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the hwork being reported on. */
    public $hwork;

    /** @var object the course module objects for the hwork being reported on. */
    public $cm;

    /** @var object the course settings for the course the hwork is in. */
    public $course;

    /**
     * @var array form field name => corresponding hwork_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => hwork_attempt::IN_PROGRESS,
        'stateoverdue'    => hwork_attempt::OVERDUE,
        'statefinished'   => hwork_attempt::FINISHED,
        'stateabandoned'  => hwork_attempt::ABANDONED,
    );

    /**
     * @var string hwork_attempts_report::ALL_WITH or hwork_attempts_report::ENROLLED_WITH
     *      hwork_attempts_report::ENROLLED_WITHOUT or hwork_attempts_report::ENROLLED_ALL
     */
    public $attempts = hwork_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of hwork_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(hwork_attempt::IN_PROGRESS, hwork_attempt::OVERDUE,
            hwork_attempt::FINISHED, hwork_attempt::ABANDONED);
    ...
    public $pagesize = hwork_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $hwork the settings for the hwork being reported on.
     * @param object $cm the course module objects for the hwork being reported on.
     * @param object $coures the course settings for the coures this hwork is in.
     */
    public function __construct($mode, $hwork, $cm, $course) {
        $this->mode   = $mode;
        $this->hwork   = $hwork;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = hwork_report_should_show_grades($hwork, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/hwork/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('hwork_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('hwork_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == hwork_attempts_report::ALL_WITH) {
                $this->attempts = hwork_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = hwork_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(hwork_attempts_report::ALL_WITH, hwork_attempts_report::ENROLLED_WITH,
                hwork_attempts_report::ENROLLED_WITHOUT, hwork_attempts_report::ENROLLED_ALL))) {
            $this->attempts = hwork_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!hwork_report_can_filter_only_graded($this->hwork)) {
        ...
        if ($this->attempts == hwork_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = hwork_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(hwork_attempt::FINISHED, $this->states);
    }
}
