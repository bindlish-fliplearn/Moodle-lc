...
/**
 * Unit tests for (some of) mod/hwork/report/reportlib.php
 *
 * @package   mod_hwork
 * @category  phpunit
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU Public License
 */
class mod_hwork_reportlib_testcase extends advanced_testcase {
    public function test_hwork_report_index_by_keys() {
        $datum = array();
        $object = new stdClass();
        $object->qid = 3;
        $object->aid = 101;
        ...
        $indexed = hwork_report_index_by_keys($datum, array('aid', 'qid'));
        ...
        $indexed = hwork_report_index_by_keys($datum, array('aid', 'qid'), false);
    ...
    public function test_hwork_report_scale_summarks_as_percentage() {
        $hwork = new stdClass();
        $hwork->sumgrades = 10;
        $hwork->decimalpoints = 2;

        $this->assertEquals('12.34567%',
            hwork_report_scale_summarks_as_percentage(1.234567, $hwork, false));
        $this->assertEquals('12.35%',
            hwork_report_scale_summarks_as_percentage(1.234567, $hwork, true));
        $this->assertEquals('-',
            hwork_report_scale_summarks_as_percentage('-', $hwork, true));
    }

    public function test_hwork_report_qm_filter_select_only_one_attempt_allowed() {
        $hwork = new stdClass();
        $hwork->attempts = 1;
        $this->assertSame('', hwork_report_qm_filter_select($hwork));
    }

    public function test_hwork_report_qm_filter_select_average() {
        $hwork = new stdClass();
        $hwork->attempts = 10;
        $hwork->grademethod = QUIZ_GRADEAVERAGE;
        $this->assertSame('', hwork_report_qm_filter_select($hwork));
    }

    public function test_hwork_report_qm_filter_select_first_last_best() {
        global $DB;
        $this->resetAfterTest();

        $fakeattempt = new stdClass();
        $fakeattempt->userid = 123;
        $fakeattempt->hwork = 456;
        $fakeattempt->layout = '1,2,0,3,4,0,5';
        $fakeattempt->state = hwork_attempt::FINISHED;

        // We intentionally insert these in a funny order, to test the SQL better.
        // The test data is:
        // id | hworkid | user | attempt | sumgrades | state
        // ---------------------------------------------------
        ...
        $DB->insert_record('hwork_attempts', $fakeattempt);
        ...
        $fakeattempt->state = hwork_attempt::IN_PROGRESS;
        $DB->insert_record('hwork_attempts', $fakeattempt);
        ...
        $hwork->grademethod = QUIZ_ATTEMPTFIRST;
        $firstattempt = $DB->get_records_sql("
                SELECT * FROM {hwork_attempts} hworka WHERE userid = ? AND hwork = ? AND "
                        . hwork_report_qm_filter_select($hwork), array(123, 456));
        $this->assertEquals(1, count($firstattempt));
        $firstattempt = reset($firstattempt);
        $this->assertEquals(1, $firstattempt->attempt);

        $hwork->grademethod = QUIZ_ATTEMPTLAST;
        ...
        $bestattempt = $DB->get_records_sql("
                SELECT * FROM {hwork_attempts} qa_alias WHERE userid = ? AND hwork = ? AND "
                . hwork_report_qm_filter_select($hwork, 'qa_alias'), array(123, 456));
