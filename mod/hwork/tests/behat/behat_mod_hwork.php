...
/**
 * Steps definitions related to mod_hwork.
 *
 * @package   mod_hwork
 * @category  test
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_hwork extends behat_question_base {

    /**
     * Put the specified questions on the specified pages of a given hwork.
     *
     ...
     *
     * @param string $hworkname the name of the hwork to add questions to.
     * @param TableNode $data information about the questions to add.
     *
     * @Given /^hwork "([^"]*)" contains the following questions:$/
     */
    public function hwork_contains_the_following_questions($hworkname, TableNode $data) {
        global $DB;

        $hwork = $DB->get_record('hwork', array('name' => $hworkname), '*', MUST_EXIST);
        ...
        if (!in_array('question', $firstrow) && !in_array('page', $firstrow)) {
            if (count($firstrow) == 2) {
                ...
                throw new ExpectationException('When adding questions to a hwork, you should give 2 or three 3 things: ' .
        ...
        foreach ($data->getHash() as $questiondata) {
            if (!array_key_exists('question', $questiondata)) {
                throw new ExpectationException('When adding questions to a hwork, ' .
            ...
            if ($question->qtype == 'random') {
                ...
                hwork_add_random_questions($hwork, $page, $question->category, 1, $includingsubcategories);
            } else {
                // Add the question.
                hwork_add_hwork_question($question->id, $hwork, $page, $maxmark);
            }

            // Require previous.
            if (array_key_exists('requireprevious', $questiondata)) {
                if ($questiondata['requireprevious'] === '1') {
                    $slot = $DB->get_field('hwork_slots', 'MAX(slot)', array('hworkid' => $hwork->id));
                    $DB->set_field('hwork_slots', 'requireprevious', 1,
                            array('hworkid' => $hwork->id, 'slot' => $slot));
    ...
    /**
     * Put the specified section headings to start at specified pages of a given hwork.
     *
     * The first row should be column names:
     ...
     *
     * @param string $hworkname the name of the hwork to add sections to.
     * @param TableNode $data information about the sections to add.
     *
     * @Given /^hwork "([^"]*)" contains the following sections:$/
     */
    public function hwork_contains_the_following_sections($hworkname, TableNode $data) {
        global $DB;
        ...
        foreach ($data->getHash() as $rownumber => $sectiondata) {
            if (!array_key_exists('heading', $sectiondata)) {
                throw new ExpectationException('When adding sections to a hwork, ' .
            ...
            if ($rownumber == 0) {
                $section = $DB->get_record('hwork_sections', array('hworkid' => $hwork->id), '*', MUST_EXIST);
            } else {
                $section = new stdClass();
                $section->hworkid = $hwork->id;
            }
            ...
            if ($rownumber == 0) {
                ...
                $DB->insert_record('hwork_sections', $section);
            }
        }

        if ($section->firstslot > $DB->count_records('hwork_slots', array('hworkid' => $hwork->id))) {
            throw new ExpectationException('The section firstslot must be less than the total number of slots in the hwork.',
                    $this->getSession());
        }
    }

    /**
     * Adds a question to the existing hwork with filling the form.
     *
     * The form for creating a question should be on one page.
     *
     * @When /^I add a "(?P<question_type_string>(?:[^"]|\\")*)" question to the "(?P<hwork_name_string>(?:[^"]|\\")*)" hwork with:$/
     * @param string $questiontype
     * @param string $hworkname
     * @param TableNode $questiondata with data for filling the add question form
     */
    public function i_add_question_to_the_hwork_with($questiontype, $hworkname, TableNode $questiondata) {
        $hworkname = $this->escape($hworkname);
        $edithwork = $this->escape(get_string('edithwork', 'hwork'));
        $hworkadmin = $this->escape(get_string('pluginadministration', 'hwork'));
        $addaquestion = $this->escape(get_string('addaquestion', 'hwork'));
        ...
        $this->execute('behat_general::click_link', $hworkname);

        $this->execute("behat_navigation::i_navigate_to_node_in", array($edithwork, $hworkadmin));
    ...
    /**
     * Set the max mark for a question on the Edit hwork page.
     *
     ...
     */
    public function i_set_the_max_mark_for_hwork_question($questionname, $newmark) {
        $this->execute('behat_general::click_link', $this->escape(get_string('editmaxmark', 'hwork')));
    ...
    /**
     * Open the add menu on a given page, or at the end of the Edit hwork page.
     * @Given /^I open the "(?P<page_n_or_last_string>(?:[^"]|\\")*)" add to hwork menu$/
     * @param string $pageorlast either "Page n" or "last".
     */
    public function i_open_the_add_to_hwork_menu_for($pageorlast) {
        ...
        if ($pageorlast == 'last') {
            ...
            throw new ExpectationException("The I open the add to hwork menu step must specify either 'Page N' or 'last'.");
        }
        $this->find('xpath', $xpath)->click();
    }

    /**
     * Check whether a particular question is on a particular page of the hwork on the Edit hwork page.
     * @Given /^I should see "(?P<question_name>(?:[^"]|\\")*)" on hwork page "(?P<page_number>\d+)"$/
     ...
     */
    public function i_should_see_on_hwork_page($questionname, $pagenumber) {
    ...
    /**
     * Check whether a particular question is not on a particular page of the hwork on the Edit hwork page.
     * @Given /^I should not see "(?P<question_name>(?:[^"]|\\")*)" on hwork page "(?P<page_number>\d+)"$/
     * @param string $questionname the name of the question we are looking for.
     * @param number $pagenumber the page it should be found on.
     */
    public function i_should_not_see_on_hwork_page($questionname, $pagenumber) {
        $xpath = "//li[contains(., '" . $this->escape($questionname) .
                "')][./preceding-sibling::li[contains(@class, 'pagenumber')][1][contains(., 'Page " .
                $pagenumber . "')]]";
    ...
    /**
     * Check whether one question comes before another on the Edit hwork page.
     * The two questions must be on the same page.
     * @Given /^I should see "(?P<first_q_name>(?:[^"]|\\")*)" before "(?P<second_q_name>(?:[^"]|\\")*)" on the edit hwork page$/
     ...
     */
    public function i_should_see_before_on_the_edit_hwork_page($firstquestionname, $secondquestionname) {
    ...
    /**
     * Check the number displayed alongside a question on the Edit hwork page.
     * @Given /^"(?P<question_name>(?:[^"]|\\")*)" should have number "(?P<number>(?:[^"]|\\")*)" on the edit hwork page$/
     * @param string $questionname the name of the question we are looking for.
     * @param number $number the number (or 'i') that should be displayed beside that question.
     */
    public function should_have_number_on_the_edit_hwork_page($questionname, $number) {
     ...
     *
     * @Given /^I click on shuffle for section "([^"]*)" on the hwork edit page$/
     */
     ...
     *
     * @Given /^shuffle for section "([^"]*)" should be "(On|Off)" on the hwork edit page$/
     */
    public function shuffle_for_section_should_be($heading, $value) {
        ...
        if ($value == 'On' && !$checkbox->isChecked()) {
            $msg = "Shuffle for section '$heading' is not checked, but you are expecting it to be checked ($value). " .
                    "Check the line with: \nshuffle for section \"$heading\" should be \"$value\" on the hwork edit page" .
    ...
    /**
     * Move a question on the Edit hwork page by first clicking on the Move icon,
     * then clicking one of the "After ..." links.
     * @When /^I move "(?P<question_name>(?:[^"]|\\")*)" to "(?P<target>(?:[^"]|\\")*)" in the hwork by clicking the move icon$/
     * @param string $questionname the name of the question we are looking for.
    ...
    /**
     * Move a question on the Edit hwork page by dragging a given question on top of another item.
     * @When /^I move "(?P<question_name>(?:[^"]|\\")*)" to "(?P<target>(?:[^"]|\\")*)" in the hwork by dragging$/
     * @param string $questionname the name of the question we are looking for.
    ...
    /**
     * Delete a question on the Edit hwork page by first clicking on the Delete icon,
     * then clicking one of the "After ..." links.
     * @When /^I delete "(?P<question_name>(?:[^"]|\\")*)" in the hwork by clicking the delete icon$/
     * @param string $questionname the name of the question we are looking for.
     * @return array of steps.
     */
    ...
    /**
     * Set the section heading for a given section on the Edit hwork page
     *
     * @When /^I change hwork section heading "(?P<section_name_string>(?:[^"]|\\")*)" to "(?P<new_section_heading_string>(?:[^"]|\\")*)"$/
     ...
     * hwork navigation block.
     *
     * @Then /^I should see question "(?P<questionnumber>\d+)" in section "(?P<section_heading_string>(?:[^"]|\\")*)" in the hwork navigation$/
     ...
     */
    public function i_should_see_question_in_section_in_the_hwork_navigation($questionnumber, $sectionheading) {
        ...
        $exception = new ExpectationException('Question "' . $questionnumber . '" is not in section "' .
                $sectionheading . '" in the hwork navigation.', $this->getSession());
        $xpath = "//div[@id = 'mod_hwork_navblock']//*[contains(concat(' ', normalize-space(@class), ' '), ' qnbutton ') and " .
