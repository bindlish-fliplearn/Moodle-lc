 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_hwork_attempt_walkthrough_testcase extends advanced_testcase {

    /**
     * Create a hwork with questions and walk through a hwork attempt.
     */
    public function test_hwork_attempt_walkthrough() {
        global $SITE;
        ...
        $hwork = $hworkgenerator->create_instance(array('course'=>$SITE->id, 'questionsperpage' => 0, 'grade' => 100.0,
                                                      'sumgrades' => 2));
        ...
        // Check hwork grades.
        $grades = hwork_get_user_grades($hwork, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'hwork', $hwork->id, $user1->id);
    ...
    /**
     * Create a hwork with a random as well as other questions and walk through hwork attempts.
     */
    public function test_hwork_with_random_question_attempt_walkthrough() {
        global $SITE;
        ...
        $hwork = $hworkgenerator->create_instance(array('course' => $SITE->id, 'questionsperpage' => 2, 'grade' => 100.0,
                                                      'sumgrades' => 4));
        ...
        // Add random question to the hwork.
        ...
        foreach (array($saq->id => 'frog', $numq->id => '3.14') as $randomqidtoselect => $randqanswer) {
            ...
            $timenow = time();
            $attempt = hwork_create_attempt($hworkobj, 1, false, $timenow);

            hwork_start_new_attempt($hworkobj, $quba, $attempt, 1, $timenow, array(1 => $randomqidtoselect));
    ...
    protected $hworkwithvariants = null;

    /**
     * Create a hwork with a single question with variants and walk through hwork attempts.
     *
     * @dataProvider get_correct_response_for_variants
     */
    public function test_hwork_with_question_with_variants_attempt_walkthrough($variantno, $correctresponse, $done = false) {
        global $SITE;
        ...
        if ($this->hworkwithvariants === null) {
            // Make a hwork.
            $hworkgenerator = $this->getDataGenerator()->get_plugin_generator('mod_hwork');

            $this->hworkwithvariants = $hworkgenerator->create_instance(array('course'=>$SITE->id,
                                                                            'questionsperpage' => 0,
                                                                            'grade' => 100.0,
                                                                            'sumgrades' => 1));
            ...
            hwork_add_hwork_question($calc->id, $this->hworkwithvariants, 0);
        }


        // Make a new user to do the hwork.
        $user1 = $this->getDataGenerator()->create_user();
        $this->setUser($user1);
        $hworkobj = hwork::create($this->hworkwithvariants->id, $user1->id);
        ...
        // Select variant.
        hwork_start_new_attempt($hworkobj, $quba, $attempt, 1, $timenow, array(), array(1 => $variantno));
        ...
        // Check hwork grades.
        $grades = hwork_get_user_grades($this->hworkwithvariants, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'hwork', $this->hworkwithvariants->id, $user1->id);
        $gradebookitem = array_shift($gradebookgrades->items);
        $gradebookgrade = array_shift($gradebookitem->grades);
        $this->assertEquals(100, $gradebookgrade->grade);
    }
