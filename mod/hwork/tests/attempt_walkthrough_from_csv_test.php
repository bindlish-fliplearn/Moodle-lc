 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_hwork_attempt_walkthrough_from_csv_testcase extends advanced_testcase {

    protected $files = array('questions', 'steps', 'results');

    /**
     * @var stdClass the hwork record we create.
     */
    protected $hwork;
     ...
     * directory.
     *
     * @param array $hworksettings of settings read from csv file hworkzes.csv
     ...
     */
    public function test_walkthrough_from_csv($hworksettings, $csvdata) {

        // CSV data files for these tests were generated using :
        // https://github.com/jamiepratt/moodle-hwork-tools/tree/master/responsegenerator

        $this->create_hwork_simulate_attempts_and_check_results($hworksettings, $csvdata);
    }

    public function create_hwork($hworksettings, $qs) {
        global $SITE, $DB;
        $this->setAdminUser();
        ...
        $aggregratedsettings = $hworksettings + array('course' => $SITE->id,
                                                     'questionsperpage' => 0,
                                                     'grade' => 100.0,
                                                     'sumgrades' => $sumofgrades);

        $this->hwork = $hworkgenerator->create_instance($aggregratedsettings);

        $this->randqids = array();
        foreach ($slots as $slotno => $slotquestion) {
            if ($slotquestion['type'] !== 'random') {
                hwork_add_hwork_question($slotquestion['id'], $this->hwork, 0, $slotquestion['mark']);
            } else {
                hwork_add_random_questions($this->hwork, 0, $slotquestion['catid'], 1, 0);
    ...
    /**
     * Create hwork, simulate attempts and check results (if resultsXX.csv exists).
     *
     * @param array $hworksettings Quiz overrides for this hwork.
     * @param PHPUnit\DbUnit\DataSet\ITable[] $csvdata Data loaded from csv files for this test.
     */
    protected function create_hwork_simulate_attempts_and_check_results($hworksettings, $csvdata) {
        $this->resetAfterTest(true);
        question_bank::get_qtype('random')->clear_caches_before_testing();

        $this->create_hwork($hworksettings, $csvdata['questions']);
    ...
    public function get_data_for_walkthrough() {
        $hworkzes = $this->load_csv_data_file('hworkzes');
        $datasets = array();
        for ($rowno = 0; $rowno < $hworkzes->getRowCount(); $rowno++) {
            $hworksettings = $hworkzes->getRow($rowno);
            $dataset = array();
            foreach ($this->files as $file) {
                if (file_exists($this->get_full_path_of_csv_file($file, $hworksettings['testnumber']))) {
                    $dataset[$file] = $this->load_csv_data_file($file, $hworksettings['testnumber']);
                }
            }
            $datasets[] = array($hworksettings, $dataset);
        }
        return $datasets;
    }
     ...
     * @return array attempt no as in csv file => the id of the hwork_attempt as stored in the db.
     */
    protected function walkthrough_attempts($steps) {
        ...
        for ($rowno = 0; $rowno < $steps->getRowCount(); $rowno++) {
            ...
            // Find existing user or make a new user to do the hwork.
            ...
            if (!isset($attemptids[$step['hworkattempt']])) {
                // Start the attempt.
                $hworkobj = hwork::create($this->hwork->id, $user->id);
                ...
                $prevattempts = hwork_get_user_attempts($this->hwork->id, $user->id, 'all', true);
                $attemptnumber = count($prevattempts) + 1;
                $timenow = time();
                $attempt = hwork_create_attempt($hworkobj, $attemptnumber, false, $timenow, false, $user->id);
                ...
                hwork_start_new_attempt($hworkobj, $quba, $attempt, $attemptnumber, $timenow, $step['randqs'], $step['variants']);
                hwork_attempt_save_started($hworkobj, $quba, $attempt);
                $attemptid = $attemptids[$step['hworkattempt']] = $attempt->id;
            } else {
                $attemptid = $attemptids[$step['hworkattempt']];
            }
     ...
     * @param $attemptids array attempt no as in csv file => the id of the hwork_attempt as stored in the db.
     */
    protected function check_attempts_results($results, $attemptids) {
        for ($rowno = 0; $rowno < $results->getRowCount(); $rowno++) {
            ...
            // Re-load hwork attempt data.
            $attemptobj = hwork_attempt::create($attemptids[$result['hworkattempt']]);
     ...
     * @param hwork_attempt $attemptobj         the attempt object loaded from db.
        ...
        foreach ($result as $fieldname => $value) {
            ...
            switch ($fieldname) {
                            ...
                            switch ($slotfieldname) {
                                case 'mark' :
                                    $this->assertEquals(round($slotvalue, 2), $attemptobj->get_question_mark($slotno),
                                                        "Mark for slot $slotno of attempt {$result['hworkattempt']}.");
                                    break;
                                default :
                ...
                case 'finished' :
                    ...
                    break;
                case 'summarks' :
                    $this->assertEquals($value, $attemptobj->get_sum_marks(), "Sum of marks of attempt {$result['hworkattempt']}.");
                    break;
                case 'hworkgrade' :
                    // Check hwork grades.
                    $grades = hwork_get_user_grades($attemptobj->get_hwork(), $attemptobj->get_userid());
                    $grade = array_shift($grades);
                    $this->assertEquals($value, $grade->rawgrade, "Quiz grade for attempt {$result['hworkattempt']}.");
                    break;
                case 'gradebookgrade' :
                    // Check grade book.
                    $gradebookgrades = grade_get_grades($attemptobj->get_courseid(),
                                                        'mod', 'hwork',
                                                        $attemptobj->get_hworkid(),
                                                        $attemptobj->get_userid());
                    $gradebookitem = array_shift($gradebookgrades->items);
                    $gradebookgrade = array_shift($gradebookitem->grades);
                    $this->assertEquals($value, $gradebookgrade->grade, "Gradebook grade for attempt {$result['hworkattempt']}.");
                    break;
                default :
