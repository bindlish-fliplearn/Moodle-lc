...
/**
 * Silly class to access mod_hwork_external internal methods.
 *
 * @package mod_hwork
 * @copyright 2016 Juan Leyva <juan@moodle.com>
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since  Moodle 3.1
 */
class testable_mod_hwork_external extends mod_hwork_external {
 ...
 * @since      Moodle 3.1
 */
class mod_hwork_external_testcase extends externallib_advanced_testcase {
    ...
    public function setUp() {
        ...
        $this->hwork = $this->getDataGenerator()->create_module('hwork', array('course' => $this->course->id));
        $this->context = context_module::instance($this->hwork->cmid);
        $this->cm = get_coursemodule_from_instance('hwork', $this->hwork->id);
    ...
    /**
     * Create a hwork with questions including a started or finished attempt optionally
     *
     * @param  boolean $startattempt whether to start a new attempt
     * @param  boolean $finishattempt whether to finish the new attempt
     * @param  string $behaviour the hwork preferredbehaviour, defaults to 'deferredfeedback'.
     * @return array array containing the hwork, context and the attempt
     */
    private function create_hwork_with_questions($startattempt = false, $finishattempt = false, $behaviour = 'deferredfeedback') {

        // Create a new hwork with attempts.
        ...
        $hworkobj = hwork::create($hwork->id, $this->student->id);
        ...
        if ($startattempt or $finishattempt) {
            ...
            $timenow = time();
            $attempt = hwork_create_attempt($hworkobj, 1, false, $timenow, false, $this->student->id);
            ...
            return array($hwork, $context, $hworkobj, $attempt, $attemptobj, $quba);
        } else {
            return array($hwork, $context, $hworkobj);
        }

    }

    /*
     * Test get hworkzes by courses
     */
    public function test_mod_hwork_get_hworkzes_by_courses() {
        global $DB;
        ...
        $hwork2 = self::getDataGenerator()->create_module('hwork', $record);
        ...
        $returndescription = mod_hwork_external::get_hworkzes_by_courses_returns();
        ...
        $hwork1 = $this->hwork;
        $hwork1->coursemodule = $hwork1->cmid;
        $hwork1->introformat = 1;
        $hwork1->section = 0;
        $hwork1->visible = true;
        $hwork1->groupmode = 0;
        $hwork1->groupingid = 0;
        $hwork1->hasquestions = 0;
        $hwork1->hasfeedback = 0;
        $hwork1->autosaveperiod = get_config('hwork', 'autosaveperiod');
        $hwork1->introfiles = [];

        $hwork2->coursemodule = $hwork2->cmid;
        $hwork2->introformat = 1;
        $hwork2->section = 0;
        $hwork2->visible = true;
        $hwork2->groupmode = 0;
        $hwork2->groupingid = 0;
        $hwork2->hasquestions = 0;
        $hwork2->hasfeedback = 0;
        $hwork2->autosaveperiod = get_config('hwork', 'autosaveperiod');
        $hwork2->introfiles = [];

        foreach (array_merge($allusersfields, $userswithaccessfields) as $field) {
            $expected1[$field] = $hwork1->{$field};
            $expected2[$field] = $hwork2->{$field};
        }

        $expectedhworkzes = array($expected2, $expected1);

        // Call the external function passing course ids.
        $result = mod_hwork_external::get_hworkzes_by_courses(array($course2->id, $this->course->id));
        $result = external_api::clean_returnvalue($returndescription, $result);

        $this->assertEquals($expectedhworkzes, $result['hworkzes']);
        $this->assertCount(0, $result['warnings']);

        // Call the external function without passing course id.
        $result = mod_hwork_external::get_hworkzes_by_courses();
        $result = external_api::clean_returnvalue($returndescription, $result);
        $this->assertEquals($expectedhworkzes, $result['hworkzes']);
        $this->assertCount(0, $result['warnings']);

        // Unenrol user from second course and alter expected hworkzes.
        $enrol->unenrol_user($instance2, $this->student->id);
        array_shift($expectedhworkzes);
        ...
        $result = mod_hwork_external::get_hworkzes_by_courses(array($course2->id));
        ...
        foreach ($managerfields as $field) {
            $expectedhworkzes[0][$field] = $hwork1->{$field};
        }
        ...
        $result = mod_hwork_external::get_hworkzes_by_courses(array($this->course->id));
        $result = external_api::clean_returnvalue($returndescription, $result);
        $this->assertEquals($expectedhworkzes, $result['hworkzes']);
        ...
        $hwork2->timeclose = time() - DAYSECS;
        $DB->update_record('hwork', $hwork2);

        $result = mod_hwork_external::get_hworkzes_by_courses();
        $result = external_api::clean_returnvalue($returndescription, $result);
        $this->assertCount(2, $result['hworkzes']);
        // We only see a limited set of fields.
        $this->assertCount(4, $result['hworkzes'][0]);
        $this->assertEquals($hwork2->id, $result['hworkzes'][0]['id']);
        $this->assertEquals($hwork2->coursemodule, $result['hworkzes'][0]['coursemodule']);
        $this->assertEquals($hwork2->course, $result['hworkzes'][0]['course']);
        $this->assertEquals($hwork2->name, $result['hworkzes'][0]['name']);
        $this->assertEquals($hwork2->course, $result['hworkzes'][0]['course']);

        $this->assertFalse(isset($result['hworkzes'][0]['timelimit']));

    }

    /**
     * Test test_view_hwork
     */
    public function test_view_hwork() {
        global $DB;

        // Test invalid instance id.
        try {
            mod_hwork_external::view_hwork(0);
            $this->fail('Exception expected due to invalid mod_hwork instance id.');
        ...
        try {
            mod_hwork_external::view_hwork($this->hwork->id);
        ...
        $result = mod_hwork_external::view_hwork($this->hwork->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::view_hwork_returns(), $result);
        ...
        $this->assertInstanceOf('\mod_hwork\event\course_module_viewed', $event);
        $this->assertEquals($this->context, $event->get_context());
        $moodlehwork = new \moodle_url('/mod/hwork/view.php', array('id' => $this->cm->id));
        $this->assertEquals($moodlehwork, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());

        // Test user with no capabilities.
        // We need a explicit prohibit since this capability is only defined in authenticated user and guest roles.
        assign_capability('mod/hwork:view', CAP_PROHIBIT, $this->studentrole->id, $this->context->id);
    ...
    public function test_get_user_attempts() {
        ...
        // Create a hwork with one attempt finished.
        list($hwork, $context, $hworkobj, $attempt, $attemptobj) = $this->create_hwork_with_questions(true, true);

        $this->setUser($this->student);
        $result = mod_hwork_external::get_user_attempts($hwork->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_user_attempts_returns(), $result);

        $this->assertCount(1, $result['attempts']);
        $this->assertEquals($attempt->id, $result['attempts'][0]['id']);
        $this->assertEquals($hwork->id, $result['attempts'][0]['hwork']);
        ...
        $result = mod_hwork_external::get_user_attempts($hwork->id, 0, 'finished', false);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_user_attempts_returns(), $result);
        ...
        $result = mod_hwork_external::get_user_attempts($hwork->id, 0, 'all', false);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_user_attempts_returns(), $result);
        ...
        $result = mod_hwork_external::get_user_attempts($hwork->id, 0, 'unfinished', false);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_user_attempts_returns(), $result);
        ...
        $timenow = time();
        $attempt = hwork_create_attempt($hworkobj, 2, false, $timenow, false, $this->student->id);
        $quba = question_engine::make_questions_usage_by_activity('mod_hwork', $hworkobj->get_context());
        $quba->set_preferred_behaviour($hworkobj->get_hwork()->preferredbehaviour);
        ...
        $result = mod_hwork_external::get_user_attempts($hwork->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_user_attempts_returns(), $result);

        $this->assertCount(1, $result['attempts']);
        $this->assertEquals($this->student->id, $result['attempts'][0]['userid']);

        $result = mod_hwork_external::get_user_attempts($hwork->id, $this->student->id, 'all');
        $result = external_api::clean_returnvalue(mod_hwork_external::get_user_attempts_returns(), $result);
        ...
        // Invalid parameters.
        try {
            mod_hwork_external::get_user_attempts($hwork->id, $this->student->id, 'INVALID_PARAMETER');
            $this->fail('Exception expected due to missing capability.');
    ...
    public function test_get_user_best_grade() {
        ...
        $result = mod_hwork_external::get_user_best_grade($this->hwork->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_user_best_grade_returns(), $result);
        ...
        try {
            mod_hwork_external::get_user_best_grade($this->hwork->id, $anotherstudent->id);
            $this->fail('Exception expected due to missing capability.');
        ...
        $result = mod_hwork_external::get_user_best_grade($this->hwork->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_user_best_grade_returns(), $result);
        ...
        // Invalid user.
        try {
            mod_hwork_external::get_user_best_grade($this->hwork->id, -1);
            $this->fail('Exception expected due to missing capability.');
        ...
        $DB->delete_records('hwork_grades', array('id' => $grade->id));

    }
    /**
     * Test get_combined_review_options.
     * This is a basic test, this is already tested in mod_hwork_display_options_testcase.
     */
    public function test_get_combined_review_options() {
        ...
        $result = mod_hwork_external::get_combined_review_options($hwork->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_combined_review_options_returns(), $result);
        ...
        $result = mod_hwork_external::get_combined_review_options($hwork->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_combined_review_options_returns(), $result);
        ...
        // Invalid user.
        try {
            mod_hwork_external::get_combined_review_options($hwork->id, -1);
            $this->fail('Exception expected due to missing capability.');
        } catch (dml_missing_record_exception $e) {
            $this->assertEquals('invaliduser', $e->errorcode);
        }
    ...
    public function test_start_attempt() {
        ...
        // Create a new hwork with questions.
        list($hwork, $context, $hworkobj) = $this->create_hwork_with_questions();

        $this->setUser($this->student);

        // Try to open attempt in closed hwork.
        $hwork->timeopen = time() - WEEKSECS;
        $hwork->timeclose = time() - DAYSECS;
        $DB->update_record('hwork', $hwork);
        $result = mod_hwork_external::start_attempt($hwork->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::start_attempt_returns(), $result);
        ...
        try {
            mod_hwork_external::start_attempt($hwork->id, array(array("name" => "hworkpassword", "value" => 'bad')));
            $this->fail('Exception expected due to invalid passwod.');
        } catch (moodle_exception $e) {
            $this->assertEquals(get_string('passworderror', 'hworkaccess_password'), $e->errorcode);
        }

        // Now, try everything correct.
        $result = mod_hwork_external::start_attempt($hwork->id, array(array("name" => "hworkpassword", "value" => 'abc')));
        $result = external_api::clean_returnvalue(mod_hwork_external::start_attempt_returns(), $result);

        $this->assertEquals(1, $result['attempt']['attempt']);
        $this->assertEquals($this->student->id, $result['attempt']['userid']);
        $this->assertEquals($hwork->id, $result['attempt']['hwork']);
        $this->assertCount(0, $result['warnings']);
        ...
        try {
            mod_hwork_external::start_attempt($hwork->id, array(array("name" => "hworkpassword", "value" => 'abc')));
            $this->fail('Exception expected due to attempt not finished.');
        } catch (moodle_hwork_exception $e) {
        ...
        // Process some responses from the student.
        $timenow = time();
        $attemptobj = hwork_attempt::create($attemptid);
        $tosubmit = array(1 => array('answer' => '3.14'));
        ...
        // Test user with no capabilities.
        // We need a explicit prohibit since this capability is only defined in authenticated user and guest roles.
        assign_capability('mod/hwork:attempt', CAP_PROHIBIT, $this->studentrole->id, $context->id);
        // Empty all the caches that may be affected  by this change.
        accesslib_clear_all_caches_for_unit_testing();
        course_modinfo::clear_instance_cache();

        try {
            mod_hwork_external::start_attempt($hwork->id);
            $this->fail('Exception expected due to missing capability.');
        } catch (required_capability_exception $e) {
            $this->assertEquals('nopermissions', $e->errorcode);
        }
    ...
    public function test_validate_attempt() {
        ...
        // Create a new hwork with one attempt started.
        list($hwork, $context, $hworkobj, $attempt, $attemptobj) = $this->create_hwork_with_questions(true);
        ...
        try {
            ...
            testable_mod_hwork_external::validate_attempt($params);
        ...
        $result = testable_mod_hwork_external::validate_attempt($params);
        ...
        try {
            $params = array('attemptid' => $attempt->id, 'page' => 0,
                            'preflightdata' => array(array("name" => "hworkpassword", "value" => 'bad')));
            testable_mod_hwork_external::validate_attempt($params);
            $this->fail('Exception expected due to invalid passwod.');
        } catch (moodle_exception $e) {
            $this->assertEquals(get_string('passworderror', 'hworkaccess_password'), $e->errorcode);
        ...
        testable_mod_hwork_external::validate_attempt($params, false);
    ...
    public function test_get_attempt_data() {
        ...
        $DB->set_field('hwork', 'reviewcorrectness', mod_hwork_display_options::IMMEDIATELY_AFTER, array('id' => $hwork->id));

        $hworkobj = $attemptobj->get_hworkobj();
        $hworkobj->preload_questions();
        $hworkobj->load_questions();
        $questions = $hworkobj->get_questions();

        $this->setUser($this->student);

        // We receive one question per page.
        $result = mod_hwork_external::get_attempt_data($attempt->id, 0);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_attempt_data_returns(), $result);
        ...
        $result = mod_hwork_external::get_attempt_data($attempt->id, 1);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_attempt_data_returns(), $result);
        ...
        $result = mod_hwork_external::get_attempt_review($attempt->id, 1);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_attempt_review_returns(), $result);
        $this->assertEquals('gaveup', $result['questions'][0]['state']);

        // Change setting and expect two pages.
        $hwork->questionsperpage = 4;
        $DB->update_record('hwork', $hwork);
    ...
    public function test_get_attempt_data_with_blocked_questions() {
        ...
        // Create a new hwork with one attempt started and using immediatefeedback.
        list($hwork, $context, $hworkobj, $attempt, $attemptobj) = $this->create_hwork_with_questions(
    ...
    public function test_get_attempt_summary() {
        ...
        $this->setUser($this->student);
        $result = mod_hwork_external::get_attempt_summary($attempt->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_attempt_summary_returns(), $result);
    ...
    public function test_save_attempt() {
        ...
        $timenow = time();
        // Create a new hwork with one attempt started.
        list($hwork, $context, $hworkobj, $attempt, $attemptobj, $quba) = $this->create_hwork_with_questions(true);
        ...
        $result = mod_hwork_external::save_attempt($attempt->id, $data);
        $result = external_api::clean_returnvalue(mod_hwork_external::save_attempt_returns(), $result);
        $this->assertTrue($result['status']);
    ...
    public function test_process_attempt() {
        ...
        $timenow = time();
        // Create a new hwork with two questions and one attempt started.
        list($hwork, $context, $hworkobj, $attempt, $attemptobj, $quba) = $this->create_hwork_with_questions(true);
        ...
        $result = mod_hwork_external::process_attempt($attempt->id, $data);
        $result = external_api::clean_returnvalue(mod_hwork_external::process_attempt_returns(), $result);
        $this->assertEquals(hwork_attempt::IN_PROGRESS, $result['state']);
        ...
        // Finish the attempt.
        $result = mod_hwork_external::process_attempt($attempt->id, array(), true);
        $result = external_api::clean_returnvalue(mod_hwork_external::process_attempt_returns(), $result);
        $this->assertEquals(hwork_attempt::FINISHED, $result['state']);
        ...
        $timenow = time();
        $attempt = hwork_create_attempt($hworkobj, 2, false, $timenow, false, $this->student->id);
        hwork_start_new_attempt($hworkobj, $quba, $attempt, 2, $timenow);
        hwork_attempt_save_started($hworkobj, $quba, $attempt);

        // Force grace period, attempt going to overdue.
        $hwork->timeclose = $timenow - 10;
        $hwork->graceperiod = 60;
        $hwork->overduehandling = 'graceperiod';
        $DB->update_record('hwork', $hwork);

        $result = mod_hwork_external::process_attempt($attempt->id, array());
        $result = external_api::clean_returnvalue(mod_hwork_external::process_attempt_returns(), $result);
        $this->assertEquals(hwork_attempt::OVERDUE, $result['state']);

        // New attempt.
        $timenow = time();
        $quba = question_engine::make_questions_usage_by_activity('mod_hwork', $hworkobj->get_context());
        $quba->set_preferred_behaviour($hworkobj->get_hwork()->preferredbehaviour);
        $attempt = hwork_create_attempt($hworkobj, 3, 2, $timenow, false, $this->student->id);
        hwork_start_new_attempt($hworkobj, $quba, $attempt, 3, $timenow);
        hwork_attempt_save_started($hworkobj, $quba, $attempt);

        // Force abandon.
        $hwork->timeclose = $timenow - HOURSECS;
        $DB->update_record('hwork', $hwork);

        $result = mod_hwork_external::process_attempt($attempt->id, array());
        $result = external_api::clean_returnvalue(mod_hwork_external::process_attempt_returns(), $result);
        $this->assertEquals(hwork_attempt::ABANDONED, $result['state']);
    ...
    public function test_validate_attempt_review() {
        ...
        try {
            ...
            testable_mod_hwork_external::validate_attempt_review($params);
    ...
    public function test_get_attempt_review() {
        ...
        // Create a new hwork with two questions and one attempt finished.
        list($hwork, $context, $hworkobj, $attempt, $attemptobj, $quba) = $this->create_hwork_with_questions(true, true);

        // Add feedback to the hwork.
        ...
        $result = mod_hwork_external::get_attempt_review($attempt->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_attempt_review_returns(), $result);
        ...
        // Only first page.
        $result = mod_hwork_external::get_attempt_review($attempt->id, 0);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_attempt_review_returns(), $result);
    ...
    public function test_view_attempt() {
        ...
        // Create a new hwork with two questions and one attempt started.
        list($hwork, $context, $hworkobj, $attempt, $attemptobj, $quba) = $this->create_hwork_with_questions(true, false);
        ...
        $result = mod_hwork_external::view_attempt($attempt->id, 0);
        $result = external_api::clean_returnvalue(mod_hwork_external::view_attempt_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        // Now, force the hwork with QUIZ_NAVMETHOD_SEQ (sequencial) navigation method.
        $DB->set_field('hwork', 'navmethod', QUIZ_NAVMETHOD_SEQ, array('id' => $hwork->id));
        // Quiz requiring preflightdata.
        $DB->set_field('hwork', 'password', 'abcdef', array('id' => $hwork->id));
        $preflightdata = array(array("name" => "hworkpassword", "value" => 'abcdef'));

        // See next page.
        $result = mod_hwork_external::view_attempt($attempt->id, 1, $preflightdata);
        $result = external_api::clean_returnvalue(mod_hwork_external::view_attempt_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        try {
            mod_hwork_external::view_attempt($attempt->id, 0);
    ...
    public function test_view_attempt_summary() {
        ...
        $result = mod_hwork_external::view_attempt_summary($attempt->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::view_attempt_summary_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        $this->assertEquals($context, $event->get_context());
        $moodlehwork = new \moodle_url('/mod/hwork/summary.php', array('attempt' => $attempt->id));
        $this->assertEquals($moodlehwork, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());
        ...
        $result = mod_hwork_external::view_attempt_summary($attempt->id, $preflightdata);
        $result = external_api::clean_returnvalue(mod_hwork_external::view_attempt_summary_returns(), $result);
        $this->assertTrue($result['status']);
    ...
    public function test_view_attempt_review() {
        ...
        $result = mod_hwork_external::view_attempt_review($attempt->id, 0);
        $result = external_api::clean_returnvalue(mod_hwork_external::view_attempt_review_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        $this->assertEquals($context, $event->get_context());
        $moodlehwork = new \moodle_url('/mod/hwork/review.php', array('attempt' => $attempt->id));
        $this->assertEquals($moodlehwork, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());
    ...
    /**
     * Test get_hwork_feedback_for_grade
     */
    public function test_get_hwork_feedback_for_grade() {
        global $DB;

        // Add feedback to the hwork.
        $feedback = new stdClass();
        $feedback->hworkid = $this->hwork->id;
        $feedback->feedbacktext = 'Feedback text 1';
        $feedback->feedbacktextformat = 1;
        $feedback->mingrade = 49;
        $feedback->maxgrade = 100;
        ...
        $result = mod_hwork_external::get_hwork_feedback_for_grade($this->hwork->id, 50);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_hwork_feedback_for_grade_returns(), $result);
        ...
        $result = mod_hwork_external::get_hwork_feedback_for_grade($this->hwork->id, 30);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_hwork_feedback_for_grade_returns(), $result);
        $this->assertEquals('Feedback text 2', $result['feedbacktext']);
        $this->assertEquals(FORMAT_HTML, $result['feedbacktextformat']);

        $result = mod_hwork_external::get_hwork_feedback_for_grade($this->hwork->id, 10);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_hwork_feedback_for_grade_returns(), $result);
    ...
    /**
     * Test get_hwork_access_information
     */
    public function test_get_hwork_access_information() {
        global $DB;
        ...
        $result = mod_hwork_external::get_hwork_access_information($hwork->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_hwork_access_information_returns(), $result);
        ...
        $expected = array(
            ...
            'activerulenames' => ['hworkaccess_openclosedate'],
        ...
        $hwork->timeopen = time() + DAYSECS;
        $hwork->timeclose = time() + WEEKSECS;
        $hwork->password = '123456';
        $DB->update_record('hwork', $hwork);
    ...
    public function test_get_attempt_access_information() {
        ...
        // Default restrictions (none).
        $result = mod_hwork_external::get_attempt_access_information($hwork->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_attempt_access_information_returns(), $result);
        ...
        $result = mod_hwork_external::get_attempt_access_information($hwork->id, $attempt->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_attempt_access_information_returns(), $result);
        ...
        $this->assertEquals(get_string('nomoreattempts', 'hwork'), $result['preventnewattemptreasons'][0]);

    }

    /**
     * Test get_hwork_required_qtypes
     */
    public function test_get_hwork_required_qtypes() {
        $this->setAdminUser();
        ...
        $result = mod_hwork_external::get_hwork_required_qtypes($hwork->id);
        $result = external_api::clean_returnvalue(mod_hwork_external::get_hwork_required_qtypes_returns(), $result);
