 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_hwork_structure_testcase extends advanced_testcase {

    /**
     * Create a course with an empty hwork.
     * @return array with three elements hwork, cm and course.
     */
    protected function prepare_hwork_data() {
     ...
     * @return hwork the created hwork.
     */
    protected function create_test_hwork($layout) {
        list($hwork, $cm, $course) = $this->prepare_hwork_data();
        ...
        foreach ($layout as $item) {
            if (is_string($item)) {
                ...
                hwork_add_hwork_question($q->id, $hwork, $page);
                $lastpage = $page;
            }
        }

        $hworkobj = new hwork($hwork, $cm, $course);
        $structure = \mod_hwork\structure::create_for_hwork($hworkobj);
        if (isset($headings[1])) {
     ...
     * @param array $expectedlayout as for $layout in {@link create_test_hwork()}.
     * @param \mod_hwork\structure $structure the structure to test.
     */
    protected function assert_hwork_layout($expectedlayout, \mod_hwork\structure $structure) {
        $sections = $structure->get_sections();
        ...
        foreach ($expectedlayout as $item) {
            if (is_string($item)) {
                ...
                if ($slot > 1 && $section->heading == '' && $section->firstslot == 1) {
                    // The array $expectedlayout did not contain default first hwork section, so skip over it.
                    $section = array_shift($sections);
                }
        ...
        if ($slot - 1 != count($structure->get_slots())) {
            $this->fail('The hwork contains more slots than expected.');
        }

        if (!empty($sections)) {
            $section = array_shift($sections);
            if ($section->heading != '' || $section->firstslot != 1) {
                $this->fail('Unexpected section (' . $section->heading .') found in the hwork.');
            }
        }
    }
     ...
     * used by create_test_hwork as assert_hwork_layout.
    ...
    public function test_get_hwork_slots() {
        $hworkobj = $this->create_test_hwork(array(
                array('TF1', 1, 'truefalse'),
    ...
    public function test_hwork_has_one_section_by_default() {
        $hworkobj = $this->create_test_hwork(array(
                array('TF1', 1, 'truefalse'),
            ));
        $structure = \mod_hwork\structure::create_for_hwork($hworkobj);
    ...
    public function test_remove_section_heading() {
        ...
        $structure = \mod_hwork\structure::create_for_hwork($hworkobj);
        $this->assert_hwork_layout(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
                array('TF2', 2, 'truefalse'),
            ), $structure);
    ...
    public function test_hwork_remove_slot() {
        $hworkobj = $this->create_test_hwork(array(
                array('TF1', 1, 'truefalse'),
                array('TF2', 1, 'truefalse'),
                'Heading 2',
    ...
    public function test_hwork_removing_a_random_question_deletes_the_question() {
        global $DB;
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category();
        hwork_add_random_questions($hworkobj->get_hwork(), 1, $cat->id, 1, false);
        $structure = \mod_hwork\structure::create_for_hwork($hworkobj);
    ...
    public function test_can_remove_last_question_in_a_hwork() {
        $hworkobj = $this->create_test_hwork(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
            ));
        ...
        hwork_add_hwork_question($q->id, $hworkobj->get_hwork(), 0);
        $structure = \mod_hwork\structure::create_for_hwork($hworkobj);
    ...
    public function test_add_question_updates_headings() {
        ...
        hwork_add_hwork_question($q->id, $hworkobj->get_hwork(), 1);
    ...
    public function test_remove_page_break() {
        ...
        $slots = $structure->update_page_break($slotid, \mod_hwork\repaginate::LINK);
    ...
    public function test_add_page_break() {
        ...
        $slotid = $structure->get_question_in_slot(2)->slotid;
        $slots = $structure->update_page_break($slotid, \mod_hwork\repaginate::UNLINK);
     ...
     * @param  array $layout Quiz layout for create_test_hwork function
    ...
    public function test_get_slot_tags_for_slot($layout, $tagnames, $slotnumber, $expected) {
        ...
        $hwork = $this->create_test_hwork($layout);
        $structure = \mod_hwork\structure::create_for_hwork($hwork);
        ...
        $DB->insert_records('hwork_slot_tags', $slottagrecords);
