...
/**
 * Unit tests for the {@link \mod_hwork\repaginate} class.
 * @package   mod_hwork
 * @category  test
...
require_once($CFG->dirroot . '/mod/hwork/classes/repaginate.php');


/**
 * Testable subclass, giving access to the protected methods of {@link \mod_hwork\repaginate}
 * @copyright 2014 The Open Univsersity
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_hwork_repaginate_testable extends \mod_hwork\repaginate {

    public function __construct($hworkid = 0, $slots = null) {
        return parent::__construct($hworkid, $slots);
    }
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_hwork_repaginate_test extends advanced_testcase {

    /** @var array stores the slots. */
    private $hworkslots;
    /** @var mod_hwork_repaginate_testable the object being tested. */
    private $repaginate = null;

    public function setUp() {
        $this->set_hwork_slots($this->get_hwork_object()->get_slots());
        $this->repaginate = new mod_hwork_repaginate_testable(0, $this->hworkslots);
    }
    ...
    /**
     * Create a hwork, add five questions to the hwork
     * which are all on one page and return the hwork object.
     */
    private function get_hwork_object() {
        global $SITE;
        ...
        $hwork = $hworkgenerator->create_instance(array(
                'course' => $SITE->id, 'questionsperpage' => 0, 'grade' => 100.0, 'sumgrades' => 2));
        $cm = get_coursemodule_from_instance('hwork', $hwork->id, $SITE->id);
        ...
        hwork_add_hwork_question($shortanswer->id, $hwork);
        hwork_add_hwork_question($numerical->id, $hwork);
        hwork_add_hwork_question($essay->id, $hwork);
        hwork_add_hwork_question($truefalse->id, $hwork);
        hwork_add_hwork_question($match->id, $hwork);

        // Return the hwork object.
        $hworkobj = new hwork($hwork, $cm, $SITE);
        return \mod_hwork\structure::create_for_hwork($hworkobj);
    }
     ...
     * @param string $slots
     */
    private function set_hwork_slots($slots = null) {
        if (!$slots) {
            $this->hworkslots = $this->get_hwork_object()->get_slots();
        } else {
            $this->hworkslots = $slots;
        }
    }
    ...
    public function test_get_this_slot() {
        $this->set_hwork_slots();
        $actual = array();
        $expected = $this->repaginate->get_slots_by_slot_number();
        $this->assertEquals($expected, $actual);

        $slotsbyno = $this->repaginate->get_slots_by_slot_number($this->hworkslots);
        $slotnumber = 5;
        $thisslot = $this->repaginate->get_this_slot($this->hworkslots, $slotnumber);
    ...
    public function test_get_slots_by_slotnumber() {
        ...
        foreach ($this->hworkslots as $slot) {
            $expected[$slot->slot] = $slot;
        }
        $actual = $this->repaginate->get_slots_by_slot_number($this->hworkslots);
        $this->assertEquals($expected, $actual);
    }
    ...
    public function test_get_slots_by_slotid() {
        ...
        $this->assertEquals($this->hworkslots, $actual);
    }
    ...
    public function test_repaginate_n_questions_per_page() {
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->hworkslots, 2);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->hworkslots, 3);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->hworkslots, 5);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->hworkslots, 10);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->hworkslots, 1);
        $this->assertEquals($expected, $actual);
    }

    public function test_repaginate_this_slot() {
        $this->set_hwork_slots();
        $slotsbyslotno = $this->repaginate->get_slots_by_slot_number($this->hworkslots);
        $slotnumber = 3;
        $newpagenumber = 2;
    ...
    public function test_repaginate_the_rest() {
        $this->set_hwork_slots();
        $slotfrom = 1;
        $type = \mod_hwork\repaginate::LINK;
        $expected = array();
        foreach ($this->hworkslots as $slot) {
        ...
        $actual = $this->repaginate->repaginate_the_rest($this->hworkslots, $slotfrom, $type, false);
        $this->assertEquals($expected, $actual);

        $slotfrom = 2;
        $newslots = array();
        foreach ($this->hworkslots as $s) {
            if ($s->slot === $slotfrom) {
                $s->page = $s->page - 1;
            }
            $newslots[$s->id] = $s;
        ...
        $type = \mod_hwork\repaginate::UNLINK;
        $expected = array();
        foreach ($this->hworkslots as $slot) {
