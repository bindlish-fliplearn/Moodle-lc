...
// Get the hwork slot.
$slot = $DB->get_record('hwork_slots', array('id' => $slotid));
if (!$slot || empty($slot->questioncategoryid)) {
    print_error('invalidrandomslot', 'mod_hwork');
}

if (!$hwork = $DB->get_record('hwork', array('id' => $slot->hworkid))) {
    print_error('invalidhworkid', 'hwork');
}

$cm = get_coursemodule_from_instance('hwork', $slot->hworkid, $hwork->course);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/hwork/edit.php', array('cmid' => $cm->id));
}

$url = new moodle_url('/mod/hwork/editrandom.php', array('slotid' => $slotid));
$PAGE->set_url($url);
...
$mform = new mod_hwork\form\randomquestion_form(new moodle_url('/mod/hwork/editrandom.php'),
...
$currentslottags = hwork_retrieve_slot_tags($slot->id);
...
if ($mform->is_cancelled()) {
    ...
    // We need to save some data into the hwork_slots table.
    ...
    foreach ($tags as $tag) {
        if ($key = array_search(['tagid' => $tag->id, 'tagname' => $tag->name], $searchableslottags)) {
            // If found, $key would be the id field in the hwork_slot_tags table.
    ...
    if (!empty($recordstokeep)) {
        ...
        $DB->delete_records_select('hwork_slot_tags', "id $select", $params);
    } else {
...
echo $OUTPUT->header();
$heading = get_string('randomediting', 'mod_hwork');
echo $OUTPUT->heading_with_help($heading, 'randomquestion', 'mod_hwork');
