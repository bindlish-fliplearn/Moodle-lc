...
/**
 * Implementaton of the hworkaccess_timelimit plugin.
 *
 * @package    hworkaccess
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class hworkaccess_timelimit extends hwork_access_rule_base {

    public static function make(hwork $hworkobj, $timenow, $canignoretimelimits) {

        if (empty($hworkobj->get_hwork()->timelimit) || $canignoretimelimits) {
            return null;
        }
    ...
    public function description() {
        return get_string('hworktimelimit', 'hworkaccess_timelimit',
                format_time($this->hwork->timelimit));
    }

    public function end_time($attempt) {
        return $attempt->timestart + $this->hwork->timelimit;
    }
    ...
    public function add_preflight_check_form_fields(mod_hwork_preflight_check_form $hworkform,
        ...
        $mform->addElement('header', 'honestycheckheader',
                get_string('confirmstartheader', 'hworkaccess_timelimit'));
        $mform->addElement('static', 'honestycheckmessage', '',
                get_string('confirmstart', 'hworkaccess_timelimit', format_time($this->hwork->timelimit)));
    }
}
