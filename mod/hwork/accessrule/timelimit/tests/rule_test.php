...
/**
 * Unit tests for the hworkaccess_timelimit plugin.
 *
 * @package    hworkaccess
...
global $CFG;
require_once($CFG->dirroot . '/mod/hwork/accessrule/timelimit/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class hworkaccess_timelimit_testcase extends basic_testcase {
    public function test_time_limit_access_rule() {
        $hwork = new stdClass();
        $hwork->timelimit = 3600;
        $cm = new stdClass();
        $cm->id = 0;
        $hworkobj = new hwork($hwork, $cm, null);
        $rule = new hworkaccess_timelimit($hworkobj, 10000);
        $attempt = new stdClass();

        $this->assertEquals($rule->description(),
            get_string('hworktimelimit', 'hworkaccess_timelimit', format_time(3600)));
