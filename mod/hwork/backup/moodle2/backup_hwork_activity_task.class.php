...
/**
 * Defines backup_hwork_activity_task class
 *
 * @package     mod_hwork
 * @category    backup
...
require_once($CFG->dirroot . '/mod/hwork/backup/moodle2/backup_hwork_stepslib.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_hwork_activity_task extends backup_activity_task {
    ...
    protected function define_my_steps() {
        // Generate the hwork.xml file containing all the hwork information
        // and annotating used questions.
        $this->add_step(new backup_hwork_activity_structure_step('hwork_structure', 'hwork.xml'));

        // Note: Following  steps must be present
        // in all the activities using question banks (only hwork for now)
    ...
    public static function encode_content_links($content) {
        ...
        // Link to the list of hworkzes.
        $search="/(".$base."\/mod\/hwork\/index.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@QUIZINDEX*$2@$', $content);

        // Link to hwork view by moduleid.
        $search="/(".$base."\/mod\/hwork\/view.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@QUIZVIEWBYID*$2@$', $content);

        // Link to hwork view by hworkid.
        $search="/(".$base."\/mod\/hwork\/view.php\?q\=)([0-9]+)/";
