...
/**
 * Define all the backup steps that will be used by the backup_hwork_activity_task
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_hwork_activity_structure_step extends backup_questions_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $hwork = new backup_nested_element('hwork', array('id'), array(
        ...
        $this->add_subplugin_structure('hworkaccess', $hwork, true);
        ...
        $this->add_subplugin_structure('hworkaccess', $attempt, true);

        // Build the tree.
        $hwork->add_child($qinstances);
        $qinstances->add_child($qinstance);

        $qinstance->add_child($qinstancetags);
        $qinstancetags->add_child($qinstancetag);

        $hwork->add_child($sections);
        $sections->add_child($section);

        $hwork->add_child($feedbacks);
        $feedbacks->add_child($feedback);

        $hwork->add_child($overrides);
        $overrides->add_child($override);

        $hwork->add_child($grades);
        $grades->add_child($grade);

        $hwork->add_child($attempts);
        $attempts->add_child($attempt);

        // Define sources.
        $hwork->set_source_table('hwork', array('id' => backup::VAR_ACTIVITYID));

        $qinstance->set_source_table('hwork_slots',
                array('hworkid' => backup::VAR_PARENTID));

        $qinstancetag->set_source_table('hwork_slot_tags',
                array('slotid' => backup::VAR_PARENTID));

        $section->set_source_table('hwork_sections',
                array('hworkid' => backup::VAR_PARENTID));

        $feedback->set_source_table('hwork_feedback',
                array('hworkid' => backup::VAR_PARENTID));

        // Quiz overrides to backup are different depending of user info.
        $overrideparams = array('hwork' => backup::VAR_PARENTID);
        if (!$userinfo) { //  Without userinfo, skip user overrides.
            $overrideparams['userid'] = backup_helper::is_sqlparam(null);

        }
        $override->set_source_table('hwork_overrides', $overrideparams);

        // All the rest of elements only happen if we are including user info.
        if ($userinfo) {
            $grade->set_source_table('hwork_grades', array('hwork' => backup::VAR_PARENTID));
            $attempt->set_source_sql('
                    SELECT *
                    FROM {hwork_attempts}
                    WHERE hwork = :hwork AND preview = 0',
                    array('hwork' => backup::VAR_PARENTID));
        }

        // Define source alias.
        $hwork->set_source_alias('attempts', 'attempts_number');
        ...
        $hwork->annotate_files('mod_hwork', 'intro', null); // This file area hasn't itemid.
        $feedback->annotate_files('mod_hwork', 'feedback', 'id');

        // Return the root element (hwork), wrapped into standard activity structure.
        return $this->prepare_activity_structure($hwork);
    }
}
