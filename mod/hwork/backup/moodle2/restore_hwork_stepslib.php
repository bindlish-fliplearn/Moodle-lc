...
/**
 * Structure step to restore one hwork activity
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_hwork_activity_structure_step extends restore_questions_activity_structure_step {

    /**
     * @var bool tracks whether the hwork contains at least one section. Before
     * Moodle 2.9 hwork sections did not exist, so if the file being restored
     * did not contain any, we need to create one in {@link after_execute()}.
     */
    protected $sectioncreated = false;

    /**
     * @var bool when restoring old hworkzes (2.8 or before) this records the
     * shufflequestionsoption hwork option which has moved to the hwork_sections table.
     */
    ...
    protected function define_structure() {
        ...
        $hwork = new restore_path_element('hwork', '/activity/hwork');
        $paths[] = $hwork;

        // A chance for access subplugings to set up their hwork data.
        $this->add_subplugin_structure('hworkaccess', $hwork);

        $paths[] = new restore_path_element('hwork_question_instance',
                '/activity/hwork/question_instances/question_instance');
        $paths[] = new restore_path_element('hwork_slot_tags',
                '/activity/hwork/question_instances/question_instance/tags/tag');
        $paths[] = new restore_path_element('hwork_section', '/activity/hwork/sections/section');
        $paths[] = new restore_path_element('hwork_feedback', '/activity/hwork/feedbacks/feedback');
        $paths[] = new restore_path_element('hwork_override', '/activity/hwork/overrides/override');

        if ($userinfo) {
            $paths[] = new restore_path_element('hwork_grade', '/activity/hwork/grades/grade');

            if ($this->task->get_old_moduleversion() > 2011010100) {
                // Restoring from a version 2.1 dev or later.
                // Process the new-style attempt data.
                $hworkattempt = new restore_path_element('hwork_attempt',
                        '/activity/hwork/attempts/attempt');
                $paths[] = $hworkattempt;

                // Add states and sessions.
                $this->add_question_usages($hworkattempt, $paths);

                // A chance for access subplugings to set up their attempt data.
                $this->add_subplugin_structure('hworkaccess', $hworkattempt);

            } else {
                // Restoring from a version 2.0.x+ or earlier.
                // Upgrade the legacy attempt data.
                $hworkattempt = new restore_path_element('hwork_attempt_legacy',
                        '/activity/hwork/attempts/attempt',
                        true);
                $paths[] = $hworkattempt;
                $this->add_legacy_question_attempt_data($hworkattempt, $paths);
            }
        }
    ...
    protected function process_hwork($data) {
        global $CFG, $DB;
        ...
        if (property_exists($data, 'questions')) {
            // Needed by {@link process_hwork_attempt_legacy}, in which case it will be present.
            $this->oldhworklayout = $data->questions;
        }

        // The setting hwork->attempts can come both in data->attempts and
        ...
        if (isset($data->review)) {
            ...
            $data->reviewattempt =
                    ...
                    ($oldreview & QUIZ_OLD_IMMEDIATELY & QUIZ_OLD_RESPONSES ?
                            mod_hwork_display_options::IMMEDIATELY_AFTER : 0) |
                    ($oldreview & QUIZ_OLD_OPEN & QUIZ_OLD_RESPONSES ?
                            mod_hwork_display_options::LATER_WHILE_OPEN : 0) |
                    ($oldreview & QUIZ_OLD_CLOSED & QUIZ_OLD_RESPONSES ?
                            mod_hwork_display_options::AFTER_CLOSE : 0);
            ...
            $data->reviewspecificfeedback =
                    ($oldreview & QUIZ_OLD_IMMEDIATELY & QUIZ_OLD_FEEDBACK ?
                            mod_hwork_display_options::DURING : 0) |
                    ($oldreview & QUIZ_OLD_IMMEDIATELY & QUIZ_OLD_FEEDBACK ?
                            mod_hwork_display_options::IMMEDIATELY_AFTER : 0) |
        ...
        if (!isset($data->overduehandling)) {
            $data->overduehandling = get_config('hwork', 'overduehandling');
        }

        // Old shufflequestions setting is now stored in hwork sections,
        ...
        // Insert the hwork record.
        $newitemid = $DB->insert_record('hwork', $data);
        // Immediately after inserting "activity" record, call this.
        $this->apply_activity_instance($newitemid);
    }

    protected function process_hwork_question_instance($data) {
        global $CFG, $DB;
        ...
        if (!property_exists($data, 'slot')) {
            $page = 1;
            $slot = 1;
            foreach (explode(',', $this->oldhworklayout) as $item) {
                if ($item == 0) {
                    $page += 1;
                    continue;
                }
        ...
        if (!property_exists($data, 'slot')) {
            ...
            // that was not acutally in the hwork. Drop it.
            $this->log('question ' . $data->questionid . ' was associated with hwork ' .
                    $this->get_new_parentid('hwork') . ' but not actually used. ' .
                    'The instance has been ignored.', backup::LOG_INFO);
            return;
        }

        $data->hworkid = $this->get_new_parentid('hwork');
        ...
        $newitemid = $DB->insert_record('hwork_slots', $data);
        // Add mapping, restore of slot tags (for random questions) need it.
        $this->set_mapping('hwork_question_instance', $oldid, $newitemid);
    }

    /**
     * Process a hwork_slot_tags restore
     *
     * @param stdClass|array $data The hwork_slot_tags data
     */
    protected function process_hwork_slot_tags($data) {
        global $DB;

        $data = (object)$data;

        $data->slotid = $this->get_new_parentid('hwork_question_instance');
        ...
        $DB->insert_record('hwork_slot_tags', $data);
    }

    protected function process_hwork_section($data) {
        global $DB;

        $data = (object) $data;
        $data->hworkid = $this->get_new_parentid('hwork');
        $newitemid = $DB->insert_record('hwork_sections', $data);
        $this->sectioncreated = true;
    }

    protected function process_hwork_feedback($data) {
        global $DB;
        ...
        $newitemid = $DB->insert_record('hwork_feedback', $data);
        $this->set_mapping('hwork_feedback', $oldid, $newitemid, true); // Has related files.
    }

    protected function process_hwork_override($data) {
        global $DB;
        ...
        $data->hwork = $this->get_new_parentid('hwork');
        ...
        $newitemid = $DB->insert_record('hwork_overrides', $data);

        // Add mapping, restore of logs needs it.
        $this->set_mapping('hwork_override', $oldid, $newitemid);
    }

    protected function process_hwork_grade($data) {
        global $DB;
        ...
        $DB->insert_record('hwork_grades', $data);
    }

    protected function process_hwork_attempt($data) {
        $data = (object)$data;
        ...
        $this->currenthworkattempt = clone($data);
    }

    protected function process_hwork_attempt_legacy($data) {
        global $DB;

        $this->process_hwork_attempt($data);

        $hwork = $DB->get_record('hwork', array('id' => $this->get_new_parentid('hwork')));
        $hwork->oldquestions = $this->oldhworklayout;
        $this->process_legacy_hwork_attempt_data($data, $hwork);
    }

    protected function inform_new_usage_id($newusageid) {
        global $DB;

        $data = $this->currenthworkattempt;

        $oldid = $data->id;
        $data->uniqueid = $newusageid;

        $newitemid = $DB->insert_record('hwork_attempts', $data);

        // Save hwork_attempt->id mapping, because logs use it.
        $this->set_mapping('hwork_attempt', $oldid, $newitemid, false);
    }
    ...
    protected function after_execute() {
        ...
        // Add hwork related files, no need to match by itemname (just internally handled context).
        $this->add_related_files('mod_hwork', 'intro', null);
        // Add feedback related files, matching by itemname = 'hwork_feedback'.
        $this->add_related_files('mod_hwork', 'feedback', 'hwork_feedback');

        if (!$this->sectioncreated) {
            $DB->insert_record('hwork_sections', array(
                    'hworkid' => $this->get_new_parentid('hwork'),
                    'firstslot' => 1, 'heading' => '',
