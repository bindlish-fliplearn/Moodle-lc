...
$PAGE->set_url('/mod/hwork/comment.php', array('attempt' => $attemptid, 'slot' => $slot));
...
if (!$attemptobj->is_finished()) {
    print_error('attemptclosed', 'hwork');
}

// Check login and permissions.
require_login($attemptobj->get_course(), false, $attemptobj->get_cm());
$attemptobj->require_capability('mod/hwork:grade');

// Print the page header.
$PAGE->set_pagelayout('popup');
$PAGE->set_title(get_string('manualgradequestion', 'hwork', array(
        'question' => format_string($attemptobj->get_question_name($slot)),
        'hwork' => format_string($attemptobj->get_hwork_name()), 'user' => fullname($student))));
...
// Quiz name.
$summarydata['hworkname'] = array(
    'title'   => get_string('modulename', 'hwork'),
    'content' => format_string($attemptobj->get_hwork_name()),
);

// Question name.
$summarydata['questionname'] = array(
    'title'   => get_string('question', 'hwork'),
...
<?php
echo '</form>';
$PAGE->requires->js_init_call('M.mod_hwork.init_comment_popup', null, false, hwork_get_js_module());
