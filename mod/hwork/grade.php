...
$cm = get_coursemodule_from_id('hwork', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$hwork = $DB->get_record('hwork', array('id' => $cm->instance), '*', MUST_EXIST);
require_login($course, false, $cm);

$reportlist = hwork_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see hwork reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/hwork/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = hwork_get_user_attempts($hwork->id, $userid, 'finished');
    $attempt = null;
    ...
    if ($attempt) {
        $attemptobj = new hwork_attempt($attempt, $hwork, $cm, $course, false);
...
redirect(new moodle_url('/mod/hwork/report.php', array(
