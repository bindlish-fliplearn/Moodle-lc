...
/**
 * Base class for the options that control what is visible in an {@link pratest_attempts_report}.
 *
 * @package   mod_pratest
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_pratest_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the pratest being reported on. */
    public $pratest;

    /** @var object the course module objects for the pratest being reported on. */
    public $cm;

    /** @var object the course settings for the course the pratest is in. */
    public $course;

    /**
     * @var array form field name => corresponding pratest_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => pratest_attempt::IN_PROGRESS,
        'stateoverdue'    => pratest_attempt::OVERDUE,
        'statefinished'   => pratest_attempt::FINISHED,
        'stateabandoned'  => pratest_attempt::ABANDONED,
    );

    /**
     * @var string pratest_attempts_report::ALL_WITH or pratest_attempts_report::ENROLLED_WITH
     *      pratest_attempts_report::ENROLLED_WITHOUT or pratest_attempts_report::ENROLLED_ALL
     */
    public $attempts = pratest_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of pratest_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(pratest_attempt::IN_PROGRESS, pratest_attempt::OVERDUE,
            pratest_attempt::FINISHED, pratest_attempt::ABANDONED);
    ...
    public $pagesize = pratest_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $pratest the settings for the pratest being reported on.
     * @param object $cm the course module objects for the pratest being reported on.
     * @param object $coures the course settings for the coures this pratest is in.
     */
    public function __construct($mode, $pratest, $cm, $course) {
        $this->mode   = $mode;
        $this->pratest   = $pratest;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = pratest_report_should_show_grades($pratest, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/pratest/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('pratest_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('pratest_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == pratest_attempts_report::ALL_WITH) {
                $this->attempts = pratest_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = pratest_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(pratest_attempts_report::ALL_WITH, pratest_attempts_report::ENROLLED_WITH,
                pratest_attempts_report::ENROLLED_WITHOUT, pratest_attempts_report::ENROLLED_ALL))) {
            $this->attempts = pratest_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!pratest_report_can_filter_only_graded($this->pratest)) {
        ...
        if ($this->attempts == pratest_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = pratest_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(pratest_attempt::FINISHED, $this->states);
    }
}
