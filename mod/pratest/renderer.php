...
/**
 * Defines the renderer for the pratest module.
 *
 * @package   mod_pratest
...
/**
 * The renderer for the pratest module.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_pratest_renderer extends plugin_renderer_base {
    /**
     * Builds the review page
     *
     * @param pratest_attempt $attemptobj an instance of pratest_attempt.
     ...
     * @param mod_pratest_display_options $displayoptions instance of mod_pratest_display_options.
     * @param array $summarydata contains all table data
     * @return $output containing html data.
     */
    public function review_page(pratest_attempt $attemptobj, $slots, $page, $showall,
                                $lastpage, mod_pratest_display_options $displayoptions,
                                $summarydata) {
     ...
     * @return $output containing html data.
     */
    public function review_question_page(pratest_attempt $attemptobj, $slot, $seq,
            mod_pratest_display_options $displayoptions, $summarydata) {
     ...
     */
    public function review_question_not_allowed(pratest_attempt $attemptobj, $message) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_pratest_name(), true,
                                  array("context" => $attemptobj->get_pratestobj()->get_context())));
    ...
    public function review_summary_table($summarydata, $page) {
        ...
        $output .= html_writer::start_tag('table', array(
                'class' => 'generaltable generalbox pratestreviewsummary'));
     ...
     *
     * @param pratest_attempt $attemptobj instance of pratest_attempt
     ...
     * @param mod_pratest_display_options $displayoptions instance of mod_pratest_display_options
     */
    public function questions(pratest_attempt $attemptobj, $reviewing, $slots, $page, $showall,
                              mod_pratest_display_options $displayoptions) {
        $output = '';
    ...
    public function review_form($page, $showall, $displayoptions, $content, $attemptobj) {
        ...
        $this->page->requires->js_init_call('M.mod_pratest.init_review_form', null, false,
                pratest_get_js_module());
     ...
     * @param pratest_attempt $attemptobj instance of pratest_attempt
     */
    public function finish_review_link(pratest_attempt $attemptobj) {
        $url = $attemptobj->view_url();

        if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            $this->page->requires->js_init_call('M.mod_pratest.secure_window.init_close_button',
                    array($url), false, pratest_get_js_module());
            return html_writer::empty_tag('input', array('type' => 'button',
                    'value' => get_string('finishreview', 'pratest'),
                    'id' => 'secureclosebutton',
                    'class' => 'mod_pratest-next-nav btn btn-primary'));

        } else {
            return html_writer::link($url, get_string('finishreview', 'pratest'),
                    array('class' => 'mod_pratest-next-nav'));
        }
    }
     ...
     */
    public function review_next_navigation(pratest_attempt $attemptobj, $page, $lastpage, $showall = null) {
        $nav = '';
        if ($page > 0) {
            $nav .= link_arrow_left(get_string('navigateprevious', 'pratest'),
                    $attemptobj->review_url(null, $page - 1, $showall), false, 'mod_pratest-prev-nav');
        }
        if ($lastpage) {
            $nav .= $this->finish_review_link($attemptobj);
        } else {
            $nav .= link_arrow_right(get_string('navigatenext', 'pratest'),
                    $attemptobj->review_url(null, $page + 1, $showall), false, 'mod_pratest-next-nav');
        }
        return html_writer::tag('div', $nav, array('class' => 'submitbtns'));
    }

    /**
     * Return the HTML of the pratest timer.
     * @return string HTML content.
     */
    public function countdown_timer(pratest_attempt $attemptobj, $timenow) {
        ...
        if ($timeleft !== false) {
            ...
            if (!$ispreview) {
                ...
                // this will just have the effect of causing the pratest to be submitted immediately.
                $timerstartvalue = max($timerstartvalue, 1);
            }
            $this->initialise_timer($timerstartvalue, $ispreview);
        }

        return html_writer::tag('div', get_string('timeleft', 'pratest') . ' ' .
                html_writer::tag('span', '', array('id' => 'pratest-time-left')),
                array('id' => 'pratest-timer', 'role' => 'timer',
    ...
    public function restart_preview_button($url) {
        return $this->single_button($url, get_string('startnewpreview', 'pratest'));
    }

    /**
     * Outputs the navigation block panel
     *
     * @param pratest_nav_panel_base $panel instance of pratest_nav_panel_base
     */
    public function navigation_panel(pratest_nav_panel_base $panel) {
        ...
        $this->page->requires->js_init_call('M.mod_pratest.nav.init', null, false,
                pratest_get_js_module());
    ...
    /**
     * Display a pratest navigation button.
     *
     * @param pratest_nav_question_button $button
     * @return string HTML fragment.
     */
    protected function render_pratest_nav_question_button(pratest_nav_question_button $button) {
        $classes = array('qnbutton', $button->stateclass, $button->navmethod, 'btn', 'btn-secondary');
        $extrainfo = array();

        if ($button->currentpage) {
            $classes[] = 'thispage';
            $extrainfo[] = get_string('onthispage', 'pratest');
        }
        ...
        $tagcontents = html_writer::tag('span', '', array('class' => 'thispageholder')) .
                        ...
                        get_string($qnostring, 'pratest', $a);
        $tagattributes = array('class' => implode(' ', $classes), 'id' => $button->id,
                                  'title' => $button->statestring, 'data-pratest-page' => $button->page);
    ...
    /**
     * Display a pratest navigation heading.
     *
     * @param pratest_nav_section_heading $heading the heading.
     * @return string HTML fragment.
     */
    protected function render_pratest_nav_section_heading(pratest_nav_section_heading $heading) {
        return $this->heading($heading->heading, 3, 'mod_pratest-section-heading');
    }

    /**
     * outputs the link the other attempts.
     *
     * @param mod_pratest_links_to_other_attempts $links
     */
    protected function render_mod_pratest_links_to_other_attempts(
            mod_pratest_links_to_other_attempts $links) {
        $attemptlinks = array();
    ...
    public function start_attempt_page(pratest $pratestobj, mod_pratest_preflight_check_form $mform) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($pratestobj->get_pratest_name(), true,
                                  array("context" => $pratestobj->get_context())));
        $output .= $this->pratest_intro($pratestobj->get_pratest(), $pratestobj->get_cm());
        $output .= $mform->render();
        $output .= $this->footer();
        return $output;
    }
     ...
     * @param pratest_attempt $attemptobj Instance of pratest_attempt
     * @param int $page Current page number
     * @param pratest_access_manager $accessmanager Instance of pratest_access_manager
    ...
    public function attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id,
        ...
        $output = '';
        $output .= $this->header();
        $output .= $this->pratest_notices($messages);
     ...
     */
    public function pratest_notices($messages) {
        if (!$messages) {
            return '';
        }
        return $this->box($this->heading(get_string('accessnoticesheader', 'pratest'), 3) .
                $this->access_messages($messages), 'pratestaccessnotices');
    }

    /**
     * Ouputs the form for making an attempt
     *
     * @param pratest_attempt $attemptobj
     * @param int $page Current page number
    ...
    public function attempt_form($attemptobj, $page, $slots, $id, $nextpage) {
        ...
        $navmethod = $attemptobj->get_pratest()->navmethod;
     ...
     * @param bool $lastpage is this the last page in the pratest?
     * @param string $navmethod Optional pratest attribute, 'free' (default) or 'sequential'
     * @return string HTML fragment.
     */
    protected function attempt_navigation_buttons($page, $lastpage, $navmethod = 'free') {
        ...
        if ($page > 0 && $navmethod == 'free') {
            $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'previous',
                    'value' => get_string('navigateprevious', 'pratest'), 'class' => 'mod_pratest-prev-nav btn btn-secondary'));
        }
        if ($lastpage) {
            $nextlabel = get_string('endtest', 'pratest');
        } else {
            $nextlabel = get_string('navigatenext', 'pratest');
        }
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'next',
                'value' => $nextlabel, 'class' => 'mod_pratest-next-nav btn btn-primary'));
        $output .= html_writer::end_tag('div');
    ...
    public function redo_question_button($slot, $disabled) {
        $attributes = array('type' => 'submit',  'name' => 'redoslot' . $slot,
                'value' => get_string('redoquestion', 'pratest'), 'class' => 'mod_pratest-redo_question_button');
        if ($disabled) {
    ...
    public function initialise_timer($timerstartvalue, $ispreview) {
        ...
        $this->page->requires->js_init_call('M.mod_pratest.timer.init', $options, false, pratest_get_js_module());
    }
    ...
    public function close_attempt_popup($url, $message = '') {
        ...
        if ($message) {
            ...
            $output .= html_writer::tag('p', get_string('windowclosing', 'pratest'));
            $delay = 5;
        } else {
            $output .= html_writer::tag('p', get_string('pleaseclose', 'pratest'));
            $delay = 0;
        }
        $this->page->requires->js_init_call('M.mod_pratest.secure_window.close',
                array($url, $delay), false, pratest_get_js_module());
     ...
     *
     * @param pratest_attempt $attemptobj
     * @param mod_pratest_display_options $displayoptions
     */
    public function summary_page($attemptobj, $displayoptions) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_pratest_name()));
        $output .= $this->heading(get_string('summaryofattempt', 'pratest'), 3);
    ...
    public function summary_table($attemptobj, $displayoptions) {
        ...
        $table->attributes['class'] = 'generaltable pratestsummaryofattempt boxaligncenter';
        $table->head = array(get_string('question', 'pratest'), get_string('status', 'pratest'));
        $table->align = array('left', 'left');
        $table->size = array('', '');
        $markscolumn = $displayoptions->marks >= question_display_options::MARK_AND_MAX;
        if ($markscolumn) {
            $table->head[] = get_string('marks', 'pratest');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($slots as $slot) {
            ...
            if ($heading) {
                ...
                $table->rowclasses[] = 'pratestsummaryheading';
            }
            ...
            $table->data[] = $row;
            $table->rowclasses[] = 'pratestsummary' . $slot . ' ' . $attemptobj->get_question_state_class(
    ...
    public function summary_page_controls($attemptobj) {
        ...
        if ($attemptobj->get_state() == pratest_attempt::IN_PROGRESS) {
            $button = new single_button(
                    new moodle_url($attemptobj->attempt_url(null, $attemptobj->get_currentpage())),
                    get_string('returnattempt', 'pratest'));
        ...
        $button = new single_button(
                ...
                get_string('submitallandfinish', 'pratest'));
        $button->id = 'responseform';
        if ($attemptobj->get_state() == pratest_attempt::IN_PROGRESS) {
            $button->add_action(new confirm_action(get_string('confirmclose', 'pratest'), null,
                    get_string('submitallandfinish', 'pratest')));
        }

        $duedate = $attemptobj->get_due_date();
        $message = '';
        if ($attemptobj->get_state() == pratest_attempt::OVERDUE) {
            $message = get_string('overduemustbesubmittedby', 'pratest', userdate($duedate));

        } else if ($duedate) {
            $message = get_string('mustbesubmittedby', 'pratest', userdate($duedate));
        }
     ...
     * @param array $pratest Array conting pratest data
     * @param int $cm Course Module ID
     * @param int $context The page context ID
     * @param array $infomessages information about this pratest
     * @param mod_pratest_view_object $viewobj
     ...
     *      attempt this pratest now, if appicable this pratest
     */
    public function view_page($course, $pratest, $cm, $context, $viewobj) {
        $output = '';
        $output .= $this->view_information($pratest, $cm, $context, $viewobj->infomessages);
        $output .= $this->view_table($pratest, $context, $viewobj);
        $output .= $this->view_result_info($pratest, $context, $cm, $viewobj);
        $output .= $this->box($this->view_page_buttons($viewobj), 'pratestattempt');
        return $output;
    }
     ...
     * @param mod_pratest_view_object $viewobj the information required to display
     * the view page.
     * @return string HTML to output.
     */
    public function view_page_buttons(mod_pratest_view_object $viewobj) {
        global $CFG;
        $output = '';
        ...
        if ($viewobj->showbacktocourse) {
            $output .= $this->single_button($viewobj->backtocourseurl,
                    get_string('backtocourse', 'pratest'), 'get',
     ...
     * @param mod_pratest_preflight_check_form $preflightcheckform deprecated.
    ...
    public function start_attempt_button($buttontext, moodle_url $url,
            mod_pratest_preflight_check_form $preflightcheckform = null,
        ...
        if (is_string($preflightcheckform)) {
            ...
            debugging('The third argument to start_attempt_button should now be the ' .
                    'mod_pratest_preflight_check_form from ' .
                    'pratest_access_manager::get_preflight_check_form, not a warning message string.');
        }

        $button = new single_button($url, $buttontext);
        $button->class .= ' prateststartbuttondiv';
        if ($popuprequired) {
            $button->class .= ' pratestsecuremoderequired';
        }
        ...
        $this->page->requires->js_call_amd('mod_pratest/preflightcheck', 'init',
                array('.prateststartbuttondiv [type=submit]', get_string('startattempt', 'pratest'),
                       '#mod_pratest_preflight_form', $popupjsoptions));
    ...
    public function no_questions_message($canedit, $editurl) {
        $output = '';
        $output .= $this->notification(get_string('noquestions', 'pratest'));
        if ($canedit) {
            $output .= $this->single_button($editurl, get_string('editpratest', 'pratest'), 'get');
        }
    ...
    /**
     * Outputs an error message for any guests accessing the pratest
     *
     * @param int $course The course ID
     * @param array $pratest Array contingin pratest data
     * @param int $cm Course Module ID
     * @param int $context The page contect ID
     * @param array $messages Array containing any messages
     */
    public function view_page_guest($course, $pratest, $cm, $context, $messages) {
        $output = '';
        $output .= $this->view_information($pratest, $cm, $context, $messages);
        $guestno = html_writer::tag('p', get_string('guestsno', 'pratest'));
     ...
     * @param array $messages Array containing any messages
     */
    public function view_page_notenrolled($course, $pratest, $cm, $context, $messages) {
        global $CFG;
        $output = '';
        $output .= $this->view_information($pratest, $cm, $context, $messages);
        $youneedtoenrol = html_writer::tag('p', get_string('youneedtoenrol', 'pratest'));
     ...
     * @return string HTML to output.
     */
    public function view_information($pratest, $cm, $context, $messages) {
        global $CFG;

        $output = '';

        // Print pratest name and description.
        $output .= $this->heading(format_string($pratest->name));
        $output .= $this->pratest_intro($pratest, $cm);

        // Output any access messages.
        if ($messages) {
            $output .= $this->box($this->access_messages($messages), 'pratestinfo');
        }

        // Show number of attempts summary to those who can view reports.
        if (has_capability('mod/pratest:viewreports', $context)) {
            if ($strattemptnum = $this->pratest_attempt_summary_link_to_reports($pratest, $cm,
                    $context)) {
                $output .= html_writer::tag('div', $strattemptnum,
                        array('class' => 'pratestattemptcounts'));
            }
        }
        return $output;
    }
    ...
    /**
     * Output the pratest intro.
     * @param object $pratest the pratest settings.
     * @param object $cm the course_module object.
     * @return string HTML to output.
     */
    public function pratest_intro($pratest, $cm) {
        if (html_is_blank($pratest->intro)) {
            return '';
        }

        return $this->box(format_module_intro('pratest', $pratest, $cm->id), 'generalbox', 'intro');
    }
    ...
    public function view_table_heading() {
        return $this->heading(get_string('summaryofattempts', 'pratest'), 3);
    }

    /**
     * Generates the table of data
     *
     * @param array $pratest Array contining pratest data
     * @param int $context The page context ID
     * @param mod_pratest_view_object $viewobj
     */
    public function view_table($pratest, $context, $viewobj) {
        if (!$viewobj->attempts) {
            return '';
        }
        ...
        $table = new html_table();
        $table->attributes['class'] = 'generaltable pratestattemptsummary';
        $table->head = array();
        $table->align = array();
        $table->size = array();
        if ($viewobj->attemptcolumn) {
            $table->head[] = get_string('attemptnumber', 'pratest');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        $table->head[] = get_string('attemptstate', 'pratest');
        $table->align[] = 'left';
        $table->size[] = '';
        if ($viewobj->markcolumn) {
            $table->head[] = get_string('marks', 'pratest') . ' / ' .
                    pratest_format_grade($pratest, $pratest->sumgrades);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        ...
        if ($viewobj->canreviewmine) {
            $table->head[] = get_string('review', 'pratest');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->feedbackcolumn) {
            $table->head[] = get_string('feedback', 'pratest');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($viewobj->attemptobjs as $attemptobj) {
            ...
            if ($viewobj->attemptcolumn) {
                if ($attemptobj->is_preview()) {
                    $row[] = get_string('preview', 'pratest');
                } else {
            ...
            if ($viewobj->markcolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    $row[] = pratest_format_grade($pratest, $attemptobj->get_sum_marks());
                } else {
                    $row[] = '';
                }
            }
            ...
            $attemptgrade = pratest_rescale_grade($attemptobj->get_sum_marks(), $pratest, false);
            ...
            if ($viewobj->gradecolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    if ($viewobj->overallstats && !$attemptobj->is_preview()
                            ...
                            && $attemptobj->get_state() == pratest_attempt::FINISHED
                            && $attemptgrade == $viewobj->mygrade
                            && $pratest->grademethod == QUIZ_GRADEHIGHEST) {
                        $table->rowclasses[$attemptobj->get_attempt_number()] = 'bestrow';
                    }

                    $row[] = pratest_format_grade($pratest, $attemptgrade);
                } else {
                    $row[] = '';
                }
            }
            ...
            if ($viewobj->feedbackcolumn && $attemptobj->is_finished()) {
                if ($attemptoptions->overallfeedback) {
                    $row[] = pratest_feedback_for_grade($attemptgrade, $pratest, $context);
                } else {
                    $row[] = '';
                }
            }
     ...
     * @param pratest_attempt $attemptobj the attempt
    ...
    public function attempt_state($attemptobj) {
        switch ($attemptobj->get_state()) {
            ...
            case pratest_attempt::OVERDUE:
                return get_string('stateoverdue', 'pratest') . html_writer::tag('span',
                        get_string('stateoverduedetails', 'pratest',
                                userdate($attemptobj->get_due_date())),
                        array('class' => 'statedetails'));

            case pratest_attempt::FINISHED:
                return get_string('statefinished', 'pratest') . html_writer::tag('span',
                        get_string('statefinisheddetails', 'pratest',
    ...
    /**
     * Generates data pertaining to pratest results
     *
     * @param array $pratest Array containing pratest data
     * @param int $context The page context ID
     * @param int $cm The Course Module Id
     * @param mod_pratest_view_object $viewobj
     */
    public function view_result_info($pratest, $context, $cm, $viewobj) {
        $output = '';
        ...
        if ($viewobj->overallstats) {
            if ($viewobj->moreattempts) {
                $a = new stdClass();
                $a->method = pratest_get_grading_option_name($pratest->grademethod);
                $a->mygrade = pratest_format_grade($pratest, $viewobj->mygrade);
                $a->pratestgrade = pratest_format_grade($pratest, $pratest->grade);
                $resultinfo .= $this->heading(get_string('gradesofar', 'pratest', $a), 3);
            } else {
                $a = new stdClass();
                $a->grade = pratest_format_grade($pratest, $viewobj->mygrade);
                $a->maxgrade = pratest_format_grade($pratest, $pratest->grade);
                $a = get_string('outofshort', 'pratest', $a);
                $resultinfo .= $this->heading(get_string('yourfinalgradeis', 'pratest', $a), 3);
            }
        }
        ...
        if ($viewobj->gradebookfeedback) {
            $resultinfo .= $this->heading(get_string('comment', 'pratest'), 3);
            $resultinfo .= html_writer::div($viewobj->gradebookfeedback, 'pratestteacherfeedback') . "\n";
        }
        if ($viewobj->feedbackcolumn) {
            $resultinfo .= $this->heading(get_string('overallfeedback', 'pratest'), 3);
            $resultinfo .= html_writer::div(
                    pratest_feedback_for_grade($viewobj->mygrade, $pratest, $context),
                    'pratestgradefeedback') . "\n";
        }
    ...
    public function review_link($url, $reviewinpopup, $popupoptions) {
        if ($reviewinpopup) {
            $button = new single_button($url, get_string('review', 'pratest'));
            $button->add_action(new popup_action('click', $url, 'pratestpopup', $popupoptions));
            return $this->render($button);

        } else {
            return html_writer::link($url, get_string('review', 'pratest'),
                    array('title' => get_string('reviewthisattempt', 'pratest')));
        }
    }
     ...
     */
    public function pratest_attempt_summary_link_to_reports($pratest, $cm, $context,
    ...
    public function connection_warning() {
        ...
        $warning = format_text(get_string('connectionerror', 'pratest'), FORMAT_MARKDOWN, $options);
        $ok = format_text(get_string('connectionok', 'pratest'), FORMAT_MARKDOWN, $options);
...
class mod_pratest_links_to_other_attempts implements renderable {
    /**
...
class mod_pratest_view_object {
    /** @var array $infomessages of messages with information to display about the pratest. */
    public $infomessages;
    /** @var array $attempts contains all the user's attempts at this pratest. */
    public $attempts;
    /** @var array $attemptobjs pratest_attempt objects corresponding to $attempts. */
    public $attemptobjs;
    /** @var pratest_access_manager $accessmanager contains various access rules. */
    public $accessmanager;
    /** @var bool $canreviewmine whether the current user has the capability to
     *       review their own attempts. */
    public $canreviewmine;
    /** @var bool $canedit whether the current user has the capability to edit the pratest. */
    public $canedit;
    /** @var moodle_url $editurl the URL for editing this pratest. */
    public $editurl;
    ...
    public $numattempts;
    /** @var float $mygrade contains the user's final grade for a pratest. */
    public $mygrade;
    ...
    public $popupoptions;
    /** @var bool $pratesthasquestions whether the pratest has any questions. */
    public $pratesthasquestions;
