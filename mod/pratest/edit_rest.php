...
/**
 * Rest endpoint for ajax editing of pratest structure.
 *
 * @package   mod_pratest
...
$pratestid     = required_param('pratestid', PARAM_INT);
...
$PAGE->set_url('/mod/pratest/edit-rest.php',
        array('pratestid' => $pratestid, 'class' => $class));
...
switch($requestmethod) {
    ...
    case 'GET': // For debugging.
        switch ($class) {
            case 'section':
                $table = 'pratest_sections';
                $section = $structure->get_section_by_id($id);
                switch ($field) {
                    case 'getsectiontitle':
                        require_capability('mod/pratest:manage', $modcontext);
            ...
            case 'resource':
                switch ($field) {
                    ...
                    case 'getmaxmark':
                        require_capability('mod/pratest:manage', $modcontext);
                        $slot = $DB->get_record('pratest_slots', array('id' => $id), '*', MUST_EXIST);
                        $result = array('instancemaxmark' => pratest_format_question_grade($pratest, $slot->maxmark));
                        break;
                    ...
                    case 'updatemaxmark':
                        ...
                        $result = array('instancemaxmark' => pratest_format_question_grade($pratest, $maxmark),
                                'newsummarks' => pratest_format_grade($pratest, $pratest->sumgrades));
                        break;
                    ...
                    case 'deletemultiple':
                        ...
                        $ids = explode(',', $ids);
                        foreach ($ids as $id) {
                            $slot = $DB->get_record('pratest_slots', array('pratestid' => $pratest->id, 'id' => $id),
                                    '*', MUST_EXIST);
                            if (pratest_has_question_use($pratest, $slot->slot)) {
                        ...
                        $result = array('newsummarks' => pratest_format_grade($pratest, $pratest->sumgrades),
    ...
    case 'DELETE':
        switch ($class) {
            ...
            case 'resource':
                require_capability('mod/pratest:manage', $modcontext);
                if (!$slot = $DB->get_record('pratest_slots', array('pratestid' => $pratest->id, 'id' => $id))) {
