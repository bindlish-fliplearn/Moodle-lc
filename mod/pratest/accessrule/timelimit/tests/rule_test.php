...
/**
 * Unit tests for the pratestaccess_timelimit plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/pratest/accessrule/timelimit/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class pratestaccess_timelimit_testcase extends basic_testcase {
    public function test_time_limit_access_rule() {
        $pratest = new stdClass();
        $pratest->timelimit = 3600;
        $cm = new stdClass();
        $cm->id = 0;
        $pratestobj = new pratest($pratest, $cm, null);
        $rule = new pratestaccess_timelimit($pratestobj, 10000);
        $attempt = new stdClass();

        $this->assertEquals($rule->description(),
            get_string('pratesttimelimit', 'pratestaccess_timelimit', format_time(3600)));
