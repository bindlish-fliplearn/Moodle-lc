...
$PAGE->set_url('/mod/clatest/comment.php', array('attempt' => $attemptid, 'slot' => $slot));
...
if (!$attemptobj->is_finished()) {
    print_error('attemptclosed', 'clatest');
}

// Check login and permissions.
require_login($attemptobj->get_course(), false, $attemptobj->get_cm());
$attemptobj->require_capability('mod/clatest:grade');

// Print the page header.
$PAGE->set_pagelayout('popup');
$PAGE->set_title(get_string('manualgradequestion', 'clatest', array(
        'question' => format_string($attemptobj->get_question_name($slot)),
        'clatest' => format_string($attemptobj->get_clatest_name()), 'user' => fullname($student))));
...
// Quiz name.
$summarydata['clatestname'] = array(
    'title'   => get_string('modulename', 'clatest'),
    'content' => format_string($attemptobj->get_clatest_name()),
);

// Question name.
$summarydata['questionname'] = array(
    'title'   => get_string('question', 'clatest'),
...
<?php
echo '</form>';
$PAGE->requires->js_init_call('M.mod_clatest.init_comment_popup', null, false, clatest_get_js_module());
