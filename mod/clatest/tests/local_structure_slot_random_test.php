...
/**
 * Unit tests for the {@link \mod_clatest\local\structure\slot_random} class.
 *
 * @package    mod_clatest
 * @category   test
...
/**
 * Class mod_clatest_local_structure_slot_random_test
 * Class for tests related to the {@link \mod_clatest\local\structure\slot_random} class.
 *
 * @copyright  2018 Shamim Rezaie <shamim@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_clatest_local_structure_slot_random_test extends advanced_testcase {
    /**
     * Constructor test.
     */
    public function test_constructor() {
        ...
        // Create a random question without adding it to a clatest.
        // We don't want to use clatest_add_random_questions because that itself, instantiates an object from the slot_random class.
        $form = new stdClass();
        ...
        $rc = new ReflectionClass('\mod_clatest\local\structure\slot_random');
        $rcp = $rc->getProperty('record');
        $rcp->setAccessible(true);
        $record = $rcp->getValue($randomslot);

        $this->assertEquals($clatest->id, $record->clatestid);
    ...
    public function test_get_clatest_clatest() {
        global $SITE, $DB;
        ...
        clatest_add_random_questions($clatest, 0, $category->id, 1, false);

        // Get the random question's id. It is at the first slot.
        $questionid = $DB->get_field('clatest_slots', 'questionid', array('clatestid' => $clatest->id, 'slot' => 1));
        ...
        // The create_instance had injected an additional cmid propery to the clatest. Let's remove that.
        unset($clatest->cmid);

        $this->assertEquals($clatest, $randomslot->get_clatest());
    }

    public function test_set_clatest() {
        global $SITE, $DB;
        ...
        $rc = new ReflectionClass('\mod_clatest\local\structure\slot_random');
        $rcp = $rc->getProperty('clatest');
        $rcp->setAccessible(true);
        $clatestpropery = $rcp->getValue($randomslot);

        $this->assertEquals($clatest, $clatestpropery);
    }
    ...
    public function test_insert() {
        ...
        // Insert the random question to the clatest.
        $randomslot = new \mod_clatest\local\structure\slot_random($randomslotdata);
        $randomslot->set_tags([$footag, $bartag]);
        $randomslot->insert(1); // Put the question on the first page of the clatest.

        // Get the random question's clatest_slot. It is at the first slot.
        $clatestslot = $DB->get_record('clatest_slots', array('clatestid' => $clatest->id, 'slot' => 1));
        // Get the random question's tags from clatest_slot_tags. It is at the first slot.
        $clatestslottags = $DB->get_records('clatest_slot_tags', array('slotid' => $clatestslot->id));

        $this->assertEquals($question->id, $clatestslot->questionid);
        $this->assertEquals($category->id, $clatestslot->questioncategoryid);
        $this->assertEquals(1, $clatestslot->includingsubcategories);
        $this->assertEquals(1, $clatestslot->maxmark);

        $this->assertCount(2, $clatestslottags);
        $this->assertEquals(
                [
