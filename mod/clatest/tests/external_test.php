...
/**
 * Silly class to access mod_clatest_external internal methods.
 *
 * @package mod_clatest
 * @copyright 2016 Juan Leyva <juan@moodle.com>
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since  Moodle 3.1
 */
class testable_mod_clatest_external extends mod_clatest_external {

    /**
     * Public accessor.
     *
     * @param  array $params Array of parameters including the attemptid and preflight data
     * @param  bool $checkaccessrules whether to check the clatest access rules or not
 ...
 * @since      Moodle 3.1
 */
class mod_clatest_external_testcase extends externallib_advanced_testcase {
    ...
    public function setUp() {
        ...
        $this->clatest = $this->getDataGenerator()->create_module('clatest', array('course' => $this->course->id));
        $this->context = context_module::instance($this->clatest->cmid);
        $this->cm = get_coursemodule_from_instance('clatest', $this->clatest->id);
    ...
    /**
     * Create a clatest with questions including a started or finished attempt optionally
     *
     * @param  boolean $startattempt whether to start a new attempt
     * @param  boolean $finishattempt whether to finish the new attempt
     * @param  string $behaviour the clatest preferredbehaviour, defaults to 'deferredfeedback'.
     * @return array array containing the clatest, context and the attempt
     */
    private function create_clatest_with_questions($startattempt = false, $finishattempt = false, $behaviour = 'deferredfeedback') {

        // Create a new clatest with attempts.
        ...
        $clatestobj = clatest::create($clatest->id, $this->student->id);
        ...
        if ($startattempt or $finishattempt) {
            ...
            $timenow = time();
            $attempt = clatest_create_attempt($clatestobj, 1, false, $timenow, false, $this->student->id);
            ...
            return array($clatest, $context, $clatestobj, $attempt, $attemptobj, $quba);
        } else {
            return array($clatest, $context, $clatestobj);
        }

    }

    /*
     * Test get clatestzes by courses
     */
    public function test_mod_clatest_get_clatestzes_by_courses() {
        global $DB;
        ...
        $clatest2 = self::getDataGenerator()->create_module('clatest', $record);
        ...
        $returndescription = mod_clatest_external::get_clatestzes_by_courses_returns();
        ...
        $clatest1 = $this->clatest;
        $clatest1->coursemodule = $clatest1->cmid;
        $clatest1->introformat = 1;
        $clatest1->section = 0;
        $clatest1->visible = true;
        $clatest1->groupmode = 0;
        $clatest1->groupingid = 0;
        $clatest1->hasquestions = 0;
        $clatest1->hasfeedback = 0;
        $clatest1->autosaveperiod = get_config('clatest', 'autosaveperiod');
        $clatest1->introfiles = [];

        $clatest2->coursemodule = $clatest2->cmid;
        $clatest2->introformat = 1;
        $clatest2->section = 0;
        $clatest2->visible = true;
        $clatest2->groupmode = 0;
        $clatest2->groupingid = 0;
        $clatest2->hasquestions = 0;
        $clatest2->hasfeedback = 0;
        $clatest2->autosaveperiod = get_config('clatest', 'autosaveperiod');
        $clatest2->introfiles = [];

        foreach (array_merge($allusersfields, $userswithaccessfields) as $field) {
            $expected1[$field] = $clatest1->{$field};
            $expected2[$field] = $clatest2->{$field};
        }

        $expectedclatestzes = array($expected2, $expected1);

        // Call the external function passing course ids.
        $result = mod_clatest_external::get_clatestzes_by_courses(array($course2->id, $this->course->id));
        $result = external_api::clean_returnvalue($returndescription, $result);

        $this->assertEquals($expectedclatestzes, $result['clatestzes']);
        $this->assertCount(0, $result['warnings']);

        // Call the external function without passing course id.
        $result = mod_clatest_external::get_clatestzes_by_courses();
        $result = external_api::clean_returnvalue($returndescription, $result);
        $this->assertEquals($expectedclatestzes, $result['clatestzes']);
        $this->assertCount(0, $result['warnings']);

        // Unenrol user from second course and alter expected clatestzes.
        $enrol->unenrol_user($instance2, $this->student->id);
        array_shift($expectedclatestzes);
        ...
        $result = mod_clatest_external::get_clatestzes_by_courses(array($course2->id));
        ...
        foreach ($managerfields as $field) {
            $expectedclatestzes[0][$field] = $clatest1->{$field};
        }
        ...
        $result = mod_clatest_external::get_clatestzes_by_courses(array($this->course->id));
        $result = external_api::clean_returnvalue($returndescription, $result);
        $this->assertEquals($expectedclatestzes, $result['clatestzes']);
        ...
        $clatest2->timeclose = time() - DAYSECS;
        $DB->update_record('clatest', $clatest2);

        $result = mod_clatest_external::get_clatestzes_by_courses();
        $result = external_api::clean_returnvalue($returndescription, $result);
        $this->assertCount(2, $result['clatestzes']);
        // We only see a limited set of fields.
        $this->assertCount(4, $result['clatestzes'][0]);
        $this->assertEquals($clatest2->id, $result['clatestzes'][0]['id']);
        $this->assertEquals($clatest2->coursemodule, $result['clatestzes'][0]['coursemodule']);
        $this->assertEquals($clatest2->course, $result['clatestzes'][0]['course']);
        $this->assertEquals($clatest2->name, $result['clatestzes'][0]['name']);
        $this->assertEquals($clatest2->course, $result['clatestzes'][0]['course']);

        $this->assertFalse(isset($result['clatestzes'][0]['timelimit']));

    }

    /**
     * Test test_view_clatest
     */
    public function test_view_clatest() {
        global $DB;

        // Test invalid instance id.
        try {
            mod_clatest_external::view_clatest(0);
            $this->fail('Exception expected due to invalid mod_clatest instance id.');
        ...
        try {
            mod_clatest_external::view_clatest($this->clatest->id);
        ...
        $result = mod_clatest_external::view_clatest($this->clatest->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::view_clatest_returns(), $result);
        ...
        $this->assertInstanceOf('\mod_clatest\event\course_module_viewed', $event);
        $this->assertEquals($this->context, $event->get_context());
        $moodleclatest = new \moodle_url('/mod/clatest/view.php', array('id' => $this->cm->id));
        $this->assertEquals($moodleclatest, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());

        // Test user with no capabilities.
        // We need a explicit prohibit since this capability is only defined in authenticated user and guest roles.
        assign_capability('mod/clatest:view', CAP_PROHIBIT, $this->studentrole->id, $this->context->id);
    ...
    public function test_get_user_attempts() {
        ...
        // Create a clatest with one attempt finished.
        list($clatest, $context, $clatestobj, $attempt, $attemptobj) = $this->create_clatest_with_questions(true, true);

        $this->setUser($this->student);
        $result = mod_clatest_external::get_user_attempts($clatest->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_user_attempts_returns(), $result);

        $this->assertCount(1, $result['attempts']);
        $this->assertEquals($attempt->id, $result['attempts'][0]['id']);
        $this->assertEquals($clatest->id, $result['attempts'][0]['clatest']);
        ...
        $result = mod_clatest_external::get_user_attempts($clatest->id, 0, 'finished', false);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_user_attempts_returns(), $result);
        ...
        $result = mod_clatest_external::get_user_attempts($clatest->id, 0, 'all', false);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_user_attempts_returns(), $result);
        ...
        $result = mod_clatest_external::get_user_attempts($clatest->id, 0, 'unfinished', false);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_user_attempts_returns(), $result);
        ...
        $timenow = time();
        $attempt = clatest_create_attempt($clatestobj, 2, false, $timenow, false, $this->student->id);
        $quba = question_engine::make_questions_usage_by_activity('mod_clatest', $clatestobj->get_context());
        $quba->set_preferred_behaviour($clatestobj->get_clatest()->preferredbehaviour);
        ...
        $result = mod_clatest_external::get_user_attempts($clatest->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_user_attempts_returns(), $result);

        $this->assertCount(1, $result['attempts']);
        $this->assertEquals($this->student->id, $result['attempts'][0]['userid']);

        $result = mod_clatest_external::get_user_attempts($clatest->id, $this->student->id, 'all');
        $result = external_api::clean_returnvalue(mod_clatest_external::get_user_attempts_returns(), $result);
        ...
        // Invalid parameters.
        try {
            mod_clatest_external::get_user_attempts($clatest->id, $this->student->id, 'INVALID_PARAMETER');
            $this->fail('Exception expected due to missing capability.');
    ...
    public function test_get_user_best_grade() {
        ...
        $result = mod_clatest_external::get_user_best_grade($this->clatest->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_user_best_grade_returns(), $result);
        ...
        try {
            mod_clatest_external::get_user_best_grade($this->clatest->id, $anotherstudent->id);
            $this->fail('Exception expected due to missing capability.');
        ...
        $result = mod_clatest_external::get_user_best_grade($this->clatest->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_user_best_grade_returns(), $result);
        ...
        // Invalid user.
        try {
            mod_clatest_external::get_user_best_grade($this->clatest->id, -1);
            $this->fail('Exception expected due to missing capability.');
        ...
        $DB->delete_records('clatest_grades', array('id' => $grade->id));

    }
    /**
     * Test get_combined_review_options.
     * This is a basic test, this is already tested in mod_clatest_display_options_testcase.
     */
    public function test_get_combined_review_options() {
        ...
        $result = mod_clatest_external::get_combined_review_options($clatest->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_combined_review_options_returns(), $result);
        ...
        $result = mod_clatest_external::get_combined_review_options($clatest->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_combined_review_options_returns(), $result);
        ...
        // Invalid user.
        try {
            mod_clatest_external::get_combined_review_options($clatest->id, -1);
            $this->fail('Exception expected due to missing capability.');
        } catch (dml_missing_record_exception $e) {
            $this->assertEquals('invaliduser', $e->errorcode);
        }
    ...
    public function test_start_attempt() {
        ...
        // Create a new clatest with questions.
        list($clatest, $context, $clatestobj) = $this->create_clatest_with_questions();

        $this->setUser($this->student);

        // Try to open attempt in closed clatest.
        $clatest->timeopen = time() - WEEKSECS;
        $clatest->timeclose = time() - DAYSECS;
        $DB->update_record('clatest', $clatest);
        $result = mod_clatest_external::start_attempt($clatest->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::start_attempt_returns(), $result);
        ...
        $clatest->timeopen = 0;
        $clatest->timeclose = 0;
        $clatest->password = 'abc';
        $DB->update_record('clatest', $clatest);

        try {
            mod_clatest_external::start_attempt($clatest->id, array(array("name" => "clatestpassword", "value" => 'bad')));
            $this->fail('Exception expected due to invalid passwod.');
        } catch (moodle_exception $e) {
            $this->assertEquals(get_string('passworderror', 'clatestaccess_password'), $e->errorcode);
        }

        // Now, try everything correct.
        $result = mod_clatest_external::start_attempt($clatest->id, array(array("name" => "clatestpassword", "value" => 'abc')));
        $result = external_api::clean_returnvalue(mod_clatest_external::start_attempt_returns(), $result);

        $this->assertEquals(1, $result['attempt']['attempt']);
        $this->assertEquals($this->student->id, $result['attempt']['userid']);
        $this->assertEquals($clatest->id, $result['attempt']['clatest']);
        $this->assertCount(0, $result['warnings']);
        ...
        try {
            mod_clatest_external::start_attempt($clatest->id, array(array("name" => "clatestpassword", "value" => 'abc')));
            $this->fail('Exception expected due to attempt not finished.');
        } catch (moodle_clatest_exception $e) {
        ...
        // Process some responses from the student.
        $timenow = time();
        $attemptobj = clatest_attempt::create($attemptid);
        $tosubmit = array(1 => array('answer' => '3.14'));
        ...
        // Test user with no capabilities.
        // We need a explicit prohibit since this capability is only defined in authenticated user and guest roles.
        assign_capability('mod/clatest:attempt', CAP_PROHIBIT, $this->studentrole->id, $context->id);
        // Empty all the caches that may be affected  by this change.
        accesslib_clear_all_caches_for_unit_testing();
        course_modinfo::clear_instance_cache();

        try {
            mod_clatest_external::start_attempt($clatest->id);
            $this->fail('Exception expected due to missing capability.');
        } catch (required_capability_exception $e) {
            $this->assertEquals('nopermissions', $e->errorcode);
        }
    ...
    public function test_validate_attempt() {
        ...
        // Create a new clatest with one attempt started.
        list($clatest, $context, $clatestobj, $attempt, $attemptobj) = $this->create_clatest_with_questions(true);
        ...
        try {
            ...
            testable_mod_clatest_external::validate_attempt($params);
        ...
        $result = testable_mod_clatest_external::validate_attempt($params);
        ...
        try {
            $params = array('attemptid' => $attempt->id, 'page' => 0,
                            'preflightdata' => array(array("name" => "clatestpassword", "value" => 'bad')));
            testable_mod_clatest_external::validate_attempt($params);
            $this->fail('Exception expected due to invalid passwod.');
        } catch (moodle_exception $e) {
            $this->assertEquals(get_string('passworderror', 'clatestaccess_password'), $e->errorcode);
        ...
        testable_mod_clatest_external::validate_attempt($params, false);
    ...
    public function test_get_attempt_data() {
        ...
        $DB->set_field('clatest', 'reviewcorrectness', mod_clatest_display_options::IMMEDIATELY_AFTER, array('id' => $clatest->id));

        $clatestobj = $attemptobj->get_clatestobj();
        $clatestobj->preload_questions();
        $clatestobj->load_questions();
        $questions = $clatestobj->get_questions();

        $this->setUser($this->student);

        // We receive one question per page.
        $result = mod_clatest_external::get_attempt_data($attempt->id, 0);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_attempt_data_returns(), $result);
        ...
        $result = mod_clatest_external::get_attempt_data($attempt->id, 1);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_attempt_data_returns(), $result);
        ...
        $result = mod_clatest_external::get_attempt_review($attempt->id, 1);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_attempt_review_returns(), $result);
        $this->assertEquals('gaveup', $result['questions'][0]['state']);

        // Change setting and expect two pages.
        $clatest->questionsperpage = 4;
        $DB->update_record('clatest', $clatest);
    ...
    public function test_get_attempt_data_with_blocked_questions() {
        ...
        // Create a new clatest with one attempt started and using immediatefeedback.
        list($clatest, $context, $clatestobj, $attempt, $attemptobj) = $this->create_clatest_with_questions(
    ...
    public function test_get_attempt_summary() {
        ...
        $this->setUser($this->student);
        $result = mod_clatest_external::get_attempt_summary($attempt->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_attempt_summary_returns(), $result);
    ...
    public function test_save_attempt() {
        ...
        $timenow = time();
        // Create a new clatest with one attempt started.
        list($clatest, $context, $clatestobj, $attempt, $attemptobj, $quba) = $this->create_clatest_with_questions(true);
        ...
        $result = mod_clatest_external::save_attempt($attempt->id, $data);
        $result = external_api::clean_returnvalue(mod_clatest_external::save_attempt_returns(), $result);
        $this->assertTrue($result['status']);
    ...
    public function test_process_attempt() {
        ...
        $timenow = time();
        // Create a new clatest with two questions and one attempt started.
        list($clatest, $context, $clatestobj, $attempt, $attemptobj, $quba) = $this->create_clatest_with_questions(true);
        ...
        $result = mod_clatest_external::process_attempt($attempt->id, $data);
        $result = external_api::clean_returnvalue(mod_clatest_external::process_attempt_returns(), $result);
        $this->assertEquals(clatest_attempt::IN_PROGRESS, $result['state']);
        ...
        // Finish the attempt.
        $result = mod_clatest_external::process_attempt($attempt->id, array(), true);
        $result = external_api::clean_returnvalue(mod_clatest_external::process_attempt_returns(), $result);
        $this->assertEquals(clatest_attempt::FINISHED, $result['state']);
        ...
        $timenow = time();
        $attempt = clatest_create_attempt($clatestobj, 2, false, $timenow, false, $this->student->id);
        clatest_start_new_attempt($clatestobj, $quba, $attempt, 2, $timenow);
        clatest_attempt_save_started($clatestobj, $quba, $attempt);

        // Force grace period, attempt going to overdue.
        $clatest->timeclose = $timenow - 10;
        $clatest->graceperiod = 60;
        $clatest->overduehandling = 'graceperiod';
        $DB->update_record('clatest', $clatest);

        $result = mod_clatest_external::process_attempt($attempt->id, array());
        $result = external_api::clean_returnvalue(mod_clatest_external::process_attempt_returns(), $result);
        $this->assertEquals(clatest_attempt::OVERDUE, $result['state']);

        // New attempt.
        $timenow = time();
        $quba = question_engine::make_questions_usage_by_activity('mod_clatest', $clatestobj->get_context());
        $quba->set_preferred_behaviour($clatestobj->get_clatest()->preferredbehaviour);
        $attempt = clatest_create_attempt($clatestobj, 3, 2, $timenow, false, $this->student->id);
        clatest_start_new_attempt($clatestobj, $quba, $attempt, 3, $timenow);
        clatest_attempt_save_started($clatestobj, $quba, $attempt);

        // Force abandon.
        $clatest->timeclose = $timenow - HOURSECS;
        $DB->update_record('clatest', $clatest);

        $result = mod_clatest_external::process_attempt($attempt->id, array());
        $result = external_api::clean_returnvalue(mod_clatest_external::process_attempt_returns(), $result);
        $this->assertEquals(clatest_attempt::ABANDONED, $result['state']);
    ...
    public function test_validate_attempt_review() {
        ...
        try {
            ...
            testable_mod_clatest_external::validate_attempt_review($params);
    ...
    public function test_get_attempt_review() {
        ...
        // Create a new clatest with two questions and one attempt finished.
        list($clatest, $context, $clatestobj, $attempt, $attemptobj, $quba) = $this->create_clatest_with_questions(true, true);

        // Add feedback to the clatest.
        ...
        $result = mod_clatest_external::get_attempt_review($attempt->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_attempt_review_returns(), $result);
        ...
        // Only first page.
        $result = mod_clatest_external::get_attempt_review($attempt->id, 0);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_attempt_review_returns(), $result);
    ...
    public function test_view_attempt() {
        ...
        // Create a new clatest with two questions and one attempt started.
        list($clatest, $context, $clatestobj, $attempt, $attemptobj, $quba) = $this->create_clatest_with_questions(true, false);
        ...
        $result = mod_clatest_external::view_attempt($attempt->id, 0);
        $result = external_api::clean_returnvalue(mod_clatest_external::view_attempt_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        // Now, force the clatest with QUIZ_NAVMETHOD_SEQ (sequencial) navigation method.
        $DB->set_field('clatest', 'navmethod', QUIZ_NAVMETHOD_SEQ, array('id' => $clatest->id));
        // Quiz requiring preflightdata.
        $DB->set_field('clatest', 'password', 'abcdef', array('id' => $clatest->id));
        $preflightdata = array(array("name" => "clatestpassword", "value" => 'abcdef'));

        // See next page.
        $result = mod_clatest_external::view_attempt($attempt->id, 1, $preflightdata);
        $result = external_api::clean_returnvalue(mod_clatest_external::view_attempt_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        try {
            mod_clatest_external::view_attempt($attempt->id, 0);
    ...
    public function test_view_attempt_summary() {
        ...
        $result = mod_clatest_external::view_attempt_summary($attempt->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::view_attempt_summary_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        $this->assertEquals($context, $event->get_context());
        $moodleclatest = new \moodle_url('/mod/clatest/summary.php', array('attempt' => $attempt->id));
        $this->assertEquals($moodleclatest, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());
        ...
        $result = mod_clatest_external::view_attempt_summary($attempt->id, $preflightdata);
        $result = external_api::clean_returnvalue(mod_clatest_external::view_attempt_summary_returns(), $result);
        $this->assertTrue($result['status']);
    ...
    public function test_view_attempt_review() {
        ...
        $result = mod_clatest_external::view_attempt_review($attempt->id, 0);
        $result = external_api::clean_returnvalue(mod_clatest_external::view_attempt_review_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        $this->assertEquals($context, $event->get_context());
        $moodleclatest = new \moodle_url('/mod/clatest/review.php', array('attempt' => $attempt->id));
        $this->assertEquals($moodleclatest, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());
    ...
    /**
     * Test get_clatest_feedback_for_grade
     */
    public function test_get_clatest_feedback_for_grade() {
        global $DB;

        // Add feedback to the clatest.
        $feedback = new stdClass();
        $feedback->clatestid = $this->clatest->id;
        $feedback->feedbacktext = 'Feedback text 1';
        $feedback->feedbacktextformat = 1;
        $feedback->mingrade = 49;
        $feedback->maxgrade = 100;
        ...
        $result = mod_clatest_external::get_clatest_feedback_for_grade($this->clatest->id, 50);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_clatest_feedback_for_grade_returns(), $result);
        ...
        $result = mod_clatest_external::get_clatest_feedback_for_grade($this->clatest->id, 30);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_clatest_feedback_for_grade_returns(), $result);
        $this->assertEquals('Feedback text 2', $result['feedbacktext']);
        $this->assertEquals(FORMAT_HTML, $result['feedbacktextformat']);

        $result = mod_clatest_external::get_clatest_feedback_for_grade($this->clatest->id, 10);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_clatest_feedback_for_grade_returns(), $result);
    ...
    /**
     * Test get_clatest_access_information
     */
    public function test_get_clatest_access_information() {
        global $DB;

        // Create a new clatest.
        $clatestgenerator = $this->getDataGenerator()->get_plugin_generator('mod_clatest');
        $data = array('course' => $this->course->id);
        $clatest = $clatestgenerator->create_instance($data);
        ...
        $result = mod_clatest_external::get_clatest_access_information($clatest->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_clatest_access_information_returns(), $result);
        ...
        $expected = array(
            ...
            'activerulenames' => ['clatestaccess_openclosedate'],
        ...
        $clatest->timeopen = time() + DAYSECS;
        $clatest->timeclose = time() + WEEKSECS;
        $clatest->password = '123456';
        $DB->update_record('clatest', $clatest);
    ...
    public function test_get_attempt_access_information() {
        ...
        // Default restrictions (none).
        $result = mod_clatest_external::get_attempt_access_information($clatest->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_attempt_access_information_returns(), $result);
        ...
        $result = mod_clatest_external::get_attempt_access_information($clatest->id, $attempt->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_attempt_access_information_returns(), $result);
        ...
        $this->assertEquals(get_string('nomoreattempts', 'clatest'), $result['preventnewattemptreasons'][0]);

    }

    /**
     * Test get_clatest_required_qtypes
     */
    public function test_get_clatest_required_qtypes() {
        $this->setAdminUser();
        ...
        $result = mod_clatest_external::get_clatest_required_qtypes($clatest->id);
        $result = external_api::clean_returnvalue(mod_clatest_external::get_clatest_required_qtypes_returns(), $result);
