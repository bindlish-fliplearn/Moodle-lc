...
/**
 * Unit tests for the {@link \mod_clatest\repaginate} class.
 * @package   mod_clatest
 * @category  test
...
require_once($CFG->dirroot . '/mod/clatest/classes/repaginate.php');


/**
 * Testable subclass, giving access to the protected methods of {@link \mod_clatest\repaginate}
 * @copyright 2014 The Open Univsersity
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_clatest_repaginate_testable extends \mod_clatest\repaginate {

    public function __construct($clatestid = 0, $slots = null) {
        return parent::__construct($clatestid, $slots);
    }
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_clatest_repaginate_test extends advanced_testcase {

    /** @var array stores the slots. */
    private $clatestslots;
    /** @var mod_clatest_repaginate_testable the object being tested. */
    private $repaginate = null;

    public function setUp() {
        $this->set_clatest_slots($this->get_clatest_object()->get_slots());
        $this->repaginate = new mod_clatest_repaginate_testable(0, $this->clatestslots);
    }
    ...
    /**
     * Create a clatest, add five questions to the clatest
     * which are all on one page and return the clatest object.
     */
    private function get_clatest_object() {
        global $SITE;
        ...
        $clatest = $clatestgenerator->create_instance(array(
                'course' => $SITE->id, 'questionsperpage' => 0, 'grade' => 100.0, 'sumgrades' => 2));
        $cm = get_coursemodule_from_instance('clatest', $clatest->id, $SITE->id);
        ...
        clatest_add_clatest_question($shortanswer->id, $clatest);
        clatest_add_clatest_question($numerical->id, $clatest);
        clatest_add_clatest_question($essay->id, $clatest);
        clatest_add_clatest_question($truefalse->id, $clatest);
        clatest_add_clatest_question($match->id, $clatest);

        // Return the clatest object.
        $clatestobj = new clatest($clatest, $cm, $SITE);
        return \mod_clatest\structure::create_for_clatest($clatestobj);
    }

    /**
     * Set the clatest slots
     * @param string $slots
     */
    private function set_clatest_slots($slots = null) {
        if (!$slots) {
            $this->clatestslots = $this->get_clatest_object()->get_slots();
        } else {
            $this->clatestslots = $slots;
        }
    }
    ...
    public function test_get_this_slot() {
        $this->set_clatest_slots();
        $actual = array();
        $expected = $this->repaginate->get_slots_by_slot_number();
        $this->assertEquals($expected, $actual);

        $slotsbyno = $this->repaginate->get_slots_by_slot_number($this->clatestslots);
        $slotnumber = 5;
        $thisslot = $this->repaginate->get_this_slot($this->clatestslots, $slotnumber);
    ...
    public function test_get_slots_by_slotnumber() {
        ...
        foreach ($this->clatestslots as $slot) {
            $expected[$slot->slot] = $slot;
        }
        $actual = $this->repaginate->get_slots_by_slot_number($this->clatestslots);
        $this->assertEquals($expected, $actual);
    }
    ...
    public function test_get_slots_by_slotid() {
        ...
        $this->assertEquals($this->clatestslots, $actual);
    }
    ...
    public function test_repaginate_n_questions_per_page() {
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->clatestslots, 2);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->clatestslots, 3);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->clatestslots, 5);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->clatestslots, 10);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->clatestslots, 1);
        $this->assertEquals($expected, $actual);
    }

    public function test_repaginate_this_slot() {
        $this->set_clatest_slots();
        $slotsbyslotno = $this->repaginate->get_slots_by_slot_number($this->clatestslots);
        $slotnumber = 3;
        $newpagenumber = 2;
    ...
    public function test_repaginate_the_rest() {
        $this->set_clatest_slots();
        $slotfrom = 1;
        $type = \mod_clatest\repaginate::LINK;
        $expected = array();
        foreach ($this->clatestslots as $slot) {
        ...
        $actual = $this->repaginate->repaginate_the_rest($this->clatestslots, $slotfrom, $type, false);
        $this->assertEquals($expected, $actual);

        $slotfrom = 2;
        $newslots = array();
        foreach ($this->clatestslots as $s) {
            if ($s->slot === $slotfrom) {
                $s->page = $s->page - 1;
            }
            $newslots[$s->id] = $s;
        ...
        $type = \mod_clatest\repaginate::UNLINK;
        $expected = array();
        foreach ($this->clatestslots as $slot) {
