 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_clatest_attempt_walkthrough_testcase extends advanced_testcase {

    /**
     * Create a clatest with questions and walk through a clatest attempt.
     */
    public function test_clatest_attempt_walkthrough() {
        global $SITE;
        ...
        $clatest = $clatestgenerator->create_instance(array('course'=>$SITE->id, 'questionsperpage' => 0, 'grade' => 100.0,
                                                      'sumgrades' => 2));
        ...
        $grades = clatest_get_user_grades($clatest, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'clatest', $clatest->id, $user1->id);
    ...
    /**
     * Create a clatest with a random as well as other questions and walk through clatest attempts.
     */
    public function test_clatest_with_random_question_attempt_walkthrough() {
        global $SITE;
        ...
        $clatest = $clatestgenerator->create_instance(array('course' => $SITE->id, 'questionsperpage' => 2, 'grade' => 100.0,
                                                      'sumgrades' => 4));
        ...
        // Add random question to the clatest.
        ...
        foreach (array($saq->id => 'frog', $numq->id => '3.14') as $randomqidtoselect => $randqanswer) {
            ...
            $timenow = time();
            $attempt = clatest_create_attempt($clatestobj, 1, false, $timenow);

            clatest_start_new_attempt($clatestobj, $quba, $attempt, 1, $timenow, array(1 => $randomqidtoselect));
    ...
    protected $clatestwithvariants = null;

    /**
     * Create a clatest with a single question with variants and walk through clatest attempts.
     *
     * @dataProvider get_correct_response_for_variants
     */
    public function test_clatest_with_question_with_variants_attempt_walkthrough($variantno, $correctresponse, $done = false) {
        global $SITE;
        ...
        if ($this->clatestwithvariants === null) {
            // Make a clatest.
            $clatestgenerator = $this->getDataGenerator()->get_plugin_generator('mod_clatest');

            $this->clatestwithvariants = $clatestgenerator->create_instance(array('course'=>$SITE->id,
                                                                            'questionsperpage' => 0,
                                                                            'grade' => 100.0,
                                                                            'sumgrades' => 1));
            ...
            clatest_add_clatest_question($calc->id, $this->clatestwithvariants, 0);
        }


        // Make a new user to do the clatest.
        $user1 = $this->getDataGenerator()->create_user();
        $this->setUser($user1);
        $clatestobj = clatest::create($this->clatestwithvariants->id, $user1->id);
        ...
        // Select variant.
        clatest_start_new_attempt($clatestobj, $quba, $attempt, 1, $timenow, array(), array(1 => $variantno));
        ...
        // Check clatest grades.
        $grades = clatest_get_user_grades($this->clatestwithvariants, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'clatest', $this->clatestwithvariants->id, $user1->id);
        $gradebookitem = array_shift($gradebookgrades->items);
        $gradebookgrade = array_shift($gradebookitem->grades);
        $this->assertEquals(100, $gradebookgrade->grade);
    }
