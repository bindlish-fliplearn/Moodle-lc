 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_clatest_attempt_walkthrough_from_csv_testcase extends advanced_testcase {

    protected $files = array('questions', 'steps', 'results');

    /**
     * @var stdClass the clatest record we create.
     */
    protected $clatest;
     ...
     * directory.
     *
     * @param array $clatestsettings of settings read from csv file clatestzes.csv
     ...
     */
    public function test_walkthrough_from_csv($clatestsettings, $csvdata) {

        // CSV data files for these tests were generated using :
        // https://github.com/jamiepratt/moodle-clatest-tools/tree/master/responsegenerator

        $this->create_clatest_simulate_attempts_and_check_results($clatestsettings, $csvdata);
    }

    public function create_clatest($clatestsettings, $qs) {
        global $SITE, $DB;
        $this->setAdminUser();
        ...
        $aggregratedsettings = $clatestsettings + array('course' => $SITE->id,
                                                     'questionsperpage' => 0,
                                                     'grade' => 100.0,
                                                     'sumgrades' => $sumofgrades);

        $this->clatest = $clatestgenerator->create_instance($aggregratedsettings);

        $this->randqids = array();
        foreach ($slots as $slotno => $slotquestion) {
            if ($slotquestion['type'] !== 'random') {
                clatest_add_clatest_question($slotquestion['id'], $this->clatest, 0, $slotquestion['mark']);
            } else {
                clatest_add_random_questions($this->clatest, 0, $slotquestion['catid'], 1, 0);
    ...
    /**
     * Create clatest, simulate attempts and check results (if resultsXX.csv exists).
     *
     * @param array $clatestsettings Quiz overrides for this clatest.
     * @param PHPUnit\DbUnit\DataSet\ITable[] $csvdata Data loaded from csv files for this test.
     */
    protected function create_clatest_simulate_attempts_and_check_results($clatestsettings, $csvdata) {
        $this->resetAfterTest(true);
        question_bank::get_qtype('random')->clear_caches_before_testing();

        $this->create_clatest($clatestsettings, $csvdata['questions']);
    ...
    public function get_data_for_walkthrough() {
        $clatestzes = $this->load_csv_data_file('clatestzes');
        $datasets = array();
        for ($rowno = 0; $rowno < $clatestzes->getRowCount(); $rowno++) {
            $clatestsettings = $clatestzes->getRow($rowno);
            $dataset = array();
            foreach ($this->files as $file) {
                if (file_exists($this->get_full_path_of_csv_file($file, $clatestsettings['testnumber']))) {
                    $dataset[$file] = $this->load_csv_data_file($file, $clatestsettings['testnumber']);
                }
            }
            $datasets[] = array($clatestsettings, $dataset);
        }
        return $datasets;
    }
     ...
     * @return array attempt no as in csv file => the id of the clatest_attempt as stored in the db.
     */
    protected function walkthrough_attempts($steps) {
        ...
        for ($rowno = 0; $rowno < $steps->getRowCount(); $rowno++) {
            ...
            // Find existing user or make a new user to do the clatest.
            ...
            if (!isset($attemptids[$step['clatestattempt']])) {
                // Start the attempt.
                $clatestobj = clatest::create($this->clatest->id, $user->id);
                ...
                $prevattempts = clatest_get_user_attempts($this->clatest->id, $user->id, 'all', true);
                $attemptnumber = count($prevattempts) + 1;
                $timenow = time();
                $attempt = clatest_create_attempt($clatestobj, $attemptnumber, false, $timenow, false, $user->id);
                ...
                clatest_start_new_attempt($clatestobj, $quba, $attempt, $attemptnumber, $timenow, $step['randqs'], $step['variants']);
                clatest_attempt_save_started($clatestobj, $quba, $attempt);
                $attemptid = $attemptids[$step['clatestattempt']] = $attempt->id;
            } else {
                $attemptid = $attemptids[$step['clatestattempt']];
            }
     ...
     * @param $attemptids array attempt no as in csv file => the id of the clatest_attempt as stored in the db.
     */
    protected function check_attempts_results($results, $attemptids) {
        for ($rowno = 0; $rowno < $results->getRowCount(); $rowno++) {
            ...
            // Re-load clatest attempt data.
            $attemptobj = clatest_attempt::create($attemptids[$result['clatestattempt']]);
     ...
     * @param clatest_attempt $attemptobj         the attempt object loaded from db.
        ...
        foreach ($result as $fieldname => $value) {
            ...
            switch ($fieldname) {
                            ...
                            switch ($slotfieldname) {
                                case 'mark' :
                                    $this->assertEquals(round($slotvalue, 2), $attemptobj->get_question_mark($slotno),
                                                        "Mark for slot $slotno of attempt {$result['clatestattempt']}.");
                                    break;
                                default :
                ...
                case 'finished' :
                    ...
                    break;
                case 'summarks' :
                    $this->assertEquals($value, $attemptobj->get_sum_marks(), "Sum of marks of attempt {$result['clatestattempt']}.");
                    break;
                case 'clatestgrade' :
                    // Check clatest grades.
                    $grades = clatest_get_user_grades($attemptobj->get_clatest(), $attemptobj->get_userid());
                    $grade = array_shift($grades);
                    $this->assertEquals($value, $grade->rawgrade, "Quiz grade for attempt {$result['clatestattempt']}.");
                    break;
                case 'gradebookgrade' :
                    // Check grade book.
                    $gradebookgrades = grade_get_grades($attemptobj->get_courseid(),
                                                        'mod', 'clatest',
                                                        $attemptobj->get_clatestid(),
                                                        $attemptobj->get_userid());
                    $gradebookitem = array_shift($gradebookgrades->items);
                    $gradebookgrade = array_shift($gradebookitem->grades);
                    $this->assertEquals($value, $gradebookgrade->grade, "Gradebook grade for attempt {$result['clatestattempt']}.");
                    break;
                default :
