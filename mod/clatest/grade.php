...
$cm = get_coursemodule_from_id('clatest', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$clatest = $DB->get_record('clatest', array('id' => $cm->instance), '*', MUST_EXIST);
require_login($course, false, $cm);

$reportlist = clatest_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see clatest reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/clatest/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = clatest_get_user_attempts($clatest->id, $userid, 'finished');
    $attempt = null;
    ...
    if ($attempt) {
        $attemptobj = new clatest_attempt($attempt, $clatest, $cm, $course, false);
...
redirect(new moodle_url('/mod/clatest/report.php', array(
