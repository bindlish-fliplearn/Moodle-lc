...
/**
 * This page handles listing of clatest overrides
 *
 * @package    mod_clatest
...
require_once($CFG->dirroot.'/mod/clatest/override_form.php');
...
$url = new moodle_url('/mod/clatest/overrides.php', array('cmid'=>$cm->id, 'mode'=>$mode));
...
require_capability('mod/clatest:manageoverrides', $context);

// Display a list of overrides.
$PAGE->set_pagelayout('admin');
$PAGE->set_title(get_string('overrides', 'clatest'));
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();
echo $OUTPUT->heading(format_string($clatest->name, true, array('context' => $context)));

// Delete orphaned group overrides.
$sql = 'SELECT o.id
            FROM {clatest_overrides} o LEFT JOIN {groups} g
            ON o.groupid = g.id
...
if (!empty($orphaned)) {
    $DB->delete_records_list('clatest_overrides', 'id', array_keys($orphaned));
}
...
if ($groupmode) {
    ...
    $sql = 'SELECT o.*, g.name
                ...
                WHERE o.clatest = :clatestid
                ORDER BY g.name';
    $params = array('clatestid' => $clatest->id);
} else {
...
$overridedeleteurl = new moodle_url('/mod/clatest/overridedelete.php');
$overrideediturl = new moodle_url('/mod/clatest/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    if (!$groupmode) {
        if (!has_capability('mod/clatest:attempt', $context, $override->userid)) {
            // User not allowed to take the clatest.
            $active = false;
    ...
    if (isset($override->timeopen)) {
        $fields[] = get_string('clatestopens', 'clatest');
        $values[] = $override->timeopen > 0 ?
                userdate($override->timeopen) : get_string('noopen', 'clatest');
    }

    // Format timeclose.
    if (isset($override->timeclose)) {
        $fields[] = get_string('clatestcloses', 'clatest');
        $values[] = $override->timeclose > 0 ?
                userdate($override->timeclose) : get_string('noclose', 'clatest');
    }

    // Format timelimit.
    if (isset($override->timelimit)) {
        $fields[] = get_string('timelimit', 'clatest');
        $values[] = $override->timelimit > 0 ?
                format_time($override->timelimit) : get_string('none', 'clatest');
    }

    // Format number of attempts.
    if (isset($override->attempts)) {
        $fields[] = get_string('attempts', 'clatest');
    ...
    if (isset($override->password)) {
        $fields[] = get_string('requirepassword', 'clatest');
        $values[] = $override->password !== '' ?
                get_string('enabled', 'clatest') : get_string('none', 'clatest');
    }
...
echo html_writer::start_tag('div', array('id' => 'clatestoverrides'));
if (count($table->data)) {
    echo html_writer::table($table);
}
if ($hasinactive) {
    echo $OUTPUT->notification(get_string('inactiveoverridehelp', 'clatest'), 'dimmed_text');
}
...
if ($groupmode) {
    if (empty($groups)) {
        // There are no groups.
        echo $OUTPUT->notification(get_string('groupsnone', 'clatest'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'addgroup', 'cmid' => $cm->id)),
            get_string('addnewgroupoverride', 'clatest'), 'post', $options);
} else {
    $users = array();
    // See if there are any students in the clatest.
    $users = get_users_by_capability($context, 'mod/clatest:attempt', 'u.id');
    $info = new \core_availability\info_module($cm);
    $users = $info->filter_user_list($users);

    if (empty($users)) {
        // There are no students.
        echo $OUTPUT->notification(get_string('usersnone', 'clatest'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'adduser', 'cmid' => $cm->id)),
            get_string('addnewuseroverride', 'clatest'), 'get', $options);
}
