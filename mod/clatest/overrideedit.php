...
/**
 * This page handles editing and creation of clatest overrides
 *
 * @package   mod_clatest
...
if ($overrideid) {
    ...
    if (! $override = $DB->get_record('clatest_overrides', array('id' => $overrideid))) {
        print_error('invalidoverrideid', 'clatest');
    }
    if (! $clatest = $DB->get_record('clatest', array('id' => $override->clatest))) {
...
$url = new moodle_url('/mod/clatest/overrideedit.php');
if ($action) {
...
// Merge clatest defaults with data.
$keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
foreach ($keys as $key) {
    if (!isset($data->{$key}) || $reset) {
        $data->{$key} = $clatest->{$key};
    }
}
...
$overridelisturl = new moodle_url('/mod/clatest/overrides.php', array('cmid'=>$cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new clatest_override_form($url, $cm, $clatest, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->clatest = $clatest->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if ($fromform->{$key} == $clatest->{$key}) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('clatest_overrides', $conditions)) {
            ...
            // Set the course module id before calling clatest_delete_override().
            $clatest->cmid = $cm->id;
            clatest_delete_override($clatest, $oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('clatest_overrides', $fromform);
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('clatest_overrides', $fromform);
    ...
    if ($groupmode) {
        ...
        clatest_update_events($clatest, $fromform);
    }
...
// Print the form.
$pagetitle = get_string('editoverride', 'clatest');
