...
/**
 * Defines the Moodle forum used to add random questions to the clatest.
 *
 * @package   mod_clatest
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class clatest_add_random_form extends moodleform {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'existingcategoryheader',
                get_string('randomfromexistingcategory', 'clatest'));
        ...
        $mform->addElement('checkbox', 'includesubcategories', '', get_string('recurse', 'clatest'));
        ...
        $options = array(
            'multiple' => true,
            'noselectionstring' => get_string('anytags', 'clatest'),
        );
        $mform->addElement('autocomplete', 'fromtags', get_string('randomquestiontags', 'mod_clatest'), $tagstrings, $options);
        $mform->addHelpButton('fromtags', 'randomquestiontags', 'mod_clatest');

        $mform->addElement('select', 'numbertoadd', get_string('randomnumber', 'clatest'),
                $this->get_number_of_questions_to_add_choices());

        $previewhtml = $OUTPUT->render_from_template('mod_clatest/random_question_form_preview', []);
        $mform->addElement('html', $previewhtml);

        $mform->addElement('submit', 'existingcategory', get_string('addrandomquestion', 'clatest'));

        // Random from a new category section.
        $mform->addElement('header', 'newcategoryheader',
                get_string('randomquestionusinganewcategory', 'clatest'));
        ...
        $mform->addElement('submit', 'newcategory',
                get_string('createcategoryandaddrandomquestion', 'clatest'));
        ...
        $PAGE->requires->js_call_amd('mod_clatest/add_random_form', 'init', [
