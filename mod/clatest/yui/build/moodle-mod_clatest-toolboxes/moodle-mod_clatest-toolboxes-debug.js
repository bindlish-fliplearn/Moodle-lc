YUI.add('moodle-mod_clatest-toolboxes', function (Y, NAME) {
...
Y.extend(TOOLBOX, Y.Base, {
    ...
    send_request: function(data, statusspinner, success_callback, optionalconfig) {
        ...
        data.clatestid = this.get('clatestid');
                ...
                success: function(tid, response) {
                    ...
                    if (responsetext.hasOwnProperty('newnumquestions')) {
                        Y.one(SELECTOR.NUMQUESTIONS).setHTML(
                                M.util.get_string('numquestionsx', 'clatest', responsetext.newnumquestions)
                            );
                    }
                    if (success_callback) {
 ...
 *
 * This class is responsible for managing AJAX interactions with activities and resources
 * when viewing a clatest in editing mode.
 *
 * @module mod_clatest-resource-toolbox
 * @namespace M.mod_clatest.resource_toolbox
 */
...
Y.extend(RESOURCETOOLBOX, TOOLBOX, {
    ...
    initializer: function() {
        M.mod_clatest.clatestbase.register_module(this);
    ...
    delete_with_confirmation: function(ev, button, activity) {
        ...
        confirmstring = M.util.get_string('confirmremovequestion', 'clatest', qtypename);
        ...
        confirm.on('complete-yes', function() {
            ...
            var data = {
                'class': 'resource',
                'action': 'DELETE',
                'id': Y.Moodle.mod_clatest.util.slot.getId(element)
            };
            this.send_request(data, spinner, function(response) {
                if (response.deleted) {
                    // Actually remove the element.
                    Y.Moodle.mod_clatest.util.slot.remove(element);
    ...
    delete_multiple_with_confirmation: function(ev) {
        ...
        Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
            var slot = Y.Moodle.mod_clatest.util.slot.getSlotFromComponent(node);
            ids += ids === '' ? '' : ',';
            ids += Y.Moodle.mod_clatest.util.slot.getId(slot);
            slots.push(slot);
        });
        var element = Y.one('div.mod-clatest-edit-content');
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('areyousureremoveselected', 'clatest'),
            modal: true
        });
        ...
        confirm.on('complete-yes', function() {
            ...
            this.send_request(data, spinner, function(response) {
                ...
                if (response.deleted) {
                    // Actually remove the element.
                    Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
                        Y.Moodle.mod_clatest.util.slot.remove(node.ancestor('li.activity'));
                    });
    ...
    edit_maxmark: function(ev, button, activity) {
        ...
        var instancemaxmark = activity.one(SELECTOR.INSTANCEMAXMARK),
            ...
            data = {
                'class': 'resource',
                'field': 'getmaxmark',
                'id': Y.Moodle.mod_clatest.util.slot.getId(activity)
            };
    ...
    update_page_break: function(ev, button, activity, action) {
        ...
        var data = {
            ...
            'id':    Y.Moodle.mod_clatest.util.slot.getId(nextactivity),
            'value': value
        };
        ...
        this.send_request(data, spinner, function(response) {
            if (response.slots) {
                if (action === 'addpagebreak') {
                    Y.Moodle.mod_clatest.util.page.add(activity);
                } else {
                    var page = activity.next(Y.Moodle.mod_clatest.util.page.SELECTORS.PAGE);
                    Y.Moodle.mod_clatest.util.page.remove(page, true);
                }
                this.reorganise_edit_page();
            }
        });
    ...
    update_dependency: function(ev, button, activity, action) {
        ...
        this.send_request(data, spinner, function(response) {
            if (response.hasOwnProperty('requireprevious')) {
                Y.Moodle.mod_clatest.util.slot.updateDependencyIcon(activity, response.requireprevious);
            }
        });
    ...
    reorganise_edit_page: function() {
        Y.Moodle.mod_clatest.util.slot.reorderSlots();
        Y.Moodle.mod_clatest.util.slot.reorderPageBreaks();
        Y.Moodle.mod_clatest.util.page.reorderPages();
        Y.Moodle.mod_clatest.util.slot.updateOneSlotSections();
        Y.Moodle.mod_clatest.util.slot.updateAllDependencyIcons();
    },

    NAME: 'mod_clatest-resource-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_clatest.resource_toolbox = null;
M.mod_clatest.init_resource_toolbox = function(config) {
    M.mod_clatest.resource_toolbox = new RESOURCETOOLBOX(config);
    return M.mod_clatest.resource_toolbox;
};
/* global TOOLBOX, BODY, SELECTOR */
 ...
 *
 * @module moodle-mod_clatest-toolboxes
 * @namespace M.mod_clatest.toolboxes
 */
 ...
 *
 * This class is responsible for managing AJAX interactions with sections
 * when adding, editing, removing section headings when editing a clatest.
 *
 * @class section
 * @constructor
 * @extends M.mod_clatest.toolboxes.toolbox
 */
...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
    ...
    delete_section_with_confirmation: function(ev, button, activity) {
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('confirmremovesectionheading', 'clatest', activity.get('aria-label')),
            modal: true
        });
            ...
            this.send_request(data, spinner, function(response) {
                if (response) {
                    ...
                    activity.one(SELECTOR.EDITSECTIONICON).set('title',
                            M.util.get_string('sectionheadingedit', 'clatest', response.instancesection));
                    activity.one(SELECTOR.EDITSECTIONICON).set('alt',
                            M.util.get_string('sectionheadingedit', 'clatest', response.instancesection));
                    var deleteicon = activity.one(SELECTOR.DELETESECTIONICON);
                    if (deleteicon) {
                        deleteicon.set('title', M.util.get_string('sectionheadingremove', 'clatest', response.instancesection));
                        deleteicon.set('alt', M.util.get_string('sectionheadingremove', 'clatest', response.instancesection));
                    }
                }
            });
        }
    ...
    NAME: 'mod_clatest-section-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_clatest.init_section_toolbox = function(config) {
    ...
    "requires": [
        ...
        "event-key",
        "io",
        "moodle-mod_clatest-clatestbase",
        "moodle-mod_clatest-util-slot",
