YUI.add('moodle-mod_clatest-modform', function (Y, NAME) {

/**
 * The modform class has all the JavaScript specific to mod/clatest/mod_form.php.
 *
 * @module moodle-mod_clatest-modform
 */
...
Y.extend(MODFORM, Y.Base, {
    ...
    initializer: function() {
        ...
        if (!this.repaginateCheckbox) {
            // The checkbox only appears when editing an existing clatest.
            return;
        }
...
// Ensure that M.mod_clatest exists and that coursebase is initialised correctly
M.mod_clatest = M.mod_clatest || {};
M.mod_clatest.modform = M.mod_clatest.modform || new MODFORM();
M.mod_clatest.modform.init = function() {
    return new MODFORM();
};
