...
/**
 * Fallback page of /mod/clatest/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $clatest, $pagevars) =
        question_edit_setup('editq', '/mod/clatest/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/clatest/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new clatest_add_random_form(new moodle_url('/mod/clatest/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    clatest_add_random_questions($clatest, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories, $tagids);
...
// Setup $PAGE.
$streditingclatest = get_string('editinga', 'moodle', get_string('modulename', 'clatest'));
$PAGE->navbar->add($streditingclatest);
$PAGE->set_title($streditingclatest);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$clatestname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            print_error('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontoclatest', 'clatest', $clatestname), 2);
$mform->display();
echo $OUTPUT->footer();
