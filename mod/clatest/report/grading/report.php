...
/**
 * This file defines the clatest manual grading report class.
 *
...
require_once($CFG->dirroot . '/mod/clatest/report/grading/gradingsettings_form.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class clatest_grading_report extends clatest_default_report {
    ...
    public function display($clatest, $cm, $course) {
        ...
        require_capability('mod/clatest:grade', $this->context);
        $shownames = has_capability('clatest/grading:viewstudentnames', $this->context);
        $showidnumbers = has_capability('clatest/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this clatest.
        $this->questions = clatest_report_get_significant_questions($clatest);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'clatest_grading');
        }
        ...
        if ($this->currentgroup == self::NO_GROUPS_ALLOWED) {
            ...
            $this->userssql = get_enrolled_sql($this->context,
                    array('mod/clatest:reviewmyattempts', 'mod/clatest:attempt'), $this->currentgroup);
        }
        ...
        if ($slot && $hasquestions) {
            ...
            if (!$counts || $counts->$grade == 0) {
                redirect($this->list_questions_url(), get_string('alldoneredirecting', 'clatest_grading'));
            }
        }

        // Start output.
        $this->print_header_and_tabs($cm, $course, $clatest, 'grading');
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "clatesta.clatest = :mangrclatestid AND
                clatesta.preview = 0 AND
                clatesta.state = :statefinished";
        $params = array('mangrclatestid' => $this->cm->instance, 'statefinished' => clatest_attempt::FINISHED);
        ...
        if ($currentgroup) {
            ...
            if ($enrolleduserscount < 1) {
                $where .= ' AND clatesta.userid = 0';
            } else {
                $usersjoin = "JOIN ({$userssql[0]}) AS enr ON clatesta.userid = enr.id";
                $params += $userssql[1];
            }
        }

        return new qubaid_join("{clatest_attempts} clatesta $usersjoin ", 'clatesta.uniqueid', $where, $params);
    }
    ...
    protected function load_attempts_by_usage_ids($qubaids) {
        ...
        $params[] = clatest_attempt::FINISHED;
        $params[] = $this->clatest->id;

        $fields = 'clatesta.*, u.idnumber, ';
        $fields .= get_all_user_name_fields(true, 'u');
        $attemptsbyid = $DB->get_records_sql("
                SELECT $fields
                FROM {clatest_attempts} clatesta
                JOIN {user} u ON u.id = clatesta.userid
                WHERE clatesta.uniqueid $asql AND clatesta.state = ? AND clatesta.clatest = ?",
                $params);
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                    ...
                    get_string($gradestring, 'clatest_grading'),
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'clatest_grading'), 3);
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'clatest_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'clatest_grading');
        }
        ...
        if (empty($data)) {
            echo $OUTPUT->notification(get_string('nothingfound', 'clatest_grading'));
            return;
        }
        ...
        $table->head[] = get_string('qno', 'clatest_grading');
        $table->head[] = get_string('qtypeveryshort', 'question');
        $table->head[] = get_string('questionname', 'clatest_grading');
        $table->head[] = get_string('tograde', 'clatest_grading');
        $table->head[] = get_string('alreadygraded', 'clatest_grading');
        if ($includeauto) {
            $table->head[] = get_string('automaticallygraded', 'clatest_grading');
        }
        $table->head[] = get_string('total', 'clatest_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new clatest_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'clatest_grading', $a), 3);
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(),
                get_string('backtothelistofquestions', 'clatest_grading')),
        ...
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'clatest_grading', $a), 3);
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = clatest_get_review_options($this->clatest, $attempt, $this->context);
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                'type' => 'submit', 'class' => 'btn btn-primary', 'value' => get_string('saveandnext', 'clatest_grading'))),
                array('class' => 'mdl-align')) .
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'clatest_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'clatest_grading', $a);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new clatest_attempt($attempt, $this->clatest, $this->cm, $this->course);
            ...
            $params = array(
                ...
                'other' => array(
                    'clatestid' => $attemptobj->get_clatestid(),
                    'attemptid' => $attemptobj->get_attemptid(),
                    'slot' => $assumedslotforevents
                )
            );
            $events[] = \mod_clatest\event\question_manually_graded::create($params);
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        if ($orderby == 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON clatesta.userid = u.id ";
