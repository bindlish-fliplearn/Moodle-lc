...
/**
 * This file defines the clatest overview report class.
 *
 * @package   clatest_overview
...
require_once($CFG->dirroot . '/mod/clatest/report/attemptsreport.php');
require_once($CFG->dirroot . '/mod/clatest/report/overview/overview_options.php');
require_once($CFG->dirroot . '/mod/clatest/report/overview/overview_form.php');
require_once($CFG->dirroot . '/mod/clatest/report/overview/overview_table.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class clatest_filp_overview_report extends clatest_attempts_report {

    public function display($clatest, $cm, $course) {
        global $DB, $OUTPUT, $PAGE;

        list($currentgroup, $studentsjoins, $groupstudentsjoins, $allowedjoins) = $this->init(
                'overview', 'clatest_overview_settings_form', $clatest, $cm, $course);

        $options = new clatest_overview_options('overview', $clatest, $cm, $course);
        ...
        $table = new clatest_overview_table($clatest, $this->context, $this->qmsubselect,
                $options, $groupstudentsjoins, $studentsjoins, $questions, $options->get_url());
//        $filename = clatest_report_download_filename(get_string('overviewfilename', 'clatest_overview'),
//                $courseshortname, $clatest->name);
//        $table->is_downloading($options->download, $filename,
//                $courseshortname . ' ' . format_string($clatest->name, true));
...
//        }
        ...
        if ($options->attempts == self::ALL_WITH) {
            ...
            // groups mode, so setting allowed to empty (which means all clatest attempts
            // are accessible, is not a security porblem.
            $allowedjoins = new \core\dml\sql_join();
        }

        $this->course = $course; // Hack to make this available in process_actions.
        $this->process_actions($clatest, $cm, $currentgroup, $groupstudentsjoins, $allowedjoins, $options->get_url());

        $hasquestions = clatest_has_questions($clatest->id);

        // Start output.
        if (!$table->is_downloading()) {
            // Only print headers if not asked to download data.
            $this->print_standard_header_and_messages($cm, $course, $clatest,
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
            ...
            if (!$table->is_downloading()) {
                ...
                if (has_capability('mod/clatest:regrade', $this->context)) {
                    $regradesneeded = $this->count_question_attempts_needing_regrade(
                            $clatest, $groupstudentsjoins);
//                    if ($currentgroup) {
//                        $a= new stdClass();
...
//                                get_string('regradealldrydogroup', 'clatest_overview', $a);
//                        $regradealldrylabel =
//                                get_string('regradealldrygroup', 'clatest_overview', $a);
//                        $regradealllabel =
//                                get_string('regradeallgroup', 'clatest_overview', $a);
//                    } else {
//                        $regradealldrydolabel =
//                                get_string('regradealldrydo', 'clatest_overview', $regradesneeded);
//                        $regradealldrylabel =
//                                get_string('regradealldry', 'clatest_overview');
//                        $regradealllabel =
//                                get_string('regradeall', 'clatest_overview');
//                    }
...
//                    echo '</div>';
                ...
                if ($strattempthighlight = clatest_report_highlighting_grading_method(
                        $clatest, $this->qmsubselect, $options->onlygraded)) {
                    echo '<div class="clatestattemptcounts">' . $strattempthighlight . '</div>';
                }
            }
...
//            $this->add_time_columns($columns, $headers);
            ...
            $this->add_grade_columns($clatest, $options->usercanseegrades, $columns, $headers, false);

            if (!$table->is_downloading() && has_capability('mod/clatest:regrade', $this->context) &&
                    $this->has_regraded_questions($table->sql->from, $table->sql->where, $table->sql->params)) {
                $columns[] = 'regraded';
                $headers[] = get_string('regrade', 'clatest_overview');
            }
            ...
            if ($options->slotmarks) {
                foreach ($questions as $slot => $question) {
                    ...
                    $header = get_string('qbrief', 'clatest', $question->number);
                    if (!$table->is_downloading()) {
                        $header .= '<br />';
                    } else {
                        $header .= ' ';
                    }
                    $header .= '/' . clatest_rescale_grade($question->maxmark, $clatest, 'question');
                    $headers[] = $header;
                }
            }
        ...
        if (!$table->is_downloading() && $options->usercanseegrades) {
            ...
            list($bands, $bandwidth) = self::get_bands_count_and_width($clatest);
            $labels = self::get_bands_labels($bands, $bandwidth, $clatest);
            ...
            if ($currentgroup && $this->hasgroupstudents) {
                $sql = "SELECT qg.id
                        ...
                        {$groupstudentsjoins->joins}
                          WHERE qg.clatest = $clatest->id AND {$groupstudentsjoins->wheres}";
                if ($DB->record_exists_sql($sql, $groupstudentsjoins->params)) {
                    $data = clatest_report_grade_bands($bandwidth, $bands, $clatest->id, $groupstudentsjoins);
                    $chart = self::get_chart($labels, $data);
                    $graphname = get_string('overviewreportgraphgroup', 'clatest_overview', groups_get_group_name($currentgroup));
                    //echo $output->chart($chart, $graphname);
                }
            }

            if ($DB->record_exists('clatest_grades', array('clatest'=> $clatest->id))) {
                $data = clatest_report_grade_bands($bandwidth, $bands, $clatest->id, new \core\dml\sql_join());
                $chart = self::get_chart($labels, $data);
                $graphname = get_string('overviewreportgraph', 'clatest_overview');
                //echo $output->chart($chart, $graphname);
            }
        }
        return true;
    ...
    protected function process_actions($clatest, $cm, $currentgroup, \core\dml\sql_join $groupstudentsjoins,
        ...
        parent::process_actions($clatest, $cm, $currentgroup, $groupstudentsjoins, $allowedjoins, $redirecturl);

        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('regrade', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    $this->start_regrade($clatest, $cm);
                    $this->regrade_attempts($clatest, false, $groupstudentsjoins, $attemptids);
        ...
        if (optional_param('regradeall', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($clatest, $cm);
            $this->regrade_attempts($clatest, false, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);

        } else if (optional_param('regradealldry', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($clatest, $cm);
            $this->regrade_attempts($clatest, true, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);

        } else if (optional_param('regradealldrydo', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($clatest, $cm);
            $this->regrade_attempts_needing_it($clatest, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);
        }
    }
     ...
     */
    protected function start_regrade($clatest, $cm) {
        require_capability('mod/clatest:regrade', $this->context);
        $this->print_header_and_tabs($cm, $this->course, $clatest, $this->mode);
    }
    ...
    protected function finish_regrade($nexturl) {
        global $OUTPUT;
        \core\notification::success(get_string('regradecomplete', 'clatest_overview'));
    ...
    /**
     * Regrade a particular clatest attempt. Either for real ($dryrun = false), or
     * as a pretend regrade to see which fractions would change. The outcome is
     * stored in the clatest_overview_regrades table.
     *
     ...
     *
     * @param object $attempt the clatest attempt to regrade.
    ...
    protected function regrade_attempt($attempt, $dryrun = false, $slots = null) {
        global $DB;
        // Need more time for a clatest with many questions.
        ...
        $finished = $attempt->state == clatest_attempt::FINISHED;
        foreach ($slots as $slot) {
            ...
            if (abs($qqr->oldfraction - $qqr->newfraction) > 1e-7) {
                ...
                $DB->insert_record('clatest_overview_regrades', $qqr, false);
            }
        }
    ...
    /**
     * Regrade attempts for this clatest, exactly which attempts are regraded is
     ...
     */
    protected function regrade_attempts($clatest, $dryrun = false,
        ...
        $where = "clatest = :qid AND preview = 0";
        $params = array('qid' => $clatest->id);
        ...
        $this->clear_regrade_table($clatest, $groupstudentsjoins);

        $progressbar = new progress_bar('clatest_overview_regrade', 500, true);
        $a = array(
        ...
        foreach ($attempts as $attempt) {
            ...
            $progressbar->update($a['done'], $a['count'],
                    get_string('regradingattemptxofy', 'clatest_overview', $a));
        }

        if (!$dryrun) {
            $this->update_overall_grades($clatest);
        }
    }

    /**
     * Regrade those questions in those attempts that are marked as needing regrading
     * in the clatest_overview_regrades table.
     * @param object $clatest the clatest settings.
     * @param \core\dml\sql_join $groupstudentsjoins empty for all attempts, otherwise regrade attempts
     * for these users.
     */
    protected function regrade_attempts_needing_it($clatest, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        $this->unlock_session();

        $join = '{clatest_overview_regrades} qqr ON qqr.questionusageid = clatesta.uniqueid';
        $where = "clatesta.clatest = :qid AND clatesta.preview = 0 AND qqr.regraded = 0";
        $params = array('qid' => $clatest->id);
        ...
        $toregrade = $DB->get_recordset_sql("
                SELECT clatesta.uniqueid, qqr.slot
                  FROM {clatest_attempts} clatesta
                  JOIN $join
                 WHERE $where", $params);
        ...
        $attempts = $DB->get_records_list('clatest_attempts', 'uniqueid',
     ...
     */
    protected function count_question_attempts_needing_regrade($clatest, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        ...
        if ($this->hasgroupstudents) {
            $userjoin = "JOIN {user} u ON u.id = clatesta.userid
                    {$groupstudentsjoins->joins}";
            $usertest = "{$groupstudentsjoins->wheres} AND u.id = clatesta.userid AND ";
            $params = $groupstudentsjoins->params;
        }

        $params['cclatest'] = $clatest->id;
        $sql = "SELECT COUNT(DISTINCT clatesta.id)
                  FROM {clatest_attempts} clatesta
                  JOIN {clatest_overview_regrades} qqr ON clatesta.uniqueid = qqr.questionusageid
                $userjoin
                 WHERE
                      $usertest
                      clatesta.clatest = :cclatest AND
    ...
    protected function has_regraded_questions($from, $where, $params) {
        ...
        return $DB->record_exists_sql("
                SELECT 1
                  FROM {$from}
                  JOIN {clatest_overview_regrades} qor ON qor.questionusageid = clatesta.uniqueid
                 WHERE {$where}", $params);
    }
     ...
     * to these users is cleared.
     */
    protected function clear_regrade_table($clatest, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        ...
        $select = "questionusageid IN (
                    SELECT uniqueid
                      FROM {clatest_attempts} clatesta";
        $where = "WHERE clatesta.clatest = :qid";
        $params = array('qid' => $clatest->id);
        if ($this->hasgroupstudents && !empty($groupstudentsjoins->joins)) {
            $select .= "\nJOIN {user} u ON u.id = clatesta.userid
                    {$groupstudentsjoins->joins}";
        ...
        $DB->delete_records_select('clatest_overview_regrades', $select, $params);
    }
     ...
     */
    protected function update_overall_grades($clatest) {
    ...
    /**
     * Get the bands configuration for the clatest.
     *
     * This returns the configuration for having between 11 and 20 bars in
     * a chart based on the maximum grade to be given on a clatest. The width of
     ...
     */
    public static function get_bands_count_and_width($clatest) {
        $bands = $clatest->grade;
        ...
        return [$bands, $clatest->grade / $bands];
    }
     ...
     */
    public static function get_bands_labels($bands, $bandwidth, $clatest) {
        $bandlabels = [];
        for ($i = 1; $i <= $bands; $i++) {
            $bandlabels[] = clatest_format_grade($clatest, ($i - 1) * $bandwidth) . ' - ' . clatest_format_grade($clatest, $i * $bandwidth);
        }
        return $bandlabels;
    }
