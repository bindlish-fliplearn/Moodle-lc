...
/**
 * This file defines the clatest grades table.
 *
...
require_once($CFG->dirroot . '/mod/clatest/report/attemptsreport_table.php');


/**
 * This is a table subclass for displaying the clatest grades report.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class clatest_overview_table extends clatest_attempts_report_table {
     ...
     * @param clatest_overview_options $options
     * @param \core\dml\sql_join $groupstudentsjoins
     * @param \core\dml\sql_join $studentsjoins
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($clatest, $context, $qmsubselect,
            clatest_overview_options $options, \core\dml\sql_join $groupstudentsjoins,
            \core\dml\sql_join $studentsjoins, $questions, $reporturl) {
        parent::__construct('mod-clatest-report-overview-report', $clatest , $context,
    ...
    /**
     * Calculate the average overall and question scores for a set of attempts at the clatest.
     *
    ...
    public function compute_average_row($label, \core\dml\sql_join $usersjoins) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(clatestaouter.sumgrades) AS grade, COUNT(clatestaouter.sumgrades) AS numaveraged
                  FROM {clatest_attempts} clatestaouter
                  JOIN (
                       SELECT DISTINCT clatesta.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON clatestaouter.id = relevant_attempt_ids.id
                ", $params);
        $record->grade = clatest_rescale_grade($record->grade, $this->clatest, false);
        ...
        $averagerow = array(
            ...
            'feedbacktext' => strip_tags(clatest_report_feedback_for_grade(
                                         $record->grade, $this->clatest->id, $this->context))
        );

        if ($this->options->slotmarks) {
            $dm = new question_engine_data_mapper();
            $qubaids = new qubaid_join("{clatest_attempts} clatestaouter
                  JOIN (
                       SELECT DISTINCT clatesta.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON clatestaouter.id = relevant_attempt_ids.id",
                    'clatestaouter.uniqueid', '1 = 1', $params);
    ...
    protected function format_average_grade_for_questions($gradeaverages) {
        ...
        foreach ($this->questions as $question) {
            if (isset($gradeaverages[$question->slot]) && $question->maxmark > 0) {
                ...
                $record->grade = clatest_rescale_grade(
                        $record->averagefraction * $question->maxmark, $this->clatest, false);
    ...
    protected function format_average($record, $question = false) {
        if (is_null($record->grade)) {
            ...
            $average = clatest_format_question_grade($this->clatest, $record->grade);
        } else {
            $average = clatest_format_grade($this->clatest, $record->grade);
        }
    ...
    protected function submit_buttons() {
        if (has_capability('mod/clatest:regrade', $this->context)) {
            echo '<input type="submit" class="btn btn-secondary m-r-1" name="regrade" value="' .
                    get_string('regradeselected', 'clatest_overview') . '"/>';
        }
    ...
    public function col_sumgrades($attempt) {
        ...
        $grade = clatest_rescale_grade($attempt->sumgrades, $this->clatest);
        if ($this->is_downloading()) {
            return $grade;
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid])) {
            ...
            $newsumgrade = clatest_rescale_grade($newsumgrade, $this->clatest);
            $oldsumgrade = clatest_rescale_grade($oldsumgrade, $this->clatest);
            $grade = html_writer::tag('del', $oldsumgrade) . '/' .
                    html_writer::empty_tag('br') . $newsumgrade;
        }
        return html_writer::link(new moodle_url('/mod/clatest/review.php',
                array('attempt' => $attempt->attempt)), $grade,
                array('title' => get_string('reviewattempt', 'clatest')));
    }

    /**
     * @param string $colname the name of the column.
     * @param object $attempt the row of data - see the SQL in display() in
     * mod/clatest/report/overview/report.php to see what fields are present,
    ...
    public function other_cols($colname, $attempt) {
        ...
        if ($question->maxmark == 0) {
            ...
            $grade = clatest_rescale_grade(
                    $stepdata->fraction * $question->maxmark, $this->clatest, 'question');
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid][$slot])) {
            $gradefromdb = $grade;
            $newgrade = clatest_rescale_grade(
                    $this->regradedqs[$attempt->usageid][$slot]->newfraction * $question->maxmark,
                    $this->clatest, 'question');
            $oldgrade = clatest_rescale_grade(
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'clatest_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'clatest_overview');
        }
    }
    ...
    protected function update_sql_after_count($fields, $from, $where, $params) {
        $fields .= ", COALESCE((
                                SELECT MAX(qqr.regraded)
                                  FROM {clatest_overview_regrades} qqr
                                 WHERE qqr.questionusageid = clatesta.uniqueid
                          ), -1) AS regraded";
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/clatest:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('clatest_overview_regrades',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return clatest_report_index_by_keys($regradedqs, array('questionusageid', 'slot'));
    }
}
