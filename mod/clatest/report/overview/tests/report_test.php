...
/**
 * Tests for the clatest overview report.
 *
...
require_once($CFG->dirroot . '/mod/clatest/report/overview/report.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class clatest_overview_report_testcase extends advanced_testcase {
    ...
    public function test_report_sql($isdownloading) {
        ...
        // Create a course and a clatest.
        $generator = $this->getDataGenerator();
        $course = $generator->create_course();
        $clatestgenerator = $generator->get_plugin_generator('mod_clatest');
        $clatest = $clatestgenerator->create_instance(array('course' => $course->id,
        ...
        clatest_add_clatest_question($q->id, $clatest, 0 , 10);
        ...
        $attempts = array(
            array($clatest, $student1, 1, 0.0,  clatest_attempt::FINISHED),
            array($clatest, $student1, 2, 5.0,  clatest_attempt::FINISHED),
            array($clatest, $student1, 3, 8.0,  clatest_attempt::FINISHED),
            array($clatest, $student1, 4, null, clatest_attempt::ABANDONED),
            array($clatest, $student1, 5, null, clatest_attempt::IN_PROGRESS),
            array($clatest, $student2, 1, null, clatest_attempt::ABANDONED),
            array($clatest, $student2, 2, null, clatest_attempt::ABANDONED),
            array($clatest, $student2, 3, 7.0,  clatest_attempt::FINISHED),
            array($clatest, $student2, 4, null, clatest_attempt::ABANDONED),
            array($clatest, $student2, 5, null, clatest_attempt::ABANDONED),
        );

        // Load it in to clatest attempts table.
        foreach ($attempts as $attemptdata) {
            list($clatest, $student, $attemptnumber, $sumgrades, $state) = $attemptdata;
            ...
            $attempt = clatest_create_attempt($clatestobj, $attemptnumber, null, $timestart, false, $student->id);

            $attempt = clatest_start_new_attempt($clatestobj, $quba, $attempt, $attemptnumber, $timestamp);
            ...
            switch ($state) {
                ...
                case clatest_attempt::FINISHED:
                    ...
                    $DB->update_record('clatest_attempts', $update);
                    clatest_save_best_grade($attemptobj->get_clatest(), $student->id);
                    break;
            }
        }
        ...
        $qmsubselect = clatest_report_qm_filter_select($clatest);
        $studentsjoins = get_enrolled_with_capabilities_join($context);
        $empty = new \core\dml\sql_join();

        // Set the options.
        $reportoptions = new clatest_overview_options('overview', $clatest, $cm, null);
        $reportoptions->attempts = clatest_attempts_report::ENROLLED_ALL;
        $reportoptions->onlygraded = true;
        $reportoptions->states = array(clatest_attempt::IN_PROGRESS, clatest_attempt::OVERDUE, clatest_attempt::FINISHED);

        // Now do a minimal set-up of the table class.
        $q->slot = 1;
        $q->maxmark = 10;
        $table = new clatest_overview_table($clatest, $context, $qmsubselect, $reportoptions,
        ...
        $table->define_baseurl(new moodle_url('/mod/clatest/report.php'));
        $table->setup();
    ...
    public function test_get_bands_count_and_width($grade, $expected) {
        ...
        $clatest = $clatestgenerator->create_instance(['course' => SITEID, 'grade' => $grade]);
        $this->assertEquals($expected, clatest_overview_report::get_bands_count_and_width($clatest));
    }
