...
/**
 * Base class for the table used by a {@link clatest_attempts_report}.
 *
 * @package   mod_clatest
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class clatest_attempts_report_table extends table_sql {
    ...
    /** @var object the clatest settings for the clatest we are reporting on. */
    protected $clatest;
    ...
    /** @var object mod_clatest_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this clatest.. */
    protected $questions;
     ...
     * @param mod_clatest_attempts_report_options $options
     ...
     */
    public function __construct($uniqueid, $clatest, $context, $qmsubselect,
            mod_clatest_attempts_report_options $options, \core\dml\sql_join $groupstudentsjoins, \core\dml\sql_join $studentsjoins,
            $questions, $reporturl) {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/clatest/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'clatest'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return clatest_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = clatest_report_feedback_for_grade(
                clatest_rescale_grade($attempt->sumgrades, $this->clatest, false),
                $this->clatest->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/clatest/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'clatest')));
    ...
    public function base_sql(\core\dml\sql_join $allowedstudentsjoins) {
        ...
        // Please note this uniqueid column is not the same as clatesta.uniqueid.
        $fields = 'DISTINCT ' . $DB->sql_concat('u.id', "'#'", 'COALESCE(clatesta.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                clatesta.uniqueid AS usageid,
                clatesta.id AS attempt,
                u.id AS userid,
                u.idnumber, ' . $allnames . ',
                u.picture,
                u.imagealt,
                ...
                clatesta.timefinish,
                clatesta.timestart,
                CASE WHEN clatesta.timefinish = 0 THEN null
                     WHEN clatesta.timefinish > clatesta.timestart THEN clatesta.timefinish - clatesta.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and clatest_attempts tables.
        $from = " {user} u";
        $from .= "\nLEFT JOIN {clatest_attempts} clatesta ON
                                    clatesta.userid = u.id AND clatesta.clatest = :clatestid";
        $params = array('clatestid' => $this->clatest->id);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (clatesta.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = clatest_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case clatest_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'clatesta.id IS NOT NULL AND clatesta.preview = 0';
                break;
            case clatest_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "clatesta.preview = 0 AND clatesta.id IS NOT NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case clatest_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "clatesta.id IS NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case clatest_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "(clatesta.preview = 0 OR clatesta.preview IS NULL) AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (clatesta.state $statesql OR clatesta.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{clatest_attempts} {$alias}clatesta", "{$alias}clatesta.uniqueid",
                "{$alias}clatesta.clatest = :{$alias}clatestid", array("{$alias}clatestid" => $this->sql->params['clatestid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = clatesta.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join("(
                SELECT DISTINCT clatesta.uniqueid
                  FROM " . $this->sql->from . "
                 WHERE " . $this->sql->where . "
                    ) clatestasubquery", 'clatestasubquery.uniqueid',
                    "1 = 1", $this->sql->params);
        }
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['clatesta.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    public function wrap_html_finish() {
        ...
        echo '<a id="checkattempts" href="#">' .
                get_string('selectall', 'clatest') . '</a> / ';
        echo '<a id="uncheckattempts" href="#">' .
                get_string('selectnone', 'clatest') . '</a> ';
