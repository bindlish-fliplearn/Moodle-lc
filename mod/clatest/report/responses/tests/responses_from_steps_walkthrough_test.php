 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class clatest_report_responses_from_steps_testcase extends mod_clatest_attempt_walkthrough_from_csv_testcase {
     ...
     *
     * @param array $clatestsettings settings to override default settings for clatest created by generator. Taken from clatestzes.csv.
    ...
    public function test_walkthrough_from_csv($clatestsettings, $csvdata) {
        ...
        $clatestattemptids = $this->walkthrough_attempts($csvdata['steps']);
        ...
        for ($rowno = 0; $rowno < $csvdata['responses']->getRowCount(); $rowno++) {
            ...
            if (!isset($clatestattemptids[$responses['clatestattempt']])) {
                throw new coding_exception("There is no clatestattempt {$responses['clatestattempt']}!");
            }
            $this->assert_response_test($clatestattemptids[$responses['clatestattempt']], $responses);
        }
    }

    protected function assert_response_test($clatestattemptid, $responses) {
        $clatestattempt = clatest_attempt::create($clatestattemptid);
        ...
        foreach ($responses['slot'] as $slot => $tests) {
            ...
            $qa = $clatestattempt->get_question_attempt($slot);
            ...
            if (null === $step) {
                throw new coding_exception("There is no step no {$responses['submittedstepno']} ".
                                           "for slot $slot in clatestattempt {$responses['clatestattempt']}!");
            }
            foreach (array('responsesummary', 'fraction', 'state') as $column) {
                if (isset($tests[$column]) && $tests[$column] != '') {
                    ...
                    $failuremessage = "Error in  clatestattempt {$responses['clatestattempt']} in $column, slot $slot, ".
