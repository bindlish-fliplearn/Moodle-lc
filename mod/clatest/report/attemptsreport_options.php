...
/**
 * Base class for the options that control what is visible in an {@link clatest_attempts_report}.
 *
 * @package   mod_clatest
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_clatest_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the clatest being reported on. */
    public $clatest;

    /** @var object the course module objects for the clatest being reported on. */
    public $cm;

    /** @var object the course settings for the course the clatest is in. */
    public $course;

    /**
     * @var array form field name => corresponding clatest_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => clatest_attempt::IN_PROGRESS,
        'stateoverdue'    => clatest_attempt::OVERDUE,
        'statefinished'   => clatest_attempt::FINISHED,
        'stateabandoned'  => clatest_attempt::ABANDONED,
    );

    /**
     * @var string clatest_attempts_report::ALL_WITH or clatest_attempts_report::ENROLLED_WITH
     *      clatest_attempts_report::ENROLLED_WITHOUT or clatest_attempts_report::ENROLLED_ALL
     */
    public $attempts = clatest_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of clatest_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(clatest_attempt::IN_PROGRESS, clatest_attempt::OVERDUE,
            clatest_attempt::FINISHED, clatest_attempt::ABANDONED);
    ...
    public $pagesize = clatest_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $clatest the settings for the clatest being reported on.
     * @param object $cm the course module objects for the clatest being reported on.
     * @param object $coures the course settings for the coures this clatest is in.
     */
    public function __construct($mode, $clatest, $cm, $course) {
        $this->mode   = $mode;
        $this->clatest   = $clatest;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = clatest_report_should_show_grades($clatest, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/clatest/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('clatest_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('clatest_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == clatest_attempts_report::ALL_WITH) {
                $this->attempts = clatest_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = clatest_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(clatest_attempts_report::ALL_WITH, clatest_attempts_report::ENROLLED_WITH,
                clatest_attempts_report::ENROLLED_WITHOUT, clatest_attempts_report::ENROLLED_ALL))) {
            $this->attempts = clatest_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!clatest_report_can_filter_only_graded($this->clatest)) {
        ...
        if ($this->attempts == clatest_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = clatest_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(clatest_attempt::FINISHED, $this->states);
    }
}
