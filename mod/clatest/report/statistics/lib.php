...
/**
 * Standard plugin entry points of the clatest statistics report.
 *
 ...
 * @package  clatest_statistics
 * @category files
 * @param context $previewcontext the clatest context
 ...
 */
function clatest_statistics_question_preview_pluginfile($previewcontext, $questionid,
    ...
    require_capability('clatest/statistics:view', $context);
 ...
 */
function clatest_statistics_cron() {
    global $DB;

    mtrace("\n  Cleaning up old clatest statistics cache records...", '');

    $expiretime = time() - 5*HOURSECS;
    $DB->delete_records_select('clatest_statistics', 'timemodified < ?', array($expiretime));
