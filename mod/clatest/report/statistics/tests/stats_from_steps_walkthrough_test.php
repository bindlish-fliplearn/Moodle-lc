 ...
 *
 * The clatest stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-clatest-tools/tree/master/statsspreadsheet
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/clatest/tests/attempt_walkthrough_from_csv_test.php');
require_once($CFG->dirroot . '/mod/clatest/report/default.php');
require_once($CFG->dirroot . '/mod/clatest/report/statistics/report.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class clatest_report_statistics_from_steps_testcase extends mod_clatest_attempt_walkthrough_from_csv_testcase {

    /**
     * @var clatest_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    /**
     * Create a clatest add questions to it, walk through clatest attempts and then check results.
     *
    ...
    public function test_walkthrough_from_csv($clatestsettings, $csvdata) {
        ...
        list($questions, $clateststats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata,
                            $whichattempts, $whichtries, $groupstudentsjoins);
        if ($clatestsettings['testnumber'] === '00') {
            $this->check_variants_count_for_clatest_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_clatest_stats_for_clatest_00($clateststats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_clatest_00($questions, $questionstats, $whichtries, $qubaids) {
     ...
     * @param $clateststats
     */
    protected function check_clatest_stats_for_clatest_00($clateststats) {
        $clateststatsexpected = array(
            'median'             => 4.5,
        ...
        foreach ($clateststatsexpected as $statname => $statvalue) {
            $this->assertEquals($statvalue, $clateststats->$statname, $clateststats->$statname, abs($statvalue) * 1.5e-5);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $clateststats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries,
        ...
        $this->report = new clatest_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->clatest);
        list($clateststats, $questionstats) = $this->report->get_all_stats_and_analysis($this->clatest,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudentsjoins,
                                                                                     $questions);

        $qubaids = clatest_statistics_qubaids_condition($this->clatest->id, $groupstudentsjoins, $whichattempts);

        // We will create some clatest and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $clatestcalc = new \clatest_statistics\calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($clatestcalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $clateststats, $questionstats, $qubaids);
        }
        return array($questions, $clateststats, $questionstats, $qubaids);
    }
