...
class calculated {
     ...
     */
    public function get_formatted_clatest_info_data($course, $cm, $clatest) {
        ...
        // General information about the clatest.
        $clatestinfo = array();
        $clatestinfo[get_string('clatestname', 'clatest_statistics')] = format_string($clatest->name);
        $clatestinfo[get_string('coursename', 'clatest_statistics')] = format_string($course->fullname);
        if ($cm->idnumber) {
            $clatestinfo[get_string('idnumbermod')] = $cm->idnumber;
        }
        if ($clatest->timeopen) {
            $clatestinfo[get_string('clatestopen', 'clatest')] = userdate($clatest->timeopen);
        }
        if ($clatest->timeclose) {
            $clatestinfo[get_string('clatestclose', 'clatest')] = userdate($clatest->timeclose);
        }
        if ($clatest->timeopen && $clatest->timeclose) {
            $clatestinfo[get_string('duration', 'clatest_statistics')] =
                format_time($clatest->timeclose - $clatest->timeopen);
        }
        ...
        foreach ($todisplay as $property => $format) {
            ...
            switch ($format) {
                case 'summarks_as_percentage':
                    $formattedvalue = clatest_report_scale_summarks_as_percentage($value, $clatest);
                    break;
                case 'number_format_percent':
                    $formattedvalue = clatest_format_grade($clatest, $value) . '%';
                    break;
                case 'number_format':
                    // 2 extra decimal places, since not a percentage,
                    // and we want the same number of sig figs.
                    $formattedvalue = format_float($value, $clatest->decimalpoints + 2);
                    break;
                case 'number':
            ...
            $clatestinfo[get_string($property, 'clatest_statistics',
    ...
    public function cache($qubaids) {
        ...
        // Store the data.
        $DB->insert_record('clatest_statistics', $toinsert);

    }

    /**
     * Given a record from 'clatest_statistics' table load the data into the properties of this class.
     *
