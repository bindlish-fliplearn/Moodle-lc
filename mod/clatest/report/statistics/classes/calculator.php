...
namespace clatest_statistics;
defined('MOODLE_INTERNAL') || die();

/**
 * Class to calculate and also manage caching of clatest statistics.
 *
 * These clatest statistics calculations are described here :
 *
...
class calculator {
    ...
    /**
     * Compute the clatest statistics.
     *
     ...
     * @return calculated $clateststats The statistics for overall attempt scores.
     */
    public function calculate($clatestid, $whichattempts, \core\dml\sql_join $groupstudentsjoins, $p, $sumofmarkvariance) {

        $this->progress->start_progress('', 3);

        $clateststats = new calculated($whichattempts);

        $countsandaverages = $this->attempt_counts_and_averages($clatestid, $groupstudentsjoins);
        $this->progress->progress(1);

        foreach ($countsandaverages as $propertyname => $value) {
            $clateststats->{$propertyname} = $value;
        }
        ...
        if ($s != 0) {
            ...
            list($fromqa, $whereqa, $qaparams) =
                clatest_statistics_attempts_sql($clatestid, $groupstudentsjoins, $whichattempts);

            $clateststats->median = $this->median($s, $fromqa, $whereqa, $qaparams);
            $this->progress->progress(2);

            if ($s > 1) {

                $powers = $this->sum_of_powers_of_difference_to_mean($clateststats->avg(), $fromqa, $whereqa, $qaparams);
                $this->progress->progress(3);

                $clateststats->standarddeviation = sqrt($powers->power2 / ($s - 1));
                ...
                if ($s > 2) {
                    ...
                    $k3 = $s * $s * $m3 / (($s - 1) * ($s - 2));
                    if ($k2 != 0) {
                        $clateststats->skewness = $k3 / (pow($k2, 3 / 2));

                        // Kurtosis.
                        if ($s > 3) {
                            $k4 = $s * $s * ((($s + 1) * $m4) - (3 * ($s - 1) * $m2 * $m2)) / (($s - 1) * ($s - 2) * ($s - 3));
                            $clateststats->kurtosis = $k4 / ($k2 * $k2);
                        }

                        if ($p > 1) {
                            $clateststats->cic = (100 * $p / ($p - 1)) * (1 - ($sumofmarkvariance / $k2));
                            $clateststats->errorratio = 100 * sqrt(1 - ($clateststats->cic / 100));
                            $clateststats->standarderror = $clateststats->errorratio *
                                $clateststats->standarddeviation / 100;
                        }
                    }
            ...
            $clateststats->cache(clatest_statistics_qubaids_condition($clatestid, $groupstudentsjoins, $whichattempts));
        }
    ...
    public function get_cached($qubaids) {
        ...
        $fromdb = $DB->get_record_select('clatest_statistics', 'hashcode = ? AND timemodified > ?',
    ...
    public function get_last_calculated_time($qubaids) {
        ...
        $timemodified = time() - self::TIME_TO_CACHE;
        return $DB->get_field_select('clatest_statistics', 'timemodified', 'hashcode = ? AND timemodified > ?',
                                         array($qubaids->get_hash_code(), $timemodified));
    }

    /**
     * Given a particular clatest grading method return a lang string describing which attempts contribute to grade.
     *
    ...
    public static function using_attempts_lang_string($whichattempts) {
         return get_string(static::using_attempts_string_id($whichattempts), 'clatest_statistics');
    }

    /**
     * Given a particular clatest grading method return a string id for use as a field name prefix in mdl_clatest_statistics or to
     ...
     */
    protected function attempt_counts_and_averages($clatestid, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;

        $attempttotals = new \stdClass();
        foreach (array_keys(clatest_get_grading_options()) as $which) {

            list($fromqa, $whereqa, $qaparams) = clatest_statistics_attempts_sql($clatestid, $groupstudentsjoins, $which);
    ...
    protected function median($s, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT clatesta.id, clatesta.sumgrades
                  FROM $fromqa
                 WHERE $whereqa
              ORDER BY sumgrades";
    ...
    protected function sum_of_powers_of_difference_to_mean($mean, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT
                    SUM(POWER((clatesta.sumgrades - $mean), 2)) AS power2,
                    SUM(POWER((clatesta.sumgrades - $mean), 3)) AS power3,
                    SUM(POWER((clatesta.sumgrades - $mean), 4)) AS power4
                  FROM $fromqa
                 WHERE $whereqa";
