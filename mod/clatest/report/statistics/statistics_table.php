...
/**
 * Quiz statistics report, table for showing statistics of each question in the clatest.
 *
...
/**
 * This table has one row for each question in the clatest, with sub-rows when
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class clatest_statistics_table extends flexible_table {
    /** @var object the clatest settings. */
    protected $clatest;

    /** @var integer the clatest course_module id. */
    protected $cmid;
    ...
    public function __construct() {
        parent::__construct('mod-clatest-report-statistics-report');
    }
     ...
     * @param int $cmid the clatest course_module id
     * @param moodle_url $reporturl the URL to redisplay this report.
     * @param int $s number of attempts included in the statistics.
     */
    public function statistics_setup($clatest, $cmid, $reporturl, $s) {
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'clatest_statistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'clatest_statistics');
        }

        $columns[] = 'name';
        $headers[] = get_string('questionname', 'clatest');

        $columns[] = 's';
        $headers[] = get_string('attempts', 'clatest_statistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'clatest_statistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'clatest_statistics');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'clatest_statistics');

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'clatest_statistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'clatest_statistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'clatest_statistics');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'clatest_statistics');
    ...
    protected function col_actions($questionstat) {
        return clatest_question_action_icons($this->clatest, $this->cmid,
    ...
    protected function col_name($questionstat) {
        ...
        if (!is_null($questionstat->variant)) {
            ...
            $name = get_string('nameforvariant', 'clatest_statistics', $a);
        }
        ...
        if (!is_null($questionstat->variant)) {
            ...
            if ($questionstat->subquestion && !$questionstat->get_variants()) {
                ...
                $name = html_writer::link($url, $name, array('title' => get_string('detailedanalysis', 'clatest_statistics')));
                ...
                if ($questionstat->get_variants() || $questionstat->get_sub_question_ids()) {
                    ...
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('slotstructureanalysis', 'clatest_statistics', $number)));
                } else {
                    // Question cannot be broken down into sub-questions or variants. Link will show response analysis page.
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('detailedanalysis', 'clatest_statistics')));
                }
            }
        }
        ...
        if (!empty($questionstat->minmedianmaxnotice)) {
            $name = get_string($questionstat->minmedianmaxnotice, 'clatest_statistics') . '<br />' . $name;
        }
     ...
     * of maximum mark for the clatest. That is, the indended influence this question
     * on the student's overall mark.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
    protected function col_intended_weight($questionstat) {
        return clatest_report_scale_summarks_as_percentage($questionstat->maxmark, $this->clatest);
    }
    ...
    protected function col_effective_weight($questionstat) {
        ...
        if ($questionstat->negcovar) {
            $negcovar = get_string('negcovar', 'clatest_statistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'clatest_statistics'),
     ...
     * other questions in this clatest.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
