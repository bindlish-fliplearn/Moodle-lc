...
/**
 * Defines the custom question bank view used on the Edit clatest page.
 *
 * @package   mod_clatest
 * @category  question
...
namespace mod_clatest\question\bank;
defined('MOODLE_INTERNAL') || die();


/**
 * Subclass to customise the view of the question bank for the clatest editing screen.
 *
 * @copyright  2009 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class custom_view extends \core_question\bank\view {
    /** @var bool whether the clatest this is used by has been attemptd. */
    protected $clatesthasattempts = false;
    /** @var \stdClass the clatest settings. */
    protected $clatest = false;
     ...
     * @param \stdClass $clatest clatest settings.
     */
    public function __construct($contexts, $pageurl, $course, $cm, $clatest) {
    ...
    protected function wanted_columns() {
        ...
        if (empty($CFG->clatestquestionbankcolumns)) {
            $clatestquestionbankcolumns = array(
                'add_action_column',
                'checkbox_column',
            ...
            $clatestquestionbankcolumns = explode(',', $CFG->clatestquestionbankcolumns);
        }

        foreach ($clatestquestionbankcolumns as $fullname) {
            if (!class_exists($fullname)) {
                if (class_exists('mod_clatest\\question\\bank\\' . $fullname)) {
                    $fullname = 'mod_clatest\\question\\bank\\' . $fullname;
                    ...
                    debugging('Legacy question bank column class question_bank_' .
                            $fullname . ' should be renamed to mod_clatest\\question\\bank\\' .
    ...
    protected function heading_column() {
        return 'mod_clatest\\question\\bank\\question_name_text_column';
    }

    protected function default_sort() {
        return array(
            'core_question\\bank\\question_type_column' => 1,
            'mod_clatest\\question\\bank\\question_name_text_column' => 1,
        );
    }

    /**
     * Let the question bank display know whether the clatest has been attempted,
     * hence whether some bits of UI, like the add this question to the clatest icon,
     * should be displayed.
     * @param bool $clatesthasattempts whether the clatest has attempts.
     */
    public function set_clatest_has_attempts($clatesthasattempts) {
        $this->clatesthasattempts = $clatesthasattempts;
        if ($clatesthasattempts && isset($this->visiblecolumns['addtoclatestaction'])) {
            unset($this->visiblecolumns['addtoclatestaction']);
        }
    }

    public function preview_question_url($question) {
        return clatest_question_preview_url($this->clatest, $question);
    }

    public function add_to_clatest_url($questionid) {
        global $CFG;
        $params = $this->baseurl->params();
        $params['addquestion'] = $questionid;
        $params['sesskey'] = sesskey();
        return new \moodle_url('/mod/clatest/edit.php', $params);
    }
    ...
    protected function display_bottom_controls($totalnumber, $recurse, $category, \context $catcontext, array $addcontexts) {
        ...
        $cmoptions->hasattempts = !empty($this->clatesthasattempts);
        ...
        if ($canuseall) {
            ...
            // Add selected questions to the clatest.
            $params = array(
                    'type' => 'submit',
                    'name' => 'add',
                    'class' => 'btn btn-primary',
                    'value' => get_string('addselectedquestionstoclatest', 'clatest'),
            );
    ...
    protected function display_options_form($showquestiontext, $scriptpath = '/mod/clatest/edit.php',
    ...
    protected function print_category_info($category) {
        ...
        $strcategory = get_string('category', 'clatest');
