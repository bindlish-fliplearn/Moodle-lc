...
/**
 * Defines the clatest repaginate class.
 *
 * @package   mod_clatest
 ...
 *
 * The clatest setting allows users to write clatestzes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the clatest being manipulated. */
    private $clatestid;
    /** @var array the clatest_slots for that clatest. */
    private $slots;

    /**
     * Constructor.
     * @param int $clatestid the id of the clatest being manipulated.
     * @param stdClass[] $slots the clatest_slots for that clatest.
     */
    public function __construct($clatestid = 0, $slots = null) {
        global $DB;
        $this->clatestid = $clatestid;
        if (!$this->clatestid) {
            $this->slots = array();
        }
        if (!$slots) {
            $this->slots = $DB->get_records('clatest_slots', array('clatestid' => $this->clatestid), 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('clatest_slots', $nextslot, true);
    ...
    /**
     * Repaginate the rest.
     * @param stdClass[] $clatestslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($clatestslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$clatestslots) {
            return null;
        }
        $newslots = array();
        foreach ($clatestslots as $slot) {
            if ($type == self::LINK) {
