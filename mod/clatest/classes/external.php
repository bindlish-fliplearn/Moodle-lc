 ...
 * @since      Moodle 3.1
 */
class mod_clatest_external extends external_api {

    /**
     * Describes the parameters for get_clatestzes_by_courses.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_clatestzes_by_courses_parameters() {
    ...
    /**
     * Returns a list of clatestzes in a provided list of courses,
     * if no list is provided all clatestzes that the user can view will be returned.
     *
     * @param array $courseids Array of course ids
     * @return array of clatestzes details
     * @since Moodle 3.1
     */
    public static function get_clatestzes_by_courses($courseids = array()) {
        global $USER;

        $warnings = array();
        $returnedclatestzes = array();

        $params = array(
            'courseids' => $courseids,
        );
        $params = self::validate_parameters(self::get_clatestzes_by_courses_parameters(), $params);
        ...
        if (!empty($params['courseids'])) {
            ...
            // Get the clatestzes in this course, this function checks users visibility permissions.
            // We can avoid then additional validate_context calls.
            $clatestzes = get_all_instances_in_courses("clatest", $courses);
            foreach ($clatestzes as $clatest) {
                ...
                $clatest = clatest_update_effective_access($clatest, $USER->id);

                // Entry to return.
                $clatestdetails = array();
                // First, we return information that any user can see in the web interface.
                $clatestdetails['id'] = $clatest->id;
                $clatestdetails['coursemodule']      = $clatest->coursemodule;
                $clatestdetails['course']            = $clatest->course;
                $clatestdetails['name']              = external_format_string($clatest->name, $context->id);

                if (has_capability('mod/clatest:view', $context)) {
                    // Format intro.
                    list($clatestdetails['intro'], $clatestdetails['introformat']) = external_format_text($clatest->intro,
                                                                    $clatest->introformat, $context->id, 'mod_clatest', 'intro', null);

                    $clatestdetails['introfiles'] = external_util::get_area_files($context->id, 'mod_clatest', 'intro', false, false);
                    ...
                    $hasfeedback = clatest_has_feedback($clatest);
                    $clatestdetails['hasfeedback'] = (!empty($hasfeedback)) ? 1 : 0;

                    $timenow = time();
                    $clatestobj = clatest::create($clatest->id, $USER->id);
                    $accessmanager = new clatest_access_manager($clatestobj, $timenow, has_capability('mod/clatest:ignoretimelimits',
                                                                $context, null, false));

                    // Fields the user could see if have access to the clatest.
                    if (!$accessmanager->prevent_access()) {
                        $clatestdetails['hasquestions'] = (int) $clatestobj->has_questions();
                        $clatestdetails['autosaveperiod'] = get_config('clatest', 'autosaveperiod');
                    ...
                    foreach ($viewablefields as $field) {
                        $clatestdetails[$field] = $clatest->{$field};
                    }
                }
                $returnedclatestzes[] = $clatestdetails;
            }
        }
        $result = array();
        $result['clatestzes'] = $returnedclatestzes;
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_clatestzes_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_clatestzes_by_courses_returns() {
        return new external_single_structure(
            array(
                'clatestzes' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            'id' => new external_value(PARAM_INT, 'Standard Moodle primary key.'),
                            'course' => new external_value(PARAM_INT, 'Foreign key reference to the course this clatest is part of.'),
                            ...
                            'timeopen' => new external_value(PARAM_INT, 'The time when this clatest opens. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timeclose' => new external_value(PARAM_INT, 'The time when this clatest closes. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timelimit' => new external_value(PARAM_INT, 'The time limit for clatest attempts, in seconds.',
                                                                VALUE_OPTIONAL),
                            ...
                            'canredoquestions' => new external_value(PARAM_INT, 'Allows students to redo any completed question
                                                                        within a clatest attempt.', VALUE_OPTIONAL),
                            ...
                            'reviewattempt' => new external_value(PARAM_INT, 'Whether users are allowed to review their clatest
                                                                    attempts at various times. This is a bit field, decoded by the
                                                                    mod_clatest_display_options class. It is formed by ORing together
                                                                    the constants defined there.', VALUE_OPTIONAL),
                            'reviewcorrectness' => new external_value(PARAM_INT, 'Whether users are allowed to review their clatest
                                                                        attempts at various times.
                                                                        A bit field, like reviewattempt.', VALUE_OPTIONAL),
                            'reviewmarks' => new external_value(PARAM_INT, 'Whether users are allowed to review their clatest attempts
                            ...
                            'sumgrades' => new external_value(PARAM_FLOAT, 'The total of all the question instance maxmarks.',
                                                                VALUE_OPTIONAL),
                            'grade' => new external_value(PARAM_FLOAT, 'The total that the clatest overall grade is scaled to be
                                                            out of.', VALUE_OPTIONAL),
                            'timecreated' => new external_value(PARAM_INT, 'The time when the clatest was added to the course.',
                                                                VALUE_OPTIONAL),
                            ...
                            'password' => new external_value(PARAM_RAW, 'A password that the student must enter before starting or
                                                                continuing a clatest attempt.', VALUE_OPTIONAL),
                            'subnet' => new external_value(PARAM_RAW, 'Used to restrict the IP addresses from which this clatest can
                            ...
                            'completionattemptsexhausted' => new external_value(PARAM_INT, 'Mark clatest complete when the student has
                            ...
                            'allowofflineattempts' => new external_value(PARAM_INT, 'Whether to allow the clatest to be attempted
                                                                            offline in the mobile app', VALUE_OPTIONAL),
                            'autosaveperiod' => new external_value(PARAM_INT, 'Auto-save delay', VALUE_OPTIONAL),
                            'hasfeedback' => new external_value(PARAM_INT, 'Whether the clatest has any non-blank feedback text',
                                                                VALUE_OPTIONAL),
                            'hasquestions' => new external_value(PARAM_INT, 'Whether the clatest has questions', VALUE_OPTIONAL),
    ...
    /**
     * Utility function for validating a clatest.
     *
     * @param int $clatestid clatest instance id
     * @return array array containing the clatest, course, context and course module objects
     * @since  Moodle 3.1
     */
    protected static function validate_clatest($clatestid) {
        global $DB;

        // Request and permission validation.
        $clatest = $DB->get_record('clatest', array('id' => $clatestid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($clatest, 'clatest');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        return array($clatest, $course, $cm, $context);
    }

    /**
     * Describes the parameters for view_clatest.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function view_clatest_parameters() {
        return new external_function_parameters (
            array(
                'clatestid' => new external_value(PARAM_INT, 'clatest instance id'),
            )
        );
    }
     ...
     */
    public static function view_clatest($clatestid) {
        global $DB;

        $params = self::validate_parameters(self::view_clatest_parameters(), array('clatestid' => $clatestid));
        $warnings = array();

        list($clatest, $course, $cm, $context) = self::validate_clatest($params['clatestid']);
    ...
    /**
     * Describes the view_clatest return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function view_clatest_returns() {
        return new external_single_structure(
            array(
    ...
    public static function get_user_attempts_parameters() {
        return new external_function_parameters (
            array(
                ...
                'status' => new external_value(PARAM_ALPHA, 'clatest status: all, finished or unfinished', VALUE_DEFAULT, 'finished'),
    ...
    /**
     * Return a list of attempts for the given clatest and user.
     *
     * @param int $clatestid clatest instance id
     * @param int $userid user id
     * @param string $status clatest status: all, finished or unfinished
     * @param bool $includepreviews whether to include previews or not
     * @return array of warnings and the list of attempts
     * @since Moodle 3.1
     * @throws invalid_parameter_exception
     */
    public static function get_user_attempts($clatestid, $userid = 0, $status = 'finished', $includepreviews = false) {
        global $DB, $USER;
        ...
        if ($USER->id != $user->id) {
            require_capability('mod/clatest:viewreports', $context);
        }

        $attempts = clatest_get_user_attempts($clatest->id, $user->id, $params['status'], $params['includepreviews']);
    ...
    private static function attempt_structure() {
        return new external_single_structure(
            array(
                ...
                'clatest' => new external_value(PARAM_INT, 'Foreign key reference to the clatest that was attempted.',
                                                VALUE_OPTIONAL),
                'userid' => new external_value(PARAM_INT, 'Foreign key reference to the user whose attempt this is.',
                                                VALUE_OPTIONAL),
                'attempt' => new external_value(PARAM_INT, 'Sequentially numbers this students attempts at this clatest.',
                                                VALUE_OPTIONAL),
                'uniqueid' => new external_value(PARAM_INT, 'Foreign key reference to the question_usage that holds the
                                                    details of the the question_attempts that make up this clatest
                ...
                'timecheckstate' => new external_value(PARAM_INT, 'Next time clatest cron should check attempt for
    ...
    /**
     * Get the best current grade for the given user on a clatest.
     *
     * @param int $clatestid clatest instance id
     * @param int $userid user id
     ...
     * @since Moodle 3.1
     */
    public static function get_user_best_grade($clatestid, $userid = 0) {
        global $DB, $USER;
        ...
        $result = array();
        $grade = clatest_get_best_grade($clatest, $user->id);
    ...
    public static function get_user_best_grade_returns() {
        return new external_single_structure(
            array(
                'hasgrade' => new external_value(PARAM_BOOL, 'Whether the user has a grade on the given clatest.'),
     ...
     * @since Moodle 3.1
     */
    public static function get_combined_review_options($clatestid, $userid = 0) {
        global $DB, $USER;
        ...
        $attempts = clatest_get_user_attempts($clatest->id, $user->id, 'all', true);
    ...
    /**
     * Starts a new attempt at a clatest.
     *
     * @param int $clatestid clatest instance id
     ...
     */
    public static function start_attempt($clatestid, $preflightdata = array(), $forcenew = false) {
        global $DB, $USER;
        ...
        if (!$clatestobj->has_questions()) {
            throw new moodle_clatest_exception($clatestobj, 'noquestionsfound');
        }
        ...
        list($currentattemptid, $attemptnumber, $lastattempt, $messages, $page) =
            clatest_validate_new_attempt($clatestobj, $accessmanager, $forcenew, -1, false);
        ...
        if (!$clatestobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new moodle_clatest_exception($clatestobj, array_shift($errors));
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == clatest_attempt::OVERDUE) {
                    throw new moodle_clatest_exception($clatestobj, 'stateoverdue');
                } else {
                    throw new moodle_clatest_exception($clatestobj, 'attemptstillinprogress');
                }
            }
            $offlineattempt = WS_SERVER ? true : false;
            $attempt = clatest_prepare_and_start_new_attempt($clatestobj, $attemptnumber, $lastattempt, $offlineattempt);
        }
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        $attemptobj = clatest_attempt::create($params['attemptid']);
        ...
        if ($attemptobj->get_userid() != $USER->id) {
            throw new moodle_clatest_exception($attemptobj->get_clatestobj(), 'notyourattempt');
        }
        ...
        if ($checkaccessrules) {
            ...
            if (!$ispreviewuser && $messages) {
                throw new moodle_clatest_exception($attemptobj->get_clatestobj(), 'attempterror');
            }
        }

        // Attempt closed?.
        if ($attemptobj->is_finished()) {
            throw new moodle_clatest_exception($attemptobj->get_clatestobj(), 'attemptalreadyclosed');
        } else if ($failifoverdue && $attemptobj->get_state() == clatest_attempt::OVERDUE) {
            throw new moodle_clatest_exception($attemptobj->get_clatestobj(), 'stateoverdue');
        }

        // User submitted data (like the clatest password).
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new moodle_clatest_exception($attemptobj->get_clatestobj(), array_shift($errors));
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new moodle_clatest_exception($attemptobj->get_clatestobj(), 'Invalid page number');
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new moodle_clatest_exception($attemptobj->get_clatestobj(), 'Out of sequence access');
            }
            ...
            if (empty($slots)) {
                throw new moodle_clatest_exception($attemptobj->get_clatestobj(), 'noquestionsfound');
            }
        }
     ...
     *
     * @return external_single_structure the question data. Some fields may not be returned depending on the clatest display settings.
     * @since  Moodle 3.1
    ...
    private static function question_structure() {
        return new external_single_structure(
            array(
                ...
                'page' => new external_value(PARAM_INT, 'page of the clatest this question appears on'),
                ...
                'number' => new external_value(PARAM_INT, 'question ordering number in the clatest', VALUE_OPTIONAL),
                'state' => new external_value(PARAM_ALPHA, 'the state where the question is in.
                    It will not be returned if the user cannot see it due to the clatest display correctness settings.',
                    VALUE_OPTIONAL),
            ...
            'The question data. Some fields may not be returned depending on the clatest display settings.'
        );
    }
     ...
     *
     * @param  clatest_attempt  $attemptobj  the clatest attempt object
     * @param  bool  $review  whether if we are in review mode or not
     * @param  mixed  $page  string 'all' or integer page number
     * @return array array of questions including data
     */
    private static function get_attempt_questions_data(clatest_attempt $attemptobj, $review, $page = 'all') {
        global $PAGE;

        $questions = array();
        $contextid = $attemptobj->get_clatestobj()->get_context()->id;
        $displayoptions = $attemptobj->get_display_options($review);
        $renderer = $PAGE->get_renderer('mod_clatest');
    ...
    /**
     * Returns information for the given attempt page for a clatest attempt in progress.
     *
     ...
     * @since Moodle 3.1
     * @throws moodle_clatest_exceptions
     */
        ...
        return new external_single_structure(
            array(
                ...
                'messages' => new external_multiple_structure(
                    ...
                    'access messages, will only be returned for users with mod/clatest:preview capability,
    ...
    /**
     * Returns a summary of a clatest attempt before it is submitted.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Processes save requests during the clatest. This function is intended for the clatest auto-save feature.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Process responses during an attempt at a clatest and also deals with attempts finishing.
     *
     * @param int $attemptid attempt id
     * @param array $data the data to be saved
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new moodle_clatest_exception($attemptobj->get_clatestobj(), 'attemptclosed');
    ...
    public static function get_attempt_review($attemptid, $page = -1) {
        ...
        foreach ($summarydata as $key => $data) {
            ...
            $result['additionaldata'][] = array(
                'id' => $key,
                'title' => $data['title'], $attemptobj->get_clatestobj()->get_context()->id,
                'content' => $data['content'],
            );
        }

        // Feedback if there is any, and the user is allowed to see it now.
        $grade = clatest_rescale_grade($attemptobj->get_attempt()->sumgrades, $attemptobj->get_clatest(), false);
    ...
    public static function get_attempt_review_returns() {
        return new external_single_structure(
            array(
                'grade' => new external_value(PARAM_RAW, 'grade for the clatest (or empty or "notyetgraded")'),
                'attempt' => self::attempt_structure(),
     ...
     * @since Moodle 3.1
     */
    public static function get_clatest_feedback_for_grade_parameters() {
        return new external_function_parameters (
            array(
                'clatestid' => new external_value(PARAM_INT, 'clatest instance id'),
    ...
    /**
     * Get the feedback text that should be show to a student who got the given grade in the given clatest.
     *
     * @param int $clatestid clatest instance id
     ...
     * @throws moodle_exception
     */
    public static function get_clatest_feedback_for_grade($clatestid, $grade) {
        global $DB;
        ...
        $params = self::validate_parameters(self::get_clatest_feedback_for_grade_parameters(), $params);
        $warnings = array();
        ...
        $feedback = clatest_feedback_record_for_grade($params['grade'], $clatest);
        if (!empty($feedback->feedbacktext)) {
            list($text, $format) = external_format_text($feedback->feedbacktext, $feedback->feedbacktextformat, $context->id,
                                                        'mod_clatest', 'feedback', $feedback->id);
            $result['feedbacktext'] = $text;
            $result['feedbacktextformat'] = $format;
            $feedbackinlinefiles = external_util::get_area_files($context->id, 'mod_clatest', 'feedback', $feedback->id);
    ...
    /**
     * Describes the get_clatest_feedback_for_grade return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_clatest_feedback_for_grade_returns() {
        return new external_single_structure(
            array(
    ...
    /**
     * Describes the parameters for get_clatest_access_information.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_clatest_access_information_parameters() {
        return new external_function_parameters (
            array(
                'clatestid' => new external_value(PARAM_INT, 'clatest instance id')
            )
    ...
    /**
     * Return access information for a given clatest.
     *
     * @param int $clatestid clatest instance id
     ...
     * @throws  moodle_clatest_exception
     */
    public static function get_clatest_access_information($clatestid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_clatest_access_information_parameters(), $params);

        list($clatest, $course, $cm, $context) = self::validate_clatest($params['clatestid']);

        $result = array();
        // Capabilities first.
        $result['canattempt'] = has_capability('mod/clatest:attempt', $context);;
        $result['canmanage'] = has_capability('mod/clatest:manage', $context);;
        $result['canpreview'] = has_capability('mod/clatest:preview', $context);;
        $result['canreviewmyattempts'] = has_capability('mod/clatest:reviewmyattempts', $context);;
        $result['canviewreports'] = has_capability('mod/clatest:viewreports', $context);;

        // Access manager now.
        $clatestobj = clatest::create($cm->instance, $USER->id);
        $ignoretimelimits = has_capability('mod/clatest:ignoretimelimits', $context, null, false);
        $timenow = time();
        $accessmanager = new clatest_access_manager($clatestobj, $timenow, $ignoretimelimits);
    ...
    /**
     * Describes the get_clatest_access_information return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_clatest_access_information_returns() {
        return new external_single_structure(
            array(
                'canattempt' => new external_value(PARAM_BOOL, 'Whether the user can do the clatest or not.'),
                'canmanage' => new external_value(PARAM_BOOL, 'Whether the user can edit the clatest settings or not.'),
                'canpreview' => new external_value(PARAM_BOOL, 'Whether the user can preview the clatest or not.'),
                'canreviewmyattempts' => new external_value(PARAM_BOOL, 'Whether the users can review their previous attempts
                                                                or not.'),
                'canviewreports' => new external_value(PARAM_BOOL, 'Whether the user can view the clatest reports or not.'),
    ...
    /**
     * Return access information for a given attempt in a clatest.
     *
     * @param int $clatestid clatest instance id
     ...
     * @throws  moodle_clatest_exception
     */
    public static function get_attempt_access_information($clatestid, $attemptid = 0) {
        global $DB, $USER;
        ...
        if ($unfinishedattempt = clatest_get_user_attempt_unfinished($clatest->id, $USER->id)) {
            ...
            if ($unfinishedattempt->state != clatest_attempt::IN_PROGRESS and $unfinishedattempt->state != clatest_attempt::OVERDUE) {
    ...
    /**
     * Describes the parameters for get_clatest_required_qtypes.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_clatest_required_qtypes_parameters() {
        return new external_function_parameters (
            array(
                'clatestid' => new external_value(PARAM_INT, 'clatest instance id')
            )
    ...
    /**
     * Return the potential question types that would be required for a given clatest.
     ...
     * @throws  moodle_clatest_exception
     */
    public static function get_clatest_required_qtypes($clatestid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_clatest_required_qtypes_parameters(), $params);
        ...
        // Question types used.
        $result = array();
        $result['questiontypes'] = $clatestobj->get_all_question_types_used(true);
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_clatest_required_qtypes return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_clatest_required_qtypes_returns() {
        return new external_single_structure(
            array(
                'questiontypes' => new external_multiple_structure(
                                    new external_value(PARAM_PLUGIN, 'question type'), 'list of question types used in the clatest'),
                'warnings' => new external_warnings(),
            )
        );
    }
