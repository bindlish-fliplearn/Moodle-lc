...
/**
 * Classes to enforce the various access rules that can apply to a clatest.
 *
 * @package   mod_clatest
 ...
 * clatest, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
 */
class clatest_access_manager {
    /** @var clatest the clatest settings object. */
    protected $clatestobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of clatest_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular clatest.
     * @param object $clatestobj An instance of the class clatest from attemptlib.php.
     *      The clatest we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/clatest:ignoretimelimits', ...)).
     */
    public function __construct($clatestobj, $timenow, $canignoretimelimits) {
        $this->clatestobj = $clatestobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($clatestobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular clatest.
     * @param clatest $clatestobj information about the clatest in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/clatest:ignoretimelimits capability.
     * @return array of {@link clatest_access_rule_base}s.
     */
    protected function make_rules($clatestobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($clatestobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['clatestaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('clatestaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the clatest table.
     *
     * @param mod_clatest_mod_form $clatestform the clatest settings form that is being built.
     * @param MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    public static function add_settings_form_fields(
            mod_clatest_mod_form $clatestform, MoodleQuickForm $mform) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($clatestform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'clatest'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_clatest_mod_form $clatestform the clatest form object.
     * @return array $errors the updated $errors array.
     */
    public static function validate_settings_form_fields(array $errors,
            array $data, $files, mod_clatest_mod_form $clatestform) {

        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $clatestform);
        }
    ...
    /**
     * Save any submitted settings when the clatest settings form is submitted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the clatest table.
     *
     * @param object $clatest the data from the clatest form, including $clatest->id
     *      which is the id of the clatest being saved.
     */
    public static function save_settings($clatest) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($clatest);
        }
    }

    /**
     * Delete any rule-specific settings when the clatest is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the clatest table.
     *
     * @param object $clatest the data from the database, including $clatest->id
     *      which is the id of the clatest being deleted.
     * @since Moodle 2.7.1, 2.6.4, 2.5.7
     */
    public static function delete_settings($clatest) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($clatest);
        }
    }
     ...
     */
    protected static function get_load_sql($clatestid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{clatest} clatest';
        $allparams = array('clatestid' => $clatestid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($clatestid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE clatest.id = :clatestid", $allparams);
    }
     ...
     */
    public static function load_settings($clatestid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($clatestid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($clatestid);
        }
    ...
    /**
     * Load the clatest settings and any settings required by the access rules.
     ...
     *
     * @param int $clatestid the clatest id.
     * @return object mdl_clatest row with extra fields.
     */
    public static function load_clatest_and_settings($clatestid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($clatestid, $rules, 'clatest.*');
        $clatest = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($clatestid) as $name => $value) {
                $clatest->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this clatest, such
     * as is shown at the top of the clatest view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this clatest now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_clatest_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        $method = 'post';
        if (!empty($_GET['_qf__mod_clatest_preflight_check_form'])) {
            $method = 'get';
        }
        return new mod_clatest_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'clatestobj' => $this->clatestobj,
     ...
     * clatest. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant clatest_attempts row.
    ...
    /**
     * @return bolean if this clatest should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the clatest view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_clatest_renderer $output the clatest renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->clatestobj->view_url(), $message);
            die();
        } else {
            redirect($this->clatestobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the clatest, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(clatest_attempt::IN_PROGRESS, clatest_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = clatest_attempt_state($this->clatestobj->get_clatest(), $attempt);
        $reviewoptions = mod_clatest_display_options::make_from_clatest(
                $this->clatestobj->get_clatest(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->clatestobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->clatestobj->review_url($attempt->id),
