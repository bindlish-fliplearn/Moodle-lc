...
/**
 * Defines the renderer for the clatest module.
 *
 * @package   mod_clatest
...
/**
 * The renderer for the clatest module.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_clatest_renderer extends plugin_renderer_base {
    /**
     * Builds the review page
     *
     * @param clatest_attempt $attemptobj an instance of clatest_attempt.
     ...
     * @param mod_clatest_display_options $displayoptions instance of mod_clatest_display_options.
     * @param array $summarydata contains all table data
     * @return $output containing html data.
     */
    public function review_page(clatest_attempt $attemptobj, $slots, $page, $showall,
                                $lastpage, mod_clatest_display_options $displayoptions,
                                $summarydata) {
     ...
     * @return $output containing html data.
     */
    public function review_question_page(clatest_attempt $attemptobj, $slot, $seq,
            mod_clatest_display_options $displayoptions, $summarydata) {
     ...
     */
    public function review_question_not_allowed(clatest_attempt $attemptobj, $message) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_clatest_name(), true,
                                  array("context" => $attemptobj->get_clatestobj()->get_context())));
    ...
    public function review_summary_table($summarydata, $page) {
        ...
        $output .= html_writer::start_tag('table', array(
                'class' => 'generaltable generalbox clatestreviewsummary'));
     ...
     *
     * @param clatest_attempt $attemptobj instance of clatest_attempt
     ...
     * @param mod_clatest_display_options $displayoptions instance of mod_clatest_display_options
     */
    public function questions(clatest_attempt $attemptobj, $reviewing, $slots, $page, $showall,
                              mod_clatest_display_options $displayoptions) {
        $output = '';
    ...
    public function review_form($page, $showall, $displayoptions, $content, $attemptobj) {
        ...
        $this->page->requires->js_init_call('M.mod_clatest.init_review_form', null, false,
                clatest_get_js_module());
     ...
     * @param clatest_attempt $attemptobj instance of clatest_attempt
     */
    public function finish_review_link(clatest_attempt $attemptobj) {
        $url = $attemptobj->view_url();

        if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            $this->page->requires->js_init_call('M.mod_clatest.secure_window.init_close_button',
                    array($url), false, clatest_get_js_module());
            return html_writer::empty_tag('input', array('type' => 'button',
                    'value' => get_string('finishreview', 'clatest'),
                    'id' => 'secureclosebutton',
                    'class' => 'mod_clatest-next-nav btn btn-primary'));

        } else {
            return html_writer::link($url, get_string('finishreview', 'clatest'),
                    array('class' => 'mod_clatest-next-nav'));
        }
    }
     ...
     */
    public function review_next_navigation(clatest_attempt $attemptobj, $page, $lastpage, $showall = null) {
        $nav = '';
        if ($page > 0) {
            $nav .= link_arrow_left(get_string('navigateprevious', 'clatest'),
                    $attemptobj->review_url(null, $page - 1, $showall), false, 'mod_clatest-prev-nav');
        }
        if ($lastpage) {
            $nav .= $this->finish_review_link($attemptobj);
        } else {
            $nav .= link_arrow_right(get_string('navigatenext', 'clatest'),
                    $attemptobj->review_url(null, $page + 1, $showall), false, 'mod_clatest-next-nav');
        }
        return html_writer::tag('div', $nav, array('class' => 'submitbtns'));
    }

    /**
     * Return the HTML of the clatest timer.
     * @return string HTML content.
     */
    public function countdown_timer(clatest_attempt $attemptobj, $timenow) {
        ...
        if ($timeleft !== false) {
            ...
            if (!$ispreview) {
                ...
                // this will just have the effect of causing the clatest to be submitted immediately.
                $timerstartvalue = max($timerstartvalue, 1);
            }
            $this->initialise_timer($timerstartvalue, $ispreview);
        }

        return html_writer::tag('div', get_string('timeleft', 'clatest') . ' ' .
                html_writer::tag('span', '', array('id' => 'clatest-time-left')),
                array('id' => 'clatest-timer', 'role' => 'timer',
    ...
    public function restart_preview_button($url) {
        return $this->single_button($url, get_string('startnewpreview', 'clatest'));
    }

    /**
     * Outputs the navigation block panel
     *
     * @param clatest_nav_panel_base $panel instance of clatest_nav_panel_base
     */
    public function navigation_panel(clatest_nav_panel_base $panel) {
        ...
        $this->page->requires->js_init_call('M.mod_clatest.nav.init', null, false,
                clatest_get_js_module());
    ...
    /**
     * Display a clatest navigation button.
     *
     * @param clatest_nav_question_button $button
     * @return string HTML fragment.
     */
    protected function render_clatest_nav_question_button(clatest_nav_question_button $button) {
        $classes = array('qnbutton', $button->stateclass, $button->navmethod, 'btn', 'btn-secondary');
        $extrainfo = array();

        if ($button->currentpage) {
            $classes[] = 'thispage';
            $extrainfo[] = get_string('onthispage', 'clatest');
        }
        ...
        $tagcontents = html_writer::tag('span', '', array('class' => 'thispageholder')) .
                        ...
                        get_string($qnostring, 'clatest', $a);
        $tagattributes = array('class' => implode(' ', $classes), 'id' => $button->id,
                                  'title' => $button->statestring, 'data-clatest-page' => $button->page);
    ...
    /**
     * Display a clatest navigation heading.
     *
     * @param clatest_nav_section_heading $heading the heading.
     * @return string HTML fragment.
     */
    protected function render_clatest_nav_section_heading(clatest_nav_section_heading $heading) {
        return $this->heading($heading->heading, 3, 'mod_clatest-section-heading');
    }

    /**
     * outputs the link the other attempts.
     *
     * @param mod_clatest_links_to_other_attempts $links
     */
    protected function render_mod_clatest_links_to_other_attempts(
            mod_clatest_links_to_other_attempts $links) {
        $attemptlinks = array();
    ...
    public function start_attempt_page(clatest $clatestobj, mod_clatest_preflight_check_form $mform) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($clatestobj->get_clatest_name(), true,
                                  array("context" => $clatestobj->get_context())));
        $output .= $this->clatest_intro($clatestobj->get_clatest(), $clatestobj->get_cm());
        $output .= $mform->render();
        $output .= $this->footer();
        return $output;
    }
     ...
     * @param clatest_attempt $attemptobj Instance of clatest_attempt
     * @param int $page Current page number
     * @param clatest_access_manager $accessmanager Instance of clatest_access_manager
    ...
    public function attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id,
        ...
        $output = '';
        $output .= $this->header();
        $output .= $this->clatest_notices($messages);
     ...
     */
    public function clatest_notices($messages) {
        if (!$messages) {
            return '';
        }
        return $this->box($this->heading(get_string('accessnoticesheader', 'clatest'), 3) .
                $this->access_messages($messages), 'clatestaccessnotices');
    }

    /**
     * Ouputs the form for making an attempt
     *
     * @param clatest_attempt $attemptobj
     * @param int $page Current page number
    ...
    public function attempt_form($attemptobj, $page, $slots, $id, $nextpage) {
        ...
        $navmethod = $attemptobj->get_clatest()->navmethod;
     ...
     * @param bool $lastpage is this the last page in the clatest?
     * @param string $navmethod Optional clatest attribute, 'free' (default) or 'sequential'
     * @return string HTML fragment.
     */
    protected function attempt_navigation_buttons($page, $lastpage, $navmethod = 'free') {
        ...
        if ($page > 0 && $navmethod == 'free') {
            $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'previous',
                    'value' => get_string('navigateprevious', 'clatest'), 'class' => 'mod_clatest-prev-nav btn btn-secondary'));
        }
        if ($lastpage) {
            $nextlabel = get_string('endtest', 'clatest');
        } else {
            $nextlabel = get_string('navigatenext', 'clatest');
        }
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'next',
                'value' => $nextlabel, 'class' => 'mod_clatest-next-nav btn btn-primary'));
        $output .= html_writer::end_tag('div');
    ...
    public function redo_question_button($slot, $disabled) {
        $attributes = array('type' => 'submit',  'name' => 'redoslot' . $slot,
                'value' => get_string('redoquestion', 'clatest'), 'class' => 'mod_clatest-redo_question_button');
        if ($disabled) {
    ...
    public function initialise_timer($timerstartvalue, $ispreview) {
        ...
        $this->page->requires->js_init_call('M.mod_clatest.timer.init', $options, false, clatest_get_js_module());
    }
    ...
    public function close_attempt_popup($url, $message = '') {
        ...
        if ($message) {
            ...
            $output .= html_writer::tag('p', get_string('windowclosing', 'clatest'));
            $delay = 5;
        } else {
            $output .= html_writer::tag('p', get_string('pleaseclose', 'clatest'));
            $delay = 0;
        }
        $this->page->requires->js_init_call('M.mod_clatest.secure_window.close',
                array($url, $delay), false, clatest_get_js_module());
     ...
     *
     * @param clatest_attempt $attemptobj
     * @param mod_clatest_display_options $displayoptions
     */
    public function summary_page($attemptobj, $displayoptions) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_clatest_name()));
        $output .= $this->heading(get_string('summaryofattempt', 'clatest'), 3);
    ...
    public function summary_table($attemptobj, $displayoptions) {
        ...
        $table->attributes['class'] = 'generaltable clatestsummaryofattempt boxaligncenter';
        $table->head = array(get_string('question', 'clatest'), get_string('status', 'clatest'));
        $table->align = array('left', 'left');
        $table->size = array('', '');
        $markscolumn = $displayoptions->marks >= question_display_options::MARK_AND_MAX;
        if ($markscolumn) {
            $table->head[] = get_string('marks', 'clatest');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($slots as $slot) {
            ...
            if ($heading) {
                ...
                $table->rowclasses[] = 'clatestsummaryheading';
            }
            ...
            $table->data[] = $row;
            $table->rowclasses[] = 'clatestsummary' . $slot . ' ' . $attemptobj->get_question_state_class(
    ...
    public function summary_page_controls($attemptobj) {
        ...
        if ($attemptobj->get_state() == clatest_attempt::IN_PROGRESS) {
            $button = new single_button(
                    new moodle_url($attemptobj->attempt_url(null, $attemptobj->get_currentpage())),
                    get_string('returnattempt', 'clatest'));
        ...
        $button = new single_button(
                ...
                get_string('submitallandfinish', 'clatest'));
        $button->id = 'responseform';
        if ($attemptobj->get_state() == clatest_attempt::IN_PROGRESS) {
            $button->add_action(new confirm_action(get_string('confirmclose', 'clatest'), null,
                    get_string('submitallandfinish', 'clatest')));
        }

        $duedate = $attemptobj->get_due_date();
        $message = '';
        if ($attemptobj->get_state() == clatest_attempt::OVERDUE) {
            $message = get_string('overduemustbesubmittedby', 'clatest', userdate($duedate));

        } else if ($duedate) {
            $message = get_string('mustbesubmittedby', 'clatest', userdate($duedate));
        }
     ...
     * @param array $clatest Array conting clatest data
     * @param int $cm Course Module ID
     * @param int $context The page context ID
     * @param array $infomessages information about this clatest
     * @param mod_clatest_view_object $viewobj
     ...
     *      attempt this clatest now, if appicable this clatest
     */
    public function view_page($course, $clatest, $cm, $context, $viewobj) {
        $output = '';
        $output .= $this->view_information($clatest, $cm, $context, $viewobj->infomessages);
        $output .= $this->view_table($clatest, $context, $viewobj);
        $output .= $this->view_result_info($clatest, $context, $cm, $viewobj);
        $output .= $this->box($this->view_page_buttons($viewobj), 'clatestattempt');
        return $output;
    }
     ...
     * @param mod_clatest_view_object $viewobj the information required to display
     * the view page.
     * @return string HTML to output.
     */
    public function view_page_buttons(mod_clatest_view_object $viewobj) {
        global $CFG;
        $output = '';
        ...
        if ($viewobj->showbacktocourse) {
            $output .= $this->single_button($viewobj->backtocourseurl,
                    get_string('backtocourse', 'clatest'), 'get',
     ...
     * @param mod_clatest_preflight_check_form $preflightcheckform deprecated.
    ...
    public function start_attempt_button($buttontext, moodle_url $url,
            mod_clatest_preflight_check_form $preflightcheckform = null,
        ...
        if (is_string($preflightcheckform)) {
            ...
            debugging('The third argument to start_attempt_button should now be the ' .
                    'mod_clatest_preflight_check_form from ' .
                    'clatest_access_manager::get_preflight_check_form, not a warning message string.');
        }

        $button = new single_button($url, $buttontext);
        $button->class .= ' clateststartbuttondiv';
        if ($popuprequired) {
            $button->class .= ' clatestsecuremoderequired';
        }
        ...
        $this->page->requires->js_call_amd('mod_clatest/preflightcheck', 'init',
                array('.clateststartbuttondiv [type=submit]', get_string('startattempt', 'clatest'),
                       '#mod_clatest_preflight_form', $popupjsoptions));
    ...
    public function no_questions_message($canedit, $editurl) {
        $output = '';
        $output .= $this->notification(get_string('noquestions', 'clatest'));
        if ($canedit) {
            $output .= $this->single_button($editurl, get_string('editclatest', 'clatest'), 'get');
        }
    ...
    /**
     * Outputs an error message for any guests accessing the clatest
     *
     * @param int $course The course ID
     * @param array $clatest Array contingin clatest data
     * @param int $cm Course Module ID
     * @param int $context The page contect ID
     * @param array $messages Array containing any messages
     */
    public function view_page_guest($course, $clatest, $cm, $context, $messages) {
        $output = '';
        $output .= $this->view_information($clatest, $cm, $context, $messages);
        $guestno = html_writer::tag('p', get_string('guestsno', 'clatest'));
     ...
     * @param array $messages Array containing any messages
     */
    public function view_page_notenrolled($course, $clatest, $cm, $context, $messages) {
        global $CFG;
        $output = '';
        $output .= $this->view_information($clatest, $cm, $context, $messages);
        $youneedtoenrol = html_writer::tag('p', get_string('youneedtoenrol', 'clatest'));
     ...
     * @return string HTML to output.
     */
    public function view_information($clatest, $cm, $context, $messages) {
        global $CFG;

        $output = '';

        // Print clatest name and description.
        $output .= $this->heading(format_string($clatest->name));
        $output .= $this->clatest_intro($clatest, $cm);

        // Output any access messages.
        if ($messages) {
            $output .= $this->box($this->access_messages($messages), 'clatestinfo');
        }

        // Show number of attempts summary to those who can view reports.
        if (has_capability('mod/clatest:viewreports', $context)) {
            if ($strattemptnum = $this->clatest_attempt_summary_link_to_reports($clatest, $cm,
                    $context)) {
                $output .= html_writer::tag('div', $strattemptnum,
                        array('class' => 'clatestattemptcounts'));
            }
        }
        return $output;
    }
    ...
    /**
     * Output the clatest intro.
     * @param object $clatest the clatest settings.
     * @param object $cm the course_module object.
     * @return string HTML to output.
     */
    public function clatest_intro($clatest, $cm) {
        if (html_is_blank($clatest->intro)) {
            return '';
        }

        return $this->box(format_module_intro('clatest', $clatest, $cm->id), 'generalbox', 'intro');
    }
    ...
    public function view_table_heading() {
        return $this->heading(get_string('summaryofattempts', 'clatest'), 3);
    }

    /**
     * Generates the table of data
     *
     * @param array $clatest Array contining clatest data
     * @param int $context The page context ID
     * @param mod_clatest_view_object $viewobj
     */
    public function view_table($clatest, $context, $viewobj) {
        if (!$viewobj->attempts) {
            return '';
        }
        ...
        $table = new html_table();
        $table->attributes['class'] = 'generaltable clatestattemptsummary';
        $table->head = array();
        $table->align = array();
        $table->size = array();
        if ($viewobj->attemptcolumn) {
            $table->head[] = get_string('attemptnumber', 'clatest');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        $table->head[] = get_string('attemptstate', 'clatest');
        $table->align[] = 'left';
        $table->size[] = '';
        if ($viewobj->markcolumn) {
            $table->head[] = get_string('marks', 'clatest') . ' / ' .
                    clatest_format_grade($clatest, $clatest->sumgrades);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        ...
        if ($viewobj->canreviewmine) {
            $table->head[] = get_string('review', 'clatest');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->feedbackcolumn) {
            $table->head[] = get_string('feedback', 'clatest');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($viewobj->attemptobjs as $attemptobj) {
            ...
            if ($viewobj->attemptcolumn) {
                if ($attemptobj->is_preview()) {
                    $row[] = get_string('preview', 'clatest');
                } else {
            ...
            if ($viewobj->markcolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    $row[] = clatest_format_grade($clatest, $attemptobj->get_sum_marks());
                } else {
                    $row[] = '';
                }
            }
            ...
            $attemptgrade = clatest_rescale_grade($attemptobj->get_sum_marks(), $clatest, false);
            ...
            if ($viewobj->gradecolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    if ($viewobj->overallstats && !$attemptobj->is_preview()
                            ...
                            && $attemptobj->get_state() == clatest_attempt::FINISHED
                            && $attemptgrade == $viewobj->mygrade
                            && $clatest->grademethod == QUIZ_GRADEHIGHEST) {
                        $table->rowclasses[$attemptobj->get_attempt_number()] = 'bestrow';
                    }

                    $row[] = clatest_format_grade($clatest, $attemptgrade);
                } else {
                    $row[] = '';
                }
            }
            ...
            if ($viewobj->feedbackcolumn && $attemptobj->is_finished()) {
                if ($attemptoptions->overallfeedback) {
                    $row[] = clatest_feedback_for_grade($attemptgrade, $clatest, $context);
                } else {
                    $row[] = '';
                }
            }
     ...
     * @param clatest_attempt $attemptobj the attempt
    ...
    public function attempt_state($attemptobj) {
        switch ($attemptobj->get_state()) {
            ...
            case clatest_attempt::OVERDUE:
                return get_string('stateoverdue', 'clatest') . html_writer::tag('span',
                        get_string('stateoverduedetails', 'clatest',
                                userdate($attemptobj->get_due_date())),
                        array('class' => 'statedetails'));

            case clatest_attempt::FINISHED:
                return get_string('statefinished', 'clatest') . html_writer::tag('span',
                        get_string('statefinisheddetails', 'clatest',
    ...
    /**
     * Generates data pertaining to clatest results
     *
     * @param array $clatest Array containing clatest data
     * @param int $context The page context ID
     * @param int $cm The Course Module Id
     * @param mod_clatest_view_object $viewobj
     */
    public function view_result_info($clatest, $context, $cm, $viewobj) {
        $output = '';
        ...
        if ($viewobj->overallstats) {
            if ($viewobj->moreattempts) {
                $a = new stdClass();
                $a->method = clatest_get_grading_option_name($clatest->grademethod);
                $a->mygrade = clatest_format_grade($clatest, $viewobj->mygrade);
                $a->clatestgrade = clatest_format_grade($clatest, $clatest->grade);
                $resultinfo .= $this->heading(get_string('gradesofar', 'clatest', $a), 3);
            } else {
                $a = new stdClass();
                $a->grade = clatest_format_grade($clatest, $viewobj->mygrade);
                $a->maxgrade = clatest_format_grade($clatest, $clatest->grade);
                $a = get_string('outofshort', 'clatest', $a);
                $resultinfo .= $this->heading(get_string('yourfinalgradeis', 'clatest', $a), 3);
            }
        }
        ...
        if ($viewobj->gradebookfeedback) {
            $resultinfo .= $this->heading(get_string('comment', 'clatest'), 3);
            $resultinfo .= html_writer::div($viewobj->gradebookfeedback, 'clatestteacherfeedback') . "\n";
        }
        if ($viewobj->feedbackcolumn) {
            $resultinfo .= $this->heading(get_string('overallfeedback', 'clatest'), 3);
            $resultinfo .= html_writer::div(
                    clatest_feedback_for_grade($viewobj->mygrade, $clatest, $context),
                    'clatestgradefeedback') . "\n";
        }
    ...
    public function review_link($url, $reviewinpopup, $popupoptions) {
        if ($reviewinpopup) {
            $button = new single_button($url, get_string('review', 'clatest'));
            $button->add_action(new popup_action('click', $url, 'clatestpopup', $popupoptions));
            return $this->render($button);

        } else {
            return html_writer::link($url, get_string('review', 'clatest'),
                    array('title' => get_string('reviewthisattempt', 'clatest')));
        }
    }
     ...
     */
    public function clatest_attempt_summary_link_to_reports($clatest, $cm, $context,
    ...
    public function connection_warning() {
        ...
        $warning = format_text(get_string('connectionerror', 'clatest'), FORMAT_MARKDOWN, $options);
        $ok = format_text(get_string('connectionok', 'clatest'), FORMAT_MARKDOWN, $options);
...
class mod_clatest_links_to_other_attempts implements renderable {
    /**
...
class mod_clatest_view_object {
    /** @var array $infomessages of messages with information to display about the clatest. */
    public $infomessages;
    /** @var array $attempts contains all the user's attempts at this clatest. */
    public $attempts;
    /** @var array $attemptobjs clatest_attempt objects corresponding to $attempts. */
    public $attemptobjs;
    /** @var clatest_access_manager $accessmanager contains various access rules. */
    public $accessmanager;
    /** @var bool $canreviewmine whether the current user has the capability to
     *       review their own attempts. */
    public $canreviewmine;
    /** @var bool $canedit whether the current user has the capability to edit the clatest. */
    public $canedit;
    /** @var moodle_url $editurl the URL for editing this clatest. */
    public $editurl;
    ...
    public $numattempts;
    /** @var float $mygrade contains the user's final grade for a clatest. */
    public $mygrade;
    ...
    public $popupoptions;
    /** @var bool $clatesthasquestions whether the clatest has any questions. */
    public $clatesthasquestions;
