...
/**
 * Define all the backup steps that will be used by the backup_clatest_activity_task
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_clatest_activity_structure_step extends backup_questions_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $clatest = new backup_nested_element('clatest', array('id'), array(
        ...
        $this->add_subplugin_structure('clatestaccess', $clatest, true);
        ...
        $this->add_subplugin_structure('clatestaccess', $attempt, true);

        // Build the tree.
        $clatest->add_child($qinstances);
        $qinstances->add_child($qinstance);

        $qinstance->add_child($qinstancetags);
        $qinstancetags->add_child($qinstancetag);

        $clatest->add_child($sections);
        $sections->add_child($section);

        $clatest->add_child($feedbacks);
        $feedbacks->add_child($feedback);

        $clatest->add_child($overrides);
        $overrides->add_child($override);

        $clatest->add_child($grades);
        $grades->add_child($grade);

        $clatest->add_child($attempts);
        $attempts->add_child($attempt);

        // Define sources.
        $clatest->set_source_table('clatest', array('id' => backup::VAR_ACTIVITYID));

        $qinstance->set_source_table('clatest_slots',
                array('clatestid' => backup::VAR_PARENTID));

        $qinstancetag->set_source_table('clatest_slot_tags',
                array('slotid' => backup::VAR_PARENTID));

        $section->set_source_table('clatest_sections',
                array('clatestid' => backup::VAR_PARENTID));

        $feedback->set_source_table('clatest_feedback',
                array('clatestid' => backup::VAR_PARENTID));

        // Quiz overrides to backup are different depending of user info.
        $overrideparams = array('clatest' => backup::VAR_PARENTID);
        if (!$userinfo) { //  Without userinfo, skip user overrides.
            $overrideparams['userid'] = backup_helper::is_sqlparam(null);

        }
        $override->set_source_table('clatest_overrides', $overrideparams);

        // All the rest of elements only happen if we are including user info.
        if ($userinfo) {
            $grade->set_source_table('clatest_grades', array('clatest' => backup::VAR_PARENTID));
            $attempt->set_source_sql('
                    SELECT *
                    FROM {clatest_attempts}
                    WHERE clatest = :clatest AND preview = 0',
                    array('clatest' => backup::VAR_PARENTID));
        }

        // Define source alias.
        $clatest->set_source_alias('attempts', 'attempts_number');
        ...
        $clatest->annotate_files('mod_clatest', 'intro', null); // This file area hasn't itemid.
        $feedback->annotate_files('mod_clatest', 'feedback', 'id');

        // Return the root element (clatest), wrapped into standard activity structure.
        return $this->prepare_activity_structure($clatest);
    }
}
