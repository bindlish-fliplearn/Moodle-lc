...
/**
 * Defines backup_clatest_activity_task class
 *
 * @package     mod_clatest
 * @category    backup
...
require_once($CFG->dirroot . '/mod/clatest/backup/moodle2/backup_clatest_stepslib.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_clatest_activity_task extends backup_activity_task {
    ...
    protected function define_my_steps() {
        // Generate the clatest.xml file containing all the clatest information
        // and annotating used questions.
        $this->add_step(new backup_clatest_activity_structure_step('clatest_structure', 'clatest.xml'));

        // Note: Following  steps must be present
        // in all the activities using question banks (only clatest for now)
    ...
    public static function encode_content_links($content) {
        ...
        // Link to the list of clatestzes.
        $search="/(".$base."\/mod\/clatest\/index.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@QUIZINDEX*$2@$', $content);

        // Link to clatest view by moduleid.
        $search="/(".$base."\/mod\/clatest\/view.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@QUIZVIEWBYID*$2@$', $content);

        // Link to clatest view by clatestid.
        $search="/(".$base."\/mod\/clatest\/view.php\?q\=)([0-9]+)/";
