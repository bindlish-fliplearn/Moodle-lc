...
/**
 * Upgrade script for the homework module.
 *
...
function xmldb_homework_upgrade($oldversion) {
    ...
    if ($oldversion < 2016092000) {
        // Define new fields to be added to homework.
        ...
        upgrade_mod_savepoint(true, 2016092000, 'homework');
    }

    if ($oldversion < 2016092001) {
        // New field for homework_attemps.
        $table = new xmldb_table('homework_attempts');
        ...
        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2016092001, 'homework');
    }

    if ($oldversion < 2016100300) {
        // Find homeworkzes with the combination of require passing grade and grade to pass 0.
        ...
        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2016100300, 'homework');
    }
    ...
    if ($oldversion < 2018020700) {
        ...
        $table = new xmldb_table('homework_slots');

        // Define field questioncategoryid to be added to homework_slots.
        ...
        // Define key questioncategoryid (foreign) to be added to homework_slots.
        ...
        // Define field includingsubcategories to be added to homework_slots.
    ...
    if ($oldversion < 2018020701) {
        ...
        // For each question, move the configuration data to the homework_slots table.
        $pbar = new progress_bar('updatehomeworkslotswithrandom', 500, true);
        foreach ($rs as $record) {
            $data = new stdClass();
            $data->id = $record->id;
            $data->questioncategoryid = $record->category;
            $data->includingsubcategories = empty($record->questiontext) ? 0 : 1;
            $DB->update_record('homework_slots', $data);

            // Update progress.
            $a->done++;
            $pbar->update($a->done, $a->total, get_string('updatehomeworkslotswithrandomxofy', 'homework', $a));
        }
        $rs->close();

        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2018020701, 'homework');
    }

    if ($oldversion < 2018040700) {

        // Define field tags to be dropped from homework_slots. This field was added earlier to master only.
        $table = new xmldb_table('homework_slots');
        $field = new xmldb_field('tags');

        // Conditionally launch drop field homeworkid.
        if ($dbman->field_exists($table, $field)) {
            $dbman->drop_field($table, $field);
        }

        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2018040700, 'homework');
    }

    if ($oldversion < 2018040800) {

        // Define table homework_slot_tags to be created.
        $table = new xmldb_table('homework_slot_tags');

        // Adding fields to table homework_slot_tags.
        ...
        // Adding keys to table homework_slot_tags.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->add_key('slotid', XMLDB_KEY_FOREIGN, array('slotid'), 'homework_slots', array('id'));
        $table->add_key('tagid', XMLDB_KEY_FOREIGN, array('tagid'), 'tag', array('id'));

        // Conditionally launch create table for homework_slot_tags.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2018040800, 'homework');
    }
