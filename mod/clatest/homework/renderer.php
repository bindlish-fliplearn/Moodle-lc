...
/**
 * Defines the renderer for the homework module.
 *
...
/**
 * The renderer for the homework module.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_homework_renderer extends plugin_renderer_base {
    /**
     * Builds the review page
     *
     * @param homework_attempt $attemptobj an instance of homework_attempt.
     ...
     * @param mod_homework_display_options $displayoptions instance of mod_homework_display_options.
     * @param array $summarydata contains all table data
     * @return $output containing html data.
     */
    public function review_page(homework_attempt $attemptobj, $slots, $page, $showall,
                                $lastpage, mod_homework_display_options $displayoptions,
                                $summarydata) {
     ...
     * @return $output containing html data.
     */
    public function review_question_page(homework_attempt $attemptobj, $slot, $seq,
            mod_homework_display_options $displayoptions, $summarydata) {
     ...
     */
    public function review_question_not_allowed(homework_attempt $attemptobj, $message) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_homework_name(), true,
                                  array("context" => $attemptobj->get_homeworkobj()->get_context())));
    ...
    public function review_summary_table($summarydata, $page) {
        ...
        $output .= html_writer::start_tag('table', array(
                'class' => 'generaltable generalbox homeworkreviewsummary'));
     ...
     *
     * @param homework_attempt $attemptobj instance of homework_attempt
     ...
     * @param mod_homework_display_options $displayoptions instance of mod_homework_display_options
     */
    public function questions(homework_attempt $attemptobj, $reviewing, $slots, $page, $showall,
                              mod_homework_display_options $displayoptions) {
        $output = '';
    ...
    public function review_form($page, $showall, $displayoptions, $content, $attemptobj) {
        ...
        $this->page->requires->js_init_call('M.mod_homework.init_review_form', null, false,
                homework_get_js_module());
     ...
     * @param homework_attempt $attemptobj instance of homework_attempt
     */
    public function finish_review_link(homework_attempt $attemptobj) {
        $url = $attemptobj->view_url();

        if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            $this->page->requires->js_init_call('M.mod_homework.secure_window.init_close_button',
                    array($url), false, homework_get_js_module());
            return html_writer::empty_tag('input', array('type' => 'button',
                    'value' => get_string('finishreview', 'homework'),
                    'id' => 'secureclosebutton',
                    'class' => 'mod_homework-next-nav btn btn-primary'));

        } else {
            return html_writer::link($url, get_string('finishreview', 'homework'),
                    array('class' => 'mod_homework-next-nav'));
        }
    }
     ...
     */
    public function review_next_navigation(homework_attempt $attemptobj, $page, $lastpage, $showall = null) {
        $nav = '';
        if ($page > 0) {
            $nav .= link_arrow_left(get_string('navigateprevious', 'homework'),
                    $attemptobj->review_url(null, $page - 1, $showall), false, 'mod_homework-prev-nav');
        }
        if ($lastpage) {
            $nav .= $this->finish_review_link($attemptobj);
        } else {
            $nav .= link_arrow_right(get_string('navigatenext', 'homework'),
                    $attemptobj->review_url(null, $page + 1, $showall), false, 'mod_homework-next-nav');
        }
        return html_writer::tag('div', $nav, array('class' => 'submitbtns'));
    }

    /**
     * Return the HTML of the homework timer.
     * @return string HTML content.
     */
    public function countdown_timer(homework_attempt $attemptobj, $timenow) {
        ...
        if ($timeleft !== false) {
            ...
            if (!$ispreview) {
                ...
                // this will just have the effect of causing the homework to be submitted immediately.
                $timerstartvalue = max($timerstartvalue, 1);
            }
            $this->initialise_timer($timerstartvalue, $ispreview);
        }

        return html_writer::tag('div', get_string('timeleft', 'homework') . ' ' .
                html_writer::tag('span', '', array('id' => 'homework-time-left')),
                array('id' => 'homework-timer', 'role' => 'timer',
    ...
    public function restart_preview_button($url) {
        return $this->single_button($url, get_string('startnewpreview', 'homework'));
    }

    /**
     * Outputs the navigation block panel
     *
     * @param homework_nav_panel_base $panel instance of homework_nav_panel_base
     */
    public function navigation_panel(homework_nav_panel_base $panel) {
        ...
        $this->page->requires->js_init_call('M.mod_homework.nav.init', null, false,
                homework_get_js_module());
    ...
    /**
     * Display a homework navigation button.
     *
     * @param homework_nav_question_button $button
     * @return string HTML fragment.
     */
    protected function render_homework_nav_question_button(homework_nav_question_button $button) {
        $classes = array('qnbutton', $button->stateclass, $button->navmethod, 'btn', 'btn-secondary');
        $extrainfo = array();

        if ($button->currentpage) {
            $classes[] = 'thispage';
            $extrainfo[] = get_string('onthispage', 'homework');
        }
        ...
        $tagcontents = html_writer::tag('span', '', array('class' => 'thispageholder')) .
                        ...
                        get_string($qnostring, 'homework', $a);
        $tagattributes = array('class' => implode(' ', $classes), 'id' => $button->id,
                                  'title' => $button->statestring, 'data-homework-page' => $button->page);
    ...
    /**
     * Display a homework navigation heading.
     *
     * @param homework_nav_section_heading $heading the heading.
     * @return string HTML fragment.
     */
    protected function render_homework_nav_section_heading(homework_nav_section_heading $heading) {
        return $this->heading($heading->heading, 3, 'mod_homework-section-heading');
    }

    /**
     * outputs the link the other attempts.
     *
     * @param mod_homework_links_to_other_attempts $links
     */
    protected function render_mod_homework_links_to_other_attempts(
            mod_homework_links_to_other_attempts $links) {
        $attemptlinks = array();
    ...
    public function start_attempt_page(homework $homeworkobj, mod_homework_preflight_check_form $mform) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($homeworkobj->get_homework_name(), true,
                                  array("context" => $homeworkobj->get_context())));
        $output .= $this->homework_intro($homeworkobj->get_homework(), $homeworkobj->get_cm());
        $output .= $mform->render();
        $output .= $this->footer();
        return $output;
    }
     ...
     * @param homework_attempt $attemptobj Instance of homework_attempt
     * @param int $page Current page number
     * @param homework_access_manager $accessmanager Instance of homework_access_manager
    ...
    public function attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id,
        ...
        $output = '';
        $output .= $this->header();
        $output .= $this->homework_notices($messages);
     ...
     */
    public function homework_notices($messages) {
        if (!$messages) {
            return '';
        }
        return $this->box($this->heading(get_string('accessnoticesheader', 'homework'), 3) .
                $this->access_messages($messages), 'homeworkaccessnotices');
    }

    /**
     * Ouputs the form for making an attempt
     *
     * @param homework_attempt $attemptobj
     * @param int $page Current page number
    ...
    public function attempt_form($attemptobj, $page, $slots, $id, $nextpage) {
        ...
        $navmethod = $attemptobj->get_homework()->navmethod;
     ...
     * @param bool $lastpage is this the last page in the homework?
     * @param string $navmethod Optional homework attribute, 'free' (default) or 'sequential'
     * @return string HTML fragment.
     */
    protected function attempt_navigation_buttons($page, $lastpage, $navmethod = 'free') {
        ...
        if ($page > 0 && $navmethod == 'free') {
            $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'previous',
                    'value' => get_string('navigateprevious', 'homework'), 'class' => 'mod_homework-prev-nav btn btn-secondary'));
        }
        if ($lastpage) {
            $nextlabel = get_string('endtest', 'homework');
        } else {
            $nextlabel = get_string('navigatenext', 'homework');
        }
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'next',
                'value' => $nextlabel, 'class' => 'mod_homework-next-nav btn btn-primary'));
        $output .= html_writer::end_tag('div');
    ...
    public function redo_question_button($slot, $disabled) {
        $attributes = array('type' => 'submit',  'name' => 'redoslot' . $slot,
                'value' => get_string('redoquestion', 'homework'), 'class' => 'mod_homework-redo_question_button');
        if ($disabled) {
    ...
    public function initialise_timer($timerstartvalue, $ispreview) {
        ...
        $this->page->requires->js_init_call('M.mod_homework.timer.init', $options, false, homework_get_js_module());
    }
    ...
    public function close_attempt_popup($url, $message = '') {
        ...
        if ($message) {
            ...
            $output .= html_writer::tag('p', get_string('windowclosing', 'homework'));
            $delay = 5;
        } else {
            $output .= html_writer::tag('p', get_string('pleaseclose', 'homework'));
            $delay = 0;
        }
        $this->page->requires->js_init_call('M.mod_homework.secure_window.close',
                array($url, $delay), false, homework_get_js_module());
     ...
     *
     * @param homework_attempt $attemptobj
     * @param mod_homework_display_options $displayoptions
     */
    public function summary_page($attemptobj, $displayoptions) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_homework_name()));
        $output .= $this->heading(get_string('summaryofattempt', 'homework'), 3);
    ...
    public function summary_table($attemptobj, $displayoptions) {
        ...
        $table->attributes['class'] = 'generaltable homeworksummaryofattempt boxaligncenter';
        $table->head = array(get_string('question', 'homework'), get_string('status', 'homework'));
        $table->align = array('left', 'left');
        $table->size = array('', '');
        $markscolumn = $displayoptions->marks >= question_display_options::MARK_AND_MAX;
        if ($markscolumn) {
            $table->head[] = get_string('marks', 'homework');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($slots as $slot) {
            ...
            if ($heading) {
                ...
                $table->rowclasses[] = 'homeworksummaryheading';
            }
            ...
            $table->data[] = $row;
            $table->rowclasses[] = 'homeworksummary' . $slot . ' ' . $attemptobj->get_question_state_class(
    ...
    public function summary_page_controls($attemptobj) {
        ...
        if ($attemptobj->get_state() == homework_attempt::IN_PROGRESS) {
            $button = new single_button(
                    new moodle_url($attemptobj->attempt_url(null, $attemptobj->get_currentpage())),
                    get_string('returnattempt', 'homework'));
        ...
        $button = new single_button(
                ...
                get_string('submitallandfinish', 'homework'));
        $button->id = 'responseform';
        if ($attemptobj->get_state() == homework_attempt::IN_PROGRESS) {
            $button->add_action(new confirm_action(get_string('confirmclose', 'homework'), null,
                    get_string('submitallandfinish', 'homework')));
        }

        $duedate = $attemptobj->get_due_date();
        $message = '';
        if ($attemptobj->get_state() == homework_attempt::OVERDUE) {
            $message = get_string('overduemustbesubmittedby', 'homework', userdate($duedate));

        } else if ($duedate) {
            $message = get_string('mustbesubmittedby', 'homework', userdate($duedate));
        }
     ...
     * @param array $homework Array conting homework data
     * @param int $cm Course Module ID
     * @param int $context The page context ID
     * @param array $infomessages information about this homework
     * @param mod_homework_view_object $viewobj
     ...
     *      attempt this homework now, if appicable this homework
     */
    public function view_page($course, $homework, $cm, $context, $viewobj) {
        $output = '';
        $output .= $this->view_information($homework, $cm, $context, $viewobj->infomessages);
        $output .= $this->view_table($homework, $context, $viewobj);
        $output .= $this->view_result_info($homework, $context, $cm, $viewobj);
        $output .= $this->box($this->view_page_buttons($viewobj), 'homeworkattempt');
        return $output;
    }
     ...
     * @param mod_homework_view_object $viewobj the information required to display
     * the view page.
     * @return string HTML to output.
     */
    public function view_page_buttons(mod_homework_view_object $viewobj) {
        global $CFG;
        $output = '';

        if (!$viewobj->homeworkhasquestions) {
        ...
        if ($viewobj->showbacktocourse) {
            $output .= $this->single_button($viewobj->backtocourseurl,
                    get_string('backtocourse', 'homework'), 'get',
     ...
     * @param mod_homework_preflight_check_form $preflightcheckform deprecated.
    ...
    public function start_attempt_button($buttontext, moodle_url $url,
            mod_homework_preflight_check_form $preflightcheckform = null,
        ...
        if (is_string($preflightcheckform)) {
            ...
            debugging('The third argument to start_attempt_button should now be the ' .
                    'mod_homework_preflight_check_form from ' .
                    'homework_access_manager::get_preflight_check_form, not a warning message string.');
        }

        $button = new single_button($url, $buttontext);
        $button->class .= ' homeworkstartbuttondiv';
        if ($popuprequired) {
            $button->class .= ' homeworksecuremoderequired';
        }
        ...
        $this->page->requires->js_call_amd('mod_homework/preflightcheck', 'init',
                array('.homeworkstartbuttondiv [type=submit]', get_string('startattempt', 'homework'),
                       '#mod_homework_preflight_form', $popupjsoptions));
    ...
    public function no_questions_message($canedit, $editurl) {
        $output = '';
        $output .= $this->notification(get_string('noquestions', 'homework'));
        if ($canedit) {
            $output .= $this->single_button($editurl, get_string('edithomework', 'homework'), 'get');
        }
    ...
    /**
     * Outputs an error message for any guests accessing the homework
     *
     * @param int $course The course ID
     * @param array $homework Array contingin homework data
     * @param int $cm Course Module ID
     * @param int $context The page contect ID
     * @param array $messages Array containing any messages
     */
    public function view_page_guest($course, $homework, $cm, $context, $messages) {
        $output = '';
        $output .= $this->view_information($homework, $cm, $context, $messages);
        $guestno = html_writer::tag('p', get_string('guestsno', 'homework'));
     ...
     * @param array $messages Array containing any messages
     */
    public function view_page_notenrolled($course, $homework, $cm, $context, $messages) {
        global $CFG;
        $output = '';
        $output .= $this->view_information($homework, $cm, $context, $messages);
        $youneedtoenrol = html_writer::tag('p', get_string('youneedtoenrol', 'homework'));
     ...
     * @return string HTML to output.
     */
    public function view_information($homework, $cm, $context, $messages) {
        global $CFG;

        $output = '';

        // Print homework name and description.
        $output .= $this->heading(format_string($homework->name));
        $output .= $this->homework_intro($homework, $cm);

        // Output any access messages.
        if ($messages) {
            $output .= $this->box($this->access_messages($messages), 'homeworkinfo');
        }

        // Show number of attempts summary to those who can view reports.
        if (has_capability('mod/homework:viewreports', $context)) {
            if ($strattemptnum = $this->homework_attempt_summary_link_to_reports($homework, $cm,
                    $context)) {
                $output .= html_writer::tag('div', $strattemptnum,
                        array('class' => 'homeworkattemptcounts'));
            }
        }
        return $output;
    }
    ...
    /**
     * Output the homework intro.
     * @param object $homework the homework settings.
     * @param object $cm the course_module object.
     * @return string HTML to output.
     */
    public function homework_intro($homework, $cm) {
        if (html_is_blank($homework->intro)) {
            return '';
        }

        return $this->box(format_module_intro('homework', $homework, $cm->id), 'generalbox', 'intro');
    }
    ...
    public function view_table_heading() {
        return $this->heading(get_string('summaryofattempts', 'homework'), 3);
    }

    /**
     * Generates the table of data
     *
     * @param array $homework Array contining homework data
     * @param int $context The page context ID
     * @param mod_homework_view_object $viewobj
     */
    public function view_table($homework, $context, $viewobj) {
        if (!$viewobj->attempts) {
            return '';
        }
        ...
        $table = new html_table();
        $table->attributes['class'] = 'generaltable homeworkattemptsummary';
        $table->head = array();
        $table->align = array();
        $table->size = array();
        if ($viewobj->attemptcolumn) {
            $table->head[] = get_string('attemptnumber', 'homework');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        $table->head[] = get_string('attemptstate', 'homework');
        $table->align[] = 'left';
        $table->size[] = '';
        if ($viewobj->markcolumn) {
            $table->head[] = get_string('marks', 'homework') . ' / ' .
                    homework_format_grade($homework, $homework->sumgrades);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->gradecolumn) {
            $table->head[] = get_string('grade') . ' / ' .
                    homework_format_grade($homework, $homework->grade);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->canreviewmine) {
            $table->head[] = get_string('review', 'homework');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->feedbackcolumn) {
            $table->head[] = get_string('feedback', 'homework');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($viewobj->attemptobjs as $attemptobj) {
            ...
            if ($viewobj->attemptcolumn) {
                if ($attemptobj->is_preview()) {
                    $row[] = get_string('preview', 'homework');
                } else {
            ...
            if ($viewobj->markcolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    $row[] = homework_format_grade($homework, $attemptobj->get_sum_marks());
                } else {
                    $row[] = '';
                }
            }
            ...
            $attemptgrade = homework_rescale_grade($attemptobj->get_sum_marks(), $homework, false);
            ...
            if ($viewobj->gradecolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    if ($viewobj->overallstats && !$attemptobj->is_preview()
                            ...
                            && $attemptobj->get_state() == homework_attempt::FINISHED
                            && $attemptgrade == $viewobj->mygrade
                            && $homework->grademethod == QUIZ_GRADEHIGHEST) {
                        $table->rowclasses[$attemptobj->get_attempt_number()] = 'bestrow';
                    }

                    $row[] = homework_format_grade($homework, $attemptgrade);
                } else {
                    $row[] = '';
                }
            }
            ...
            if ($viewobj->feedbackcolumn && $attemptobj->is_finished()) {
                if ($attemptoptions->overallfeedback) {
                    $row[] = homework_feedback_for_grade($attemptgrade, $homework, $context);
                } else {
                    $row[] = '';
                }
            }
     ...
     * @param homework_attempt $attemptobj the attempt
    ...
    public function attempt_state($attemptobj) {
        switch ($attemptobj->get_state()) {
            ...
            case homework_attempt::OVERDUE:
                return get_string('stateoverdue', 'homework') . html_writer::tag('span',
                        get_string('stateoverduedetails', 'homework',
                                userdate($attemptobj->get_due_date())),
                        array('class' => 'statedetails'));

            case homework_attempt::FINISHED:
                return get_string('statefinished', 'homework') . html_writer::tag('span',
                        get_string('statefinisheddetails', 'homework',
    ...
    /**
     * Generates data pertaining to homework results
     *
     * @param array $homework Array containing homework data
     * @param int $context The page context ID
     * @param int $cm The Course Module Id
     * @param mod_homework_view_object $viewobj
     */
    public function view_result_info($homework, $context, $cm, $viewobj) {
        $output = '';
        ...
        if ($viewobj->overallstats) {
            if ($viewobj->moreattempts) {
                $a = new stdClass();
                $a->method = homework_get_grading_option_name($homework->grademethod);
                $a->mygrade = homework_format_grade($homework, $viewobj->mygrade);
                $a->homeworkgrade = homework_format_grade($homework, $homework->grade);
                $resultinfo .= $this->heading(get_string('gradesofar', 'homework', $a), 3);
            } else {
                $a = new stdClass();
                $a->grade = homework_format_grade($homework, $viewobj->mygrade);
                $a->maxgrade = homework_format_grade($homework, $homework->grade);
                $a = get_string('outofshort', 'homework', $a);
                $resultinfo .= $this->heading(get_string('yourfinalgradeis', 'homework', $a), 3);
            }
        }
        ...
        if ($viewobj->gradebookfeedback) {
            $resultinfo .= $this->heading(get_string('comment', 'homework'), 3);
            $resultinfo .= html_writer::div($viewobj->gradebookfeedback, 'homeworkteacherfeedback') . "\n";
        }
        if ($viewobj->feedbackcolumn) {
            $resultinfo .= $this->heading(get_string('overallfeedback', 'homework'), 3);
            $resultinfo .= html_writer::div(
                    homework_feedback_for_grade($viewobj->mygrade, $homework, $context),
                    'homeworkgradefeedback') . "\n";
        }
    ...
    public function review_link($url, $reviewinpopup, $popupoptions) {
        if ($reviewinpopup) {
            $button = new single_button($url, get_string('review', 'homework'));
            $button->add_action(new popup_action('click', $url, 'homeworkpopup', $popupoptions));
            return $this->render($button);

        } else {
            return html_writer::link($url, get_string('review', 'homework'),
                    array('title' => get_string('reviewthisattempt', 'homework')));
        }
    }
     ...
     */
    public function homework_attempt_summary_link_to_reports($homework, $cm, $context,
    ...
    public function connection_warning() {
        ...
        $warning = format_text(get_string('connectionerror', 'homework'), FORMAT_MARKDOWN, $options);
        $ok = format_text(get_string('connectionok', 'homework'), FORMAT_MARKDOWN, $options);
...
class mod_homework_links_to_other_attempts implements renderable {
    /**
...
class mod_homework_view_object {
    /** @var array $infomessages of messages with information to display about the homework. */
    public $infomessages;
    /** @var array $attempts contains all the user's attempts at this homework. */
    public $attempts;
    /** @var array $attemptobjs homework_attempt objects corresponding to $attempts. */
    public $attemptobjs;
    /** @var homework_access_manager $accessmanager contains various access rules. */
    public $accessmanager;
    /** @var bool $canreviewmine whether the current user has the capability to
     *       review their own attempts. */
    public $canreviewmine;
    /** @var bool $canedit whether the current user has the capability to edit the homework. */
    public $canedit;
    /** @var moodle_url $editurl the URL for editing this homework. */
    public $editurl;
    ...
    public $numattempts;
    /** @var float $mygrade contains the user's final grade for a homework. */
    public $mygrade;
    ...
    public $popupoptions;
    /** @var bool $homeworkhasquestions whether the homework has any questions. */
    public $homeworkhasquestions;
