...
/**
 * Implementaton of the homeworkaccess_ipaddress plugin.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class homeworkaccess_ipaddress extends homework_access_rule_base {

    public static function make(homework $homeworkobj, $timenow, $canignoretimelimits) {
        if (empty($homeworkobj->get_homework()->subnet)) {
            return null;
        }
    ...
    public function prevent_access() {
        if (address_in_subnet(getremoteaddr(), $this->homework->subnet)) {
            return false;
        } else {
            return get_string('subnetwrong', 'homeworkaccess_ipaddress');
        }
    }
}
