...
/**
 * Unit tests for the homeworkaccess_ipaddress plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/homework/accessrule/ipaddress/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class homeworkaccess_ipaddress_testcase extends basic_testcase {
    public function test_ipaddress_access_rule() {
        ...
        $homework->subnet = getremoteaddr(null);
        if (!empty($homework->subnet)) {
            $homeworkobj = new homework($homework, $cm, null);
            $rule = new homeworkaccess_ipaddress($homeworkobj, 0);
