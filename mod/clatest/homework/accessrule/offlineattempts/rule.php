...
/**
 * Implementaton of the homeworkaccess_offlineattempts plugin.
 *
 ...
 * @since      Moodle 3.2
 */
class homeworkaccess_offlineattempts extends homework_access_rule_base {
    ...
    public static function make(homework $homeworkobj, $timenow, $canignoretimelimits) {
        ...
        if (empty($CFG->enablemobilewebservice) or empty($homeworkobj->get_homework()->allowofflineattempts)) {
            return null;
        }
    ...
    public function is_preflight_check_required($attemptid) {
        ...
        if (!empty($attemptid)) {
            $timemodifiedoffline = $DB->get_field('homework_attempts', 'timemodifiedoffline', array('id' => $attemptid));
            if (empty($timemodifiedoffline)) {
                return false;
            }
            return empty($SESSION->offlineattemptscheckedhomeworkzes[$this->homework->id]);
        } else {
    ...
    public function add_preflight_check_form_fields(mod_homework_preflight_check_form $homeworkform,
        ...
        $mform->addElement('header', 'offlineattemptsheader', get_string('mobileapp', 'homeworkaccess_offlineattempts'));
        $mform->addElement('static', 'offlinedatamessage', '',
                get_string('offlinedatamessage', 'homeworkaccess_offlineattempts', $lasttime));
        $mform->addElement('advcheckbox', 'confirmdatasaved', null,
                get_string('confirmdatasaved', 'homeworkaccess_offlineattempts'));
    }
    ...
    public function validate_preflight_check($data, $files, $errors, $attemptid) {
        ...
        $errors['confirmdatasaved'] = get_string('pleaseconfirm', 'homeworkaccess_offlineattempts');
        return $errors;
    }

    public function notify_preflight_check_passed($attemptid) {
        global $SESSION;
        $SESSION->offlineattemptscheckedhomeworkzes[$this->homework->id] = true;
    }

    public function current_attempt_finished() {
        global $SESSION;
        // Clear the flag in the session that says that the user has already agreed to the notice.
        if (!empty($SESSION->offlineattemptscheckedhomeworkzes[$this->homework->id])) {
            unset($SESSION->offlineattemptscheckedhomeworkzes[$this->homework->id]);
        }
    }
    ...
    public static function add_settings_form_fields(
        ...
        if ($CFG->enablemobilewebservice) {
            $mform->addElement('selectyesno', 'allowofflineattempts',
                                get_string('allowofflineattempts', 'homeworkaccess_offlineattempts'));
            $mform->addHelpButton('allowofflineattempts', 'allowofflineattempts', 'homeworkaccess_offlineattempts');
    ...
    public static function validate_settings_form_fields(array $errors,
        ...
        if ($CFG->enablemobilewebservice) {
            ...
            // - The homework uses a timer.
            // - The homework is restricted by subnet.
            ...
            if (!empty($data['allowofflineattempts']) and
                ...
                $errors['allowofflineattempts'] = get_string('offlineattemptserror', 'homeworkaccess_offlineattempts');
            }
        }
