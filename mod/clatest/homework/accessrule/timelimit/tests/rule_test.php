...
/**
 * Unit tests for the homeworkaccess_timelimit plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/homework/accessrule/timelimit/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class homeworkaccess_timelimit_testcase extends basic_testcase {
    public function test_time_limit_access_rule() {
        $homework = new stdClass();
        $homework->timelimit = 3600;
        $cm = new stdClass();
        $cm->id = 0;
        $homeworkobj = new homework($homework, $cm, null);
        $rule = new homeworkaccess_timelimit($homeworkobj, 10000);
        $attempt = new stdClass();

        $this->assertEquals($rule->description(),
            get_string('homeworktimelimit', 'homeworkaccess_timelimit', format_time(3600)));
