...
/**
 * This page handles deleting homework overrides
 *
...
if (! $cm = get_coursemodule_from_instance("homework", $homework->id, $homework->course)) {
    print_error('invalidcoursemodule');
}
...
$url = new moodle_url('/mod/homework/overridedelete.php', array('id'=>$override->id));
$confirmurl = new moodle_url($url, array('id'=>$override->id, 'confirm'=>1));
$cancelurl = new moodle_url('/mod/homework/overrides.php', array('cmid'=>$cm->id));
...
$stroverride = get_string('override', 'homework');
...
if ($override->groupid) {
    ...
    $confirmstr = get_string("overridedeletegroupsure", "homework", $group->name);
} else {
    $namefields = get_all_user_name_fields(true);
    $user = $DB->get_record('user', array('id' => $override->userid),
            'id, ' . $namefields);
    $confirmstr = get_string("overridedeleteusersure", "homework", fullname($user));
}
