...
/**
 * Classes to enforce the various access rules that can apply to a homework.
 *
 ...
 * homework, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
 */
class homework_access_manager {
    /** @var homework the homework settings object. */
    protected $homeworkobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of homework_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular homework.
     * @param object $homeworkobj An instance of the class homework from attemptlib.php.
     *      The homework we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/homework:ignoretimelimits', ...)).
     */
    public function __construct($homeworkobj, $timenow, $canignoretimelimits) {
        $this->homeworkobj = $homeworkobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($homeworkobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular homework.
     * @param homework $homeworkobj information about the homework in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/homework:ignoretimelimits capability.
     * @return array of {@link homework_access_rule_base}s.
     */
    protected function make_rules($homeworkobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($homeworkobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['homeworkaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('homeworkaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the homework table.
     *
     * @param mod_homework_mod_form $homeworkform the homework settings form that is being built.
     * @param MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    public static function add_settings_form_fields(
            mod_homework_mod_form $homeworkform, MoodleQuickForm $mform) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($homeworkform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'homework'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_homework_mod_form $homeworkform the homework form object.
     * @return array $errors the updated $errors array.
     */
    public static function validate_settings_form_fields(array $errors,
            array $data, $files, mod_homework_mod_form $homeworkform) {

        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $homeworkform);
        }
    ...
    /**
     * Save any submitted settings when the homework settings form is submitted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the homework table.
     *
     * @param object $homework the data from the homework form, including $homework->id
     *      which is the id of the homework being saved.
     */
    public static function save_settings($homework) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($homework);
        }
    }

    /**
     * Delete any rule-specific settings when the homework is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the homework table.
     *
     * @param object $homework the data from the database, including $homework->id
     *      which is the id of the homework being deleted.
     * @since Moodle 2.7.1, 2.6.4, 2.5.7
     */
    public static function delete_settings($homework) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($homework);
        }
    }
     ...
     */
    protected static function get_load_sql($homeworkid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{homework} homework';
        $allparams = array('homeworkid' => $homeworkid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($homeworkid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE homework.id = :homeworkid", $allparams);
    }
     ...
     */
    public static function load_settings($homeworkid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($homeworkid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($homeworkid);
        }
    ...
    /**
     * Load the homework settings and any settings required by the access rules.
     ...
     *
     * @param int $homeworkid the homework id.
     * @return object mdl_homework row with extra fields.
     */
    public static function load_homework_and_settings($homeworkid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($homeworkid, $rules, 'homework.*');
        $homework = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($homeworkid) as $name => $value) {
                $homework->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this homework, such
     * as is shown at the top of the homework view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this homework now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_homework_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        $method = 'post';
        if (!empty($_GET['_qf__mod_homework_preflight_check_form'])) {
            $method = 'get';
        }
        return new mod_homework_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'homeworkobj' => $this->homeworkobj,
     ...
     * homework. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant homework_attempts row.
    ...
    /**
     * @return bolean if this homework should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the homework view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_homework_renderer $output the homework renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->homeworkobj->view_url(), $message);
            die();
        } else {
            redirect($this->homeworkobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the homework, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(homework_attempt::IN_PROGRESS, homework_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = homework_attempt_state($this->homeworkobj->get_homework(), $attempt);
        $reviewoptions = mod_homework_display_options::make_from_homework(
                $this->homeworkobj->get_homework(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->homeworkobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->homeworkobj->review_url($attempt->id),
