...
/**
 * Back-end code for handling data about homeworkzes and the current user's attempt.
 *
 * There are classes for loading all the information about a homework and attempts,
...
/**
 * Class for homework exceptions. Just saves a couple of arguments on the
 ...
 * @since     Moodle 2.0
 */
class moodle_homework_exception extends moodle_exception {
    public function __construct($homeworkobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $homeworkobj->view_url();
        }
        parent::__construct($errorcode, 'homework', $link, $a, $debuginfo);
    }
}


/**
 * A class encapsulating a homework and the questions it contains, and making the
...
class homework {
    ...
    protected $cm;
    /** @var stdClass the homework settings from the database. */
    protected $homework;
    /** @var context the homework context. */
    protected $context;

    /** @var array of questions augmented with slot information. */
    protected $questions = null;
    /** @var array of homework_section rows. */
    protected $sections = null;
    /** @var homework_access_manager the access manager for this homework. */
    protected $accessmanager = null;
    /** @var bool whether the current user has capability mod/homework:preview. */
     ...
     *
     * @param object $homework the row from the homework table.
     * @param object $cm the course_module object for this homework.
     * @param object $course the row from the course table for the course we belong to.
     * @param bool $getcontext intended for testing - stops the constructor getting the context.
     */
    public function __construct($homework, $cm, $course, $getcontext = true) {
        $this->homework = $homework;
        $this->cm = $cm;
        $this->homework->cmid = $this->cm->id;
        $this->course = $course;
    ...
    /**
     * Static function to create a new homework object for a specific user.
     *
     * @param int $homeworkid the the homework id.
     * @param int $userid the the userid.
     * @return homework the new homework object
     */
    public static function create($homeworkid, $userid = null) {
        global $DB;

        $homework = homework_access_manager::load_homework_and_settings($homeworkid);
        $course = $DB->get_record('course', array('id' => $homework->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('homework', $homework->id, $course->id, false, MUST_EXIST);

        // Update homework with override information.
        if ($userid) {
            $homework = homework_update_effective_access($homework, $userid);
        }

        return new homework($homework, $cm, $course);
    }

    /**
     * Create a {@link homework_attempt} for an attempt at this homework.
     * @param object $attemptdata row from the homework_attempts table.
     * @return homework_attempt the new homework_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new homework_attempt($attemptdata, $this->homework, $this->cm, $this->course);
    }

    // Functions for loading more data =========================================

    /**
     * Load just basic information about all the questions in this homework.
     */
    public function preload_questions() {
        $this->questions = question_preload_questions(null,
                ...
                '{homework_slots} slot ON slot.homeworkid = :homeworkid AND q.id = slot.questionid',
                array('homeworkid' => $this->homework->id), 'slot.slot');
    }

    /**
     * Fully load some or all of the questions for this homework. You must call
    ...
    /**
     * Get an instance of the {@link \mod_homework\structure} class for this homework.
     * @return \mod_homework\structure describes the questions in the homework.
     */
    public function get_structure() {
        return \mod_homework\structure::create_for_homework($this);
    }
    ...
    /** @return int the homework id. */
    public function get_homeworkid() {
        return $this->homework->id;
    }

    /** @return object the row of the homework table. */
    public function get_homework() {
        return $this->homework;
    }

    /** @return string the name of this homework. */
    public function get_homework_name() {
        return $this->homework->name;
    }

    /** @return int the homework navigation method. */
    public function get_navigation_method() {
        return $this->homework->navmethod;
    }

    /** @return int the number of attempts allowed at this homework (0 = infinite). */
    ...
    /** @return object the module context for this homework. */
    public function get_context() {
        return $this->context;
    }

    /**
     * @return bool wether the current user is someone who previews the homework,
     * rather than attempting it.
     */
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/homework:preview', $this->context);
        }
        return $this->ispreviewuser;
    }

    /**
     * @return whether any questions have been added to this homework.
     */
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'homework', $this->view_url());
            }
    ...
    /**
     * Get all the sections in this homework.
     * @return array 0, 1, 2, ... => homework_sections row from the database.
     */
    public function get_sections() {
        global $DB;
        if ($this->sections === null) {
            $this->sections = array_values($DB->get_records('homework_sections',
                    array('homeworkid' => $this->get_homeworkid()), 'firstslot'));
        }
        return $this->sections;
    }

    /**
     * Return homework_access_manager and instance of the homework_access_manager class
     * for this homework at this time.
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new homework_access_manager($this, $timenow,
                    has_capability('mod/homework:ignoretimelimits', $this->context, null, false));
        }
        return $this->accessmanager;
    }

    /**
     * Wrapper round the has_capability funciton that automatically passes in the homework context.
     */
    ...
    /**
     * Wrapper round the require_capability funciton that automatically passes in the homework context.
     */
    ...
    /**
     * @return string the URL of this homework's view page.
     */
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/homework/view.php?id=' . $this->cm->id;
    }

    /**
     * @return string the URL of this homework's edit page.
     */
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/homework/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/homework/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
    ...
    /**
     * @return string the URL of this homework's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/homework/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/homework/review.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/homework/summary.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
     ...
     * @deprecated since 3.1. This sort of functionality is now entirely handled by homework access rules.
     */
    public function confirm_start_attempt_message($notused) {
        debugging('confirm_start_attempt_message is deprecated. ' .
                'This sort of functionality is now entirely handled by homework access rules.');
        return '';
    }
     ...
     *
     * @param int $when One of the mod_homework_display_options::DURING,
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_homework_display_options::DURING ||
                $when == mod_homework_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_homework_display_options::LATER_WHILE_OPEN && $this->homework->timeclose &&
                $this->homework->reviewattempt & mod_homework_display_options::AFTER_CLOSE) {
            return get_string('noreviewuntil' . $langstrsuffix, 'homework',
                    userdate($this->homework->timeclose, $dateformat));
        } else {
            return get_string('noreview' . $langstrsuffix, 'homework');
        }
    }

    /**
     * @param string $title the name of this particular homework page.
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_homework_exception($this, 'questionnotloaded', $id);
        }
    }

    /**
     * Return all the question types used in this homework.
     *
     * @param  boolean $includepotential if the homework include random questions, setting this flag to true will make the function to
    ...
    public function get_all_question_types_used($includepotential = false) {
        ...
        // We must be careful with random questions, if we find a random question we must assume that the homework may content
...
/**
 * This class extends the homework class to hold data about the state of a particular attempt,
 * in addition to the data about the homework.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class homework_attempt {
    ...
    /** @var int maximum number of slots in the homework for the review page to default to show all. */
    const MAX_SLOTS_FOR_DEFAULT_REVIEW_SHOW_ALL = 50;

    /** @var homework object containing the homework settings. */
    protected $homeworkobj;

    /** @var stdClass the homework_attempts row. */
    protected $attempt;

    /** @var question_usage_by_activity the question usage for this homework attempt. */
    protected $quba;
    ...
    /** @var array of homework_sections rows, with a ->lastslot field added. */
    protected $sections;
    ...
    /** @var mod_homework_display_options cache for the appropriate review options. */
     ...
     * Constructor assuming we already have the necessary data loaded.
     *
     * @param object $attempt the row of the homework_attempts table.
     * @param object $homework the homework object for this attempt and user.
     * @param object $cm the course_module object for this homework.
     * @param object $course the row from the course table for the course we belong to.
     ...
     */
    public function __construct($attempt, $homework, $cm, $course, $loadquestions = true) {
        global $DB;

        $this->attempt = $attempt;
        $this->homeworkobj = new homework($homework, $cm, $course);
        ...
        $this->slots = $DB->get_records('homework_slots',
                array('homeworkid' => $this->get_homeworkid()), 'slot',
     ...
     * @param array $conditions passed to $DB->get_record('homework_attempts', $conditions).
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('homework_attempts', $conditions, '*', MUST_EXIST);
        $homework = homework_access_manager::load_homework_and_settings($attempt->homework);
        $course = $DB->get_record('course', array('id' => $homework->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('homework', $homework->id, $course->id, false, MUST_EXIST);

        // Update homework with override information.
        $homework = homework_update_effective_access($homework, $attempt->userid);

        return new homework_attempt($attempt, $homework, $cm, $course);
    }

    /**
     * Static function to create a new homework_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new homework_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return homework_attempt_state_name($state);
    }
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'homework');
                }
    ...
    public function get_homeworkobj() {
    ...
    /** @return int the course id. */
    public function get_course() {
        return $this->homeworkobj->get_course();
    }
    ...
    /** @return string the name of this homework. */
    public function get_homework_name() {
        return $this->homeworkobj->get_homework_name();
    }

    /** @return int the homework navigation method. */
    public function get_navigation_method() {
        return $this->homeworkobj->get_navigation_method();
    }

    /** @return object the course_module object. */
    public function get_cm() {
        return $this->homeworkobj->get_cm();
    }
     ...
     */
    public function is_preview_user() {
        return $this->homeworkobj->is_preview_user();
    }

    /** @return int the number of attempts allowed at this homework (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->homeworkobj->get_num_attempts_allowed();
    }

    /** @return int number fo pages in this homework. */
     ...
     */
    public function get_access_manager($timenow) {
        return $this->homeworkobj->get_access_manager($timenow);
    }
    ...
    /** @return object the row from the homework_attempts table. */
    ...
    /** @return string one of the homework_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/homework:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the homework in a non-trivial way?
     ...
     * To see how this works, try setting a homework to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the homework
     ...
     *
     * @param question_display_options $options the display options for this homework attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return homework_feedback_for_grade($grade, $this->get_homework(),
                $this->homeworkobj->get_context());
    }
     ...
     */
    public function has_capability($capability, $userid = null, $doanything = true) {
        return $this->homeworkobj->has_capability($capability, $userid, $doanything);
    }
     ...
     */
    public function require_capability($capability, $userid = null, $doanything = true) {
        return $this->homeworkobj->require_capability($capability, $userid, $doanything);
    }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_homework_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/homework:attempt';
        } else {
            $capability = 'mod/homework:reviewmyattempts';
        }

        // These next tests are in a slighly funny order. The point is that the
        // common and most performance-critical case is students attempting a homework
        // so we want to check that permisison first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the homework as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/homework:viewreports') ||
                $this->has_capability('mod/homework:preview')) {
    ...
    public function get_attempt_state() {
        return homework_attempt_state($this->get_homework(), $this->attempt);
    }

    /**
     * Wrapper that the correct mod_homework_display_options for this homework at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = homework_get_review_options($this->get_homework(),
                        $this->attempt, $this->homeworkobj->get_context());
            ...
            $options = mod_homework_display_options::make_from_homework($this->get_homework(),
                    mod_homework_display_options::DURING);
            $options->flags = homework_get_flag_option($this->attempt, $this->homeworkobj->get_context());
            return $options;
        }
    }
     ...
     * @return bool true if this is the last page of the homework.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the homework, or for the
     * whole homework.
     *
    ...
    public function get_question_usage() {
        if (!PHPUNIT_TEST) {
            throw new coding_exception('get_question_usage is only for use in unit tests. ' .
                    'For other operations, use the homework_attempt api, or extend it properly.');
        }
        return $this->quba;
    }
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_homework()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the homework where this question appears.
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the homework this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the homework.
     */
    public function get_question_mark($slot) {
        return homework_format_question_grade($this->get_homework(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = array();
        if ($this->homeworkobj->get_homework()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->homeworkobj->get_homework()->timelimit;
        }
        if ($this->homeworkobj->get_homework()->timeclose) {
            $deadlines[] = $this->homeworkobj->get_homework()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->homeworkobj->get_homework()->graceperiod;
    ...
    // URLs related to this attempt ============================================
    /**
     * @return string homework view url.
     */
    public function view_url() {
        return $this->homeworkobj->view_url();
    }
    ...
    public function start_attempt_url($slot = null, $page = -1) {
        ...
        return $this->homeworkobj->start_attempt_url($page);
    }
    ...
    /**
     * @return string the URL of this homework's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/homework/summary.php', array('attempt' => $this->attempt->id, 'cmid' => $this->get_cmid()));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/homework/processattempt.php');
    }
    ...
    public function cannot_review_message($short = false) {
        return $this->homeworkobj->cannot_review_message(
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'homework'));
        } else {
            return '';
        }
    }
     ...
     * @param mod_homework_renderer $renderer the homework renderer.
     ...
     */
    public function render_question($slot, $reviewing, mod_homework_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions,
                    ...
                    'mod_homework-blocked_question_warning');
        }
     ...
     */
    protected function render_question_helper($slot, $reviewing, $thispageurl, mod_homework_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'homework', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'homework');
        ...
        $placeholderqa->start($this->get_homework()->preferredbehaviour, 1);
     ...
     *
     * @param int $id the id of a question in this homework attempt.
     ...
     * @return string HTML for the question in its current state.
     */
    public function render_question_at_step($slot, $seq, $reviewing, mod_homework_renderer $renderer, $thispageurl = '') {
     ...
     *
     * @param $panelclass The type of panel, homework_attempt_nav_panel or homework_review_nav_panel
     * @param $page the current page number.
     * @param $showall whether we are showing the whole homework on one page. (Used by review.php)
     * @return homework_nav_panel_base the requested object.
     */
    public function get_navigation_panel(mod_homework_renderer $output,
             $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_homework_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->attributes['aria-labelledby'] = 'mod_homework_navblock_title';
        $bc->title = html_writer::span(get_string('homeworknavigation', 'homework'), '', array('id' => 'mod_homework_navblock_title'));
        $bc->content = $output->navigation_panel($panel);
        return $bc;
    }

    /**
     * Return an array of variant URLs to other attempts at this homework.
     *
     * The $url passed in must contain an attempt parameter.
     *
     * The {@link mod_homework_links_to_other_attempts} object returned contains an
     ...
     * @return mod_homework_links_to_other_attempts containing array int => null|moodle_url.
     */
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = homework_get_user_attempts($this->get_homework()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }

        $links = new mod_homework_links_to_other_attempts();
     ...
     * @return mod_homework_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->homeworkobj->get_homework()->graceperiod;
        ...
        // Otherwise, we were in homework_attempt::IN_PROGRESS, and time has now expired.
        // Transition to the appropriate state.
        switch ($this->homeworkobj->get_homework()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if ($becomingoverdue) {
            ...
            $DB->update_record('homework_attempts', $this->attempt);
        }

        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            homework_save_best_grade($this->get_homework(), $this->get_userid());
        }
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_homework\question\qubaids_for_users_attempts(
                $this->get_homeworkid(), $this->get_userid());
        ...
        if ($questiondata->qtype != 'random') {
            ...
            $tagids = homework_retrieve_slot_tag_ids($this->slots[$slot]->id);
            ...
            if ($newqusetionid === null) {
                throw new moodle_exception('notenoughrandomquestions', 'homework',
                        $this->homeworkobj->view_url(), $questiondata);
            }
        }
    ...
    public function process_finish($timestamp, $processsubmitted) {
        ...
        if (!$this->is_preview()) {
            homework_save_best_grade($this->get_homework(), $this->attempt->userid);

            // Trigger event.
            $this->fire_state_transition_event('\mod_homework\event\attempt_submitted', $timestamp);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('homework_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_homework\event\attempt_becameoverdue', $timestamp);

        $transaction->allow_commit();

        homework_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_homework\event\attempt_abandoned', $timestamp);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp) {
        global $USER;
        $homeworkrecord = $this->get_homework();
        $params = array(
            'context' => $this->get_homeworkobj()->get_context(),
        ...
        $event->add_record_snapshot('homework', $this->get_homework());
        $event->add_record_snapshot('homework_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    /**
     * Get a URL for a particular question on a particular page of the homework.
     * Used by {@link attempt_url()} and {@link review_url()}.
     *
     * @param string $script. Used in the URL like /mod/homework/$script.php
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/homework/' . $script . '.php' . $fragment,
    ...
    /**
     * Process responses during an attempt at a homework.
     *
    ...
    public function process_attempt($timenow, $finishattempt, $timeup, $thispage) {
        ...
        // to show the student another page of the homework. Just finish now.
        $graceperiodmin = null;
        ...
        if ($timeup) {
            if ($this->get_homework()->overduehandling == 'graceperiod') {
                if (is_null($graceperiodmin)) {
                    $graceperiodmin = get_config('homework', 'graceperiodmin');
                }
                if ($timenow > $timeclose + $this->get_homework()->graceperiod + $graceperiodmin) {
        ...
        // Update the homework attempt record.
        try {
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('homework_attempts', 'currentpage', $page, array('id' => $this->get_attemptid()));
            return true;
        }
        return false;
    }
    ...
    public function fire_attempt_viewed_event() {
        $params = array(
            ...
            'other' => array(
                'homeworkid' => $this->get_homeworkid()
            )
        );
 ...
 * @since      Moodle 2.9
 */
class homework_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class homework_nav_question_button implements renderable {
 ...
 * @since      Moodle 2.0
 */
abstract class homework_nav_panel_base {
    /** @var homework_attempt */
    ...
    public function __construct(homework_attempt $attemptobj,
    ...
    /**
     * Get the buttons and section headings to go in the homework navigation block.
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            if ($heading = $this->attemptobj->get_heading_before_slot($slot)) {
                $buttons[] = new homework_nav_section_heading(format_string($heading));
            }

            $qa = $this->attemptobj->get_question_attempt($slot);
            $showcorrectness = $this->options->correctness && $qa->has_marks();

            $button = new homework_nav_question_button();
            $button->id          = 'homeworknavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'homework');
            }
            $buttons[] = $button;
        }
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'homework');
        } else {
            return get_string('viewed', 'homework');
        }
    }

    public function render_before_button_bits(mod_homework_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_homework_renderer $output);
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_homework()->showuserpicture == QUIZ_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', array('id' => $this->attemptobj->get_userid()));
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_homework()->showuserpicture == QUIZ_SHOWIMAGE_LARGE) {
...
/**
 * Specialisation of {@link homework_nav_panel_base} for the attempt homework page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class homework_attempt_nav_panel extends homework_nav_panel_base {
    ...
    public function render_before_button_bits(mod_homework_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'homework'),
                array('id' => 'homeworknojswarning'));
    }

    public function render_end_bits(mod_homework_renderer $output) {
        return html_writer::link($this->attemptobj->summary_url(),
                get_string('endtest', 'homework'), array('class' => 'endtestlink')) .
...
/**
 * Specialisation of {@link homework_nav_panel_base} for the review homework page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class homework_review_nav_panel extends homework_nav_panel_base {
    public function get_question_url($slot) {
