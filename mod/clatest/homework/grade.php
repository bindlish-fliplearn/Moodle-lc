...
$cm = get_coursemodule_from_id('homework', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$homework = $DB->get_record('homework', array('id' => $cm->instance), '*', MUST_EXIST);
require_login($course, false, $cm);

$reportlist = homework_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see homework reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/homework/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = homework_get_user_attempts($homework->id, $userid, 'finished');
    $attempt = null;
    ...
    if ($attempt) {
        $attemptobj = new homework_attempt($attempt, $homework, $cm, $course, false);
...
redirect(new moodle_url('/mod/homework/report.php', array(
