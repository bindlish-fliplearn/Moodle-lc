...
/**
 * This file defines the homework grades table.
 *
...
require_once($CFG->dirroot . '/mod/homework/report/attemptsreport_table.php');


/**
 * This is a table subclass for displaying the homework grades report.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class homework_overview_table extends homework_attempts_report_table {
     ...
     * @param homework_overview_options $options
     * @param \core\dml\sql_join $groupstudentsjoins
     * @param \core\dml\sql_join $studentsjoins
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($homework, $context, $qmsubselect,
            homework_overview_options $options, \core\dml\sql_join $groupstudentsjoins,
            \core\dml\sql_join $studentsjoins, $questions, $reporturl) {
        parent::__construct('mod-homework-report-overview-report', $homework , $context,
    ...
    /**
     * Calculate the average overall and question scores for a set of attempts at the homework.
     *
    ...
    public function compute_average_row($label, \core\dml\sql_join $usersjoins) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(homeworkaouter.sumgrades) AS grade, COUNT(homeworkaouter.sumgrades) AS numaveraged
                  FROM {homework_attempts} homeworkaouter
                  JOIN (
                       SELECT DISTINCT homeworka.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON homeworkaouter.id = relevant_attempt_ids.id
                ", $params);
        $record->grade = homework_rescale_grade($record->grade, $this->homework, false);
        ...
        $averagerow = array(
            ...
            'feedbacktext' => strip_tags(homework_report_feedback_for_grade(
                                         $record->grade, $this->homework->id, $this->context))
        );

        if ($this->options->slotmarks) {
            $dm = new question_engine_data_mapper();
            $qubaids = new qubaid_join("{homework_attempts} homeworkaouter
                  JOIN (
                       SELECT DISTINCT homeworka.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON homeworkaouter.id = relevant_attempt_ids.id",
                    'homeworkaouter.uniqueid', '1 = 1', $params);
    ...
    protected function format_average_grade_for_questions($gradeaverages) {
        ...
        foreach ($this->questions as $question) {
            if (isset($gradeaverages[$question->slot]) && $question->maxmark > 0) {
                ...
                $record->grade = homework_rescale_grade(
                        $record->averagefraction * $question->maxmark, $this->homework, false);
    ...
    protected function format_average($record, $question = false) {
        if (is_null($record->grade)) {
            ...
            $average = homework_format_question_grade($this->homework, $record->grade);
        } else {
            $average = homework_format_grade($this->homework, $record->grade);
        }
    ...
    protected function submit_buttons() {
        if (has_capability('mod/homework:regrade', $this->context)) {
            echo '<input type="submit" class="btn btn-secondary m-r-1" name="regrade" value="' .
                    get_string('regradeselected', 'homework_overview') . '"/>';
        }
    ...
    public function col_sumgrades($attempt) {
        ...
        $grade = homework_rescale_grade($attempt->sumgrades, $this->homework);
        if ($this->is_downloading()) {
            return $grade;
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid])) {
            ...
            $newsumgrade = homework_rescale_grade($newsumgrade, $this->homework);
            $oldsumgrade = homework_rescale_grade($oldsumgrade, $this->homework);
            $grade = html_writer::tag('del', $oldsumgrade) . '/' .
                    html_writer::empty_tag('br') . $newsumgrade;
        }
        return html_writer::link(new moodle_url('/mod/homework/review.php',
                array('attempt' => $attempt->attempt)), $grade,
                array('title' => get_string('reviewattempt', 'homework')));
    }

    /**
     * @param string $colname the name of the column.
     * @param object $attempt the row of data - see the SQL in display() in
     * mod/homework/report/overview/report.php to see what fields are present,
    ...
    public function other_cols($colname, $attempt) {
        ...
        if ($question->maxmark == 0) {
            ...
            $grade = homework_rescale_grade(
                    $stepdata->fraction * $question->maxmark, $this->homework, 'question');
        }
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'homework_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'homework_overview');
        }
    }
    ...
    protected function update_sql_after_count($fields, $from, $where, $params) {
        $fields .= ", COALESCE((
                                SELECT MAX(qqr.regraded)
                                  FROM {homework_overview_regrades} qqr
                                 WHERE qqr.questionusageid = homeworka.uniqueid
                          ), -1) AS regraded";
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/homework:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('homework_overview_regrades',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return homework_report_index_by_keys($regradedqs, array('questionusageid', 'slot'));
    }
}
