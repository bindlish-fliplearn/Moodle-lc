...
require_once($CFG->dirroot . '/mod/homework/report/statistics/statistics_form.php');
require_once($CFG->dirroot . '/mod/homework/report/statistics/statistics_table.php');
require_once($CFG->dirroot . '/mod/homework/report/statistics/statistics_question_table.php');
require_once($CFG->dirroot . '/mod/homework/report/statistics/statisticslib.php');
/**
 * The homework statistics report provides summary information about each question in
 * a homework, compared to the whole homework. It also provides a drill-down to more
 * detailed information about each question.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class homework_statistics_report extends homework_default_report {

    /** @var context_module context of this homework.*/
    protected $context;

    /** @var homework_statistics_table instance of table class used for main questions stats table. */
    protected $table;
    ...
    public function display($homework, $cm, $course) {
        ...
        if (!homework_has_questions($homework->id)) {
            $this->print_header_and_tabs($cm, $course, $homework, 'statistics');
            echo homework_no_questions_message($homework, $cm, $this->context);
            return true;
        }
        ...
        $whichattempts = optional_param('whichattempts', $homework->grademethod, PARAM_INT);
        ...
        $reporturl = new moodle_url('/mod/homework/report.php', $pageoptions);

        $mform = new homework_statistics_settings_form($reporturl, compact('homework'));

        $mform->set_data(array('whichattempts' => $whichattempts, 'whichtries' => $whichtries));

        if ($whichattempts != $homework->grademethod) {
        ...
        if (empty($currentgroup)) {
            ...
            // All users who can attempt homeworkzes and who are in the currently selected group.
            $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                    array('mod/homework:reviewmyattempts', 'mod/homework:attempt'), $currentgroup);
        ...
        $qubaids = homework_statistics_qubaids_condition($homework->id, $groupstudentsjoins, $whichattempts);
        ...
        $this->table = new homework_statistics_table();
        if ($everything) {
            $report = get_string('completestatsfilename', 'homework_statistics');
        } else {
            $report = get_string('questionstatsfilename', 'homework_statistics');
        }
        $courseshortname = format_string($course->shortname, true,
                array('context' => context_course::instance($course->id)));
        $filename = homework_report_download_filename($report, $courseshortname, $homework->name);
        $this->table->is_downloading($download, $filename,
                get_string('homeworkstructureanalysis', 'homework_statistics'));
        $questions = $this->load_and_initialise_questions_for_calculations($homework);
        ...
        if (!$nostudentsingroup) {
            ...
            list($homeworkstats, $questionstats) =
                $this->get_all_stats_and_analysis($homework, $whichattempts, $whichtries, $groupstudentsjoins, $questions, $progress);
        } else {
            // Or create empty stats containers.
            $homeworkstats = new \homework_statistics\calculated($whichattempts);
            $questionstats = new \core_question\statistics\questions\all_calculated_for_qubaid_condition();
        }

        // Set up the table.
        $this->table->statistics_setup($homework, $cm->id, $reporturl, $homeworkstats->s());
        ...
        if (!$this->table->is_downloading()) {
            ...
            if (groups_get_activity_groupmode($cm)) {
                ...
                if ($currentgroup && $nostudentsingroup) {
                    $OUTPUT->notification(get_string('nostudentsingroup', 'homework_statistics'));
                }
            }

            if (!$this->table->is_downloading() && $homeworkstats->s() == 0) {
                echo $OUTPUT->notification(get_string('nogradedattempts', 'homework_statistics'));
            }
        ...
        if ($everything) { // Implies is downloading.
            ...
            $homeworkinfo = $homeworkstats->get_formatted_homework_info_data($course, $cm, $homework);
            $this->download_homework_info_table($homeworkinfo);

            if ($homeworkstats->s()) {
                $this->output_homework_structure_analysis_table($questionstats);

                if ($this->table->is_downloading() == 'html' && $homeworkstats->s() != 0) {
                    $this->output_statistics_graph($homework->id, $qubaids);
                }
            ...
            $this->output_individual_question_data($homework, $questionstats->for_subq($qid, $variantno));
            ...
            echo $OUTPUT->box('<a href="' . $reporturl->out() . '">' .
                              get_string('backtohomeworkreport', 'homework_statistics') . '</a>',
            ...
            if ($variantno === null &&
                ...
                if (!$this->table->is_downloading()) {
                    ...
                    echo $OUTPUT->heading(get_string('slotstructureanalysis', 'homework_statistics', $number), 3);
                }
                ...
                $this->output_individual_question_data($homework, $questionstats->for_slot($slot, $variantno));
            ...
            echo $OUTPUT->heading(get_string('homeworkinformation', 'homework_statistics'), 3);
            echo $this->output_caching_info($homeworkstats->timemodified, $homework->id, $groupstudentsjoins, $whichattempts, $reporturl);
            echo $this->everything_download_options($reporturl);
            $homeworkinfo = $homeworkstats->get_formatted_homework_info_data($course, $cm, $homework);
            echo $this->output_homework_info_table($homeworkinfo);
            if ($homeworkstats->s()) {
                echo $OUTPUT->heading(get_string('homeworkstructureanalysis', 'homework_statistics'), 3);
                $this->output_homework_structure_analysis_table($questionstats);
                $this->output_statistics_graph($homework, $qubaids);
            }
        }
     ...
     */
    protected function output_individual_question_data($homework, $questionstat) {
        global $OUTPUT;

        // On-screen display. Show a summary of the question's place in the homework,
        ...
        $questioninfotable->data[] = array(get_string('modulename', 'homework'), $homework->name);
        $questioninfotable->data[] = array(get_string('questionname', 'homework_statistics'),
                $questionstat->question->name.'&nbsp;'.$datumfromtable['actions']);

        if ($questionstat->variant !== null) {
            $questioninfotable->data[] = array(get_string('variant', 'homework_statistics'), $questionstat->variant);

        }
        $questioninfotable->data[] = array(get_string('questiontype', 'homework_statistics'),
                $datumfromtable['icon'] . '&nbsp;' .
                question_bank::get_qtype($questionstat->question->qtype, false)->menu_name() . '&nbsp;' .
                $datumfromtable['icon']);
        $questioninfotable->data[] = array(get_string('positions', 'homework_statistics'),
        ...
        $labels = array(
            's' => get_string('attempts', 'homework_statistics'),
            'facility' => get_string('facility', 'homework_statistics'),
            'sd' => get_string('standarddeviationq', 'homework_statistics'),
            'random_guess_score' => get_string('random_guess_score', 'homework_statistics'),
            'intended_weight' => get_string('intended_weight', 'homework_statistics'),
            'effective_weight' => get_string('effective_weight', 'homework_statistics'),
            'discrimination_index' => get_string('discrimination_index', 'homework_statistics'),
            'discriminative_efficiency' =>
                                get_string('discriminative_efficiency', 'homework_statistics')
        );
        ...
        echo $OUTPUT->heading(get_string('questioninformation', 'homework_statistics'), 3);
        echo html_writer::table($questioninfotable);
        echo $this->render_question_text($questionstat->question);
        echo $OUTPUT->heading(get_string('questionstatistics', 'homework_statistics'), 3);
    ...
    protected function render_question_text($question) {
        ...
        $text = question_rewrite_question_preview_urls($question->questiontext, $question->id,
                ...
                $this->context->id, 'homework_statistics');
    ...
    protected function output_individual_question_response_analysis($question, $variantno, $s, $reporturl, $qubaids,
        ...
        $qtable = new homework_statistics_question_table($question->id);
        $exportclass = $this->table->export_class_instance();
        $qtable->export_class_instance($exportclass);
        if (!$this->table->is_downloading()) {
            // Output an appropriate title.
            echo $OUTPUT->heading(get_string('analysisofresponses', 'homework_statistics'), 3);
            ...
            if (!empty($question->number) && !is_null($variantno)) {
                $questiontabletitle = get_string('analysisnovariant', 'homework_statistics', $a);
            } else if (!empty($question->number)) {
                $questiontabletitle = get_string('analysisno', 'homework_statistics', $a);
            } else if (!is_null($variantno)) {
                $questiontabletitle = get_string('analysisvariant', 'homework_statistics', $a);
            } else {
                $questiontabletitle = get_string('analysisnameonly', 'homework_statistics', $a);
            }

            if ($this->table->is_downloading() == 'html') {
                $questiontabletitle = get_string('analysisofresponsesfor', 'homework_statistics', $questiontabletitle);
            }
    ...
    /**
     * Output the table that lists all the questions in the homework with their statistics.
     *
     ...
     *                                                                                               the homework including subqs and
     *                                                                                               variants.
     */
    protected function output_homework_structure_analysis_table($questionstats) {
        $tooutput = array();
    ...
    /**
     * Return HTML for table of overall homework statistics.
     *
     * @param array $homeworkinfo as returned by {@link get_formatted_homework_info_data()}.
     * @return string the HTML.
     */
    protected function output_homework_info_table($homeworkinfo) {

        $homeworkinfotable = new html_table();
        $homeworkinfotable->align = array('center', 'center');
        $homeworkinfotable->width = '60%';
        $homeworkinfotable->attributes['class'] = 'generaltable titlesleft';
        $homeworkinfotable->data = array();

        foreach ($homeworkinfo as $heading => $value) {
             $homeworkinfotable->data[] = array($heading, $value);
        }

        return html_writer::table($homeworkinfotable);
    }

    /**
     * Download the table of overall homework statistics.
     *
     * @param array $homeworkinfo as returned by {@link get_formatted_homework_info_data()}.
     */
    protected function download_homework_info_table($homeworkinfo) {
        global $OUTPUT;
        ...
        // Do the output.
        $exportclass = $this->table->export_class_instance();
        $exportclass->start_table(get_string('homeworkinformation', 'homework_statistics'));
     ...
     *
     * @param int|object $homeworkorid The homework, or its ID.
     ...
     */
    protected function output_statistics_graph($homeworkorid, $qubaids) {
        global $DB, $PAGE;

        $homework = $homeworkorid;
        if (!is_object($homework)) {
            $homework = $DB->get_record('homework', array('id' => $homeworkorid), '*', MUST_EXIST);
        }
        ...
        $fieldstoplot = [
            'facility' => get_string('facility', 'homework_statistics'),
            'discriminativeefficiency' => get_string('discriminative_efficiency', 'homework_statistics')
        ];
        ...
        $chart->get_xaxis(0, true)->set_label(get_string('position', 'homework_statistics'));
        ...
        $graphname = get_string('statisticsreportgraph', 'homework_statistics');
        echo $output->chart($chart, $graphname);
    }

    /**
     * Get the homework and question statistics, either by loading the cached results,
     ...
     * @return array with 2 elements:    - $homeworkstats The statistics for overall attempt scores.
    ...
    public function get_all_stats_and_analysis(
            $homework, $whichattempts, $whichtries, \core\dml\sql_join $groupstudentsjoins, $questions, $progress = null) {
        ...
        $homeworkcalc = new \homework_statistics\calculator($progress);

        $progress->start_progress('', 3);
        if ($homeworkcalc->get_last_calculated_time($qubaids) === false) {

            // Recalculate now.
            $questionstats = $qcalc->calculate($qubaids);
            $progress->progress(1);

            $homeworkstats = $homeworkcalc->calculate($homework->id, $whichattempts, $groupstudentsjoins, count($questions),
                                              $qcalc->get_sum_of_mark_variance());
            $progress->progress(2);
        } else {
            $homeworkstats = $homeworkcalc->get_cached($qubaids);
            $progress->progress(1);
        ...
        return array($homeworkstats, $questionstats);
    }
    ...
    protected function get_progress_trace_instance() {
        if ($this->progress === null) {
            if (!$this->table->is_downloading()) {
                $this->progress = new \core\progress\display_if_slow(get_string('calculatingallstats', 'homework_statistics'));
    ...
    /**
     * Analyse responses for all questions and sub questions in this homework.
     *
    ...
    /**
     * Return a little form for the user to request to download the full report, including homework stats and response analysis for
    ...
    protected function everything_download_options(moodle_url $reporturl) {
        global $OUTPUT;
        return $OUTPUT->download_dataformat_selector(get_string('downloadeverything', 'homework_statistics'),
     ...
     * @return string HTML.
     */
    protected function output_caching_info($lastcachetime, $homeworkid, $groupstudentsjoins, $whichattempts, $reporturl) {
        global $DB, $OUTPUT;
        ...
        list($fromqa, $whereqa, $qaparams) = homework_statistics_attempts_sql($homeworkid, $groupstudentsjoins, $whichattempts, true);
        $count = $DB->count_records_sql("
                SELECT COUNT(1)
                FROM $fromqa
                WHERE $whereqa
                AND homeworka.timefinish > {$lastcachetime}", $qaparams);
        ...
        $output .= get_string('lastcalculated', 'homework_statistics', $a);
        $output .= $OUTPUT->single_button($recalcualteurl,
                get_string('recalculatenow', 'homework_statistics'));
    ...
    protected function clear_cached_data($qubaids) {
        global $DB;
        $DB->delete_records('homework_statistics', array('hashcode' => $qubaids->get_hash_code()));
    ...
    /**
     * Load the questions in this homework and add some properties to the objects needed in the reports.
     *
     * @param object $homework the homework.
     * @return array of questions for this homework.
     */
    public function load_and_initialise_questions_for_calculations($homework) {
        // Load the questions.
        $questions = homework_report_get_significant_questions($homework);
        $questionids = array();
        foreach ($questions as $question) {
