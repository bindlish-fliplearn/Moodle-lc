...
/**
 * Standard plugin entry points of the homework statistics report.
 *
 ...
 * @package  homework_statistics
 * @category files
 * @param context $previewcontext the homework context
 ...
 */
function homework_statistics_question_preview_pluginfile($previewcontext, $questionid,
    ...
    require_capability('homework/statistics:view', $context);
 ...
 */
function homework_statistics_cron() {
    global $DB;

    mtrace("\n  Cleaning up old homework statistics cache records...", '');

    $expiretime = time() - 5*HOURSECS;
    $DB->delete_records_select('homework_statistics', 'timemodified < ?', array($expiretime));
