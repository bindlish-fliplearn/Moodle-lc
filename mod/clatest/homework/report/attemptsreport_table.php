...
/**
 * Base class for the table used by a {@link homework_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class homework_attempts_report_table extends table_sql {
    ...
    /** @var object the homework settings for the homework we are reporting on. */
    protected $homework;
    ...
    /** @var object mod_homework_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this homework.. */
    protected $questions;
     ...
     * @param mod_homework_attempts_report_options $options
     ...
     */
    public function __construct($uniqueid, $homework, $context, $qmsubselect,
            mod_homework_attempts_report_options $options, \core\dml\sql_join $groupstudentsjoins, \core\dml\sql_join $studentsjoins,
            $questions, $reporturl) {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/homework/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'homework'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return homework_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = homework_report_feedback_for_grade(
                homework_rescale_grade($attempt->sumgrades, $this->homework, false),
                $this->homework->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/homework/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'homework')));
    ...
    public function base_sql(\core\dml\sql_join $allowedstudentsjoins) {
        ...
        // Please note this uniqueid column is not the same as homeworka.uniqueid.
        $fields = 'DISTINCT ' . $DB->sql_concat('u.id', "'#'", 'COALESCE(homeworka.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                homeworka.uniqueid AS usageid,
                homeworka.id AS attempt,
                u.id AS userid,
                u.idnumber, ' . $allnames . ',
                u.picture,
                u.imagealt,
                ...
                homeworka.state,
                homeworka.sumgrades,
                homeworka.timefinish,
                homeworka.timestart,
                CASE WHEN homeworka.timefinish = 0 THEN null
                     WHEN homeworka.timefinish > homeworka.timestart THEN homeworka.timefinish - homeworka.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and homework_attempts tables.
        $from = " {user} u";
        $from .= "\nLEFT JOIN {homework_attempts} homeworka ON
                                    homeworka.userid = u.id AND homeworka.homework = :homeworkid";
        $params = array('homeworkid' => $this->homework->id);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (homeworka.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = homework_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case homework_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'homeworka.id IS NOT NULL AND homeworka.preview = 0';
                break;
            case homework_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "homeworka.preview = 0 AND homeworka.id IS NOT NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case homework_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "homeworka.id IS NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case homework_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "(homeworka.preview = 0 OR homeworka.preview IS NULL) AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (homeworka.state $statesql OR homeworka.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{homework_attempts} {$alias}homeworka", "{$alias}homeworka.uniqueid",
                "{$alias}homeworka.homework = :{$alias}homeworkid", array("{$alias}homeworkid" => $this->sql->params['homeworkid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = homeworka.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join("(
                SELECT DISTINCT homeworka.uniqueid
                  FROM " . $this->sql->from . "
                 WHERE " . $this->sql->where . "
                    ) homeworkasubquery", 'homeworkasubquery.uniqueid',
                    "1 = 1", $this->sql->params);
        }
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['homeworka.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    public function wrap_html_finish() {
        ...
        echo '<a id="checkattempts" href="#">' .
                get_string('selectall', 'homework') . '</a> / ';
        echo '<a id="uncheckattempts" href="#">' .
                get_string('selectnone', 'homework') . '</a> ';
