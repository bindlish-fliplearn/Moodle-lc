...
/**
 * This file defines the homework manual grading report class.
 *
 * @package   homework_grading
...
require_once($CFG->dirroot . '/mod/homework/report/grading/gradingsettings_form.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class homework_grading_report extends homework_default_report {
    ...
    public function display($homework, $cm, $course) {
        ...
        require_capability('mod/homework:grade', $this->context);
        $shownames = has_capability('homework/grading:viewstudentnames', $this->context);
        $showidnumbers = has_capability('homework/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this homework.
        $this->questions = homework_report_get_significant_questions($homework);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'homework_grading');
        }
        ...
        if ($this->currentgroup == self::NO_GROUPS_ALLOWED) {
            ...
            $this->userssql = get_enrolled_sql($this->context,
                    array('mod/homework:reviewmyattempts', 'mod/homework:attempt'), $this->currentgroup);
        }
        ...
        if ($slot && $hasquestions) {
            ...
            if (!$counts || $counts->$grade == 0) {
                redirect($this->list_questions_url(), get_string('alldoneredirecting', 'homework_grading'));
            }
        }

        // Start output.
        $this->print_header_and_tabs($cm, $course, $homework, 'grading');
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "homeworka.homework = :mangrhomeworkid AND
                homeworka.preview = 0 AND
                homeworka.state = :statefinished";
        $params = array('mangrhomeworkid' => $this->cm->instance, 'statefinished' => homework_attempt::FINISHED);
        ...
        if ($currentgroup) {
            ...
            if ($enrolleduserscount < 1) {
                $where .= ' AND homeworka.userid = 0';
            } else {
                $usersjoin = "JOIN ({$userssql[0]}) AS enr ON homeworka.userid = enr.id";
                $params += $userssql[1];
            }
        }

        return new qubaid_join("{homework_attempts} homeworka $usersjoin ", 'homeworka.uniqueid', $where, $params);
    }
    ...
    protected function load_attempts_by_usage_ids($qubaids) {
        ...
        $params[] = homework_attempt::FINISHED;
        $params[] = $this->homework->id;

        $fields = 'homeworka.*, u.idnumber, ';
        $fields .= get_all_user_name_fields(true, 'u');
        $attemptsbyid = $DB->get_records_sql("
                SELECT $fields
                FROM {homework_attempts} homeworka
                JOIN {user} u ON u.id = homeworka.userid
                WHERE homeworka.uniqueid $asql AND homeworka.state = ? AND homeworka.homework = ?",
                $params);
    ...
    protected function base_url() {
        return new moodle_url('/mod/homework/report.php',
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                    ...
                    get_string($gradestring, 'homework_grading'),
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'homework_grading'), 3);
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'homework_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'homework_grading');
        }
        ...
        if (empty($data)) {
            echo $OUTPUT->notification(get_string('nothingfound', 'homework_grading'));
            return;
        }
        ...
        $table->head[] = get_string('qno', 'homework_grading');
        $table->head[] = get_string('qtypeveryshort', 'question');
        $table->head[] = get_string('questionname', 'homework_grading');
        $table->head[] = get_string('tograde', 'homework_grading');
        $table->head[] = get_string('alreadygraded', 'homework_grading');
        if ($includeauto) {
            $table->head[] = get_string('automaticallygraded', 'homework_grading');
        }
        $table->head[] = get_string('total', 'homework_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new homework_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'homework_grading', $a), 3);
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(),
                get_string('backtothelistofquestions', 'homework_grading')),
        ...
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'homework_grading', $a), 3);
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = homework_get_review_options($this->homework, $attempt, $this->context);
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                'type' => 'submit', 'class' => 'btn btn-primary', 'value' => get_string('saveandnext', 'homework_grading'))),
                array('class' => 'mdl-align')) .
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'homework_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'homework_grading', $a);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new homework_attempt($attempt, $this->homework, $this->cm, $this->course);
            ...
            $params = array(
                ...
                'other' => array(
                    'homeworkid' => $attemptobj->get_homeworkid(),
                    'attemptid' => $attemptobj->get_attemptid(),
                    'slot' => $assumedslotforevents
                )
            );
            $events[] = \mod_homework\event\question_manually_graded::create($params);
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        if ($orderby == 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON homeworka.userid = u.id ";
