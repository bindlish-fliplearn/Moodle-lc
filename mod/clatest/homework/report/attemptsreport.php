...
/**
 * Base class for homework reports that are basically a table with one row for each attempt.
 *
 * @copyright 2010 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class homework_attempts_report extends homework_default_report {
    ...
    /** @var object the homework context. */
    protected $context;

    /** @var mod_homework_attempts_report_form The settings form to use. */
    protected $form;
     ...
     */
    protected function init($mode, $formclass, $homework, $cm, $course) {
        $this->mode = $mode;
        ...
        $this->qmsubselect = homework_report_qm_filter_select($homework);

        $this->form = new $formclass($this->get_base_url(),
                array('homework' => $homework, 'currentgroup' => $currentgroup, 'context' => $this->context));
    ...
    protected function get_students_joins($cm, $course = null) {
        ...
        $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                array('mod/homework:attempt', 'mod/homework:reviewmyattempts'), $currentgroup);

        return array($currentgroup, $studentsjoins, $groupstudentsjoins, $groupstudentsjoins);
    }

    /**
     * Outputs the things you commonly want at the top of a homework report.
     *
     ...
     * @param mod_homework_attempts_report_options $options the current report settings.
     * @param int $currentgroup the current group.
     * @param bool $hasquestions whether there are any questions in the homework.
     * @param bool $hasstudents whether there are any relevant students.
     */
    protected function print_standard_header_and_messages($cm, $course, $homework,
            $options, $currentgroup, $hasquestions, $hasstudents) {
        global $OUTPUT;

        $this->print_header_and_tabs($cm, $course, $homework, $this->mode);
        ...
        if ($strattemptnum = homework_num_attempt_summary($homework, $cm, true, $currentgroup)) {
            echo '<div class="homeworkattemptcounts">' . $strattemptnum . '</div>';
        }
...
//        }
    ...
    protected function add_state_column(&$columns, &$headers) {
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'homework');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'homework');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'homework');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'homework');
    }
     ...
     * @param bool $usercanseegrades whether the user is allowed to see grades for this homework.
     * @param array $columns the list of columns. Added to.
     * @param array $headers the columns headings. Added to.
     * @param bool $includefeedback whether to include the feedbacktext columns
     */
    protected function add_grade_columns($homework, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            $columns[] = 'sumgrades';
            $headers[] = get_string('grade', 'homework') . '/' .
                    homework_format_grade($homework, $homework->grade);
        }

        if ($includefeedback && homework_has_feedback($homework)) {
            $columns[] = 'feedbacktext';
            $headers[] = get_string('feedback', 'homework');
        }
    }
     ...
     * @param mod_homework_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_homework_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/homework:deleteattempts', $this->context);
                    $this->delete_selected_attempts($homework, $cm, $attemptids, $allowedjoins);
    ...
    /**
     * Delete the homework attempts
     * @param object $homework the homework settings. Attempts that don't belong to
     * this homework are not deleted.
     ...
     */
    protected function delete_selected_attempts($homework, $cm, $attemptids, \core\dml\sql_join $allowedjoins) {
        global $DB;
        ...
        foreach ($attemptids as $attemptid) {
            if (empty($allowedjoins->joins)) {
                $sql = "SELECT homeworka.*
                         ...
                         WHERE homeworka.id = :attemptid";
            } else {
                $sql = "SELECT homeworka.*
                          FROM {homework_attempts} homeworka
                          JOIN {user} u ON u.id = homeworka.userid
                        {$allowedjoins->joins}
                         WHERE {$allowedjoins->wheres} AND homeworka.id = :attemptid";
            }
            $params = $allowedjoins->params + array('attemptid' => $attemptid);
            $attempt = $DB->get_record_sql($sql, $params);
            if (!$attempt || $attempt->homework != $homework->id || $attempt->preview != 0) {
                // Ensure the attempt exists, belongs to this homework and belongs to
                // a student included in the report. If not skip.
                continue;
            }

            // Set the course module id before calling homework_delete_attempt().
            $homework->cmid = $cm->id;
