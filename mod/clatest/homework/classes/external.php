 ...
 * @since      Moodle 3.1
 */
class mod_homework_external extends external_api {

    /**
     * Describes the parameters for get_homeworkzes_by_courses.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_homeworkzes_by_courses_parameters() {
    ...
    /**
     * Returns a list of homeworkzes in a provided list of courses,
     * if no list is provided all homeworkzes that the user can view will be returned.
     *
     * @param array $courseids Array of course ids
     * @return array of homeworkzes details
     * @since Moodle 3.1
     */
    public static function get_homeworkzes_by_courses($courseids = array()) {
        global $USER;

        $warnings = array();
        $returnedhomeworkzes = array();

        $params = array(
            'courseids' => $courseids,
        );
        $params = self::validate_parameters(self::get_homeworkzes_by_courses_parameters(), $params);
        ...
        if (!empty($params['courseids'])) {
            ...
            // Get the homeworkzes in this course, this function checks users visibility permissions.
            // We can avoid then additional validate_context calls.
            $homeworkzes = get_all_instances_in_courses("homework", $courses);
            foreach ($homeworkzes as $homework) {
                ...
                $homework = homework_update_effective_access($homework, $USER->id);

                // Entry to return.
                $homeworkdetails = array();
                // First, we return information that any user can see in the web interface.
                $homeworkdetails['id'] = $homework->id;
                $homeworkdetails['coursemodule']      = $homework->coursemodule;
                $homeworkdetails['course']            = $homework->course;
                $homeworkdetails['name']              = external_format_string($homework->name, $context->id);

                if (has_capability('mod/homework:view', $context)) {
                    // Format intro.
                    list($homeworkdetails['intro'], $homeworkdetails['introformat']) = external_format_text($homework->intro,
                                                                    $homework->introformat, $context->id, 'mod_homework', 'intro', null);

                    $homeworkdetails['introfiles'] = external_util::get_area_files($context->id, 'mod_homework', 'intro', false, false);
                    ...
                    $hasfeedback = homework_has_feedback($homework);
                    $homeworkdetails['hasfeedback'] = (!empty($hasfeedback)) ? 1 : 0;

                    $timenow = time();
                    $homeworkobj = homework::create($homework->id, $USER->id);
                    $accessmanager = new homework_access_manager($homeworkobj, $timenow, has_capability('mod/homework:ignoretimelimits',
                                                                $context, null, false));

                    // Fields the user could see if have access to the homework.
                    if (!$accessmanager->prevent_access()) {
                        $homeworkdetails['hasquestions'] = (int) $homeworkobj->has_questions();
                        $homeworkdetails['autosaveperiod'] = get_config('homework', 'autosaveperiod');
                    ...
                    foreach ($viewablefields as $field) {
                        $homeworkdetails[$field] = $homework->{$field};
                    }
                }
                $returnedhomeworkzes[] = $homeworkdetails;
            }
        }
        $result = array();
        $result['homeworkzes'] = $returnedhomeworkzes;
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_homeworkzes_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_homeworkzes_by_courses_returns() {
        return new external_single_structure(
            array(
                'homeworkzes' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            'id' => new external_value(PARAM_INT, 'Standard Moodle primary key.'),
                            'course' => new external_value(PARAM_INT, 'Foreign key reference to the course this homework is part of.'),
                            ...
                            'timeopen' => new external_value(PARAM_INT, 'The time when this homework opens. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timeclose' => new external_value(PARAM_INT, 'The time when this homework closes. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timelimit' => new external_value(PARAM_INT, 'The time limit for homework attempts, in seconds.',
                                                                VALUE_OPTIONAL),
                            ...
                            'canredoquestions' => new external_value(PARAM_INT, 'Allows students to redo any completed question
                                                                        within a homework attempt.', VALUE_OPTIONAL),
                            ...
                            'reviewattempt' => new external_value(PARAM_INT, 'Whether users are allowed to review their homework
                                                                    attempts at various times. This is a bit field, decoded by the
                                                                    mod_homework_display_options class. It is formed by ORing together
                                                                    the constants defined there.', VALUE_OPTIONAL),
                            'reviewcorrectness' => new external_value(PARAM_INT, 'Whether users are allowed to review their homework
                                                                        attempts at various times.
                                                                        A bit field, like reviewattempt.', VALUE_OPTIONAL),
                            'reviewmarks' => new external_value(PARAM_INT, 'Whether users are allowed to review their homework attempts
                            ...
                            'reviewspecificfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            homework attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'reviewgeneralfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            homework attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'reviewrightanswer' => new external_value(PARAM_INT, 'Whether users are allowed to review their homework
                                                                        attempts at various times. A bit field, like
                                                                        reviewattempt.', VALUE_OPTIONAL),
                            'reviewoverallfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their homework
                                                                            attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'questionsperpage' => new external_value(PARAM_INT, 'How often to insert a page break when editing
                                                                        the homework, or when shuffling the question order.',
                                                                        VALUE_OPTIONAL),
                            'navmethod' => new external_value(PARAM_ALPHA, 'Any constraints on how the user is allowed to navigate
                                                                around the homework. Currently recognised values are
                            ...
                            'sumgrades' => new external_value(PARAM_FLOAT, 'The total of all the question instance maxmarks.',
                                                                VALUE_OPTIONAL),
                            'grade' => new external_value(PARAM_FLOAT, 'The total that the homework overall grade is scaled to be
                                                            out of.', VALUE_OPTIONAL),
                            'timecreated' => new external_value(PARAM_INT, 'The time when the homework was added to the course.',
                                                                VALUE_OPTIONAL),
                            ...
                            'password' => new external_value(PARAM_RAW, 'A password that the student must enter before starting or
                                                                continuing a homework attempt.', VALUE_OPTIONAL),
                            'subnet' => new external_value(PARAM_RAW, 'Used to restrict the IP addresses from which this homework can
                            ...
                            'completionattemptsexhausted' => new external_value(PARAM_INT, 'Mark homework complete when the student has
                            ...
                            'allowofflineattempts' => new external_value(PARAM_INT, 'Whether to allow the homework to be attempted
                                                                            offline in the mobile app', VALUE_OPTIONAL),
                            'autosaveperiod' => new external_value(PARAM_INT, 'Auto-save delay', VALUE_OPTIONAL),
                            'hasfeedback' => new external_value(PARAM_INT, 'Whether the homework has any non-blank feedback text',
                                                                VALUE_OPTIONAL),
                            'hasquestions' => new external_value(PARAM_INT, 'Whether the homework has questions', VALUE_OPTIONAL),
    ...
    /**
     * Utility function for validating a homework.
     *
     * @param int $homeworkid homework instance id
     * @return array array containing the homework, course, context and course module objects
     * @since  Moodle 3.1
     */
    protected static function validate_homework($homeworkid) {
        global $DB;

        // Request and permission validation.
        $homework = $DB->get_record('homework', array('id' => $homeworkid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($homework, 'homework');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        return array($homework, $course, $cm, $context);
    }

    /**
     * Describes the parameters for view_homework.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function view_homework_parameters() {
        return new external_function_parameters (
            array(
                'homeworkid' => new external_value(PARAM_INT, 'homework instance id'),
            )
        );
    }
     ...
     */
    public static function view_homework($homeworkid) {
        global $DB;

        $params = self::validate_parameters(self::view_homework_parameters(), array('homeworkid' => $homeworkid));
        $warnings = array();

        list($homework, $course, $cm, $context) = self::validate_homework($params['homeworkid']);

        // Trigger course_module_viewed event and completion.
        homework_view($homework, $course, $cm, $context);
    ...
    /**
     * Describes the view_homework return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function view_homework_returns() {
        return new external_single_structure(
            array(
    ...
    public static function get_user_attempts_parameters() {
        return new external_function_parameters (
            array(
                ...
                'status' => new external_value(PARAM_ALPHA, 'homework status: all, finished or unfinished', VALUE_DEFAULT, 'finished'),
    ...
    /**
     * Return a list of attempts for the given homework and user.
     *
     * @param int $homeworkid homework instance id
     * @param int $userid user id
     * @param string $status homework status: all, finished or unfinished
     * @param bool $includepreviews whether to include previews or not
     * @return array of warnings and the list of attempts
     * @since Moodle 3.1
     * @throws invalid_parameter_exception
     */
    public static function get_user_attempts($homeworkid, $userid = 0, $status = 'finished', $includepreviews = false) {
        global $DB, $USER;
        ...
        if ($USER->id != $user->id) {
            require_capability('mod/homework:viewreports', $context);
        }

        $attempts = homework_get_user_attempts($homework->id, $user->id, $params['status'], $params['includepreviews']);
    ...
    private static function attempt_structure() {
        return new external_single_structure(
            array(
                ...
                'homework' => new external_value(PARAM_INT, 'Foreign key reference to the homework that was attempted.',
                                                VALUE_OPTIONAL),
                'userid' => new external_value(PARAM_INT, 'Foreign key reference to the user whose attempt this is.',
                                                VALUE_OPTIONAL),
                'attempt' => new external_value(PARAM_INT, 'Sequentially numbers this students attempts at this homework.',
                                                VALUE_OPTIONAL),
                'uniqueid' => new external_value(PARAM_INT, 'Foreign key reference to the question_usage that holds the
                                                    details of the the question_attempts that make up this homework
                ...
                'timecheckstate' => new external_value(PARAM_INT, 'Next time homework cron should check attempt for
    ...
    /**
     * Get the best current grade for the given user on a homework.
     *
     * @param int $homeworkid homework instance id
     * @param int $userid user id
     ...
     * @since Moodle 3.1
     */
    public static function get_user_best_grade($homeworkid, $userid = 0) {
        global $DB, $USER;
        ...
        $result = array();
        $grade = homework_get_best_grade($homework, $user->id);
    ...
    public static function get_user_best_grade_returns() {
        return new external_single_structure(
            array(
                'hasgrade' => new external_value(PARAM_BOOL, 'Whether the user has a grade on the given homework.'),
     ...
     * @since Moodle 3.1
     */
    public static function get_combined_review_options($homeworkid, $userid = 0) {
        global $DB, $USER;
        ...
        $attempts = homework_get_user_attempts($homework->id, $user->id, 'all', true);

        $result = array();
        $result['someoptions'] = [];
        $result['alloptions'] = [];

        list($someoptions, $alloptions) = homework_get_combined_reviewoptions($homework, $attempts);
    ...
    /**
     * Starts a new attempt at a homework.
     *
     * @param int $homeworkid homework instance id
     ...
     */
    public static function start_attempt($homeworkid, $preflightdata = array(), $forcenew = false) {
        global $DB, $USER;
        ...
        if (!$homeworkobj->has_questions()) {
            throw new moodle_homework_exception($homeworkobj, 'noquestionsfound');
        }
        ...
        list($currentattemptid, $attemptnumber, $lastattempt, $messages, $page) =
            homework_validate_new_attempt($homeworkobj, $accessmanager, $forcenew, -1, false);
        ...
        if (!$homeworkobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new moodle_homework_exception($homeworkobj, array_shift($errors));
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == homework_attempt::OVERDUE) {
                    throw new moodle_homework_exception($homeworkobj, 'stateoverdue');
                } else {
                    throw new moodle_homework_exception($homeworkobj, 'attemptstillinprogress');
                }
            }
            $offlineattempt = WS_SERVER ? true : false;
            $attempt = homework_prepare_and_start_new_attempt($homeworkobj, $attemptnumber, $lastattempt, $offlineattempt);
        }
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        $attemptobj = homework_attempt::create($params['attemptid']);
        ...
        if ($attemptobj->get_userid() != $USER->id) {
            throw new moodle_homework_exception($attemptobj->get_homeworkobj(), 'notyourattempt');
        }
        ...
        if ($checkaccessrules) {
            ...
            if (!$ispreviewuser && $messages) {
                throw new moodle_homework_exception($attemptobj->get_homeworkobj(), 'attempterror');
            }
        }

        // Attempt closed?.
        if ($attemptobj->is_finished()) {
            throw new moodle_homework_exception($attemptobj->get_homeworkobj(), 'attemptalreadyclosed');
        } else if ($failifoverdue && $attemptobj->get_state() == homework_attempt::OVERDUE) {
            throw new moodle_homework_exception($attemptobj->get_homeworkobj(), 'stateoverdue');
        }

        // User submitted data (like the homework password).
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new moodle_homework_exception($attemptobj->get_homeworkobj(), array_shift($errors));
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new moodle_homework_exception($attemptobj->get_homeworkobj(), 'Invalid page number');
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new moodle_homework_exception($attemptobj->get_homeworkobj(), 'Out of sequence access');
            }
            ...
            if (empty($slots)) {
                throw new moodle_homework_exception($attemptobj->get_homeworkobj(), 'noquestionsfound');
            }
        }
     ...
     *
     * @return external_single_structure the question data. Some fields may not be returned depending on the homework display settings.
     * @since  Moodle 3.1
    ...
    private static function question_structure() {
        return new external_single_structure(
            array(
                ...
                'page' => new external_value(PARAM_INT, 'page of the homework this question appears on'),
                ...
                'number' => new external_value(PARAM_INT, 'question ordering number in the homework', VALUE_OPTIONAL),
                'state' => new external_value(PARAM_ALPHA, 'the state where the question is in.
                    It will not be returned if the user cannot see it due to the homework display correctness settings.',
                    VALUE_OPTIONAL),
            ...
            'The question data. Some fields may not be returned depending on the homework display settings.'
        );
    }
     ...
     *
     * @param  homework_attempt  $attemptobj  the homework attempt object
     * @param  bool  $review  whether if we are in review mode or not
     * @param  mixed  $page  string 'all' or integer page number
     * @return array array of questions including data
     */
    private static function get_attempt_questions_data(homework_attempt $attemptobj, $review, $page = 'all') {
        global $PAGE;

        $questions = array();
        $contextid = $attemptobj->get_homeworkobj()->get_context()->id;
        $displayoptions = $attemptobj->get_display_options($review);
        $renderer = $PAGE->get_renderer('mod_homework');
    ...
    /**
     * Returns information for the given attempt page for a homework attempt in progress.
     *
     ...
     * @since Moodle 3.1
     * @throws moodle_homework_exceptions
     */
        ...
        return new external_single_structure(
            array(
                ...
                'messages' => new external_multiple_structure(
                    ...
                    'access messages, will only be returned for users with mod/homework:preview capability,
    ...
    /**
     * Returns a summary of a homework attempt before it is submitted.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Processes save requests during the homework. This function is intended for the homework auto-save feature.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Process responses during an attempt at a homework and also deals with attempts finishing.
     *
     * @param int $attemptid attempt id
     * @param array $data the data to be saved
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new moodle_homework_exception($attemptobj->get_homeworkobj(), 'attemptclosed');
    ...
    public static function get_attempt_review($attemptid, $page = -1) {
        ...
        foreach ($summarydata as $key => $data) {
            ...
            $result['additionaldata'][] = array(
                'id' => $key,
                'title' => $data['title'], $attemptobj->get_homeworkobj()->get_context()->id,
                'content' => $data['content'],
            );
        }

        // Feedback if there is any, and the user is allowed to see it now.
        $grade = homework_rescale_grade($attemptobj->get_attempt()->sumgrades, $attemptobj->get_homework(), false);
    ...
    public static function get_attempt_review_returns() {
        return new external_single_structure(
            array(
                'grade' => new external_value(PARAM_RAW, 'grade for the homework (or empty or "notyetgraded")'),
                'attempt' => self::attempt_structure(),
     ...
     * @since Moodle 3.1
     */
    public static function get_homework_feedback_for_grade_parameters() {
        return new external_function_parameters (
            array(
                'homeworkid' => new external_value(PARAM_INT, 'homework instance id'),
    ...
    /**
     * Get the feedback text that should be show to a student who got the given grade in the given homework.
     *
     * @param int $homeworkid homework instance id
     ...
     * @throws moodle_exception
     */
    public static function get_homework_feedback_for_grade($homeworkid, $grade) {
        global $DB;
        ...
        $params = self::validate_parameters(self::get_homework_feedback_for_grade_parameters(), $params);
        $warnings = array();
        ...
        $feedback = homework_feedback_record_for_grade($params['grade'], $homework);
        if (!empty($feedback->feedbacktext)) {
            list($text, $format) = external_format_text($feedback->feedbacktext, $feedback->feedbacktextformat, $context->id,
                                                        'mod_homework', 'feedback', $feedback->id);
            $result['feedbacktext'] = $text;
            $result['feedbacktextformat'] = $format;
            $feedbackinlinefiles = external_util::get_area_files($context->id, 'mod_homework', 'feedback', $feedback->id);
    ...
    /**
     * Describes the get_homework_feedback_for_grade return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_homework_feedback_for_grade_returns() {
        return new external_single_structure(
            array(
    ...
    /**
     * Describes the parameters for get_homework_access_information.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_homework_access_information_parameters() {
        return new external_function_parameters (
            array(
                'homeworkid' => new external_value(PARAM_INT, 'homework instance id')
            )
    ...
    /**
     * Return access information for a given homework.
     *
     * @param int $homeworkid homework instance id
     ...
     * @throws  moodle_homework_exception
     */
    public static function get_homework_access_information($homeworkid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_homework_access_information_parameters(), $params);

        list($homework, $course, $cm, $context) = self::validate_homework($params['homeworkid']);

        $result = array();
        // Capabilities first.
        $result['canattempt'] = has_capability('mod/homework:attempt', $context);;
        $result['canmanage'] = has_capability('mod/homework:manage', $context);;
        $result['canpreview'] = has_capability('mod/homework:preview', $context);;
        $result['canreviewmyattempts'] = has_capability('mod/homework:reviewmyattempts', $context);;
        $result['canviewreports'] = has_capability('mod/homework:viewreports', $context);;

        // Access manager now.
        $homeworkobj = homework::create($cm->instance, $USER->id);
        $ignoretimelimits = has_capability('mod/homework:ignoretimelimits', $context, null, false);
        $timenow = time();
        $accessmanager = new homework_access_manager($homeworkobj, $timenow, $ignoretimelimits);
    ...
    /**
     * Describes the get_homework_access_information return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_homework_access_information_returns() {
        return new external_single_structure(
            array(
                'canattempt' => new external_value(PARAM_BOOL, 'Whether the user can do the homework or not.'),
                'canmanage' => new external_value(PARAM_BOOL, 'Whether the user can edit the homework settings or not.'),
                'canpreview' => new external_value(PARAM_BOOL, 'Whether the user can preview the homework or not.'),
                'canreviewmyattempts' => new external_value(PARAM_BOOL, 'Whether the users can review their previous attempts
                                                                or not.'),
                'canviewreports' => new external_value(PARAM_BOOL, 'Whether the user can view the homework reports or not.'),
    ...
    /**
     * Return access information for a given attempt in a homework.
     *
     * @param int $homeworkid homework instance id
     ...
     * @throws  moodle_homework_exception
     */
    public static function get_attempt_access_information($homeworkid, $attemptid = 0) {
        global $DB, $USER;
        ...
        $attempts = homework_get_user_attempts($homework->id, $USER->id, 'finished', true);
        $lastfinishedattempt = end($attempts);
        if ($unfinishedattempt = homework_get_user_attempt_unfinished($homework->id, $USER->id)) {
            $attempts[] = $unfinishedattempt;

            // Check if the attempt is now overdue. In that case the state will change.
            $homeworkobj->create_attempt_object($unfinishedattempt)->handle_if_time_expired(time(), false);

            if ($unfinishedattempt->state != homework_attempt::IN_PROGRESS and $unfinishedattempt->state != homework_attempt::OVERDUE) {
    ...
    /**
     * Describes the parameters for get_homework_required_qtypes.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_homework_required_qtypes_parameters() {
        return new external_function_parameters (
            array(
                'homeworkid' => new external_value(PARAM_INT, 'homework instance id')
            )
    ...
    /**
     * Return the potential question types that would be required for a given homework.
     ...
     * @throws  moodle_homework_exception
     */
    public static function get_homework_required_qtypes($homeworkid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_homework_required_qtypes_parameters(), $params);
        ...
        // Question types used.
        $result = array();
        $result['questiontypes'] = $homeworkobj->get_all_question_types_used(true);
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_homework_required_qtypes return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_homework_required_qtypes_returns() {
        return new external_single_structure(
            array(
                'questiontypes' => new external_multiple_structure(
                                    new external_value(PARAM_PLUGIN, 'question type'), 'list of question types used in the homework'),
                'warnings' => new external_warnings(),
            )
        );
    }
