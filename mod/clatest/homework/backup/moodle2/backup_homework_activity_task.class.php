...
/**
 * Defines backup_homework_activity_task class
 *
...
require_once($CFG->dirroot . '/mod/homework/backup/moodle2/backup_homework_stepslib.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_homework_activity_task extends backup_activity_task {
    ...
    protected function define_my_steps() {
        // Generate the homework.xml file containing all the homework information
        // and annotating used questions.
        $this->add_step(new backup_homework_activity_structure_step('homework_structure', 'homework.xml'));

        // Note: Following  steps must be present
        // in all the activities using question banks (only homework for now)
    ...
    public static function encode_content_links($content) {
        ...
        // Link to the list of homeworkzes.
        $search="/(".$base."\/mod\/homework\/index.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@QUIZINDEX*$2@$', $content);

        // Link to homework view by moduleid.
        $search="/(".$base."\/mod\/homework\/view.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@QUIZVIEWBYID*$2@$', $content);

        // Link to homework view by homeworkid.
        $search="/(".$base."\/mod\/homework\/view.php\?q\=)([0-9]+)/";
