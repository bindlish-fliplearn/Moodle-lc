...
/**
 * Define all the backup steps that will be used by the backup_homework_activity_task
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_homework_activity_structure_step extends backup_questions_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $homework = new backup_nested_element('homework', array('id'), array(
        ...
        $this->add_subplugin_structure('homeworkaccess', $homework, true);
        ...
        $this->add_subplugin_structure('homeworkaccess', $attempt, true);

        // Build the tree.
        $homework->add_child($qinstances);
        $qinstances->add_child($qinstance);

        $qinstance->add_child($qinstancetags);
        $qinstancetags->add_child($qinstancetag);

        $homework->add_child($sections);
        $sections->add_child($section);

        $homework->add_child($feedbacks);
        $feedbacks->add_child($feedback);

        $homework->add_child($overrides);
        $overrides->add_child($override);

        $homework->add_child($grades);
        $grades->add_child($grade);

        $homework->add_child($attempts);
        $attempts->add_child($attempt);

        // Define sources.
        $homework->set_source_table('homework', array('id' => backup::VAR_ACTIVITYID));

        $qinstance->set_source_table('homework_slots',
                array('homeworkid' => backup::VAR_PARENTID));

        $qinstancetag->set_source_table('homework_slot_tags',
                array('slotid' => backup::VAR_PARENTID));

        $section->set_source_table('homework_sections',
                array('homeworkid' => backup::VAR_PARENTID));

        $feedback->set_source_table('homework_feedback',
                array('homeworkid' => backup::VAR_PARENTID));

        // Quiz overrides to backup are different depending of user info.
        $overrideparams = array('homework' => backup::VAR_PARENTID);
        if (!$userinfo) { //  Without userinfo, skip user overrides.
            $overrideparams['userid'] = backup_helper::is_sqlparam(null);

        }
        $override->set_source_table('homework_overrides', $overrideparams);

        // All the rest of elements only happen if we are including user info.
        if ($userinfo) {
            $grade->set_source_table('homework_grades', array('homework' => backup::VAR_PARENTID));
            $attempt->set_source_sql('
                    SELECT *
                    FROM {homework_attempts}
                    WHERE homework = :homework AND preview = 0',
                    array('homework' => backup::VAR_PARENTID));
        }

        // Define source alias.
        $homework->set_source_alias('attempts', 'attempts_number');
        ...
        $homework->annotate_files('mod_homework', 'intro', null); // This file area hasn't itemid.
        $feedback->annotate_files('mod_homework', 'feedback', 'id');

        // Return the root element (homework), wrapped into standard activity structure.
        return $this->prepare_activity_structure($homework);
    }
}
