...
/**
 * This page handles listing of homework overrides
 *
...
$url = new moodle_url('/mod/homework/overrides.php', array('cmid'=>$cm->id, 'mode'=>$mode));
...
$PAGE->set_title(get_string('overrides', 'homework'));
...
$sql = 'SELECT o.id
            FROM {homework_overrides} o LEFT JOIN {groups} g
            ON o.groupid = g.id
...
if (!empty($orphaned)) {
    $DB->delete_records_list('homework_overrides', 'id', array_keys($orphaned));
}
...
if ($groupmode) {
    ...
    $sql = 'SELECT o.*, g.name
                ...
                WHERE o.homework = :homeworkid
                ORDER BY g.name';
    $params = array('homeworkid' => $homework->id);
} else {
...
$overridedeleteurl = new moodle_url('/mod/homework/overridedelete.php');
$overrideediturl = new moodle_url('/mod/homework/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    if (!$groupmode) {
        if (!has_capability('mod/homework:attempt', $context, $override->userid)) {
            // User not allowed to take the homework.
            $active = false;
    ...
    if (isset($override->timeopen)) {
        $fields[] = get_string('homeworkopens', 'homework');
        $values[] = $override->timeopen > 0 ?
                userdate($override->timeopen) : get_string('noopen', 'homework');
    }

    // Format timeclose.
    if (isset($override->timeclose)) {
        $fields[] = get_string('homeworkcloses', 'homework');
        $values[] = $override->timeclose > 0 ?
                userdate($override->timeclose) : get_string('noclose', 'homework');
    }

    // Format timelimit.
    if (isset($override->timelimit)) {
        $fields[] = get_string('timelimit', 'homework');
        $values[] = $override->timelimit > 0 ?
                format_time($override->timelimit) : get_string('none', 'homework');
    }

    // Format number of attempts.
    if (isset($override->attempts)) {
        $fields[] = get_string('attempts', 'homework');
    ...
    if (isset($override->password)) {
        $fields[] = get_string('requirepassword', 'homework');
        $values[] = $override->password !== '' ?
                get_string('enabled', 'homework') : get_string('none', 'homework');
    }
...
echo html_writer::start_tag('div', array('id' => 'homeworkoverrides'));
if (count($table->data)) {
    echo html_writer::table($table);
}
if ($hasinactive) {
    echo $OUTPUT->notification(get_string('inactiveoverridehelp', 'homework'), 'dimmed_text');
}
...
if ($groupmode) {
    if (empty($groups)) {
        // There are no groups.
        echo $OUTPUT->notification(get_string('groupsnone', 'homework'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'addgroup', 'cmid' => $cm->id)),
            get_string('addnewgroupoverride', 'homework'), 'post', $options);
} else {
    $users = array();
    // See if there are any students in the homework.
    $users = get_users_by_capability($context, 'mod/homework:attempt', 'u.id');
    $info = new \core_availability\info_module($cm);
    $users = $info->filter_user_list($users);

    if (empty($users)) {
        // There are no students.
        echo $OUTPUT->notification(get_string('usersnone', 'homework'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'adduser', 'cmid' => $cm->id)),
            get_string('addnewuseroverride', 'homework'), 'get', $options);
}
