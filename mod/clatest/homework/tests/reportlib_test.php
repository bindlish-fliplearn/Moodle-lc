...
/**
 * Unit tests for (some of) mod/homework/report/reportlib.php
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU Public License
 */
class mod_homework_reportlib_testcase extends advanced_testcase {
    public function test_homework_report_index_by_keys() {
        $datum = array();
        $object = new stdClass();
        $object->qid = 3;
        $object->aid = 101;
        ...
        $indexed = homework_report_index_by_keys($datum, array('aid', 'qid'));
        ...
        $indexed = homework_report_index_by_keys($datum, array('aid', 'qid'), false);
    ...
    public function test_homework_report_scale_summarks_as_percentage() {
        $homework = new stdClass();
        $homework->sumgrades = 10;
        $homework->decimalpoints = 2;

        $this->assertEquals('12.34567%',
            homework_report_scale_summarks_as_percentage(1.234567, $homework, false));
        $this->assertEquals('12.35%',
            homework_report_scale_summarks_as_percentage(1.234567, $homework, true));
        $this->assertEquals('-',
            homework_report_scale_summarks_as_percentage('-', $homework, true));
    }

    public function test_homework_report_qm_filter_select_only_one_attempt_allowed() {
        $homework = new stdClass();
        $homework->attempts = 1;
        $this->assertSame('', homework_report_qm_filter_select($homework));
    }

    public function test_homework_report_qm_filter_select_average() {
        $homework = new stdClass();
        $homework->attempts = 10;
        $homework->grademethod = QUIZ_GRADEAVERAGE;
        $this->assertSame('', homework_report_qm_filter_select($homework));
    }

    public function test_homework_report_qm_filter_select_first_last_best() {
        global $DB;
        $this->resetAfterTest();

        $fakeattempt = new stdClass();
        $fakeattempt->userid = 123;
        $fakeattempt->homework = 456;
        $fakeattempt->layout = '1,2,0,3,4,0,5';
        $fakeattempt->state = homework_attempt::FINISHED;

        // We intentionally insert these in a funny order, to test the SQL better.
        // The test data is:
        // id | homeworkid | user | attempt | sumgrades | state
        // ---------------------------------------------------
        ...
        $DB->insert_record('homework_attempts', $fakeattempt);
        ...
        $fakeattempt->state = homework_attempt::IN_PROGRESS;
        $DB->insert_record('homework_attempts', $fakeattempt);
        ...
        $homework->grademethod = QUIZ_ATTEMPTFIRST;
        $firstattempt = $DB->get_records_sql("
                SELECT * FROM {homework_attempts} homeworka WHERE userid = ? AND homework = ? AND "
                        . homework_report_qm_filter_select($homework), array(123, 456));
        $this->assertEquals(1, count($firstattempt));
        $firstattempt = reset($firstattempt);
        $this->assertEquals(1, $firstattempt->attempt);

        $homework->grademethod = QUIZ_ATTEMPTLAST;
        ...
        $bestattempt = $DB->get_records_sql("
                SELECT * FROM {homework_attempts} qa_alias WHERE userid = ? AND homework = ? AND "
                . homework_report_qm_filter_select($homework, 'qa_alias'), array(123, 456));
