...
/**
 * Unit tests for the {@link \mod_homework\repaginate} class.
...
require_once($CFG->dirroot . '/mod/homework/classes/repaginate.php');


/**
 * Testable subclass, giving access to the protected methods of {@link \mod_homework\repaginate}
 * @copyright 2014 The Open Univsersity
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_homework_repaginate_testable extends \mod_homework\repaginate {

    public function __construct($homeworkid = 0, $slots = null) {
        return parent::__construct($homeworkid, $slots);
    }
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_homework_repaginate_test extends advanced_testcase {

    /** @var array stores the slots. */
    private $homeworkslots;
    /** @var mod_homework_repaginate_testable the object being tested. */
    private $repaginate = null;

    public function setUp() {
        $this->set_homework_slots($this->get_homework_object()->get_slots());
        $this->repaginate = new mod_homework_repaginate_testable(0, $this->homeworkslots);
    }
    ...
    /**
     * Create a homework, add five questions to the homework
     * which are all on one page and return the homework object.
     */
    private function get_homework_object() {
        global $SITE;
        ...
        $homework = $homeworkgenerator->create_instance(array(
                'course' => $SITE->id, 'questionsperpage' => 0, 'grade' => 100.0, 'sumgrades' => 2));
        $cm = get_coursemodule_from_instance('homework', $homework->id, $SITE->id);
        ...
        homework_add_homework_question($shortanswer->id, $homework);
        homework_add_homework_question($numerical->id, $homework);
        homework_add_homework_question($essay->id, $homework);
        homework_add_homework_question($truefalse->id, $homework);
        homework_add_homework_question($match->id, $homework);

        // Return the homework object.
        $homeworkobj = new homework($homework, $cm, $SITE);
        return \mod_homework\structure::create_for_homework($homeworkobj);
    }

    /**
     * Set the homework slots
     * @param string $slots
     */
    private function set_homework_slots($slots = null) {
        if (!$slots) {
            $this->homeworkslots = $this->get_homework_object()->get_slots();
        } else {
            $this->homeworkslots = $slots;
        }
    }
    ...
    public function test_get_this_slot() {
        $this->set_homework_slots();
        $actual = array();
        $expected = $this->repaginate->get_slots_by_slot_number();
        $this->assertEquals($expected, $actual);

        $slotsbyno = $this->repaginate->get_slots_by_slot_number($this->homeworkslots);
        $slotnumber = 5;
        $thisslot = $this->repaginate->get_this_slot($this->homeworkslots, $slotnumber);
    ...
    public function test_get_slots_by_slotnumber() {
        ...
        foreach ($this->homeworkslots as $slot) {
            $expected[$slot->slot] = $slot;
        }
        $actual = $this->repaginate->get_slots_by_slot_number($this->homeworkslots);
        $this->assertEquals($expected, $actual);
    }
    ...
    public function test_get_slots_by_slotid() {
        ...
        $this->assertEquals($this->homeworkslots, $actual);
    }
    ...
    public function test_repaginate_n_questions_per_page() {
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->homeworkslots, 2);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->homeworkslots, 3);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->homeworkslots, 5);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->homeworkslots, 10);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->homeworkslots, 1);
        $this->assertEquals($expected, $actual);
    }

    public function test_repaginate_this_slot() {
        $this->set_homework_slots();
        $slotsbyslotno = $this->repaginate->get_slots_by_slot_number($this->homeworkslots);
        $slotnumber = 3;
        $newpagenumber = 2;
    ...
    public function test_repaginate_the_rest() {
        $this->set_homework_slots();
        $slotfrom = 1;
        $type = \mod_homework\repaginate::LINK;
        $expected = array();
        foreach ($this->homeworkslots as $slot) {
        ...
        $actual = $this->repaginate->repaginate_the_rest($this->homeworkslots, $slotfrom, $type, false);
        $this->assertEquals($expected, $actual);

        $slotfrom = 2;
        $newslots = array();
        foreach ($this->homeworkslots as $s) {
            if ($s->slot === $slotfrom) {
                $s->page = $s->page - 1;
            }
            $newslots[$s->id] = $s;
        ...
        $type = \mod_homework\repaginate::UNLINK;
        $expected = array();
        foreach ($this->homeworkslots as $slot) {
