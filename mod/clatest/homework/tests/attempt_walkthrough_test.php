 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_homework_attempt_walkthrough_testcase extends advanced_testcase {

    /**
     * Create a homework with questions and walk through a homework attempt.
     */
    public function test_homework_attempt_walkthrough() {
        global $SITE;
        ...
        $homework = $homeworkgenerator->create_instance(array('course'=>$SITE->id, 'questionsperpage' => 0, 'grade' => 100.0,
                                                      'sumgrades' => 2));
        ...
        $grades = homework_get_user_grades($homework, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'homework', $homework->id, $user1->id);
    ...
    /**
     * Create a homework with a random as well as other questions and walk through homework attempts.
     */
    public function test_homework_with_random_question_attempt_walkthrough() {
        global $SITE;
        ...
        $homework = $homeworkgenerator->create_instance(array('course' => $SITE->id, 'questionsperpage' => 2, 'grade' => 100.0,
                                                      'sumgrades' => 4));
        ...
        // Add random question to the homework.
        ...
        foreach (array($saq->id => 'frog', $numq->id => '3.14') as $randomqidtoselect => $randqanswer) {
            ...
            $timenow = time();
            $attempt = homework_create_attempt($homeworkobj, 1, false, $timenow);

            homework_start_new_attempt($homeworkobj, $quba, $attempt, 1, $timenow, array(1 => $randomqidtoselect));
    ...
    protected $homeworkwithvariants = null;

    /**
     * Create a homework with a single question with variants and walk through homework attempts.
     *
     * @dataProvider get_correct_response_for_variants
     */
    public function test_homework_with_question_with_variants_attempt_walkthrough($variantno, $correctresponse, $done = false) {
        global $SITE;
        ...
        if ($this->homeworkwithvariants === null) {
            // Make a homework.
            $homeworkgenerator = $this->getDataGenerator()->get_plugin_generator('mod_homework');

            $this->homeworkwithvariants = $homeworkgenerator->create_instance(array('course'=>$SITE->id,
                                                                            'questionsperpage' => 0,
                                                                            'grade' => 100.0,
                                                                            'sumgrades' => 1));
            ...
            homework_add_homework_question($calc->id, $this->homeworkwithvariants, 0);
        }


        // Make a new user to do the homework.
        $user1 = $this->getDataGenerator()->create_user();
        $this->setUser($user1);
        $homeworkobj = homework::create($this->homeworkwithvariants->id, $user1->id);
        ...
        // Select variant.
        homework_start_new_attempt($homeworkobj, $quba, $attempt, 1, $timenow, array(), array(1 => $variantno));
        ...
        // Check homework grades.
        $grades = homework_get_user_grades($this->homeworkwithvariants, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'homework', $this->homeworkwithvariants->id, $user1->id);
        $gradebookitem = array_shift($gradebookgrades->items);
        $gradebookgrade = array_shift($gradebookitem->grades);
        $this->assertEquals(100, $gradebookgrade->grade);
    }
