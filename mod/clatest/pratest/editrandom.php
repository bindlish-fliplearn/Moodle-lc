...
// Get the pratest slot.
$slot = $DB->get_record('pratest_slots', array('id' => $slotid));
if (!$slot || empty($slot->questioncategoryid)) {
    print_error('invalidrandomslot', 'mod_pratest');
}

if (!$pratest = $DB->get_record('pratest', array('id' => $slot->pratestid))) {
    print_error('invalidpratestid', 'pratest');
}

$cm = get_coursemodule_from_instance('pratest', $slot->pratestid, $pratest->course);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/pratest/edit.php', array('cmid' => $cm->id));
}

$url = new moodle_url('/mod/pratest/editrandom.php', array('slotid' => $slotid));
$PAGE->set_url($url);
...
$mform = new mod_pratest\form\randomquestion_form(new moodle_url('/mod/pratest/editrandom.php'),
...
$currentslottags = pratest_retrieve_slot_tags($slot->id);
...
if ($mform->is_cancelled()) {
    ...
    // We need to save some data into the pratest_slots table.
    ...
    foreach ($tags as $tag) {
        if ($key = array_search(['tagid' => $tag->id, 'tagname' => $tag->name], $searchableslottags)) {
            // If found, $key would be the id field in the pratest_slot_tags table.
    ...
    if (!empty($recordstokeep)) {
        ...
        $DB->delete_records_select('pratest_slot_tags', "id $select", $params);
    } else {
...
echo $OUTPUT->header();
$heading = get_string('randomediting', 'mod_pratest');
echo $OUTPUT->heading_with_help($heading, 'randomquestion', 'mod_pratest');
