 ...
 *
 * The pratest stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-pratest-tools/tree/master/statsspreadsheet
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class pratest_report_statistics_from_steps_testcase extends mod_pratest_attempt_walkthrough_from_csv_testcase {

    /**
     * @var pratest_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    public function test_walkthrough_from_csv($pratestsettings, $csvdata) {
        ...
        list($questions, $prateststats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata,
                            $whichattempts, $whichtries, $groupstudentsjoins);
        if ($pratestsettings['testnumber'] === '00') {
            $this->check_variants_count_for_pratest_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_pratest_stats_for_pratest_00($prateststats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_pratest_00($questions, $questionstats, $whichtries, $qubaids) {
     ...
     * @param $prateststats
     */
    protected function check_pratest_stats_for_pratest_00($prateststats) {
        $prateststatsexpected = array(
            'median'             => 4.5,
        ...
        foreach ($prateststatsexpected as $statname => $statvalue) {
            $this->assertEquals($statvalue, $prateststats->$statname, $prateststats->$statname, abs($statvalue) * 1.5e-5);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $prateststats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries,
        ...
        $this->report = new pratest_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->pratest);
        list($prateststats, $questionstats) = $this->report->get_all_stats_and_analysis($this->pratest,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudentsjoins,
                                                                                     $questions);

        $qubaids = pratest_statistics_qubaids_condition($this->pratest->id, $groupstudentsjoins, $whichattempts);

        // We will create some pratest and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $pratestcalc = new \pratest_statistics\calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($pratestcalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $prateststats, $questionstats, $qubaids);
        }
        return array($questions, $prateststats, $questionstats, $qubaids);
    }
