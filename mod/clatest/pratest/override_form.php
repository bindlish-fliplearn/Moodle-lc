...
/**
 * Settings form for overrides in the pratest module.
 *
 * @package    mod_pratest
...
require_once($CFG->dirroot . '/mod/pratest/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class pratest_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the pratest settings object. */
    protected $pratest;
     ...
     * @param object the pratest settings object.
     ...
     */
    public function __construct($submiturl, $cm, $pratest, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'pratest'));
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'pratest'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/pratest/overrides.php', array('cmid'=>$cm->id));
                    print_error('groupsnone', 'pratest', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'pratest'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                $users = get_users_by_capability($this->context, 'mod/pratest:attempt',
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/pratest/overrides.php', array('cmid'=>$cm->id));
                    print_error('usersnone', 'pratest', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'pratest'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'pratest');
        $mform->setDefault('password', $this->pratest->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('pratestopen', 'pratest'), mod_pratest_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->pratest->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('pratestclose', 'pratest'), mod_pratest_mod_form::$datefieldoptions);
        $mform->setDefault('timeclose', $this->pratest->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'pratest'), array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'pratest');
        $mform->setDefault('timelimit', $this->pratest->timelimit);
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'pratest'), $attemptoptions);
        $mform->addHelpButton('attempts', 'attempts', 'pratest');
        $mform->setDefault('attempts', $this->pratest->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'pratest'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'pratest'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'pratest'));
    ...
    public function validation($data, $files) {
        ...
        // Ensure that at least one pratest setting was changed.
        $changed = false;
        $keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
        foreach ($keys as $key) {
            if ($data[$key] != $pratest->{$key}) {
                $changed = true;
                break;
            }
        }
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'pratest');
        }
