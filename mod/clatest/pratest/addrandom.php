...
/**
 * Fallback page of /mod/pratest/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $pratest, $pagevars) =
        question_edit_setup('editq', '/mod/pratest/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/pratest/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new pratest_add_random_form(new moodle_url('/mod/pratest/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    pratest_add_random_questions($pratest, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories, $tagids);
...
// Setup $PAGE.
$streditingpratest = get_string('editinga', 'moodle', get_string('modulename', 'pratest'));
$PAGE->navbar->add($streditingpratest);
$PAGE->set_title($streditingpratest);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$pratestname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            print_error('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontopratest', 'pratest', $pratestname), 2);
$mform->display();
echo $OUTPUT->footer();
