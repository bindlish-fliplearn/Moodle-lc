 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_pratest_structure_testcase extends advanced_testcase {

    /**
     * Create a course with an empty pratest.
     * @return array with three elements pratest, cm and course.
     */
    protected function prepare_pratest_data() {
        ...
        return array($pratest, $cm, $course);
    }

    /**
     * Creat a test pratest.
     *
     ...
     * @return pratest the created pratest.
     */
    protected function create_test_pratest($layout) {
        list($pratest, $cm, $course) = $this->prepare_pratest_data();
        ...
        foreach ($layout as $item) {
            if (is_string($item)) {
                ...
                pratest_add_pratest_question($q->id, $pratest, $page);
                $lastpage = $page;
            }
        }

        $pratestobj = new pratest($pratest, $cm, $course);
        $structure = \mod_pratest\structure::create_for_pratest($pratestobj);
        if (isset($headings[1])) {
     ...
     * @param array $expectedlayout as for $layout in {@link create_test_pratest()}.
     * @param \mod_pratest\structure $structure the structure to test.
     */
    protected function assert_pratest_layout($expectedlayout, \mod_pratest\structure $structure) {
        $sections = $structure->get_sections();
        ...
        foreach ($expectedlayout as $item) {
            if (is_string($item)) {
                ...
                if ($slot > 1 && $section->heading == '' && $section->firstslot == 1) {
                    // The array $expectedlayout did not contain default first pratest section, so skip over it.
                    $section = array_shift($sections);
                }
        ...
        if ($slot - 1 != count($structure->get_slots())) {
            $this->fail('The pratest contains more slots than expected.');
        }

        if (!empty($sections)) {
            $section = array_shift($sections);
            if ($section->heading != '' || $section->firstslot != 1) {
                $this->fail('Unexpected section (' . $section->heading .') found in the pratest.');
            }
        }
    }
     ...
     * used by create_test_pratest as assert_pratest_layout.
    ...
    public function test_get_pratest_slots() {
        $pratestobj = $this->create_test_pratest(array(
                array('TF1', 1, 'truefalse'),
    ...
    public function test_pratest_has_one_section_by_default() {
        $pratestobj = $this->create_test_pratest(array(
                array('TF1', 1, 'truefalse'),
            ));
        $structure = \mod_pratest\structure::create_for_pratest($pratestobj);
    ...
    public function test_remove_section_heading() {
        ...
        $structure = \mod_pratest\structure::create_for_pratest($pratestobj);
        $this->assert_pratest_layout(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
                array('TF2', 2, 'truefalse'),
            ), $structure);
    ...
    public function test_pratest_remove_slot() {
        $pratestobj = $this->create_test_pratest(array(
                array('TF1', 1, 'truefalse'),
                array('TF2', 1, 'truefalse'),
                'Heading 2',
    ...
    public function test_pratest_removing_a_random_question_deletes_the_question() {
        global $DB;
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category();
        pratest_add_random_questions($pratestobj->get_pratest(), 1, $cat->id, 1, false);
        $structure = \mod_pratest\structure::create_for_pratest($pratestobj);
    ...
    public function test_can_remove_last_question_in_a_pratest() {
        $pratestobj = $this->create_test_pratest(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
            ));
        ...
        pratest_add_pratest_question($q->id, $pratestobj->get_pratest(), 0);
        $structure = \mod_pratest\structure::create_for_pratest($pratestobj);
    ...
    public function test_add_question_updates_headings() {
        ...
        pratest_add_pratest_question($q->id, $pratestobj->get_pratest(), 1);
    ...
    public function test_remove_page_break() {
        ...
        $slots = $structure->update_page_break($slotid, \mod_pratest\repaginate::LINK);
    ...
    public function test_add_page_break() {
        ...
        $slotid = $structure->get_question_in_slot(2)->slotid;
        $slots = $structure->update_page_break($slotid, \mod_pratest\repaginate::UNLINK);
     ...
     * @param  array $layout Quiz layout for create_test_pratest function
    ...
    public function test_get_slot_tags_for_slot($layout, $tagnames, $slotnumber, $expected) {
        ...
        $pratest = $this->create_test_pratest($layout);
        $structure = \mod_pratest\structure::create_for_pratest($pratest);
        ...
        $DB->insert_records('pratest_slot_tags', $slottagrecords);
