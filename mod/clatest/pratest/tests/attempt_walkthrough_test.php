 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_pratest_attempt_walkthrough_testcase extends advanced_testcase {

    /**
     * Create a pratest with questions and walk through a pratest attempt.
     */
    public function test_pratest_attempt_walkthrough() {
        global $SITE;
        ...
        $pratest = $pratestgenerator->create_instance(array('course'=>$SITE->id, 'questionsperpage' => 0, 'grade' => 100.0,
                                                      'sumgrades' => 2));
        ...
        $grades = pratest_get_user_grades($pratest, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'pratest', $pratest->id, $user1->id);
    ...
    /**
     * Create a pratest with a random as well as other questions and walk through pratest attempts.
     */
    public function test_pratest_with_random_question_attempt_walkthrough() {
        global $SITE;
        ...
        $pratest = $pratestgenerator->create_instance(array('course' => $SITE->id, 'questionsperpage' => 2, 'grade' => 100.0,
                                                      'sumgrades' => 4));
        ...
        // Add random question to the pratest.
        ...
        foreach (array($saq->id => 'frog', $numq->id => '3.14') as $randomqidtoselect => $randqanswer) {
            ...
            $timenow = time();
            $attempt = pratest_create_attempt($pratestobj, 1, false, $timenow);

            pratest_start_new_attempt($pratestobj, $quba, $attempt, 1, $timenow, array(1 => $randomqidtoselect));
    ...
    protected $pratestwithvariants = null;

    /**
     * Create a pratest with a single question with variants and walk through pratest attempts.
     *
     * @dataProvider get_correct_response_for_variants
     */
    public function test_pratest_with_question_with_variants_attempt_walkthrough($variantno, $correctresponse, $done = false) {
        global $SITE;
        ...
        if ($this->pratestwithvariants === null) {
            // Make a pratest.
            $pratestgenerator = $this->getDataGenerator()->get_plugin_generator('mod_pratest');

            $this->pratestwithvariants = $pratestgenerator->create_instance(array('course'=>$SITE->id,
                                                                            'questionsperpage' => 0,
                                                                            'grade' => 100.0,
                                                                            'sumgrades' => 1));
            ...
            pratest_add_pratest_question($calc->id, $this->pratestwithvariants, 0);
        }


        // Make a new user to do the pratest.
        $user1 = $this->getDataGenerator()->create_user();
        $this->setUser($user1);
        $pratestobj = pratest::create($this->pratestwithvariants->id, $user1->id);
        ...
        // Select variant.
        pratest_start_new_attempt($pratestobj, $quba, $attempt, 1, $timenow, array(), array(1 => $variantno));
        ...
        // Check pratest grades.
        $grades = pratest_get_user_grades($this->pratestwithvariants, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'pratest', $this->pratestwithvariants->id, $user1->id);
        $gradebookitem = array_shift($gradebookgrades->items);
        $gradebookgrade = array_shift($gradebookitem->grades);
        $this->assertEquals(100, $gradebookgrade->grade);
    }
