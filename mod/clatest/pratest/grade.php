...
$cm = get_coursemodule_from_id('pratest', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$pratest = $DB->get_record('pratest', array('id' => $cm->instance), '*', MUST_EXIST);
require_login($course, false, $cm);

$reportlist = pratest_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see pratest reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/pratest/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = pratest_get_user_attempts($pratest->id, $userid, 'finished');
    $attempt = null;
    ...
    if ($attempt) {
        $attemptobj = new pratest_attempt($attempt, $pratest, $cm, $course, false);
...
redirect(new moodle_url('/mod/pratest/report.php', array(
