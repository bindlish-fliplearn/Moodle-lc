...
/**
 * Defines the custom question bank view used on the Edit pratest page.
 *
 * @package   mod_pratest
 * @category  question
...
namespace mod_pratest\question\bank;
defined('MOODLE_INTERNAL') || die();


/**
 * Subclass to customise the view of the question bank for the pratest editing screen.
 *
 * @copyright  2009 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class custom_view extends \core_question\bank\view {
    /** @var bool whether the pratest this is used by has been attemptd. */
    protected $pratesthasattempts = false;
    /** @var \stdClass the pratest settings. */
    protected $pratest = false;
     ...
     * @param \stdClass $pratest pratest settings.
     */
    public function __construct($contexts, $pageurl, $course, $cm, $pratest) {
    ...
    protected function wanted_columns() {
        ...
        if (empty($CFG->pratestquestionbankcolumns)) {
            $pratestquestionbankcolumns = array(
                'add_action_column',
                'checkbox_column',
            ...
            $pratestquestionbankcolumns = explode(',', $CFG->pratestquestionbankcolumns);
        }

        foreach ($pratestquestionbankcolumns as $fullname) {
            if (!class_exists($fullname)) {
                if (class_exists('mod_pratest\\question\\bank\\' . $fullname)) {
                    $fullname = 'mod_pratest\\question\\bank\\' . $fullname;
                    ...
                    debugging('Legacy question bank column class question_bank_' .
                            $fullname . ' should be renamed to mod_pratest\\question\\bank\\' .
    ...
    protected function heading_column() {
        return 'mod_pratest\\question\\bank\\question_name_text_column';
    }

    protected function default_sort() {
        return array(
            'core_question\\bank\\question_type_column' => 1,
            'mod_pratest\\question\\bank\\question_name_text_column' => 1,
        );
    }

    /**
     * Let the question bank display know whether the pratest has been attempted,
     * hence whether some bits of UI, like the add this question to the pratest icon,
     * should be displayed.
     * @param bool $pratesthasattempts whether the pratest has attempts.
     */
    public function set_pratest_has_attempts($pratesthasattempts) {
        $this->pratesthasattempts = $pratesthasattempts;
        if ($pratesthasattempts && isset($this->visiblecolumns['addtopratestaction'])) {
            unset($this->visiblecolumns['addtopratestaction']);
        }
    }

    public function preview_question_url($question) {
        return pratest_question_preview_url($this->pratest, $question);
    }

    public function add_to_pratest_url($questionid) {
        global $CFG;
        $params = $this->baseurl->params();
        $params['addquestion'] = $questionid;
        $params['sesskey'] = sesskey();
        return new \moodle_url('/mod/pratest/edit.php', $params);
    }
    ...
    protected function display_bottom_controls($totalnumber, $recurse, $category, \context $catcontext, array $addcontexts) {
        ...
        $cmoptions->hasattempts = !empty($this->pratesthasattempts);
        ...
        if ($canuseall) {
            ...
            // Add selected questions to the pratest.
            $params = array(
                    'type' => 'submit',
                    'name' => 'add',
                    'class' => 'btn btn-primary',
                    'value' => get_string('addselectedquestionstopratest', 'pratest'),
            );
    ...
    protected function display_options_form($showquestiontext, $scriptpath = '/mod/pratest/edit.php',
    ...
    protected function print_category_info($category) {
        ...
        $strcategory = get_string('category', 'pratest');
