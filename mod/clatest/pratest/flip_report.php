...
/**
 * This script controls the display of the pratest reports.
 *
 * @package   mod_pratest
...
$url = new moodle_url('/mod/pratest/report.php', array('id' => $cm->id));
if ($mode !== '') {
    $url->param('mode', $mode);
}
$PAGE->set_url($url);
...
$reportlist = pratest_report_list($context);
if (empty($reportlist)) {
    print_error('erroraccessingreport', 'pratest');
}
...
if (!is_readable("report/$mode/report.php")) {
    print_error('reportnotfound', 'pratest', '', $mode);
}

// Open the selected pratest report and display it.
$file = $CFG->dirroot . '/mod/pratest/report/' . $mode . '/report.php';
if (is_readable($file)) {
    include_once($file);
}
$reportclassname = 'pratest_' . $mode . '_report';
if (!class_exists($reportclassname)) {
    print_error('preprocesserror', 'pratest');
}

$report = new $reportclassname();
$report->display($pratest, $cm, $course);
