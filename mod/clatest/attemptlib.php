...
/**
 * Back-end code for handling data about clatestzes and the current user's attempt.
 *
 * There are classes for loading all the information about a clatest and attempts,
...
/**
 * Class for clatest exceptions. Just saves a couple of arguments on the
 ...
 * @since     Moodle 2.0
 */
class moodle_clatest_exception extends moodle_exception {
    public function __construct($clatestobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $clatestobj->view_url();
        }
        parent::__construct($errorcode, 'clatest', $link, $a, $debuginfo);
    }
}


/**
 * A class encapsulating a clatest and the questions it contains, and making the
...
class clatest {
    ...
    protected $cm;
    /** @var stdClass the clatest settings from the database. */
    protected $clatest;
    /** @var context the clatest context. */
    protected $context;

    /** @var array of questions augmented with slot information. */
    protected $questions = null;
    /** @var array of clatest_section rows. */
    protected $sections = null;
    /** @var clatest_access_manager the access manager for this clatest. */
    protected $accessmanager = null;
    /** @var bool whether the current user has capability mod/clatest:preview. */
     ...
     *
     * @param object $clatest the row from the clatest table.
     * @param object $cm the course_module object for this clatest.
     * @param object $course the row from the course table for the course we belong to.
     * @param bool $getcontext intended for testing - stops the constructor getting the context.
     */
    public function __construct($clatest, $cm, $course, $getcontext = true) {
        $this->clatest = $clatest;
        $this->cm = $cm;
        $this->clatest->cmid = $this->cm->id;
        $this->course = $course;
    ...
    /**
     * Static function to create a new clatest object for a specific user.
     *
     * @param int $clatestid the the clatest id.
     * @param int $userid the the userid.
     * @return clatest the new clatest object
     */
    public static function create($clatestid, $userid = null) {
        global $DB;

        $clatest = clatest_access_manager::load_clatest_and_settings($clatestid);
        $course = $DB->get_record('course', array('id' => $clatest->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('clatest', $clatest->id, $course->id, false, MUST_EXIST);

        // Update clatest with override information.
        if ($userid) {
            $clatest = clatest_update_effective_access($clatest, $userid);
        }

        return new clatest($clatest, $cm, $course);
    }

    /**
     * Create a {@link clatest_attempt} for an attempt at this clatest.
     * @param object $attemptdata row from the clatest_attempts table.
     * @return clatest_attempt the new clatest_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new clatest_attempt($attemptdata, $this->clatest, $this->cm, $this->course);
    }

    // Functions for loading more data =========================================

    /**
     * Load just basic information about all the questions in this clatest.
     */
    public function preload_questions() {
        $this->questions = question_preload_questions(null,
                ...
                '{clatest_slots} slot ON slot.clatestid = :clatestid AND q.id = slot.questionid',
                array('clatestid' => $this->clatest->id), 'slot.slot');
    }

    /**
     * Fully load some or all of the questions for this clatest. You must call
    ...
    /**
     * Get an instance of the {@link \mod_clatest\structure} class for this clatest.
     * @return \mod_clatest\structure describes the questions in the clatest.
     */
    public function get_structure() {
        return \mod_clatest\structure::create_for_clatest($this);
    }
    ...
    /** @return int the clatest id. */
    public function get_clatestid() {
        return $this->clatest->id;
    }

    /** @return object the row of the clatest table. */
    public function get_clatest() {
        return $this->clatest;
    }

    /** @return string the name of this clatest. */
    public function get_clatest_name() {
        return $this->clatest->name;
    }

    /** @return int the clatest navigation method. */
    public function get_navigation_method() {
        return $this->clatest->navmethod;
    }

    /** @return int the number of attempts allowed at this clatest (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->clatest->attempts;
    }
    ...
    /** @return object the module context for this clatest. */
    public function get_context() {
        return $this->context;
    }

    /**
     * @return bool wether the current user is someone who previews the clatest,
     * rather than attempting it.
     */
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/clatest:preview', $this->context);
        }
        return $this->ispreviewuser;
    }

    /**
     * @return whether any questions have been added to this clatest.
     */
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'clatest', $this->view_url());
            }
    ...
    /**
     * Get all the sections in this clatest.
     * @return array 0, 1, 2, ... => clatest_sections row from the database.
     */
    public function get_sections() {
        global $DB;
        if ($this->sections === null) {
            $this->sections = array_values($DB->get_records('clatest_sections',
                    array('clatestid' => $this->get_clatestid()), 'firstslot'));
        }
        return $this->sections;
    }

    /**
     * Return clatest_access_manager and instance of the clatest_access_manager class
     * for this clatest at this time.
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new clatest_access_manager($this, $timenow,
                    has_capability('mod/clatest:ignoretimelimits', $this->context, null, false));
        }
        return $this->accessmanager;
    }

    /**
     * Wrapper round the has_capability funciton that automatically passes in the clatest context.
     */
    ...
    /**
     * Wrapper round the require_capability funciton that automatically passes in the clatest context.
     */
    ...
    /**
     * @return string the URL of this clatest's view page.
     */
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/clatest/view.php?id=' . $this->cm->id;
    }

    /**
     * @return string the URL of this clatest's edit page.
     */
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/clatest/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/clatest/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
    ...
    /**
     * @return string the URL of this clatest's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/clatest/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/clatest/review.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/clatest/summary.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
     ...
     * @deprecated since 3.1. This sort of functionality is now entirely handled by clatest access rules.
     */
    public function confirm_start_attempt_message($notused) {
        debugging('confirm_start_attempt_message is deprecated. ' .
                'This sort of functionality is now entirely handled by clatest access rules.');
        return '';
    }
     ...
     *
     * @param int $when One of the mod_clatest_display_options::DURING,
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_clatest_display_options::DURING ||
                $when == mod_clatest_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_clatest_display_options::LATER_WHILE_OPEN && $this->clatest->timeclose &&
                $this->clatest->reviewattempt & mod_clatest_display_options::AFTER_CLOSE) {
            return get_string('noreviewuntil' . $langstrsuffix, 'clatest',
                    userdate($this->clatest->timeclose, $dateformat));
        } else {
            return get_string('noreview' . $langstrsuffix, 'clatest');
        }
    }

    /**
     * @param string $title the name of this particular clatest page.
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_clatest_exception($this, 'questionnotloaded', $id);
        }
    }

    /**
     * Return all the question types used in this clatest.
     *
     * @param  boolean $includepotential if the clatest include random questions, setting this flag to true will make the function to
    ...
    public function get_all_question_types_used($includepotential = false) {
        ...
        // We must be careful with random questions, if we find a random question we must assume that the clatest may content
...
/**
 * This class extends the clatest class to hold data about the state of a particular attempt,
 * in addition to the data about the clatest.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class clatest_attempt {
    ...
    /** @var int maximum number of slots in the clatest for the review page to default to show all. */
    const MAX_SLOTS_FOR_DEFAULT_REVIEW_SHOW_ALL = 50;

    /** @var clatest object containing the clatest settings. */
    protected $clatestobj;

    /** @var stdClass the clatest_attempts row. */
    protected $attempt;

    /** @var question_usage_by_activity the question usage for this clatest attempt. */
    protected $quba;
    ...
    /** @var array of clatest_sections rows, with a ->lastslot field added. */
    protected $sections;
    ...
    /** @var mod_clatest_display_options cache for the appropriate review options. */
     ...
     * Constructor assuming we already have the necessary data loaded.
     *
     * @param object $attempt the row of the clatest_attempts table.
     * @param object $clatest the clatest object for this attempt and user.
     * @param object $cm the course_module object for this clatest.
     * @param object $course the row from the course table for the course we belong to.
     ...
     */
    public function __construct($attempt, $clatest, $cm, $course, $loadquestions = true) {
        global $DB;

        $this->attempt = $attempt;
        $this->clatestobj = new clatest($clatest, $cm, $course);
        ...
        $this->slots = $DB->get_records('clatest_slots',
                array('clatestid' => $this->get_clatestid()), 'slot',
     ...
     * @param array $conditions passed to $DB->get_record('clatest_attempts', $conditions).
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('clatest_attempts', $conditions, '*', MUST_EXIST);
        $clatest = clatest_access_manager::load_clatest_and_settings($attempt->clatest);
        $course = $DB->get_record('course', array('id' => $clatest->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('clatest', $clatest->id, $course->id, false, MUST_EXIST);

        // Update clatest with override information.
        $clatest = clatest_update_effective_access($clatest, $attempt->userid);

        return new clatest_attempt($attempt, $clatest, $cm, $course);
    }

    /**
     * Static function to create a new clatest_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new clatest_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return clatest_attempt_state_name($state);
    }
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'clatest');
                }
    ...
    public function get_clatestobj() {
        return $this->clatestobj;
    }
    ...
    /** @return int the course id. */
    public function get_course() {
        return $this->clatestobj->get_course();
    }
    ...
    /** @return string the name of this clatest. */
    public function get_clatest_name() {
        return $this->clatestobj->get_clatest_name();
    }

    /** @return int the clatest navigation method. */
    public function get_navigation_method() {
        return $this->clatestobj->get_navigation_method();
    }

    /** @return object the course_module object. */
    public function get_cm() {
        return $this->clatestobj->get_cm();
    }
     ...
     */
    public function is_preview_user() {
        return $this->clatestobj->is_preview_user();
    }

    /** @return int the number of attempts allowed at this clatest (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->clatestobj->get_num_attempts_allowed();
    }

    /** @return int number fo pages in this clatest. */
     ...
     */
    public function get_access_manager($timenow) {
        return $this->clatestobj->get_access_manager($timenow);
    }
    ...
    /** @return object the row from the clatest_attempts table. */
    ...
    /** @return string one of the clatest_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/clatest:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the clatest in a non-trivial way?
     ...
     * To see how this works, try setting a clatest to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the clatest
     ...
     *
     * @param question_display_options $options the display options for this clatest attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return clatest_feedback_for_grade($grade, $this->get_clatest(),
                $this->clatestobj->get_context());
    }
     ...
     */
    public function has_capability($capability, $userid = null, $doanything = true) {
        return $this->clatestobj->has_capability($capability, $userid, $doanything);
    }
     ...
     */
    public function require_capability($capability, $userid = null, $doanything = true) {
        return $this->clatestobj->require_capability($capability, $userid, $doanything);
    }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_clatest_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/clatest:attempt';
        } else {
            $capability = 'mod/clatest:reviewmyattempts';
        }

        // These next tests are in a slighly funny order. The point is that the
        // common and most performance-critical case is students attempting a clatest
        // so we want to check that permisison first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the clatest as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/clatest:viewreports') ||
                $this->has_capability('mod/clatest:preview')) {
    ...
    public function get_attempt_state() {
        return clatest_attempt_state($this->get_clatest(), $this->attempt);
    }

    /**
     * Wrapper that the correct mod_clatest_display_options for this clatest at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = clatest_get_review_options($this->get_clatest(),
                        $this->attempt, $this->clatestobj->get_context());
            ...
            $options = mod_clatest_display_options::make_from_clatest($this->get_clatest(),
                    mod_clatest_display_options::DURING);
            $options->flags = clatest_get_flag_option($this->attempt, $this->clatestobj->get_context());
            return $options;
        }
    }
     ...
     * @return bool true if this is the last page of the clatest.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the clatest, or for the
     * whole clatest.
     *
    ...
    public function get_question_usage() {
        if (!PHPUNIT_TEST) {
            throw new coding_exception('get_question_usage is only for use in unit tests. ' .
                    'For other operations, use the clatest_attempt api, or extend it properly.');
        }
        return $this->quba;
    }
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_clatest()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the clatest where this question appears.
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the clatest this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the clatest.
     */
    public function get_question_mark($slot) {
        return clatest_format_question_grade($this->get_clatest(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = array();
        if ($this->clatestobj->get_clatest()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->clatestobj->get_clatest()->timelimit;
        }
        if ($this->clatestobj->get_clatest()->timeclose) {
            $deadlines[] = $this->clatestobj->get_clatest()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->clatestobj->get_clatest()->graceperiod;
    ...
    // URLs related to this attempt ============================================
    /**
     * @return string clatest view url.
     */
    public function view_url() {
        return $this->clatestobj->view_url();
    }
    ...
    public function start_attempt_url($slot = null, $page = -1) {
        ...
        return $this->clatestobj->start_attempt_url($page);
    }
    ...
    /**
     * @return string the URL of this clatest's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/clatest/summary.php', array('attempt' => $this->attempt->id, 'cmid' => $this->get_cmid()));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/clatest/processattempt.php');
    }
    ...
    public function cannot_review_message($short = false) {
        return $this->clatestobj->cannot_review_message(
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'clatest'));
        } else {
            return '';
        }
    }
     ...
     * @param mod_clatest_renderer $renderer the clatest renderer.
     ...
     */
    public function render_question($slot, $reviewing, mod_clatest_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions,
                    ...
                    'mod_clatest-blocked_question_warning');
        }
     ...
     */
    protected function render_question_helper($slot, $reviewing, $thispageurl, mod_clatest_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'clatest', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'clatest');
        ...
        $placeholderqa->start($this->get_clatest()->preferredbehaviour, 1);
     ...
     *
     * @param int $id the id of a question in this clatest attempt.
     ...
     * @return string HTML for the question in its current state.
     */
    public function render_question_at_step($slot, $seq, $reviewing, mod_clatest_renderer $renderer, $thispageurl = '') {
     ...
     *
     * @param $panelclass The type of panel, clatest_attempt_nav_panel or clatest_review_nav_panel
     * @param $page the current page number.
     * @param $showall whether we are showing the whole clatest on one page. (Used by review.php)
     * @return clatest_nav_panel_base the requested object.
     */
    public function get_navigation_panel(mod_clatest_renderer $output,
             $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_clatest_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->attributes['aria-labelledby'] = 'mod_clatest_navblock_title';
        $bc->title = html_writer::span(get_string('clatestnavigation', 'clatest'), '', array('id' => 'mod_clatest_navblock_title'));
        $bc->content = $output->navigation_panel($panel);
        return $bc;
    }

    /**
     * Return an array of variant URLs to other attempts at this clatest.
     *
     * The $url passed in must contain an attempt parameter.
     *
     * The {@link mod_clatest_links_to_other_attempts} object returned contains an
     ...
     * @return mod_clatest_links_to_other_attempts containing array int => null|moodle_url.
     */
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = clatest_get_user_attempts($this->get_clatest()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }

        $links = new mod_clatest_links_to_other_attempts();
     ...
     * @return mod_clatest_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->clatestobj->get_clatest()->graceperiod;
        ...
        // Otherwise, we were in clatest_attempt::IN_PROGRESS, and time has now expired.
        // Transition to the appropriate state.
        switch ($this->clatestobj->get_clatest()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if ($becomingoverdue) {
            ...
            $DB->update_record('clatest_attempts', $this->attempt);
        }

        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            clatest_save_best_grade($this->get_clatest(), $this->get_userid());
        }
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_clatest\question\qubaids_for_users_attempts(
                $this->get_clatestid(), $this->get_userid());
        ...
        if ($questiondata->qtype != 'random') {
            ...
            $tagids = clatest_retrieve_slot_tag_ids($this->slots[$slot]->id);
            ...
            if ($newqusetionid === null) {
                throw new moodle_exception('notenoughrandomquestions', 'clatest',
                        $this->clatestobj->view_url(), $questiondata);
            }
        }
    ...
    public function process_finish($timestamp, $processsubmitted) {
        ...
        if (!$this->is_preview()) {
            clatest_save_best_grade($this->get_clatest(), $this->attempt->userid);

            // Trigger event.
            $this->fire_state_transition_event('\mod_clatest\event\attempt_submitted', $timestamp);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('clatest_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_clatest\event\attempt_becameoverdue', $timestamp);

        $transaction->allow_commit();

        clatest_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_clatest\event\attempt_abandoned', $timestamp);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp) {
        global $USER;
        $clatestrecord = $this->get_clatest();
        $params = array(
            'context' => $this->get_clatestobj()->get_context(),
            'courseid' => $this->get_courseid(),
            'objectid' => $this->attempt->id,
            'relateduserid' => $this->attempt->userid,
            'other' => array(
                'submitterid' => CLI_SCRIPT ? null : $USER->id,
                'clatestid' => $clatestrecord->id
            )
        );

        $event = $eventclass::create($params);
        $event->add_record_snapshot('clatest', $this->get_clatest());
        $event->add_record_snapshot('clatest_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    /**
     * Get a URL for a particular question on a particular page of the clatest.
     * Used by {@link attempt_url()} and {@link review_url()}.
     *
     * @param string $script. Used in the URL like /mod/clatest/$script.php
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/clatest/' . $script . '.php' . $fragment,
    ...
    /**
     * Process responses during an attempt at a clatest.
     *
    ...
    public function process_attempt($timenow, $finishattempt, $timeup, $thispage) {
        ...
        // to show the student another page of the clatest. Just finish now.
        $graceperiodmin = null;
        ...
        if ($timeup) {
            if ($this->get_clatest()->overduehandling == 'graceperiod') {
                if (is_null($graceperiodmin)) {
                    $graceperiodmin = get_config('clatest', 'graceperiodmin');
                }
                if ($timenow > $timeclose + $this->get_clatest()->graceperiod + $graceperiodmin) {
        ...
        // Update the clatest attempt record.
        try {
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('clatest_attempts', 'currentpage', $page, array('id' => $this->get_attemptid()));
            return true;
        }
        return false;
    }
 ...
 * @since      Moodle 2.9
 */
class clatest_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class clatest_nav_question_button implements renderable {
 ...
 * @since      Moodle 2.0
 */
abstract class clatest_nav_panel_base {
    /** @var clatest_attempt */
    ...
    public function __construct(clatest_attempt $attemptobj,
    ...
    /**
     * Get the buttons and section headings to go in the clatest navigation block.
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            if ($heading = $this->attemptobj->get_heading_before_slot($slot)) {
                $buttons[] = new clatest_nav_section_heading(format_string($heading));
            }

            $qa = $this->attemptobj->get_question_attempt($slot);
            $showcorrectness = $this->options->correctness && $qa->has_marks();

            $button = new clatest_nav_question_button();
            $button->id          = 'clatestnavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'clatest');
            }
            $buttons[] = $button;
        }
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'clatest');
        } else {
            return get_string('viewed', 'clatest');
        }
    }

    public function render_before_button_bits(mod_clatest_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_clatest_renderer $output);
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_clatest()->showuserpicture == QUIZ_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', array('id' => $this->attemptobj->get_userid()));
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_clatest()->showuserpicture == QUIZ_SHOWIMAGE_LARGE) {
...
/**
 * Specialisation of {@link clatest_nav_panel_base} for the attempt clatest page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class clatest_attempt_nav_panel extends clatest_nav_panel_base {
    ...
    public function render_before_button_bits(mod_clatest_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'clatest'),
                array('id' => 'clatestnojswarning'));
    }

    public function render_end_bits(mod_clatest_renderer $output) {
        return html_writer::link($this->attemptobj->summary_url(),
                get_string('endtest', 'clatest'), array('class' => 'endtestlink')) .
...
/**
 * Specialisation of {@link clatest_nav_panel_base} for the review clatest page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class clatest_review_nav_panel extends clatest_nav_panel_base {
    public function get_question_url($slot) {
