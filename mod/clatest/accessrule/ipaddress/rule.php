...
/**
 * Implementaton of the clatestaccess_ipaddress plugin.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class clatestaccess_ipaddress extends clatest_access_rule_base {

    public static function make(clatest $clatestobj, $timenow, $canignoretimelimits) {
        if (empty($clatestobj->get_clatest()->subnet)) {
            return null;
        }
    ...
    public function prevent_access() {
        if (address_in_subnet(getremoteaddr(), $this->clatest->subnet)) {
            return false;
        } else {
            return get_string('subnetwrong', 'clatestaccess_ipaddress');
        }
    }
}
