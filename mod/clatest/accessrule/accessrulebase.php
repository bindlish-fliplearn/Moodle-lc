...
/**
 * Base class for rules that restrict the ability to attempt a clatest.
 *
 * @package   mod_clatest
...
/**
 * A base class that defines the interface for the various clatest access rules.
 ...
 * @since     Moodle 2.2
 */
abstract class clatest_access_rule_base {
    /** @var stdClass the clatest settings. */
    protected $clatest;
    /** @var clatest the clatest object. */
    protected $clatestobj;
    /** @var int the time to use as 'now'. */
    protected $timenow;

    /**
     * Create an instance of this rule for a particular clatest.
     * @param clatest $clatestobj information about the clatest in question.
     * @param int $timenow the time that should be considered as 'now'.
     */
    public function __construct($clatestobj, $timenow) {
        $this->clatestobj = $clatestobj;
        $this->clatest = $clatestobj->get_clatest();
        $this->timenow = $timenow;
    }

    /**
     * Return an appropriately configured instance of this rule, if it is applicable
     * to the given clatest, otherwise return null.
     * @param clatest $clatestobj information about the clatest in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/clatest:ignoretimelimits capability.
     * @return clatest_access_rule_base|null the rule, if applicable, else null.
     */
     ...
     *
     * @param mod_clatest_preflight_check_form $clatestform the form being built.
    ...
    /**
     * This is called when the current attempt at the clatest is finished. This is
    ...
    /**
     * Information, such as might be shown on the clatest view page, relating to this restriction.
     ...
     * this clatest, then return true. This is used so we can know whether to display a
    ...
    /**
     * Add any fields that this rule requires to the clatest settings form. This
     * method is called from {@link mod_clatest_mod_form::definition()}, while the
    ...
    /**
     * @return array key => lang string any choices to add to the clatest Browser
    ...
    /**
     * Save any submitted settings when the clatest settings form is submitted. This
     * is called from {@link clatest_after_add_or_update()} in lib.php.
    ...
    /**
     * Delete any rule-specific settings when the clatest is deleted. This is called
     * from {@link clatest_delete_instance()} in lib.php.
     ...
     * here is probalby to read the code of {@link clatest_access_manager::load_settings()}.
     *
     * If you have some settings that cannot be loaded in this way, then you can
     * use the {@link get_extra_settings()} method instead, but that has
     * performance implications.
     *
     * @param int $clatestid the id of the clatest we are loading settings for. This
     *     can also be accessed as clatest.id in the SQL. (clatest is a table alisas for {clatest}.)
     ...
     */
    public static function get_settings_sql($clatestid) {
        return array('', '', array());
    }
     ...
     */
    public static function get_extra_settings($clatestid) {
        return array();
    }
}
