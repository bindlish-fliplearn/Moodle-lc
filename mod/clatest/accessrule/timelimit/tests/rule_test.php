...
/**
 * Unit tests for the clatestaccess_timelimit plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/clatest/accessrule/timelimit/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class clatestaccess_timelimit_testcase extends basic_testcase {
    public function test_time_limit_access_rule() {
        $clatest = new stdClass();
        $clatest->timelimit = 3600;
        $cm = new stdClass();
        $cm->id = 0;
        $clatestobj = new clatest($clatest, $cm, null);
        $rule = new clatestaccess_timelimit($clatestobj, 10000);
        $attempt = new stdClass();

        $this->assertEquals($rule->description(),
            get_string('clatesttimelimit', 'clatestaccess_timelimit', format_time(3600)));
