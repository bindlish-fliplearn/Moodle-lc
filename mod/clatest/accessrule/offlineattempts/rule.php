...
/**
 * Implementaton of the clatestaccess_offlineattempts plugin.
 *
 ...
 * @since      Moodle 3.2
 */
class clatestaccess_offlineattempts extends clatest_access_rule_base {
    ...
    public static function make(clatest $clatestobj, $timenow, $canignoretimelimits) {
        ...
        if (empty($CFG->enablemobilewebservice) or empty($clatestobj->get_clatest()->allowofflineattempts)) {
            return null;
        }
    ...
    public function is_preflight_check_required($attemptid) {
        ...
        if (!empty($attemptid)) {
            $timemodifiedoffline = $DB->get_field('clatest_attempts', 'timemodifiedoffline', array('id' => $attemptid));
            if (empty($timemodifiedoffline)) {
                return false;
            }
            return empty($SESSION->offlineattemptscheckedclatestzes[$this->clatest->id]);
        } else {
    ...
    public function add_preflight_check_form_fields(mod_clatest_preflight_check_form $clatestform,
        ...
        $mform->addElement('header', 'offlineattemptsheader', get_string('mobileapp', 'clatestaccess_offlineattempts'));
        $mform->addElement('static', 'offlinedatamessage', '',
                get_string('offlinedatamessage', 'clatestaccess_offlineattempts', $lasttime));
        $mform->addElement('advcheckbox', 'confirmdatasaved', null,
                get_string('confirmdatasaved', 'clatestaccess_offlineattempts'));
    }
    ...
    public function validate_preflight_check($data, $files, $errors, $attemptid) {
        ...
        $errors['confirmdatasaved'] = get_string('pleaseconfirm', 'clatestaccess_offlineattempts');
        return $errors;
    }

    public function notify_preflight_check_passed($attemptid) {
        global $SESSION;
        $SESSION->offlineattemptscheckedclatestzes[$this->clatest->id] = true;
    }

    public function current_attempt_finished() {
        global $SESSION;
        // Clear the flag in the session that says that the user has already agreed to the notice.
        if (!empty($SESSION->offlineattemptscheckedclatestzes[$this->clatest->id])) {
            unset($SESSION->offlineattemptscheckedclatestzes[$this->clatest->id]);
        }
    }
    ...
    public static function add_settings_form_fields(
        ...
        if ($CFG->enablemobilewebservice) {
            $mform->addElement('selectyesno', 'allowofflineattempts',
                                get_string('allowofflineattempts', 'clatestaccess_offlineattempts'));
            $mform->addHelpButton('allowofflineattempts', 'allowofflineattempts', 'clatestaccess_offlineattempts');
    ...
    public static function validate_settings_form_fields(array $errors,
        ...
        if ($CFG->enablemobilewebservice) {
            ...
            // - The clatest uses a timer.
            // - The clatest is restricted by subnet.
            ...
            if (!empty($data['allowofflineattempts']) and
                ...
                $errors['allowofflineattempts'] = get_string('offlineattemptserror', 'clatestaccess_offlineattempts');
            }
        }
