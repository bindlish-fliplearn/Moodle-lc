...
// Get the clatest slot.
$slot = $DB->get_record('clatest_slots', array('id' => $slotid));
if (!$slot || empty($slot->questioncategoryid)) {
    print_error('invalidrandomslot', 'mod_clatest');
}

if (!$clatest = $DB->get_record('clatest', array('id' => $slot->clatestid))) {
    print_error('invalidclatestid', 'clatest');
}

$cm = get_coursemodule_from_instance('clatest', $slot->clatestid, $clatest->course);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/clatest/edit.php', array('cmid' => $cm->id));
}

$url = new moodle_url('/mod/clatest/editrandom.php', array('slotid' => $slotid));
$PAGE->set_url($url);
...
$mform = new mod_clatest\form\randomquestion_form(new moodle_url('/mod/clatest/editrandom.php'),
...
$currentslottags = clatest_retrieve_slot_tags($slot->id);
...
if ($mform->is_cancelled()) {
    ...
    // We need to save some data into the clatest_slots table.
    ...
    foreach ($tags as $tag) {
        if ($key = array_search(['tagid' => $tag->id, 'tagname' => $tag->name], $searchableslottags)) {
            // If found, $key would be the id field in the clatest_slot_tags table.
    ...
    if (!empty($recordstokeep)) {
        ...
        $DB->delete_records_select('clatest_slot_tags', "id $select", $params);
    } else {
...
echo $OUTPUT->header();
$heading = get_string('randomediting', 'mod_clatest');
echo $OUTPUT->heading_with_help($heading, 'randomquestion', 'mod_clatest');
