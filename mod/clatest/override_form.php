...
/**
 * Settings form for overrides in the clatest module.
 *
 * @package    mod_clatest
...
require_once($CFG->dirroot . '/mod/clatest/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class clatest_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the clatest settings object. */
    protected $clatest;
     ...
     * @param object the clatest settings object.
     ...
     */
    public function __construct($submiturl, $cm, $clatest, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'clatest'));
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'clatest'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/clatest/overrides.php', array('cmid'=>$cm->id));
                    print_error('groupsnone', 'clatest', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'clatest'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                $users = get_users_by_capability($this->context, 'mod/clatest:attempt',
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/clatest/overrides.php', array('cmid'=>$cm->id));
                    print_error('usersnone', 'clatest', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'clatest'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'clatest');
        $mform->setDefault('password', $this->clatest->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('clatestopen', 'clatest'), mod_clatest_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->clatest->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('clatestclose', 'clatest'), mod_clatest_mod_form::$datefieldoptions);
        $mform->setDefault('timeclose', $this->clatest->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'clatest'), array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'clatest');
        $mform->setDefault('timelimit', $this->clatest->timelimit);
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'clatest'), $attemptoptions);
        $mform->addHelpButton('attempts', 'attempts', 'clatest');
        $mform->setDefault('attempts', $this->clatest->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'clatest'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'clatest'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'clatest'));
    ...
    public function validation($data, $files) {
        ...
        // Ensure that at least one clatest setting was changed.
        $changed = false;
        $keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
        foreach ($keys as $key) {
            if ($data[$key] != $clatest->{$key}) {
                $changed = true;
                break;
            }
        }
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'clatest');
        }
