...
/**
 * Settings form for overrides in the homework module.
 *
...
require_once($CFG->dirroot . '/mod/homework/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class homework_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the homework settings object. */
    protected $homework;
     ...
     * @param object the homework settings object.
     ...
     */
    public function __construct($submiturl, $cm, $homework, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'homework'));
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'homework'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/homework/overrides.php', array('cmid'=>$cm->id));
                    print_error('groupsnone', 'homework', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'homework'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                $users = get_users_by_capability($this->context, 'mod/homework:attempt',
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/homework/overrides.php', array('cmid'=>$cm->id));
                    print_error('usersnone', 'homework', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'homework'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'homework');
        $mform->setDefault('password', $this->homework->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('homeworkopen', 'homework'), mod_homework_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->homework->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('homeworkclose', 'homework'), mod_homework_mod_form::$datefieldoptions);
        $mform->setDefault('timeclose', $this->homework->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'homework'), array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'homework');
        $mform->setDefault('timelimit', $this->homework->timelimit);
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'homework'), $attemptoptions);
        $mform->addHelpButton('attempts', 'attempts', 'homework');
        $mform->setDefault('attempts', $this->homework->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'homework'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'homework'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'homework'));
    ...
    public function validation($data, $files) {
        ...
        // Ensure that at least one homework setting was changed.
        $changed = false;
        $keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
        foreach ($keys as $key) {
            if ($data[$key] != $homework->{$key}) {
                $changed = true;
                break;
            }
        }
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'homework');
        }
