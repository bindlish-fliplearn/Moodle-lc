...
/**
 * Defines the Moodle forum used to add random questions to the homework.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class homework_add_random_form extends moodleform {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'existingcategoryheader',
                get_string('randomfromexistingcategory', 'homework'));
        ...
        $mform->addElement('checkbox', 'includesubcategories', '', get_string('recurse', 'homework'));
        ...
        $options = array(
            'multiple' => true,
            'noselectionstring' => get_string('anytags', 'homework'),
        );
        $mform->addElement('autocomplete', 'fromtags', get_string('randomquestiontags', 'mod_homework'), $tagstrings, $options);
        $mform->addHelpButton('fromtags', 'randomquestiontags', 'mod_homework');

        $mform->addElement('select', 'numbertoadd', get_string('randomnumber', 'homework'),
                $this->get_number_of_questions_to_add_choices());

        $previewhtml = $OUTPUT->render_from_template('mod_homework/random_question_form_preview', []);
        $mform->addElement('html', $previewhtml);

        $mform->addElement('submit', 'existingcategory', get_string('addrandomquestion', 'homework'));

        // Random from a new category section.
        $mform->addElement('header', 'newcategoryheader',
                get_string('randomquestionusinganewcategory', 'homework'));
        ...
        $mform->addElement('submit', 'newcategory',
                get_string('createcategoryandaddrandomquestion', 'homework'));
        ...
        $PAGE->requires->js_call_amd('mod_homework/add_random_form', 'init', [
