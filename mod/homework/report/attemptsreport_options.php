...
/**
 * Base class for the options that control what is visible in an {@link homework_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_homework_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the homework being reported on. */
    public $homework;

    /** @var object the course module objects for the homework being reported on. */
    public $cm;

    /** @var object the course settings for the course the homework is in. */
    public $course;

    /**
     * @var array form field name => corresponding homework_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => homework_attempt::IN_PROGRESS,
        'stateoverdue'    => homework_attempt::OVERDUE,
        'statefinished'   => homework_attempt::FINISHED,
        'stateabandoned'  => homework_attempt::ABANDONED,
    );

    /**
     * @var string homework_attempts_report::ALL_WITH or homework_attempts_report::ENROLLED_WITH
     *      homework_attempts_report::ENROLLED_WITHOUT or homework_attempts_report::ENROLLED_ALL
     */
    public $attempts = homework_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of homework_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(homework_attempt::IN_PROGRESS, homework_attempt::OVERDUE,
            homework_attempt::FINISHED, homework_attempt::ABANDONED);
    ...
    public $pagesize = homework_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $homework the settings for the homework being reported on.
     * @param object $cm the course module objects for the homework being reported on.
     * @param object $coures the course settings for the coures this homework is in.
     */
    public function __construct($mode, $homework, $cm, $course) {
        $this->mode   = $mode;
        $this->homework   = $homework;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = homework_report_should_show_grades($homework, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/homework/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('homework_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('homework_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == homework_attempts_report::ALL_WITH) {
                $this->attempts = homework_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = homework_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(homework_attempts_report::ALL_WITH, homework_attempts_report::ENROLLED_WITH,
                homework_attempts_report::ENROLLED_WITHOUT, homework_attempts_report::ENROLLED_ALL))) {
            $this->attempts = homework_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!homework_report_can_filter_only_graded($this->homework)) {
        ...
        if ($this->attempts == homework_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = homework_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(homework_attempt::FINISHED, $this->states);
    }
}
