 ...
 *
 * The homework stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-homework-tools/tree/master/statsspreadsheet
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class homework_report_statistics_from_steps_testcase extends mod_homework_attempt_walkthrough_from_csv_testcase {

    /**
     * @var homework_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    public function test_walkthrough_from_csv($homeworksettings, $csvdata) {
        ...
        list($questions, $homeworkstats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata,
                            $whichattempts, $whichtries, $groupstudentsjoins);
        if ($homeworksettings['testnumber'] === '00') {
            $this->check_variants_count_for_homework_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_homework_stats_for_homework_00($homeworkstats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_homework_00($questions, $questionstats, $whichtries, $qubaids) {
     ...
     * @param $homeworkstats
     */
    protected function check_homework_stats_for_homework_00($homeworkstats) {
        $homeworkstatsexpected = array(
            'median'             => 4.5,
        ...
        foreach ($homeworkstatsexpected as $statname => $statvalue) {
            $this->assertEquals($statvalue, $homeworkstats->$statname, $homeworkstats->$statname, abs($statvalue) * 1.5e-5);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $homeworkstats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries,
        ...
        $this->report = new homework_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->homework);
        list($homeworkstats, $questionstats) = $this->report->get_all_stats_and_analysis($this->homework,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudentsjoins,
                                                                                     $questions);

        $qubaids = homework_statistics_qubaids_condition($this->homework->id, $groupstudentsjoins, $whichattempts);

        // We will create some homework and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $homeworkcalc = new \homework_statistics\calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($homeworkcalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $homeworkstats, $questionstats, $qubaids);
        }
        return array($questions, $homeworkstats, $questionstats, $qubaids);
    }
