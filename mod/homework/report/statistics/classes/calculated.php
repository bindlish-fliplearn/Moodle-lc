...
class calculated {
     ...
     */
    public function get_formatted_homework_info_data($course, $cm, $homework) {
        ...
        $homeworkinfo = array();
        $homeworkinfo[get_string('homeworkname', 'homework_statistics')] = format_string($homework->name);
        $homeworkinfo[get_string('coursename', 'homework_statistics')] = format_string($course->fullname);
        if ($cm->idnumber) {
            $homeworkinfo[get_string('idnumbermod')] = $cm->idnumber;
        }
        if ($homework->timeopen) {
            $homeworkinfo[get_string('homeworkopen', 'homework')] = userdate($homework->timeopen);
        }
        if ($homework->timeclose) {
            $homeworkinfo[get_string('homeworkclose', 'homework')] = userdate($homework->timeclose);
        }
        if ($homework->timeopen && $homework->timeclose) {
            $homeworkinfo[get_string('duration', 'homework_statistics')] =
                format_time($homework->timeclose - $homework->timeopen);
        }
        ...
        foreach ($todisplay as $property => $format) {
            ...
            switch ($format) {
                case 'summarks_as_percentage':
                    $formattedvalue = homework_report_scale_summarks_as_percentage($value, $homework);
                    break;
                case 'number_format_percent':
                    $formattedvalue = homework_format_grade($homework, $value) . '%';
                    break;
                case 'number_format':
                    // 2 extra decimal places, since not a percentage,
                    // and we want the same number of sig figs.
                    $formattedvalue = format_float($value, $homework->decimalpoints + 2);
                    break;
                case 'number':
            ...
            $homeworkinfo[get_string($property, 'homework_statistics',
    ...
    public function cache($qubaids) {
        ...
        // Store the data.
        $DB->insert_record('homework_statistics', $toinsert);

    }

    /**
     * Given a record from 'homework_statistics' table load the data into the properties of this class.
     *
