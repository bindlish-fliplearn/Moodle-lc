...
namespace homework_statistics;
defined('MOODLE_INTERNAL') || die();

/**
 * Class to calculate and also manage caching of homework statistics.
 *
 * These homework statistics calculations are described here :
 *
...
class calculator {
    ...
    /**
     * Compute the homework statistics.
     *
     ...
     * @return calculated $homeworkstats The statistics for overall attempt scores.
     */
    public function calculate($homeworkid, $whichattempts, \core\dml\sql_join $groupstudentsjoins, $p, $sumofmarkvariance) {

        $this->progress->start_progress('', 3);

        $homeworkstats = new calculated($whichattempts);

        $countsandaverages = $this->attempt_counts_and_averages($homeworkid, $groupstudentsjoins);
        $this->progress->progress(1);

        foreach ($countsandaverages as $propertyname => $value) {
            $homeworkstats->{$propertyname} = $value;
        }
        ...
        if ($s != 0) {
            ...
            list($fromqa, $whereqa, $qaparams) =
                homework_statistics_attempts_sql($homeworkid, $groupstudentsjoins, $whichattempts);

            $homeworkstats->median = $this->median($s, $fromqa, $whereqa, $qaparams);
            $this->progress->progress(2);

            if ($s > 1) {

                $powers = $this->sum_of_powers_of_difference_to_mean($homeworkstats->avg(), $fromqa, $whereqa, $qaparams);
                $this->progress->progress(3);

                $homeworkstats->standarddeviation = sqrt($powers->power2 / ($s - 1));
                ...
                if ($s > 2) {
                    ...
                    $k3 = $s * $s * $m3 / (($s - 1) * ($s - 2));
                    if ($k2 != 0) {
                        $homeworkstats->skewness = $k3 / (pow($k2, 3 / 2));

                        // Kurtosis.
                        if ($s > 3) {
                            $k4 = $s * $s * ((($s + 1) * $m4) - (3 * ($s - 1) * $m2 * $m2)) / (($s - 1) * ($s - 2) * ($s - 3));
                            $homeworkstats->kurtosis = $k4 / ($k2 * $k2);
                        }

                        if ($p > 1) {
                            $homeworkstats->cic = (100 * $p / ($p - 1)) * (1 - ($sumofmarkvariance / $k2));
                            $homeworkstats->errorratio = 100 * sqrt(1 - ($homeworkstats->cic / 100));
                            $homeworkstats->standarderror = $homeworkstats->errorratio *
                                $homeworkstats->standarddeviation / 100;
                        }
                    }
            ...
            $homeworkstats->cache(homework_statistics_qubaids_condition($homeworkid, $groupstudentsjoins, $whichattempts));
        }
    ...
    public function get_cached($qubaids) {
        ...
        $fromdb = $DB->get_record_select('homework_statistics', 'hashcode = ? AND timemodified > ?',
    ...
    public function get_last_calculated_time($qubaids) {
        ...
        $timemodified = time() - self::TIME_TO_CACHE;
        return $DB->get_field_select('homework_statistics', 'timemodified', 'hashcode = ? AND timemodified > ?',
                                         array($qubaids->get_hash_code(), $timemodified));
    }

    /**
     * Given a particular homework grading method return a lang string describing which attempts contribute to grade.
     *
    ...
    public static function using_attempts_lang_string($whichattempts) {
         return get_string(static::using_attempts_string_id($whichattempts), 'homework_statistics');
    }

    /**
     * Given a particular homework grading method return a string id for use as a field name prefix in mdl_homework_statistics or to
     ...
     */
    protected function attempt_counts_and_averages($homeworkid, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;

        $attempttotals = new \stdClass();
        foreach (array_keys(homework_get_grading_options()) as $which) {

            list($fromqa, $whereqa, $qaparams) = homework_statistics_attempts_sql($homeworkid, $groupstudentsjoins, $which);
    ...
    protected function median($s, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT homeworka.id, homeworka.sumgrades
                  FROM $fromqa
                 WHERE $whereqa
              ORDER BY sumgrades";
    ...
    protected function sum_of_powers_of_difference_to_mean($mean, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT
                    SUM(POWER((homeworka.sumgrades - $mean), 2)) AS power2,
                    SUM(POWER((homeworka.sumgrades - $mean), 3)) AS power3,
                    SUM(POWER((homeworka.sumgrades - $mean), 4)) AS power4
                  FROM $fromqa
                 WHERE $whereqa";
