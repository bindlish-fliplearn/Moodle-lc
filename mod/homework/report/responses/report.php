...
/**
 * This file defines the homework responses report class.
 *
...
require_once($CFG->dirroot . '/mod/homework/report/responses/responses_options.php');
require_once($CFG->dirroot . '/mod/homework/report/responses/responses_form.php');
require_once($CFG->dirroot . '/mod/homework/report/responses/last_responses_table.php');
require_once($CFG->dirroot . '/mod/homework/report/responses/first_or_all_responses_table.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class homework_responses_report extends homework_attempts_report {

    public function display($homework, $cm, $course) {
        global $OUTPUT, $DB;

        list($currentgroup, $studentsjoins, $groupstudentsjoins, $allowedjoins) = $this->init(
                'responses', 'homework_responses_settings_form', $homework, $cm, $course);

        $options = new homework_responses_options('responses', $homework, $cm, $course);
        ...
        if ($options->whichtries === question_attempt::LAST_TRY) {
            $tableclassname = 'homework_last_responses_table';
        } else {
            $tableclassname = 'homework_first_or_all_responses_table';
        }
        $table = new $tableclassname($homework, $this->context, $this->qmsubselect,
                $options, $groupstudentsjoins, $studentsjoins, $questions, $options->get_url());
        $filename = homework_report_download_filename(get_string('responsesfilename', 'homework_responses'),
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
            ...
            $this->add_grade_columns($homework, $options->usercanseegrades, $columns, $headers);
            ...
            foreach ($questions as $id => $question) {
                ...
                if ($options->showresponses) {
                    $columns[] = 'response' . $id;
                    $headers[] = get_string('responsex', 'homework_responses', $question->number);
                }
                if ($options->showright) {
                    $columns[] = 'right' . $id;
                    $headers[] = get_string('rightanswerx', 'homework_responses', $question->number);
                }
            }
