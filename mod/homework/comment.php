...
$PAGE->set_url('/mod/homework/comment.php', array('attempt' => $attemptid, 'slot' => $slot));
...
if (!$attemptobj->is_finished()) {
    print_error('attemptclosed', 'homework');
}

// Check login and permissions.
require_login($attemptobj->get_course(), false, $attemptobj->get_cm());
$attemptobj->require_capability('mod/homework:grade');

// Print the page header.
$PAGE->set_pagelayout('popup');
$PAGE->set_title(get_string('manualgradequestion', 'homework', array(
        'question' => format_string($attemptobj->get_question_name($slot)),
        'homework' => format_string($attemptobj->get_homework_name()), 'user' => fullname($student))));
...
// Quiz name.
$summarydata['homeworkname'] = array(
    'title'   => get_string('modulename', 'homework'),
    'content' => format_string($attemptobj->get_homework_name()),
);

// Question name.
$summarydata['questionname'] = array(
    'title'   => get_string('question', 'homework'),
...
echo '<form method="post" class="mform" id="manualgradingform" action="' .
        $CFG->wwwroot . '/mod/homework/comment.php">';
...
<?php
echo '</form>';
$PAGE->requires->js_init_call('M.mod_homework.init_comment_popup', null, false, homework_get_js_module());
