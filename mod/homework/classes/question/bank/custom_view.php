...
/**
 * Defines the custom question bank view used on the Edit homework page.
 *
...
namespace mod_homework\question\bank;
defined('MOODLE_INTERNAL') || die();


/**
 * Subclass to customise the view of the question bank for the homework editing screen.
 *
 * @copyright  2009 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class custom_view extends \core_question\bank\view {
    /** @var bool whether the homework this is used by has been attemptd. */
    protected $homeworkhasattempts = false;
     ...
     * @param \stdClass $homework homework settings.
     */
    public function __construct($contexts, $pageurl, $course, $cm, $homework) {
    ...
    protected function wanted_columns() {
        ...
        if (empty($CFG->homeworkquestionbankcolumns)) {
            $homeworkquestionbankcolumns = array(
                'add_action_column',
                'checkbox_column',
            ...
            $homeworkquestionbankcolumns = explode(',', $CFG->homeworkquestionbankcolumns);
        }

        foreach ($homeworkquestionbankcolumns as $fullname) {
            if (!class_exists($fullname)) {
                if (class_exists('mod_homework\\question\\bank\\' . $fullname)) {
                    $fullname = 'mod_homework\\question\\bank\\' . $fullname;
                    ...
                    debugging('Legacy question bank column class question_bank_' .
                            $fullname . ' should be renamed to mod_homework\\question\\bank\\' .
    ...
    protected function heading_column() {
        return 'mod_homework\\question\\bank\\question_name_text_column';
    }

    protected function default_sort() {
        return array(
            'core_question\\bank\\question_type_column' => 1,
            'mod_homework\\question\\bank\\question_name_text_column' => 1,
        );
    }

    /**
     * Let the question bank display know whether the homework has been attempted,
     * hence whether some bits of UI, like the add this question to the homework icon,
     * should be displayed.
     * @param bool $homeworkhasattempts whether the homework has attempts.
     */
    public function set_homework_has_attempts($homeworkhasattempts) {
        $this->homeworkhasattempts = $homeworkhasattempts;
        if ($homeworkhasattempts && isset($this->visiblecolumns['addtohomeworkaction'])) {
            unset($this->visiblecolumns['addtohomeworkaction']);
        }
    }

    public function preview_question_url($question) {
        return homework_question_preview_url($this->homework, $question);
    }

    public function add_to_homework_url($questionid) {
        global $CFG;
        $params = $this->baseurl->params();
        $params['addquestion'] = $questionid;
        $params['sesskey'] = sesskey();
        return new \moodle_url('/mod/homework/edit.php', $params);
    }
    ...
    protected function display_bottom_controls($totalnumber, $recurse, $category, \context $catcontext, array $addcontexts) {
        ...
        $cmoptions->hasattempts = !empty($this->homeworkhasattempts);
        ...
        if ($canuseall) {
            ...
            // Add selected questions to the homework.
            $params = array(
                    'type' => 'submit',
                    'name' => 'add',
                    'class' => 'btn btn-primary',
                    'value' => get_string('addselectedquestionstohomework', 'homework'),
            );
    ...
    protected function display_options_form($showquestiontext, $scriptpath = '/mod/homework/edit.php',
    ...
    protected function print_category_info($category) {
        ...
        $strcategory = get_string('category', 'homework');
