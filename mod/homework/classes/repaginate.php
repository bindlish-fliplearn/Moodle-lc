...
/**
 * Defines the homework repaginate class.
 *
 ...
 *
 * The homework setting allows users to write homeworkzes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the homework being manipulated. */
    private $homeworkid;
    /** @var array the homework_slots for that homework. */
    private $slots;

    /**
     * Constructor.
     * @param int $homeworkid the id of the homework being manipulated.
     * @param stdClass[] $slots the homework_slots for that homework.
     */
    public function __construct($homeworkid = 0, $slots = null) {
        ...
        if (!$slots) {
            $this->slots = $DB->get_records('homework_slots', array('homeworkid' => $this->homeworkid), 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('homework_slots', $nextslot, true);
    ...
    /**
     * Repaginate the rest.
     * @param stdClass[] $homeworkslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($homeworkslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$homeworkslots) {
            return null;
        }
        $newslots = array();
        foreach ($homeworkslots as $slot) {
            if ($type == self::LINK) {
