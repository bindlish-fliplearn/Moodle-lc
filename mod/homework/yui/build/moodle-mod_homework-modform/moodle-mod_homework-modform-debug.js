YUI.add('moodle-mod_homework-modform', function (Y, NAME) {

/**
 * The modform class has all the JavaScript specific to mod/homework/mod_form.php.
 *
 * @module moodle-mod_homework-modform
 */
...
Y.extend(MODFORM, Y.Base, {
    ...
    initializer: function() {
        ...
        if (!this.repaginateCheckbox) {
            // The checkbox only appears when editing an existing homework.
            return;
        }
...
// Ensure that M.mod_homework exists and that coursebase is initialised correctly
M.mod_homework = M.mod_homework || {};
M.mod_homework.modform = M.mod_homework.modform || new MODFORM();
M.mod_homework.modform.init = function() {
    return new MODFORM();
};
