 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_homework_attempt_walkthrough_from_csv_testcase extends advanced_testcase {

    protected $files = array('questions', 'steps', 'results');

    /**
     * @var stdClass the homework record we create.
     */
    protected $homework;
     ...
     * directory.
     *
     * @param array $homeworksettings of settings read from csv file homeworkzes.csv
    ...
    public function test_walkthrough_from_csv($homeworksettings, $csvdata) {
        ...
        // https://github.com/jamiepratt/moodle-homework-tools/tree/master/responsegenerator

        $this->create_homework_simulate_attempts_and_check_results($homeworksettings, $csvdata);
    }

    public function create_homework($homeworksettings, $qs) {
        global $SITE, $DB;
        $this->setAdminUser();
        ...
        $aggregratedsettings = $homeworksettings + array('course' => $SITE->id,
                                                     'questionsperpage' => 0,
                                                     'grade' => 100.0,
                                                     'sumgrades' => $sumofgrades);

        $this->homework = $homeworkgenerator->create_instance($aggregratedsettings);

        $this->randqids = array();
        foreach ($slots as $slotno => $slotquestion) {
            if ($slotquestion['type'] !== 'random') {
                homework_add_homework_question($slotquestion['id'], $this->homework, 0, $slotquestion['mark']);
            } else {
                homework_add_random_questions($this->homework, 0, $slotquestion['catid'], 1, 0);
    ...
    /**
     * Create homework, simulate attempts and check results (if resultsXX.csv exists).
     *
     * @param array $homeworksettings Quiz overrides for this homework.
     * @param PHPUnit\DbUnit\DataSet\ITable[] $csvdata Data loaded from csv files for this test.
     */
    protected function create_homework_simulate_attempts_and_check_results($homeworksettings, $csvdata) {
    ...
    public function get_data_for_walkthrough() {
        $homeworkzes = $this->load_csv_data_file('homeworkzes');
        $datasets = array();
        for ($rowno = 0; $rowno < $homeworkzes->getRowCount(); $rowno++) {
            $homeworksettings = $homeworkzes->getRow($rowno);
            $dataset = array();
            foreach ($this->files as $file) {
                if (file_exists($this->get_full_path_of_csv_file($file, $homeworksettings['testnumber']))) {
                    $dataset[$file] = $this->load_csv_data_file($file, $homeworksettings['testnumber']);
                }
            }
            $datasets[] = array($homeworksettings, $dataset);
        }
        return $datasets;
    }
     ...
     * @return array attempt no as in csv file => the id of the homework_attempt as stored in the db.
     */
    protected function walkthrough_attempts($steps) {
        ...
        for ($rowno = 0; $rowno < $steps->getRowCount(); $rowno++) {
            ...
            // Find existing user or make a new user to do the homework.
            ...
            if (!isset($attemptids[$step['homeworkattempt']])) {
                // Start the attempt.
                $homeworkobj = homework::create($this->homework->id, $user->id);
                ...
                $prevattempts = homework_get_user_attempts($this->homework->id, $user->id, 'all', true);
                $attemptnumber = count($prevattempts) + 1;
                $timenow = time();
                $attempt = homework_create_attempt($homeworkobj, $attemptnumber, false, $timenow, false, $user->id);
                ...
                homework_start_new_attempt($homeworkobj, $quba, $attempt, $attemptnumber, $timenow, $step['randqs'], $step['variants']);
                homework_attempt_save_started($homeworkobj, $quba, $attempt);
                $attemptid = $attemptids[$step['homeworkattempt']] = $attempt->id;
            } else {
                $attemptid = $attemptids[$step['homeworkattempt']];
            }
     ...
     * @param $attemptids array attempt no as in csv file => the id of the homework_attempt as stored in the db.
     */
    protected function check_attempts_results($results, $attemptids) {
        for ($rowno = 0; $rowno < $results->getRowCount(); $rowno++) {
            ...
            // Re-load homework attempt data.
            $attemptobj = homework_attempt::create($attemptids[$result['homeworkattempt']]);
     ...
     * @param homework_attempt $attemptobj         the attempt object loaded from db.
        ...
        foreach ($result as $fieldname => $value) {
            ...
            switch ($fieldname) {
                            ...
                            switch ($slotfieldname) {
                                case 'mark' :
                                    $this->assertEquals(round($slotvalue, 2), $attemptobj->get_question_mark($slotno),
                                                        "Mark for slot $slotno of attempt {$result['homeworkattempt']}.");
                                    break;
                                default :
                ...
                case 'finished' :
                    ...
                    break;
                case 'summarks' :
                    $this->assertEquals($value, $attemptobj->get_sum_marks(), "Sum of marks of attempt {$result['homeworkattempt']}.");
                    break;
                case 'homeworkgrade' :
                    // Check homework grades.
                    $grades = homework_get_user_grades($attemptobj->get_homework(), $attemptobj->get_userid());
                    $grade = array_shift($grades);
                    $this->assertEquals($value, $grade->rawgrade, "Quiz grade for attempt {$result['homeworkattempt']}.");
                    break;
                case 'gradebookgrade' :
                    // Check grade book.
                    $gradebookgrades = grade_get_grades($attemptobj->get_courseid(),
                                                        'mod', 'homework',
                                                        $attemptobj->get_homeworkid(),
                                                        $attemptobj->get_userid());
                    $gradebookitem = array_shift($gradebookgrades->items);
                    $gradebookgrade = array_shift($gradebookitem->grades);
                    $this->assertEquals($value, $gradebookgrade->grade, "Gradebook grade for attempt {$result['homeworkattempt']}.");
                    break;
                default :
