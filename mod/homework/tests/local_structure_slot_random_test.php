...
/**
 * Unit tests for the {@link \mod_homework\local\structure\slot_random} class.
 *
...
/**
 * Class mod_homework_local_structure_slot_random_test
 * Class for tests related to the {@link \mod_homework\local\structure\slot_random} class.
 *
 * @copyright  2018 Shamim Rezaie <shamim@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_homework_local_structure_slot_random_test extends advanced_testcase {
    /**
     * Constructor test.
     */
    public function test_constructor() {
        ...
        // Create a random question without adding it to a homework.
        // We don't want to use homework_add_random_questions because that itself, instantiates an object from the slot_random class.
        $form = new stdClass();
        ...
        $rc = new ReflectionClass('\mod_homework\local\structure\slot_random');
        $rcp = $rc->getProperty('record');
        $rcp->setAccessible(true);
        $record = $rcp->getValue($randomslot);

        $this->assertEquals($homework->id, $record->homeworkid);
    ...
    public function test_get_homework_homework() {
        global $SITE, $DB;
        ...
        homework_add_random_questions($homework, 0, $category->id, 1, false);

        // Get the random question's id. It is at the first slot.
        $questionid = $DB->get_field('homework_slots', 'questionid', array('homeworkid' => $homework->id, 'slot' => 1));
        ...
        // The create_instance had injected an additional cmid propery to the homework. Let's remove that.
        unset($homework->cmid);

        $this->assertEquals($homework, $randomslot->get_homework());
    }

    public function test_set_homework() {
        global $SITE, $DB;
        ...
        $rc = new ReflectionClass('\mod_homework\local\structure\slot_random');
        $rcp = $rc->getProperty('homework');
        $rcp->setAccessible(true);
        $homeworkpropery = $rcp->getValue($randomslot);

        $this->assertEquals($homework, $homeworkpropery);
    }
    ...
    public function test_insert() {
        ...
        // Insert the random question to the homework.
        $randomslot = new \mod_homework\local\structure\slot_random($randomslotdata);
        $randomslot->set_tags([$footag, $bartag]);
        $randomslot->insert(1); // Put the question on the first page of the homework.

        // Get the random question's homework_slot. It is at the first slot.
        $homeworkslot = $DB->get_record('homework_slots', array('homeworkid' => $homework->id, 'slot' => 1));
        // Get the random question's tags from homework_slot_tags. It is at the first slot.
        $homeworkslottags = $DB->get_records('homework_slot_tags', array('slotid' => $homeworkslot->id));

        $this->assertEquals($question->id, $homeworkslot->questionid);
        $this->assertEquals($category->id, $homeworkslot->questioncategoryid);
        $this->assertEquals(1, $homeworkslot->includingsubcategories);
        $this->assertEquals(1, $homeworkslot->maxmark);

        $this->assertCount(2, $homeworkslottags);
        $this->assertEquals(
                [
