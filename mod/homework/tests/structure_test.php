 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_homework_structure_testcase extends advanced_testcase {

    /**
     * Create a course with an empty homework.
     * @return array with three elements homework, cm and course.
     */
    protected function prepare_homework_data() {
        ...
        return array($homework, $cm, $course);
    }

    /**
     * Creat a test homework.
     *
     ...
     * @return homework the created homework.
     */
    protected function create_test_homework($layout) {
        list($homework, $cm, $course) = $this->prepare_homework_data();
        ...
        foreach ($layout as $item) {
            if (is_string($item)) {
                ...
                homework_add_homework_question($q->id, $homework, $page);
                $lastpage = $page;
            }
        }

        $homeworkobj = new homework($homework, $cm, $course);
        $structure = \mod_homework\structure::create_for_homework($homeworkobj);
        if (isset($headings[1])) {
     ...
     * @param array $expectedlayout as for $layout in {@link create_test_homework()}.
     * @param \mod_homework\structure $structure the structure to test.
     */
    protected function assert_homework_layout($expectedlayout, \mod_homework\structure $structure) {
        $sections = $structure->get_sections();
        ...
        foreach ($expectedlayout as $item) {
            if (is_string($item)) {
                ...
                if ($slot > 1 && $section->heading == '' && $section->firstslot == 1) {
                    // The array $expectedlayout did not contain default first homework section, so skip over it.
                    $section = array_shift($sections);
                }
        ...
        if ($slot - 1 != count($structure->get_slots())) {
            $this->fail('The homework contains more slots than expected.');
        }

        if (!empty($sections)) {
            $section = array_shift($sections);
            if ($section->heading != '' || $section->firstslot != 1) {
                $this->fail('Unexpected section (' . $section->heading .') found in the homework.');
            }
        }
    }
     ...
     * used by create_test_homework as assert_homework_layout.
    ...
    public function test_get_homework_slots() {
        $homeworkobj = $this->create_test_homework(array(
                array('TF1', 1, 'truefalse'),
    ...
    public function test_homework_has_one_section_by_default() {
        $homeworkobj = $this->create_test_homework(array(
                array('TF1', 1, 'truefalse'),
            ));
        $structure = \mod_homework\structure::create_for_homework($homeworkobj);
    ...
    public function test_remove_section_heading() {
        ...
        $structure = \mod_homework\structure::create_for_homework($homeworkobj);
        $this->assert_homework_layout(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
                array('TF2', 2, 'truefalse'),
            ), $structure);
    ...
    public function test_homework_remove_slot() {
        $homeworkobj = $this->create_test_homework(array(
                array('TF1', 1, 'truefalse'),
                array('TF2', 1, 'truefalse'),
                'Heading 2',
    ...
    public function test_homework_removing_a_random_question_deletes_the_question() {
        global $DB;
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category();
        homework_add_random_questions($homeworkobj->get_homework(), 1, $cat->id, 1, false);
        $structure = \mod_homework\structure::create_for_homework($homeworkobj);
    ...
    public function test_can_remove_last_question_in_a_homework() {
        $homeworkobj = $this->create_test_homework(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
            ));
        ...
        homework_add_homework_question($q->id, $homeworkobj->get_homework(), 0);
        $structure = \mod_homework\structure::create_for_homework($homeworkobj);
    ...
    public function test_add_question_updates_headings() {
        ...
        homework_add_homework_question($q->id, $homeworkobj->get_homework(), 1);
    ...
    public function test_remove_page_break() {
        ...
        $slots = $structure->update_page_break($slotid, \mod_homework\repaginate::LINK);
    ...
    public function test_add_page_break() {
        ...
        $slotid = $structure->get_question_in_slot(2)->slotid;
        $slots = $structure->update_page_break($slotid, \mod_homework\repaginate::UNLINK);
     ...
     * @param  array $layout Quiz layout for create_test_homework function
    ...
    public function test_get_slot_tags_for_slot($layout, $tagnames, $slotnumber, $expected) {
        ...
        $homework = $this->create_test_homework($layout);
        $structure = \mod_homework\structure::create_for_homework($homework);
        ...
        $DB->insert_records('homework_slot_tags', $slottagrecords);
