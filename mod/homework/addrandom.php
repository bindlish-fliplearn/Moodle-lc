...
/**
 * Fallback page of /mod/homework/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $homework, $pagevars) =
        question_edit_setup('editq', '/mod/homework/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/homework/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new homework_add_random_form(new moodle_url('/mod/homework/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    homework_add_random_questions($homework, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories, $tagids);
...
// Setup $PAGE.
$streditinghomework = get_string('editinga', 'moodle', get_string('modulename', 'homework'));
$PAGE->navbar->add($streditinghomework);
$PAGE->set_title($streditinghomework);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$homeworkname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            print_error('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontohomework', 'homework', $homeworkname), 2);
$mform->display();
echo $OUTPUT->footer();
