...
// Get the homework slot.
$slot = $DB->get_record('homework_slots', array('id' => $slotid));
if (!$slot || empty($slot->questioncategoryid)) {
    print_error('invalidrandomslot', 'mod_homework');
}

if (!$homework = $DB->get_record('homework', array('id' => $slot->homeworkid))) {
    print_error('invalidhomeworkid', 'homework');
}

$cm = get_coursemodule_from_instance('homework', $slot->homeworkid, $homework->course);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/homework/edit.php', array('cmid' => $cm->id));
}

$url = new moodle_url('/mod/homework/editrandom.php', array('slotid' => $slotid));
$PAGE->set_url($url);
...
$mform = new mod_homework\form\randomquestion_form(new moodle_url('/mod/homework/editrandom.php'),
...
$currentslottags = homework_retrieve_slot_tags($slot->id);
...
if ($mform->is_cancelled()) {
    ...
    // We need to save some data into the homework_slots table.
    ...
    foreach ($tags as $tag) {
        if ($key = array_search(['tagid' => $tag->id, 'tagname' => $tag->name], $searchableslottags)) {
            // If found, $key would be the id field in the homework_slot_tags table.
    ...
    if (!empty($recordstokeep)) {
        ...
        $DB->delete_records_select('homework_slot_tags', "id $select", $params);
    } else {
...
echo $OUTPUT->header();
$heading = get_string('randomediting', 'mod_homework');
echo $OUTPUT->heading_with_help($heading, 'randomquestion', 'mod_homework');
