...
/**
 * Rest endpoint for ajax editing of homework structure.
 *
...
$PAGE->set_url('/mod/homework/edit-rest.php',
        array('homeworkid' => $homeworkid, 'class' => $class));
...
switch($requestmethod) {
    ...
    case 'GET': // For debugging.
        switch ($class) {
            case 'section':
                $table = 'homework_sections';
                $section = $structure->get_section_by_id($id);
                switch ($field) {
                    case 'getsectiontitle':
                        require_capability('mod/homework:manage', $modcontext);
            ...
            case 'resource':
                switch ($field) {
                    ...
                    case 'getmaxmark':
                        require_capability('mod/homework:manage', $modcontext);
                        $slot = $DB->get_record('homework_slots', array('id' => $id), '*', MUST_EXIST);
                        $result = array('instancemaxmark' => homework_format_question_grade($homework, $slot->maxmark));
                        break;
                    ...
                    case 'updatemaxmark':
                        ...
                        $result = array('instancemaxmark' => homework_format_question_grade($homework, $maxmark),
                                'newsummarks' => homework_format_grade($homework, $homework->sumgrades));
                        break;
                    ...
                    case 'deletemultiple':
                        ...
                        $ids = explode(',', $ids);
                        foreach ($ids as $id) {
                            $slot = $DB->get_record('homework_slots', array('homeworkid' => $homework->id, 'id' => $id),
                                    '*', MUST_EXIST);
                            if (homework_has_question_use($homework, $slot->slot)) {
                        ...
                        $result = array('newsummarks' => homework_format_grade($homework, $homework->sumgrades),
    ...
    case 'DELETE':
        switch ($class) {
            ...
            case 'resource':
                require_capability('mod/homework:manage', $modcontext);
                if (!$slot = $DB->get_record('homework_slots', array('homeworkid' => $homework->id, 'id' => $id))) {
