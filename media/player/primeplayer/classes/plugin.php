...
/**
 * Main class for 'media_primeplayer'.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class media_primeplayer_plugin extends core_media_player {
  ...
  public function embed($urls, $name, $width, $height, $options) {
    ...
    if (isset($userInfo->uuid) && !empty($userInfo->uuid)) {
      ...
      if (isset($result3->response)) {
        foreach ($result3->response as $key => $value) {
          if (isset($value->status) && $value->product == "prime") {
        ...
        foreach ($res_urls as $source) {
          ...
          if (!empty($chunks)) {
            ...
            if (!empty($query['resourceId'])) {
              ...
              $sql = "SELECT * FROM {guru_resourse_mapping} grm
                    join {guru_prime_mapping} gpm on gpm.subject_id = grm.subject_id
    ...
    if ($this->ismobileapp) {
      ...
      if (count($sources)) {
        ...
        } else if ($poster = get_config('media_primeplayer', 'defaultposter')) {
          $syscontext = context_system::instance();
          $poster = moodle_url::make_pluginfile_url($syscontext->id, 'media_primeplayer', 'defaultposter', 0, null, $poster)->out(true);
        }
        ...
        if ($tagtype === 'a') {
          ...
          } else if (!get_config('media_primeplayer', 'emptytitle')) {
            $attributes['title'] = $this->get_name($name, $urls);
          }

          // Set size.
          if (get_config('media_primeplayer', 'displaystyle') !== 'responsive') {
    ...
    // Embeding JWPlayer.
    return $this->embed_primeplayer($urls, $name, $width, $height, $playeroptions);
  }
  ...
  private function get_options_from_media_tag($originalhtml) {
    ...
    if (preg_match_all('~</?track\b[^>]*>~im', $originalhtml, $matches)) {
      ...
      foreach ($matches[0] as $trackhtml) {
        ...
        // https://developer.primeplayer.com/jw-player/docs/developer-guide/jw7/configuration-reference/#playlist-tracks
   ...
   *
   * This function is separating data-primeplayer-* attributes, format them and
  ...
  private function get_options_from_a_tag_attributes($attributes) {
    ...
    foreach ($attributes as $attrib => $atval) {
      // Process data-primeplayer-* attributes.
      if (strpos($attrib, 'data-primeplayer-') === 0) {
        // Treat attributes starting data-primeplayer as options.
        $opt = preg_replace('~^data-primeplayer-~', '', $attrib);
   ...
   * @return string HTML code for embed
   */
  private function embed_primeplayer($urls, $name, $width, $height, $options) {
    global $PAGE, $CFG;
    ...
    $playerid = 'media_primeplayer_media_' . html_writer::random_id();
    if (isset($options['mediaid']) && strlen(trim($options['mediaid']))) {
      $playlistitem['mediaid'] = $options['mediaid'];
      $playerid = 'media_primeplayer_media_' . preg_replace('/\s+/', '', $options['mediaid']);
    }
    ...
    if (isset($options['image']) && $options['image'] instanceof moodle_url) {
      ...
      $syscontext = context_system::instance();
      $playlistitem['image'] = moodle_url::make_pluginfile_url($syscontext->id, 'media_primeplayer', 'defaultposter', 0, null, $poster)->out(true);
    }
    ...
    $outerspanargs = array('class' => 'primeplayer_playerblock');
    if (!is_numeric($width)) {
    ...
    // Load skin.
    if ($customskincss = get_config('media_primeplayer', 'customskincss')) {
      $playersetupdata['skin'] = $customskincss;
    } else if ($skin = get_config('media_primeplayer', 'skin')) {
      $playersetupdata['skin'] = $skin;
    }

    // Set Google Analytics settings if enabled.
    if (get_config('media_primeplayer', 'googleanalytics')) {
      if (isset($options['gaidstring'])) {
        $gaidstring = $options['gaidstring'];
      } else {
        $gaidstring = get_config('media_primeplayer', 'gaidstring');
      }

      if (isset($options['galabel'])) {
        $galabel = $options['galabel'];
      } else {
        $galabel = get_config('media_primeplayer', 'galabel');
      }
    ...
    if (get_config('media_primeplayer', 'downloadbutton') && !count($streams)) {
      $playersetup->downloadbtn = array(
        'img' => $CFG->wwwroot . '/media/player/primeplayer/pix/download.png',
        'tttext' => get_string('videodownloadbtntttext', 'media_primeplayer'),
      );
    }
    ...
    if (isset($options['globalattributes']['class'])) {
      $options['globalattributes']['class'] .= ' primeplayer_media';
    } else {
      $options['globalattributes']['class'] = 'primeplayer_media';
    }

    // Set up the player.
    $PAGE->requires->js_call_amd('media_primeplayer/primeplayer', 'setupPlayer', array($playersetup));
  ...
  public function get_supported_extensions() {
    return explode(',', get_config('media_primeplayer', 'enabledextensions'));
  }
  ...
  public function get_supported_events() {
    return explode(',', get_config('media_primeplayer', 'enabledevents'));
  }
  ...
  public function get_embeddable_markers() {
    ...
    if (get_config('media_primeplayer', 'supportrtmp')) {
      $markers[] = 'rtmp://';
    }
    return $markers;
  }
  ...
  public function list_supported_urls(array $urls, array $options = array()) {
    ...
    foreach ($urls as $url) {
      ...
      if (!get_config('media_primeplayer', 'supportrtmp') && ($url->get_scheme() === 'rtmp')) {
        continue;
      }
  ...
  public function is_enabled() {
    global $CFG;
    $hostingmethod = get_config('media_primeplayer', 'hostingmethod');
    $licensekey = get_config('media_primeplayer', 'licensekey');
    if (($hostingmethod === 'cloud') && empty($licensekey)) {
      // Cloud mode, but no license key.
      return false;
    }
    $hostedprimeplayerpath = $CFG->dirroot . '/media/player/primeplayer/primeplayer/primeplayer.js';
    if (($hostingmethod === 'self') && (!is_readable($hostedprimeplayerpath) || empty($licensekey))) {
      // Self-hosted mode, but no primeplayer files and/or no license.
      return false;
    }
    return true;
  }
  ...
  public function setup($page) {
    ...
    if (!$hostingmethod = get_config('media_primeplayer', 'hostingmethod')) {
      $hostingmethod = 'cloud';
    }
    if ($hostingmethod === 'cloud') {
      // Well, this is not really a "cloud" version any more, we are just
      // using primeplayer libraries hosted on JW Player CDN.
      $primeplayer = new moodle_url('https://ssl.p.jwpcdn.com/player/v/' . MEDIA_JWPLAYER_CLOUD_VERSION . '/jwplayer');
    } else if ($hostingmethod === 'self') {
      // For self-hosted option, we are looking for player files presence in
      // /media/player/primeplayer/primeplayer/ directory.
      $primeplayer = new moodle_url($CFG->httpswwwroot . '/media/player/primeplayer/primeplayer/primeplayer');
    }
    // We need to define primeplayer, since primeplayer doesn't
    // define a module for require.js.
    $requirejs = 'require.config({ paths: {\'primeplayer\': \'' . $primeplayer->out() . '\'}})';
    $page->requires->js_amd_inline($requirejs);

    // Set player license key.
    $licensekey = get_config('media_primeplayer', 'licensekey');
    $licensejs = 'require.config({ config: {\'media_primeplayer/primeplayer\': { licensekey: \'' . $licensekey . '\'}}})';
  ...
  public function supports($usedextensions = []) {
    ...
    if (get_config('media_primeplayer', 'supportrtmp')) {
      $supports .= ($supports ? '<br>' : '') . get_string('supportrtmp', 'media_primeplayer');
    }
    return $supports;
  }
