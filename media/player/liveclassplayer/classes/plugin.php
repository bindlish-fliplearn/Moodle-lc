...
/**
 * Main class for 'media_liveclassplayer'.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class media_liveclassplayer_plugin extends core_media_player {
    ...
    public function embed($urls, $name, $width, $height, $options) {
        ...
        if ($this->ismobileapp) {
            ...
            if (count($sources)) {
                ...
                } else if ($poster = get_config('media_liveclassplayer', 'defaultposter')) {
                    $syscontext = context_system::instance();
                    $poster = moodle_url::make_pluginfile_url($syscontext->id, 'media_liveclassplayer', 'defaultposter', 0, null, $poster)->out(true);
                }
                ...
                if ($tagtype === 'a') {
                    ...
                    } else if (!get_config('media_liveclassplayer', 'emptytitle')) {
                        $attributes['title'] = $this->get_name($name, $urls);
                    }

                    // Set size.
                    if (get_config('media_liveclassplayer', 'displaystyle') !== 'responsive') {
        ...
        // Embeding JWPlayer.
        return $this->embed_liveclassplayer($urls, $name, $width, $height, $playeroptions);
    }
    ...
    private function get_options_from_media_tag($originalhtml) {
        ...
        if (preg_match_all('~</?track\b[^>]*>~im', $originalhtml, $matches)) {
            ...
            foreach ($matches[0] as $trackhtml) {
                ...
                // https://developer.liveclassplayer.com/jw-player/docs/developer-guide/jw7/configuration-reference/#playlist-tracks
     ...
     *
     * This function is separating data-liveclassplayer-* attributes, format them and
    ...
    private function get_options_from_a_tag_attributes($attributes) {
        ...
        foreach ($attributes as $attrib => $atval) {
            // Process data-liveclassplayer-* attributes.
            if (strpos($attrib, 'data-liveclassplayer-') === 0) {
                // Treat attributes starting data-liveclassplayer as options.
                $opt = preg_replace('~^data-liveclassplayer-~', '', $attrib);
     ...
     * @return string HTML code for embed
     */
    private function embed_liveclassplayer($urls, $name, $width, $height, $options) {
        global $PAGE, $CFG;
        ...
        $playerid = 'media_liveclassplayer_media_' . html_writer::random_id();
        if (isset($options['mediaid']) && strlen(trim($options['mediaid']))) {
            $playlistitem['mediaid'] = $options['mediaid'];
            $playerid = 'media_liveclassplayer_media_' . preg_replace('/\s+/', '', $options['mediaid']);
        }
        ...
        if (isset($options['image']) && $options['image'] instanceof moodle_url) {
            ...
            $syscontext = context_system::instance();
            $playlistitem['image'] = moodle_url::make_pluginfile_url($syscontext->id, 'media_liveclassplayer', 'defaultposter', 0, null, $poster)->out(true);
        }
        ...
        $outerspanargs = array('class' => 'liveclassplayer_playerblock');
        if (!is_numeric($width)) {
        ...
        // Load skin.
        if ($customskincss = get_config('media_liveclassplayer', 'customskincss')) {
            $playersetupdata['skin'] = $customskincss;
        } else if ($skin = get_config('media_liveclassplayer', 'skin')) {
            $playersetupdata['skin'] = $skin;
        }

        // Set Google Analytics settings if enabled.
        if (get_config('media_liveclassplayer', 'googleanalytics')) {
            if (isset($options['gaidstring'])) {
                $gaidstring = $options['gaidstring'];
            } else {
                $gaidstring = get_config('media_liveclassplayer', 'gaidstring');
            }

            if (isset($options['galabel'])) {
                $galabel = $options['galabel'];
            } else {
                $galabel = get_config('media_liveclassplayer', 'galabel');
            }
        ...
        if (get_config('media_liveclassplayer', 'downloadbutton') && !count($streams)) {
            $playersetup->downloadbtn = array(
                'img' => $CFG->wwwroot.'/media/player/liveclassplayer/pix/download.png',
                'tttext' => get_string('videodownloadbtntttext', 'media_liveclassplayer'),
            );
        }
        ...
        if (isset($options['globalattributes']['class'])) {
            $options['globalattributes']['class'] .= ' liveclassplayer_media';
        } else {
            $options['globalattributes']['class'] = 'liveclassplayer_media';
        }
    ...
    public function get_supported_extensions() {
        return explode(',', get_config('media_liveclassplayer', 'enabledextensions'));
    }
    ...
    public function get_supported_events() {
        return explode(',', get_config('media_liveclassplayer', 'enabledevents'));
    }
    ...
    public function get_embeddable_markers() {
        ...
        if (get_config('media_liveclassplayer', 'supportrtmp')) {
            $markers[] = 'rtmp://';
        }
        return $markers;
    }
    ...
    public function list_supported_urls(array $urls, array $options = array()) {
        ...
        foreach ($urls as $url) {
            ...
            if (!get_config('media_liveclassplayer', 'supportrtmp') && ($url->get_scheme() === 'rtmp')) {
                continue;
            }
    ...
    public function is_enabled() {
        global $CFG;
        $hostingmethod = get_config('media_liveclassplayer', 'hostingmethod');
        $licensekey = get_config('media_liveclassplayer', 'licensekey');
        if (($hostingmethod === 'cloud') && empty($licensekey)) {
            // Cloud mode, but no license key.
            return false;
        }
        $hostedliveclassplayerpath = $CFG->dirroot . '/media/player/liveclassplayer/liveclassplayer/liveclassplayer.js';
        if (($hostingmethod === 'self') && (!is_readable($hostedliveclassplayerpath) || empty($licensekey))) {
            // Self-hosted mode, but no liveclassplayer files and/or no license.
            return false;
        }
        return true;
    }
    ...
    */
    public function setup($page) {
        ...
        if (!$hostingmethod = get_config('media_liveclassplayer', 'hostingmethod')) {
            $hostingmethod = 'cloud';
        }
        if ($hostingmethod === 'cloud') {
            // Well, this is not really a "cloud" version any more, we are just
            // using liveclassplayer libraries hosted on JW Player CDN.
            $liveclassplayer = new moodle_url('https://ssl.p.jwpcdn.com/player/v/' . MEDIA_JWPLAYER_CLOUD_VERSION . '/jwplayer');
        } else if ($hostingmethod === 'self') {
            // For self-hosted option, we are looking for player files presence in
            // /media/player/liveclassplayer/liveclassplayer/ directory.
            $liveclassplayer = new moodle_url($CFG->httpswwwroot.'/media/player/liveclassplayer/liveclassplayer/liveclassplayer');
        }
        // We need to define liveclassplayer, since liveclassplayer doesn't
        // define a module for require.js.
        $requirejs = 'require.config({ paths: {\'liveclassplayer\': \'' . $liveclassplayer->out() . '\'}})';
        $page->requires->js_amd_inline($requirejs);

        // Set player license key.
        $licensekey = get_config('media_liveclassplayer', 'licensekey');
        $licensejs = 'require.config({ config: {\'media_liveclassplayer/liveclassplayer\': { licensekey: \'' . $licensekey . '\'}}})';
    ...
    public function supports($usedextensions = []) {
        ...
        if (get_config('media_liveclassplayer', 'supportrtmp')) {
            $supports .= ($supports ? '<br>' : '') . get_string('supportrtmp', 'media_liveclassplayer');
        }
        return $supports;
    }
