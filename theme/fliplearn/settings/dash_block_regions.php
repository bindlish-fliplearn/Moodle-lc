...
if ($usedashboard) {
    ...
    $temp = new admin_settingpage('theme_fliplearn_dash_block_regions',
        get_string('dashboardblockregionsettings', 'theme_fliplearn'));

    $temp->add(new admin_setting_heading('theme_fliplearn_heading', get_string('dashblocklayoutbuilder', 'theme_fliplearn'),
        format_text(get_string('dashblocklayoutbuilderdesc', 'theme_fliplearn'), FORMAT_MARKDOWN)));

    $name = 'theme_fliplearn/dashblocksenabled';
    $title = get_string('dashblocksenabled', 'theme_fliplearn');
    $description = get_string('dashblocksenableddesc', 'theme_fliplearn');
    ...
    $imgblder = '';
    for ($i = 1; $i <= 8; $i++) {
        $name = 'theme_fliplearn/dashblocklayoutlayoutrow' . $i;
        $title = get_string('dashblocklayoutlayoutrow', 'theme_fliplearn');
        $description = get_string('dashblocklayoutlayoutrowdesc', 'theme_fliplearn');
    ...
    $temp->add(new admin_setting_heading('theme_fliplearn_dashlayoutblockscount', '', $mktcountmsg));

    $temp->add(new admin_setting_heading('theme_fliplearn_dashlayoutbuilder', '', $imgblder));
