 ...
 */
function theme_fliplearn_process_css($css, $theme) {
    ...
    $css = theme_fliplearn_set_customcss($css, $customcss);
    ...
    if (empty($theme->settings->tilesshowallcontacts) || $theme->settings->tilesshowallcontacts == false) {
        $css = theme_fliplearn_set_tilesshowallcontacts($css, false);
    } else {
        $css = theme_fliplearn_set_tilesshowallcontacts($css, true);
    }
    return $css;
}
 ...
 */
function theme_fliplearn_set_customcss($css, $customcss) {
 ...
 * @return $string
 */
function theme_fliplearn_set_tilesshowallcontacts($css, $display) {
 ...
 */
function theme_fliplearn_get_zoom() {
    return get_user_preferences('theme_fliplearn_zoom', '');
}
 ...
 * @return void
 */
function theme_fliplearn_initialise_zoom(moodle_page $page) {
    user_preference_allow_ajax_update('theme_fliplearn_zoom', PARAM_TEXT);
}
 ...
 * @return void
 */
function theme_fliplearn_initialise_full(moodle_page $page) {
    user_preference_allow_ajax_update('theme_fliplearn_full', PARAM_TEXT);
}

/**
 * Get the user preference for the zoom function.
 */
function theme_fliplearn_get_full() {
    return get_user_preferences('theme_fliplearn_full', '');
}
 ...
 */
function theme_fliplearn_get_alertkey($alertindex) {
    user_preference_allow_ajax_update('theme_fliplearn_alertkey' . $alertindex, PARAM_TEXT);
    return get_user_preferences('theme_fliplearn_alertkey' . $alertindex, '');
}
 ...
 * @param moodle_page $page
 */
function theme_fliplearn_get_html_for_settings(renderer_base $output, moodle_page $page) {
    global $CFG;
    $return = new stdClass;
 ...
 */
function theme_fliplearn_get_setting($setting, $format = false) {
    static $theme;
    if (empty($theme)) {
        $theme = theme_config::load('fliplearn');
    }
 ...
 * @return bool
 */
function theme_fliplearn_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    static $theme;
    if (empty($theme)) {
        $theme = theme_config::load('fliplearn');
    }
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        ...
        } else if ($filearea === 'fliplearnmarkettingimages') {
            return $theme->setting_file_serve('fliplearnmarkettingimages', $args, $forcedownload, $options);
        } else {
            send_file_not_found();
        }
    } else {
 ...
 */
function theme_fliplearn_get_course_activities() {
 ...
 * @param string $param
 */
function theme_fliplearn_performance_output($param) {
    $html = html_writer::tag('span', get_string('loadtime', 'theme_fliplearn').' '. round($param['realtime'], 2) . ' ' .
 ...
 * @param moodle_page $page
 */
function theme_fliplearn_page_init(moodle_page $page) {
    global $CFG;
    $page->requires->jquery();
    // REMOVED: Deprecated function: error_log($CFG->version).
    if ($CFG->version < 2015051100) {
        $page->requires->jquery_plugin('bootstrap', 'theme_fliplearn');
        $page->requires->jquery_plugin('dropdown', 'theme_fliplearn');
    }

    $page->requires->jquery_plugin('pace', 'theme_fliplearn');
    $page->requires->jquery_plugin('flexslider', 'theme_fliplearn');
    $page->requires->jquery_plugin('ticker', 'theme_fliplearn');
    $page->requires->jquery_plugin('easing', 'theme_fliplearn');
    $page->requires->jquery_plugin('fliplearn', 'theme_fliplearn');
 ...
 */
function theme_fliplearn_remove_site_fullname($heading) {
    global $SITE, $PAGE;
 ...
 */
function theme_fliplearn_grid($left, $hassitepost) {
    if ($hassitepost) {
        if ($left) {
