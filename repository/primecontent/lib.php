...
class repository_primecontent extends repository {
    ...
    public function get_maxwidth() {
        $param = optional_param('primecontent_keyword', 0, PARAM_INT);
        $pref = get_user_preferences('repository_primecontent_maxwidth', WIKIMEDIA_IMAGE_SIDE_LENGTH);
        if ($param > 0 && $param != $pref) {
            $pref = $param;
            set_user_preference('repository_primecontent_maxwidth', $pref);
        }
        return $pref;
    }
    ...
    public function get_maxheight() {
        $param = optional_param('primecontent_maxheight', 0, PARAM_INT);
        $pref = get_user_preferences('repository_primecontent_maxheight', WIKIMEDIA_IMAGE_SIDE_LENGTH);
        if ($param > 0 && $param != $pref) {
            $pref = $param;
            set_user_preference('repository_primecontent_maxheight', $pref);
        }
        return $pref;
    }
