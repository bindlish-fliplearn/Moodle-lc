  ...
  $ptmsql = "SELECT gup.*, u.firstname, u.lastname 
              ...
              AND gup.user_id = $id ORDER BY ptm_date DESC";
  ...
  if (!empty($ptmRecord)) {
    foreach ($ptmRecord as $remark) {
      ...
      $teacher_remark = str_replace("'","~",$remark->teacher_remark);
      $parent_feedback = str_replace("'","~",$remark->parent_feedback);
   
        $row[] = $remark->firstname;
      if(!user_has_role_assignment($USER->id,5) && $USER->id == $remark->teacher_id ) {
      $row[] = "<a href='#0' onclick='showPtmPopup(\"$remark->id\", \"$remark->user_id\", \"$USER->id\", \"$remark->ptm_date\", \"$teacher_remark\", \"$parent_feedback\")'>Edit</a>"; 
