...
namespace local_flipapi\event\strategies;
...
class raw_event_retrieval_strategy implements raw_event_retrieval_strategy_interface {
    ...
    protected function get_raw_events_legacy_implementation(
        ...
        $sql = "SELECT e.*,  cmc.completionstate, cmc.timemodified , cm.completionexpected
                  FROM {event} e
            INNER JOIN ($subquery) fe
             ...
             LEFT JOIN {modules} m
                ...
                ON cm.instance = e.instance
                JOIN {course_modules_completion} as cmc 
                ON cmc.coursemoduleid = cm.id
                WHERE (m.visible = 1 OR m.visible IS NULL) AND 
                $whereclause
